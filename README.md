...
| HSF (HSFV037) | Model | 🟡 | ⛔ | HAL Laboratory tool-export model (Mario Party 4-8 `.hsf`, extracted from MPBIN by `wmpbdump`; same GX object model as Kirby Air Ride/doldecomp's `sysdolphin`); decodes geometry (positions, normals incl. packed-s8 variant, UVs, triangle/quad/indexed-tristrip primitives) to COLLADA DAE, one mesh per named part — single-part and real multi-part retail character models (e.g. Mario Party 4 Luigi, 13 parts) both work; materials, textures and bone/skin-weight rigging not decoded (parts export unbound, in bind pose) |
