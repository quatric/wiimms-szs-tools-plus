...
| BCWAV | Audio track | ✅ | ✅ | 3DS Sound Wave; DSP-ADPCM, IMA-ADPCM, PCM16, PCM8 decoding to WAV, plus WAV to little-endian CWAV (ADPCM/PCM16) via `wrwav` |
...
| BFWAV | Audio track | ✅ | ✅ | Wii U / Switch Sound Wave; DSP-ADPCM, IMA-ADPCM, PCM16, PCM8 decoding to WAV, plus WAV to big-endian FWAV (ADPCM/PCM16) via `wrwav` -- layout ported from real retail BFSAR wave archives (Nintendo Land) |
...
| HSD (sysdolphin `.dat`) | Model/Texture | 🟡 | ⛔ | HAL Laboratory's own "sysdolphin" engine (Super Smash Bros. Melee, Kirby Air Ride, Wii's "TV no Tomo" channel) -- a serialized-object-graph container, not directory-style, identified structurally (no magic). Texture export (`HSD_Image`/`HSD_Tlut`) already covered every real sample tried. Model export is new: walks the real JOBJ/DOBJ/POBJ tree and decodes each POBJ's GX display list (including the vertex-attribute array's own per-attribute DIRECT/INDEX8/INDEX16 addressing, not a fixed layout) to GLB, field layout and opcode semantics ported from Ploaj/HSDLib and confirmed byte-for-byte against real files first. Validated on Melee's real item/object corpus: 346 of 352 retail `Ty*.dat` files decode to real, glTF-validated geometry (9,564 meshes; the 6 that don't are genuinely not models -- data tables, lighting, 2D menu backgrounds). Scope, deliberately: static geometry bound to each POBJ's owning joint only (ENVELOPE/SHAPEANIM-weighted meshes and MOBJ material/texture binding are real further work); playable-character files aren't reached yet (their root table entry is per-fighter data, not a JOBJ directly -- rejected cleanly rather than guessed at, see lib-hsd.h). No encoder. |
