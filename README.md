...
| **HGO** | `.hgo` | — | — | — | — | — | Unidentified. The `0OGH`/`0MXT`/`0TST`/`LBTN` magics this once matched occur in none of *Mario Golf: Toadstool Tour* (GC), *Mario Power Tennis* (GC & Wii), *We Love Golf!* (Wii) or *Mario Golf: World Tour* (3DS), in any byte order, neither as a file magic nor embedded anywhere; Camelot ships models as PPC relocatable modules (`elfbin/xcmdl_*.sbn`) instead. Since such a magic can only mislabel unrelated data, content detection was removed and the extension alone names it |
