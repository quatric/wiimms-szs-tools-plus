...
| GTX / GSH | Texture / shader | 🟡 | 🟡 | Wii U GX2 "Gfx2" container; lossless bounds-checked block parsing for GTX and shader-only GSH, with vertex/pixel/geometry/compute header↔program associations and unknown blocks retained; all textures, GX2 views/slices, component selectors, and packed mip metadata are parsed. RGBA8/R10G10B10A2/R8/R4G4/R8G8/565/5551/4444 + BC1-5 decode supports tile modes 0-11 (including bank and pipe swizzles); selected mip decoding is available through the library API. RGBA8 encode uses genuine 2D macro tiling and an explicit EOF block, with a pixel-identical round-trip test. Semantic shader disassembly/recompilation, arrays/3D/MSAA/depth, and broader encode formats remain future work. |
