...
| BG4 | Archive | 🟡 | ⛔ | Mario & Luigi: Paper Jam / Paper Mario MIX (3DS) flat archive (`"BG4\0"`, little-endian): a 16-byte header, a 14-byte-per-file table of {offset, size, CRC, u16 name offset}, then a name blob. A member whose table offset has bit 31 set is BLZ ("backward LZSS") compressed and is decompressed through this fork's existing BLZ decoder; offset 0 marks an unused slot. `wszst EXTRACT`/`xx` triggers on the magic and recurses into the extracted members. Layout ported from aluigi's `mario_luigi_paper.bms`. Marked 🟡 rather than ✅ because no retail Paper Jam cartridge dump was reachable while porting: coverage is a from-spec decoder plus a synthetic fixture (`tests/mk-bms-fixtures.py`) whose BLZ path in particular is untested against real data. Creation is unavailable -- the per-entry CRC's algorithm has not been recovered |
...
| CA01 / SA01 | Archive | 🟡 | ⛔ | Mii Maker (Wii U, `"SA01"`) and amiibo Settings (3DS, `"CA01"`) archives. Both wrap a zlib payload -- Mii Maker behind a bare big-endian u32 uncompressed size, amiibo behind a `"ZCMP"` header with the stream at 0x80 -- around a flat inner archive of three parallel arrays: member offsets (relative to a base offset), member sizes, and, for SA01 only, one 0x80-byte fixed-width name per file (CA01 is nameless and its members get ordinal `.sar` names, matching what the script's own note says they are). Inner word order is recovered from the file count rather than assumed, mirroring the scripts' `endian guess`. Every accepted wrapper must decode to an `SA01`/`CA01` image, so the bare size+zlib form cannot fire on an unrelated zlib file. Layouts ported from aluigi's `mii_maker.bms` and `amiibo.bms`. 🟡 because no retail Mii Maker or amiibo Settings title was reachable while porting -- from-spec decoder plus synthetic fixtures (`tests/mk-bms-fixtures.py`), not retail-verified. Creation is unavailable: the archives are system-title data whose surrounding container fields are not understood |
| cram (`.arc`) | Archive | 🟡 | ⛔ | Xenoblade Chronicles 3D (3DS) `"cram"` archive: flat, named, uncompressed -- a 16-byte header, a 16-byte-per-file table of {name CRC, 4-char type, offset, size}, then one u32 name offset per file into a trailing string blob. `wszst EXTRACT`/`xx` triggers on the magic and recurses into the members. Layout ported from aluigi's `xenoblade_arc.bms`. 🟡 because no retail Xenoblade Chronicles 3D dump was reachable while porting -- from-spec decoder plus a synthetic fixture (`tests/mk-bms-fixtures.py`), not retail-verified. Encode is ⛔ deliberately: the per-entry name checksum's algorithm could not be confirmed (CRC32 is a plausible but unverified guess), and writing files with fabricated checksums would be worse than not writing them |
...
| FSYS | Archive / Compression | ✅ | ⛔ | Genius Sonority archive used by Pokémon Colosseum, XD and Battle Revolution; `wszst XX` extracts big-endian v1/v2 member tables, decodes bounded LZSS members, assigns stable ordinal names when retail entries are `(null)`, and recursively dispatches extracted children |
...
| MVDK | Compression | ✅ | ⛔ | Mario vs. Donkey Kong custom Deflate/Huffman/LZSS variant (ported from Garhoogin's NitroPaint reverse-engineering); auto-detected and extracted transparently via `wszst EXTRACT`/`xx` |
...
| NUTEXB | Texture | 🟡 | ❌ | Switch texture wrapper used by Super Smash Bros. Ultimate and other titles; a distinct container from BNTX despite both being Tegra/NX formats. Struct layout verified field-by-field against Switch-Toolbox's NUTEXB.cs (KillzXGaming/Switch-Toolbox); decode reuses lib-bntx.c's already-verified Tegra deswizzle and BC1-BC7 block decoders via a synthetic single-texture BNTX wrapper rather than reimplementing that math, covering RGBA8/BGRA8/BC1-BC7 (R32G32B32A32_FLOAT is rejected with an explicit "unsupported format" error, since lib-bntx.c has no decoder for it either). No retail NUTEXB sample was available to verify against, only a hand-built synthetic RGBA8 file exercising the footer parsing and decode path end-to-end; the block-height-log2 heuristic (NUTEXB doesn't store it explicitly) reuses this codebase's own BNTX-encoder derivation but is unverified against a real BC-compressed texture with non-power-of-two height. Encode is not implemented |
...
| SMDH | Icon/metadata | 🟡 | ⛔ | 3DS application icon + title metadata block (icon.bin, and embedded in every CIA/3DSX/CCI title). Struct layout verified field-by-field against Gericom's EveryFileExplorer (3DS/SMDH.cs) -- 3dbrew.org itself was unreachable while this was written. `wimgt DECODE` exports the large (48x48) icon as PNG via the standard 8x8 Morton-tiled RGB565 swizzle shared with this codebase's other 3DS GPU textures; the small (24x24) icon and all 16 per-language titles/publishers are available via the library API (`ScanSMDH`/`TextSMDH`) but not yet wired into a CLI dump command. No retail SMDH sample was available, only a hand-built synthetic file exercising header parsing, UTF-16LE title decoding, and icon decode end-to-end -- not yet retail-verified. Encode (regenerating regional title metadata) is not implemented |
