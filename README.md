...
| **DARC / BCMA** | `.darc`, `.bcma`, `.arc` | ✅ | ✅ | ✅ | ✅ | NintendoWare NW4C directory archive & 3DS electronic manual archive (3DS). Verified against retail *Nintendo 3DS Camera* and manual (`CTR-P-CTAP`). Fixtures: `tests/fixtures/3ds_samples/darc/retail_retro.arc` (1,192 B), `tests/fixtures/3ds_samples/bcma/retail_bcmainfo.arc` (2,372 B) |
...
| **RPAK** | `.rpak`, `.pak` | ✅ | ✅ | — | ✅ | Retro Studios asset container (*Donkey Kong Country Returns*, Wii). Decoder verified against a retail level pak (3672 entries); the original GameCube *Metroid Prime* (2002) uses an older, unrelated PAK layout without this format's STRG/RSHD header, so it doesn't apply here despite the similar extension. `CreateRPAK()` builds a fresh archive (header, empty STRG, RSHD table, per-entry CMPD/zlib wrapping with a raw-stored fallback when zlib doesn't shrink the data) from a caller-supplied entry list; round-tripped synthetically through `ScanRPAK()`/`DecompressRPAKEntry()` byte-for-byte, but not against a real retail file, since a retail archive's version/hash/STRG header fields aren't reverse-engineered and the encoder writes them as zero |
...
| **BCFNT / BFFNT / BRFNT** | `.bcfnt`, `.bffnt`, `.brfnt` | ✅ | ✅ | ✅ | ✅ | NintendoWare font resource (3DS / Wii U / Wii). Verified against retail *Nintendo 3DS Camera* (`HudNOTES.bcfnt`, 66,268 B, TGLP 24x24 4 sheets IA4). Fixture: `tests/fixtures/3ds_samples/bcfnt/retail_hudnotes.bcfnt` |
...
| **BCSAR / BCWAR / BCWAV** | `.bcsar`, `.bcwar`, `.bcwav` | ✅ | ✅ | ✅ | NintendoWare NW4C sound archive & wave format (3DS). Verified: `GreenCube.bcsar` (Luigi's Mansion 3DS, 16 MB), `QueenSound.bcsar` (OoT3D, 7 MB), and *Nintendo 3DS Camera* (`PNOTE_Sound.bcsar`, 1.8 MB; standalone `b_str_dog1_32k.imaadpcm.bcwav`, 11,050 B IMA-ADPCM 44.1kHz). BCSTM verified against *Yo-Kai Watch* retail RomFS: `ev01_0020_01.dspadpcm.bcstm` (5,728 bytes) decodes to WAV via the bfstm demuxer (adpcm_thp_le). Fixture: `tests/fixtures/3ds_samples/bcwav/retail_dog_adpcm.bcwav` |
