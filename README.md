...
| GSH | Shader | ✅ | ✅ | Shader-only Wii U Gfx2 container; lossless bounds-checked blocks, relocation/string-table metadata, vertex/pixel/geometry/compute header↔program associations, and unknown blocks retained. `wszst EXTRACT file.gsh` emits named Latte listings only after reassembly proves complete program-byte equality; 12 programs across five real Nintendo Land containers pass, plus a deterministic semantic assembler test. Known instructions receive Decaf semantic ALU/TEX/VTX/CF output and unsupported encodings retain lossless RAW words. `wszst CREATE <folder> --dest <file.gsh>` and `EncodeGSHShaders` / `EncodeGSHFromLatte` re-assemble Latte source listings and compile complete, valid Gfx2/GSH container binaries with headers, stages and program blocks |
| Hyrule Warriors Legends (`.idx`/`.bin`) | Archive | ✅ | ✅ | Hyrule Warriors Legends (3DS) split archive pair. The `.idx` half is an array of {u32 size, u32 offset} little-endian records addressing the sibling `.bin`, with size==0 marking a hole. `wszst EXTRACT`/`xx` triggers on a `.idx` file and locates the sibling `.bin` automatically. `wszst CREATE <folder> --dest <file.idx>` serializes index tables with sparse/ordinal member indexing and builds aligned sibling `.bin` archives |
...
| NDS / SRL / DSI | ROM / Archive | ✅ | ✅ | Nintendo DS ROM image (`.nds`, `.srl`, `.dsi`); transparent pass-through extraction with recursive unpacking of inner Nitro assets (NARCs, SDATs, fonts, textures, models) and repacking via `wszst CREATE` |
...
| RFL_Res (`RFL_Res.dat` / `RFLRes.dat`) | Archive / database | ✅ | ✅ | Wii / 3DS / Wii U Revolution Face Library (Mii resource database) archive holding categorized sub-archives (shapes, textures, facelines, hair, beards, etc.); `wszst EXTRACT`/`xx` extracts nested category trees and `wszst CREATE <folder> --dest <file.dat>` repacks multi-table binary containers |
