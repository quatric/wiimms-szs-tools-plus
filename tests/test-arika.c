...
static int check_alz1_hand_vectors(void)
{
    ...
    // followed by the 5 literal bytes "ABCDE".
    {
        ...
        u8 out[5];
        if (DecodeALZ1(out,5,stream,sizeof(stream))) return 1;
        if (memcmp(out,"ABCDE",5)) return 2;
    }
    ...
    // reads from ring address (p-1)&0xfff = 0xfee = 0xf<<8 | 0xee.
    {
        ...
        u8 out[8];
        if (DecodeALZ1(out,8,stream,sizeof(stream))) return 3;
        for (int i=0;i<8;i++) if (out[i]!='A') return 4;
    }
    ...
    // must start zero-filled rather than empty.
    {
        ...
        if (DecodeALZ1(out,18,stream,sizeof(stream))) return 5;
        for (int i=0;i<18;i++) if (out[i]!=0) return 6;
    }
...
static int check_alz1_roundtrip(void)
{
    ...
    for (uint s = 0; s < sizeof(sizes)/sizeof(*sizes); s++)
    {
        ...
        u8 *z = 0; uint zn = 0;
        if (EncodeALZ1(&z,&zn,src,n)) { free(src); return 10; }

        u8 *out = n ? malloc(n) : 0;
        enumError derr = DecodeALZ1(out,n,z,zn);
        int bad = derr || (n && memcmp(out,src,n));
        free(src); free(out); free(z);
        if (bad) return 11;
    }
    return 0;
}
...
static int check_crypt(void)
{
    ...
    if (EncryptArikaInfo(enc,sizeof(enc))) return 20;
    // Title bytes themselves are never touched.
    if (memcmp(enc,plain,0x10)) return 21;

    u8 dec[sizeof(buf)];
    memcpy(dec,enc,sizeof(buf));
    if (DecryptArikaInfo(dec,sizeof(dec))) return 22;
    if (memcmp(dec,plain,sizeof(dec))) return 23;
    ...
    if (EncryptArikaInfo(nokey,sizeof(nokey)) || memcmp(nokey,nokey_orig,sizeof(nokey))) return 24;
    if (DecryptArikaInfo(nokey,sizeof(nokey)) || memcmp(nokey,nokey_orig,sizeof(nokey))) return 25;
...
{
    for (uint i = 0; i < n; i++) { free((void*)e[i].name); free((void*)e[i].data); }
    free(e);
}
...
static int check_dr_mario_style_roundtrip(void)
{
    ...
    u8 *info=0, *game=0; uint info_size=0, game_size=0;
    ...
    if (DecryptArikaInfo(redecoded,info_size)) { free(redecoded); free(info); free(game); free_entries(in,N); return 31; }
    if (!memcmp(redecoded,info,info_size)) { free(redecoded); free(info); free(game); free_entries(in,N); return 32; } // should have changed
    free(redecoded);

    nintendo_sarc_entry_t *out = 0; uint n_out = 0;
    enumError err = ExtractArika(&out,&n_out,info,info_size,game,game_size);
    free(info); free(game);
    if (err) { free_entries(in,N); return 33; }
    if (n_out != N) { free_entries(in,N); free_entries(out,n_out); return 34; }

    int rc = 0;
    for (uint i = 0; i < N && !rc; i++)
    {
        if (strcmp(in[i].name,out[i].name)) rc = 35;
        else if (in[i].size != out[i].size) rc = 36;
        else if (in[i].size && memcmp(in[i].data,out[i].data,in[i].size)) rc = 37;
    }
...
static int check_unencrypted_uncompressed_roundtrip(void)
{
    ...
    if (CreateArika(&info,&info_size,&game,&game_size,in,N,NULL,false))
        { free_entries(in,N); return 40; }
    if (info[0]) { free(info); free(game); free_entries(in,N); return 41; } // no title -> unencrypted

    nintendo_sarc_entry_t *out = 0; uint n_out = 0;
    enumError err = ExtractArika(&out,&n_out,info,info_size,game,game_size);
    free(info); free(game);
    if (err || n_out != N) { free_entries(in,N); free_entries(out,n_out); return 42; }

    int rc = 0;
    for (uint i = 0; i < N && !rc; i++)
        if (in[i].size != out[i].size || (in[i].size && memcmp(in[i].data,out[i].data,in[i].size)))
            rc = 43;
...
static int check_rf2_grouping(void)
{
    ...
    enum { OUT_HDR=16, SUB=0x20 };
    enum { N_SUB = 3 };
    // relative offsets for the 3 sub-entries' data, chosen so nothing overlaps
    enum { NEST_OFF = 0x80, LEAFB_OFF = 0xC0, LEAFC_OFF = 0xD0, GAME_SIZE = 0x100 };

    u8 *game = calloc(1,GAME_SIZE);
    // outer RF2 header
    memcpy(game+0,"RF2",3);
    memcpy(game+3,"TYP",3);
    game[6]=1; game[7]=0; // VER
    u32 info_size = N_SUB*0x20;
    game[8]=(u8)info_size; game[9]=(u8)(info_size>>8); game[10]=(u8)(info_size>>16);
    game[11]=0;
    ...
    rec[20]=1; rec[21]=rec[22]=rec[23]=0;
    rec[24]=(u8)NEST_OFF; rec[25]=rec[26]=rec[27]=0; // OFFSET (relative)
    rec += 0x20;
    // sub 1: leaf_b
    memcpy(rec+0,"leaf_b",6);
    u32 sz = sizeof(leaf_b);
    rec[20]=(u8)sz; rec[21]=(u8)(sz>>8); rec[22]=(u8)(sz>>16); rec[23]=(u8)(sz>>24);
    rec[24]=(u8)LEAFB_OFF; rec[25]=rec[26]=rec[27]=0;
    rec += 0x20;
    // sub 2: leaf_c
    memcpy(rec+0,"leaf_c",6);
    sz = sizeof(leaf_c);
    rec[20]=(u8)sz; rec[21]=(u8)(sz>>8); rec[22]=(u8)(sz>>16); rec[23]=(u8)(sz>>24);
    rec[24]=(u8)LEAFC_OFF; rec[25]=rec[26]=rec[27]=0;
    ...
    u32 rsize = GAME_SIZE;
    info[0x30+0x20]=(u8)rsize; info[0x30+0x21]=(u8)(rsize>>8);
    info[0x30+0x22]=(u8)(rsize>>16); info[0x30+0x23]=(u8)(rsize>>24);
    info[0x30+0x24]=0; // offset in sectors (=bytes here) = 0
    info[0x30+0x2c]=(u8)rsize; info[0x30+0x2d]=(u8)(rsize>>8); // dsize == zsize -> "raw"
    ...
    if (n_out != 3) { free_entries(out,n_out); return 51; }
    ...
    for (uint i = 0; i < n_out; i++)
    {
        ...
        {
            if (out[i].size!=sizeof(leaf_a) || memcmp(out[i].data,leaf_a,sizeof(leaf_a))) return (free_entries(out,n_out),52);
            found_a=1;
        }
        else if (!strcmp(out[i].name,"group/leaf_b"))
        {
            if (out[i].size!=sizeof(leaf_b) || memcmp(out[i].data,leaf_b,sizeof(leaf_b))) return (free_entries(out,n_out),53);
            found_b=1;
        }
        else if (!strcmp(out[i].name,"group/leaf_c"))
        {
            if (out[i].size!=sizeof(leaf_c) || memcmp(out[i].data,leaf_c,sizeof(leaf_c))) return (free_entries(out,n_out),54);
            found_c=1;
        }
    }
    free_entries(out,n_out);
    if (!found_a || !found_b || !found_c) return 55;
    return 0;
}

int main(void)
{
    struct { ccp name; int (*fn)(void); } tests[] = {
        ...
        { "Dr. Mario style archive (encrypted INFO.DAT, ALZ1 GAME.DAT) round trip", check_dr_mario_style_roundtrip },
        { "Unencrypted/uncompressed archive round trip (DS Endless Ocean style)", check_unencrypted_uncompressed_roundtrip },
