...
int main (void)
{
	...
	if (check (literals, sizeof (literals), "ABCD", 4))
		return 1;
	static const u8 m1[] = { 18, 'A', 0, 0, 0x11, 0, 0 };
	if (check (m1, sizeof (m1), "AAA", 3))
		return 2;
	static const u8 m2[] = { 22, 'A', 'B', 'C', 'D', 'E', 0x40, 0, 0x11, 0, 0 };
	if (check (m2, sizeof (m2), "ABCDEEEE", 8))
		return 3;
	static const u8 m3[] = { 22, 'A', 'B', 'C', 'D', 'E', 0x80, 0, 0x11, 0, 0 };
	if (check (m3, sizeof (m3), "ABCDEEEEEE", 10))
		return 4;
	...
	uint p = 0;
	long_stream[p++] = 0;
	memset (long_stream + p, 0, 64);
	p += 64;
	long_stream[p++] = 47; // 3 + 15 + 64*255 + 47 = 16385 literals
	for (uint i = 0; i < 16385; i++)
		long_stream[p++] = (u8)(i * 29u);
	long_stream[p++] = 0x21;
	long_stream[p++] = 0;
	long_stream[p++] = 0x40;
	long_stream[p++] = 0x11;
	...
	static const u8 cmpd[] = { 'C', 'M', 'P', 'D', 0, 0, 0, 1, 0xc0, 0, 0, 13, 0, 0, 0, 5, 0xff,
		0xfe, 'X', 'Y', 0, 7, 18, 'A', 0, 0, 0x11, 0, 0 };
