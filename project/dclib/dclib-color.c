...
const KeywordTab_t color_mode_keywords[]
	= { { COLMD_OFF, "OFF", "NONE", 0 }, { COLMD_OFF, "NO-COLORS", "NOCOLORS", 0 },
		  { COLMD_OFF, "0-COLORS", "0COLORS", 0 }, { COLMD_AUTO, "AUTO", 0, 0 },
		  { COLMD_ON, "ON", 0, 0 }, { COLMD_8_COLORS, "8-COLORS", "8COLORS", 0 },
		  { COLMD_256_COLORS, "256-COLORS", "256COLORS", 0 }, { 0, 0, 0, 0 } };
...
int ScanOptColorize (ColorMode_t *opt, ccp arg, ccp err_prefix)
{
	...
	if (!key)
		return PrintKeywordError (color_mode_keywords, arg, abbrev,
			err_prefix ? err_prefix : "Option --color: ", "keyword");
...
ColorMode_t NormalizeColorMode (ColorMode_t col_mode, int n_colors)
{
	...
	switch (col_mode)
	{
		...
		default:
			// case COLMD_AUTO:
			return n_colors < 8 ? COLMD_OFF : n_colors < 256 ? COLMD_8_COLORS : COLMD_256_COLORS;
	}
}

///////////////////////////////////////////////////////////////////////////////

ColorMode_t NormalizeColorModeByTermName (ColorMode_t col_mode, // predefind mode
...
ColorMode_t GetNextColorMode (ColorMode_t col_mode)
{
	...
	{
		case COLMD_8_COLORS:
			return COLMD_256_COLORS;
		case COLMD_256_COLORS:
			return COLMD_OFF;
		default:
			return COLMD_8_COLORS;
	}
}
...
)
{
	...
	while (*text)
	{
		...
		// print out
		if (*text || text > start)
			fprintf (f, "%s%*s%.*s%s", prefix1, indent1, "", (int)(text - start), start, eol);
		...
		switch (code)
		{
			...
			case '}':
				if (!cur.col_embed)
					goto is_char;
				cur.col_embed = 0;
				cur.dest = StringCopyE (
					cur.dest, buf + sizeof (buf), cur.col_active ? cur.col_active : colset->reset);
				break;
...
)
{
	...
	va_list arg;
	va_start (arg, format);
	const uint res = PrintArgColoredLines (f, colset, indent, fw, prefix, eol, format, arg);
	va_end (arg);
	return res;
}
...
static void PrintColorInfo (FILE *f, color_info_t *ci)
{
	...
	if (ci->valid)
		fprintf (f,
			...
			ci->src_color, ci->ref_color, ci->m_index, ci->m_red, ci->m_green, ci->m_blue,
			ci->m_gray, ci->m_index, ci->m_index, ci->m_index, ci->m_index, ci->m_index,
			ci->m_index);
	else
		fputs (" --\n", f);
}
...
u8 ConvertColorRGB3ToM256 (u8 r, u8 g, u8 b)
{
	...
	uint delta = abs ((r6 ? 55 + 40 * r6 : 0) - r) + abs ((g6 ? 55 + 40 * g6 : 0) - g)
		+ abs ((b6 ? 55 + 40 * b6 : 0) - b);
...
u32 ConvertColorM256ToRGB (u8 m256)
{
	...
	if (m256 < 232)
	{
		...
		u32 col = 0;
		if (r)
			col |= 0x370000 + 0x280000 * r;
		if (g)
			col |= 0x003700 + 0x002800 * g;
		if (b)
			col |= 0x000037 + 0x000028 * b;
		return col;
	}
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&opt_width, // u32 * num
			arg, // ccp source
			1, // default_factor1
			0, // int force_base
			"width", // ccp opt_name
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&opt_max_width, // u32 * num
			arg, // ccp source
			1, // default_factor1
			0, // int force_base
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&opt_height, // u32 * num
			arg, // ccp source
			1, // default_factor1
			0, // int force_base
			"height", // ccp opt_name
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&opt_max_height, // u32 * num
			arg, // ccp source
			1, // default_factor1
			0, // int force_base
...
{
	PRINT0 ("GetTermWidth(%d,%d) opt=%d,%d, max=%d,%d\n", default_width, default_height, opt_width,
		opt_height, opt_max_width, opt_max_height);
...
bool GetAutoTermSize ()
{
	...
	if (fd >= 0)
	{
		term_size_t size = GetTermSizeFD (fd, auto_term_size.width, auto_term_size.height);
...
good_term_width_t good_term_width
	= { .min_width = 40, .def_width = 100, .max_width = 120, .good_width = 120 };
...
const int Color18Index[18 + 1] = // 18 elements + -1 as terminator
	{ TCI_BLACK, // gray scale
		TCI_DARKGRAY, TCI_LIGHTGRAY, TCI_WHITE,

		TCI_RED, // normal colors in rainbow order
		TCI_ORANGE, TCI_YELLOW, TCI_GREEN, TCI_CYAN, TCI_BLUE, TCI_MAGENTA,

		TCI_B_RED, // bold colors in rainbow order
		TCI_B_ORANGE, TCI_B_YELLOW, TCI_B_GREEN, TCI_B_CYAN, TCI_B_BLUE, TCI_B_MAGENTA,
...
)
{
	...
	else if (mode & TTM_F_GRAYSCALE)
	{
		if (sep++)
			*dest++ = ';';
		dest = snprintfE (dest, end, "38;5;%u", grayscale_tab[mode & TTM_M_GRAYSCALE]);
	}
	...
	else if (mode & TTM_F_BG_GRAYSCALE)
	{
		if (sep++)
			*dest++ = ';';
		dest = snprintfE (
			dest, end, "48;5;%u", grayscale_tab[(mode & TTM_M_BG_GRAYSCALE) >> TTM_SHIFT_BG]);
	}
	...
	)
{
	...
	if (option & GCM_ALT && (uint)font_index < TCI__IGNORE && (uint)bg_index < TCI__IGNORE)
	{
		...
		const uint font = font_index < TCI_B_RED ? font_index : font_index - (TCI_B_RED - TCI_RED);
...
static const color_helper_tab_t ch_tab_color[] = { SEP1 JOB1 (red_magenta) JOB1 (red)
		JOB1 (red_orange) JOB1 (orange_red) JOB1 (orange) JOB1 (orange_yellow) JOB1 (yellow_orange)
			JOB1 (yellow) JOB1 (yellow_green) JOB1 (green_yellow) JOB1 (green) JOB1 (green_cyan)
				JOB1 (cyan_green) JOB1 (cyan) JOB1 (cyan_blue) JOB1 (blue_cyan) JOB1 (blue) JOB1 (
					blue_magenta) JOB1 (magenta_blue) JOB1 (magenta) JOB1 (magenta_red) JOB0 };

//-----------------------------------------------------------------------------

static const color_helper_tab_t ch_tab_bcolor[] = { // [[new-color]]

	SEP1 JOB1 (b_red_magenta) JOB1 (b_red) JOB1 (b_red_orange) JOB1 (b_orange_red) JOB1 (b_orange)
		JOB1 (b_orange_yellow) JOB1 (b_yellow_orange) JOB1 (b_yellow) JOB1 (b_yellow_green) JOB1 (
			b_green_yellow) JOB1 (b_green) JOB1 (b_green_cyan) JOB1 (b_cyan_green) JOB1 (b_cyan)
			JOB1 (b_cyan_blue) JOB1 (b_blue_cyan) JOB1 (b_blue) JOB1 (b_blue_magenta)
				JOB1 (b_magenta_blue) JOB1 (b_magenta) JOB1 (b_magenta_red) JOB0
};

//-----------------------------------------------------------------------------
_Static_assert (N_COLORSET_GREY == 10, "wrong N_COLORSET_GREY");

static const color_helper_tab_t ch_tab_grey[] = { // [[new-color]]

	SEP1 JOB1 (black) JOB1 (b_black) JOB1 (white) JOB1 (b_white)

		SEP1 JOB2 (grey0, grey[0], 0) JOB2 (grey1, grey[1], 0) JOB2 (grey2, grey[2], 0) JOB2 (grey3,
			grey[3], 0) JOB2 (grey4, grey[4], 0) JOB2 (grey5, grey[5], 0) JOB2 (grey6, grey[6], 0)
			JOB2 (grey7, grey[7], 0) JOB2 (grey8, grey[8], 0) JOB2 (grey9, grey[9], 0)
...
static const color_helper_tab_t ch_tab_name1[] = { // [[new-color]]

	SEP1

		JOB1 (setup) JOB1 (run) JOB1 (abort) JOB1 (finish) JOB1 (script) JOB1 (open) JOB1 (close)
			JOB1 (file) JOB1 (job) JOB1 (debug) JOB1 (log)

				JOB1 (mark) JOB1 (info) JOB1 (hint) JOB1 (warn) JOB1 (err) JOB1 (bad)

					JOB1 (name) JOB1 (op) JOB1 (value) JOB2 (val, value, 1) JOB1 (success)
						JOB1 (error) JOB1 (fail) JOB1 (fail2) JOB1 (fatal)

							JOB1 (select) JOB1 (differ) JOB1 (stat_line) JOB1 (warn_line)
								JOB1 (proc_line)
...
static const color_helper_tab_t ch_tab_name2[] = { // [[new-color]]

	SEP1

		JOB2 (hl_line0, hl_line[0], 0) JOB2 (hl_line1, hl_line[1], 0) JOB2 (hl_line2, hl_line[2], 0)
			JOB2 (hl_line3, hl_line[3], 0) JOB2 (order0, order[0], 0) JOB2 (order1, order[1], 0)
				JOB2 (order2, order[2], 0) JOB2 (order3, order[3], 0) JOB2 (order4, order[4], 0)
					JOB2 (order5, order[5], 0) JOB2 (order6, order[6], 0) JOB2 (order7, order[7], 0)
						JOB2 (order8, order[8], 0) JOB1 (cite) JOB1 (status) JOB1 (highlight)
							JOB2 (hl, highlight, 1) JOB1 (hide)

								JOB1 (heading) JOB2 (head, heading, 1) JOB1 (caption) JOB1 (section)
									JOB1 (syntax) JOB1 (cmd) JOB1 (option) JOB2 (opt, option, 1)
										JOB1 (param) JOB2 (par, param, 1)
...
static const color_helper_tab_t ch_tab_name[] = { // [[new-color]]

	SEP1

		JOB1 (setup) JOB1 (run) JOB1 (abort) JOB1 (finish) JOB1 (script) JOB1 (open) JOB1 (
			close) JOB1 (file) JOB1 (job) JOB1 (debug) JOB1 (log)

			JOB1 (mark) JOB1 (info) JOB1 (hint) JOB1 (warn) JOB1 (err) JOB1 (bad)

				JOB1 (name) JOB1 (op) JOB1 (value) JOB2 (val, value, 1) JOB1 (success) JOB1 (
					error) JOB1 (fail) JOB1 (fail2) JOB1 (fatal)

					JOB1 (select) JOB1 (differ) JOB1 (stat_line) JOB1 (warn_line) JOB1 (
						proc_line) JOB2 (hl_line0, hl_line[0], 0) JOB2 (hl_line1, hl_line[1], 0)
						JOB2 (hl_line2, hl_line[2], 0) JOB2 (hl_line3, hl_line[3], 0) JOB2 (
							order0, order[0], 0) JOB2 (order1, order[1], 0) JOB2 (order2, order[2],
							0) JOB2 (order3, order[3], 0) JOB2 (order4, order[4], 0)
							JOB2 (order5, order[5], 0) JOB2 (order6, order[6], 0) JOB2 (
								order7, order[7], 0) JOB2 (order8, order[8], 0) JOB1 (cite)
								JOB1 (status) JOB1 (highlight) JOB2 (hl, highlight, 1) JOB1 (hide)

									JOB1 (heading) JOB2 (head, heading, 1) JOB1 (caption)
										JOB1 (section) JOB1 (syntax) JOB1 (cmd) JOB1 (option)
											JOB2 (opt, option, 1) JOB1 (param) JOB2 (par, param, 1)
...
const ColorSet_t *GetColorSet8 ()
{
	...
	if (!col.col_mode)
	{
		...
		col.grey[0] = col.grey[1] = col.black = STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_BLACK));
		col.grey[2] = col.grey[3] = col.grey[4] = col.b_black
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_BLACK));
		col.grey[5] = col.grey[6] = col.grey[7] = col.white
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_WHITE));
		col.grey[8] = col.grey[9] = col.b_white = STRDUP (GetTextMode (1, TTM_BOLD | TTM_WHITE));

		col.red_magenta = col.red = col.red_orange = col.orange_red = col.orange
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_RED));
		col.orange_yellow = col.yellow_orange = col.yellow = col.yellow_green
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_YELLOW));
		col.green_yellow = col.green = col.green_cyan
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_GREEN));
		col.cyan_green = col.cyan = col.cyan_blue
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_CYAN));
		col.blue_cyan = col.blue = col.blue_magenta
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_BLUE));
		col.magenta_blue = col.magenta = col.magenta_red
			= STRDUP (GetTextMode (1, TTM_NO_BOLD | TTM_MAGENTA));

		col.b_red_magenta = col.b_red = col.b_red_orange = col.b_orange_red = col.b_orange
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_RED));
		col.b_orange_yellow = col.b_yellow_orange = col.b_yellow = col.b_yellow_green
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_YELLOW));
		col.b_green_yellow = col.b_green = col.b_green_cyan
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_GREEN));
		col.b_cyan_green = col.b_cyan = col.b_cyan_blue
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_CYAN));
		col.b_blue_cyan = col.b_blue = col.b_blue_magenta
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_BLUE));
		col.b_magenta_blue = col.b_magenta = col.b_magenta_red
			= STRDUP (GetTextMode (1, TTM_BOLD | TTM_MAGENTA));
...
const ColorSet_t *GetColorSet256 ()
{
	...
	if (!col.col_mode)
	{
		...
		// ./mkw-ana testcol 401 400 410 420 320 330 230 130 030 031 032 033 024 015 005 204 304 404
		// 402 401

		col.red_magenta = "\e[38;5;161m";
		col.red = "\e[38;5;160m";
		col.red_orange = col.orange_red = "\e[38;5;166m";
		col.orange = "\e[38;5;172m";
		col.orange_yellow = col.yellow_orange = "\e[38;5;136m";
		...
		// ./mkw-ana testcol 502 500 510 520 530 540 550 350 250 050 053 054 055 135 235 225 325 425
		// 515 513 502

		col.b_red_magenta = "\e[38;5;198m";
		col.b_red = "\e[38;5;196m";
		col.b_red_orange = col.b_orange_red = "\e[38;5;202m";
...
const ColorSet_t *GetColorSetAuto (bool force_on)
{
	...
	if (auto_mode == COLMD_AUTO)
	{
		...
		fprintf (stderr, ">>> GetColorSetAuto(%d) => \"%s\" n=%d => %d [%s]\n", force_on, term,
			ncol, auto_mode, GetColorModeName (auto_mode, 0));
#endif
	}
...
#define DEF2(t, n) { offsetof (ColorSet_t, n), #t },
	...
	static const struct cdef_t tab[] = { // [[new-color]] order by name

		DEF1 (abort) DEF1 (b_black) DEF1 (b_blue) DEF1 (b_blue_cyan) DEF1 (b_blue_magenta) DEF1 (
			b_cyan) DEF1 (b_cyan_blue) DEF1 (b_cyan_green) DEF1 (b_green) DEF1 (b_green_cyan)
			DEF1 (b_green_yellow) DEF1 (b_magenta) DEF1 (b_magenta_blue) DEF1 (b_magenta_red) DEF1 (
				b_orange) DEF1 (b_orange_red) DEF1 (b_orange_yellow) DEF1 (b_red)
				DEF1 (b_red_magenta) DEF1 (b_red_orange) DEF1 (b_white) DEF1 (b_yellow) DEF1 (
					b_yellow_green) DEF1 (b_yellow_orange) DEF1 (bad) DEF1 (black) DEF1 (blue)
					DEF1 (blue_cyan) DEF1 (blue_magenta) DEF1 (caption) DEF1 (cite) DEF1 (close)
						DEF1 (cmd) DEF1 (cyan) DEF1 (cyan_blue) DEF1 (cyan_green) DEF1 (debug)
							DEF1 (differ) DEF1 (err) DEF1 (error) DEF1 (fail) DEF1 (fail2) DEF1 (
								fatal) DEF1 (file) DEF1 (finish) DEF2 (gray0, grey[0]) DEF2 (gray1,
								grey[1]) DEF2 (gray2, grey[2]) DEF2 (gray3, grey[3]) DEF2 (gray4,
								grey[4]) DEF2 (gray5, grey[5]) DEF2 (gray6, grey[6]) DEF2 (gray7,
								grey[7]) DEF2 (gray8, grey[8]) DEF2 (gray9, grey[9]) DEF1 (green)
								DEF1 (green_cyan) DEF1 (green_yellow) DEF2 (grey0, grey[0]) DEF2 (
									grey1, grey[1]) DEF2 (grey2, grey[2]) DEF2 (grey3,
									grey[3]) DEF2 (grey4, grey[4]) DEF2 (grey5,
									grey[5]) DEF2 (grey6, grey[6]) DEF2 (grey7,
									grey[7]) DEF2 (grey8, grey[8]) DEF2 (grey9, grey[9]) DEF2 (head,
									heading) DEF1 (heading) DEF1 (hide) DEF1 (highlight) DEF1 (hint)
									DEF2 (hl, highlight) DEF2 (hl_line0, hl_line[0]) DEF2 (hl_line1,
										hl_line[1]) DEF2 (hl_line2, hl_line[2]) DEF2 (hl_line3,
										hl_line[3]) DEF1 (info) DEF1 (job) DEF1 (log)
										DEF1 (magenta) DEF1 (magenta_blue) DEF1 (magenta_red) DEF1 (
											mark) DEF1 (name) DEF1 (off) DEF1 (on) DEF1 (op)
											DEF1 (open) DEF2 (opt, option) DEF1 (option) DEF1 (
												orange) DEF1 (orange_red) DEF1 (orange_yellow)
												DEF2 (order0, order[0]) DEF2 (order1,
													order[1]) DEF2 (order2, order[2]) DEF2 (order3,
													order[3]) DEF2 (order4, order[4]) DEF2 (order5,
													order[5]) DEF2 (order6, order[6]) DEF2 (order7,
													order[7]) DEF2 (order8, order[8]) DEF2 (par,
													param) DEF1 (param) DEF1 (proc_line) DEF1 (red)
													DEF1 (red_magenta) DEF1 (red_orange)
														DEF1 (reset) DEF1 (run) DEF1 (script) DEF1 (
															section) DEF1 (select) DEF1 (setup)
															DEF1 (stat_line) DEF1 (status)
																DEF1 (success) DEF1 (syntax) DEF2 (
																	val, value) DEF1 (value)
																	DEF1 (warn) DEF1 (warn_line)
																		DEF1 (white) DEF1 (yellow)
																			DEF1 (yellow_green)
																				DEF1 (yellow_orange)
	};
	...
	if (!done)
	{
		...
		DASSERT_MSG (max_len < sizeof (cd->name), "%s(): max name len = %d, possible = %zd\n",
			__FUNCTION__, max_len, sizeof (cd->name) - 1);
		DASSERT_MSG (!err_count, "%s(): color order is %d* wrong!\n", __FUNCTION__, err_count);
	}
#endif
...
void ResetLineColor (const ColorSet_t *colset)
{
	...
	cs->line_begin = cs->line_end = EmptyString;
...
void SetLineColor (const ColorSet_t *colset, int level)
{
	...
	{
		cs->line_begin = cs->line_reset
			= cs->hl_line[level < N_COLORSET_HL_LINE ? level : N_COLORSET_HL_LINE - 1];
...
static void print_by_helper_tab (color_helper_t *ch, // valid param
...
)
{
	...
	if (ch->multi_assign <= 0)
	{
		for (; tab->name; tab++)
		{
			if (!tab->is_alt || ch->print_alt)
			{
				...
				{
					ch->allow_sep = true;
					ch->func (ch->f, ch->indent, col, tab->name, *(ccp *)((u8 *)col + tab->offset));
				}
	...
	if (ch->multi_assign > 1)
	{
		...
		for (; tab->name; tab++)
		{
			...
			{
				snprintf (
					key, sizeof (key), "%s\1%s", *(ccp *)((u8 *)col + tab->offset), tab->name);
...
void PrintColorSetHelper2 (color_helper_t *ch // valid params
)
{
	// [[new-color]]
	...
	if (ch->multi_assign > 1)
	{
		...
		for (ptr = ch->sf.field; ptr < end; ptr++)
		{
			...
			if (!name)
				continue; // this may never happen!!
...
void PrintColorSetHelper (FILE *f, // valid output file
	int indent, // indention of output
	const ColorSet_t *cs, // valid color set; if NULL: use std colors
	PrintColorFunc func, // output function, never NULL
	ColorSelect_t select, // select color groups
...
void PrintColorSet (FILE *f, // valid output file
	int indent, // indention of output
	const ColorSet_t *cs, // valid color set; if NULL: use std colors
	ColorSelect_t select, // select color groups
...
#ifdef DCLIB_TERMINAL
	cv->col_mode = cv->colset ? cv->colset->col_mode
							  : NormalizeColorMode (cv->col_mode, cv->term ? cv->term->colors : 8);
#else
	cv->col_mode = cv->colset ? cv->colset->col_mode : NormalizeColorMode (cv->col_mode, 8);
#endif
	if (!cv->colset)
...
static ccp col_names[]
	= { "black:", "red:", "green:", "yellow:", "blue:", "magenta:", "cyan:", "white:", 0 };
...
void ViewColorsAttrib8 (ColorView_t *cv)
{
	...
	fprintf (cv->f, "%*s%s() %s, %u colors, mode %d, stdcol %d, order %d\n", cv->indent, "",
		__FUNCTION__, GetColorModeName (cv->colset->col_mode, 0), cv->colset->n_colors, cv->mode,
		cv->std_col, cv->order);
#endif
	...
	fprintf (cv->f, "%s\n%*sStandard font colors with attributes (ul=underline) for mode %s:\n\n",
		cv->colset->reset, cv->indent, "", GetColorModeName (cv->col_mode, 0));
	...
	for (cname = col_names; *cname; cname++, mode += 0x01)
	{
		fprintf (cv->f, "%*s%s %u %-9s %snormal %sbold %sbold+ul %sul%s reset %s\n", cv->indent, "",
			GetTextMode (cv->col_mode, fg | bg), (int)(cname - col_names) + 30, *cname,
			GetTextMode (cv->col_mode, bg | mode), GetTextMode (cv->col_mode, bg | TTM_BOLD),
			GetTextMode (cv->col_mode, bg | TTM_UL), GetTextMode (cv->col_mode, bg | TTM_NO_BOLD),
			GetTextMode (cv->col_mode, bg | TTM_RESET), cv->colset->reset);
	}
	...
	for (cname = col_names; *cname; cname++, mode += 0x10)
	{
		fprintf (cv->f, "%*s %u %-9s %s white %s bold %sul%s   black %sbold %sul%s %s\n",
			cv->indent, "", (int)(cname - col_names) + 40, *cname,
			GetTextMode (cv->col_mode, mode | TTM_WHITE), GetTextMode (cv->col_mode, TTM_BOLD),
			GetTextMode (cv->col_mode, TTM_UL | TTM_NO_BOLD),
			GetTextMode (cv->col_mode, mode | TTM_BLACK | TTM_RESET),
			GetTextMode (cv->col_mode, TTM_BOLD), GetTextMode (cv->col_mode, TTM_UL | TTM_NO_BOLD),
			GetTextMode (cv->col_mode, TTM_NO_UL), GetTextMode (cv->col_mode, TTM_RESET));
	}
}
...
void ViewColorsCombi8 (ColorView_t *cv)
{
	...
	for (cname = col_names; *cname; cname++, mode += 0x10)
	{
		fprintf (cv->f, "%*s %u %-9s ", cv->indent, "", (int)(cname - col_names) + 40, *cname);

		TermTextMode_t mode2;
		for (mode2 = TTM_BLACK; mode2 <= TTM_WHITE; mode2++)
			fprintf (cv->f, "%s no %sbo ", GetTextMode (cv->col_mode, mode | mode2 | TTM_NO_BOLD),
...
void ViewColors18 (ColorView_t *cv)
{
	...
	fprintf (cv->f, "%s\n%*s18x18 color table ('0'=std, '1'=optimized%s) for mode %s:\n\n",
		cv->colset->reset, cv->indent, "", col_option & GCM_SHORT ? "; allow short codes" : "",
	...
	for (pbg = Color18Index; *pbg >= 0; pbg++)
	{
		...
		if (bg == TCI_RED || bg == TCI_B_RED)
			fputc ('\n', cv->f);
		fprintf (cv->f, "%*s %-12s ", cv->indent, "", ColorIndexName[bg]);
		...
		for (pfont = Color18Index; *pfont >= 0; pfont++)
		{
			...
			fprintf (cv->f, "%s 0%s1", GetColorMode (cv->col_mode, font, bg, col_option),
...
void ViewColorsDC (ColorView_t *cv, bool optimized)
{
	...
	fprintf (cv->f, "%s\n%*s%ux%u color table (%s%s) for mode %s:\n\n", cv->colset->reset,
		cv->indent, "", TCI__N, TCI__N, optimized ? "optimized" : "standard",
		col_option & GCM_SHORT ? "; allow short codes" : "", GetColorModeName (cv->col_mode, 0));
	...
	for (bg = 0; bg < TCI__N; bg++)
	{
		if (bg == TCI_RED || bg == TCI_B_RED)
			fputc ('\n', cv->f);
		fprintf (cv->f, "%*s %-15s ", cv->indent, "", ColorIndexName[bg]);
		...
		for (font = 0; font < TCI__N; font++)
		{
			...
			fprintf (cv->f, "%s%sx", GetColorMode (cv->col_mode, font, bg, col_option), fill);
			fill = "";
		}
		fprintf (cv->f, " %s\n", TERM_TEXT_MODE_RESET);
	}
...
static void ViewColors256_helper (ColorView_t *cv, // basic data
...
static void ViewColorsPredef_helper (ColorView_t *cv, // basic data
	const color_helper_tab_t *tab // valid color list
)
{
	DASSERT (cv);
...
#pragma GCC diagnostic ignored "-Wformat-overflow"
	fprintf (cv->f, "%s\n%*sPredefined%s color names for mode %s:\n\n", cv->colset->reset,
		cv->indent, "", colsel & COLSEL_NAME ? " semantic" : "",
		GetColorModeName (cv->col_mode, 0));
...
void ViewColorsPage (ColorView_t *cv, uint page)
{
	...
	switch (page)
	{
		...
		case 1:
			...
			break;
		case 2:
			ViewColorsDC (cv, false);
			break;
		case 3:
			ViewColorsDC (cv, true);
			break;
		case 4:
			cv->mode = 0;
			ViewColors256 (cv);
		...
		case 5:
			...
			break;
		case 6:
			ViewColorsPredef (cv, COLSEL_GREY);
			break;
		case 7:
			ViewColorsPredef (cv, COLSEL_M_COLOR);
			break;
		case 8:
			ViewColorsPredef (cv, COLSEL_NAME1 | COLSEL_F_ALT);
			break;
		case 9:
			ViewColorsPredef (cv, COLSEL_NAME2 | COLSEL_F_ALT);
			break;
	}
	cv->mode = saved_mode;
}
...
void PrintTextModes (FILE *f, // valid output file
	int indent, // indention of output
...
void PrintColorModes (FILE *f, // valid output file
	int indent, // indention of output
...
ColorMode_t GetFileColorized (FILE *f)
{
	...
	return !f												? COLMD_OFF
		...
		: opt_colorize == COLMD_AUTO && isatty (fileno (f)) ? COLMD_ON
															: COLMD_OFF;
}
...
void SetupStdMsg ()
{
	...
	noPRINT ("SetupStdMsg() : out=%d[%zd], err=%d[%zd], log=%d[%zd], msg=%d[%zd]\n",
		colorize_stdout, colout ? strlen (colout->reset) : -1, colorize_stderr,
		colerr ? strlen (colerr->reset) : -1, colorize_stdlog, collog ? strlen (collog->reset) : -1,
		colorize_stdmsg, colmsg ? strlen (colmsg->reset) : -1);
}

///////////////////////////////////////////////////////////////////////////////

enumError OpenStdLog (ccp fname, // If NULL or empty: don't open and use 'fallback'
	...
	if (!fname || !*fname)
	{
		if (fallback)
		{
			...
			PRINT ("COLORIZE/LOG/FALLBACK: opt=%d, out=%d, err=%d, log=%d\n", opt_colorize,
				colorize_stdout, colorize_stderr, colorize_stdlog);
		}
	}
	else
	{
		...
		else
		{
			...
			if (!stdlog)
				return ERROR1 (ERR_CANT_CREATE, "Can't open log file (mode=%s): %s\n",
		...
		PRINT ("COLORIZE/LOG: opt=%d, out=%d, err=%d, log=%d\n", opt_colorize, colorize_stdout,
			colorize_stderr, colorize_stdlog);
	}
...
void RedirectStdFiles (SavedStdFiles_t *ssf, // not NULL: save old output here
...
enumError CatchStdFiles (SavedStdFiles_t *ssf, // save old output and data here, never NULL
