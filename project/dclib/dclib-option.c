...
void AddGenericOpt (GenericOpt_t *go, // data structure
	...
	for (; gp->name; gp++)
	{
		...
		for (;;)
		{
			...
			*name_ptr = 0;
			...
			{
				uchar ch = *name_buf;
				if (ch > ' ' && ch < 0x80 && ch != ':' && !go->sopt_id[ch] && sopt_ptr < sopt_end)
				{
					go->sopt_id[ch] = gp->id;
					*sopt_ptr++ = ch;
...
ccp PrintGParam (const GParam_t *par)
{
	...
	switch (par->ptype)
	{
		...
		case GOPT_IDX_BIT:
			snprintf (buf, sizeof (buf), "[BIT] set=%x, clr=%x", par->bit.val,
				par->bit.mask & ~par->bit.val);
			break;
		...
		case GOPT_IDX_SIZE:
			snprintf (buf, sizeof (buf), "[SIZE] %llu = %s", par->un64,
				PrintSize1024 (0, 0, par->un64, 0));
			break;
		...
		case GOPT_IDX_DURATION:
			snprintf (buf, sizeof (buf), "[DUR] %12g = %s", par->d,
				PrintTimerUSec6 (0, 0, double2usec (par->d), 0));
			break;
...
const GenericOptParm_t StandardGOptions[] = { { GOPT_O_TEST_OPT, 0, "@opt" },
	{ GOPT_O_HELP, 0, "h|help" }, { GOPT_O_CLEAR, 0, "c|clear" }, { GOPT_O_PAGER, 0, "p|pager" },
	{ GOPT_O_WIDTH, 1, "w|width" }, { GOPT_O_QUIT, 0, "Q|quit" }, { GOPT_O_QUIET, 0, "q|quiet" },
	{ GOPT_O_VERBOSE, 3, "v|verbose" }, { GOPT_O_DEBUG, 3, "D|d|debug" }, { 0, 0, 0 } };
...
enumError ScanGOptionsHelper (GOptions_t *gopt, // valid pointer
...
)
{
		...
		if (opt_stat >= GOPT_RESERVED_BEG && opt_stat < GOPT_RESERVED_END)
		{
			...
			if (pidx >= GOPT_N_PARAM)
			{
				printf ("%s!!! %s: Parameter index %u must be <%u: %s%s\n", colout->warn, *argv,
					pidx, GOPT_N_PARAM, argv[optind - 1], colout->reset);
				stat = ERR_SYNTAX;
				break;
			}
			...
			switch (ptype)
			{
				...
				case GOPT_IDX_SIZE:
					...
					ScanSizeU64 (&par->un64, optarg, 1, 1, 0);
					break;
					break;
				...
				case GOPT_IDX_DURATION:
					...
					ScanDuration (&par->d, optarg, 1, SDUMD_M_DEFAULT);
					break;
					break;
				...
				default:
					printf ("!!! %s: Undefinef GOPT_* type 0x%02x [0x%02x]: %s\n", *argv,
						GetGOPType (opt_stat), opt_stat, argv[optind - 1]);
					stat = ERR_ERROR;
			}
			...
			case '?':
				if (optopt && (uint)optopt < 0x7f)
					printf ("%s!!! %s: Invalid option: %s-%c [0x%02x]%s\n", colout->warn, *argv,
						colout->value, optopt, optopt, colout->reset);
				else
					printf ("%s!!! %s: Invalid option: %s%s%s\n", colout->warn, *argv,
						colout->value, argv[optind - 1], colout->reset);
				stat = ERR_SYNTAX;
				break;
	...
	{
		printf ("\n## %sShort options:%s %s\n## %sLong options (%u/%u):%s\n", colout->caption,
			colout->reset, go->sopt, colout->caption, go->lopt_used, go->lopt_size, colout->reset);

		for (const struct option *opt = go->lopt; opt->name; opt++)
			printf ("## %6x %c %s\n", opt->val, opt->has_arg ? '0' + opt->has_arg : '-', opt->name);
		putchar ('\n');
	}
	...
	{
		if (!max_param)
			printf ("%s!!! %s: No parameters allowed, but %u found!%s\n", colout->warn, *argv,
				gopt->argc, colout->reset);
		else
			printf ("%s!!! %s: Maximal %u parameter%s allowed, but %u found!%s\n", colout->warn,
				*argv, max_param, max_param == 1 ? "" : "s", gopt->argc, colout->reset);
		stat = ERR_SYNTAX;
		goto abort;
	}
