...
const KeywordTab_t PatchKeysBMG[] = { // 'opt'
									  //  0: no param
									  //  1: file param
									  //  2: string param

	{ BMG_PM_PRINT, "PRINT", 0, 1 }, { BMG_PM_FORMAT, "FORMAT", 0, 2 },
#if ENABLE_BMG_REGEX
	{ BMG_PM_REGEX, "REGEXP", 0, 2 }, { BMG_PM_RM_REGEX, "RM-REGEXP", "RMREGEXP", 2 },
#endif
	{ BMG_PM_ID, "ID", 0, 0 }, { BMG_PM_ID_ALL, "ID-ALL", "IDALL", 0 },
	{ BMG_PM_UNICODE, "UNICODE", 0, 0 }, { BMG_PM_RM_ESCAPES, "RM-ESCAPES", "RMESCAPES", 0 },

	{ BMG_PM_REPLACE, "REPLACE", 0, 1 }, { BMG_PM_INSERT, "INSERT", 0, 1 },
	{ BMG_PM_OVERWRITE, "OVERWRITE", 0, 1 }, { BMG_PM_DELETE, "DELETE", 0, 1 },
	{ BMG_PM_MASK, "MASK", 0, 1 }, { BMG_PM_EQUAL, "EQUAL", 0, 1 },
	{ BMG_PM_NOT_EQUAL, "NOTEQUAL", "NEQUAL", 1 },

	{ BMG_PM_GENERIC, "GENERIC", 0, 0 }, { BMG_PM_RM_CUPS, "RM-CUPS", "RMCUPS", 0 },
...
{
	return opt_bmg_endian == BMG_LITTLE_ENDIAN	   ? &le_func
		: opt_bmg_endian == BMG_BIG_ENDIAN || !bmg ? &be_func
												   : bmg->endian;
}
...
static const KeywordTab_t bmg_color[] = { { 0x0000, "OFF", "NONE", BMG_CNL_BASICS },
	{ 0x0000, "GRAY", "GREY", BMG_CNL_BASICS }, { 0x0001, "RED4", 0, BMG_CNL_BASICS },
	{ 0x0002, "WHITE", 0, BMG_CNL_BASICS },

	{ 0x0008, "TRANSP", "TRANSPARENT", BMG_CNL_BASICS }, { 0x0008, "CLEAR", 0, BMG_CNL_BASICS },

	{ 0x0010, "YOR0", "YR0", BMG_CNL_YOR }, { 0x0011, "YOR1", "YR1", BMG_CNL_YOR },
	{ 0x0012, "YOR2", "YR2", BMG_CNL_YOR }, { 0x0013, "YOR3", "YR3", BMG_CNL_YOR },
	{ 0x0014, "YOR4", "YR4", BMG_CNL_YOR }, { 0x0015, "YOR5", "YR5", BMG_CNL_YOR },
	{ 0x0016, "YOR6", "YR6", BMG_CNL_YOR }, { 0x0017, "YOR7", "YR7", BMG_CNL_YOR },

	{ 0x0020, "RED2", 0, BMG_CNL_BASICS }, { 0x0021, "BLUE1", "BLUE", BMG_CNL_BASICS },

	{ 0x0030, "YELLOW", 0, BMG_CNL_BASICS }, { 0x0031, "BLUE2", 0, BMG_CNL_BASICS },
	{ 0x0032, "RED3", 0, BMG_CNL_BASICS }, { 0x0033, "GREEN", 0, BMG_CNL_BASICS },
...
uint GetLength16BMG (const u16 *source, // source
...
uint PrintString16BMG (char *buf, // destination buffer
...
#undef PRINT_UTF8_MAX
	...
	while (dest < dest_end && src < src_end)
	{
		...
		else if (code > utf8_max || code >= 0xdb80 && code <= 0xf8ff // surrogates & private use
...
uint ScanString16BMG (u16 *buf, // destination buffer
				...
				case 'z':
				{
					...
					if (dest + total_words <= dest_end)
					{
						...
						*dest++ = htons (code1);
						...
						{
							case 3 & 3:
								write_be16 (dest, xcode);
								dest += 1;
								break;
							case 4 & 3:
								write_be32 (dest, xcode);
								dest += 2;
								break;
							case 5 & 3:
								write_be48 (dest, xcode);
								dest += 3;
								break;
							case 6 & 3:
...
static unsigned long int strtoul16 (const u16 *ptr16, u16 **end16, int base)
{
	DASSERT (ptr16);
...
static u16 *ScanRange16U32 (const u16 *arg, // text to scan (valid pointer)
...
bmg_sect_list_t *ScanSectionsBMG (cvp data, uint size, const endian_func_t *endian)
{
	//--- initialize
...
void ResetAttribBMG (const bmg_t *bmg, // pointer to bmg
...
void CopyAttribBMG (const bmg_t *dest, // pointer to destination bmg
...
static void copy_param (bmg_t *dest, // pointer to destination bmg
...
void AssignItemText16BMG (bmg_item_t *bi, // valid item
...
void AssignItemScanTextBMG (bmg_item_t *bi, // valid item
...
void AssignItemTextBMG (bmg_item_t *bi, // valid item
...
bool IsItemEqualBMG (const bmg_item_t *bi1, // NULL or valid item
...
)
{
	...
	return bi1->len == bi2->len && bi1->attrib_used == bi2->attrib_used
...
bmg_item_t *InsertItemBMG (bmg_t *bmg, u32 mid, u8 *attrib, uint attrib_used, bool *old_item)
{
	DASSERT (bmg);
...
{
	static const u8 track_pos[BMG_N_TRACK] = { 8, 1, 2, 4, 0, 5, 6, 7, 9, 15, 11, 3, 14, 10, 12, 13,
		16, 20, 25, 26, 27, 31, 23, 18, 21, 30, 29, 17, 24, 22, 19, 28 };

	return idx < BMG_N_TRACK ? track_pos[idx] : idx < BMG_N_CT_TRACK ? idx : result_on_invalid;
}
...
static int GetArenaIndexBMG0 (uint idx, int result_on_invalid)
{
	...
	return idx < BMG_N_ARENA ? arena_pos[idx] : result_on_invalid;
}
...
const KeywordTab_t TabEndianBMG[] = { { BMG_AUTO_ENDIAN, "AUTO", "UNDEFINED", 0 },
	{ BMG_BIG_ENDIAN, "BIG", "BE", 0 }, { BMG_LITTLE_ENDIAN, "LITTLE", "LE", 0 }, { 0, 0, 0, 0 } };
...
ccp GetEndianNameBMG (int endian, ccp return_if_invalid)
{
	switch (endian)
	{
		case BMG_AUTO_ENDIAN:
			return "AUTO";
		case BMG_BIG_ENDIAN:
			return "BIG";
		case BMG_LITTLE_ENDIAN:
			return "LITTLE";
	}
...
const KeywordTab_t TabEncodingBMG[] = { // if opt==1: not implemented yet
	{ BMG_ENC_CP1252, "CP-1252", "CP1252", 0 }, { BMG_ENC_CP1252, "1", 0, 0 },

	{ BMG_ENC_UTF16BE, "UTF-16BE", "UTF16BE", 0 }, { BMG_ENC_UTF16BE, "2", 0, 0 },

	{ BMG_ENC_SHIFT_JIS, "SHIFT-JIS", "SHIFTJIS", 0 }, { BMG_ENC_SHIFT_JIS, "3", "SJIS", 0 },

	{ BMG_ENC_UTF8, "UTF-8", "UTF8", 0 }, { BMG_ENC_UTF8, "4", 0, 0 },

	{ BMG_ENC__UNDEFINED, "AUTO", "UNDEFINED", 0 }, { 0, 0, 0, 0 }
};
...
{
	return encoding >= BMG_ENC__MIN && encoding <= BMG_ENC__MAX ? encoding : return_if_invalid;
}
...
void AssignInfSizeBMG (bmg_t *bmg, uint inf_size)
{
	...
	{
		ERROR0 (ERR_WARNING, "INF data size is outside allowed range 4..%u => ignored: %s\n",
...
{
	ASSERT (MID_VERSUS_POINTS >= MID_PARAM_BEG && MID_VERSUS_POINTS < MID_PARAM_END);
...
static void scan_raw_cp1252 (bmg_item_t *bi, const u8 *start, const u8 *end, FastBuf_t *fb)
{
	u8 buf[4];
	const u8 *ptr;
...
static void scan_raw_utf16 (
	bmg_item_t *bi, const u8 *start, const u8 *end, const endian_func_t *endian)
{
...
static void scan_raw_shift_jis (bmg_item_t *bi, const u8 *start, const u8 *end, FastBuf_t *fb)
{
	u8 buf[4];
	const u8 *ptr;
...
static void scan_raw_utf8 (bmg_item_t *bi, const u8 *start, const u8 *end, FastBuf_t *fb)
{
	u8 buf[4];
	const u8 *ptr;
	for (ptr = start; ptr < end && *ptr;)
...
enumError ScanRawBMG (bmg_t *bmg)
{
	...
	{
		ERROR0 (ERR_WARNING, "BMG is %u bytes shorter than announced: %s",
	...
	switch (bmg->encoding)
	{
		...
		default:
			if (!opt_bmg_force_count)
				return ERROR0 (ERR_ERROR, "Can't read BMG with encoding #%u (%s): %s\n",
					bmg->encoding, GetEncodingNameBMG (bmg->encoding, "?"), bmg->fname);
			ERROR0 (ERR_WARNING, "Try to read BMG with unsupported encoding #%u (%s): %s\n",
				bmg->encoding, GetEncodingNameBMG (bmg->encoding, "?"), bmg->fname);
	}
	...
	for (si = sl->info; si->sect; si++)
	{
		if (si->real_size < si->sect_size)
			ERROR0 (ERR_WARNING,
				...
				PrintID (si->sect->magic, 4, 0), si->real_size, si->sect_size - si->real_size,
				bmg->fname);
		...
		if (!si->supported)
		{
			...
			DASSERT (raw);
			AssignFastBuf (&raw->data, si->sect->data, si->real_size - sizeof (bmg_section_t));
			raw->total_size = si->real_size;
			PRINT ("SCAN/RAW SECTION »%s« data: %u = 0x%x, total: %u = 0x%x\n",
				PrintID (raw->magic, 4, 0), GetFastBufLen (&raw->data), GetFastBufLen (&raw->data),
	...
	PRINT ("BMG: %u+%u=%u/%u messages inserted => %u | n(unsort)=%d, n(ffff)=%d\n", empty_count,
		ins_count, empty_count + ins_count, max_item, bmg->item_used, unsort_count, ffff_count);
#endif
	ResetFastBuf (&fb_mgr.b);

	bmg->have_predef_slots = pmid && max_item > 1 && (unsort_count > 1 || ffff_count > 1);
	...
	for (;;)
	{
		...
		if (*start == '@')
		{
			...
			*nptr = 0;
			...
			{
				bmg->current_raw = 0;
				uint len = ScanEscapedString (
					namebuf, sizeof (namebuf), start, ptr - start, false, -1, 0);
				if (!len)
					continue;
				if (len > 4)
				{
					ERROR0 (
						ERR_WARNING, "@SECTION name exceeds 4 [%u] characters: %s\n", len, namebuf);
					continue;
				}
				while (len < 4)
					namebuf[len++] = 0;
				PRINT ("@SECTION »%s« %02x %02x %02x %02x\n", PrintID (namebuf, 4, 0), namebuf[0],
					namebuf[1], namebuf[2], namebuf[3]);
		...
		const int mid_stat = ScanMidBMG (bmg, &mid1, &mid2, &mid3, start, end, (char **)&start);
...
enumError ScanBMG (bmg_t *bmg, // pointer to valid bmg
...
enumError LoadBMG (bmg_t *bmg, // pointer to valid bmg
	bool initialize, // true: initialize 'bmg'
...
)
{
	...
	u8 *data;
	uint size;
	enumError err = OpenReadFile (fname, 0, file_mode, 0, 0, &data, &size, &bmg->fname, &bmg->fatt);
	if (err == ERR_OK)
	{
		bmg->data = data;
		bmg->data_size = size;
...
void InitializeCreateBMG (bmg_create_t *bc, bmg_t *bmg)
{
	...
	bc->have_mid = opt_bmg_mid == OFFON_AUTO ? bmg->have_mid : opt_bmg_mid >= OFFON_ON;
...
bmg_item_t *GetNextBI (bmg_create_t *bc)
{
	...
	u32 *pmid = (u32 *)WriteFastBuf (&bc->mid, bc->slot * sizeof (u32), 0, sizeof (u32));
	DASSERT (pmid);
	*pmid = bc->endian->h2nl (bi->mid);

	const uint inf_size = bmg->inf_size;
	bmg_inf_item_t *ii
		= (bmg_inf_item_t *)WriteFastBuf (&bc->inf, bc->slot * inf_size, 0, inf_size);
	DASSERT (ii);
...
enumError CreateRawBMG (bmg_t *bmg // pointer to valid BMG
)
{
	DASSERT (bmg);
	DASSERT (bmg->endian);
	...
	const int encoding
		= bmg->legacy ? BMG_ENC_CP1252 : CheckEncodingBMG (opt_bmg_encoding, bmg->encoding);
	switch (encoding)
	{
		case BMG_ENC_CP1252:
		...
		default:
			return ERROR0 (ERR_ERROR, "Can't create BMG with encoding #%u (%s): %s\n", encoding,
				GetEncodingNameBMG (encoding, "?"), bmg->fname);
	}
	...
	const u32 mid_size = bc.have_mid ? ALIGN32 (sizeof (bmg_mid_t) + mid.len, align) : 0;

	const u32 total_size = sizeof (bmg_header_t) + inf_size + dat_size + mid_size + total_raw_size;
...
enumError SaveRawBMG (bmg_t *bmg, // pointer to valid bmg
...
enumError SaveRawFileBMG (bmg_t *bmg, // pointer to valid bmg
...
char *PrintAttribBMG (char *buf, // destination buffer
...
static void print_message (FILE *f, bmg_t *bmg, const bmg_item_t *bi, ccp mid_name)
{
	DASSERT (f);
	DASSERT (bmg);
	DASSERT (bi);
	...
	if (memcmp (&bmg->active_cond, &bi->cond, sizeof (bmg->active_cond)))
	{
		...
		{
			if (bi->cond.active)
				fprintf (f, "@? %s%x\n", bi->cond.not? "!" : bi->cond.mid ? "" : "=", bi->cond.mid);
			else
				fprintf (f, "@? 0\n");
		}
	}
	...
	const bool print_attrib = !opt_bmg_no_attrib && opt_bmg_single_line < 2 && bi->attrib_used
	...
	if (print_attrib && !opt_bmg_inline_attrib)
	{
		...
		else
		{
			...
			PrintAttribBMG (
				attrib, sizeof (attrib), bi->attrib, bi->attrib_used, bmg->attrib, true);
	...
	if (print_attrib && opt_bmg_inline_attrib)
	{
		...
		dest = PrintAttribBMG (
			dest, attrib + sizeof (attrib) - dest, bi->attrib, bi->attrib_used, bmg->attrib, false);
		*dest++ = ' ';
		*dest = 0;
	...
	{
		fputs (attrib, f);
		const int use_colors = opt_bmg_colors != 1 ? opt_bmg_colors : bmg->use_color_names > 0;
		// [[xbuf?]]
		uint len = PrintString16BMG (
			xbuf, sizeof (xbuf), bi->text, bi->len, BMG_UTF8_MAX, 0, use_colors);
		// [[xbuf?]]
		ccp ptr = xbuf;
		ccp end = ptr + len;
		ccp sep = "=";
...
enumError SaveTextBMG (bmg_t *bmg, // pointer to valid bmg
	ccp fname, // filename of destination
	FileMode_t fmode, // create-file mode
	bool set_time, // true: set time stamps
...
enumError SaveTextFileBMG (bmg_t *bmg, // pointer to valid bmg
	FILE *f, // valid output file
	ccp fname, // NULL or filename for error messages
	bool force_numeric, // true: force numeric MIDs
	uint brief_count // >0: suppress syntax info
...
)
{
	...
	const bool print_settings // print only, if unusual
		= endian->is_le || encoding != BMG_ENC__DEFAULT || bmg->attrib_used != 4
		|| be32 (bmg->attrib) != BMG_INF_STD_ATTRIB || !bmg->have_mid || !bmg->use_color_names
	...
	if (print_settings)
	{
		...
		if (print_full)
		{
			fprintf (f, text_param, param_announce, endian->is_le, bmg->legacy, encoding,
				bmg->have_mid, bmg->inf_size, abuf, bmg->use_color_names, bmg->use_mkw_messages);
			param_announce = "";
		}
		else if (brief_count < BRIEF_NO_MAGIC)
			fprintf (f,
				...
				"@MKW-MESSAGES    = %u\r\n"
				"\r\n",
				endian->is_le, bmg->legacy, encoding, bmg->have_mid, bmg->inf_size, abuf,
		...
		|| bmg->unknown_mid_0a != BMG_MID_DEFAULT_0A || bmg->unknown_mid_0c != BMG_MID_DEFAULT_0C)
	{
		if (print_full)
			...
			fprintf (f,
				...
				"@UNKNOWN-MID32-0C = %#x\r\n"
				"\r\n",
				bmg->unknown_inf_0c, bmg->unknown_mid_0a, bmg->unknown_mid_0c);
	}
	...
	const bool support_mkw = opt_bmg_support_mkw && !force_numeric && !opt_bmg_export
		&& bmg->attrib_used == 4 && be32 (bmg->attrib) == BMG_INF_STD_ATTRIB && bmg->have_mid
	...
	static const struct sep_t sep_std_tab[] = { { 0, 0 } };

	static const struct sep_t sep_mkw_tab[] = { { MID_ENGINE_BEG, "engines" },
		{ MID_ENGINE_END, 0 }, { MID_CHAT_BEG, "online chat" }, { MID_CHAT_END, 0 },

		{ MID_RCUP_BEG, "racing cup names" }, { MID_RCUP_END, 0 },
		{ MID_TRACK1_BEG, "track names, 1. copy" }, { MID_TRACK1_END, 0 },
		{ MID_BCUP_BEG, "battle cup names" }, { MID_BCUP_END, 0 },
		{ MID_TRACK2_BEG, "track names, 2. copy" }, { MID_TRACK2_END, 0 },

		{ MID_ARENA1_BEG, "arena names, 1. copy" }, { MID_ARENA1_END, 0 },
		{ MID_ARENA2_BEG, "arena names, 2. copy" }, { MID_ARENA2_END, 0 },

		{ MID_PARAM_IDENTIFY, "identification for third party tools" }, { MID_PARAM_IDENTIFY, 0 },

		{ MID_PARAM_BEG, "parameters for mkw-ana" }, { MID_PARAM_END, 0 },

		{ MID_CT_TRACK_BEG, "CT-CODE: track names" }, { MID_CT_TRACK_END, 0 },
		{ MID_CT_CUP_BEG, "CT-CODE: cup names" }, { MID_CT_CUP_END, 0 },
		{ MID_CT_CUP_REF_BEG, "CT-CODE: track-to-cup reference" }, { MID_CT_CUP_REF_END, 0 },

		{ MID_X_MESSAGE_BEG, "new messages for CT/LE-CODE" }, { MID_X_MESSAGE_END, 0 },

		{ MID_LE_ALT_ARENA_BEG, "LE-CODE: alternative arena names for random slots" },
		{ MID_LE_ALT_ARENA_END, 0 },

		{ MID_LE_TRACK_BEG, "LE-CODE: track names" }, { MID_LE_TRACK_END, 0 },
		{ MID_LE_CUP_BEG, "LE-CODE: cup names" }, { MID_LE_CUP_END, 0 },
		{ MID_LE_CUP_REF_BEG, "LE-CODE: track-to-cup reference" }, { MID_LE_CUP_REF_END, 0 },
	...
	for (; bi < bi_end; bi++)
	{
		...
		if (mid < MID__VIP_BEG || mid >= MID__VIP_END || !already_printed[mid - MID__VIP_BEG])
		{
	...
	if (bmg->use_raw_sections && bmg->first_raw)
	{
		...
		for (raw = bmg->first_raw; raw; raw = raw->next)
		{
			...
			fprintf (f,
				...
				"@SECTION \"%s\"\r\n\r\n",
				PrintEscapedString (
					name, sizeof (name), raw->magic, sizeof (raw->magic), CHMD_ESC, '"', 0));
...
)
{
	...
	if (keylen == 3)
	{
		if ((*key == 'T' || *key == 't') && key[1] >= '1' && key[1] <= '8' && key[2] >= '1'
			&& key[2] <= '4')
		{
		...
		else if ((*key == 'U' || *key == 'u') && key[1] >= '1' && key[1] <= '2' && key[2] >= '1'
			&& key[2] <= '5')
		{
...
static bool format_message (format_message_t *fm, // valid object
...
bool PatchPrintBMG (bmg_t *bmg_dest, // pointer to destination bmg
...
bool PatchFormatBMG (bmg_t *bmg_dest, // pointer to destination bmg
	const bmg_t *bmg_src, // pointer to source bmg
...
bool PatchRegexBMG (bmg_t *bmg, // pointer to destination bmg
...
)
{
	...
	{
		uint len1
			= PrintString16BMG (buf1, sizeof (buf1), iptr->text, iptr->len, BMG_UTF8_MAX, 0, true);
		// printf("-> [%d] %s\n",len1,buf1);
...
bool PatchRmRegexBMG (bmg_t *bmg, // pointer to destination bmg
	ccp regex // format parameter
)
{
#if ENABLE_BMG_REGEX
...
bool PatchReplaceBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	//--------------------------------------//
	...
	for (; dptr < dend; dptr++)
	{
		...
		if (sptr->mid == mid && !IsItemEqualBMG (dptr, sptr) && CheckItemCondBMG (dest, sptr->cond))
		{
			FreeItemBMG (dptr);
...
bool PatchInsertBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	//--------------------------------------//
...
bool PatchOverwriteBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src, // pointer to source bmg
...
bool PatchDeleteBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	//--------------------------------------//
...
bool PatchMaskBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	//--------------------------------------//
...
bool PatchEqualBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	//--------------------------------------//
...
bool PatchNotEqualBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	//--------------------------------------//
...
static bool generic_helper (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src, // pointer to source bmg
...
)
{
	...
	PRINT ("GENERIC: |%c%c%c| |%c%c%c|\n", be16 (b0 + 0) & 0xff, be16 (b0 + 1) & 0xff,
		be16 (b0 + 2) & 0xff, be16 (b1 + 0) & 0xff, be16 (b1 + 1) & 0xff, be16 (b1 + 2) & 0xff);
	...
	if (b0 < e0)
	{
		...
		DASSERT (dptr);
		if (!dptr->text || dptr->len != len || memcmp (dptr->text, b0, len * sizeof (*b0)))
		{
...
bool PatchGenericBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src // pointer to source bmg
)
{
	DASSERT (dest);
...
bool PatchIdBMG (bmg_t *bmg_dest, // pointer to destination bmg
...
bool PatchRmEscapesBMG (bmg_t *bmg, // pointer to destination bmg
...
bool PatchRmCupsBMG (bmg_t *bmg // pointer to destination bmg
)
{
	DASSERT (bmg);
	bool dirty = false;
...
static uint bmg_copy_helper (bmg_t *bmg, // pointer to bmg
...
bool PatchCTCopyBMG (bmg_t *bmg // pointer to destination bmg
)
{
	uint dirty_count = bmg_copy_helper (bmg, false, MID_TRACK1_BEG, MID_TRACK2_BEG, 0,
						   MID_CT_TRACK_BEG, BMG_N_TRACK)
		+ bmg_copy_helper (
			bmg, false, MID_ARENA1_BEG, MID_ARENA2_BEG, 0, MID_CT_ARENA_BEG, BMG_N_ARENA)
		+ bmg_copy_helper (
			bmg, false, MID_RCUP_BEG, 0, 0, MID_CT_CUP_BEG, MID_RCUP_END - MID_RCUP_BEG)
		+ bmg_copy_helper (
			bmg, false, MID_BCUP_BEG, 0, 0, MID_CT_BCUP_BEG, MID_BCUP_END - MID_BCUP_BEG);
	return dirty_count > 0;
}

//-----------------------------------------------------------------------------

bool PatchLECopyBMG (bmg_t *bmg // pointer to destination bmg
)
{
	uint dirty_count = bmg_copy_helper (bmg, false, MID_ARENA1_BEG, MID_ARENA2_BEG,
						   MID_CT_ARENA_BEG, MID_LE_ARENA_BEG, BMG_N_ARENA)
		+ bmg_copy_helper (bmg, false, MID_TRACK1_BEG, MID_TRACK2_BEG, MID_CT_TRACK_BEG,
			MID_LE_TRACK_BEG, BMG_N_TRACK)
		+ bmg_copy_helper (bmg, false, MID_RCUP_BEG, MID_CT_CUP_BEG, 0, MID_LE_CUP_BEG,
			MID_RCUP_END - MID_RCUP_BEG)
		+ bmg_copy_helper (bmg, false, MID_BCUP_BEG, MID_CT_BCUP_BEG, 0, MID_LE_BCUP_BEG,
...
bool PatchXCopyBMG (bmg_t *bmg // pointer to destination bmg
)
{
	if (opt_bmg_support_lecode)
...
bool PatchCTForceCopyBMG (bmg_t *bmg // pointer to destination bmg
)
{
	uint dirty_count = bmg_copy_helper (bmg, true, MID_TRACK1_BEG, MID_TRACK2_BEG, 0,
						   MID_CT_TRACK_BEG, BMG_N_TRACK)
		+ bmg_copy_helper (
			bmg, true, MID_ARENA1_BEG, MID_ARENA2_BEG, 0, MID_CT_ARENA_BEG, BMG_N_ARENA)
		+ bmg_copy_helper (
			bmg, true, MID_RCUP_BEG, 0, 0, MID_CT_CUP_BEG, MID_RCUP_END - MID_RCUP_BEG)
		+ bmg_copy_helper (
			bmg, true, MID_BCUP_BEG, 0, 0, MID_CT_BCUP_BEG, MID_BCUP_END - MID_BCUP_BEG);
	return dirty_count > 0;
}

//-----------------------------------------------------------------------------

bool PatchLEForceCopyBMG (bmg_t *bmg // pointer to destination bmg
)
{
	uint dirty_count = bmg_copy_helper (bmg, true, MID_TRACK1_BEG, MID_TRACK2_BEG, MID_CT_TRACK_BEG,
						   MID_LE_TRACK_BEG, BMG_N_TRACK)
		+ bmg_copy_helper (bmg, true, MID_ARENA1_BEG, MID_ARENA2_BEG, MID_CT_ARENA_BEG,
			MID_LE_ARENA_BEG, BMG_N_ARENA)
		+ bmg_copy_helper (
			bmg, true, MID_RCUP_BEG, MID_CT_CUP_BEG, 0, MID_LE_CUP_BEG, MID_RCUP_END - MID_RCUP_BEG)
		+ bmg_copy_helper (bmg, true, MID_BCUP_BEG, MID_CT_BCUP_BEG, 0, MID_LE_BCUP_BEG,
			MID_BCUP_END - MID_BCUP_BEG);
	return dirty_count > 0;
}
...
bool PatchXForceCopyBMG (bmg_t *bmg // pointer to destination bmg
)
{
	if (opt_bmg_support_lecode)
...
bool PatchFillBMG (bmg_t *bmg, ct_mode_t ct_mode, uint rcup_limit, uint track_limit)
{
	ct_bmg_t ctb;
	...
	for (; dptr < dend; dptr++)
	{
		...
		if (m >= MID_TRACK1_BEG && m < MID_TRACK1_END || m >= MID_TRACK2_BEG && m < MID_TRACK2_END
			|| m >= MID_ARENA1_BEG && m < MID_ARENA1_END
			|| m >= MID_ARENA2_BEG && m < MID_ARENA2_END || m >= MID_RCUP_BEG && m < MID_RCUP_END
			...
			|| m >= ctb.bcup_name.beg && m < ctb.bcup_name.end || m == ctb.random.beg)
		{
	...
	for (mid = ctb.arena_name1.beg; mid < ctb.arena_name1.end; mid++)
	{
		...
		if (!old_item || !dptr->len)
		{
			const uint len = snprintf (buf, sizeof (buf), "_A%02u_", mid - ctb.arena_name1.beg + 1);
	...
	for (mid = ctb.track_name1.beg; mid < max_track; mid++)
	{
		...
		if (!old_item || !dptr->len)
		{
			const uint len = snprintf (buf, sizeof (buf), "_T%03X_", mid - ctb.track_name1.beg);
			AssignItemTextBMG (dptr, buf, len);
			ResetAttribBMG (bmg, dptr);
			dirty = true;
		}
	...
	for (mid = ctb.bcup_name.beg; mid < ctb.bcup_name.end; mid++)
	{
		...
		if (!old_item || !dptr->len)
		{
			const uint len = snprintf (buf, sizeof (buf), "_B%u_", mid - ctb.bcup_name.beg);
			AssignItemTextBMG (dptr, buf, len);
			ResetAttribBMG (bmg, dptr);
			dirty = true;
		}
	...
	for (mid = ctb.rcup_name.beg; mid < max_cup; mid++)
	{
		...
		if (!old_item || !dptr->len)
		{
			const uint len = snprintf (buf, sizeof (buf), "_R%u_", mid - ctb.rcup_name.beg);
			AssignItemTextBMG (dptr, buf, len);
			ResetAttribBMG (bmg, dptr);
			dirty = true;
		}
...
bool PatchRmFilledBMG (bmg_t *bmg)
{
	...
	for (; iptr < iend; iptr++)
	{
		if (iptr->mid >= 0x4000 && iptr->len > 3 && ntohs (iptr->text[0]) == '_'
...
enumError PatchBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src, // pointer to source bmg
...
)
{
	...
	{
		case BMG_PM_PRINT:
			dirty = PatchPrintBMG (dest, src);
			break;
		case BMG_PM_FORMAT:
			dirty = PatchFormatBMG (dest, src, string);
			break;
		case BMG_PM_REGEX:
			dirty = PatchRegexBMG (dest, string);
			break;
		case BMG_PM_RM_REGEX:
			dirty = PatchRmRegexBMG (dest, string);
			break;
		case BMG_PM_ID:
			dirty = PatchIdBMG (dest, false);
			break;
		case BMG_PM_ID_ALL:
			dirty = PatchIdBMG (dest, true);
			break;
		case BMG_PM_UNICODE:
			dirty = PatchRmEscapesBMG (dest, true, false);
			break;
		case BMG_PM_RM_ESCAPES:
			dirty = PatchRmEscapesBMG (dest, false, true);
			break;

		case BMG_PM_REPLACE:
			dirty = PatchReplaceBMG (dest, src);
			break;
		case BMG_PM_INSERT:
			dirty = PatchInsertBMG (dest, src);
			break;
		case BMG_PM_OVERWRITE:
			dirty = PatchOverwriteBMG (dest, src, dup_strings);
			break;
		case BMG_PM_DELETE:
			dirty = PatchDeleteBMG (dest, src);
			break;
		case BMG_PM_MASK:
			dirty = PatchMaskBMG (dest, src);
			break;
		case BMG_PM_EQUAL:
			dirty = PatchEqualBMG (dest, src);
			break;
		case BMG_PM_NOT_EQUAL:
			dirty = PatchNotEqualBMG (dest, src);
			break;

		case BMG_PM_GENERIC:
			dirty = PatchGenericBMG (dest, dest);
			break;
		case BMG_PM_RM_CUPS:
			dirty = PatchRmCupsBMG (dest);
			break;

		case BMG_PM_CT_COPY:
			dirty = PatchCTCopyBMG (dest);
			break;
		case BMG_PM_CT_FORCE_COPY:
			dirty = PatchCTForceCopyBMG (dest);
			break;
		case BMG_PM_CT_FILL:
			dirty = PatchCTFillBMG (dest, 0, 0);
			break;

		case BMG_PM_LE_COPY:
			dirty = PatchLECopyBMG (dest);
			break;
		case BMG_PM_LE_FORCE_COPY:
			dirty = PatchLEForceCopyBMG (dest);
			break;
		case BMG_PM_LE_FILL:
			dirty = PatchLEFillBMG (dest, 0, 0);
			break;

		case BMG_PM_X_COPY:
			dirty = PatchXCopyBMG (dest);
			break;
		case BMG_PM_X_FORCE_COPY:
			dirty = PatchXForceCopyBMG (dest);
			break;
		case BMG_PM_X_FILL:
			dirty = PatchXFillBMG (dest, 0, 0);
			break;

		case BMG_PM_RM_FILLED:
			dirty = PatchRmFilledBMG (dest);
			break;

		default:
			return ERROR0 (ERR_NOT_IMPLEMENTED, 0);
	}
...
ccp GetCtModeNameBMG (ct_mode_t mode, bool full)
{
	switch (mode)
	{
		...
		case CTM_CTCODE:
			return "CT-CODE";
		case CTM_LECODE1:
			return full ? "LE-CODE v1" : "LE-CODE";
		case CTM_LECODE2:
			return full ? "LE-CODE v2" : "LE-CODE";
		default:
			return "?";
	}
}
...
ccp GetCtBMGIdentification (ct_bmg_t *ctb, bool full)
{
	...
	if (full)
		len = snprintf (buf, sizeof (buf),
			...
			ctb->ct_mode, name, ctb->version, ctb->is_ct_code == 1, ctb->le_phase, TAB (param),
			TAB (rcup_name), TAB (bcup_name), TAB (track_name1), TAB (track_name2),
			TAB (arena_name1), TAB (arena_name2), TAB (cup_ref));
	else
...
const sizeof_info_t sizeof_info_bmg[] = { SIZEOF_INFO_TITLE ("BMG") SIZEOF_INFO_ENTRY (bmg_range_t)
		SIZEOF_INFO_ENTRY (bmg_section_t) SIZEOF_INFO_ENTRY (bmg_header_t) SIZEOF_INFO_ENTRY (
			bmg_inf_item_t) SIZEOF_INFO_ENTRY (bmg_inf_t) SIZEOF_INFO_ENTRY (bmg_dat_t)
			SIZEOF_INFO_ENTRY (bmg_mid_t) SIZEOF_INFO_ENTRY (bmg_flw_t)
				SIZEOF_INFO_ENTRY (bmg_fli_t) SIZEOF_INFO_ENTRY (bmg_sect_info_t)
					SIZEOF_INFO_ENTRY (bmg_sect_list_t) SIZEOF_INFO_ENTRY (bmg_raw_section_t)
						SIZEOF_INFO_ENTRY (bmg_item_cond_t) SIZEOF_INFO_ENTRY (bmg_item_t)
							SIZEOF_INFO_ENTRY (bmg_t) SIZEOF_INFO_ENTRY (bmg_create_t)
								SIZEOF_INFO_ENTRY (bmg_scan_mid_t) SIZEOF_INFO_ENTRY (ct_bmg_t)
