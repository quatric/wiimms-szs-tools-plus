...
static RegexReplace_t *GetNextRegexRepl (RegexElem_t *elem, char *ptr, int len, int ref)
{
	DASSERT (elem);
	if (!len && ref < 0)
		return 0;
		...
		if (*regex == sep)
		{
			...
			while (*regex && *regex != ';')
				switch (*regex++)
				{
					...
					case 'i':
						elem->icase = true;
						elem->opt |= REOPT_ICASE;
						break;
				}
		}

		PRINT ("RE: PATTERN #%u/%u: [%s] %s -> %.*s : %s\n", re->re_used, re->re_size,
			GetFastBufStatus (&temp.b), GetFastBufString (&temp.b), repl_len, repl, regex);
	...
	for (e = 0, elem = re->re_list; e < re->re_used; e++, elem++)
	{
		...
		while (pos < src_len)
		{
			...
			PRINT ("RE: pos=%2d, stat=%2d : %2d %2d : %2d %2d : %2d %2d\n", pos, stat,
				match[0].rm_so, match[0].rm_eo, match[1].rm_so, match[1].rm_eo, match[2].rm_so,
				match[2].rm_eo);
			if (stat)
			{
				if (stat == REG_NOMATCH)
...
#error
				...
				if (repl->ref >= 0 && repl->ref < N_MATCH)
				{
					...
					if (m->rm_so >= 0)
						AppendFastBuf (dest, src + m->rm_so + pos, m->rm_eo - m->rm_so);
				}
#endif
			}
