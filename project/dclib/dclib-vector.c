...
void InitializeF3L (float3List_t *f3l, // valid data
	...
	if (!fast)
	{
		if (f3l->sort)
		{
			...
			while (beg <= end)
			{
				...
				DASSERT_MSG (f3l->sort[idx] < f3l->used, "idx=%d, sort-idx=%d, used=%u, size=%u\n",
		...
		if (f3l->used < 10)
		{
			...
			for (i = 0; i < f3l->used; i++)
			{
				...
				PRINT ("INS %2u: %2u -> %08x %08x %08x %11.5f %11.5f %11.5f\n", i, f3l->sort[i],
					be32 (&f->x), be32 (&f->y), be32 (&f->z), f->x, f->y, f->z);
			}
			PRINT ("---\n");
		}
#endif
...
bool IsEqualD3 (const double3 *a, const double3 *b, double null_epsilon, double diff_epsilon)
{
	DASSERT (a);
	DASSERT (b);
...
{
	return IsSameF (a[0], b[0], bit_diff) && IsSameF (a[1], b[1], bit_diff)
		&& IsSameF (a[2], b[2], bit_diff);
}

bool IsSameD3 (const double *a, const double *b, int bit_diff)
{
	return IsSameD (a[0], b[0], bit_diff) && IsSameD (a[1], b[1], bit_diff)
...
void SetupMatrixF34 (float34 *mat, // data to setup
...
void CalcInverseMatrixF34 (float34 *i, // store inverse matrix here
...
)
{
	...
	const double det = t->v[0] * t->v[4 + 1] * t->v[8 + 2] + t->v[1] * t->v[4 + 2] * t->v[8 + 0]
		+ t->v[2] * t->v[4 + 0] * t->v[8 + 1] - t->v[2] * t->v[4 + 1] * t->v[8 + 0]
		- t->v[1] * t->v[4 + 0] * t->v[8 + 2] - t->v[0] * t->v[4 + 2] * t->v[8 + 1];
	...
	i->v[3] = -t->v[3] * i->v[0] - t->v[4 + 3] * i->v[1] - t->v[8 + 3] * i->v[2];

	i->v[4 + 3] = -t->v[3] * i->v[4 + 0] - t->v[4 + 3] * i->v[4 + 1] - t->v[8 + 3] * i->v[4 + 2];

	i->v[8 + 3] = -t->v[3] * i->v[8 + 0] - t->v[4 + 3] * i->v[8 + 1] - t->v[8 + 3] * i->v[8 + 2];
}

///////////////////////////////////////////////////////////////////////////////

void MultiplyF34 (float34 *dest, // valid, store result here, may be 'src1' or 'src2'
...
void SetupMatrixD34 (double34 *mat, // data to setup
...
void CalcInverseMatrixD34 (double34 *i, // store inverse matrix here
...
void MultiplyD34 (double34 *dest, // valid, store result here, may be 'src1' or 'src2'
...
void CopyF34toD34 (double34 *dest, // destination matrix
...
void CopyD34toF34 (float34 *dest, // destination matrix
...
void CopyMatrixD (MatrixD_t *dest, // valid destination
	const MatrixD_t *src // NULL or source
)
{
	DASSERT (dest);
...
MatrixD_t *SetScaleMatrixD (MatrixD_t *mat, // valid data structure
...
MatrixD_t *SetShiftMatrixD (MatrixD_t *mat, // valid data structure
...
MatrixD_t *SetScaleShiftMatrixD (MatrixD_t *mat, // valid data structure
...
MatrixD_t *SetRotateMatrixD (MatrixD_t *mat, // valid data structure
	uint xyz, // 0..2: x/y/z coordinate to set
...
MatrixD_t *SetTranslateMatrixD (MatrixD_t *mat, // valid data structure
...
MatrixD_t *CheckStatusMatrixD (MatrixD_t *mat // valid data structure
)
{
	DASSERT (mat);
	...
	if (mat->tmatrix_valid)
	{
		...
		if (mat->rotate_enabled & 1 && fabs (t[4 + 2]) < NULL_EPSILON
		...
		if (mat->rotate_enabled & 2 && fabs (t[2]) < NULL_EPSILON && fabs (t[8 + 0]) < NULL_EPSILON)
		{
		...
		if (mat->rotate_enabled & 4 && fabs (t[1]) < NULL_EPSILON && fabs (t[4 + 0]) < NULL_EPSILON)
		{
		...
		if (mat->scale_enabled & 1 && !(mat->rotate_enabled & ~1)
		...
		if (mat->scale_enabled & 2 && !(mat->rotate_enabled & ~2)
		...
		if (mat->scale_enabled & 4 && !(mat->rotate_enabled & ~4)
		...
		mat->transform_enabled = mat->scale_enabled | mat->rotate_enabled | mat->translate_enabled
			| mat->use_matrix << 3;
	}
...
MatrixD_t *SetMatrixD (MatrixD_t *mat, // valid data structure
	const double34 *src // NULL or matrix to set
)
{
	DASSERT (mat);
	InitializeMatrixD (mat);
...
MatrixD_t *CalcNormMatrixD (MatrixD_t *mat // valid data structure
)
{
	DASSERT (mat);
	if (!mat->valid)
	...
	for (xyz = 0; xyz < 3; xyz++)
	{
		...
		trans.v[xyz] = mat->shift.v[xyz] + (1.0 - scale) * mat->scale_origin.v[xyz];
	}
	...
	{
		double deg
			= fmod (mat->rotate_deg.v[xyz] + mat->rotate_rad.v[xyz] * (180.0 / M_PI) + 180.0, 360.0)
			- 180.0;
	...
	if ((mat->rotate_enabled | 1) == 7)
		mat->norm_pos2d.x /= 2;
	if ((mat->rotate_enabled | 2) == 7)
		mat->norm_pos2d.y /= 2;
	if ((mat->rotate_enabled | 4) == 7)
		mat->norm_pos2d.z /= 2;
	...
	mat->transform_enabled
		= mat->scale_enabled | mat->rotate_enabled | mat->translate_enabled | mat->use_matrix << 3;

	TRACE ("%%%%%%%%%% CalcNormMatrixD(%p)    => %u|%u|%u = %02x %%%%%%%%%%\n", mat,
		mat->scale_enabled, mat->rotate_enabled, mat->translate_enabled, mat->transform_enabled);
...
MatrixD_t *CalcTransMatrixD (MatrixD_t *mat, // valid data structure
...
)
{
	...
	SetupMatrixD34 (
		&mat->trans_matrix, &mat->norm_scale, &mat->norm_rotate_rad, &mat->norm_translate);
...
MatrixD_t *CalcMatrixD (MatrixD_t *mat, // valid data structure
...
MatrixD_t *MultiplyMatrixD (MatrixD_t *dest, // valid, store result here, may be 'src1' or 'src2'
...
)
{
	...
	InitializeMatrixD (dest);
	MultiplyD34 (&dest->trans_matrix, inverse_mode & 1 ? &src1->inv_matrix : &src1->trans_matrix,
...
MatrixD_t *CalcVectorsMatrixD (MatrixD_t *mat, // valid data structure
...
double3 TransformD3MatrixD (MatrixD_t *mat, // valid data structure
	const double3 *val // value to transform
)
{
	DASSERT (mat);
	DASSERT (val);
...
double3 InvTransformD3MatrixD (MatrixD_t *mat, // valid data structure
	const double3 *val // value to transform
)
{
	DASSERT (mat);
...
void TransformD3NMatrixD (MatrixD_t *mat, // valid data structure
...
void InvTransformD3NMatrixD (MatrixD_t *mat, // valid data structure
	double3 *val, // value array to transform
	int n, // number of 3D vectors in 'val'
	uint off // if n>1: offset from one to next 'val' vector
)
...
float3 TransformF3MatrixD (MatrixD_t *mat, // valid data structure
	const float3 *val // value to transform
)
{
	DASSERT (mat);
	DASSERT (val);
...
float3 InvTransformF3MatrixD (MatrixD_t *mat, // valid data structure
	const float3 *val // value to transform
)
{
	DASSERT (mat);
...
void TransformF3NMatrixD (MatrixD_t *mat, // valid data structure
...
void InvTransformF3NMatrixD (MatrixD_t *mat, // valid data structure
	float3 *val, // value array to transform
	int n, // number of 3D vectors in 'val'
	uint off // if n>1: offset from one to next 'val' vector
)
...
)
{
	...
	return TransformHelperD3MatrixD (&mat->scale, &mat->scale_origin, &mat->shift, &rot,
		mat->rotate_origin + 0, mat->rotate_origin + 1, mat->rotate_origin + 2, &mat->translate,
		val);
}
...
)
{
	...
	return TransformHelperD3MatrixD (
		&mat->norm_scale, 0, 0, &mat->norm_rotate_deg, 0, 0, 0, &mat->norm_translate, val);
}
...
static ccp print_matrix_stat (MatrixD_t *mat, char *buf, uint bufsize, bool for_inverse)
{
	DASSERT (buf);
	DASSERT (bufsize > 10);
...
void PrintMatrixD (FILE *f, // file to print to
...
)
{
	...
	if (print & 1)
	{
		...
		if (print_all || !IsOne3 (mat->scale))
			fprintf (f, "%*s  Scale:      %11.3f %11.3f %11.3f%s", indent, "", mat->scale.x,
				mat->scale.y, mat->scale.z, eol);

		if (print_all || !IsNull3 (mat->scale_origin))
			fprintf (f, "%*s    Origin:   %11.3f %11.3f %11.3f%s", indent, "", mat->scale_origin.x,
				mat->scale_origin.y, mat->scale_origin.z, eol);

		if (print_all || !IsNull3 (mat->shift))
			fprintf (f, "%*s  Shift:      %11.3f %11.3f %11.3f%s", indent, "", mat->shift.x,
				mat->shift.y, mat->shift.z, eol);

		bool print_rot = false;
		if (print_all || !IsNull3 (mat->rotate_deg))
		{
			print_rot = true;
			fprintf (f, "%*s  Rotate/deg: %11.3f %11.3f %11.3f%s", indent, "", mat->rotate_deg.x,
				mat->rotate_deg.y, mat->rotate_deg.z, eol);
		}

		if (print_all || !IsNull3 (mat->rotate_rad))
		{
			print_rot = true;
			fprintf (f, "%*s  Rotate/rad: %11.3f %11.3f %11.3f%s", indent, "", mat->rotate_rad.x,
				mat->rotate_rad.y, mat->rotate_rad.z, eol);
		}
		...
		if (print_rot)
		{
			...
			for (xyz = 0; xyz < 3; xyz++, d++)
				if (print_all || !IsNull3 (*d))
					fprintf (f, "%*s    %c-origin: %11.3f %11.3f %11.3f%s", indent, "", 'x' + xyz,
						d->x, d->y, d->z, eol);
		}

		if (print_all || !IsNull3 (mat->translate))
			fprintf (f, "%*s  Translate:  %11.3f %11.3f %11.3f%s", indent, "", mat->translate.x,
				mat->translate.y, mat->translate.z, eol);
	...
	{
		if (print_head)
			fprintf (f, "%*sNormalized Vectors (seq=%u):%s", indent, "", mat->sequence_number, eol);
		...
		fprintf (f,
			...
			indent, "", mat->norm_scale.x, mat->norm_scale.y, mat->norm_scale.z,
			mat->scale_enabled & 1 ? 'x' : '-', mat->scale_enabled & 2 ? 'y' : '-',
			mat->scale_enabled & 4 ? 'z' : '-', eol,

			indent, "", mat->norm_rotate_deg.x, mat->norm_rotate_deg.y, mat->norm_rotate_deg.z,
			mat->rotate_enabled & 1 ? 'x' : '-', mat->rotate_enabled & 2 ? 'y' : '-',
			mat->rotate_enabled & 4 ? 'z' : '-', eol,
#if TEST0
			indent, "", mat->norm_rotate_rad.x, mat->norm_rotate_rad.y, mat->norm_rotate_rad.z, eol,
#endif
			indent, "", mat->norm_translate.x, mat->norm_translate.y, mat->norm_translate.z,
			mat->translate_enabled & 1 ? 'x' : '-', mat->translate_enabled & 2 ? 'y' : '-',
			mat->translate_enabled & 4 ? 'z' : '-', eol);

		if (print_all || !IsNull3 (mat->norm_pos2d))
			fprintf (f, "%*s  2D tf-base: %11.3f %11.3f %11.3f       (for 2D transform)%s", indent,
				"", mat->norm_pos2d.x, mat->norm_pos2d.y, mat->norm_pos2d.z, eol);
	...
	{
		if (print_head)
			fprintf (f, "%*sTransformation Matrix (%s):%s", indent, "",
				print_matrix_stat (mat, buf, sizeof (buf), false), eol);
		...
		fprintf (f,
			...
			indent, "", mat->trans_matrix.x[0], mat->trans_matrix.x[1], mat->trans_matrix.x[2],
			mat->trans_matrix.x[3], eol,

			indent, "", mat->trans_matrix.y[0], mat->trans_matrix.y[1], mat->trans_matrix.y[2],
			mat->trans_matrix.y[3], eol,

			indent, "", mat->trans_matrix.z[0], mat->trans_matrix.z[1], mat->trans_matrix.z[2],
			mat->trans_matrix.z[3], eol, sep);
	}
	...
	{
		if (print_head)
			fprintf (f, "%*sInverse Matrix (%s):%s", indent, "",
				print_matrix_stat (mat, buf, sizeof (buf), true), eol);
		...
		fprintf (f,
			...
			indent, "", mat->inv_matrix.x[0], mat->inv_matrix.x[1], mat->inv_matrix.x[2],
			mat->inv_matrix.x[3], eol,

			indent, "", mat->inv_matrix.y[0], mat->inv_matrix.y[1], mat->inv_matrix.y[2],
			mat->inv_matrix.y[3], eol,

			indent, "", mat->inv_matrix.z[0], mat->inv_matrix.z[1], mat->inv_matrix.z[2],
			mat->inv_matrix.z[3], eol, sep);
	}
}
