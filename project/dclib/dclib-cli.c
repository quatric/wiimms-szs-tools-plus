...
enumError Command_COLORS (int level, // only used, if select==0
...
)
{
	...
	if (level < 0)
	{
		...
		printf ("%s--color=%d [%s], colorize=%d [%s]\n"
			...
			colout->status, opt_colorize, GetColorModeName (opt_colorize, "?"), colorize_stdout,
			GetColorModeName (colorize_stdout, "?"), term, isatty (fileno (stdout)),
			colout->col_mode, GetColorModeName (colout->col_mode, "?"), colout->colorize,
			colout->n_colors, colout->reset);
		return ERR_OK;
	}
	...
	if (!select && level > 0)
		switch (level)
		{
			case 1:
				...
				break;
			case 2:
				select = COLSEL_NAME | COLSEL_F_ALT;
				break;
			case 3:
				select = COLSEL_M_NONAME;
				break;
			default:
				select = COLSEL_M_ALL;
				break;
