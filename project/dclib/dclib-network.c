...
{
	return ResolveHostMem (
		host, init_host, MemByString (name), default_port, check_filename, store_name);
}
...
)
{
	...
	if (ptr < end && (*ptr >= 'a' && *ptr <= 'z' || *ptr >= 'A' && *ptr <= 'Z'))
	{
		while (ptr < end
	...
	while (ptr < end
		&& (*ptr >= 'a' && *ptr <= 'z' || *ptr >= 'A' && *ptr <= 'Z' || *ptr >= '0' && *ptr <= '9'
			|| *ptr == '_' || *ptr == '-' || *ptr == '.'))
	{
		ptr++;
	}
	...
	if (len > 0 && len < sizeof (buf))
	{
		...
		if (ptr < end && *ptr == ':')
		{
			...
			else
			{
				ccp pstart = pptr;
				while (pptr < end
					&& (*pptr >= 'a' && *pptr <= 'z' || *pptr >= 'A' && *pptr <= 'Z'
						|| *pptr >= '0' && *pptr <= '9' || *pptr == '_' || *pptr == '-'))
				{
					pptr++;
				}
				len = pptr - pstart;
...
ccp PrintPrintModeIP (PrintModeIP_t val, bool align)
{
	...
	*dest++ = "nria"[val & PMIP_M_BITS];
	...
	if (val & PMIP_MASK)
		...
		*dest++ = '-';
	if (val & PMIP_BRACKETS)
		*dest++ = 'b';
	else if (align)
		*dest++ = '-';
	if (val & PMIP_FULL_IPV6)
		*dest++ = 'f';
	else if (align)
		*dest++ = '-';
...
ccp GetIPClassColor (const ColorSet_t *colset, IPClass_t ipc)
{
	...
	switch (ipc)
	{
		case IPCL_INVALID:
			return colset->b_red;
		case IPCL_LOOPBACK:
			return colset->b_blue;
		case IPCL_LINK_LOCAL:
			return colset->b_cyan;
		case IPCL_LOCAL:
			return colset->b_green_cyan;
		case IPCL_UNIQUE_LOCAL:
			return colset->b_green;
		case IPCL_SPECIAL:
			return colset->b_yellow;
		case IPCL_STANDARD:
			return colset->reset;
		default:
			return colset->b_magenta;
	}
}
...
bool SplitIP (SplitIP_t *sip, ccp addr)
{
	...
	{
		char *end_name = slash ? StringCopySM (sip->name, sizeof (sip->name), addr, slash - addr)
...
static const struct ip_names_list_t ip_names_list[] = { RECORD (binaddr) RECORD (ipvers)
		RECORD (addr1) RECORD (addr2) RECORD (bits) RECORD (mask1) RECORD (mask2) { -1, "" } };
...
{
	return nip ? PrintCircBuf ("ba=%s, iv=%s, a1=%s, a2=%s, b=%s, m1=%s, m2=%s\n", nip->binaddr,
					 nip->ipvers, nip->addr1, nip->addr2, nip->bits, nip->mask1, nip->mask2)
			   : 0;
}
...
IPClass_t GetIPClassBIP (const BinIP_t *bip)
{
	...
	if (bip)
	{
		...
		else if (bip->ipvers == 6)
		{
			...
			for (struct ip6_tab *ptr = ip6_tab; ptr->name; ptr++)
			{
				PRINT0 ("  %016llx.%016llx / %016llx.%016llx : %s\n", ntoh64 (ptr->addr.v64[0]),
					ntoh64 (ptr->addr.v64[1]), ntoh64 (ptr->mask.v64[0]), ntoh64 (ptr->mask.v64[1]),
					ptr->name);
...
ccp PrintBIP (const BinIP_t *bip)
{
	...
	return bip->bits < bitlen ? PrintCircBuf ("%s/%u", res, bip->bits)
	...
	if (a1 && a2 && a1->ipvers == a2->ipvers)
	{
		...
		switch (use_mask & 3)
		{
			...
			case 2:
				...
				break;
			case 3:
				bits = a1->bits < a2->bits ? a1->bits : a2->bits;
				break;
			default:
				bits = 128;
				break;
...
ManageIP_t GetAddrInfoMIP (ccp addr, int protocol, IpMode_t ipm)
{
	...
	if (SplitIP (&sip, addr))
	{
		struct addrinfo hints
			= { .ai_family = AF_UNSPEC, .ai_protocol = protocol < 0 ? IPPROTO_TCP : protocol };
	...
	switch (mip->bin.ipvers)
	{
		case 4:
			...
			if (pmode & PMIP_0DOTS)
				...
				snprintf (buf, sizeof (buf), "%u.%u", mip->bin.ip4x.v8[0],
					ntohl (mip->bin.ip4) & 0xffffff);
			else if (pmode & PMIP_2DOTS)
				snprintf (buf, sizeof (buf), "%u.%u.%u", mip->bin.ip4x.v8[0], mip->bin.ip4x.v8[1],
					ntohs (mip->bin.ip4x.v16[1]));
			else
				snprintf (buf, sizeof (buf), "%u.%u.%u.%u", mip->bin.ip4x.v8[0],
					mip->bin.ip4x.v8[1], mip->bin.ip4x.v8[2], mip->bin.ip4x.v8[3]);
			break;
		...
		case 6:
			if (pmode & PMIP_FULL_IPV6)
			{
				snprintf (buf, sizeof (buf), "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
					ntohs (mip->bin.ip6.v16[0]), ntohs (mip->bin.ip6.v16[1]),
					ntohs (mip->bin.ip6.v16[2]), ntohs (mip->bin.ip6.v16[3]),
					ntohs (mip->bin.ip6.v16[4]), ntohs (mip->bin.ip6.v16[5]),
					ntohs (mip->bin.ip6.v16[6]), ntohs (mip->bin.ip6.v16[7]));
				res = buf;
			}
			else
	...
	*port = 0;
	if (mip->port && pmode & (PMIP_SERVICE | PMIP_PORT))
	{
		if (pmode & PMIP_SERVICE)
		{
			...
			char buf[1000];
			const int stat
				= getservbyport_r (htons (mip->port), 0, &result_buf, buf, sizeof (buf), &result);
			if (!stat && result)
	...
	if (PrintBitsMIP (mip, pmode))
	{
		if (mip->bin.ipvers == 4 && pmode & PMIP_MASK)
		{
			...
			snprintf (bits, sizeof (bits), "/%d.%d.%d.%d", mask.v8[0], mask.v8[1], mask.v8[2],
				mask.v8[3]);
		}
		else
...
ccp GetSqlSetAddrMIP (const ManageIP_t *mip, const NamesIP_t *names)
{
	...
	if (mip->bin.ipvers == 4)
	{
		...
		return PrintCircBuf (
			"%s=4,%s=0,%s=0x%08x", nip.ipvers, nip.addr1, nip.addr2, ntohl (mip->bin.ip4 & mask));
	}
	else if (mip->bin.ipvers == 6)
	{
		const ipv6_t mask = GetIP6Mask (mip->bin.bits);
		return PrintCircBuf ("%s=6,%s=0x%llx,%s=0x%llx", nip.ipvers, nip.addr1,
			ntoh64 (mip->bin.ip6_64[0] & mask.v64[0]), nip.addr2,
			ntoh64 (mip->bin.ip6_64[1] & mask.v64[1]));
	}
	return 0;
}
...
ccp GetSqlSetMaskMIP (const ManageIP_t *mip, const NamesIP_t *names)
{
	...
	{
		const ipv4_t mask = GetIP4Mask (mip->bin.bits);
		return PrintCircBuf ("%s=4,%s=0,%s=0x%08x,%s=0,%s=0x%08x", nip.ipvers, nip.addr1, nip.addr2,
			ntohl (mip->bin.ip4 & mask), nip.mask1, nip.mask2, ntohl (mask));
	}
	else if (mip->bin.ipvers == 6)
	{
		const ipv6_t mask = GetIP6Mask (mip->bin.bits);
		return PrintCircBuf ("%s=6,%s=0x%llx,%s=0x%llx,%s=0x%llx,%s=0x%llx", nip.ipvers, nip.addr1,
			ntoh64 (mip->bin.ip6_64[0] & mask.v64[0]), nip.addr2,
			ntoh64 (mip->bin.ip6_64[1] & mask.v64[1]), nip.mask1, ntoh64 (mask.v64[0]), nip.mask2,
			ntoh64 (mask.v64[1]));
	}
	return 0;
}
...
ccp GetSqlSetBinMIP (const ManageIP_t *mip, const NamesIP_t *names)
{
	...
	return PrintCircBuf ("%s=unhex('%02x%02x%016llx%016llx')", nip.binaddr, mip->bin.ipvers,
		mip->bin.bits, ntoh64 (mip->bin.ip6_64[0]), ntoh64 (mip->bin.ip6_64[0]));
}
...
ccp GetSqlCondAddrMIP (const ManageIP_t *mip, const NamesIP_t *names)
{
	...
	if (mip->bin.ipvers == 4)
		return PrintCircBuf ("%s=4 && %s=0x%08x", nip.ipvers, nip.addr2, ntohl (mip->bin.ip4));
	else if (mip->bin.ipvers == 6)
		return PrintCircBuf ("%s=6 && %s=0x%llx && %s=0x%llx", nip.ipvers, nip.addr1,
			ntoh64 (mip->bin.ip6_64[0]), nip.addr2, ntoh64 (mip->bin.ip6_64[1]));
...
ccp GetSqlCondMaskMIP (const ManageIP_t *mip, const NamesIP_t *names)
{
	...
	if (mip->bin.ipvers == 4)
		return PrintCircBuf (
			"%s=4 && %s=(%s&0x%08x)", nip.ipvers, nip.addr2, nip.mask2, ntohl (mip->bin.ip4));
	else if (mip->bin.ipvers == 6)
		return PrintCircBuf ("%s=6 && %s=(%s&0x%llx) && %s=(%s&0x%llx)", nip.ipvers, nip.addr1,
			nip.mask1, ntoh64 (mip->bin.ip6_64[0]), nip.addr2, nip.mask2,
			ntoh64 (mip->bin.ip6_64[1]));
...
ccp GetSqlCondBinMaskMIP (const ManageIP_t *mip, const NamesIP_t *names)
{
	...
	if (mip->bin.ipvers == 4)
		return PrintCircBuf ("compare_binaddr(%s,4,0,0x%08x,%s,%s)", nip.binaddr,
			ntohl (mip->bin.ip4), nip.mask1, nip.mask2);
	else if (mip->bin.ipvers == 6)
		return PrintCircBuf ("compare_binaddr(%s,6,0x%llx,0x%llx,%s,%s)", nip.binaddr,
			ntoh64 (mip->bin.ip6_64[0]), ntoh64 (mip->bin.ip6_64[1]), nip.mask1, nip.mask2);
...
const KeywordTab_t AllowIP4KeyTable[] = { { 0, "0", 0, 1 }, { ALLOW_MODE_DENY, "DENY", 0, 1 },
	{ ALLOW_MODE_ALLOW, "ALLOW", 0, 0 }, { ALLOW_MODE_AKEY, "AKEY", "JACC", 0 },

	{ ALLOW_MODE_LOCAL, "LOCAL", 0, 0 }, { ALLOW_MODE_LAN, "LAN", 0, 0 },
	{ ALLOW_MODE_EXTERN, "EXTERN", 0, 0 },

	{ ALLOW_MODE_PUBLIC, "PUBLIC", 0, 0 }, { ALLOW_MODE_USER, "USER", 0, 0 },
	{ ALLOW_MODE_MOD, "MOD", "MODERATOR", 0 }, { ALLOW_MODE_ADMIN, "ADMIN", "ADMINISTRATOR", 0 },
	{ ALLOW_MODE_DEVELOP, "DEVELOP", "DEVELOPER", 0 },

	{ ALLOW_MODE_LOG, "LOG", 0, 0 }, { ALLOW_MODE_VERBOSE, "VERBOSE", 0, 0 },

	{ ALLOW_RELEASE, "RELEASE", 0, 0 }, { ALLOW_DEBUG, "DEBUG", 0, 0 },

	{ ALLOW_MODE_NOT, "NOT", 0, 0 }, { ALLOW_MODE_SET, "SET", 0, ALLOW_MODE__OP },
	{ ALLOW_MODE_AND, "AND", 0, ALLOW_MODE__OP }, { 0, "OR", 0, ALLOW_MODE__OP },
	{ ALLOW_MODE_CONTINUE, "CONTINUE", 0, 0 }, { 0, 0, 0, 0 } };
...
enumError ScanLineAllowIP4 (AllowIP4_t *ai, // valid structure; new elements are appended
...
enumError ScanFileAllowIP4 (AllowIP4_t *ai, // valid structure; new elements are appended
...
void ResetCountersAllowIP4 (AllowIP4_t *ai // NULL or dest for addition
)
{
	if (ai)
	{
...
void AddCountersAllowIP4 (AllowIP4_t *dest, // NULL or dest for addition
	...
	if (dest && dest->used && src && src->used)
	{
		...
		for (is = 0; is < src->used; is++, ps++)
		{
			...
			for (id = 0; id < dest->used; id++, pd++)
				if (pd->addr == ps->addr && pd->mask == ps->mask && pd->mode == ps->mode)
				{
					pd->count += ps->count;
					break;
				}
...
static uint print_mode_helper (char *buf, uint bufsize, const KeywordTab_t *keytab, u64 mode)
{
	ASSERT (buf);
	ASSERT (bufsize > 1);
...
void DumpAllowIP4 (FILE *f, // NULL or output file
...
)
{
	...
	indent = NormalizeIndent (indent);
	fprintf (f,
		"%*sAllowIP4[%s]: %u reference%s, %u/%u elements,"
		" fallback:%llx, allow:%llx\n",
		indent, "", title ? title : "", ai->ref_counter, ai->ref_counter == 1 ? "" : "s", ai->used,
		ai->size, ai->fallback_mode, ai->allow_mode);
	...
	if (print_tab_head)
		fprintf (f,
			...
			"  settings\n%*s%.*s\n",
			indent, "", fw_mode + 4, "operation", indent, "", seplen, Minus300);
	...
	for (i = 0; i < ai->used; i++)
	{
		...
		fprintf (f, "%*s%8s  %-15s %08x/%08x > %s %*llx  %s\n", indent, "",
			...
			PrintIP4 (0, 0, it->addr, -1), it->addr, it->mask,
			it->mode & ALLOW_MODE_SET		? "SET"
				: it->mode & ALLOW_MODE_AND ? "AND"
											: "OR ",
...
u64 GetAllowIP4ByName (const AllowIP4_t *ai, mem_t name, u64 if_not_found)
{
	...
	return host.ip4_valid ? GetAllowIP4ByAddr (ai, host.ip4, if_not_found) : if_not_found;
}
...
u64 GetAutoAllowIP4ByName (const AllowIP4_t *ai, mem_t name)
{
	...
	NetworkHost_t host;
	ResolveHostMem (&host, true, name, 0, false, false);
	return host.ip4_valid ? GetAllowIP4ByAddr (ai, host.ip4, ai->fallback_mode) : ai->fallback_mode;
}
...
void SaveCurrentStateAllowIP4 (FILE *f, // output file
...
)
{
	...
	fprintf (f,
		...
		"%s"
		"n-rules%.*s= %u\n",
		name_prefix, (base - 8) / 8, Tabs20, ai->fallback_mode, name_prefix, (base - 5) / 8, Tabs20,
		ai->allow_mode, name_prefix, (base - 7) / 8, Tabs20, ai->used);
	...
	for (i = 0; i < ai->used; i++)
	{
		...
		fprintf (f, "%s%s%.*s= 0x%08x,0x%08x,%#llx,%llu\n", name_prefix, name, (base - len) / 8,
			Tabs20, it->addr, it->mask, it->mode, it->count);
	}
}
...
void RestoreStateAllowIP4 (RestoreState_t *rs, // info data, can be modified (cleaned after call)
...
void RestoreStateAllowIP4Ex (RestoreState_t *rs, // info data, modified
...
int ConnectByHost (NetworkHost_t *host, // valid pointer to NetworkHost_t
...
)
{
	...
	{
		if (!silent)
			ERROR1 (ERR_CANT_CONNECT, "Can't create socket: %s\n", GetHostName (host));
		return -1;
	}
	...
	{
		if (!silent)
			ERROR1 (ERR_CANT_CONNECT, "Can't connect to %s:%d\n", GetHostName (host), host->port);
		close (sock);
		return -1;
	}
...
int ConnectByHostTCP (NetworkHost_t *host, // valid pointer to NetworkHost_t
	bool silent // true: suppress error messages
)
{
	DASSERT (host);
...
int ConnectTCP (ccp addr, // optional service + server + optional host
...
int ConnectByHostUDP (NetworkHost_t *host, // valid pointer to NetworkHost_t
	bool silent // true: suppress error messages
)
{
	DASSERT (host);
...
int ConnectUDP (ccp name, // optional service + server + optional host
						  //   e.g.: "service://host:port/....."
...
u16 CalcChecksumIP4 (const void *data, // ip4 header
...
u16 CalcChecksumUDP (const ip4_head_t *ip4, // IP4 header
...
)
{
	...
	u32 csum = ntohs (((u16 *)&ip4->ip_src)[0]) + ntohs (((u16 *)&ip4->ip_src)[1])
		+ ntohs (((u16 *)&ip4->ip_dest)[0]) + ntohs (((u16 *)&ip4->ip_dest)[1]) + ip4->protocol
		+ ntohs (udp->port_src) + ntohs (udp->port_dest) + 2 * dlen;
...
enumError SendUnixUDP (ccp path1, // NULL or part #1 of path
...
)
{
	...
	if (sendto (sock, data, size, 0, (struct sockaddr *)&sa, sizeof (sa)) < 0)
	{
		if (!silent)
			ERROR1 (
				ERR_WRITE_FAILED, "Can't send %u bytes to UNIX stream socket: %s\n", size, path);
...
)
{
	...
	const uint len
		= snprintf (buf, buf_size, "%sconn=%u, %srecv_n=%u, %srecv=%llu, %ssend_n=%u, %ssend=%llu",
			prefix, ts->conn_count, prefix, ts->recv_count, prefix, ts->recv_size, prefix,
			ts->send_count, prefix, ts->send_size);
...
void PrintTranferStatistics (FILE *f, // output file, never NULL
...
)
{
	...
	if (stat->conn_count)
	{
		fprintf (f,
			...
			prefix, color1, name, stat->conn_count, stat->conn_count == 1 ? "" : "s",
			stat->recv_count, stat->recv_count == 1 ? "" : "s",
			PrintSize1024 (0, 0, stat->recv_size, 0), stat->send_count,
			stat->send_count == 1 ? "" : "s", PrintSize1024 (0, 0, stat->send_size, 0), color0);
	}
	else if (stat->recv_count || stat->send_count)
	{
		fprintf (f, "%s%s#STAT-%-5s %u packet%s (%s) received, %u packet%s (%s) send.%s\n", prefix,
			color1, name, stat->recv_count, stat->recv_count == 1 ? "" : "s",
			PrintSize1024 (0, 0, stat->recv_size, 0), stat->send_count,
			stat->send_count == 1 ? "" : "s", PrintSize1024 (0, 0, stat->send_size, 0), color0);
	}
	else
	...
	if (duration > 0 && prev)
	{
		...
		if (delta.recv_count)
		{
			...
			fprintf (f,
				...
				prefix, color1, name, duration_info, delta.recv_count, crate,
				delta.recv_count == 1 ? "" : "s", PrintSize1024 (0, 0, delta.recv_size, 0),
				PrintSize1024 (0, 0, srate, 0), color0);
		}
		...
		if (delta.send_count)
		{
			...
			fprintf (f,
				...
				prefix, color1, name, duration_info, delta.send_count, crate,
				delta.send_count == 1 ? "" : "s", PrintSize1024 (0, 0, delta.send_size, 0),
				PrintSize1024 (0, 0, srate, 0), color0);
		}
	}
}
...
void SaveCurrentStateTransferStats (FILE *f, // output file
...
)
{
	...
	fprintf (f,
		...
		"send-count	= %u\n"
		"send-size	= %llu\n",
		stat->conn_count, stat->recv_count, stat->recv_size, stat->send_count, stat->send_size);
}
...
)
{
	...
	fprintf (f, "%s%.*s= %u,%u,%llu,%u,%llu\n", name, tab_size > 0 ? tab_size : 0, "\t\t",
		stat->conn_count, stat->recv_count, stat->recv_size, stat->send_count, stat->send_size);
}
...
)
{
	...
	if (res > 0)
	{
		...
		char *ptr;
		stat->conn_count = strtoul (buf, &ptr, 10);
		if (*ptr == ',')
			ptr++;
		stat->recv_count = strtoul (ptr, &ptr, 10);
		if (*ptr == ',')
			ptr++;
		stat->recv_size = strtoull (ptr, &ptr, 10);
		if (*ptr == ',')
			ptr++;
		stat->send_count = strtoul (ptr, &ptr, 10);
		if (*ptr == ',')
			ptr++;
...
void ResetTCPStream (TCPStream_t *ts // valid TCP handler
)
{
	DASSERT (ts);
...
void DestroyTCPStream (TCPStream_t *ts // valid TCP handler
)
{
	DASSERT (ts);
...
)
{
	...
	uint count = 0;
	const int stat = SendDirectGrowBuffer (&ts->obuf, ts->sock, flush_output, data, size, &count);
	if (count > 0)
...
int PrintBinary1TCPStream (TCPStream_t *ts, // NULL or destination
...
int PrintBinary2TCPStream (TCPStream_t *ts, // NULL or destination
	ccp cmd, // command name
...
void LogTCPStream (FILE *f, // output file
	int indent, // indention
...
)
{
	...
	fprintf (f, "%*sTS: id:%x, sock=%d, %u packets received (%s), %u packets send (%s)", indent, "",
		ts->unique_id, ts->sock, ts->stat.recv_count, PrintSize1024 (0, 0, ts->stat.recv_size, 0),
...
void LogTCPStreamActivity (const TCPStream_t *ts, // valid TCP handler
	ccp activity)
{
	if (!ts->tracelog)
		return;
	...
	if (ts->handler)
	{
		...
		snprintf (hbuf, sizeof (hbuf), " | TH#%u: %u/%u/%u, sock=%d,%d,%d", th->unique_id,
			th->used_streams, th->max_used_streams, th->total_streams, so[0].sock, so[1].sock,
			so[2].sock);
	}
	else
		*hbuf = 0;

	TraceLogPrint (ts->tracelog, "%p %-20s sock=%d%s\n", ts, activity, ts->sock, hbuf);
}
...
)
{
	...
	if (ts->connect_usec)
		PrintTimerUSec6 (
			connect_usec, sizeof (connect_usec), now_usec - ts->connect_usec, DC_SFORM_ALIGN);
	else
	...
	if (ts->receive_usec)
		PrintTimerUSec6 (
			receive_usec, sizeof (receive_usec), now_usec - ts->receive_usec, DC_SFORM_ALIGN);
	else
		receive_usec[0] = '-', receive_usec[1] = 0;

	char send_usec[7];
	if (ts->send_usec)
		PrintTimerUSec6 (send_usec, sizeof (send_usec), now_usec - ts->send_usec, DC_SFORM_ALIGN);
	else
	...
	snprintf (buf, buf_size, "%6.6s%5d %c%c%c%c%c%c%c %s/%s %s/%s %6s %s %s %6s %s %s  %s",
		connect_usec, ts->sock, ts->protect > 0 ? 'P' : '-', ts->auto_close > 0 ? 'A' : '-',
		ts->rescan > 0 ? 'S' : '-', ts->eof > 0 ? 'X' : '-', ts->error > 0 ? 'E' : '-',
		ts->ibuf.disabled ? 'D' : 'i', ts->obuf.disabled ? 'D' : 'o',
		PrintNumberU4 (0, 0, ts->ibuf.used, true), PrintNumberU4 (0, 0, ts->ibuf.max_used, true),
		PrintNumberU4 (0, 0, ts->obuf.used, true), PrintNumberU4 (0, 0, ts->obuf.max_used, true),
		receive_usec, PrintNumberU5 (0, 0, ts->stat.recv_count, true),
		PrintSize1024 (0, 0, ts->stat.recv_size, DC_SFORM_TINY | DC_SFORM_ALIGN), send_usec,
		PrintNumberU5 (0, 0, ts->stat.send_count, true),
		PrintSize1024 (0, 0, ts->stat.send_size, DC_SFORM_TINY | DC_SFORM_ALIGN), ts->info);
...
void AddSocketTCPStream (TCPStream_t *ts, // valid TCP handler
...
void CheckSocketTCPStream (TCPStream_t *ts, // valid TCP handler
...
#define PRINT_TIMEOUT(ts, now, info)                                                               \
	noPRINT ("%s[%d]: now=%lld, trig=%lld, accept=%lld, OnTimeout=%d\n", info, ts->sock, now,      \
		ts->trigger_usec ? ts->trigger_usec - now : 0,                                             \
...
{
	DASSERT (ts);
	const u64 ref = ts->connect_usec > ts->receive_usec ? ts->connect_usec : ts->receive_usec;
...
{
	DASSERT (ts);
	return ts->timeout_usec ? GetLastActivityTCPStream (ts) + ts->timeout_usec : 0;
}
...
void SetupTriggerTCPStream (TCPStream_t *ts, u64 now_usec)
{
	...
	if (next)
	{
		...
		noPRINT ("next=%lld [%lld], accept=%lld [%lld]\n", next, next - now_usec, ts->accept_usec,
			ts->accept_usec - now_usec);
	}
}
...
void InitializeTCPHandler (TCPHandler_t *th, // not NULL
...
void ResetTCPHandler (TCPHandler_t *th // valid TCP handler
)
{
	DASSERT (th);
...
TCPStream_t *AddTCPStream (TCPHandler_t *th, // valid TCP handler
...
TCPStream_t *CreateTCPStream (TCPHandler_t *th, // valid TCP handler
...
)
{
	...
	TCPStream_t *ts = listen && listen->OnCreateStream ? listen->OnCreateStream (th, sock) : 0;
	if (!ts)
	{
...
TCPStream_t *ConnectUnixTCPStream (TCPHandler_t *th, // valid TCP handler
...
TCPStream_t *ConnectTCPStream (TCPHandler_t *th, // valid TCP handler
...
)
{
	...
	if (sock == -1)
	{
		if (!silent)
			ERROR1 (
				ERR_CANT_CREATE, "Can't create TCP socket: %s\n", PrintIP4 (0, 0, nh.ip4, nh.port));
		return 0;
	}
	...
	if (stat && errno != EINPROGRESS)
	{
		if (!silent)
			ERROR1 (
				ERR_CANT_OPEN, "Can't connect TCP socket: %s\n", PrintIP4 (0, 0, nh.ip4, nh.port));
		shutdown (sock, SHUT_RDWR);
		close (sock);
		ResetHost (&nh);
		return 0;
...
TCPStream_t *FindTCPStreamByUniqueID (TCPHandler_t *th, // valid TCP handler
...
uint NumOfSocketsTCP (const TCPHandler_t *th)
{
	...
	for (ts = th->first; ts; ts = ts->next)
		if (ts->sock != -1
			&& (!ts->eof && !ts->ibuf.disabled || !ts->obuf.disabled && ts->obuf.used))
		{
			count++;
		}
	return count;
...
enumError ListenUnixTCP (TCPHandler_t *th, // valid TCP handler
...
enumError ListenTCP (TCPHandler_t *th, // valid TCP handler
...
)
{
	...
	for (i = 0; i < TCP_HANDLER_MAX_LISTEN; i++)
		if (th->listen[i].sock != -1)
		{
			...
			if (si.is_valid && si.protocol == AF_UNIX && si.address && *si.address)
			{
				PRINT ("unlink(%s)\n", si.address);
				unlink (si.address);
			}
...
void AddSocketsTCP (TCPHandler_t *th, // valid TCP handler
	FDList_t *fdl // valid file descriptor list
)
{
	DASSERT (th);
...
TCPStream_t *OnAcceptStream (TCPHandler_t *th, // valid TCP handler
		...
		else
		{
			...
			if (ts)
			{
				if (!ts->info[0])
				{
					if (lsock->info[0])
						snprintf (
							ts->info, sizeof (ts->info), "by %s [%x]", lsock->info, ts->unique_id);
					else
						snprintf (ts->info, sizeof (ts->info), "by socket %d [%x]", lsock->sock,
							ts->unique_id);
				}
			}
			else
...
void OnCloseStream (TCPStream_t *ts, // valid TCP stream
	u64 now_usec // time for timestamps, GetTimeUSec(false)
)
{
	DASSERT (ts);
...
void SetNotSocketStream (TCPStream_t *ts // valid TCP stream
)
{
	DASSERT (ts);
	if (ts->sock != -1)
...
void OnReceivedStream (TCPStream_t *ts, // valid TCP stream
	u64 now_usec // time for timestamps, GetTimeUSec(false)
)
{
	DASSERT (ts);
...
void OnWriteStream (TCPStream_t *ts, // valid TCP stream
	u64 now_usec // time for timestamps, GetTimeUSec(false)
)
{
	DASSERT (ts);
...
void CheckSocketsTCP (TCPHandler_t *th, // valid TCP handler
	FDList_t *fdl, // valid socket list
	bool check_timeout // true: enable timeout checks
)
{
...
void LogTCPHandler (FILE *f, // output file
	int indent, // indention
...
)
{
	...
	fprintf (f, "%*sTH: id:%x, n=%u/%u/%u, listen=", indent, "", th->unique_id, th->used_streams,
		th->max_used_streams, th->total_streams);

	uint i;
	for (i = 0; i < TCP_HANDLER_MAX_LISTEN; i++)
		fprintf (f, "%d,", th->listen[i].sock);

	fprintf (f, " max=%u, dsize=%u, ready=%u", th->max_conn, th->data_size, NumOfSocketsTCP (th));
	...
	fprintf (f, "\n%*s    %u connects, %u packets received (%s), %u packets send (%s)\n", indent,
		"", th->stat.conn_count, th->stat.recv_count, PrintSize1024 (0, 0, th->stat.recv_size, 0),
...
void PrintStreamTableTCPHandler (FILE *f, // output file
...
)
{
	...
	fprintf (f, "%s%*s%.*s\n%s%*s%s\n%s%*s%s\n%s%*s%.*s%s\n", colset->heading, indent, "", sep_len,
		Minus300, colset->heading, indent, "", line1, colset->heading, indent, "", line2,
		colset->heading, indent, "", sep_len, Minus300, colset->reset);
	...
	{
		snprintf (buf, sizeof (buf), "TOTAL: %u current client%s, %u max", th->used_streams,
			th->used_streams == 1 ? "" : "s", th->max_used_streams);

		fprintf (f, "%s%*s%-45s%s %s%13s %s  Summary of %u stream%s\n%s%*s%.*s%s\n", colset->status,
			indent, "", buf, PrintNumberU7 (0, 0, th->stat.recv_count, true),
			...
			th->total_streams, th->total_streams == 1 ? "" : "s", colset->heading, indent, "",
			sep_len, Minus300, colset->reset);
	}
}
...
uint GetSocketNameBySA (char *buf, // dest buffer
...
static int OnTimeoutSingleTCP (struct TCPStream_t *ts, u64 now_usec)
{
	PRINT ("OnTimeoutSingleTCP(%p,%llu) sock=%d, to=%lld,%lld d=%lld,%lld\n", ts, now_usec,
		ts->sock, ts->trigger_usec, ts->trigger_usec - now_usec, ts->accept_usec,
		ts->accept_usec - now_usec);
	DASSERT (ts);
	OnCloseStream (ts, now_usec);
	return 0;
...
static TCPStream_t *SendSingleTCPHelper (TCPHandler_t *th, // valid handle
...
TCPStream_t *SendSingleUnixTCP (TCPHandler_t *th, // valid handle
...
TCPStream_t *SendSingleTCP (TCPHandler_t *th, // valid handle
	ccp addr, // address -> NetworkHost_t
	u16 default_port, // default port
...
void SetTimeoutCommandTCP (TCPStream_t *ts, // valid stream
...
static char *ScanLineCommandTCP (
	...
	struct TCPStream_t *ts, char *line, char *line_end, u64 now_usec)
{
	DASSERT (ts);
	DASSERT (line);
	DASSERT (line_end);
...
void SaveCurrentStateSocket (FILE *f, // output file
...
)
{
	...
	fprintf (f,
		...
		"is-unix		= %d\n"
		"info		= %s\n",
		sock->sock, sock->is_unix, info);
}

///////////////////////////////////////////////////////////////////////////////

void SaveCurrentStateTCPStream (FILE *f, // output file
...
)
{
	...
	fprintf (f,
		...
		"send-usec	= %llu\n"
		"info		= %s\n",
		ts->sock, ts->rescan, ts->eof, ts->protect, ts->auto_close, ts->accept_usec,
		ts->trigger_usec, ts->delay_usec, ts->timeout_usec, ts->connect_usec, ts->receive_usec,
		ts->send_usec, info);
...
void SaveCurrentStateCommandTCP (FILE *f, // output file
	const TCPStream_t *ts // valid TCP stream
)
{
	DASSERT (f);
	...
	fprintf (f,
		"cmd-comma	= %d\n"
		"cmd-timeout	= %llu\n",
		ci->comma_is_eol, ci->timeout_usec);
}

///////////////////////////////////////////////////////////////////////////////

void SaveCurrentStateTCPHandler (FILE *f, // output file
...
)
{
	...
	fprintf (f,
		...
		"max-used	= %u\n"
		"total-streams	= %u\n",
		Minus300, sect_name, th->data_size, th->max_conn, th->need_maintenance, th->used_streams,
		th->max_used_streams, th->total_streams);
...
void RestoreStateSocket (RestoreState_t *rs, // info data, can be modified (cleaned after call)
	cvp user_table // pointer provided by RestoreStateTab_t[]
)
{
	DASSERT (rs);
...
void RestoreStateTCPHandler_base (TCPHandler_t *th, // valid TCPHandler_t
	RestoreState_t *rs // info data, can be modified (cleaned after call)
)
{
	DASSERT (th);
	...
	const struct uint_parm_t uint_parm_tab[] = { { "max-conn", &th->max_conn },
		{ "maintenance", &th->need_maintenance }, { "max-used", &th->max_used_streams },
		{ "total-streams", &th->total_streams }, { 0, 0 } };
...
Socket_t *RestoreStateTCPHandler_socket (TCPHandler_t *th, // valid TCPHandler_t
	RestoreState_t *rs // info data, can be modified (cleaned after call)
)
{
	DASSERT (th);
...
TCPStream_t *RestoreStateTCPHandler_stream (TCPHandler_t *th, // valid TCPHandler_t
	RestoreState_t *rs // info data, can be modified (cleaned after call)
)
{
	DASSERT (th);
...
void RestoreStateTCPHandler (RestoreState_t *rs, // info data, can be modified (cleaned after call)
	cvp user_table // pointer provided by RestoreStateTab_t[]
)
{
	DASSERT (rs);
...
void RestoreStateCommandTCP (RestoreState_t *rs, // info data, can be modified (cleaned after call)
	cvp user_table // pointer provided by RestoreStateTab_t[]
)
{
	DASSERT (rs);
