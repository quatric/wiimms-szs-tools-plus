...
bool IsDolHeader (const void *data, uint data_size, uint file_size)
{
	...
	for (i = 0; i < DOL_N_SECTIONS; i++)
	{
		...
		if (off || size || addr)
		{
			if (off & 3 || size & 3 || addr & 3 || off < DOL_HEADER_SIZE
...
const char dol_section_name[DOL_NN_SECTIONS + 1][4] = { "T0", "T1", "T2", "T3", "T4", "T5", "T6",
...
bool GetDolSectionInfo (dol_sect_info_t *info, // valid pointer: returned data
			...
			if (addr < addr_end && addr + size > addr_beg)
			{
				...
				if (mi)
				{
					if (i < DOL_N_TEXT_SECTIONS)
						...
						snprintf (mi->info, sizeof (mi->info), "D%u", i - DOL_N_TEXT_SECTIONS);
				}
			}
		}
	}
...
void DumpDolHeader (FILE *f, // dump to this file
...
u32 GetDolOffsetByAddr (const dol_header_t *dol_head, // valid DOL header
	u32 addr, // address to search
	u32 size, // >0: wanted size
...
u32 GetDolAddrByOffset (const dol_header_t *dol_head, // valid DOL header
...
uint AddDolAddrByOffset (const dol_header_t *dol_head, // valid DOL header
...
uint TranslateDolOffsets (const dol_header_t *dol_head, // valid DOL header
	MemMap_t *mm, // valid destination mem map, not cleared
	bool use_tie, // TRUE: use InsertMemMapTie()
...
uint TranslateAllDolOffsets (const dol_header_t *dol_head, // valid DOL header
	MemMap_t *mm, // valid destination mem map, not cleared
	bool use_tie // TRUE: use InsertMemMapTie()
)
{
	...
	for (i = 0; i < DOL_N_SECTIONS; i++)
	{
		...
		if (size)
		{
			...
			const u32 addr = ntohl (dol_head->sect_addr[i]);
			MemMapItem_t *mi
				= use_tie ? InsertMemMapTie (mm, addr, size) : InsertMemMap (mm, addr, size);
			if (*mi->info)
...
uint TranslateDolSections (const dol_header_t *dol_head, // valid DOL header
	MemMap_t *mm, // valid destination mem map, not cleared
	bool use_tie, // TRUE: use InsertMemMapTie()
...
)
{
	...
	for (sect = 0; sect < DOL_NN_SECTIONS; sect++)
	{
			...
			&& GetDolSectionInfo (&sinfo, dol_head, sizeof (*dol_head), sect))
		{
			...
			MemMapItem_t *mi = use_tie ? InsertMemMapTie (mm, sinfo.addr, size)
...
static u32 FindDolAddress (const u8 *data, // DOL data beginning with dol_header_t
...
)
{
	...
	{
		const u8 *res = memmem (data + off_begin, off_end - off_begin, search, search_size);
		if (!res)
			return 0;
		off_begin = res - data;
...
)
{
	...
	static const u8 search1[] = { 0x7C, 0xE3, 0x3B, 0x78, 0x38, 0x87, 0x00, 0x34, 0x38, 0xA7, 0x00,
		0x38, 0x38, 0xC7, 0x00, 0x4C };
	...
	for (sect = 0; sect < DOL_N_SECTIONS; sect++)
	{
		...
		{
			off2 = FindDolAddress (
				data, off2 + sizeof (search1), off_end, search2, sizeof (search2));
			if (off2)
...
bool IsValidGCH (const gch_header_t *gh, // valid header
...
const sizeof_info_t sizeof_info_dol[] = { SIZEOF_INFO_TITLE ("DOL") SIZEOF_INFO_ENTRY (dol_header_t)
		SIZEOF_INFO_ENTRY (dol_sect_info_t) SIZEOF_INFO_ENTRY (dol_sect_select_t)
			SIZEOF_INFO_ENTRY (dol_sect_addr_t) SIZEOF_INFO_ENTRY (gch_header_t) SIZEOF_INFO_ENTRY (
				wch_segment_t) SIZEOF_INFO_ENTRY (wch_header_t) SIZEOF_INFO_ENTRY (wch_control_t)
