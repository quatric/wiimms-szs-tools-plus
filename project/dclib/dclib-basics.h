...
#define SIZEOFRANGE(type, a, b) (offsetof (type, b) + sizeof (((type *)0)->b) - offsetof (type, a))
...
{
	const u8 *d = (u8 *)&data;
	return TableBitCount[d[0]] + TableBitCount[d[1]];
}
...
static inline uint Count1Bits32 (u32 data)
{
	...
	const u8 *d = (u8 *)&data;
	return TableBitCount[d[0]] + TableBitCount[d[1]] + TableBitCount[d[2]] + TableBitCount[d[3]];
}
...
static inline uint Count1Bits64 (u64 data)
{
	...
	const u8 *d = (u8 *)&data;
	return TableBitCount[d[0]] + TableBitCount[d[1]] + TableBitCount[d[2]] + TableBitCount[d[3]]
		+ TableBitCount[d[4]] + TableBitCount[d[5]] + TableBitCount[d[6]] + TableBitCount[d[7]];
#endif
}
...
{
	return PrintBitfieldEx (buf, buf_size, bit_field, 0, 8 * bit_field_size - 1, use_ranges, 1, 0);
}
...
void *CopyData (const void *data, // data to copy/move/link
...
char *CopyString (ccp string, // string to copy/move/link
	CopyMode_t mode, // copy mode
	bool *res_alloced // not NULL:
					  //   store true, if data must be freed
					  //   otherwise store false
...
static inline void FreeData (const void *data, // data to free
	CopyMode_t mode // copy mode
)
{
	if (mode != CPM_LINK)
...
exmem_t ExMemCat (exmem_dest_t *dest, // kind of destination, if NULL then MALLOC()
	mem_t sep, // insert separators between sources with len>0
	const mem_src_t *src // sources. NULL allowed
);

exmem_t ExMemCat2 (exmem_dest_t *dest, // kind of destination, if NULL then MALLOC()
	mem_t sep, // insert separators between sources with len>0
	mem_t src1, // first source
	mem_t src2 // second source
);

exmem_t ExMemCat3 (exmem_dest_t *dest, // kind of destination, if NULL then MALLOC()
	mem_t sep, // insert separators between sources with len>0
	mem_t src1, // first source
	mem_t src2, // second source
...
exmem_t AllocExMemS (cvp src, // NULL or source
...
{
	exmem_t em = { { 0 } };
	AssignExMem (&em, source, copy_mode);
	return em;
}
...
{
	exmem_t em = { { 0 } };
	AssignExMemS (&em, source, slen, copy_mode);
	return em;
}
...
{
	exmem_t em = { { 0 } };
	AssignExMemS (&em, source.ptr, source.len, copy_mode);
	return em;
}
...
void InsertMemListN (mem_list_t *ml, // valid destination mem_list
...
static inline void InsertMemList2 (mem_list_t *ml, // valid destination mem_list
	int pos, // insert position => CheckIndex1()
...
static inline void AppendMemListN (mem_list_t *ml, // valid destination mem_list
	const mem_t *src, // source list
	uint src_len, // number of elements in source list
	mem_list_mode_t src_ignore // how to manage NULL and empty strings
)
...
static inline void AppendMemList2 (mem_list_t *ml, // valid destination mem_list
	const mem_list_t *src, // source mem_list
	mem_list_mode_t src_ignore // how to manage NULL and empty strings
)
{
...
static inline void AssignMemListN (mem_list_t *ml, // valid destination mem_list
	const mem_t *src, // source list
	uint src_len, // number of elements in source list
	mem_list_mode_t src_ignore // how to manage NULL and empty strings
)
...
static inline void AssignMemList2 (mem_list_t *ml, // valid destination mem_list
	const mem_list_t *src, // source mem_list
	mem_list_mode_t src_ignore // how to manage NULL and empty strings
)
{
...
void CatMemListN (mem_list_t *dest, // valid destination mem_list
...
static inline void CatMemList2 (mem_list_t *dest, // valid destination mem_list
...
static inline void CatMemList3 (mem_list_t *dest, // valid destination mem_list
	const mem_list_t *src1, // source mem_list
	const mem_list_t *src2, // source mem_list
...
{
	DASSERT (mp);
	return source ? MemDupMemPool (mp, source, strlen (source)) : 0;
}
...
typedef struct PrintMode_t
{
	...
	const ColorSet_t *cout; // NULL (no colors) or color set for 'fout'

	FILE *ferr; // NULL or error file
	const ColorSet_t *cerr; // NULL (no colors) or color set for 'ferr'

	FILE *flog; // if NULL: use 'ferr' or 'fout' as fallback
	const ColorSet_t *clog; // NULL (no colors) or color set for 'flog'
...
uint SplitByCharMemList (mem_list_t *ml, // valid destination mem_list
...
uint SplitByTextMemList (mem_list_t *ml, // valid destination mem_list
	bool init_ml, // true: initialize 'ml' first
	mem_t source, // source string
	mem_t sep, // separator text
	int max_fields // >0: max number of result fields
...
{
	return (need + alloc_info_add1 & alloc_info_mask) + alloc_info_add2;
}

static inline uint GetGoodAllocSize2 (uint n_elem, uint itemsize)
{
	return ((n_elem * itemsize + alloc_info_add1 & alloc_info_mask) + alloc_info_add2) / itemsize;
}

static inline uint GetGoodAllocSizeA (uint need, uint align)
{
	need = (need + align - 1) / align * align;
	return ((need + alloc_info_add1 & alloc_info_mask) + alloc_info_add2) / align * align;
}
...
void SetupProgname (int argc, char **argv, ccp tool_name, ccp tool_vers, ccp tool_title);
...
#else
static inline ccp ProgramPathNoExt (void)
{
	return ProgramPath ();
}
#endif
...
char *StringCat3LS (
	char *buf, ssize_t buf_size, ccp src1, int len1, ccp src2, int len2, ccp src3, int len3);
char *StringCat2LE (char *buf, ccp buf_end, ccp src1, int len1, ccp src2, int len2);
char *StringCat3LE (
	char *buf, ccp buf_end, ccp src1, int len1, ccp src2, int len2, ccp src3, int len3);
...
int PathCmp (ccp path1, // NULL or first path
...
uint TrimBlanks (char *str, // pointer to string, modified
...
char *PrintVersion (char *buf, // result buffer
							   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u32 version // version number to print
);

char *PrintID (const void *id, // ID to convert in printable format
...
void ReleaseCircBuf (ccp end_buf, // pointer to end of previous alloced buffer
...
bool MatchPatternFull (ccp pattern, // pattern text
	ccp text // raw text
);

bool MatchPattern (ccp pattern, // pattern text
	ccp text, // raw text
...
void SetupArgManager (ArgManager_t *am, LowerUpper_t force_case, int argc, char **argv, bool clone);
...
enumError ScanFileArgManager (ArgManager_t *am, // valid arg-manager
...
enumError ExpandAtArgManager (ArgManager_t *am, // valid arg-manager
...
void ListSizeofInfo (const PrintMode_t *p_pm, // NULL or print mode
...
static inline void ListSizeofInfoMgr (const PrintMode_t *p_pm, // NULL or print mode
...
void ResetContainer (Container_t *c // container to reset => no data
);

//-----------------------------------------------------------------------------

void DeleteContainer (Container_t *c // container to reset and to free => no data
);

//-----------------------------------------------------------------------------

void UnlinkContainerData (Container_t *c // container to reset => no data
);
...
{
	return c && c->cdata
		&& (c->cdata->ref_count <= 1 || c->cdata->protect_count <= (c->protect_level > 0));
}
...
{
	return c && c->cdata && ptr && (u8 *)ptr >= c->cdata->data
...
{
	return c && c->cdata && ptr && (u8 *)ptr >= c->cdata->data && end
		&& (u8 *)end <= c->cdata->data + c->cdata->size;
}
...
uint DumpInfoContainerData (
	...
	ccp prefix, // not NULL: use it as prefix behind indention
	const ContainerData_t *cdata, // dump infos for this container-data, NULL allowed
	uint hexdump_len, // max number of bytes used for a hexdump
...
void PutLines (FILE *f, // valid output stream
	int indent, // indent of output
...
void PrintArgLines (FILE *f, // valid output stream
	int indent, // indent of output
	int fw, // field width of output
	int first_line, // length without prefix of already printed first line
	ccp prefix, // NULL or prefix for each line
...
void PrintLines (FILE *f, // valid output stream
	int indent, // indent of output
	int fw, // field width of output
	int first_line, // length without prefix of already printed first line
	ccp prefix, // NULL or prefix for each line
...
{
	return ScanEscapedString (
		buf, buf_size, source, -1, true, source && *source == '"' ? '"' : '|', scanned_len);
}
...
{
	return code < SJIS_TAB_MAPPING_BEG || code > SJIS_TAB_MAPPING_END && code < SJIS_INVALID_CODE;
}
...
typedef s64 (*KeywordCallbackFunc) (void *param, // NULL or user defined parameter
...
const KeywordTab_t *GetKewordById (const KeywordTab_t *key_tab, // NULL or pointer to command table
	s64 id // id to search
);
...
const KeywordTab_t *ScanKeywordEx (int *res_abbrev, // NULL or pointer to result 'abbrev_count'
	ccp arg, // argument to scan
...
s64 ScanKeywordListP (KeyListParam_t *par, // NULL or parameters and extended result
...
s64 ScanKeywordListEx (ccp arg, // argument to scan
	const KeywordTab_t *key_tab, // valid pointer to command table
...
static inline s64 ScanKeywordList (ccp arg, // argument to scan
	const KeywordTab_t *key_tab, // valid pointer to command table
	KeywordCallbackFunc func, // NULL or calculation function
	bool allow_prefix, // allow '-' | '+' | '=' as prefix
	u32 max_number, // allow numbers < 'max_number' (0=disabled)
...
{
	return ScanKeywordListEx (
		arg, key_tab, func, allow_prefix, max_number, result, 0, err_msg, err_code, 0);
}

//-----------------------------------------------------------------------------

enumError ScanKeywordListFunc (ccp arg, // argument to scan
	const KeywordTab_t *key_tab, // valid pointer to command table
...
s64 ScanKeywordListMask (ccp arg, // argument to scan
	const KeywordTab_t *key_tab // valid pointer to command table
);
...
enumError PrintKeywordError (const KeywordTab_t *key_tab, // NULL or pointer to command table
	ccp key_arg, // analyzed command
	int key_stat, // status of ScanKeyword()
...
uint CollectAmbiguousKeywords (char *buf, // destination buffer, 200 bytes are good
	uint buf_size, // size of buffer
	const KeywordTab_t *key_tab, // NULL or pointer to command table
	ccp key_arg // analyzed command
);
...
enumError Command_COLORS (int level, // only used, if mode==NULL
...
typedef struct CommandList_t
{
	...
	struct TCPStream_t *user_ts; // NULL or a stream for replies
...
extern u64 (*ScanSizeFactorHook) (char ch_factor, // char to analyze
...
static inline u_sec_t double2sec (double d)
{
	return d > 0.0 ? (u_msec_t)trunc (d + 0.5) : 0;
}
static inline u_msec_t double2msec (double d)
{
	return d > 0.0 ? (u_msec_t)trunc (1e3 * d + 0.5) : 0;
}
static inline u_usec_t double2usec (double d)
{
	return d > 0.0 ? (u_usec_t)trunc (1e6 * d + 0.5) : 0;
}
static inline u_nsec_t double2nsec (double d)
{
	return d > 0.0 ? (u_nsec_t)trunc (1e9 * d + 0.5) : 0;
}

static inline s_sec_t double2ssec (double d)
{
	return (s_sec_t)floor (d + 0.5);
}
static inline s_msec_t double2smsec (double d)
{
	return (s_msec_t)floor (1e3 * d + 0.5);
}
static inline s_usec_t double2susec (double d)
{
	return (s_usec_t)floor (1e6 * d + 0.5);
}
static inline s_nsec_t double2snsec (double d)
{
	return (s_nsec_t)floor (1e9 * d + 0.5);
}

static inline double sec2double (s_sec_t num)
{
	return num;
}
static inline double msec2double (s_msec_t num)
{
	return num * 1e-3;
}
static inline double usec2double (s_usec_t num)
{
	return num * 1e-6;
}
static inline double nsec2double (s_nsec_t num)
{
	return num * 1e-9;
}
...
static inline ccp PrintTimerSec3 (char *buf, // result buffer (>3 bytes)
											 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
static inline ccp PrintTimerMSec3 (char *buf, // result buffer (>3 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
static inline ccp PrintTimerUSec3 (char *buf, // result buffer (>3 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_usec_t usec, // microseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_DASH
...
static inline ccp PrintTimerSec4 (char *buf, // result buffer (>4 bytes)
											 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_sec_t sec, // seconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_DASH
...
static inline ccp PrintTimerMSec4 (char *buf, // result buffer (>4 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_msec_t msec, // microseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_DASH
...
{
	return PrintTimer4 (
		buf, buf_size, msec / MSEC_PER_SEC, msec % MSEC_PER_SEC * USEC_PER_SEC, mode);
}

static inline ccp PrintTimerUSec4 (char *buf, // result buffer (>4 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_usec_t usec, // microseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_DASH
...
static inline ccp PrintTimerNSec4 (char *buf, // result buffer (>4 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_nsec_t nsec, // nanoseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_DASH
...
static inline ccp PrintTimerSec6 (char *buf, // result buffer (>6 bytes)
											 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_sec_t sec, // seconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_CENTER | DC_SFORM_DASH
...
static inline ccp PrintTimerMSec6 (char *buf, // result buffer (>6 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_msec_t msec, // microseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_CENTER | DC_SFORM_DASH
...
{
	return PrintTimer6N (
		buf, buf_size, msec / MSEC_PER_SEC, msec % MSEC_PER_SEC * NSEC_PER_MSEC, mode);
}

static inline ccp PrintTimerUSec6 (char *buf, // result buffer (>6 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_usec_t usec, // microseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_CENTER | DC_SFORM_DASH
...
{
	return PrintTimer6N (
		buf, buf_size, usec / USEC_PER_SEC, usec % USEC_PER_SEC * NSEC_PER_USEC, mode);
}

static inline ccp PrintTimerNSec6 (char *buf, // result buffer (>6 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u_nsec_t nsec, // nanoseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_CENTER | DC_SFORM_DASH
...
{
	return PrintTimer6N (buf, buf_size, nsec / NSEC_PER_SEC, nsec % NSEC_PER_SEC, mode);
}
...
static inline ccp PrintTimerNSec4s (char *buf, // result buffer (>4 bytes)
											   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	s_nsec_t nsec, // nanoseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_PLUS | DC_SFORM_DASH
...
static inline ccp PrintTimerMSec4s (char *buf, // result buffer (>4 bytes)
											   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
static inline ccp PrintTimerSec4s (char *buf, // result buffer (>4 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
static inline ccp PrintTimerMSec7s (char *buf, // result buffer (>7 bytes)
											   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	s_msec_t msec, // milliseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_PLUS | DC_SFORM_DASH
...
static inline ccp PrintTimerSec7s (char *buf, // result buffer (>7 bytes)
											  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	int sec, // seconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_PLUS | DC_SFORM_DASH
...
char *ScanDateTime (time_t *res_time, // not NULL: store seconds
	u32 *res_usec, // not NULL: store microseconds of second
	ccp source, // source text
...
static inline ccp year2text (int year, // year to print
	int fw, // wanted field width assuming separators are 1 char
...
static inline void DisableUsageCount (void)
{
	usage_count_enabled--;
}
...
typedef void (*ListUsageFunc) (PrintMode_t *pm, // valid print mode
...
typedef void (*UpdateUsageFunc) (const UsageCtrl_t *uci, // valid object to update
...
{
	DASSERT (uci);
	return uci->par ? uci->par : uci->uc ? &uci->uc->par : uci->ud ? &uci->ud->par : 0;
}
...
{
	sockaddr_info_t i = { .sa = (sockaddr_t *)sa, .size = size };
	return i;
}
...
static inline ipv4_class_t CheckIP4 (ccp addr, // address to analyze
	int addr_len // length of addr; if <0: use strlen(addr)
)
{
...
static inline char *PrintIP4sa (char *buf, // result buffer
										   // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
{
	return sa ? PrintIP4 (buf, buf_size, ntohl (sa->sin_addr.s_addr), ntohs (sa->sin_port)) : "-";
}

static inline char *PrintRightIP4sa (char *buf, // result buffer
												// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
{
	DASSERT (sa);
	return PrintRightIP4 (buf, buf_size, ntohl (sa->sin_addr.s_addr), ntohs (sa->sin_port), 0);
}

static inline char *PrintAlignedIP4sa (char *buf, // result buffer
												  // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
{
	DASSERT (sa);
	return PrintAlignedIP4 (buf, buf_size, ntohl (sa->sin_addr.s_addr), ntohs (sa->sin_port));
}
...
DynData_t *InsertDDL (DynDataList_t *ddl, // valid dynamic data list
	uint index, // element index to change
	const void *data, // NULL or data to assign
	uint data_size // size of data
);

DynData_t *GetDDL (DynDataList_t *ddl, // valid dynamic data list
...
enumError LoadStringField (StringField_t *sf, // string field
...
enumError SaveStringField (StringField_t *sf, // valid string field
	ccp filename, // filename of dest file
	bool rm_if_empty // true: rm dest file if 'sf' is empty
);

enumError WriteStringField (FILE *f, // open destination file
...
void SaveCurrentStateStringField (FILE *f, // output file
...
void RestoreStateStringField (struct RestoreState_t *rs, // info data, may be modified
	ccp name_prefix, // NULL or prefix of name, up to 40 chars
...
// append at the end and do not sort
ParamFieldItem_t *AppendParamField (ParamField_t *pf, ccp key, bool move_key, uint num, cvp data);
...
bool InsertEML (
	exmem_list_t *eml, ccp key, CopyMode_t cm_key, const exmem_t *data, CopyMode_t cm_data);
...
typedef struct MultiColumn_t
{
	...
	const ColorSet_t *colset; // NULL or color set for the frame
...
void CopyMemMap (MemMap_t *mm1, // merged mem map, not NULL, cleared
	const MemMap_t *mm2, // NULL or second mem map
	bool use_tie // TRUE: use InsertMemMapTie()
);

void MergeMemMap (MemMap_t *mm1, // merged mem map, not NULL, not cleared
	const MemMap_t *mm2, // NULL or second mem map
	bool use_tie // TRUE: use InsertMemMapTie()
);
...
MemMapItem_t *FindMemMap (
	...
	MemMap_t *mm, off_t off, off_t size);
...
void InsertMemMapWrapper (void *param, // user defined parameter
...
void InitializeDataBuf (DataBuf_t *db, uint buf_size, DataBufFill_t func, void *user_data);
...
void InitializeResize (ResizeHelper_t *r, // data structure
...
enumError ScanRestoreState (RestoreState_t *rs, // valid control
...
enumError RestoreState (const RestoreStateTab_t *table, // section table, terminated by name==0
	void *data, // file data, modified, terminated by NULL or LF
	uint size, // size of file data
...
ParamFieldItem_t *GetParamField (const RestoreState_t *rs, // valid restore-state structure
	ccp name // name of member
);

//-----------------------------------------------------------------------------

int GetParamFieldINT (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
uint GetParamFieldUINT (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
u64 GetParamFieldS64 (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
u64 GetParamFieldU64 (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
float GetParamFieldFLOAT (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
double GetParamFieldDBL (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
long double GetParamFieldLDBL (const RestoreState_t *rs, // valid restore-state structure
	ccp name, // name of member
...
#define DEF_SRT_VAR(v, ne, n, t, e)                                                                \
	{ offsetof (SRT_NAME, v), sizeof (((SRT_NAME *)0)->v), n, .n_elem = ne, .type = t, .emode = e }
...
#define DEF_SRT_ARRAY(v, n, t, e)                                                                  \
	{ offsetof (SRT_NAME, v), sizeof (((SRT_NAME *)0)->v[0]), n,                                   \
		.n_elem = sizeof (((SRT_NAME *)0)->v) / sizeof (*((SRT_NAME *)0)->v), .type = t,           \
		.emode = e }
...
#define DEF_SRT_ARRAY_C(v, n, t, e)                                                                \
	{ offsetof (SRT_NAME, v), sizeof (((SRT_NAME *)0)->v[0]), n,                                   \
		.n_elem = -(s16)(sizeof (((SRT_NAME *)0)->v) / sizeof (*((SRT_NAME *)0)->v)), .type = t,   \
		.emode = e }
...
#define DEF_SRT_ARRAY_FLC(f, l)                                                                    \
	{ offsetof (SRT_NAME, f), sizeof (((SRT_NAME *)0)->f), 0,                                      \
		.n_elem                                                                                    \
		= -(s16)((offsetof (SRT_NAME, l) - offsetof (SRT_NAME, f)) / sizeof (((SRT_NAME *)0)->f)   \
			+ 1),                                                                                  \
		.type = SRT_DEF_ARRAY }
...
#define DEF_SRT_ARRAY_GA(v)                                                                        \
	{ 0, sizeof (*v), 0, .n_elem = sizeof (v) / sizeof (*v), .type = SRT_DEF_ARRAY }
...
void DumpStateTable (FILE *f, // valid output file
	int indent,
	const SaveRestoreTab_t *srt // list of variables
);

void SaveCurrentStateByTable (FILE *f, // valid output file
...
void RestoreStateByTable (RestoreState_t *rs, // info data, can be modified (cleaned after call)
...
void LogGrowBuffer (FILE *f, // output file
...
void SaveCurrentStateGrowBuffer (FILE *f, // output file
	ccp name_prefix, // NULL or prefix of name
	uint tab_pos, // tab pos of '='
	const GrowBuffer_t *gb // valid grow-buffer
);

void RestoreStateGrowBuffer (GrowBuffer_t *gb, // valid grow-buffer
	ccp name_prefix, // NULL or prefix of name
	RestoreState_t *rs // info data, can be modified (cleaned after call)
);
...
static inline uint CreateUniqueId (void)
{
	return CreateUniqueIdN (1);
}
static inline uint CreateUniqueIdNBO (void)
{
	return htonl (CreateUniqueIdN (1));
}
