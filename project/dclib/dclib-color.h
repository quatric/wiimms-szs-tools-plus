...
ColorMode_t NormalizeColorModeByTerm (ColorMode_t col_mode, // predefind mode
	const struct TerminalInfo_t *term // if NULL: use getenv("TERM") to find terminal
);
...
typedef struct ColorView_t
{
	...
	const struct TerminalInfo_t *term; // not NULL: us it to find 'colset'
