...
{
	return tid < MKW_ARENA_END || tid >= MKW_EXTRA_BEG && tid < MKW_EXTRA_END
...
{
	return tid < MKW_ARENA_END || tid >= MKW_LE_SLOT_BEG && tid < MKW_LE_SLOT_END;
}
...
static inline uint NextLecodeSlotRnd (uint slot)
{
	return slot == MKW_ARENA_END - 1	? MKW_LE_RANDOM_BEG
		: slot == MKW_LE_RANDOM_END - 1 ? MKW_LE_SLOT_BEG
										: slot + 1;
}
...
static inline u8 GetMkwPoints (uint n_player, uint idx_player)
{
	...
	n_player--;
	idx_player--;
	return n_player < 12 && idx_player < 12 ? MkwPointsTab[12 * n_player + idx_player] : 0;
}
...
static inline ccp PrintLEFLcol (
	le_flags_t flags_bits, le_flags_t flags, bool aligned, ColorSet_t *col)
{
