...
#define SETUP_TRY_OPEN_FILE(f, fname, mode)                                                        \
	static FILE *f = TRY_OPEN_FILE;                                                                \
	f = TryOpenFile (f, fname, mode)
...
int PrintLogFile (LogFile_t *lf, ccp format, ...) __attribute__ ((__format__ (__printf__, 2, 3)));
...
typedef enum FileMode_t
{
	...
	FM_M_OPEN = FM_TEST | FM_SILENT | FM_SILENT_ON_LIMIT | FM_IGNORE | FM_MODIFY | FM_STDIO | FM_DEV
		| FM_SPC | FM_TCP | FM_TEMP,

	// relevant for CreateFile()
	FM_M_CREATE = FM_TEST | FM_SILENT | FM_MODIFY | FM_APPEND | FM_UPDATE | FM_OVERWRITE | FM_NUMBER
		| FM_REMOVE | FM_MKDIR | FM_STDIO | FM_DEV | FM_SOCK | FM_SPC | FM_TOUCH | FM_TEMP,

	FM_M_ALL = FM_M_OPEN | FM_M_CREATE
} FileMode_t;
...
static inline int CompareTimeSpec (const struct timespec *a, const struct timespec *b)
{
	DASSERT (a);
	DASSERT (b);
...
// NULL pointers forbidden
static inline int CompareTimeSpecVal (const struct timespec *a, const struct timeval *b)
{
	DASSERT (a);
	DASSERT (b);
	return a->tv_sec < b->tv_sec		 ? -1
...
{
	DASSERT (a);
	return a->tv_sec < tim ? -1 : a->tv_sec > tim ? 1 : a->tv_nsec > 0;
}

static inline bool IsTimeSpecNull (const struct timespec *ts)
{
	return !ts || (unsigned long)ts->tv_nsec > 999999999 // includes UTIME_NOW and UTIME_OMIT
...
static inline FileAttrib_t *UseFileAttrib (FileAttrib_t *dest, // valid destination attribute
	const FileAttrib_t *src_fa, // NULL or source attribute
	const struct stat *src_stat, // NULL or source attribute
...
static inline enumError AppendFile (File_t *f, // file structure
	bool initialize, // true: initialize 'f'
	ccp fname, // file to open
	FileMode_t file_mode // open modes
)
...
static inline enumError WriteMemFile (MemFile_t *mf, // valid MemFile_t data
	const void *data, // data to write
	uint size // size to write
)
{
...
static inline uint ReadMemFile (MemFile_t *mf, // valid MemFile_t data
	void *buf, // destination buffer
	uint size // size to read
)
{
...
typedef enum FindConfigFile_t
{
	...
	FCF_STD_SHARE = FCF_REL_SHARE | FCF_LOC_SHARE | FCF_USR_SHARE,

	FCF_STD_PROG = FCF_PROG_SHARE | FCF_PROG,
...
typedef enumError (*SearchPathsFunc) (mem_t path, // full path of existing file, never NULL
...
typedef int (*CatchOutputFunc) (struct CatchOutput_t *ctrl, // control struct incl. data
