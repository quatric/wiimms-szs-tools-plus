...
#define SRT_NAME MySql_t
	...
	DEF_SRT_COMMENT ("mysql stats"), DEF_SRT_SEPARATOR (),

	DEF_SRT_MODE_ADD (), DEF_SRT_UINT (total_connect_count, "mystat-connect"),
...
void SetupQueryLogMYSQL (ccp fname, // filename of log file, openend with mode "a"
...
enumError OpenMYSQL4 (MySql_t *my, uint loglevel)
{
	...
	PRINT0 ("connect(%p,%s,%s,%s,%s,%u,,)\n", my->mysql, host.name, my->user, my->password,
		my->database, host.port);
	MYSQL *stat = mysql_real_connect (
		my->mysql, host.name, my->user, my->password, my->database, host.port, NULL, 0);
...
enumError OpenMYSQL (MySql_t *my, uint loglevel)
{
	...
	PRINT0 ("connect(%p,%s,%s,%s,%s,%u,,)\n", my->mysql, my->server, my->user, my->password,
		my->database, port);
	MYSQL *stat = mysql_real_connect (
		my->mysql, my->server, my->user, my->password, my->database, port, NULL, 0);
...
void DefineDatabase5 (MySql_t *my, // valid struct
...
void DefineDatabasePrefix (MySql_t *my, // valid struct
...
static void update_query_duration (MySql_t *my, u_nsec_t start_time, ccp query, uint qlen)
{
	DASSERT (my);
		...
		if (lf)
		{
			...
			*dest = 0;
			...
			PrintLogFile (lf, "%5u %s : %s\n", qlen,
				PrintTimerNSec6 (0, 0, dur_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH), buf);
		}
	}
}
...
enumError DoQueryMYSQL (MySql_t *my, // valid struct
		...
		if (fail)
		{
			if (loglevel >= MYLL_QUERY_ON_ERROR)
				ERROR0 (ERR_DATABASE, "%s (#%d)\n--QUERY--\n%.*s\n", my->status.message,
					my->status.status, query_length, query);
...
enumError PrintArgQueryMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
...
enumError PrintQueryMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
	ccp format, // format string for vsnprintf()
...
enumError DoMultiQueryMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
	ccp query, // query string
	int query_length // length of 'query', -1 if unknown
)
...
enumError PrintMultiQueryMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
	ccp format, // format string for vsnprintf()
	... // parameters for 'format'
)
...
s64 FetchIntMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
...
u64 FetchUIntMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
	bool allow_trail, // allow additional chars behind number
...
s64 PrintFetchIntMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
	bool allow_trail, // allow additional chars behind number
	s64 return_default, // return this value on non existent result
	ccp format, // format string for vsnprintf()
...
u64 PrintFetchUIntMYSQL (MySql_t *my, // valid struct
	LogLevelMYSQL loglevel, // log level for mysql errors and queries
	bool allow_trail, // allow additional chars behind number
	u64 return_default, // return this value on non existent result
	ccp format, // format string for vsnprintf()
...
int CountTablesMYSQL (MySql_t *my, // valid DB connection
...
int CountColumnsMYSQL (MySql_t *my, // valid DB connection
...
void AssignStatusMYSQL (MySqlStatus_t *stat, // valid struct
...
void CopyStatusMYSQL (MySqlStatus_t *dest, // NULL or initialized status
	bool init_dest, // true: initialize 'dest'
...
void MoveStatusMYSQL (MySqlStatus_t *dest, // NULL or initialized status
	bool init_dest, // true: initialize 'dest'
...
void PrintStatusMYSQL (MySqlStatus_t *stat, // valid struct
	int stat_code, // error status to assign
	ccp format, // format string for vsnprintf()
	... // parameters for 'format'
)
...
enumError GetMySqlServerStats (MySql_t *my, MySqlServerStats_t *stat)
{
	...
	static const char query[] = "show status where variable_name in"
								" ('Uptime','Max_used_connections','Connections','Queries')";

	enumError err = DoQueryMYSQL (my, MYLL_QUERY_ON_ERROR, query, sizeof (query) - 1);
	if (!err)
	{
	...
	if (dest)
	{
		...
		else
		{
			...
			dest->max_connections = src1->max_connections > src2->max_connections
				? src1->max_connections
				: src2->max_connections;
