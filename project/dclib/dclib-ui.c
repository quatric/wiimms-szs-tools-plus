...
enumError RegisterOptionByIndex (const InfoUI_t *iu, // valid pointer
...
)
{
	...
	if (opt_index >= 0 && opt_index < iu->n_opt_total && opt_index < UIOPT_INDEX_SIZE)
	{
		TRACE ("OPT: RegisterOptionByIndex(,%02x,%d,%d) opt_index=%02x,%s\n", opt_index, level,
			is_env, opt_index, iu->opt_info[opt_index].long_name);
	...
	PRINT ("OPT: RegisterOptionbyIndex(,%02x/%02x/%02x,%d,%d) => WARNING\n", opt_index,
		iu->n_opt_total, UIOPT_INDEX_SIZE, level, is_env);
	return ERR_WARNING;
}

///////////////////////////////////////////////////////////////////////////////

enumError RegisterOptionByName (const InfoUI_t *iu, // valid pointer
	...
	for (i = 0; i < iu->n_opt_total; i++)
	{
		...
		if (opt)
			fprintf (f, "%*s%s %s %2u* [%02x,%s]\n", indent, "",
				i <= iu->n_opt_specific ? "CMD" : "GLB", opt & 0x80 ? "PRM" : "ENV",
				opt & UIOPT_USED_MASK, i, iu->opt_info[i].long_name);
	}
};
...
void WarnDepractedOptions (const struct InfoUI_t *iu)
{
	...
	if (count)
		ERROR0 (ERR_WARNING, "Attention: Deprecated option%s:%s\n", count == 1 ? "" : "s", buf);
}
...
enumError CheckEnvOptions (ccp varname, check_opt_func func)
{
	...
	if (stat)
		fprintf (stderr, "Error while scanning the environment variable '%s'\n", varname);
...
void DumpText (FILE *f, // output file, if NULL: 'pbuf' must be set
...
static void print_single_option (FILE *f, // valid output stream
...
)
{
	...
	if (!io->short_name)
		len = fprintf (
			f, "%*s     --%s%s%s%s ", indent, "", io->long_name, sep, col_param, iop ? iop : "");
	else if (!io->long_name)
		len = fprintf (f, "%*s  -%c%s%s%s ", indent, "", io->short_name, iop ? " " : "", col_param,
			iop ? iop : "");
	else
		len = fprintf (f, "%*s  -%c --%s%s%s%s ", indent, "", io->short_name, io->long_name, sep,
			col_param, iop ? iop : "");
...
static inline bool is_opt_selected (const InfoUI_t *iu, // valid pointer
	const InfoOption_t *io, // active option
	bool use_filter // true: user filter
)
{
	DASSERT (io);
	return use_filter ? (iu->opt_used[io->id] & 0x80) != 0 : !io->hidden;
}

///////////////////////////////////////////////////////////////////////////////

static void print_help_options (const InfoUI_t *iu, // valid pointer
...
)
{
	...
	if (title)
	{
		...
		fprintf (f, "%*s%s%s:%s\n\n", indent, "", GetTextMode (colmode, TTM_COL_HEADING), title,
...
void PrintHelpOptions (const InfoUI_t *iu, // valid pointer
	FILE *f, // valid output stream
	int indent // indent of output
)
{
	...
	{
		print_help_options (
			iu, f, true, indent, 1, iu->n_opt_total, "Selected options with common description");
	}
	else
	{
		print_help_options (
			iu, f, false, indent, iu->n_opt_specific + 1, iu->n_opt_total, "Global options");

		print_help_options (iu, f, false, indent, 1, iu->n_opt_specific,
...
void PrintHelpCommands (const InfoUI_t *iu, // valid pointer
	FILE *f, // valid output stream
	int indent, // indent of output
...
)
{
	...
	for (ic = iu->cmd_info + 1; ic->name1; ic++)
		if (!ic->hidden)
		{
			...
			int len;
			if (ic->name2)
				len = fprintf (f, "%*s  %s%-*s%s | %s%-*s%s : ", indent, "", col_cmd, fw1,
						  ic->name1, col_reset, col_cmd, fw2, ic->name2, col_reset)
					- 2 * col_len;
			else
				len = fprintf (
						  f, "%*s  %s%-*s%s : ", indent, "", col_cmd, fw12, ic->name1, col_reset)
					- col_len;
...
void PrintHelp (const InfoUI_t *iu, // valid pointer
	FILE *f, // valid output stream
	int indent, // indent of output
	ccp help_cmd, // NULL or name of help command
			...
			else
			{
				...
				*dest = 0;
				...
				if (key_len >= 3)
				{
					...
					if (!memcmp (key_buf, "COMMANDS", key_len) || !memcmp (key_buf, "CMD", key_len))
					{
...
void PrintHelpCmd (const InfoUI_t *iu, // valid pointer
	FILE *f, // valid output stream
	int indent, // indent of output
...
)
{
	...
	if (ic->n_opt)
	{
		fprintf (f, "%*s%s%sptions:%s\n\n", indent, "", col_head,
			!cmd && iu->n_cmd ? "Global o" : "O", col_reset);
	...
	if (base_uri && *base_uri)
	{
		if (!cmd)
			fprintf (
				f, "%*sMore help is available at %s%s\n\n", indent, "", base_uri, iu->tool_name);
		else if (!ic->hidden)
		{
			char *dest = iobuf;
			ccp src;
