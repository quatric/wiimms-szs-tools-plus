...
)
{
	...
	if (!dest_addr)
	{
		...
		return recvfrom (sock, buf, buf_size, flags, (struct sockaddr *)src_addr, &src_len);
	}
...
)
{
	...
	TRACE ("Send %zu+%u bytes from %s to %s\n", sizeof (ip4_head_t) + sizeof (udp_head_t), size,
		PrintIP4sa (0, 0, sa_send), PrintIP4sa (0, 0, sa_recv));
	...
	if (log_mode >= 0x10)
		HexDump16 (stdwrn, 0, 0, &pkt, total_len < log_mode ? total_len : log_mode);
...
)
{
	...
	static const u8 ip4_head[] = { 0x45, 0x00, 0x00, 0x00, 0x12, 0x34, 0x40, 0x00, 0x40, 0x11 };
...
)
{
	...
	for (col = 0; *ptr; col++)
	{
		...
		if (!strcasecmp (beg, "Iface"))
			rt->col_iface = col;
		else if (!strcasecmp (beg, "Destination"))
			rt->col_dest = col;
		else if (!strcasecmp (beg, "Mask"))
			rt->col_mask = col;
		else if (!strcasecmp (beg, "Gateway"))
			rt->col_gate = col;
		else if (!strcasecmp (beg, "Flags"))
			rt->col_flags = col;
	}

	TRACE ("f=%p, COLS: if=%d, dest=%d, mask=%d, gate=%d, flags=%d\n", rt->f, rt->col_iface,
		rt->col_dest, rt->col_mask, rt->col_gate, rt->col_flags);
...
)
{
	...
	for (col = 0; *ptr; col++)
	{
		...
		if (col == rt->col_iface)
			rt->iface = beg;
		else if (col == rt->col_dest)
			rt->dest = strtoul (beg, 0, 16);
		else if (col == rt->col_mask)
			rt->mask = strtoul (beg, 0, 16);
		else if (col == rt->col_gate)
			rt->gate = strtoul (beg, 0, 16);
		else if (col == rt->col_flags)
			rt->flags = strtoul (beg, 0, 16);
	}
