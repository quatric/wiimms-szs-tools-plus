...
int GetLogTimestamp (char *buf, uint buf_size, TimestampMode_t ts_mode)
{
	...
	if (ts_mode >= TSM_USEC)
	{
		...
		len = snprintf (buf, buf_size, "%02u:%02u:%02u.%06llu ", sec / 3600, sec / 60 % 60,
			sec % 60, usec % 1000000);
	}
	else if (ts_mode >= TSM_MSEC)
	{
		...
		len = snprintf (buf, buf_size, "%02u:%02u:%02u.%03llu ", sec / 3600, sec / 60 % 60,
			sec % 60, msec % 1000);
	}
	else if (ts_mode >= TSM_SEC)
	{
		const uint sec = GetTimeSec (false) % SEC_PER_DAY;
		len = snprintf (buf, buf_size, "%02u:%02u:%02u ", sec / 3600, sec / 60 % 60, sec % 60);
	}
	else
	{
		*buf = 0;
...
int PutLogFile (LogFile_t *lf, ccp text, int text_len)
{
	...
	dest = StringCopyEM (dest, buf + sizeof (buf), text, text_len < 0 ? strlen (text) : text_len);
...
FILE *OpenPipeToPager ()
{
	...
	if (!done)
	{
		...
		static ccp search_tab[]
			= { ">DC_PAGER", ">DC_PAGER1", ">DC_PAGER2", ">PAGER", "less", "more",
#ifdef __CYGWIN__
				  "less.exe", "/usr/bin/less.exe", "more.exe", "/usr/bin/more.exe",
#endif
				  0 };
...
void ClosePagerFile ()
{
	if (pager_file)
	{
		...
		if (stderr == pager_file)
			stderr = 0;
		if (stdwrn == pager_file)
			stdwrn = 0;
		if (stdlog == pager_file)
			stdlog = 0;
		pclose (pager_file);
		pager_file = 0;
	}
...
ccp GetFileModeStatus (char *buf, // result buffer
...
)
{
	...
	static const struct info_t info[] = { { FM_TEST, 't', "test" }, { FM_SILENT, 's', "silent" },
		{ FM_IGNORE, 'i', "ignore" }, { FM_MODIFY, 'M', "modify" }, { FM_APPEND, 'A', "append" },
		{ FM_UPDATE, 'U', "update" }, { FM_OVERWRITE, 'O', "overwrite" },
		{ FM_NUMBER, 'N', "number" }, { FM_REMOVE, 'R', "remove" }, { FM_MKDIR, 'M', "mkdir" },
		{ FM_STDIO, 'S', "stdio" }, { FM_TCP, 'I', "tcp" }, { FM_DEV, 'D', "dev" },
		{ FM_SPC, 'P', "spc" }, { FM_TOUCH, 'H', "touch" }, { FM_TEMP, 'T', "temp" }, { 0, 0, 0 } };
...
ccp GetFileOpenMode (bool create, // false: open, true: create
...
FileAttrib_t *ClearFileAttrib (FileAttrib_t *dest // NULL or destination attribute
)
{
	if (dest)
	{
		memset (dest, 0, sizeof (*dest));
		dest->atime.tv_nsec = dest->mtime.tv_nsec = dest->ctime.tv_nsec = dest->itime.tv_nsec =
...
FileAttrib_t *TouchFileAttrib (FileAttrib_t *dest // valid destination attribute
)
{
	DASSERT (dest);
...
FileAttrib_t *SetFileAttrib (FileAttrib_t *dest, // valid destination attribute
...
#endif
			dest->itime
				= CompareTimeSpec (&dest->mtime, &dest->ctime) > 0 ? dest->mtime : dest->ctime;
...
FileAttrib_t *MaxFileAttrib (FileAttrib_t *dest, // valid source and destination attribute
...
)
{
	...
	if (src_fa)
	{
		if (CompareTimeSpec (&dest->atime, &src_fa->atime) < 0)
			dest->atime = src_fa->atime;
		if (CompareTimeSpec (&dest->mtime, &src_fa->mtime) < 0)
			dest->mtime = src_fa->mtime;
		if (CompareTimeSpec (&dest->ctime, &src_fa->ctime) < 0)
			dest->ctime = src_fa->ctime;
		if (CompareTimeSpec (&dest->itime, &src_fa->itime) < 0)
			dest->itime = src_fa->itime;
...
FileAttrib_t *NormalizeFileAttrib (FileAttrib_t *fa // valid attribute
)
{
	DASSERT (fa);
	...
	if (IsTimeSpecNull (&fa->itime))
		fa->itime = CompareTimeSpec (&fa->ctime, &fa->mtime) > 0 ? fa->ctime : fa->mtime;

	if (IsTimeSpecNull (&fa->ctime))
		fa->ctime = CompareTimeSpec (&fa->itime, &fa->mtime) > 0 ? fa->itime : fa->mtime;

	if (IsTimeSpecNull (&fa->atime))
		fa->atime = CompareTimeSpec (&fa->itime, &fa->ctime) > 0 ? fa->itime : fa->ctime;
...
)
{
	...
	{
		if (!silent)
			ERROR0 (ERR_CANT_CONNECT, "Path name to long for UNIX/STREAM socket: %s\n", fname);
		return -1;
	}
	...
	{
		if (!silent)
			ERROR1 (ERR_CANT_CONNECT, "Can't create UNIX/STREAM socket: %s\n", fname);
		return -1;
	}
	...
	{
		if (!silent)
			ERROR1 (ERR_CANT_CONNECT, "Can't connect to UNIX/STREAM socket: %s\n", fname);
		close (sock);
		return -1;
	}
...
)
{
	...
	{
		if (!silent)
			ERROR1 (ERR_CANT_CONNECT, "Invalid IPv4 address: %s\n", addr);
		return -1;
	}
	...
	{
		if (!silent)
			ERROR1 (ERR_CANT_CONNECT, "Can't connect to %s\n", PrintIP4 (0, 0, ip4, port));
		close (sock);
		return -1;
	}
...
void InitializeFile (File_t *f // file structure
)
{
	DASSERT (f);
	memset (f, 0, sizeof (*f));
...
enumError ResetFile (File_t *f, // file structure
...
enumError CloseFile (File_t *f, // file structure
	uint set_time // 0: don't set
				  // 1: set time before closing using 'fatt'
				  // 2: set current time before closing
)
	...
	if (f->f)
	{
		if (!f->is_stdio)
		{
			...
			if (fclose (f->f) && f->max_err <= ERR_WARNING)
				f->max_err = ERROR1 (ERR_WRITE_FAILED, "Error while closing file: %s\n", f->fname);
...
)
{
	...
	TRACE ("OpenFile(%p,%d,,%s,%llu) [%s] %s\n", f, initialize,
		GetFileModeStatus (0, 0, file_mode, 0), (u64)limit, GetFileModeStatus (0, 0, f->fmode, 1),
		fname);
	...
	if (limit && f->st.st_size > limit)
	{
		...
		if (strcmp (sbuf, lbuf))
		{
			if (!(f->fmode & (FM_SILENT | FM_SILENT_ON_LIMIT)))
				ERROR0 (ERR_FILE_TOO_BIG, "File too big (size=%s, limit=%s): %s\n%s", sbuf, lbuf,
					fname, limit_message ? limit_message : "");
...
)
{
	...
	if (!stat (fname, st))
	{
		if (S_ISBLK (st->st_mode) || S_ISCHR (st->st_mode))
		{
			...
			if (!(file_mode & FM_SILENT))
				ERROR0 (ERR_ALREADY_EXISTS, "Can't write to %s device: %s\n",
		...
		if (S_ISSOCK (st->st_mode))
		{
			...
			if (!(file_mode & FM_SILENT))
				ERROR0 (ERR_ALREADY_EXISTS, "Can't write to UNIX socket: %s\n", fname);
			return ERR_WRONG_FILE_TYPE;
		}
		...
		if (!S_ISREG (st->st_mode))
		{
			...
			if (!(file_mode & FM_SILENT))
				ERROR0 (ERR_WRONG_FILE_TYPE, "Not a plain file: %s\n", fname);
			return ERR_WRONG_FILE_TYPE;
		}
		...
		if (!(file_mode & (FM_OVERWRITE | FM_REMOVE | FM_UPDATE | FM_APPEND)))
		{
			...
			if (!(file_mode & FM_SILENT))
				ERROR0 (ERR_ALREADY_EXISTS, "File already exists: %s\n", fname);
	...
	else
	{
		...
		{
			if (!(file_mode & FM_SILENT))
				ERROR0 (ERR_CANT_CREATE, "Try to update non existing file: %s\n", fname);
...
)
{
	...
	noPRINT ("CreateFile(%p,%d,,%s) [%s] %s\n", f, initialize,
		GetFileModeStatus (0, 0, file_mode, 0), GetFileModeStatus (0, 0, f->fmode, 1), fname);
	...
	f->f = fopen (fname, omode);
	noPRINT ("CreateFile(%s,%s) : [%s], %p\n", fname, omode, GetFileModeStatus (0, 0, f->fmode, 1),
		f->f);
...
enumError WriteFileAt (File_t *F, // file to write
...
)
{
	...
	if (offset != *cur_offset)
	{
		...
		else if (offset < *cur_offset)
		{
			noPRINT ("FSEEK %zx -> %zx\n", *cur_offset, offset);
			return ERROR0 (
				ERR_WRITE_FAILED, "Can't set file pointer on non seekable file: %s\n", F->fname);
		}
		else
		{
			char buf[0x8000] = { 0 };
...
enumError SetFileSize (File_t *F, // file to write
	size_t *cur_offset, // pointer to current file offset, modified
...
)
{
	...
	if (ftruncate (fileno (F->f), size))
	{
		...
		return ERROR1 (ERR_WRITE_FAILED, "Set file size to %llu failed: %s\n", (u64)size, F->fname);
	}
	return ERR_OK;
}
...
enumError SkipFile (File_t *F, // file to write
...
enumError RegisterFileError (File_t *f, // file structure
...
s64 GetFileSize (ccp path1, // NULL or part 1 of path
...
enumError LoadFileAlloc (ccp path1, // NULL or part #1 of path
	ccp path2, // NULL or part #2 of path
	size_t skip, // skip num of bytes before reading
...
enumError OpenWriteFile (File_t *f, // file structure
	bool initialize, // true: initialize 'f'
...
uint CloseAllExcept (int *list, // exception list of unordered FD, max contain -1
...
)
{
	...
	enum
	{
		LOCAL_LIST_SIZE = 1000
	};
	int local_list[LOCAL_LIST_SIZE];
	int *except_list
		= n_list < LOCAL_LIST_SIZE ? local_list : MALLOC ((n_list + 1) * sizeof (*except_list));
...
enumError CopyFileHelper (ccp src, // source path
...
enumError CopyFile (ccp src, // source path
	ccp dest, // destination path
	mode_t open_mode // mode for open()
)
{
...
enumError CopyFileCreate (ccp src, // source path
	ccp dest, // destination path
	mode_t open_mode // mode for open()
)
{
...
enumError CopyFileTemp (ccp src, // source path
	ccp dest, // destination path
	mode_t open_mode // mode for open()
)
{
...
enumError TransferFile (LogFile_t *log, // not NULL: log activities
	ccp dest, // destination path
	ccp src, // source path
...
)
{
	...
	if (tfer_mode & TFMD_J_MOVE1)
	{
		...
		struct stat st;
		if (!stat (src, &st) && st.st_nlink == 1 && (testmode || !rename (src, dest)))
		{
			if (log)
				PrintLogFile (
	...
	if (tfer_mode & TFMD_J_COPY)
	{
		...
		{
			enumError err = tfer_mode & TFMD_J_RM_DEST ? CopyFileTemp (src, dest, open_mode)
...
{
	return mf && (mf->data || !mf->size) && mf->size >= mf->fend && mf->fend >= mf->fpos;
}

///////////////////////////////////////////////////////////////////////////////

void DumpMemFile (FILE *f, // valid output stream
...
)
{
	...
	fprintf (f, "%*sfpos=0x%x/0x%x, size=0x%x/0x%x, zero=%d, valid=%d\n", indent, "", mf->fpos,
		mf->fend, mf->size, mf->max_size, mf->zero_extend, IsValidMemFile (mf));
}

///////////////////////////////////////////////////////////////////////////////

bool AssertMemFile (MemFile_t *mf // valid MemFile_t data
)
{
...
void InitializeMemFile (MemFile_t *mf, // structure to initialize
...
void ResetMemFile (MemFile_t *mf // valid MemFile_t data
)
{
	DASSERT (mf);
	FREE (mf->data);
...
enumError WriteMemFileAt (MemFile_t *mf, // valid MemFile_t data
...
uint ReadMemFileAt (MemFile_t *mf, // valid MemFile_t data
	uint fpos, // fpos to read from
...
enumError LoadMemFile (MemFile_t *mf, // MemFile_t data
...
enumError SaveMemFile (MemFile_t *mf, // MemFile_t data
...
void RedirectLineBuffer (LineBuffer_t *lb, // valid line buffer
...
LineBuffer_t *OpenLineBuffer (LineBuffer_t *lb, // line buffer; if NULL, malloc() one
...
void FixMemListLineBuffer (LineBuffer_t *lb, bool force)
{
	...
	if (!lb->buf.used)
		lb->used_lines = 0;
	else if (force || lb->prev_buf_ptr != lb->buf.ptr || lb->prev_seq_count != lb->seq_count)
	{
		u8 *beg = lb->buf.ptr;
...
)
{
	...
	if (share_name && !*share_name)
		share_name = ProgInfo.progname;
	if (file_name && !*file_name)
		file_name = 0;
	if (prog_ext && !*prog_ext)
		prog_ext = 0;
		...
		if (ppath)
		{
			if (mode & FCF_PROG_SHARE)
			{
				...
				if (pdir)
				{
					...
					if (prog_ext)
						snprintf (path, sizeof (path), "%s/share/%s%s", pdir, fname, prog_ext);
					else
						snprintf (path, sizeof (path), "%s/share/%s.%s", pdir, fname, file_name);
					if (mode & FCF_F_DEBUG)
						fprintf (stderr, "SEARCH: %s\n", path);
					if (!stat (path, &st) && S_ISREG (st.st_mode))
						return STRDUP (path);
...
int SearchFiles (ccp path1, // not NULL: part #1 of base path
...
enumError CreatePath (ccp path, // path to create
		...
		*dest = 0;
		if (mkdir (buf, 0777) && errno != EEXIST && !IsDirectory (buf, 0))
		{
			...
			return ERROR1 (ERR_CANT_CREATE_DIR,
				errno == ENOTDIR ? "Not a directory: %s\n" : "Can't create directory: %s\n", buf);
		}
...
enumError RemoveSource (ccp fname, // file to remove
...
)
{
	...
	if (print_log || testmode)
		fprintf (stdlog ? stdlog : stdout, "%sREMOVE %s\n", testmode ? "WOULD " : "", fname);
...
)
{
	...
	PRINT ("|%s| : |%.*s|%.*s|#%u|%.*s|\n", search_dir, dir_len, dir, name_len, name, index,
		ext_len, ext);
	...
	if (fdir)
	{
		for (;;)
		{
			...
			if (fname_len > name_len + ext_len && !strncasecmp (name, fname, name_len))
			{
...
static char *GetBlockDeviceHolderHelper (char *buf, char *endbuf, ccp name, ccp sep, int *level)
{
	DASSERT (buf);
	DASSERT (endbuf);
	DASSERT (level);
...
ccp GetBlockDeviceHolder (ccp name, ccp sep, int *ret_level)
{
	...
	if (!old_found)
	{
		int level = 0;
		char buf[1000];
		ccp end = GetBlockDeviceHolderHelper (
			buf, buf + sizeof (buf) - strlen (sep) - 1, name, sep, &level);
...
static void search_paths_dir (search_paths_t *sp, ParamField_t *collect, int min, int max)
{
	...
	if (fdir)
	{
		for (;;)
		{
			...
			if (dent->d_name[0] == '.'
				&& (!(local.wc_mode & WM_HIDDEN) || !dent->d_name[1]
...
static void search_paths_helper (search_paths_t *sp)
{
	...
	char *path_ptr
		= StringCopyEM (local.path_ptr, local.path_end, local.source.ptr, pat - local.source.ptr);
	...
	PRINT0 ("SPLIT: %s  %s%s%s  %.*s  [want_dir=%d]\n", local.path_buf, colerr->highlight,
		pattern.ptr, colerr->reset, local.source.len, local.source.ptr, want_dir);
...
search_paths_stat_t SearchPaths (ccp path1, // not NULL: part #1 of base path
	ccp path2, // not NULL: part #2 of base path
	wildcard_mode_t wc_mode, // bit field: modes for wildcard handling
...
)
{
	...
	else
	{
		...
		PRINT0 ("abort=%d, max_err = %d, %u dirs scanned, %u func calls, %u hits\n", sp.abort,
			sp.max_err, sp.dir_count, sp.func_count, sp.hit_count);
	}
	return sp.status;
}
...
static enumError insert_string_field (mem_t path, // full path of existing file, never NULL
...
static enumError append_string_field (mem_t path, // full path of existing file, never NULL
	uint st_mode, // copy of struct stat.st_mode, see "man 2 stat"
	void *param // user defined parameter
)
{
...
static enumError insert_param_field (mem_t path, // full path of existing file, never NULL
	uint st_mode, // copy of struct stat.st_mode, see "man 2 stat"
	void *param // user defined parameter
)
{
...
static enumError append_param_field (mem_t path, // full path of existing file, never NULL
	uint st_mode, // copy of struct stat.st_mode, see "man 2 stat"
	void *param // user defined parameter
)
{
...
{
	if (src && src[1] == ':' && (*src >= 'a' && *src <= 'z' || *src >= 'A' && *src <= 'Z'))
	{
		if (!src[2])
			return 2;
...
)
{
	...
	if ((*src >= 'a' && *src <= 'z' || *src >= 'A' && *src <= 'Z') && src[1] == ':'
		&& (src[2] == 0 || src[2] == '/' || src[2] == '\\'))
	{
...
void DumpSearchFile (
	FILE *f, int indent, const search_file_list_t *sfl, bool show_symbols, ccp info)
{
	if (f && sfl && (sfl->used || show_symbols && sfl->symbols.used))
	{
		...
		fprintf (f,
			"%*s%s%s"
			"SearchList %p, N=%u/%u\n",
			indent, "", info ? info : "", info ? " : " : "", sfl, sfl->used, sfl->size);

		char buf[20];
		const int fw_idx
			= indent + snprintf (buf, sizeof (buf), "%d", sfl->used) + (show_symbols ? 3 : 1);

		uint idx = 0;
		search_file_t *ptr, *end = sfl->list + sfl->used;
		for (ptr = sfl->list; ptr < end; ptr++, idx++)
			fprintf (f, "%*d [%c,%c:%02x] %s\n", fw_idx, idx, ptr->alloced ? 'A' : '-',
...
)
{
	...
	for (; n_list > 0; n_list--, list++)
	{
		...
		search_file_t *sf = AppendSearchFile (
			sfl, res.data.ptr, res.is_alloced ? CPM_MOVE : CPM_COPY, config_fname);
		if (sf)
		{
			sf->hint |= hint;
...
)
{
	...
	if (add_search (
			sfl, config_fname, xdg_home, n_xdg_home, "$(xdg_home)", CONF_HINT_HOME, stop_mode)
		> 1)
		return true;
	...
	if (add_search (sfl, config_fname, install, n_install, "$(install)", CONF_HINT_INST, stop_mode)
		> 1)
		return true;
...
static s_usec_t prepare_wait_fdl (FDList_t *fdl)
{
	...
	if (fdl->timeout_nsec)
	{
		const u_usec_t wait_until
			= ((s_usec_t)fdl->timeout_nsec - (s_usec_t)GetTimerNSec ()) / NSEC_PER_USEC + now_usec
			+ 1;
...
int WaitFDList (FDList_t *fdl // valid socket list
)
{
	DASSERT (fdl);
	...
	else
	{
		...
		if (fdl->debug_file)
		{
			if (ptv)
				fprintf (fdl->debug_file, "# SELECT: usec=%lld, Δnow=%lld, timeout=%llu.%06llu\n",
					fdl->timeout_usec, fdl->timeout_usec - now_usec, (u64)ptv->tv_sec,
					(u64)ptv->tv_usec);
			else
		...
		stat = select (fdl->max_fd + 1, &fdl->readfds, &fdl->writefds, &fdl->exceptfds, ptv);
	}
...
int PWaitFDList (FDList_t *fdl, // valid socket list
...
#ifdef __APPLE__
	const int stat
		= pselect (fdl->max_fd + 1, &fdl->readfds, &fdl->writefds, &fdl->exceptfds, pts, sigmask);
#else
...
enumError CatchOutput (ccp command, // command to execute
	...
	else if (fork_stat)
	{
		...
		for (;;)
		{
			...
			PRINT ("RESUME: %d = %x: exited=%d, wait_stat=%d [%s]\n", wait_stat, wait_stat,
				WIFEXITED (wait_stat), WEXITSTATUS (wait_stat),
				GetErrorName (WEXITSTATUS (wait_stat), "?"));
			if (pid == fork_stat)
				break;
		}
...
enumError CatchOutputLine (ccp command_line, // command line to execute
	CatchOutputFunc stdout_func, // NULL or function to catch stdout
	CatchOutputFunc stderr_func, // NULL or function to catch stderr
	void *user_ptr, // NULL or user defined parameter
	bool silent // true: suppress error messages
)
{
	...
	if (sa.argc > 1)
		err = CatchOutput (
			sa.argv[1], sa.argc - 2, sa.argv + 2, stdout_func, stderr_func, user_ptr, silent);
	ResetSplitArg (&sa);
	return err;
}
				...
				if (!ptr->num)
				{
					...
					if (len > 20)
						fprintf (log_file, "! #RESTORE[%s]: Unused: %s [%u] %.20s...\n",
							PrintRestoreStateSection (&rs), ptr->key, len, (ccp)ptr->data);
					else
...
)
{
	...
	ParamFieldItem_t *it = GetParamField (rs, name);
	return it ? DecodeByModeMem (buf, buf_size, (ccp)it->data, -1, decode, 0) : not_found;
}
...
)
{
	...
	indent = NormalizeIndent (indent);
	printf ("%*s off size *n(e) ty em name\n"
			"%*s----------------------------------\n",
		indent, "", indent, "");
	...
	for (; srt->type != SRT__TERM; srt++)
	{
		if (srt->type == SRT__IS_LIST)
		{
			...
			fprintf (f, "%*s%4u %4u *%-4d %2u %2u %s\n", indent, "", srt->offset, srt->size,
				srt->n_elem, srt->type, srt->emode,
				srt->name						 ? srt->name
					...
					for (i = 0; i < n_elem; i++)
					{
						...
						*val = 0;
						if (i < ml.used)
						{
							...
							if (m->ptr)
								*val
									= m->ptr == EmptyString ? EmptyString : MEMDUP (m->ptr, m->len);
						}
...
)
{
	...
	static const KeywordTab_t keytab[] = { { -1, "STREAM", 0, SOCK_STREAM },
		{ -1, "DGRAM", "DATAGRAM", SOCK_DGRAM },

		{ AF_UNIX, "UNIX", "FILE", -1 }, { AF_UNIX, "UNIX-TCP", "FILE-TCP", SOCK_STREAM },
		...
		{ AF_UNIX, "UNIX-SEQ", "FILE-SEQ", SOCK_SEQPACKET }, { AF_UNIX, "SEQ", 0, SOCK_SEQPACKET },

		{ AF_INET_ANY, "IP", 0, -1 }, { AF_INET_ANY, "TCP", 0, SOCK_STREAM },
		{ AF_INET_ANY, "UDP", 0, SOCK_DGRAM }, { AF_INET_ANY, "RAW", 0, SOCK_RAW },

		{ AF_INET, "IP4", "IPV4", -1 }, { AF_INET, "TCP4", "TCPV4", SOCK_STREAM },
		{ AF_INET, "UDP4", "UDPV4", SOCK_DGRAM }, { AF_INET, "RAW4", "RAWV4", SOCK_RAW },

		{ AF_INET6, "IP6", "IPV6", -1 }, { AF_INET6, "TCP6", "TCPV6", SOCK_STREAM },
		{ AF_INET6, "UDP6", "UDPV6", SOCK_DGRAM }, { AF_INET6, "RAW6", "RAWV6", SOCK_RAW },
	...
	if (proto < 0 && tolerance > 0)
	{
		...
		if (tolerance > 1)
		{
			...
			while (*ptr)
			{
				...
				if (ch == ':')
					count++;
				else if (ch != '.' && !(ch >= '0' && ch <= '9') && !(ch >= 'A' && ch <= 'F')
					&& !(ch >= 'a' && ch <= 'f'))
				{
					break;
				}
...
ccp PrintOpenFiles (bool count_current)
{
	...
	return PrintCircBuf ("cur=%u, max=%u, limit=%u/%u", stat_file_count.cur_files,
		stat_file_count.max_files, stat_file_count.cur_limit, stat_file_count.max_limit);
};
...
int PutScriptVars (PrintScript_t *ps, // valid control struct
...
)
{
	...
	if (mode & 1)
	{
		...
		if (ps->add_index)
			snprintf (ps->prefix, sizeof (ps->prefix), "%s%u", ps->var_prefix, ps->index);
		else
	...
	for (;;)
	{
		...
		switch (ps->fform)
		{
			...
			case PSFF_JSON:
				fprintf (f, "%s\"%.*s\":%s%.*s%s", ps->count ? "," : "", varlen, varname, quote,
					plen, param, quote);
				break;
			...
			case PSFF_BASH:
				if (ps->create_array)
				{
					fprintf (f, "%s%.*s%s=(%s%.*s%s)\n", ps->prefix, varlen, varname,
						ps->index ? "+" : "", quote, plen, param, quote);
					break;
				}
				// fall through
			...
			default:
				fprintf (f, "%*.*s = %s%.*s%s\n", ps->var_size > 0 ? ps->var_size : 20, varlen,
					varname, quote, plen, param, quote);
				break;
		}
		ps->count++;
		if (free_param)
...
int PrintScriptVars (PrintScript_t *ps, // valid control struct
	uint mode, // bit field: 1=open var, 2:close var
...
CpuStatus_t GetCpuStatus (CpuStatus_t *prev_cpuinfo)
{
	...
	CpuStatus_t current
		= { .valid = true, .nsec = GetTimerNSec (), .n_cpu = n_cpu, .clock_ticks = clock_ticks };
...
ccp PrintCpuStatus (const CpuStatus_t *cpuinfo)
{
	...
	return PrintCircBuf ("ticks=%d, cpus=%d, times=%lld+%lld+%lld+%lld+%lld+%lld+%lld+%lld=%lld, "
						 "avg=%4.2f,%4.2f,%4.2f %u/%u, nsec=%llu",
		cpuinfo->clock_ticks, cpuinfo->n_cpu, cpuinfo->user, cpuinfo->nice, cpuinfo->system,
		cpuinfo->idle, cpuinfo->iowait, cpuinfo->irq, cpuinfo->virtual, cpuinfo->unknown,
		cpuinfo->user + cpuinfo->nice + cpuinfo->system + cpuinfo->idle + cpuinfo->iowait
			+ cpuinfo->irq + cpuinfo->virtual + cpuinfo->unknown,
		cpuinfo->loadavg[0], cpuinfo->loadavg[1], cpuinfo->loadavg[2], cpuinfo->running_proc,
		cpuinfo->total_proc, cpuinfo->nsec);
}
		...
		while (count > 0 && fgets (buf, sizeof (buf), f))
		{
			...
			if (colon)
			{
				...
				switch (colon - buf)
				{
					...
					case 7:
						if (!memcmp (buf, "MemFree", 7))
						{
							count--;
							mem.free = num;
							break;
						}
						if (!memcmp (buf, "Buffers", 7))
						{
							count--;
							mem.buffers = num;
							break;
...
ccp PrintMemoryStatus (const MemoryStatus_t *ms)
{
	...
	return PrintCircBuf ("tot=%s, free=%s, avail=%s, used=%s, buf=%s, cache=%s\n",
		PrintNumberU7 (0, 0, ms->total, mode), PrintNumberU7 (0, 0, ms->free, mode),
		PrintNumberU7 (0, 0, ms->avail, mode), PrintNumberU7 (0, 0, ms->used, mode),
		PrintNumberU7 (0, 0, ms->buffers, mode), PrintNumberU7 (0, 0, ms->cached, mode));
}
