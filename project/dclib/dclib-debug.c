...
ccp GetErrorName (int stat, // error status, err := abs(stat)
...
)
{
	...
	switch (stat)
	{
		...
		case ERR_SOURCE_FOUND:
			return "SOURCE FOUND";
		case ERR_NO_SOURCE_FOUND:
			return "NO SOURCE";
		case ERR_JOB_IGNORED:
			return "JOB IGNORED";
		case ERR_SUBJOB_WARNING:
			return "SUB JOB WARNINGS";
		case ERR_NOT_EXISTS:
			return "NOT EXISTS";
		...
		case ERR_SUBJOB_FAILED:
			return "SUB JOB FAILED";

		case ERR_CANT_REMOVE:
			return "CAN'T REMOVE FILE";
		case ERR_CANT_RENAME:
			return "CAN'T RENAME FILE";
		case ERR_CANT_CLOSE:
		...
		case ERR_CANT_APPEND:
			return "CAN'T APPEND FILE";
		case ERR_CANT_CREATE:
			return "CAN'T CREATE FILE";
		case ERR_CANT_CREATE_DIR:
			return "CAN'T CREATE DIRECTORY";

		case ERR_READ_FAILED:
			return "READ FILE FAILED";
		case ERR_REMOVE_FAILED:
...
ccp GetErrorText (int stat, // error status, err := abs(stat)
	ccp ret_not_found // not NULL: return this, if no message is found
					  // GENERIC_ERROR_MESSAGE: return a generic message
)
{
	...
	switch (stat)
	{
		...
		case ERR_JOB_IGNORED:
			return "Job ignored";
		case ERR_SUBJOB_WARNING:
			return "Sub job had warnings";
		case ERR_NOT_EXISTS:
			return "File does not exists";

		case ERR_WARNING:
			return "Unspecific warning";
		...
		case ERR_CANT_CONNECT:
			return "Can't connect";
		case ERR_FILE_TOO_BIG:
			return "Can't open, because file is too big";
		case ERR_CANT_OPEN:
			return "Can't open file";
		case ERR_CANT_APPEND:
			return "Can't open file for appending";
		case ERR_CANT_CREATE:
			return "Can't create file";
		case ERR_CANT_CREATE_DIR:
			return "Can't create directory";
		...
		case ERR_REMOVE_FAILED:
			return "Removing a file failed";
		case ERR_WRITE_FAILED:
			return "Writing to file failed";
		case ERR_DATABASE:
			return "Access to database failed";

		case ERR_MISSING_PARAM:
			return "Missing at least one parameter";
		case ERR_SEMANTIC:
			return "Semantic error";
		case ERR_SYNTAX:
			return "Syntax error";
		...
		case ERR_INTERNAL:
			return "Internal error";
		case ERR_OUT_OF_MEMORY:
			return "Allocation of dynamic memory failed";

		case ERR_FATAL:
			return "Unspecific fatal error";
	}
...
void ListErrorCodes (FILE *f, // valid output stream
...
)
{
	...
	fprintf (f,
		"%*s err | %-*s | %-*s\n"
		"%*s-----+%.*s+%.*s\n",
		indent, "", fw_name, "name", fw_text, "message", indent, "", fw_name + 2, Minus300,
		fw_text + 2, Minus300);
	...
	for (i = 0; i < ERR__N; i++)
	{
		...
		if (name && *name || text && *text)
			fprintf (f, "%*s%4u | %-*s | %-*s\n", indent, "", i, fw_name, name ? name : "-",
				fw_text, text ? text : "-");

		if (i == ERR_OK || i == ERR_WARNING || i == ERR_ERROR)
			fprintf (f, "%*s-----+%.*s+%.*s\n", indent, "", fw_name + 2, Minus300, fw_text + 2,
				Minus300);
	}

	fprintf (f, "%*s-----+%.*s+%.*s\n\n", indent, "", fw_name + 2, Minus300, fw_text + 2, Minus300);
}
...
enumError PrintError (
	ccp func, ccp file, uint line, int syserr, enumError err_code, ccp format, ...)
{
	va_list arg;
	va_start (arg, format);
...
enumError PrintErrorFile (ccp func, ccp file, uint line, struct File_t *F, int syserr,
	enumError err_code, ccp format, ...)
{
	va_list arg;
	va_start (arg, format);
	const int stat = PrintErrorArg (func, file, line, syserr, err_code, format, arg);
...
enumError PrintErrorArg (
	ccp func, ccp file, unsigned int line, int syserr, enumError err_code, ccp format, va_list arg)
{
	if (err_code > ERR_OK)
	{
		...
		if (err_code >= ERR_NOT_IMPLEMENTED || ProgInfo.error_level >= ERRLEV_EXTENDED)
		{
			if (err_code > ERR_WARNING)
				fprintf (stdwrn, "%s%s%s:%s ERROR #%d [%s] in %s() @ %s#%d%s\n", prefix, coln,
					ProgInfo.progname, col1, err_code, GetErrorName (err_code, 0), func, file, line,
					col0);
			else
				fprintf (stdwrn, "%s%s%s:%s WARNING in %s() @ %s#%d%s\n", prefix, coln,
					ProgInfo.progname, col1, func, file, line, col0);
		}
		else
		{
			if (err_code > ERR_WARNING)
				fprintf (stdwrn, "%s%s%s:%s ERROR #%d [%s]%s\n", prefix, coln, ProgInfo.progname,
					col1, err_code, GetErrorName (err_code, 0), col0);
			else
				fprintf (
					stdwrn, "%s%s%s:%s WARNING%s\n", prefix, coln, ProgInfo.progname, col1, col0);
		}
...
{
	if (verbose > 0 && err >= ERR_WARNING || verbose > 1 && err || err == ERR_NOT_IMPLEMENTED)
	{
		if (!stdwrn)
		{
			SetupStdMsg ();
		...
		fprintf (stdwrn, "%s: Command '%s' returns with status #%d [%s]\n", ProgInfo.progname,
			cmdname, err, GetErrorName (err, 0));
	}
	return err;
}
...
void PRINT_TIME (time_t time, ccp title)
{
	...
	PRINT ("%s: UTC= %4u-%02u-%02u %2u:%02u:%02u dst=%d"
		...
		title ? title : "TIME", utc.tm_year + 1900, utc.tm_mon + 1, utc.tm_mday, utc.tm_hour,
		utc.tm_min, utc.tm_sec, utc.tm_isdst, loc.tm_year + 1900, loc.tm_mon + 1, loc.tm_mday,
...
uint HexDump (
	FILE *f, int indent, u64 addr, int addr_fw, int row_len, const void *p_data, size_t count)
{
...
uint HexDump16 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump20 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump16BE2 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump20BE2 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump16LE2 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump20LE2 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump16BE4 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump20BE4 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump16LE4 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump20LE4 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
uint HexDump016 (FILE *f, int indent, u64 addr, const void *data, size_t count)
{
	// return the number of printed lines
...
void HexDiff16 (FILE *f, int indent, u64 addr, const void *data1, size_t count1, const void *data2,
	size_t count2)
{
...
void HexDiff (FILE *f, int indent, u64 addr, int addr_fw, int row_len, const void *p_data1,
	size_t count1, const void *p_data2, size_t count2)
{
	if (!f)
		return;
...
static void trace_helper (int print_stderr, ccp format, va_list arg)
{
	...
	if (last_day != dt.day)
	{
		...
		snprintf (pre, sizeof (pre), "\n#DAY: %s, pid=%d\n%02u:%02u:%02u.%03u ",
			PrintTimeByFormat ("%F %T %z", dt.time), getpid (), dt.hour, dt.min, dt.sec,
			dt.usec / 1000);
	}
	else
		snprintf (
			pre, sizeof (pre), "%02u:%02u:%02u.%03u ", dt.hour, dt.min, dt.sec, dt.usec / 1000);
...
#define PRINT_OOM(...) PrintError (func, file, line, 0, ERR_OUT_OF_MEMORY, __VA_ARGS__)
#else
#define MPARAM
#define MCALL
#define PRINT_OOM(...)                                                                             \
...
void *dclib_memdup3 (MPARAM cvp src1, size_t len1, cvp src2, size_t len2, cvp src3, size_t len3)
{
	char *res = dclib_malloc (MCALL len1 + len2 + len3 + 1);
	char *dest = res;
	if (len1)
...
static FILE *OpenMemLog ()
{
	...
	if (!done)
	{
		...
		if (ProgInfo.progname && *ProgInfo.progname && *ProgInfo.progname != '?')
			snprintf (
				fname, sizeof (fname), "_trace_alloc-%s-%u.tmp", ProgInfo.progname, getpid ());
		else
...
static void MemLog (ccp func, ccp file, uint line, ccp info, const u8 *ptr, bool no_stderr)
{
...
static void MemLogItem (ccp func, ccp file, uint line, ccp info, const mem_info_t *mi)
{
	...
	if (OpenMemLog ())
	{
		...
		fprintf (MEM_LOG_FILE, "[%s,%07u] %s() @ %s#%u\n", info, mi->seqnum, mi->func, mi->file,
			mi->line);
		if (mi->filler)
		{
			HexDump (MEM_LOG_FILE, 8, 0, 7, 16, mi->data - MEM_FILLER_SIZE, 2 * MEM_FILLER_SIZE);
...
int CheckTraceAlloc (ccp func, ccp file, uint line)
{
	...
	if (count)
	{
		fprintf (stderr, "MEM-ERR: %u error%s found -> %s() @ %s#%u\n", count,
			count == 1 ? "" : "s", func, file, line);
		fflush (stderr);

		if (OpenMemLog ())
		{
			fprintf (
				MEM_LOG_FILE, "--- %u errors found -> %s() @ %s#%u\n\n", count, func, file, line);
			fflush (MEM_LOG_FILE);
		}
	}
	return count;
...
mem_info_t *RegisterAlloc (ccp func, ccp file, uint line, cvp data, uint size, bool filler)
{
	if (!mem_seqnum)
...
uint trace_test_alloc (ccp func, ccp file, uint line, const void *p_data, bool hexdump)
{
#if TRACE_ALLOC_MODE > 2
	if (!p_data)
		return 0;
...
void *trace_memdup2 (ccp func, ccp file, uint line, cvp src1, size_t len1, cvp src2, size_t len2)
{
	char *res = trace_malloc (func, file, line, len1 + len2 + 1);
	char *dest = res;
...
void *trace_memdup3 (ccp func, ccp file, uint line, cvp src1, size_t len1, cvp src2, size_t len2,
	cvp src3, size_t len3)
{
	char *res = trace_malloc (func, file, line, len1 + len2 + len3 + 1);
	char *dest = res;
	if (len1)
...
void *trace_allocdup (ccp func, ccp file, uint line, const void *src, size_t copylen)
{
...
char *trace_printdup (ccp func, ccp file, uint line, ccp format, ...)
{
	char buf[2000];
