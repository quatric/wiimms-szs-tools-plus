...
enum
{
	...
	BMG_N_LE_RCUP = 0x800,
	BMG_N_LE_BCUP = 2,
	BMG_N_LE_TRACK = 0x2000,
	BMG_N_LE_ARENA = BMG_N_ARENA,
	BMG_LE_FIRST_CTRACK = 0x44,

	MID_LE_ALT_ARENA_BEG = 0x6400, // alternate BMG of MID_LE_TRACK_BEG for arenas
	MID_LE_ALT_ARENA_END = 0x6480,

	MID_LE_TRACK_BEG = 0x7000, // resevrved 0x7000 .. 0xafff
	...
	MID_LE_CUP_REF_BEG = 0xc000, // resevrved 0xc000 .. 0xffff
	MID_LE_CUP_REF_END = MID_LE_CUP_REF_BEG + BMG_N_LE_TRACK,

	MID_LE_BEG = MID_LE_ALT_ARENA_BEG,
	MID_LE_END = MID_LE_CUP_REF_END,

	//-- max(CT,LE)

	BMG_MAX_RCUP = BMG_N_LE_RCUP,
	BMG_MAX_BCUP = BMG_N_LE_BCUP,
	BMG_MAX_TRACK = BMG_N_LE_TRACK,
	BMG_MAX_ARENA = BMG_N_LE_ARENA,
...
uint GetLength16BMG (const u16 *sourse, // source
...
uint PrintFastBuf16BMG (FastBuf_t *fb, // destination buffer (data appended)
	const u16 *src, // source
	int src_len, // length of source in u16 words, -1:NULL term
	u16 utf8_max, // max code printed as UTF-8
	uint quote, // 0:no quotes, 1:escape ", 2: print in quotes
...
// [[xbuf]] [[2do]] not implemented yet
uint ScanFastBuf16BMG (FastBuf_t *fb, // destination buffer (data appended)
	ccp src, // UTF-8 source
	int src_len, // length of source, -1: determine with strlen()
	const bmg_t *bmg // NULL or support for \m{...}
);
...
typedef struct bmg_raw_section_t
{
	...
	uint total_size; // total size of section
					 // if 0: calculate it
	struct bmg_raw_section_t *next; // pointer to next element
} bmg_raw_section_t;
...
{
	u32 mid; // message ID
	bmg_item_cond_t cond; // condition
...
bool PatchFormatBMG (bmg_t *dest, // pointer to destination bmg
	const bmg_t *src, // pointer to source bmg
...
bool PatchIdBMG (bmg_t *bmg, // pointer to destination bmg
