...
static bool PrintFormat (struct XDump_t *xd, // internal copy of params
...
)
{
	...
	if (force > 1 || (force > 0 && xd->print_format) || (xd->print_format && !xd->format_printed))
	{
		xd->format_printed = true;
		fprintf (xd->f, "%s%*s%sFORMAT: %s", xd->prefix, xd->indent, "", xd->mode_c ? "//" : "#",
			xd->format_name);

		if (xd->format > XDUMPF_INT_1)
			fprintf (xd->f, " %s%s", xd->endian == DC_BIG_ENDIAN ? "BE" : "LE", xd->eol);
		else
			fputs (xd->eol, xd->f);
...
int XDumpStandardOutput (struct XDump_t *xd, // internal copy of params
...
)
{
	...
	if (xd->f)
	{
		fprintf (xd->f, "%s%*s%s", xd->prefix, xd->indent, "", prefix ? prefix : "");
		...
		if (xd->print_addr)
		{
			if (xd->mode_c)
				...
				fprintf (xd->f, "%*llx:%s", xd->addr_fw, xd->addr, xd->extra_space >= 0 ? " " : "");
		}
		...
		if (text && xd->print_text)
		{
			if (xd->mode_c)
				...
				fprintf (xd->f, "%s :%s:", xd->extra_space >= 0 ? " " : "", text);
		}
...
static void SetupXParam (XParam_t *p, XDump_t *xd, cvp data, uint size, bool term)
{
	DASSERT (p);
	DASSERT (xd);
	DASSERT (data);
...
static uint SetupXParamLine (XParam_t *p, XDump_t *xd)
{
	...
	p->next_line = false;
	p->line_size = xd->bytes_per_line < p->size ? xd->bytes_per_line : p->term ? p->size : 0;
...
static bool PrintNullSummary (XDump_t *xd)
{
	...
	snprintf (buf, sizeof (buf), "%s#NULL: 0x%x bytes", xd->extra_space >= 0 ? " " : "", skipped);
...
static bool CheckXParamNull (XParam_t *p, XDump_t *xd)
{
	...
	const bool skip
		= p->line_size == xd->bytes_per_line && !memcmp (p->d, nullbuf, xd->bytes_per_line);
	if (skip)
	{
		xd->null_lines++;
		xd->addr += p->line_size;
		...
		if (xd->print_number)
		{
			...
			for (col = 0; col < xd->cols_per_line; col++)
			{
				...
				else if (col < p.max_col)
				{
					...
					{
						case 2:
							val = xd->endian_func->rd16 (p.d);
							p.d += 2;
							break;
						case 3:
							val = xd->endian_func->rd24 (p.d);
							p.d += 3;
							break;
						case 4:
							val = xd->endian_func->rd32 (p.d);
							p.d += 4;
							break;
						default:
							val = *p.d++;
				...
				else if (p.term && !p.next_line && !xd->mode_c)
				{
					if (p.d < p.end)
						num += sprintf (
							num, "%*c%u", xd->num_format_fw - 1, '>', (int)(p.end - p.d));
		...
		if (xd->print_number)
		{
			...
			for (col = 0; col < xd->cols_per_line; col++)
			{
				...
				else if (col < p.max_col)
				{
					...
					{
						case 5:
							val = xd->endian_func->rd40 (p.d);
							p.d += 5;
							break;
						case 6:
							val = xd->endian_func->rd48 (p.d);
							p.d += 6;
							break;
						case 7:
							val = xd->endian_func->rd56 (p.d);
							p.d += 7;
							break;
						default:
							val = xd->endian_func->rd64 (p.d);
							p.d += 8;
							break;
					}
					num += sprintf (num, xd->num_format1, val);
			...
			for (col = 0; col < xd->cols_per_line; col++)
			{
				...
				else if (col < p.max_col)
				{
					...
					if (xd->bytes_per_col == 4)
					{
						...
						format = fabs (val) <= 9999.0 && fabs (val) >= 1e-3 ? xd->num_format1
																			: xd->num_format2;
						p.d += 4;
					}
					else
					{
						...
						format = fabs (val) <= 999999999.0 && fabs (val) >= 1e-6 ? xd->num_format1
																				 : xd->num_format2;
						p.d += 8;
					}
...
void InitializeXDump (XDump_t *xd)
{
	...
	xd->f = stdout;
	xd->print_format = xd->print_addr = xd->print_number = xd->print_endmarker = xd->print_text
		= xd->print_diff_sep = xd->print_summary = true;
}
...
void InitializeXDumpEx (XDump_t *xd, FILE *f, int indent, u64 addr, int addr_fw, int row_len)
{
	DASSERT (xd);
	InitializeXDump (xd);
...
void SetupXDump (XDump_t *xd, XDumpCommand_t cmd)
{
	...
	switch (xd->format)
	{
		case XDUMPF_INT_2:
			...
			xd->max_format_fw = 7;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%5u," : "0x%04x,")
		...
		case XDUMPF_INT_3:
			...
			xd->max_format_fw = 9;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%8u," : "0x%06x,")
		...
		case XDUMPF_INT_4:
			...
			xd->max_format_fw = 11;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%10u," : "0x%08x,")
		...
		case XDUMPF_INT_5:
			...
			xd->max_format_fw = 13;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%12llu," : "0x%010llx,")
		...
		case XDUMPF_INT_6:
			...
			xd->max_format_fw = 16;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%15llu," : "0x%012llx,")
		...
		case XDUMPF_INT_7:
			...
			xd->max_format_fw = 18;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%17llu," : "0x%014llx,")
		...
		case XDUMPF_INT_8:
			...
			xd->max_format_fw = 20;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%19llu," : "0x%016llx,")
		...
		default:
			...
			xd->max_format_fw = 5;
			xd->num_format1 = xd->mode_c ? (xd->mode_dec ? "%3u," : "0x%02x,")
...
void PrintXDumpParam (FILE *f, // output file, abort if NULL
...
)
{
	...
	fprintf (f,
		...
		(uint)xd->cmd < XDUMPC__N ? XDumpCommandName[xd->cmd] : "?", colset->reset, indent, "",
		colset->name, colset->value,
		(uint)xd->format < XDUMPF__N ? XDumpFormatName[xd->format] : "?",
		xd->mode_dec ? "decimal" : "hexadecimal", xd->mode_zero ? " with leading zeros" : "",
		xd->mode_c ? ", print in C syntax" : "", xd->format_name ? xd->format_name : "?",
		colset->reset, indent, "", colset->name, colset->value,
	...
	if (xd->num_format2)
		fprintf (f,
			"%*s%s"
			"Num format:   %s'%s' & '%s' (fw=%u,max=%u)%s\n",
			indent, "", colset->name, colset->value, xd->num_format1 ? xd->num_format1 : "?",
			xd->num_format2, xd->num_format_fw, xd->max_format_fw, colset->reset);
	else
		fprintf (f,
			"%*s%s"
			"Num format:   %s'%s' (fw=%u,max=%u)%s\n",
			indent, "", colset->name, colset->value, xd->num_format1 ? xd->num_format1 : "?",
			xd->num_format_fw, xd->max_format_fw, colset->reset);

	if (xd->assumed_size)
		fprintf (f,
			"%*s%s"
			"Address:      %sstart=0x%llx, current=0x%llx, size=0x%llx%s%s\n",
			indent, "", colset->name, colset->value, xd->start_addr, xd->addr, xd->assumed_size,
			xd->mode_align ? ", aligned" : "", colset->reset);
	else
		fprintf (f,
			"%*s%s"
			"Address:      %sstart=0x%llx, current=0x%llx%s%s\n",
			indent, "", colset->name, colset->value, xd->start_addr, xd->addr,
			xd->mode_align ? ", aligned" : "", colset->reset);
	...
	if (xd->have_trigger)
		fprintf (f,
			...
			indent, "", colset->name, colset->value, xd->trigger, xd->trigger,
			xd->trigger == '\n'		  ? " (LF)"
				: xd->trigger == '\r' ? " (CR)"
				: xd->trigger == '\t' ? " (TAB)"
				: xd->trigger == ' '  ? " (SPACE)"
	...
	fprintf (f,
		"%*s%s"
		"Address fw:   %smin=%u, max=%u, active=%u%s\n",
		indent, "", colset->name, colset->value, xd->min_addr_fw, xd->max_addr_fw, xd->addr_fw,
		colset->reset);

	fprintf (f,
		"%*s%s"
		"Print:        %s%sformat, %saddress, %snumbers, %stext, %ssummary, %sseparator%s\n",
		indent, "", colset->name, colset->value, xd->print_format ? "" : "no ",
		xd->print_addr ? "" : "no ", xd->print_number ? "" : "no ", xd->print_text ? "" : "no ",
		xd->print_summary ? "" : "no ", xd->print_diff_sep ? "" : "no ", colset->reset);

	fprintf (f,
		"%*s%s"
		"Output file:  %s%s, indent=%d, prefix=%s, eol=%s%s\n",
		indent, "", colset->name, colset->value,
		xd->f == stdout		  ? "STDOUT"
			: xd->f == stderr ? "STDERR"
			: xd->f			  ? "FILE"
	...
	fprintf (f,
		"%*s%s"
		"Bytes/line:   %smin=%u, max=%u%s\n"
		"%*s%s"
		"        =>:   %sbytes/column = %u, columns/line = %u, bytes/line = %u%s\n"
		"%*s%s"
		"Extra space:  %severy %u column => every %u column%s\n",
		indent, "", colset->name, colset->value, xd->min_width, xd->max_width, colset->reset,
		indent, "", colset->name, colset->value, xd->bytes_per_col, xd->cols_per_line,
		xd->bytes_per_line, colset->reset, indent, "", colset->name, colset->value, xd->extra_space,
		xd->col_space, colset->reset);

	if (xd->mode_ignore)
		fprintf (f,
			"%*s"
			"%sNULL bytes:   %sPrint a summary for lines with NULL bytes only.%s\n",
			indent, "", colset->name, colset->value, colset->reset);
	fprintf (f,
		"%*s%s"
		"Functions:    %sdump=%s, output=%s, endian=%s%s\n",
		indent, "", colset->name, colset->value, !xd->dump_func ? "-" : "SET",
		!xd->out_func							  ? "-"
			: xd->out_func == XDumpStandardOutput ? "STANDARD"
												  : "SET",
		!xd->endian_func				  ? "-"
	...
	fprintf (f,
		"%*s%s"
		"User param:   %sint=%d, ptr=%s%s\n",
		indent, "", colset->name, colset->value, xd->user_int, xd->user_ptr ? "SET" : "-",
		colset->reset);
}
...
static void SetupRemainder (XDump_t *xd, const XDump_t *src, XDumpCommand_t cmd, uint size)
{
	DASSERT (xd);
	DASSERT (src);
...
)
{
	...
	if (term)
	{
		...
		if (xd->f && xd->print_summary)
		{
			...
			{
				if (xd->mode_c)
					fprintf (xd->f, "//%*llx// [%llu Bytes]%s", xd->addr_fw, xd->addr, xd->written,
						xd->eol);

				else
					fprintf (xd->f, "%*llx:%s [%llu Bytes]%s", xd->addr_fw, xd->addr,
						xd->extra_space >= 0 ? " " : "", xd->written, xd->eol);
			}
			else
			{
				if (xd->mode_c)
					fprintf (xd->f, "// [%llu Bytes]%s", xd->written, xd->eol);

				else
					fprintf (xd->f, "%s [%llu Bytes]%s", xd->extra_space >= 0 ? " " : "",
						xd->written, xd->eol);
			}
		}
	}
...
static int XDiffCatchOutput (struct XDump_t *xd, // internal copy of params
	ccp prefix, // NULL or additional prefix (used by diff)
	ccp dump, // dump to print
...
static void SetupXDiff (XDiff_t *xdiff, // data to setup
...
)
{
	...
	int done = 0;
	while (size1 > 0 && size2 > 0 || !done && (size1 > 0 || size2 > 0))
	{
		//--- optimized search
...
#if !OPTIMIZED_SEARCH
		if (diffsize > 0 && (diffsize < xd->bytes_per_line || memcmp (p1, p2, diffsize)))
#endif
		{
...
)
{
	...
	return XDiff (&xd, data1, size1, true, data2, size2, true, limit_lines, colorize);
}
...
static void SetupXScan (XScan_t *scan, // data to setup
	XDump_t *xd, // NULL or template
...
)
{
	...
	static const KeywordTab_t keytab[] = { { MD_HEX, "HEX", 0, XDUMPF_AUTO },
		{ MD_HEX, "HEX-1", "HEX1", XDUMPF_INT_1 }, { MD_HEX, "HEX-2", "HEX2", XDUMPF_INT_2 },
		{ MD_HEX, "HEX-3", "HEX3", XDUMPF_INT_3 }, { MD_HEX, "HEX-4", "HEX4", XDUMPF_INT_4 },
		{ MD_HEX, "HEX-5", "HEX5", XDUMPF_INT_5 }, { MD_HEX, "HEX-6", "HEX6", XDUMPF_INT_6 },
		{ MD_HEX, "HEX-7", "HEX7", XDUMPF_INT_7 }, { MD_HEX, "HEX-8", "HEX8", XDUMPF_INT_8 },

		{ MD_DEC, "DEC", 0, XDUMPF_AUTO }, { MD_DEC, "DEC-1", "DEC1", XDUMPF_INT_1 },
		{ MD_DEC, "DEC-2", "DEC2", XDUMPF_INT_2 }, { MD_DEC, "DEC-3", "DEC3", XDUMPF_INT_3 },
		{ MD_DEC, "DEC-4", "DEC4", XDUMPF_INT_4 }, { MD_DEC, "DEC-5", "DEC5", XDUMPF_INT_5 },
		{ MD_DEC, "DEC-6", "DEC6", XDUMPF_INT_6 }, { MD_DEC, "DEC-7", "DEC7", XDUMPF_INT_7 },
		{ MD_DEC, "DEC-8", "DEC8", XDUMPF_INT_8 },

		{ MD_OCT, "OCT", 0, XDUMPF_AUTO }, { MD_OCT, "OCT-1", "OCT1", XDUMPF_INT_1 },
		{ MD_OCT, "OCT-2", "OCT2", XDUMPF_INT_2 }, { MD_OCT, "OCT-3", "OCT3", XDUMPF_INT_3 },
		{ MD_OCT, "OCT-4", "OCT4", XDUMPF_INT_4 }, { MD_OCT, "OCT-5", "OCT5", XDUMPF_INT_5 },
		{ MD_OCT, "OCT-6", "OCT6", XDUMPF_INT_6 }, { MD_OCT, "OCT-7", "OCT7", XDUMPF_INT_7 },
		{ MD_OCT, "OCT-8", "OCT8", XDUMPF_INT_8 },

		{ MD_NUM, "1", 0, XDUMPF_INT_1 }, { MD_NUM, "2", 0, XDUMPF_INT_2 },
		{ MD_NUM, "3", 0, XDUMPF_INT_3 }, { MD_NUM, "4", 0, XDUMPF_INT_4 },
		{ MD_NUM, "5", 0, XDUMPF_INT_5 }, { MD_NUM, "6", 0, XDUMPF_INT_6 },
		{ MD_NUM, "7", 0, XDUMPF_INT_7 }, { MD_NUM, "8", 0, XDUMPF_INT_8 },

		{ MD_NUM, "FLOAT", 0, XDUMPF_FLOAT }, { MD_NUM, "DOUBLE", 0, XDUMPF_DOUBLE },

		{ MD_ENDIAN, "LE", "LITTLE", DC_LITTLE_ENDIAN }, { MD_ENDIAN, "BE", "BIG", DC_BIG_ENDIAN },
...
)
{
	...
	while (src < end && max_size > 0)
	{
		...
		if (scan->prefix_len && scan->prefix_len <= eol - src
