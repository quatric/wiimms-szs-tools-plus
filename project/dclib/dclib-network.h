...
{
	DASSERT (sip);
	NormalizeIP4 (sip->name, sizeof (sip->name), true, sip->name, -1);
}
...
enum
{
	...
	IP_SIZE_PREFIX = 22, ///< Maximale Zeichenlänge einer Adresse, die mit MaskPrefixMIP() gekürzt
						 ///< wurde.
...
static inline u8 *GetCorePtrBIP (const BinIP_t *src)
{
	return (u8 *)&src->ipvers;
}
static inline uint GetCoreSizeBIP (const BinIP_t *src)
{
	return IP_SIZE_BINIP;
}
...
static inline bool IsLocalAddress (BinIP_t *addr)
{
	return IsLocalBIL (addr, 2);
}
static inline bool IsLocalNetwork (BinIP_t *addr)
{
	return IsLocalBIL (addr, 3);
}
...
static inline bool PrintBitsMIP (const ManageIP_t *mip, PrintModeIP_t pmode)
{
	...
	return pmode > PMIP_NEVER
		&& (mip->bin.bits < mip->relevant_bits || pmode == PMIP_IF_SET && mip->have_bits
			|| pmode >= PMIP_ALWAYS);
}
...
int ConnectTCP (ccp name, // optional service + server + optional host
...
uint GetSocketNameBySA (char *buf, uint bufsize, const struct sockaddr *sa, socklen_t sa_len);
...
typedef enumError (*CommandTCPArgFunc) (TCPStream_t *ts, // valid stream data
...
typedef void (*TCPStreamSaveFunc) (FILE *f, // output file
