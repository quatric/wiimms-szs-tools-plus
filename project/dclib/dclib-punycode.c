...
)
{
	...
	while (src < src_end)
	{
		...
		else
		{
			...
			const enum punycode_status stat = punycode_decode (point - src, src, &olen, pname, 0);
			char *dest = name;
...
)
{
	...
	while (src < src_end)
	{
		...
		punycode_uint olen = MAX_LEN;
		const enum punycode_status stat = punycode_encode (dest - pname, pname, 0, &olen, name);
		if (stat)
			goto error;
