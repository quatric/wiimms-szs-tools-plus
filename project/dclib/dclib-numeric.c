...
)
{
	...
	while (min_idx <= max_idx)
	{
		...
		noPRINT ("FOUND-BEG[%zx.%u=%u] %02x, %u..%u\n", src - (u8 *)bit_field - 1, bit_idx, first,
			ch, min_idx, max_idx);
		...
		noPRINT ("FOUND-END[%zx.%u=%u] %02x, %u..%u\n", src - (u8 *)bit_field - 1, bit_idx, last,
			ch, min_idx, max_idx);

		if (first == last)
			dest = snprintfE (dest, end, "%s%d", sep, first * mult + add);
		else if (use_ranges)
			dest = snprintfE (dest, end, "%s%d:%d", sep, first * mult + add, last * mult + add);
		else
			while (first <= last)
			{
				dest = snprintfE (dest, end, "%s%d", sep, first++ * mult + add);
...
static inline u16 ident16 (u16 data)
{
	return data;
}
static inline u32 ident32 (u32 data)
{
	return data;
}
static inline u64 ident64 (u64 data)
{
	return data;
}

static inline u16 reverse16 (u16 data)
{
	return swap16 (&data);
}
static inline u32 reverse32 (u32 data)
{
	return swap32 (&data);
}
static inline u64 reverse64 (u64 data)
{
	return swap64 (&data);
}
...
const endian_func_t *GetEndianFunc (const void *byte_order_mark)
{
	...
	return bom[0] == 0xfe && bom[1] == 0xff ? &be_func
		: bom[0] == 0xff && bom[1] == 0xfe	? &le_func
											: 0;
}
	...
	if (!done && TRACE_FILE)
	{
		...
		for (i = 0; i < 0x100; i += 8)
			fprintf (TRACE_FILE, " %#010x,%#010x,%#010x,%#010x, %#010x,%#010x,%#010x,%#010x,\n",
				TableCRC32[i + 0], TableCRC32[i + 1], TableCRC32[i + 2], TableCRC32[i + 3],
				TableCRC32[i + 4], TableCRC32[i + 5], TableCRC32[i + 6], TableCRC32[i + 7]);
		fprintf (TRACE_FILE, "};\n");
	}
#endif
...
u64 DecodeU64 (ccp source, // string to decode
	int len, // length of string. if -1, str is null terminated
...
ccp EncodeU64 (char *buf, // result buffer (size depends on base)
	...
	)
{
	...
	int pre_pos;
	snprintf (
		linesep, sizeof (linesep), "%s%n%*s%s", eol, &pre_pos, indent, "", prefix ? prefix : "");

	char *end = buf + buf_size;
	char *dest = StringCopyE (buf, end, linesep + pre_pos);
	const uint scanned = EncodeBase64 (
		dest, end - dest, source, source_len, encode64, use_filler, linesep, bytes_per_line);
	dest = buf + strlen (buf);
...
)
{
	...
	switch (emode)
	{
		...
		case ENCODE_BASE64STAR:
			len = DecodeBase64 (
				buf, buf_size - 1, source, slen, TableDecode64url, true, scanned_len);
			break;

		case ENCODE_BASE64XML:
			len = DecodeBase64 (
				buf, buf_size - 1, source, slen, TableDecode64xml, true, scanned_len);
			break;
...
)
{
	...
	switch (emode)
	{
		...
		case ENCODE_PIPE8:
			PrintEscapedString (
				buf, buf_size, source, slen, CHMD__MODERN | CHMD_IF_REQUIRED | CHMD_PIPE, 0, &len);
			break;
...
)
{
	...
	exmem_t res
		= { .data = { buf, len + quote_mode }, .is_circ_buf = use_circ, .is_alloced = !use_circ };
	return res;
};
...
)
{
...
#if 1
	exmem_t res = EscapeStringEx (
		src, src_len, return_if_null, return_if_empty, char_mode, quote, try_circ);
	if (dest_len)
		*dest_len = res.data.len;
	return res.is_circ_buf || res.is_alloced ? (char *)res.data.ptr
	...
	if (!size && char_mode & CHMD_IF_REQUIRED)
	{
		char *buf = try_circ && size <= CIRC_BUF_MAX_ALLOC ? GetCircBuf (src_len + 1)
														   : MALLOC (src_len + 1);
...
int GetTimezoneAdjust (time_t tim)
{
	...
	gmtime_r (&tim, &gmt);
	localtime_r (&tim, &loc);
	int delta = (gmt.tm_hour - loc.tm_hour) * 3600 + (gmt.tm_min - loc.tm_min) * 60
		+ (gmt.tm_sec - loc.tm_sec);
	if (gmt.tm_yday != loc.tm_yday)
	{
		if (gmt.tm_year < loc.tm_year || gmt.tm_year == loc.tm_year && gmt.tm_yday < loc.tm_yday)
		{
			delta -= 24 * 3600;
		}
		else
...
)
{
	...
	ccp format = tim >= ref - 10 * SEC_PER_HOUR && tim <= ref + 10 * SEC_PER_HOUR ? "  %k:%M:%S"
		: tim >= ref - 12 * SEC_PER_DAY && tim <= ref + 10 * SEC_PER_DAY		  ? " %e. %k:%M"
																				  : "%F";
...
ccp PrintUTimeByFormatUTC (
	...
	u_usec_t utime)
{
	return PrintNSecByFormatUTC (
		format, utime / USEC_PER_SEC, utime % USEC_PER_SEC * NSEC_PER_USEC);
}
...
{
	return PrintNSecByFormatUTC (
		format, mtime / MSEC_PER_SEC, mtime % MSEC_PER_SEC * NSEC_PER_MSEC);
}
...
ccp PrintTimeSec (char *buf, // result buffer (>19 bytes are good)
							 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
ccp PrintTimeMSec (char *buf, // result buffer (>23 bytes are good)
							  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
ccp PrintTimeUSec (char *buf, // result buffer (>26 bytes are good)
							  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
ccp PrintTimeNSec (char *buf, // result buffer (>26 bytes are good)
							  // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
ccp PrintTimerMSec (char *buf, // result buffer (>12 bytes are good)
							   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	if (fraction)
	{
		const uint len = snprintf (buf, buf_size, "%02d:%02d:%02d.%03d", msec / 3600000,
			msec / 60000 % 60, msec / 1000 % 60, msec % 1000);
		if (len > 3 && fraction < 3)
		{
		...
		snprintf (
			buf, buf_size, "%02d:%02d:%02d", msec / 3600000, msec / 60000 % 60, msec / 1000 % 60);
...
ccp PrintTimerUSec (char *buf, // result buffer (>16 bytes are good)
							   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	if (fraction)
	{
		const uint len = snprintf (buf + 1, buf_size - 1, "%02d:%02d:%02d.%06lld", sec / 3600,
			sec / 60 % 60, sec % 60, (u64)usec % USEC_PER_SEC);
		if (len > 6 && fraction < 6)
		{
		...
		snprintf (buf + 1, buf_size - 1, "%02d:%02d:%02d", sec / 3600, sec / 60 % 60, sec % 60);
...
ccp PrintTimerNSec (char *buf, // result buffer (>16 bytes are good)
							   // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	if (fraction)
	{
		const uint len = snprintf (buf + 1, buf_size - 1, "%02d:%02d:%02d.%09lld", sec / 3600,
			sec / 60 % 60, sec % 60, (u64)nsec % NSEC_PER_SEC);
		if (len > 9 && fraction < 9)
		{
...
ccp PrintTimer3 (char *buf, // result buffer (>3 bytes)
							// If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
ccp PrintTimer4 (char *buf, // result buffer (>4 bytes)
							// If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 sec, // seconds to print
	int usec, // 0...999999: usec fraction,
...
ccp PrintTimer6N (char *buf, // result buffer (>6 bytes)
							 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 sec, // seconds to print
...
ccp PrintTimerUSec4s (char *buf, // result buffer (>4 bytes)
								 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	s_usec_t usec, // microseconds to print
...
ccp PrintTimerUSec7s (char *buf, // result buffer (>7 bytes)
								 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	s_usec_t usec, // microseconds to print
	sizeform_mode_t mode // DC_SFORM_ALIGN | DC_SFORM_PLUS | DC_SFORM_DASH
...
ccp PrintTimerNSec7s (char *buf, // result buffer (>7 bytes)
								 // If NULL, a local circulary static buffer is used
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
ccp PrintAge3Sec (u_sec_t now, // NULL or current time by GetTimeSec(false)
...
ccp PrintAge4Sec (u_sec_t now, // NULL or current time by GetTimeSec(false)
	u_sec_t time // time to print
)
{
	if (!time)
	...
	now -= time;
	return now < 0 ? PrintTimerSec4s (0, 0, now, DC_SFORM_ALIGN)
				   : PrintTimer4 (0, 0, now, -1, DC_SFORM_ALIGN);
}

///////////////////////////////////////////////////////////////////////////////

ccp PrintAge6Sec (u_sec_t now, // NULL or current time by GetTimeSec(false)
	u_sec_t time // time to print
)
{
	if (!time)
...
ccp PrintAge7Sec (u_sec_t now, // NULL or current time by GetTimeSec(false)
	u_sec_t time // time to print
)
{
	if (!time)
...
char *ScanIntervalTS (struct timespec *res_ts, // not NULL: store result here
...
char *ScanIntervalUSec (u_usec_t *res_usec, // not NULL: store total microseconds
	ccp source // source text
)
{
	struct timespec ts;
...
char *ScanIntervalNSec (u_nsec_t *res_nsec, // not NULL: store total nanoseconds
	ccp source // source text
)
{
	struct timespec ts;
...
char *ScanInterval (time_t *res_time, // not NULL: store seconds
...
char *ScanInterval64 (u64 *res_usec, // not NULL: store total microseconds
	ccp source // source text
)
{
	u_usec_t usec;
...
char *ScanDateTime (time_t *res_time, // not NULL: store seconds here
...
)
{
	...
	while (*src2 > 0 && *src2 <= ' ')
		src2++;
	if ((src2[0] == '-' || src2[0] == '+') && src2[1] >= '0' && src2[1] <= '9' && src2[2] >= '0'
		&& src2[2] <= '9' && src2[3] >= '0' && src2[3] <= '9' && src2[4] >= '0' && src2[4] <= '9'
		&& (src2[5] < '0' || src2[5] > '9'))
	{
		int delta = 36000 * src2[1] + 3600 * src2[2] + 600 * src2[3] + 60 * src2[4] - 40260 * '0';
		if (src2[0] == '-')
			delta = -delta;
...
char *ScanDateTime64 (u64 *res_usec, // not NULL: store total microseconds
	ccp source, // source text
	bool allow_delta, // true: allow +|- interval
	bool use_localtime // false: UTC, true: local time
)
...
ccp hour2text (int hour, // hour to print
...
)
{
	...
	return fw >= 13 ? PrintCircBuf ("%04d%s%02u%s%02u%s%02u", tm.tm_year + 1900, sep_date,
						  tm.tm_mon + 1, sep_date, tm.tm_mday, sep_dt, tm.tm_hour)
		: fw >= 11	? PrintCircBuf ("%02d%s%02u%s%02u%s%02u", tm.tm_year % 100, sep_date,
						  tm.tm_mon + 1, sep_date, tm.tm_mday, sep_dt, tm.tm_hour)
		: fw >= 10	? PrintCircBuf ("%2u.%s%02u:xx", tm.tm_mday, sep_dt, tm.tm_hour)
					: PrintCircBuf ("%02u%s%02u%s%02u", tm.tm_mon + 1, sep_date, tm.tm_mday, sep_dt,
						  tm.tm_hour);
}
...
ccp day2text (int day, // day to print
	int fw, // wanted field width assuming separators are 1 char
	ccp sep_date, // NULL or separator within date
				  // if NULL: use '-'
	ccp sep_dt // ignored
)
{
	...
	return fw >= 10 ? PrintCircBuf ("%04d%s%02u%s%02u", tm.tm_year + 1900, sep_date, tm.tm_mon + 1,
						  sep_date, tm.tm_mday)
					: PrintCircBuf ("%02d%s%02u%s%02u", tm.tm_year % 100, sep_date, tm.tm_mon + 1,
						  sep_date, tm.tm_mday);
}
...
ccp week2text (int week, // week to print
	int fw, // wanted field width assuming separators are 1 char
	ccp sep_date, // NULL or separator within date
				  // if NULL: use 'w'
	ccp sep_dt // ignored
)
{
	...
	return PrintCircBuf (
		"%04d%s%02u", tm.tm_year + 1900, sep_date ? sep_date : "w", tm.tm_yday / 7 + 1);
}
...
ccp month2text (int month, // month to print
	int fw, // wanted field width assuming separators are 1 char
	ccp sep_date, // NULL or separator within date
				  // if NULL: use '-'
	ccp sep_dt // ignored
...
{
	month += 2001 * 12;
	return PrintCircBuf ("%4d%s%02u", month / 12, sep_date ? sep_date : "-", month % 12 + 1);
}
...
ccp quarter2text (int quarter, // quarter to print
	int fw, // wanted field width assuming separators are 1 char
	ccp sep_date, // NULL or separator within date
				  // if NULL: use 'q'
	ccp sep_dt // ignored
...
{
	quarter += 2001 * 4;
	return PrintCircBuf ("%4d%s%u", quarter / 4, sep_date ? sep_date : "q", quarter % 4 + 1);
}
...
#define SRT_NAME UsageCount_t
	...
	DEF_SRT_UINT (par.enabled, "enabled"), DEF_SRT_UINT (par.used, "used"), DEF_SRT_ARRAY_A (entry),
	DEF_SRT_UINT (entry[0].count, "count"), DEF_SRT_UINT (entry[0].elapsed_nsec, "elapsed"),
	DEF_SRT_UINT (entry[0].time_sec, "time"), DEF_SRT_FLOAT (entry[0].top1s, "top1s"),
	DEF_SRT_FLOAT (entry[0].top5s, "top5s"), DEF_SRT_ARRAY_END (),

	DEF_SRT_SEPARATOR (), DEF_SRT_TERM ()
};
	...
	if (cur.elapsed_nsec >= uc->par.update_nsec)
	{
		...
		if (uc->ref.elapsed_nsec)
		{
			...
			else
			{
				...
				for (index = USAGE_COUNT_FIRST_ADD; index < uc->par.used;
		...
		if (uci)
		{
			...
			if (uci->srt_prefix)
			{
				if (uci->uc)
					...
					SaveCurrentStateByTable (
						f, uci->ud, SRT_UsageDuration, uci->srt_prefix, fw_name);
			}
		}
	}
}
...
#define SRT_NAME UsageDuration_t
	...
	DEF_SRT_UINT (par.enabled, "enabled"), DEF_SRT_UINT (par.used, "used"), DEF_SRT_ARRAY_A (entry),
	DEF_SRT_UINT (entry[0].elapsed_nsec, "elapsed"), DEF_SRT_UINT (entry[0].total_nsec, "total"),
	DEF_SRT_UINT (entry[0].top_nsec, "top"), DEF_SRT_UINT (entry[0].count, "count"),
	DEF_SRT_UINT (entry[0].time_sec, "time"), DEF_SRT_FLOAT (entry[0].top_cnt_1s, "top-c1"),
	DEF_SRT_FLOAT (entry[0].top_cnt_5s, "top-c5"), DEF_SRT_ARRAY_END (),
...
void UpdateUsageDuration (UsageDuration_t *ud, u_nsec_t timer_nsec)
{
	...
	if (cur.elapsed_nsec >= ud->par.update_nsec)
	{
		...
		if (ud->ref.elapsed_nsec)
		{
			...
			{
				u_nsec_t min = USAGE_COUNT_NEXT_MIN (NSEC_PER_SEC);
				for (index = USAGE_COUNT_FIRST_ADD; index < ud->par.used;
					index++, min = USAGE_COUNT_NEXT_MIN (min))
				{
					UsageDurationEntry_t *e = ud->entry + index;
					if (e->elapsed_nsec < min)
...
static ccp print_usage_count_val (double val, const float *color_val)
{
	...
	{
		ccp col = val < color_val[2] ? (val < color_val[1] ? colout->mark : colout->info)
			: val < color_val[4]	 ? (val < color_val[3] ? colout->hint : colout->warn)
									 : (val < color_val[5] ? colout->err : colout->bad);

		return val < 9999.9994 ? PrintCircBuf ("%s%8.3f%s", col, val, colout->reset)
			: val < 999999.94  ? PrintCircBuf ("%s%8.1f%s", col, val, colout->reset)
...
static ccp print_usage_count (u32 n_wait, u_nsec_t elapsed_nsec, const float *color_val)
{
	return elapsed_nsec
...
static ccp print_usage_dur_val (u_nsec_t dur, const u_nsec_t *color_dur)
{
	...
	if (dur > color_dur[0])
	{
		ccp col = dur < color_dur[2] ? (dur < color_dur[1] ? colout->mark : colout->info)
			: dur < color_dur[4]	 ? (dur < color_dur[3] ? colout->hint : colout->warn)
...
static ccp print_usage_dur (u_nsec_t total, u64 count, const u_nsec_t *color_dur)
{
	return count ? print_usage_dur_val (total / count, color_dur) : "     -";
}

//
///////////////////////////////////////////////////////////////////////////////

void ListUsageCount (PrintMode_t *p_pm, // NULL or print mode
...
)
{
	...
	UsageCount_t *uc = uci->uc;
	fprintf (pm.fout, "%s%s%*s %s%s (%u record%s%s%s)%s%s", pm.compact ? "" : pm.eol, pm.prefix,
		pm.indent, "", pm.cout->caption, uci->title, uc->par.used, uc->par.used == 1 ? "" : "s",
		usage_count_enabled > 0 ? "" : ", global disabled", uc->par.enabled ? "" : ", disabled",
	...
	if (print_mode == LUC_PM_SUMMARY)
	{
		fprintf (pm.fout,
			...
			"%s%*s%s╠════════╬════════╪═════════╪══════════╬═══════════╪═══════════╣%s%s",
			pm.prefix, pm.indent, "", colout->heading, pm.eol, pm.prefix, pm.indent, "",
			colout->heading, pm.eol, pm.prefix, pm.indent, "", colout->heading, colout->reset,
			pm.eol);
		...
		for (int i = 0; i < uc->par.used; i++, e++)
		{
			...
			fprintf (pm.fout, "%s%*s%s║%s %-6.6s %s║%s %s %s│%s %s %s│%s %s %s║%s", pm.prefix,
				pm.indent, "", colout->heading, colout->reset, name, colout->heading, colout->reset,
				PrintTimerNSec6 (0, 0, sum.elapsed_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset,
				PrintNumberU7 (0, 0, sum.count, DC_SFORM_ALIGN | DC_SFORM_DASH), colout->heading,
				colout->reset, print_usage_count (sum.count, sum.elapsed_nsec, uci->color_val),
				colout->heading, colout->reset);

			fprintf (pm.fout, " %s  %s│%s  %s %s║%s",
				print_usage_count_val (sum.top1s, uci->color_val), colout->heading, colout->reset,
				print_usage_count_val (sum.top5s, uci->color_val), colout->heading, colout->reset);
	...
	else
	{
		fprintf (pm.fout,
			...
			"s╔═══════════╦═════════════════════════════╦══════════╦══════════╦════════════════════"
			"═════════╗%s"
			"%s%*s%s║           ║       Single Record         ║  top 1s  ║  top 5s  ║       Summed "
			"Values         ║%s"
			"%s%*s%s║ real time ║ period │ counter │  count/s ║  count/s ║  count/s ║ period │ "
			"counter │  count/s ║%s"
			"%s%*s%"
			"s╠═══════════╬════════╪═════════╪══════════╬══════════╬══════════╬════════╪═════════╪═"
			"═════════╣%s%s",
			pm.prefix, pm.indent, "", colout->heading, pm.eol, pm.prefix, pm.indent, "",
			colout->heading, pm.eol, pm.prefix, pm.indent, "", colout->heading, pm.eol, pm.prefix,
			pm.indent, "", colout->heading, colout->reset, pm.eol);
...
#endif
		{
			...
			fprintf (pm.fout, "%s%*s%s║%s%s %s║%s %s %s│%s %s %s│%s %s %s║%s", pm.prefix, pm.indent,
				"", colout->heading, colout->reset, PrintRefTime (e->time_sec, &reftime),
				colout->heading, colout->reset,
				PrintTimerNSec6 (0, 0, e->elapsed_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset,
				PrintNumberU7 (0, 0, e->count, DC_SFORM_ALIGN | DC_SFORM_DASH), colout->heading,
				colout->reset, print_usage_count (e->count, e->elapsed_nsec, uci->color_val),
				colout->heading, colout->reset);

			fprintf (pm.fout, " %s %s║%s %s %s║%s",
				print_usage_count_val (e->top1s, uci->color_val), colout->heading, colout->reset,
				print_usage_count_val (e->top5s, uci->color_val), colout->heading, colout->reset);

			fprintf (pm.fout, " %s %s│%s %s %s│%s %s %s║%s",
				PrintTimerNSec6 (0, 0, sum.elapsed_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset,
				PrintNumberU7 (0, 0, sum.count, DC_SFORM_ALIGN | DC_SFORM_DASH), colout->heading,
				colout->reset, print_usage_count (sum.count, sum.elapsed_nsec, uci->color_val),
			...
			if (pm.debug)
			{
				...
				{
					fprintf (pm.flog, " %2u  %s %s%4llu%%%s %s%5llu%%%s%s", i,
						PrintTimerNSec6 (0, 0, min, DC_SFORM_ALIGN | DC_SFORM_DASH),
						e->elapsed_nsec < min			? ""
							: e->elapsed_nsec < min * 2 ? colout->hint
														: colout->warn,
						100 * e->elapsed_nsec / min, colout->reset,
						sum.elapsed_nsec < min ? colout->info : "", 100 * sum.elapsed_nsec / min,
						colout->reset, pm.eol);
...
void ListUsageDuration (PrintMode_t *p_pm, // NULL or print mode
	const UsageCtrl_t *uci, // NULL or object to list
	int print_mode, // see LUC_PM_*
	s_nsec_t limit_nsec // >0: limit output until # is reached
)
{
	...
	UsageDuration_t *ud = uci->ud;
	fprintf (pm.fout, "%s%s%*s %s%s (%u record%s%s%s)%s%s", pm.compact ? "" : pm.eol, pm.prefix,
		pm.indent, "", pm.cout->caption, uci->title, ud->par.used, ud->par.used == 1 ? "" : "s",
		usage_count_enabled > 0 ? "" : ", global disabled", ud->par.enabled ? "" : ", disabled",
		pm.cout->reset, pm.eol);
	...
	if (print_mode == LUC_PM_SUMMARY)
	{
		printf (
			...
			"%s ╠════════╬════════╪═════════╪═════════╪════════╬═════════╪═════════╪════════╣%s%s",
			colout->heading, pm.eol, colout->heading, pm.eol, colout->heading, pm.eol,
			colout->heading, colout->reset, pm.eol);
		...
		for (int i = 0; i < ud->par.used; i++, e++)
		{
			...
			fprintf (pm.fout, "%s%*s%s║%s %-6.6s %s║%s %s %s│%s %s %s│%s%s %s│%s %s %s║%s",
				pm.prefix, pm.indent, "", colout->heading, colout->reset, name, colout->heading,
				colout->reset,
				PrintTimerNSec6 (0, 0, sum.elapsed_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset,
				PrintNumberU7 (0, 0, sum.count, DC_SFORM_ALIGN | DC_SFORM_DASH), colout->heading,
				colout->reset, print_usage_count (sum.count, sum.elapsed_nsec, uci->color_val),
				colout->heading, colout->reset,
				print_usage_dur (sum.total_nsec, sum.count, uci->color_dur), colout->heading,
				colout->reset);

			fprintf (pm.fout, "%s %s│%s%s %s│%s %s %s║%s",
				print_usage_count_val (sum.top_cnt_1s, uci->color_val), colout->heading,
				colout->reset, print_usage_count_val (sum.top_cnt_5s, uci->color_val),
				colout->heading, colout->reset, print_usage_dur_val (sum.top_nsec, uci->color_dur),
				colout->heading, colout->reset);
	...
	else
	{
		printf ("%s "
				"╔═══════════╦═════════════════════════════════════╦═════════╤═════════╤════════╦══"
				"═══════════════════════════════════╗%s"
				"%s ║           ║            Single Record            ║ top 1s  │ top 5s  │  top   "
				"║            Summed Values            ║%s"
				"%s ║ real time ║ period │ counter │ count/s │ Ø time ║ count/s │ count/s │  time  "
				"║ period │ counter │ count/s │ Ø time ║%s"
				"%s "
				"╠═══════════╬════════╪═════════╪═════════╪════════╬═════════╪═════════╪════════╬══"
				"══════╪═════════╪═════════╪════════╣%s%s",
			colout->heading, pm.eol, colout->heading, pm.eol, colout->heading, pm.eol,
...
#endif
		{
			...
			fprintf (pm.fout, "%s%*s%s║%s%s %s║%s %s %s│%s %s %s│%s%s %s│%s %s %s║%s", pm.prefix,
				pm.indent, "", colout->heading, colout->reset, PrintRefTime (e->time_sec, &reftime),
				colout->heading, colout->reset,
				PrintTimerNSec6 (0, 0, e->elapsed_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset,
				PrintNumberU7 (0, 0, e->count, DC_SFORM_ALIGN | DC_SFORM_DASH), colout->heading,
				colout->reset, print_usage_count (e->count, e->elapsed_nsec, uci->color_val),
				colout->heading, colout->reset,
				print_usage_dur (e->total_nsec, e->count, uci->color_dur), colout->heading,
				colout->reset);

			fprintf (pm.fout, "%s %s│%s%s %s│%s %s %s║%s",
				print_usage_count_val (e->top_cnt_1s, uci->color_val), colout->heading,
				colout->reset, print_usage_count_val (e->top_cnt_5s, uci->color_val),
				colout->heading, colout->reset, print_usage_dur_val (e->top_nsec, uci->color_dur),
				colout->heading, colout->reset);

			fprintf (pm.fout, " %s %s│%s %s %s│%s%s %s│%s %s %s║%s",
				PrintTimerNSec6 (0, 0, sum.elapsed_nsec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset,
				PrintNumberU7 (0, 0, sum.count, DC_SFORM_ALIGN | DC_SFORM_DASH), colout->heading,
				colout->reset, print_usage_count (sum.count, sum.elapsed_nsec, uci->color_val),
...
static ccp print_cpu_percent (double val)
{
	...
	if (val >= collist[0])
	{
		ccp col = val < collist[2] ? (val < collist[1] ? colout->info : colout->hint)
...
{
	return !val				 ? "       -"
		: val < USEC_PER_SEC ? PrintCircBuf ("%8llu", val)
		: val < 10 * USEC_PER_SEC
...
static ccp print_cpu_wait_val (double val)
{
	...
	if (val > collist[0])
	{
		...
		ccp col = val < collist[2] ? (val < collist[1] ? colout->mark : colout->info)
			: val < collist[4]	   ? (val < collist[3] ? colout->hint : colout->warn)
...
{
	return elapsed_usec ? print_cpu_wait_val ((double)(USEC_PER_SEC * n_wait) / elapsed_usec)
						: "      ×";
}

//-----------------------------------------------------------------------------

static void print_cpu_debug (int idx, u_usec_t *min, u_usec_t elapsed1, u_usec_t elapsed2)
{
	DASSERT (min);
	...
	if (idx >= CPU_USAGE_FIRST_ADD && *min)
	{
		printf (" %2u  %s %s%4llu%%%s %s%4llu%%%s\n", idx,
			...
			elapsed1 < *min			  ? ""
				: elapsed1 < *min * 2 ? colout->hint
									  : colout->warn,
			100 * elapsed1 / *min, colout->reset, elapsed2 < *min ? colout->info : "",
			100 * elapsed2 / *min, colout->reset);
...
void ListCpuUsage (PrintMode_t *p_pm, // NULL or print mode
...
)
{
	...
	printf ("%s%s  CPU Usage (%u record%s%s)%s\n", empty_line, colout->caption, cpu_usage.par.used,
		cpu_usage.par.used == 1 ? "" : "s", cpu_usage.par.enabled ? "" : ", disabled",
		colout->reset);
	...
	if (print_mode == LUC_PM_SUMMARY)
	{
		printf ("%s "
				"╔════════╦═════════════════════════════════════════╦════════════════╦═════════════"
				"═══╗\n"
				"%s ║        ║              Summed Values              ║    top 1s      ║    top "
				"5s      ║\n"
				"%s ║ range  ║ period │ user%% │system%%│total%% │ wait/s ║total%% │ wait/s "
				"║total%% │ wait/s ║\n"
				"%s "
				"╠════════╬════════╪═══════╪═══════╪═══════╪════════╬═══════╪════════╬═══════╪═════"
				"═══╣%s\n",
			colout->heading, colout->heading, colout->heading, colout->heading, colout->reset);
		...
		for (int i = 0; i < cpu_usage.par.used; i++, e++)
		{
			...
			printf (" %s║%s %-6.6s %s║%s %s %s│%s%s %s│%s%s %s│%s%s %s│%s%s %s║%s", colout->heading,
				colout->reset, name, colout->heading, colout->reset,
				PrintTimerUSec6 (0, 0, sum.elapsed_usec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset, print_cpu_percent (sum.utime_usec * elapsed),
				colout->heading, colout->reset, print_cpu_percent (sum.stime_usec * elapsed),
				colout->heading, colout->reset,
				print_cpu_percent ((sum.utime_usec + sum.stime_usec) * elapsed), colout->heading,
				colout->reset, print_cpu_wait (sum.n_wait, sum.elapsed_usec), colout->heading,
				colout->reset);

			printf ("%s %s│%s%s %s║%s%s %s│%s%s %s║%s", print_cpu_percent (sum.top1s),
				colout->heading, colout->reset, print_cpu_wait_val (sum.topw1s), colout->heading,
				colout->reset, print_cpu_percent (sum.top5s), colout->heading, colout->reset,
				print_cpu_wait_val (sum.topw5s), colout->heading, colout->reset);
	...
	else if (print_mode == LUC_PM_RAW)
	{
		printf ("%s "
				...
				"%s ║           ║                Single Record (µs)                ║               "
				" Summed Values (µs)                ║\n"
				"%s ║ real time ║ period │   user   │  system  │   total  │ wait/s ║ period │   "
				"user   │  system  │   total  │ wait/s ║\n"
				"%s "
				"╠═══════════╬════════╪══════════╪══════════╪══════════╪════════╬════════╪═════════"
				"═╪══════════╪══════════╪════════╣%s\n",
			colout->heading, colout->heading, colout->heading, colout->heading, colout->reset);
		...
		for (int i = 0; i < cpu_usage.par.used && sum.elapsed_usec < limit_usec; i++)
		{
			...
			printf (" %s║%s%s %s║%s %s %s│%s %s %s│%s %s %s│%s %s %s│%s%s %s║%s", colout->heading,
				colout->reset, PrintRefTime (e->time_sec, &reftime), colout->heading, colout->reset,
				PrintTimerUSec6 (0, 0, e->elapsed_usec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset, print_cpu_seconds (e->utime_usec), colout->heading,
				colout->reset, print_cpu_seconds (e->stime_usec), colout->heading, colout->reset,
				print_cpu_seconds (e->utime_usec + e->stime_usec), colout->heading, colout->reset,
				print_cpu_wait (e->n_wait, e->elapsed_usec), colout->heading, colout->reset);

			printf (" %s %s│%s %s %s│%s %s %s│%s %s %s│%s%s %s║%s",
				PrintTimerUSec6 (0, 0, sum.elapsed_usec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset, print_cpu_seconds (sum.utime_usec), colout->heading,
				colout->reset, print_cpu_seconds (sum.stime_usec), colout->heading, colout->reset,
				print_cpu_seconds (sum.utime_usec + sum.stime_usec), colout->heading, colout->reset,
				print_cpu_wait (sum.n_wait, sum.elapsed_usec), colout->heading, colout->reset);
	...
	else
	{
		printf ("%s "
				"╔═══════════╦═════════════════════════════════════════╦════════════════╦══════════"
				"══════╦═════════════════════════════════════════╗\n"
				"%s ║           ║              Single Record              ║    top 1s      ║    "
				"top 5s      ║              Summed Values              ║\n"
				"%s ║ real time ║ period │ user%% │system%%│total%% │ wait/s ║total%% │ wait/s "
				"║total%% │ wait/s ║ period │ user%% │system%%│total%% │ wait/s ║\n"
				"%s "
				"╠═══════════╬════════╪═══════╪═══════╪═══════╪════════╬═══════╪════════╬═══════╪══"
				"══════╬════════╪═══════╪═══════╪═══════╪════════╣%s\n",
			colout->heading, colout->heading, colout->heading, colout->heading, colout->reset);
...
#endif
		{
			...
			printf (" %s║%s%s %s║%s %s %s│%s%s %s│%s%s %s│%s%s %s│%s%s %s║%s", colout->heading,
				colout->reset, PrintRefTime (e->time_sec, &reftime), colout->heading, colout->reset,
				PrintTimerUSec6 (0, 0, e->elapsed_usec, DC_SFORM_ALIGN | DC_SFORM_DASH),
				colout->heading, colout->reset, print_cpu_percent (e->utime_usec * elapsed),
				colout->heading, colout->reset, print_cpu_percent (e->stime_usec * elapsed),
				colout->heading, colout->reset,
				print_cpu_percent ((e->utime_usec + e->stime_usec) * elapsed), colout->heading,
				colout->reset, print_cpu_wait (e->n_wait, e->elapsed_usec), colout->heading,
				colout->reset);

			printf ("%s %s│%s%s %s║%s%s %s│%s%s %s║%s", print_cpu_percent (e->top1s),
				colout->heading, colout->reset, print_cpu_wait_val (e->topw1s), colout->heading,
				colout->reset, print_cpu_percent (e->top5s), colout->heading, colout->reset,
				print_cpu_wait_val (e->topw5s), colout->heading, colout->reset);
...
#define SRT_NAME CpuUsage_t
	...
	DEF_SRT_COMMENT ("cpu_usage"), DEF_SRT_SEPARATOR (),

	DEF_SRT_INT (par.enabled, "cpu-enabled"), DEF_SRT_UINT (par.used, "cpu-used"),
	DEF_SRT_ARRAY_A (entry), DEF_SRT_UINT (entry[0].elapsed_usec, "cpu-elapsed"),
	DEF_SRT_UINT (entry[0].utime_usec, "cpu-utime"),
	DEF_SRT_UINT (entry[0].stime_usec, "cpu-stime"), DEF_SRT_UINT (entry[0].time_sec, "cpu-rtime"),
	DEF_SRT_FLOAT (entry[0].top1s, "cpu-top1s"), DEF_SRT_FLOAT (entry[0].top5s, "cpu-top5s"),
	DEF_SRT_UINT (entry[0].n_wait, "cpu-nwait"), DEF_SRT_FLOAT (entry[0].topw1s, "cpu-topw1s"),
	DEF_SRT_FLOAT (entry[0].topw5s, "cpu-topw5s"), DEF_SRT_ARRAY_END (),
	...
	if (cur.elapsed_usec >= cpu_usage.ref.elapsed_usec + USEC_PER_SEC)
	{
		...
		if (!getrusage (RUSAGE_SELF, &ru))
		{
			...
			if (cpu_usage.ref.elapsed_usec)
			{
				...
				{
					u_usec_t min = CPU_USAGE_NEXT_MIN (USEC_PER_SEC);
					for (index = CPU_USAGE_FIRST_ADD; index < cpu_usage.par.used;
...
double get_rusage_percent (void)
{
	...
	if (prev_time && cur_time > prev_time)
		percent
			= (double)(100 * NSEC_PER_USEC * (cur_rusage - prev_rusage)) / (cur_time - prev_time);
...
static u32 random32_a, random32_c,
	random32_X; // Die letzten Werte

static u32 random32_a_tab[] = // Init-Tabelle
	{ 0xbb40e62d, 0x3dc8f2f5, 0xdc024635, 0x7a5b6c8d, 0x583feb95, 0x91e06dbd, 0xa7ec03f5, 0 };
...
)
{
	...
	while (count < buf_size)
	{
		...
		{
			uint scanned;
			uint num = ScanDigits (source, end_source, 16, n_digits & 1 ? 1 : 2, &scanned);
			if (!scanned)
				return count;
...
#else
	const uint base = src[0] == '0' && (src[1] == 'x' || src[1] == 'X') ? 16 : default_base;
#endif
...
char *PrintNumberU4 (char *buf, // result buffer
...
char *PrintNumberU5 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN, DC_SFORM_DASH
...
char *PrintNumberU6 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN, DC_SFORM_DASH
...
char *PrintNumberU7 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN, DC_SFORM_DASH
...
char *PrintNumberS5 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
char *PrintNumberS6 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	s64 num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN, DC_SFORM_DASH
...
char *PrintNumberS7 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	s64 num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN, DC_SFORM_DASH
...
char *PrintNumberD5 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
char *PrintNumberD6 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	double num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN
...
char *PrintNumberD7 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	double num, // number to print
	sizeform_mode_t mode // any of DC_SFORM_ALIGN
	...
	)
{
	...
	switch (mode & DC_SIZE_M_MODE)
	{
		...
		case DC_SIZE_K:
			return force_1000 ? "kB" : "KiB";
		case DC_SIZE_M:
			return force_1000 ? "MB" : "MiB";
		case DC_SIZE_G:
			return force_1000 ? "GB" : "GiB";
		case DC_SIZE_T:
			return force_1000 ? "TB" : "TiB";
		case DC_SIZE_P:
			return force_1000 ? "PB" : "PiB";
		case DC_SIZE_E:
			return force_1000 ? "EB" : "EiB";
	}
...
char *PrintSize (char *buf, // result buffer
							// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	switch (mode & DC_SIZE_M_MODE)
	{
		...
		case DC_SIZE_K:
			if (force_1000)
				snprintf (
					buf, buf_size, aligned ? "%5llu kB" : "%llu kB", (size + KB_SI / 2) / KB_SI);
			else
				snprintf (
					buf, buf_size, aligned ? "%4llu KiB" : "%llu KiB", (size + KiB / 2) / KiB);
			break;

		case DC_SIZE_M:
			if (force_1000)
				snprintf (
					buf, buf_size, aligned ? "%5llu MB" : "%llu MB", (size + MB_SI / 2) / MB_SI);
			else
				snprintf (
					buf, buf_size, aligned ? "%4llu MiB" : "%llu MiB", (size + MiB / 2) / MiB);
			break;

		case DC_SIZE_G:
			if (force_1000)
				snprintf (
					buf, buf_size, aligned ? "%5llu GB" : "%llu GB", (size + GB_SI / 2) / GB_SI);
			else
				snprintf (
					buf, buf_size, aligned ? "%4llu GiB" : "%llu GiB", (size + GiB / 2) / GiB);
			break;

		case DC_SIZE_T:
			if (force_1000)
				snprintf (
					buf, buf_size, aligned ? "%5llu TB" : "%llu TB", (size + TB_SI / 2) / TB_SI);
			else
				snprintf (
					buf, buf_size, aligned ? "%4llu TiB" : "%llu TiB", (size + TiB / 2) / TiB);
			break;

		case DC_SIZE_P:
			if (force_1000)
				snprintf (
					buf, buf_size, aligned ? "%5llu PB" : "%llu PB", (size + PB_SI / 2) / PB_SI);
			else
				snprintf (
					buf, buf_size, aligned ? "%4llu PiB" : "%llu PiB", (size + PiB / 2) / PiB);
			break;

		case DC_SIZE_E:
			if (force_1000)
				snprintf (
					buf, buf_size, aligned ? "%5llu EB" : "%llu EB", (size + EB_SI / 2) / EB_SI);
			else
				snprintf (
					buf, buf_size, aligned ? "%4llu EiB" : "%llu EiB", (size + EiB / 2) / EiB);
			break;

			//----- default == auto

		default:
			buf = force_1000 ? PrintSize1000 (buf, buf_size, size, sform_mode)
...
char *PrintSize1000 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 size, // size to print
	sizeform_mode_t sform_mode // output format, bit field
)
{
	...
	if (sform_mode & DC_SFORM_INC && num && !(num % 1000) && dc_size_tab_1000[unit + 1])
	{
		unit++;
		num /= 1000;
	}
...
char *PrintSize1024 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u64 size, // size to print
	sizeform_mode_t sform_mode // output format, bit field
)
{
	...
	if (sform_mode & DC_SFORM_INC && num && !(num & 0x3ff) && dc_size_tab_1024[unit + 1])
	{
		unit++;
		num /= 0x400;
	}
...
char *PrintHMS (char *buf, // result buffer
						   // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
...
)
{
	...
	if (!buf)
		return hours  ? PrintCircBuf (
							"%s%s%u:%02u:%02u%s", pre_text, sign, hours, minutes, seconds, post_text)
			: minutes ? PrintCircBuf ("%s%s%u:%02u%s", pre_text, sign, minutes, seconds, post_text)
					  : PrintCircBuf ("%s%s%02us%s", pre_text, sign, seconds, post_text);

	if (hours)
		snprintf (buf, buf_size, "%s%s%u:%02u:%02u%s", pre_text, sign, hours, minutes, seconds,
			post_text);
	else if (minutes)
...
u64 (*ScanSizeFactorHook) (char ch_factor, // char to analyze
	int force_base // if 1000|1024: force multiple of this
	) = 0;

//-----------------------------------------------------------------------------

u64 ScanSizeFactor (char ch_factor, // char to analyze
	int force_base // if 1000|1024: force multiple of this
)
{
...
char *ScanSizeTerm (double *num, // not NULL: store result
	ccp source, // source text
...
char *ScanSize (double *num, // not NULL: store result
	ccp source, // source text
	u64 default_factor, // use this factor if number hasn't one
...
char *ScanSizeU32 (u32 *num, // not NULL: store result
	ccp source, // source text
	u64 default_factor, // use this factor if number hasn't one
	u64 default_factor_add, // use this factor for summands
	int force_base // if 1000|1024: force multiple of this
...
char *ScanSizeU64 (u64 *num, // not NULL: store result
	ccp source, // source text
	u64 default_factor, // use this factor if number hasn't one
	u64 default_factor_add, // use this factor for summands
	int force_base // if 1000|1024: force multiple of this
...
char *ScanSizeRange (int *stat, // if not NULL: store result
...
)
{
	...
	{
		have_d1 = *source == ':' && opt & RAOPT_COLON || *source == '#' && opt & RAOPT_HASH
	...
	else
	{
		...
		const bool have_range
			= *end == ':' && opt & RAOPT_COLON || *end == '-' && opt & RAOPT_MINUS;
		const bool have_size = *end == '#' && opt & RAOPT_HASH || *end == ',' && opt & RAOPT_COMMA;
...
char *ScanSizeRangeU32 (int *stat, // if not NULL: store result
								   //	0:none, 1:single, 2:range
...
{
	double d1, d2;
	char *end = ScanSizeRange (stat, &d1, &d2, source, default_factor, default_factor_add,
		force_base, max_value > 0 ? max_value : U32_MAX, opt);
...
char *ScanSizeRangeS32 (int *stat, // if not NULL: store result
								   //	0:none, 1:single, 2:range
...
)
{
	double d1, d2;
	char *end = ScanSizeRange (stat, &d1, &d2, source, default_factor, default_factor_add,
		force_base, max_value > 0 ? max_value : S32_MAX, opt);
...
char *ScanSizeRangeU64 (int *stat, // if not NULL: store result
								   //	0:none, 1:single, 2:range
...
)
{
	double d1, d2;
	char *end = ScanSizeRange (stat, &d1, &d2, source, default_factor, default_factor_add,
		force_base, max_value > 0 ? max_value : U64_MAX, opt);
...
char *ScanSizeRangeS64 (int *stat, // if not NULL: store result
								   //	0:none, 1:single, 2:range
...
)
{
	double d1, d2;
	char *end = ScanSizeRange (stat, &d1, &d2, source, default_factor, default_factor_add,
		force_base, max_value > 0 ? max_value : S64_MAX, opt);
...
char *ScanSizeRangeList (uint *n_range, // not NULL: store number of scanned ranges
...
)
{
	...
	while (i < max_range)
	{
		...
		int stat;
		ptr = ScanSizeRange (&stat, num, num + 1, ptr, default_factor, default_factor_add,
...
char *ScanSizeRangeListU32 (uint *n_range, // not NULL: store number of scanned ranges
...
)
{
	...
	char *end = ScanSizeRangeList (n_range, d, max_range, source, default_factor,
		default_factor_add, force_base, max_value, opt);
...
char *ScanSizeRangeListU64 (uint *n_range, // not NULL: store number of scanned ranges
...
enumError ScanSizeOpt (double *num, // not NULL: store result
	ccp source, // source text
	u64 default_factor, // use this factor if number hasn't one
	u64 default_factor_add, // use this factor for summands
	int force_base, // if 1000|1024: force multiple of this
...
)
{
...
#ifdef DEBUG
	{
		...
		TRACE ("--%s %8.6g ~ %llu ~ %llu GiB ~ %llu GB\n", opt_name, d, size,
			(size + GiB / 2) / GiB, (size + 500000000) / 1000000000);
	}
#endif
	...
	if (source == end || *end)
	{
		err = ERR_SYNTAX;
		if (print_err)
			ERROR0 (ERR_SYNTAX, "Invalid number for option --%s: %s\n", opt_name, source);
	}
	else if (min > 0 && d < min)
	{
		err = ERR_SYNTAX;
		if (print_err)
			ERROR0 (ERR_SEMANTIC, "Value of --%s too small (must not <%llu): %s\n", opt_name, min,
				source);
	}
	else if (max > 0 && d > max)
	{
		err = ERR_SYNTAX;
		if (print_err)
			ERROR0 (ERR_SEMANTIC, "Value of --%s too large (must not >%llu): %s\n", opt_name, max,
				source);
	}
...
enumError ScanSizeOptU64 (u64 *num, // not NULL: store result
	ccp source, // source text
	u64 default_factor, // use this factor if number hasn't one
	int force_base, // if 1000|1024: force multiple of this
...
)
{
	...
	if (pow2 && !force_base)
	{
		...
		u64 val;
		if (!ScanSizeOptU64 (
				&val, source, default_factor, 1024, opt_name, min, max, multiple, pow2, false))
		{
			if (num)
				*num = val;
			return ERR_OK;
	...
	double d = 0.0;
	enumError err = ScanSizeOpt (&d, source, default_factor, multiple ? multiple : 1, force_base,
		opt_name, min, max, print_err);
	...
	if (err == ERR_OK && pow2 > 0)
	{
		...
		if (shift_val != 1 || shift_count / pow2 * pow2 != shift_count)
		{
			err = ERR_SEMANTIC;
			if (print_err)
				ERROR0 (ERR_SYNTAX, "--%s: value must be a power of %d but not %llu\n", opt_name,
					1 << pow2, val);
		}
	}
...
enumError ScanSizeOptU32 (u32 *num, // not NULL: store result
	ccp source, // source text
	u64 default_factor, // use this factor if number hasn't one
	int force_base, // if 1000|1024: force multiple of this
...
)
{
	...
	enumError err = ScanSizeOptU64 (
		&val, source, default_factor, force_base, opt_name, min, max, multiple, pow2, print_err);
...
)
{
	...
	{
		case 'y':
			default_factor = 1e-24;
			break;
		case 'z':
			default_factor = 1e-21;
			break;
		case 'a':
			default_factor = 1e-18;
			break;
		case 'f':
			default_factor = 1e-15;
			break;
		case 'p':
			default_factor = 1e-12;
			break;
		case 'n':
			default_factor = 1e-9;
			break;
		...
		case 'P':
			default_factor = 1e15;
			i = 1125899906842624.0;
			break;
		case 'E':
			default_factor = 1e18;
			i = 1152921504606846976.0;
			break;
		case 'Z':
			default_factor = 1e21;
			i = 1180591620717411303424.0;
			break;
		case 'Y':
			default_factor = 1e24;
			i = 1208925819614629174706176.0;
			break;
...
ipv4_class_t ScanIP4 (ipv4_t *ret_ip4, // not NULL: return numeric IPv4 here
...
)
{
	...
	if (port >= 0 && port <= 0xffff)
		snprintf (buf, buf_size, "%u.%u:%u", addr[0], ip4 & 0xffffff, port);
	else
		snprintf (buf, buf_size, "%u.%u", addr[0], ip4 & 0xffffff);
	return buf;
}
...
)
{
	...
	if (port >= 0 && port <= 0xffff)
		snprintf (buf, buf_size, "%u.%u.%u:%u", addr[0], addr[1], ip4 & 0xffff, port);
	else
		snprintf (buf, buf_size, "%u.%u.%u", addr[0], addr[1], ip4 & 0xffff);
	return buf;
}

///////////////////////////////////////////////////////////////////////////////

char *PrintIP4 (char *buf, // result buffer
						   // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u32 ip4, // IP4 to print
	s32 port // 0..0xffff: add ':port'
)
{
	...
	if (port >= 0 && port <= 0xffff)
		snprintf (buf, buf_size, "%u.%u.%u.%u:%u", addr[0], addr[1], addr[2], addr[3], port);
	else
...
char *PrintLeftIP4 (char *buf, // result buffer
							   // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u32 ip4, // IP4 to print
	s32 port // 0..0xffff: add ':port'
)
{
	...
	{
		fw = 21;
		len = snprintf (buf, buf_size, "%u.%u.%u.%u:%u", addr[0], addr[1], addr[2], addr[3], port);
	}
	else
	{
		fw = 15;
		len = snprintf (buf, buf_size, "%u.%u.%u.%u", addr[0], addr[1], addr[2], addr[3]);
	}
...
char *PrintRightIP4 (char *buf, // result buffer
								// NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u32 ip4, // IP4 to print
	s32 port, // 0..0xffff: add ':port'
...
)
{
	...
	char tempbuf[16];
	snprintf (tempbuf, sizeof (tempbuf), "%u.%u.%u.%u", addr[0], addr[1], addr[2], addr[3]);
...
char *PrintAlignedIP4 (char *buf, // result buffer
								  // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u32 ip4, // IP4 to print
	s32 port // 0..0xffff: add ':port'
)
{
	...
	if (port >= 0 && port <= 0xffff)
		snprintf (buf, buf_size, "%3u.%3u.%3u.%3u:%5u", addr[0], addr[1], addr[2], addr[3], port);
	else
		snprintf (buf, buf_size, "%3u.%3u.%3u.%3u", addr[0], addr[1], addr[2], addr[3]);
	return buf;
}

///////////////////////////////////////////////////////////////////////////////

char *PrintIP4ByMode (char *buf, // result buffer
								 // NULL: use a local circulary static buffer
	size_t buf_size, // size of 'buf', ignored if buf==NULL
	u32 ip4, // IP4 to print
...
)
{
	...
	{
		case CIP4_NUMERIC:
			return PrintIP4N (buf, buf_size, ip4, port);
		case CIP4_CLASS_A:
			return PrintIP4A (buf, buf_size, ip4, port);
		case CIP4_CLASS_B:
			return PrintIP4B (buf, buf_size, ip4, port);
		default:
			return PrintIP4 (buf, buf_size, ip4, port);
	}
}
...
u64 MulDivU64 (u64 factor1, u64 factor2, u64 divisor)
{
	...
	return product < 0xfffffffffffffff0 ? factor1 * factor2 / divisor
