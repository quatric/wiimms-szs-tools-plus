...
const char LoDigits[0x41] = "0123456789"
							"abcdefghijklmnopqrstuvwxyz"
							"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
							".+";

const char HiDigits[0x41] = "0123456789"
							"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
							"abcdefghijklmnopqrstuvwxyz"
							".+";
...
)
{
	...
	if (check_proc)
	{
		static ccp tab[] = { "/proc/self/exe", "/proc/curproc/file", "/proc/self/path/a.out", 0 };
		...
		for (ptr = tab; *ptr; ptr++)
		{
			if (realpath (*ptr, path) && *path)
			{
				...
				if (!memcmp (path, "/SZS/", 5))
					return StringCat2S (buf, buf_size, "/cygdrive/c/Program Files/Wiimm", path)
						- buf;
#endif
				return StringCopyS (buf, buf_size, path) - buf;
			}
...
char *StringCatSep3S (char *buf, ssize_t buf_size, ccp sep, ccp src1, ccp src2, ccp src3)
{
...
{
	mem_t mem = MemCat3A (
		MemByStringS (src1, len1), MemByStringS (src2, len2), MemByStringS (src3, len3));
	return (char *)mem.ptr;
}
...
{
	mem_t mem = MemCatSep2A (MemByString0 (sep), MemByString0 (src1), MemByString0 (src2));
	return (char *)mem.ptr;
}
...
{
	mem_t mem = MemCatSep3A (
		MemByString0 (sep), MemByString0 (src1), MemByString0 (src2), MemByString0 (src3));
	return (char *)mem.ptr;
}

///////////////////////////////////////////////////////////////////////////////

ccp StringCenterE (char *buf, // result buffer; if NULL, circ-buf is used
...
char *NewFileExtE (char *buf, ccp buf_end, ccp path, ccp ext)
{
	...
	char *end = path && path != buf ? StringCopyE (buf, buf_end - 1, path) : buf + strlen (buf);
	DASSERT (end < buf_end);
...
)
{
	...
	noPRINT ("CIRC-BUF: %3u -> %3zu / %3zu / %3zu\n", buf_size, result - circ_buf,
		circ_ptr - circ_buf, sizeof (circ_buf));
	return result;
}
...
)
{
	...
	return try_circ && src_len <= CIRC_BUF_MAX_ALLOC ? CopyCircBuf (src, src_len)
													 : MEMDUP (src, src_len);
}
...
)
{
	...
	if (end_buf == circ_ptr && release_size <= CIRC_BUF_MAX_ALLOC
...
ccp GetFastBufStatus (const FastBuf_t *fb)
{
	char info[100];
	int len
		= snprintf (info, sizeof (info), "%s=%u/%u", fb->buf == fb->fast_buf ? "FastBuf" : "Alloc",
...
void AppendInt64FastBuf (FastBuf_t *fb, u64 val, IntMode_t mode)
{
	...
	switch (mode)
	{
		...
		case IMD_BE1:
		case IMD_LE1:
			AppendCharFastBuf (fb, val);
			break;

		case IMD_BE2:
			write_be16 (GetSpaceFastBuf (fb, 2), val);
			break;
		case IMD_BE3:
			write_be24 (GetSpaceFastBuf (fb, 3), val);
			break;
		case IMD_BE4:
			write_be32 (GetSpaceFastBuf (fb, 4), val);
			break;
		case IMD_BE5:
			write_be40 (GetSpaceFastBuf (fb, 5), val);
			break;
		case IMD_BE6:
			write_be48 (GetSpaceFastBuf (fb, 6), val);
			break;
		case IMD_BE7:
			write_be56 (GetSpaceFastBuf (fb, 7), val);
			break;
		case IMD_BE0:
		case IMD_BE8:
			write_be64 (GetSpaceFastBuf (fb, 8), val);
			break;

		case IMD_LE2:
			write_le16 (GetSpaceFastBuf (fb, 2), val);
			break;
		case IMD_LE3:
			write_le24 (GetSpaceFastBuf (fb, 3), val);
			break;
		case IMD_LE4:
			write_le32 (GetSpaceFastBuf (fb, 4), val);
			break;
		case IMD_LE5:
			write_le40 (GetSpaceFastBuf (fb, 5), val);
			break;
		case IMD_LE6:
			write_le48 (GetSpaceFastBuf (fb, 6), val);
			break;
		case IMD_LE7:
			write_le56 (GetSpaceFastBuf (fb, 7), val);
			break;
		case IMD_LE0:
		case IMD_LE8:
			write_le64 (GetSpaceFastBuf (fb, 8), val);
			break;
	}
}
...
uint SetupAllocInfo ()
{
	if (!alloc_info_calced)
	{
		...
		for (i = 1; i <= MAX; i++)
		{
			...
			if (i > 1)
			{
				const int diff = abs ((int)(uintptr_t)(alloc[i - 1]) - (int)(uintptr_t)(alloc[i]));
				mask_found &= diff - 1;
			}
		}
		...
		if (hi1 + 1 == lo0)
		{
			...
			{
				const int diff
					= abs ((int)(uintptr_t)(alloc[i - 1]) - (int)(uintptr_t)(alloc[i])) - i;
			...
			if (found < MAX)
			{
				...
				PRINT ("alloc_info_overhead = %d, alloc_info_add = %d, %d\n", alloc_info_overhead,
					alloc_info_add1, alloc_info_add2);
			}
		}
...
)
{
...
#endif
	char *argv_predef[N_PREDEF + 1], **argv_buf = argv_predef, **argv_dest = argv_predef,
	...
	*argv_dest++ = ProgInfo.progname ? (char *)ProgInfo.progname : "";
	...
	{
		while (
			src < src_end && (!*src || *src == ' ' || *src == '\t' || *src == '\r' || *src == '\n'))
		{
			src++;
		}
		if (src == src_end)
		...
		{
			PRINT ("NEED MORE: %zu/%zu size=%u: |%s|\n", argv_dest - argv_buf, argv_end - argv_buf,
				argv_size, src);
...
void **ReplacePointerMgr (PointerList_t *pl, bool reset, const PointerList_t **old_list)
{
	FREE (old_list);
...
static char **insert_arg_manager (ArgManager_t *am, char **dest, ccp p_arg, bool move_arg)
{
	DASSERT (am);
	if (p_arg)
	{
...
mem_t MemCatSep3A (const mem_t sep, const mem_t m1, const mem_t m2, const mem_t m3)
{
	...
	char *dest;
	mem_t res;
	res.len = len1 + len2 + len3 + ((len1 > 0) + (len2 > 0) + (len3 > 0) - 1) * slen;
...
)
{
	...
	if (src)
	{
		...
		{
			em.data.len = src_len;
			if (orig && src_len == (orig_len < 0 ? strlen (orig) : orig_len)
...
ccp PrintExMem (const exmem_t *em) // print to circ-buf
{
	...
	return PrintCircBuf ("[%c%c%c%c:%x] %u/%u %s",
		em->is_key_alloced ? 'a' : '-', // flags: same as in DumpEML()
		em->is_alloced ? 'A' : '-', em->is_circ_buf ? 'C' : '-', em->is_original ? 'O' : '-',
		em->attrib, len, em->data.len, ebuf);
}
...
)
{
	...
	DASSERT_MSG (dest == ml->list + pos + add_elem, "DEST= %zu != %u+%u == %u", dest - ml->list,
		pos, add_elem, pos + add_elem);
...
)
{
	...
	char *new_buf = dest_is_source || need_size > dest->buf_size ? MALLOC (need_size) : dest->buf;
	mem_t *new_list = dest_is_source || need_elem > dest->size
		? MALLOC (sizeof (*new_list) * need_elem)
		: dest->list;
...
)
{
	...
	fprintf (f, "%*s%s%s%smod=%d, PL=%d, ", indent, "", colset->caption, prefix ? prefix : "",
...
)
{
	...
	fprintf (f, "%*s%s%s%sPC=%d, RC=%d, alloc=%d, size=%u=0x%x, addr=%p\n", indent, "",
		colset->caption, prefix ? prefix : "", colset->status, cdata->protect_count,
		cdata->ref_count, cdata->data_alloced, cdata->size, cdata->size, cdata->data);
...
static ccp AnalyseBrackets (ccp pattern, ccp *p_start, bool *p_negate, int *p_multiple)
{
	ASSERT (pattern);
...
)
{
	...
	for (;;)
	{
		...
		const KeywordTab_t *cptr
			= ScanKeywordEx (&abbrev_count, key, arg - key, par->force_case, key_tab);
		...
		if (abbrev_count)
		{
			if (par->min_number >= 0 && par->max_number < 0
				&& par->min_number <= (u64)par->max_number)
			{
		...
		switch (prefix)
		{
			case 0:
			case '+':
				par->result = par->result & ~cptr->opt | cptr->id;
				break;
			case '-':
				par->result &= ~cptr->id;
				break;
...
)
{
	...
	for (;;)
	{
		...
		*dest = 0;
		...
		noPRINT ("cptr=%p [%s], opt=%llx, prefix=%02x\n", cptr, cptr ? cptr->name1 : "",
			cptr ? cptr->opt : 0, prefix);
		if (!cptr || cptr->opt && prefix && prefix != '+')
		{
			if (err_msg)
			{
		...
		else
		{
			switch (prefix)
			{
				case 0:
				case '+':
					result = result & ~cptr->opt | cptr->id;
					break;
				case '-':
					result &= ~cptr->id;
					break;
...
)
{
	...
	for (;;)
	{
		...
		char *dest = key_buf;
		while (*arg > ' ' && *arg != ',' && *arg != '.' && (*arg != '+' || dest == key_buf)
			&& dest < end)
			*dest++ = *arg++;
		*dest = 0;
		char prefix = 0;
...
static s64 ScanKeywordListMaskHelper (void *param, // NULL or user defined parameter
...
{
	return key->opt ? result & ~key->opt | key->id : key->id;
}
...
ccp GetKeywordError (const KeywordTab_t *key_tab, // NULL or pointer to command table
	ccp key_arg, // analyzed command
...
)
{
	...
	*dest = 0;
	...
	return PrintCircBuf (
		"%c%s abbreviation is ambiguous: %s%s", toupper ((int)*object), object + 1, key_arg, buf);
}
...
const KeywordTab_t KeyTab_OFF_AUTO_ON[] = { { OFFON_OFF, "OFF", "-1", 0 },
	{ OFFON_OFF, "DISABLED", "NO", 0 }, { OFFON_AUTO, "AUTO", "0", 0 }, { OFFON_ON, "ON", "1", 0 },
	{ OFFON_ON, "ENABLED", "YES", 0 }, { OFFON_FORCE, "FORCE", "2", 0 }, { 0, 0, 0, 0 } };
...
const KeywordTab_t KeyTab_LOWER_AUTO_UPPER[] = { { LOUP_LOWER, "LOWER", "LOWERCASE", 0 },
	{ LOUP_LOWER, "LOCASE", "-1", 0 }, { LOUP_AUTO, "AUTO", "0", 0 },
	{ LOUP_UPPER, "UPPER", "UPPERCASE", 0 }, { LOUP_UPPER, "UPCASE", "1", 0 }, { 0, 0, 0, 0 } };
...
int ExecCommandOfList (CommandList_t *cli)
{
	...
	if (cli->n_bin)
	{
		printf ("%3u. %s [n_bin=%u] = |%.*s|\n", cli->cmd_count, cli->cmd, cli->n_bin,
			cli->param_len, cli->param);
		uint i;
		for (i = 0; i < cli->n_bin; i++)
		{
			printf (" > binary #%u, len=%u\n", i, cli->bin[0].len);
			HexDump16 (
				stdout, 5, 0, cli->bin[i].ptr, cli->bin[i].len < 0x20 ? cli->bin[i].len : 0x20);
		}
	}
	else
...
)
{
	...
	for (;;)
	{
		...
		while (cli->n_bin < COMMAND_LIST_N_BIN)
		{
			...
			if (log_level > 2)
			{
				fprintf (log, "bin %u,%u..%u/%u%s\n", bin_len, (uint)(param - cli->command),
					(uint)(param - cli->command + bin_len), cli->command_len,
...
abort:;
	...
	{
		fprintf (log, "-- %u,%u/%u%s\n", cli->cmd_count, cli->read_cmd_len, cli->command_len,
...
)
{
	...
	return index < ddl->used ? ddl->list + ddl->used
		: create			 ? InsertDDL (ddl, index >= create ? index : create - 1, 0, 0)
							 : 0;
}
...
uint FindStringFieldHelper (const StringField_t *sf, bool *p_found, ccp key)
{
	...
	if (sf && key)
	{
		...
		while (beg <= end)
		{
			...
			{
				noTRACE (
					"FindStringFieldHelper(%s) FOUND=%d/%d/%d\n", key, idx, sf->used, sf->size);
				if (p_found)
					*p_found = true;
				return idx;
			}
...
)
{
	...
	for (end = ptr + sf->used; ptr < end; ptr++)
		if (fprintf (f, "%s%s%s", prefix, *ptr, eol) < 0)
			return ERROR1 (ERR_WRITE_FAILED, "Error while writing string list: %s\n",
				filename ? filename : "?");
	return ERR_OK;
}
...
void RestoreStateStringField (RestoreState_t *rs, // info data, may be modified
	ccp name_prefix, // NULL or prefix of name, up to 40 chars
...
static exmem_key_t *InsertHelperEML (
	exmem_list_t *eml, int idx, ccp key, CopyMode_t cm_key, const exmem_t *data, CopyMode_t cm_data)
{
	DASSERT (eml);
...
void DumpEML (FILE *f, int indent, const exmem_list_t *eml, ccp info)
{
	if (f && eml && eml->used)
	{
		...
		fprintf (f,
			"%*s%s%s"
			"exmem list %p, N=%u/%u%s\n",
			indent, "", info ? info : "", info ? " : " : "", eml, eml->used, eml->size,
			eml->is_unsorted ? ", not sorted" : "");
		...
		for (ptr = eml->list; ptr < end; ptr++, idx++)
			fprintf (f, "%*d [%c%c%c%c] %-*s = [%x] %s\n", fw_idx, idx,
				...
				ptr->data.is_alloced ? 'A' : '-', ptr->data.is_circ_buf ? 'C' : '-',
				ptr->data.is_original ? 'O' : '-', fw_key, ptr->key ? ptr->key : "",
...
exmem_t ResolveSymbolsEML (const exmem_list_t *eml, ccp source)
{
	...
	if (source)
	{
		...
		for (try = 0; try < 50; try ++)
		{
			...
			char *temp
				= MEMDUP3 (source, p1 - source, replace.ptr, replace.len, p2 + 1, strlen (p2 + 1));
			if (em.is_alloced)
				FreeString (source);
			source = temp;
			em.is_alloced = true;
	...
	for (try = 0; try < 20; try ++)
	{
		...
		for (ptr = eml->list; ptr < end; ptr++)
		{
			for (int try2 = 0; try2 < 10; try2++)
			{
				...
				char *temp
					= MEMDUP3 (str, p1 - str, replace.ptr, replace.len, p2 + 1, strlen (p2 + 1));
				if (ptr->data.is_alloced)
					FreeString (str);
				ASSERT (temp);
...
{
	FILE *f; // output file
	const ColorSet_t *colset; // color set for the frame
...
{
	fprintf (p->f, "%s%*s%s%.3s", p->prefix, p->indent, "", p->colset->heading, template);
...
static void mc_print_line2 (const mc_param_t *p, ccp template)
{
	...
	for (int col = 0; col < p->n_cols; col++, fw++)
	{
		...
		fprintf (p->f, "%.*s%.3s%.*s", 3 * (fw->key + 2), DoubleLine300_3, template + 3,
...
int PrintMultiColumn (const MultiColumn_t *mc)
{
	...
	if (p.debug > 1)
		fprintf (p.f,
			...
			p.debug, QuoteStringS (p.prefix, 0, CHMD__MODERN), p.indent,
			QuoteStringS (p.eol, 0, CHMD__MODERN), p.fw, p.frame_mode, mc->print_header,
			mc->print_footer, mc->min_rows, mc->max_cols, p.eol);
	...
	if (p.debug > 1)
		fprintf (p.f, "#> src-lines=%d, min-fw=%d, min-rows=%d, max-cols=%d/%d%s", mc->list.used,
			min_fw, min_rows, max_cols, MULTI_COLUMN_MAX_COLS, p.eol);
	...
	for (p.n_cols = max_cols; p.n_cols > 0; p.n_cols--)
	{
		...
		if (p.debug > 1)
			fprintf (p.f, "#> … try %2u columns and %3u rows, %3u cells total%s", p.n_cols,
				p.n_rows, p.n_cols * p.n_rows, p.eol);
	...
	//----------------------
	{
		...
		if (mc->print_header)
		{
			...
			for (int col = 0; col < p.n_cols; col++, fw++)
				fprintf (p.f, " %s %s│%s %s %s║%s",
					AlignEUTF8ToCircBuf (p.head_key, -fw->key, fw->key), p.colset->heading,
					p.colset->reset, AlignEUTF8ToCircBuf (p.head_val, -fw->val, fw->val),
		...
		for (int row = 0; row < p.n_rows; row++)
		{
			...
			for (int col = 0; col < p.n_cols; col++, ptr += p.n_rows, fw++)
			{
				if (ptr < list_end)
				{
					fprintf (p.f, " %s %s│%s ", AlignEUTF8ToCircBuf (ptr->key, -fw->key, fw->key),
						p.colset->heading, p.colset->reset);

					if (ptr->data.attrib > 1)
					{
						const int fw_attrib
							= ptr->data.attrib < fw->val ? ptr->data.attrib : fw->val;
						const int fw_val = p.len_list[ptr - mc->list.list].val;
						if (fw_attrib <= fw_val)
							goto std2;
						fprintf (p.f, "%s%*s %s║%s",
							AlignEUTF8ToCircBuf (ptr->data.data.ptr, fw_attrib, fw_attrib),
							fw_attrib - fw->val, "", p.colset->heading, p.colset->reset);
					}
					else
					{
					std2:;
					...
					fprintf (p.f, " %*s %s│%s %*s %s║%s", fw->key, "", p.colset->heading,
						p.colset->reset, fw->val, "", p.colset->heading, p.colset->reset);
			}
			fputs (p.eol, p.f);
		}
	...
	//---------------------------
	{
		...
		if (mc->print_header)
		{
			...
			const mc_len_t *fw = p.fw_col;
			for (int col = 0; col < p.n_cols; col++, fw++)
				fprintf (p.f, " %s%s%s %s│%s", AlignEUTF8ToCircBuf (p.head_key, -fw->key, fw->key),
					p.sep, AlignEUTF8ToCircBuf (p.head_val, -fw->val, fw->val), p.colset->heading,
					p.colset->reset);
		...
		for (int row = 0; row < p.n_rows; row++)
		{
			...
			for (int col = 0; col < p.n_cols; col++, ptr += p.n_rows, fw++)
			{
				...
				{
					fprintf (
						p.f, " %s%s", AlignEUTF8ToCircBuf (ptr->key, -fw->key, fw->key), p.sep);
					...
					fprintf (p.f, " %*s %s│%s", fw->key + fw->val + p.sep_fw, "", p.colset->heading,
						p.colset->reset);
			}
			fputs (p.eol, p.f);
		}
	...
	//-------------------
	{
		if (mc->print_header)
		{
			...
			for (int col = 0; col < p.n_cols; col++, fw++)
			{
				if (col)
					fputs (" | ", p.f);
				fprintf (p.f, "%s%s%s", AlignEUTF8ToCircBuf (p.head_key, -fw->key, fw->key), p.sep,
		...
		for (int row = 0; row < p.n_rows; row++)
		{
			...
			for (int col = 0; col < p.n_cols; col++, ptr += p.n_rows, fw++)
			{
				...
				if (ptr < list_end)
				{
					fprintf (p.f, "%s%s", AlignEUTF8ToCircBuf (ptr->key, -fw->key, fw->key), p.sep);
	...
	if (p.debug > 0)
	{
		...
		fprintf (p.f, "#> setup in %llu us, %u ouput lines in %llu us, total %llu us%s",
			output_time - start_time, out_lines, now - output_time, now - start_time, p.eol);
	}
...
)
{
	...
	if (mm2)
	{
		...
		for (i = 0; i < mm2->used; i++)
		{
			...
			if (src)
			{
				MemMapItem_t *it = use_tie ? InsertMemMapTie (mm1, src->off, src->size)
...
)
{
	...
	if (force || i1->off + i1->size >= i2->off)
	{
		...
		idx++;
		mm->used--;
		memmove (mm->field + idx, mm->field + idx + 1, (mm->used - idx) * sizeof (MemMapItem_t *));
...
void PrintMemMap (MemMap_t *mm, FILE *f, int indent, ccp info_head)
{
	...
	fprintf (f, "%*s      unused :  off(beg) ..  off(end) :      size : %s\n%*s%.*s\n", indent, "",
		info_head, indent, "", max_ilen + 52, Minus300);
	...
	for (i = 0; i < mm->used; i++)
	{
		...
		if (ptr->off > max_end)
			fprintf (f, "%*s%s%10llx :%10llx ..%10llx :%10llx : %s\n", indent, "",
				ovl[ptr->overlap & 3], (u64)(ptr->off - max_end), (u64)ptr->off, (u64)end,
				(u64)ptr->size, ptr->info);
		else
			fprintf (f, "%*s%s           :%10llx ..%10llx :%10llx : %s\n", indent, "",
				ovl[ptr->overlap & 3], (u64)ptr->off, (u64)end, (u64)ptr->size, ptr->info);
		if (max_end < end)
			max_end = end;
	}
}
...
bool IsValidDataBuf (DataBuf_t *db)
{
	...
	if (db->buf)
	{
		if (db->buf_end != db->buf + db->size || db->data < db->buf || db->data >= db->buf_end)
			return false;
...
bool DumpDataBuf (FILE *f, uint indent, DataBuf_t *db, bool if_invalid)
{
	...
	fprintf (f, "%*sdata_buf_t %p .. %d, data %d .. %d, used %u/%u, off=%llu\n", indent, "",
		db->buf, (int)(db->buf_end - db->buf), (int)(db->data - db->buf),
		(int)(db->data_end - db->buf), db->used, db->size, db->offset);
...
void InitializeGrowBuffer (GrowBuffer_t *gb, uint max_buf_size)
{
	...
	gb->max_size = (max_buf_size + grow_buffer_size_factor - 1) / grow_buffer_size_factor
		* grow_buffer_size_factor;
}
...
)
{
	...
	if (!gb->buf)
	{
		...
		if (gb->grow_size < grow_buffer_size_factor)
			...
			gb->grow_size = gb->grow_size / grow_buffer_size_factor * grow_buffer_size_factor;
		gb->size = (size / gb->grow_size + 1) * gb->grow_size;

		if (gb->max_size < grow_buffer_size_factor)
			gb->max_size = grow_buffer_size_factor;
		else
			gb->max_size = gb->max_size / grow_buffer_size_factor * grow_buffer_size_factor;
		if (gb->size > gb->max_size)
			gb->size = gb->max_size;

		if (gb->size < size && force)
			gb->size = (size + gb->grow_size) / gb->grow_size * gb->grow_size - 1;
	...
	if (size > avail && (force || gb->size < gb->max_size))
	{
		...
		gb->size = (size + gb->used + gb->grow_size) / gb->grow_size * gb->grow_size - 1;
		DASSERT (gb->used <= gb->size);
...
)
{
	...
	fprintf (f, "%*sTB: size=%s/%s/%s  idx=%s  grow=%s/%s  disabled=%d", indent, "",
		...
		PrintSize1024 (0, 0, gb->max_size, DC_SFORM_TINY | DC_SFORM_ALIGN), gb->disabled);
...
)
{
	...
	fprintf (f,
		...
		"%s"
		"max-size%.*s= %u\n",
		name_prefix, (base - 8) / 8, Tabs20, gb->disabled, name_prefix, (base - 4) / 8, Tabs20,
		gb->used, name_prefix, (base - 4) / 8, Tabs20, gb->size, name_prefix, (base - 9) / 8,
		Tabs20, gb->grow_size, name_prefix, (base - 8) / 8, Tabs20, gb->max_size);
...
)
{
	...
	{
		static ccp name_tab[] = { "size", "grow-size", "max-size", 0 };
...
{
	return bits > 0 && bits < 32 ? htonl (0xffffffffu << (32 - bits)) : bits ? 0xffffffffu : 0;
}
...
void Sha1Bin2Hex (sha1_hex_t hex, cvp bin)
{
	...
	snprintf (hex, sizeof (sha1_hex_t), "%08x%08x%08x%08x%08x", be32 (bin), be32 (bin + 4),
		be32 (bin + 8), be32 (bin + 12), be32 (bin + 16));
}
