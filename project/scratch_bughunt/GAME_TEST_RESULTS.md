...
- **BFFNT / BCFNT Outline Font & Non-Raster Sheet Handling Fix (`lib-image2.c`, `wszst.c`)**:
  - *Issue*: In Wii U titles containing scalable, outline, or glyph-direct fonts (e.g. *Mario Kart 8*, *The Wind Waker HD*, *Wii Party U*, *Splatoon*, *Super Mario Maker* with fonts like `turbo_messageFontOutline_50.bffnt`), `AssignIMG()` checked for `data[bcfnt_hdr + 8] == 1` and failed with `ERROR #38 [INVALID IMAGE FORMAT] Unsupported FINF type in BCFNT/BFFNT`. This caused the extraction pipeline to exit with `EXIT_38`.
  - *Fix*: Updated FINF header parser in `lib-image2.c` to decode any valid TGLP texture sheet regardless of `fontType`, and gracefully return `ERR_NOTHING_TO_DO` for outline/vector/glyph fonts lacking raster atlases. Updated `decode_cfnt_if_possible()` and `decode_brfnt_if_possible()` in `wszst.c` to run non-failing probes so non-raster font files do not escalate to exit code 38.
