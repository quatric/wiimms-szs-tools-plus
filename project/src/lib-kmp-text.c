...
bool IsConditionRefEnabled (const gobj_cond_mask_t *cm, u16 ref_id, const gobj_cond_ref_t *cref)
{
	DASSERT (cm);
	...
	if (IsConditionRefId (ref_id))
	{
		...
		if (cref)
		{
			...
			{
				case 0:
					return cref->cond_mask & cm->bit_mask || cref->set_mask & cm->game_mask;
...
bool GetConditionRefInfo (cond_ref_info_t *cri, u16 ref_id, u16 obj_id, bool create_cond)
{
	DASSERT (cri);
	...
	if (IsConditionRefTestId (ref_id))
	{
		...
		if (create_cond)
		{
			snprintf (cri->cond, sizeof (cri->cond), "IF$TEST(%u,%u)", ref_id >> 6 & 15,
				ref_id >> 1 & 31);
			StringCopyS (cri->info, sizeof (cri->info), "Test condition for LE-CODE developers");
		}
	}
	else if (IsConditionRefEngineId (ref_id))
	{
		...
		if (create_cond)
		{
			...
			*dest = 0;
			...
			StringCopyS (cri->info, sizeof (cri->info), "if selected engines are active");
		}
	}
	else if (IsConditionRefRandomId (ref_id))
	{
		...
		if (create_cond)
		{
			...
			*dest = 0;
			...
			StringCopyS (cri->info, sizeof (cri->info), "if selected random scenario is active");
		}
	}
	else
	{
		...
		if (cri->condref)
		{
			...
			if (create_cond)
			{
				const gobj_cond_ref_t *iref
					= cri->xor ? FindConditionRef (ref_id ^ cri->xor) : cri->condref;
				if (iref)
				{
					if (iref->option & 4)
					{
						snprintf (cri->cond, sizeof (cri->cond), "IFDEV$%s", iref->name);
						snprintf (cri->info, sizeof (cri->info), "DEVELOPERS: %s", iref->info);
					}
					else
					{
						snprintf (cri->cond, sizeof (cri->cond), "IF$%s", iref->name);
						snprintf (cri->info, sizeof (cri->info), "%s%s",
							iref->option & 1 ? "if " : "", iref->info);
					}
				}
			}
		}
...
ccp PrintLinkInfoMode (kmp_linfo_mode mode, int print_group, bool adjust)
{
	...
	{
		if (mode & LINFO_D_PREV)
			*dest++ = 'P';
		if (mode & LINFO_D_NEXT)
			*dest++ = 'N';
		if (mode & LINFO_D_BY_PREV)
			*dest++ = 'p';
		if (mode & LINFO_D_BY_NEXT)
			*dest++ = 'n';

		if (print_group >= 0 && mode & LINFO_M_G)
		{
			*dest++ = ',';
			if (mode & LINFO_G_PREV)
				*dest++ = 'P';
...
static const kmp_func_t kmp_func_list[] = {
	{ "KTPT$N", KMP_KTPT, 0, GTMD_N, 0, 0, "Returns the number of start points.", 0 },
	{ "KTPT$POS", KMP_KTPT, 0x00, GTMD_DBL3, 1, 1, "Returns a start point position.", 0 },
	{ "KTPT$ROT", KMP_KTPT, 0x0c, GTMD_DBL3, 1, 1, "Returns a start point rotation.", 0 },
	{ "KTPT$ID", KMP_KTPT, 0x18, GTMD_U16M1, 1, 1, "Returns a start point ID.", 0 },
	{ "KTPT$UNKNOWN", KMP_KTPT, 0x1a, GTMD_U16, 1, 1, "Returns the unknown value of a start point.",
		0 },

	//---------------

	{ "ENPT$N", KMP_ENPT, 0, GTMD_N, 0, 0, "Returns the number of enemy points.", 0 },
	{ "ENPT$POS", KMP_ENPT, 0x00, GTMD_DBL3, 1, 1, "Returns the position of an enemy point.", 0 },
	{ "ENPT$SCALE", KMP_ENPT, 0x0c, GTMD_DBL, 1, 1, "Returns the scale of an enemy point.", 0 },
	{ "ENPT$PROP", KMP_ENPT, 0x10, GTMD_U16, 2, 2, "Returns property 0 or 1 of an enemy point.",
		0 },

	//---------------

	{ "ENPH$N", KMP_ENPH, 0, GTMD_N, 0, 0, "Returns the number of enemy point groups.", 0 },
	{ "ENPH$START", KMP_ENPH, 0x00, GTMD_U8, 1, 1,
		"Returns the enemy point start index of an enemy point group.", 0 },
	{ "ENPH$LEN", KMP_ENPH, 0x01, GTMD_U8, 1, 1,
		"Returns the number of enemy points of an enemy point group.", 0 },
	{ "ENPH$PREV", KMP_ENPH, 0x02, GTMD_U8M1, KMP_MAX_PH_LINK, 2,
		"Returns link 0..5 to the previous enemy point group.", 0 },
	{ "ENPH$NEXT", KMP_ENPH, 0x08, GTMD_U8M1, KMP_MAX_PH_LINK, 2,
		"Returns link 0..5 to the next enemy point group.", 0 },
	{ "ENPH$UNKNOWN", KMP_ENPH, 0x0e, GTMD_U16, 1, 1,
		"Returns the unknown value of an enemy point group.", 0 },

	//---------------

	{ "ITPT$N", KMP_ITPT, 0, GTMD_N, 0, 0, "Returns the number of item points.", 0 },
	{ "ITPT$POS", KMP_ITPT, 0x00, GTMD_DBL3, 1, 1, "Returns the position of an item point.",
		"Set the position of an item point"
		" and returns 1 on success ans 0 , if failed." },
	{ "ITPT$SCALE", KMP_ITPT, 0x0c, GTMD_DBL, 1, 1, "Returns the scale of an item point.", 0 },
	{ "ITPT$PROP", KMP_ITPT, 0x10, GTMD_U16, 2, 2, "Returns property 0 or 1 of an item point.", 0 },

	//---------------

	{ "ITPH$N", KMP_ITPH, 0, GTMD_N, 0, 0, "Returns the number of item point groups.", 0 },
	{ "ITPH$START", KMP_ITPH, 0x00, GTMD_U8, 1, 1,
		"Returns the item point start index of an item point group.", 0 },
	{ "ITPH$LEN", KMP_ITPH, 0x01, GTMD_U8, 1, 1,
		"Returns the number of item points of an item point group.", 0 },
	{ "ITPH$PREV", KMP_ITPH, 0x02, GTMD_U8M1, KMP_MAX_PH_LINK, 2,
		"Returns link 0..5 to the previous item point group.", 0 },
	{ "ITPH$NEXT", KMP_ITPH, 0x08, GTMD_U8M1, KMP_MAX_PH_LINK, 2,
		"Returns link 0..5 to the next item point group.", 0 },
	{ "ITPH$UNKNOWN", KMP_ITPH, 0x0e, GTMD_U16, 1, 1,
		"Returns the unknown value of an item point group.", 0 },

	//---------------

	{ "CKPT$N", KMP_CKPT, 0, GTMD_N, 0, 0, "Returns the number of check points.", 0 },
	{ "CKPT$LEFT", KMP_CKPT, 0x00, GTMD_DBL2, 1, 1, "Returns the left position of a check point.",
		0 },
	{ "CKPT$RIGHT", KMP_CKPT, 0x08, GTMD_DBL2, 1, 1, "Returns the right position of a check point.",
		0 },
	{ "CKPT$RESPAWN", KMP_CKPT, 0x10, GTMD_U8, 1, 1, "Returns the respawn index of a check point.",
		0 },
	{ "CKPT$MODE", KMP_CKPT, 0x11, GTMD_U8M1, 1, 1, "Returns the mode of check point.", 0 },
	{ "CKPT$PREV", KMP_CKPT, 0x12, GTMD_U8M1, 1, 1,
		"Returns the index of the previous check point.", 0 },
	{ "CKPT$NEXT", KMP_CKPT, 0x13, GTMD_U8M1, 1, 1, "Returns the index of the next check point.",
		0 },

	//---------------

	{ "CKPH$N", KMP_CKPH, 0, GTMD_N, 0, 0, "Returns the number of check point groups.", 0 },
	{ "CKPH$START", KMP_CKPH, 0x00, GTMD_U8, 1, 1,
		"Returns the check point start index of a check point group.", 0 },
	{ "CKPH$LEN", KMP_CKPH, 0x01, GTMD_U8, 1, 1,
		"Returns the number of check points of a check point group.", 0 },
	{ "CKPH$PREV", KMP_CKPH, 0x02, GTMD_U8M1, KMP_MAX_PH_LINK, 2,
		"Returns link 0..5 to the previous check point group.", 0 },
	{ "CKPH$NEXT", KMP_CKPH, 0x08, GTMD_U8M1, KMP_MAX_PH_LINK, 2,
		"Returns link 0..5 to the next check point group.", 0 },
	{ "CKPH$UNKNOWN", KMP_CKPH, 0x0e, GTMD_U16, 1, 1,
		"Returns the unknown value of a check point group.", 0 },

	//---------------

	{ "GOBJ$N", KMP_GOBJ, 0, GTMD_N, 0, 0, "Returns the number of global objects.", 0 },
	{ "GOBJ$ID", KMP_GOBJ, 0x00, GTMD_U16, 1, 1, "Returns the object ID of a global object.", 0 },
	{ "GOBJ$UNKNOWN", KMP_GOBJ, 0x02, GTMD_U16, 1, 1,
		"Returns the unknown value of a global object.", 0 },
	{ "GOBJ$POS", KMP_GOBJ, 0x04, GTMD_DBL3, 1, 1, "Returns the position of a global object.", 0 },
	{ "GOBJ$ROT", KMP_GOBJ, 0x10, GTMD_DBL3, 1, 1, "Returns the rotation of a global object.", 0 },
	{ "GOBJ$SCALE", KMP_GOBJ, 0x1c, GTMD_DBL3, 1, 1, "Returns the scale of a global object.", 0 },
	{ "GOBJ$ROUTE", KMP_GOBJ, 0x28, GTMD_U16M1, 1, 1,
		"Returns -1 or the index of a related route"
		" of a global object.",
		0 },
	{ "GOBJ$SET", KMP_GOBJ, 0x2a, GTMD_U16, 8, 2, "Returns setting 0..7 of a global object.", 0 },
	{ "GOBJ$PFLAGS", KMP_GOBJ, 0x3a, GTMD_U16, 1, 1,
		"Returns the presence flags of a global object.", 0 },

	//---------------

	{ "POTI$N", KMP_POTI, 0, GTMD_N, 0, 0, "Returns the number of routes.", 0 },
	{ "POTI$NN", KMP_POTI, 0, GTMD_HD_U16, 0, 0,
		"Returns the total number of points of all routes.", 0 },
	{ "POTI$NP", KMP_POTI, 0x00, GTMD_U8, 1, 1, "Returns the number of points of a route.", 0 },
	...
	{ "AREA$N", KMP_AREA, 0, GTMD_N, 0, 0, "Returns the number of areas.", 0 },
	{ "AREA$MODE", KMP_AREA, 0x00, GTMD_U8M1, 1, 1, "Returns the mode of an area.", 0 },
	{ "AREA$TYPE", KMP_AREA, 0x01, GTMD_U8M1, 1, 1,
		"Returns the type of an area. Cameras are type 0.", 0 },
	{ "AREA$DEST", KMP_AREA, 0x02, GTMD_U8M1, 1, 1,
		"Returns the destination index (e.g. camera index) of an area.", 0 },
	{ "AREA$UNKNOWN", KMP_AREA, 0x03, GTMD_U8, 1, 1, "Returns the unknown value of an area.", 0 },
	{ "AREA$POS", KMP_AREA, 0x04, GTMD_DBL3, 1, 1, "Returns the position of an area.", 0 },
	{ "AREA$ROT", KMP_AREA, 0x10, GTMD_DBL3, 1, 1, "Returns the rotation of an area.", 0 },
	{ "AREA$SCALE", KMP_AREA, 0x1c, GTMD_DBL3, 1, 1, "Returns the scale of an area.", 0 },
	{ "AREA$SET", KMP_AREA, 0x28, GTMD_U16, 4, 2, "Returns setting 0..3 of an area.", 0 },

	//---------------

	{ "CAME$N", KMP_CAME, 0, GTMD_N, 0, 0, "Returns the number of cameras.", 0 },
	{ "CAME$OCAM", KMP_CAME, 0, GTMD_HD_U8HI, 0, 0, "Returns the index of the opening camera.", 0 },
	{ "CAME$SCAM", KMP_CAME, 0, GTMD_HD_U8LO, 0, 0, "Returns the index of the selection camera.",
		0 },
	{ "CAME$TYPE", KMP_CAME, 0x00, GTMD_U8, 1, 1, "Returns the type of a camera.", 0 },
	{ "CAME$NEXT", KMP_CAME, 0x01, GTMD_U8M1, 1, 1, "Returns -1 or the link to the next camera.",
		0 },
	{ "CAME$UNKNOWN_02", KMP_CAME, 0x02, GTMD_U8, 1, 1,
		"Returns the unknown value at offser 0x02 of a camera.", 0 },
	{ "CAME$ROUTE", KMP_CAME, 0x03, GTMD_U8M1, 1, 1, "Returns -1 or the link to the related route.",
		0 },
	{ "CAME$UNKNOWN_04", KMP_CAME, 0x04, GTMD_U16, 1, 1,
		"Returns the unknown value at offser 0x04 of a camera.", 0 },
	{ "CAME$ZOOM_SPEED", KMP_CAME, 0x06, GTMD_U16, 1, 1,
		"Returns the zoom speed in units per 100/60 sec.", 0 },
	{ "CAME$VIEWPT_SPEED", KMP_CAME, 0x08, GTMD_U16, 1, 1,
		"Returns the view point speed in units per 100/60 sec.", 0 },
	{ "CAME$UNKNOWN_0A", KMP_CAME, 0x0a, GTMD_U16, 1, 1,
		"Returns the unknown value at offser 0x0a of a camera.", 0 },
	{ "CAME$POS", KMP_CAME, 0x0c, GTMD_DBL3, 1, 1, "Returns the position of a camera.", 0 },
	{ "CAME$UNKNOWN_18", KMP_CAME, 0x18, GTMD_DBL3, 1, 1,
		"Returns the unknown value at offser 0x18 of a camera.", 0 },
	{ "CAME$ZOOM_BEGIN", KMP_CAME, 0x24, GTMD_DBL, 1, 1,
		"Returns the zoom beginning angle of a camera.", 0 },
	{ "CAME$ZOOM_END", KMP_CAME, 0x28, GTMD_DBL, 1, 1, "Returns the zoom ending angle of a camera.",
		0 },
	{ "CAME$VIEWPT_BEGIN", KMP_CAME, 0x2c, GTMD_DBL3, 1, 1,
		"Returns the start position of the view point.", 0 },
	{ "CAME$VIEWPT_END", KMP_CAME, 0x38, GTMD_DBL3, 1, 1,
		"Returns the end position of the view point.", 0 },
	{ "CAME$TIME", KMP_CAME, 0x44, GTMD_DBL, 1, 1, "Returns the run time of a camera in 1/60 sec.",
		0 },

	//---------------

	{ "JGPT$N", KMP_JGPT, 0, GTMD_N, 0, 0, "Returns the number of respan points.", 0 },
	{ "JGPT$POS", KMP_JGPT, 0x00, GTMD_DBL3, 1, 1, "Returns the position of a respawn point.", 0 },
	{ "JGPT$ROT", KMP_JGPT, 0x0c, GTMD_DBL3, 1, 1, "Returns the rotation of a respawn point.", 0 },
	{ "JGPT$ID", KMP_JGPT, 0x18, GTMD_U16, 1, 1, "Returns the ID of a respawn point.", 0 },
	{ "JGPT$EFFECT", KMP_JGPT, 0x1a, GTMD_U16, 1, 1, "Returns the effect value of a respawn point.",
		0 },

	//---------------

	{ "CNPT$N", KMP_CNPT, 0, GTMD_N, 0, 0, "Returns the number of canon point points.", 0 },
	{ "CNPT$POS", KMP_CNPT, 0x00, GTMD_DBL3, 1, 1, "Returns the position of a canon point.", 0 },
	{ "CNPT$ROT", KMP_CNPT, 0x0c, GTMD_DBL3, 1, 1, "Returns the rotation of a canon point.", 0 },
	{ "CNPT$ID", KMP_CNPT, 0x18, GTMD_U16, 1, 1, "Returns the ID of a canon point.", 0 },
	{ "CNPT$EFFECT", KMP_CNPT, 0x1a, GTMD_U16, 1, 1, "Returns the efffect value of a canon point.",
		0 },

	//---------------

	{ "MSPT$N", KMP_MSPT, 0, GTMD_N, 0, 0, "Returns the number of battle points.", 0 },
	{ "MSPT$POS", KMP_MSPT, 0x00, GTMD_DBL3, 1, 1, "Returns the position of a battle point.", 0 },
	{ "MSPT$ROT", KMP_MSPT, 0x0c, GTMD_DBL3, 1, 1, "Returns the rotation of a battle point.", 0 },
	{ "MSPT$ID", KMP_MSPT, 0x18, GTMD_U16, 1, 1, "Returns the ID of a battle point.", 0 },
	{ "MSPT$EFFECT", KMP_MSPT, 0x1a, GTMD_U16, 1, 1, "Returns the efffect value of a battle point.",
		0 },

	//---------------

	{ "STGI$N", KMP_STGI, 0, GTMD_N, 0, 0, "Returns the number of stage infos.", 0 },
	{ "STGI$BYTE", KMP_STGI, 0x00, GTMD_U8, 12, 2, "Returns setting byte 0..11 of a stage info.",
		0 },
};
...
static enumError F_get (struct Var_t *res, // store result here
...
)
{
	...
	if (si && si->kmp)
	{
		...
		if (fp->sect == KMP_POTI && fp->off >= POTI_PT_OFF && index1 < max_index1)
		{
			...
			data = dlist->list + dlist->elem_size * (pt_start + index1) + fp->off - POTI_PT_OFF;
		}
		...
		switch (fp->get_mode)
		{
			...
			case GTMD_U16M1:
				if (valid_index12)
				{
					...
					res->i = val == 0xffff && fp->get_mode == GTMD_U16M1 ? -1 : (int)val;
					res->mode = VAR_INT;
					return ERR_OK;
				}
				break;
...
static enumError F_set (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static void InitializeGL (kmp_group_list_t *gl, // list to initialize
	int selector,
...
static uint ScanGroupNames (kmp_group_list_t *gl, // group list
...
)
{
	...
	{
		const kmp_oneway_mode oneway = NextCharSI (si, false) == '>' ? ONEWAY_BOTH : ONEWAY_NONE;
...
static enumError GroupPrevGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si // valid scan info
)
{
	DASSERT (gl);
...
static enumError DefaultClassGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si // valid scan info
)
{
	DASSERT (gl);
...
static enumError ClassGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si, // valid scan info
...
)
{
	...
	if (stat >= 0)
	{
		go2->prev.rclass = go2->prev.lclass = go2->next.rclass = go2->next.lclass = stat;
...
static enumError ACClassGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si, // valid scan info
	kmp_gopt2_t *go2, // options to set
...
static enumError GroupSettingsGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si, // valid scan info
	kmp_gopt2_t *go2 // options to set
)
{
	...
	noPRINT ("\e[36;1m rtype %d, g-settings: %u 0x%02x [%d]\e[0m\n", go2->rtype_user,
		go2->setting[0], go2->setting[1], err);
	if (err)
	{
		GotoEolSI (si);
		return err;
...
static enumError GroupOnewayGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si, // valid scan info
	kmp_gopt2_t *go2 // options to set
)
{
...
static enumError AddGroupGL (kmp_group_list_t *gl, // group list
	ScanInfo_t *si, // valid scan info
...
)
{
	...
	if (split_group)
	{
		...
		if (!gl->used || index == gl->group[gl->used - 1].first_index)
		{
			...
			CheckEolSI (si);
			if (!si->no_warn)
				ERROR0 (ERR_WARNING, "Command '$SPLIT' ignored [%s @%u].\n", sf->name, sf->line);
			return ERR_WARNING;
		}
	}
	else if (strcmp (name, "$GROUP"))
	{
		...
		sf->line_err++;
		CheckEolSI (si);
		return ERROR0 (ERR_WARNING, "Line ignored [%s @%u]: %s ...\n", sf->name, sf->line, name);
	}
	...
	if (gl->used >= KMP_MAX_GROUP)
	{
		...
		CheckEolSI (si);
		return ERROR0 (
			ERR_WARNING, "To many groups, new group ignored: %s @%u\n", sf->name, sf->line);
	}
	...
	if (gl->ph)
	{
		...
		go2->prev.rclass = go2->prev.lclass = go2->next.rclass = go2->next.lclass
			= gl->ph->default_class;
	}
...
static uint AddPointGL (kmp_group_list_t *gl, // group list
	uint index // index of first point
)
{
	if (!gl->used)
...
static int SearchNearestDP (kmp_group_list_t *gl, // group list
...
)
{
	...
	for (i = 0, grp = gl->group; i < gl->used; i++, grp++)
	{
		...
		noPRINT ("--- CANDIDATE %2u: rtype=%d>%d, class=%d,%d|%d,%d\n", i, go2->rtype_user,
			go2->rtype_active, go2->prev.rclass, go2->prev.lclass, go2->next.rclass,
			go2->next.lclass);

		const kmp_enpt_entry_t *pt
			= (kmp_enpt_entry_t *)kmp->dlist[ph->sect_pt].list + ph_list[i].pt_start;
...
static void TermGL (kmp_group_list_t *gl, // group list
...
// [[auto-prev]]
		...
		const bool auto_prev
			= gl->ph->ac_mode == KMP_AC_PREV || gl->ph->ac_mode == KMP_AC_AUTO_PREV;
#endif

		uint l;
		for (l = 0; l < KMP_MAX_PH_LINK; l++)
		{
			const uint l2 = grp->no_rotate ? l : (l + rotate_links) % KMP_MAX_PH_LINK;
			...
			if (!found && !si->no_warn)
				ERROR0 (ERR_WARNING, "%s group %s: $GROUP name not found: %s",
		...
		if (!auto_prev)
		{
			for (l = 0; l < KMP_MAX_PH_LINK; l++)
			{
				...
				if (!found && !si->no_warn)
					ERROR0 (ERR_WARNING, "%s group %s: $PREV name not found: %s",
						kmp_section_name[gl->ph->sect_pt].name1, grp->name, lname);
			}
		}
	}
	...
	if (ph && ph->ac_mode == KMP_AC_DISPATCH && (sect_ph == KMP_ENPH || sect_ph == KMP_ITPH))
	{
		...
		for (i = 0, grp = gl->group; i < gl->used; i++, grp++)
		{
			...
			PRINT (">>> STDROUTE %2u: %d %s, rtype=%d>%d, oneway=%d, class=%d,%d|%d,%d\n", i,
				li->gmode[i] & LINFOG_DISPATCH, PrintLinkInfoMode (li->summary[i], 1, true),
				go2->rtype_user, go2->rtype_active, go2->ac_oneway, go2->prev.rclass,
				go2->prev.lclass, go2->next.rclass, go2->next.lclass);
			...
			if (!(mode & LINFO_G_PREV) && go2->prev.lclass != KCLS_OFF)
			{
				...
				const kmp_enpt_entry_t *pt
					= (kmp_enpt_entry_t *)kmp->dlist[li->sect_pt].list + entry->pt_start;
			...
			if (!(mode & LINFO_G_NEXT) && go2->next.lclass != KCLS_OFF)
			{
				...
				const kmp_enpt_entry_t *pt = (kmp_enpt_entry_t *)kmp->dlist[li->sect_pt].list
...
static int get_next (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
...
)
{
	...
	if (si && si->kmp)
	{
		...
		if (idx >= 0 && idx < ptlist->used)
		{
			...
			noPRINT (">> %u %zu[%u+%u]\n", idx, ph - (kmp_enph_entry_t *)phlist->list, ph->pt_start,
				ph->pt_len);
...
static enumError F_get_next (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_get_hdir (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (next >= 0)
	{
		...
		res->d = atan2 (pt[next].position[0] - pt[current].position[0],
					 pt[next].position[2] - pt[current].position[2])
			* (180 / M_PI);
		res->mode = VAR_DOUBLE;
	}
	return ERR_OK;
...
static enumError F_isKMP (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si && si->kmp)
	{
		...
		if (si->kmp->fname)
		{
			...
			if (!strcasecmp (fname, "course.kmp") || !strcasecmp (fname, "course.txt")
...
static const struct FuncTable_t kmp_func_tab[]
	= { { 1, 2, "ENPT$NEXT", F_get_next, KMP_ENPT, "int", "enpt$next(enpt_index[,next_index])",
			"Returns the index of the next enemy point."
			" The 'next_index' is only used, if multiple links are available." },

		  { 1, 2, "ITPT$NEXT", F_get_next, KMP_ITPT, "int", "itpt$next(itpt_index[,next_index])",
			  "Returns the index of the next item point."
			  " The 'next_index' is only used, if multiple links are available." },

		  { 1, 2, "ENPT$HDIR", F_get_hdir, KMP_ENPT, "float", "enpt$hDir(enpt_index[,next_index])",
			  "Calculate the horizontal direction in degree of an enemy point."
			  " The 'next_index' is only used, if multiple links are available." },

		  { 1, 2, "ITPT$HDIR", F_get_hdir, KMP_ITPT, "float", "itpt$hDir(itpt_index[,next_index])",
	...
	if (!vm.used)
	{
		...
		for (i = 0; i < KMP_FUNC_LIST_N; i++, fp++)
		{
			...
			if (fp->get_info)
			{
				...
				ccp retval = fp->get_mode == GTMD_DBL ? "float"
					: fp->get_mode > GTMD_DBL		  ? "vector"
													  : "int";
				char *dest = syntax;
				ccp src = fp->name;
				while (*src)
				...
				*dest++ = '(';
				if (fp->n_param > 0)
				{
					...
					switch (fp->n_param)
					{
						...
						case 2:
							src = fp->sect == KMP_POTI && fp->off >= POTI_PT_OFF ? "_idx,pt_idx"
																				 : "_idx,val_idx";
							break;
		...
		static const struct inttab_t inttab[] = { { "MODE$SLOT", 0 }, { "MODE$D", 0 },

			{ "MODE$TEST", 0 }, { "MODE$OCAM", 0 }, { "MODE$LAPS", 3 }, { "MODE$SHOWRT", 0 },
			{ "MODE$SHOWCK", 0 }, { "MODE$AUTOMODE", 0 },

			{ "AC$OFF", KMP_AC_OFF }, { "AC$PREV", KMP_AC_PREV },
			{ "AC$AUTO_PREV", KMP_AC_AUTO_PREV }, { "AC$DISPATCH", KMP_AC_DISPATCH },
			{ "ACF$FIX_PREV", KMP_ACF_FIX_PREV }, { "ACF$FIX_NEXT", KMP_ACF_FIX_NEXT },
			{ "ACF$FIX", KMP_ACF_FIX }, { "ACF$PR_PREV", KMP_ACF_PR_PREV },

			{ "AM$OFF", KMP_AM_OFF }, { "AM$NORM", KMP_AM_NORM },
			{ "AM$RENUMBER", KMP_AM_RENUMBER }, { "AM$1LAP", KMP_AM_1_LAP },
			{ "AM$SHORT", KMP_AM_SHORT }, { "AM$UNLIMIT", KMP_AM_UNLIMIT },

			{ "SORT$OFF", KSORT_OFF }, { "SORT$GROUPING", KSORT_GROUP },
			{ "SORT$ANGLE", KSORT_ANGLE }, { "SORT$XYZ", KSORT_XYZ }, { "SORT$TINY", KSORT_TINY },

			{ "DOB$OFF", KMP_DOB_OFF }, { "DOB$BEGIN", KMP_DOB_BEGIN }, { "DOB$END", KMP_DOB_END },
			{ "DOB$BEFORE", KMP_DOB_BEFORE }, { "DOB$BEHIND", KMP_DOB_BEHIND },

			{ "EN$BATTLE", ENGM_BATTLE }, { "EN$50", ENGM_50 }, { "EN$100", ENGM_100 },
			{ "EN$150", ENGM_150 }, { "EN$200", ENGM_200 }, { "EN$150M", ENGM_150M },
			{ "EN$200M", ENGM_200M }, { "EN$150X", ENGM_150M | ENGM_150 },
			{ "EN$200X", ENGM_200M | ENGM_200 }, { "EN$MIRROR", ENGM_150M | ENGM_200M },
			{ "EN$ALL", ENGM__ALL },

			{ "F$FALL", KPFL_FALL }, { "F$SNAP", KPFL_SNAP }, { "F$JGPT", KPFL_JGPT },

			{ "ITEM$RANDOM", 0x00 }, { "ITEM$BANANA", 0x01 }, { "ITEM$MUSHROOM", 0x02 },
			{ "ITEM$3MUSHROOMS", 0x03 }, { "ITEM$STAR", 0x04 }, { "ITEM$3GREEN", 0x05 },
			{ "ITEM$BANANA_MUSHROOM", 0x06 }, { "ITEM$GREEN", 0x07 }, { "ITEM$RED", 0x08 },
			{ "ITEM$MEGA", 0x0a }, { "ITEM$CLOUD", 0x0b }, { "ITEM$STAR_MUSHROOM", 0x0c },
			{ "ITEM$MUSHROOM_GREEN", 0x0d }, { "ITEM$3BANANAS_GREEN", 0x24 },

			{ "PF$OFFLINE1", 0x01 }, { "PF$OFFLINE2", 0x02 }, { "PF$OFFLINE3", 0x04 },
			{ "PF$OFFLINE", 0x07 }, { "PF$ONLINE1", 0x08 }, { "PF$ONLINE2", 0x10 },
			{ "PF$ONLINE3", 0x20 }, { "PF$ONLINE", 0x38 }, { "PF$ALL", 0x3f },
...
#define KCL_FALL_VARS(k, v)                                                                        \
	{ "KCL-FALL-ADD-ROAD", SPM_BOOL, &k->enable_fall_kcl },                                        \
		{ "KCL-FALL-WIDTH", SPM_INT, &v.width }, { "KCL-FALL-PRE-ADD", SPM_VAR, &v.pre_add },      \
		{ "KCL-FALL-POST-ADD", SPM_VAR, &v.post_add }, { "KCL-FALL-TYPES", SPM_U32, &v.types },
...
#define ROUTE_OBJECT_VARS(v)                                                                       \
	{ "ROUTE-OBJECT", SPM_INT, &v.route_object },                                                  \
		...
		{ "SHOW-ROUTE", SPM_INT, &v.show_route }, { "ROUTE-ITEMBOX", SPM_INT, &v.route_itembox },
...
bool CheckRouteInfo (rtobj_info_t *ri)
{
	DASSERT (ri);
	DASSERT (ri->def_obj);
	noPRINT_IF (ri->route_object != N_KMP_GOBJ, ">> CheckRouteInfo() obj=%d, shrt=%d\n",
...
#define TFORM_VARS(v)                                                                              \
	{ "TFORM-RESET", SPM_INC, &v.reset }, { "TFORM-SCALE", SPM_VAR, &v.scale },                    \
		{ "TFORM-SHIFT", SPM_VAR, &v.shift }, { "TFORM-ROTATE", SPM_VAR, &v.rotate },              \
...
static enumError ScanText_PT_PH (kmp_t *kmp, // KMP data structure
...
)
{
	...
	ScanParam_t ptab[] = { { pt_hd_name, SPM_U16, kmp->value + sect_pt },
		{ ph_hd_name, SPM_U16, kmp->value + sect_ph },
		{ "AUTO-GROUP-NAME", SPM_INT, &auto_group_name },
		{ "ROTATE-GROUP-LINKS", SPM_INT, &rotate_group_links },
		{ "EXPORT-FLAGS", SPM_INT, &export_flags }, { "AUTO-FILL", SPM_DOUBLE, &auto_fill },
		{ "SHOW-OPTIONS", SPM_INT, &show_options }, { "AUTO-CONNECT", SPM_U8, &auto_connect },
		{ "MASK-1", SPM_U16, &mask1 }, { "MASK-2", SPM_U16, &mask2 },
		ROUTE_OBJECT_VARS (rtinfo) TFORM_VARS (tform_pos) KCL_FALL_VARS (kmp, kfall) { 0 } };
	...
	for (;;)
	{
		...
		if (!ptlist->used) // no previous point available
			gl.line.n = 1;
		else if (auto_fill >= 1.0 && gl.line.n == 1 && gl.used
...
static enumError ScanTextKTPT (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "KTPT-HEAD-VALUE", SPM_U16, kmp->value + KMP_KTPT },
		TFORM_VARS (tform_pos) KCL_FALL_VARS (kmp, kfall) { 0 } };
...
static enumError ScanTextCKPT (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "AUTO-MODE", SPM_INT, &auto_mode },
		{ "AUTO-NEXT", SPM_INT, &auto_next }, { "AUTO-GROUP-NAME", SPM_INT, &auto_group_name },
		...
		{ "CKPH-HEAD-VALUE", SPM_U16, kmp->value + KMP_CKPH }, { 0 } };
			...
			else
			{
				...
				if (gl.line.line_mode == LM_BEZIER && !gl.line.n_pt)
				{
					...
					PRINT ("\n-> %11.3f %11.3f\n"
						   ...
						   "   %11.3f %11.3f\n"
						   "   %11.3f %11.3f\n",
						p1[0], p1[1], gl.line.pt[0][0], gl.line.pt[0][1], gl.line.pt[1][0],
						gl.line.pt[1][1], p2[0], p2[1]);
				}
			}
		}
		...
		for (p = 1; p <= gl.line.n; p++)
		{
			...
			switch (auto_mode)
			{
				...
				case KMP_AM_UNLIMIT: // remove lap counter for unlimited race
				{
					...
					entry->mode
						= ptlist->used < sizeof (tab) / sizeof (*tab) ? tab[ptlist->used] : -1;
		...
		else
		{
			...
			for (p = 1; p < gl.line.n; p++)
			{
				...
				noPRINT ("%2u: %6.0f %6.0f %6.0f | %6.0f %6.0f %6.0f"
					...
					p, f[0][0], f[0][1], f[0][3], f[1][0], f[1][1], f[1][3], f[2][0], f[2][1],
					f[2][3], angle1 * 180.0 / M_PI, angle2 * 180.0 / M_PI);
...
static void disable_enable (ScanInfo_t *si, List_t *dlist, bool enable)
{
	...
	if (!ScanExprSI (si, &var))
	{
		...
		else if (si->no_warn <= 0)
		{
			...
			ScanFile_t *sf = si->cur_file;
			ERROR0 (ERR_WARNING, "Invalid ID %#x for predefinied condition [%s @%u]:\n%.*s\n",
...
static void disable_enable_by (ScanInfo_t *si, List_t *dlist, bool enable, bool not)
{
	DEFINE_VAR (var);
	if (!ScanExprSI (si, &var))
	{
		...
		else if (si->no_warn <= 0)
		{
			...
			ERROR0 (ERR_WARNING,
				...
				sf->name, sf->line, (int)(eol - sf->prev_ptr), sf->prev_ptr);
		}
	}
	FreeV (&var);
	CheckEolSI (si);
...
static enumError ScanTextGOBJ (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "GOBJ-HEAD-VALUE", SPM_U16, kmp->value + KMP_GOBJ },
		{ "AUTO-ENEMY-ITEM", SPM_UINT, &auto_enemy_item },
		{ "AUTO-OBJECT-NAME", SPM_INT, &auto_object_name },
		{ "SORT-OBJECTS", SPM_INT, &sort_objects },

		{ "FIX-OBJDEF", SPM_INT, &fix_objdef }, { "SORT-OBJDEF", SPM_INT, &sort_objdef },
	...
	bool ignore_itembox = kmp->rtobj_enpt.obj == GOBJ_ITEMBOX || kmp->rtobj_itpt.obj == GOBJ_ITEMBOX
		|| kmp->rtobj_jgpt.obj == GOBJ_ITEMBOX || kmp->rtobj_cnpt.obj == GOBJ_ITEMBOX
	...
	for (;;)
	{
		...
		if (ignore_itembox && entry->obj_id < N_KMP_GOBJ
			&& ObjectInfo[entry->obj_id].flags & OBF_ITEMBOX)
		{
			dlist->used--;
		}
		else if (auto_enemy_item && entry->obj_id == GOBJ_ITEMBOX && !entry->setting[2])
		{
			for (;;)
			{
...
static enumError ScanTextAREA (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "AREA-HEAD-VALUE", SPM_U16, kmp->value + KMP_AREA }, { 0 } };
...
static enumError ScanTextCAME (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "CAME-HEAD-VALUE", SPM_U16, (u16 *)value },
#ifdef LITTLE_ENDIAN
		{ "OPENING-INDEX", SPM_U8, value + 1 }, { "SELECTION-INDEX", SPM_U8, value + 0 },
#else
		{ "OPENING-INDEX", SPM_U8, value + 0 }, { "SELECTION-INDEX", SPM_U8, value + 1 },
#endif
		{ 0 } };
...
static enumError ScanTextPT2 (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si, // valid data
...
)
{
	...
	int auto_id = 0;
	char hd_name[20];
	snprintf (hd_name, sizeof (hd_name), "%s-HEAD-VALUE", kmp_section_name[sect].name1);
	ScanParam_t ptab[] = { { hd_name, SPM_U16, kmp->value + sect },
		{ "AUTO-ID", SPM_INT, &auto_id }, { "SNAP2-TO-ENPT", SPM_VAR, snap2_to_enpt },
		{ "SNAP-TO-ENPT", SPM_VAR, snap_to_enpt }, { "HSNAP-TO-ENPT", SPM_VAR, hsnap_to_enpt },
		{ "AUTO-DEFINE", SPM_INT, auto_define }, { "AUTO-DISTANCE", SPM_FLOAT, auto_distance },
		{ "AUTO-ADJUST", SPM_VAR, auto_adjust }, { "AUTO-REMOVE", SPM_INT, auto_remove },
	...
	for (;;)
	{
		...
		const u8 flags = !kf ? KPFL_M_ALL : NextCharSI (si, false) ? ScanPtFlags (si) : kf->mask;
	...
	PRINT_IF (snap2_to_enpt && snap2_to_enpt->mode,
		...
		kmp->jgpt_snap2.mode, kmp->cnpt_snap2.mode, kmp->mspt_snap2.mode);
	PRINT_IF (snap_to_enpt && snap_to_enpt->mode, "SNAP-TO-ENPT: %d -> jgpt=%d, cnpt=%d, mspt=%d\n",
		snap_to_enpt->mode, kmp->jgpt_snap.mode, kmp->cnpt_snap.mode, kmp->mspt_snap.mode);
...
static enumError ScanTextSTGI (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "STGI-HEAD-VALUE", SPM_U16, kmp->value + KMP_STGI }, { 0 } };
...
static enumError ScanTextSETUP (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { ROUTE_OBJECT_VARS (rtinfo) { 0 } };
	...
	for (;;)
	{
		...
		char name[50];
		if (!ScanNameSI (si, name, sizeof (name), true, true, 0) || NextCharSI (si, true) != '=')
		{
			CheckEolSI (si);
			continue;
		}
...
static void CloseRoute (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si, // valid data
	const kmp_poti_group_t *grp, // group to close
...
)
{
	...
	kmp_poti_point_t *pp0 = (kmp_poti_point_t *)kmp->poti_point.list + kmp->poti_point.used - np;
	...
	if (ri->f_hrotate)
	{
		...
		PRINT (
			"HROTATE[%d]: %11.3f %11.3f %11.3f -> %6.3f\n", ri->f_hrotate, d->x, d->y, d->z, rot);
	...
	if (ri->route_time > 0.0 && total_tim > 0.0)
	{
		PRINT ("SET ROUTE TIME %s, N=%d+%u, time=%4.2f, len=%5.3f, set=%4.2f\n", last_index_name,
			kmp->poti_point.used - np, np, total_tim, total_len, ri->route_time);
		...
		for (i = 0; i < np - 1; i++, pp++)
		{
			if (pp->speed > 0)
			{
				...
				noPRINT ("%6.2f/%4.2f - %6.2f/%4.2f - %6.2f -> %5u\n", sum_src / 60.0, total_tim,
					sum_dest / 60.0, total_tim * factor, wanted, pp->speed);
			}
		}
	...
	if (AssignRouteInfo (&ri->rtinfo, &rtobj))
	{
		...
		for (i = 0; i <= kmp->n_rtobj_poti; i++)
		{
			if (i >= max)
			{
				ERROR0 (ERR_WARNING, "Route list full -> @ROUTE-OBJECT ignored for route #%u",
					ri->index);
				break;
			}
...
static enumError ScanTextPOTI (kmp_t *kmp, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (kmp);
	...
	ScanParam_t ptab[] = { { "AUTO-ROUTE-NAME", SPM_INT, &auto_route_name },
		...
		ROUTE_OBJECT_VARS (ri.rtinfo) KCL_FALL_VARS (kmp, ri.kfall) { 0 } };
		...
		if (ch == '$')
		{
			...
			if (!strcmp (name, "$COPY"))
			{
				...
				if (r_idx >= glist->used)
				{
					...
					si->total_err++;
					sf->line_err++;
					ERROR0 (ERR_WARNING, "Can't copy unknown route #%u [%s @%u]\n", r_idx, sf->name,
						sf->line);
					GotoEolSI (si);
				}
			...
			if (!cmd_close && strcmp (name, "$ROUTE") && strcmp (name, "$GROUP"))
			{
				...
				sf->line_err++;
				CheckEolSI (si);
				ERROR0 (ERR_WARNING, "Line ignored [%s @%u]: %s ...\n", sf->name, sf->line, name);
				continue;
			}
...
static kmp_gobj_entry_t *AddObject (kmp_t *kmp, // KMP data structure
...
static void AddEnemyRouteObjects (kmp_t *kmp, // KMP data structure
	uint sect, // KMP section index
	kmp_rtobj_t *rtobj // object to insert
)
{
...
static void AddJugemRouteObjects (kmp_t *kmp, // KMP data structure
	uint sect, // KMP section index
	kmp_rtobj_t *rtobj // object to insert
)
{
...
static void AddRouteObjects (kmp_t *kmp // KMP data structure
)
{
	DASSERT (kmp);
	...
	for (ilist = 0; ilist < kmp->n_rtobj_poti; ilist++, rptr++)
	{
		...
		{
			float3 rot = CalcDirection3F (pt[i].position, pt[(i + 1) % grp->n_point].position);
...
static void AddCheckpointObject (kmp_t *kmp // KMP data structure
)
{
	DASSERT (kmp);
	...
	obj1.object = kmp->ckpt_object1 > 0 && kmp->ckpt_object1 < N_KMP_GOBJ ? kmp->ckpt_object1 : 0;
	setup_scale (&obj1.scale, &kmp->ckpt_object1_scale);
	obj1.base = kmp->ckpt_object1_base;

	obj2.object = kmp->ckpt_object2 > 0 && kmp->ckpt_object2 < N_KMP_GOBJ ? kmp->ckpt_object2 : 0;
...
static void SnapToENPT (kmp_t *kmp, // KMP data structure
	uint sect // KMP section index
)
{
	...
	for (j = 0; pt < pt_end; j++, pt++)
	{
		...
		else
		{
			...
			pt->rotation[1] = atan2 (next->position[0] - enpt->position[0],
								  next->position[2] - enpt->position[2])
				* (180 / M_PI);
		}
	}
}
...
static int FindNearestJGPT2 (kmp_t *kmp, // valid KMP
...
static int FindNearestJGPT3 (kmp_t *kmp, // valid KMP
...
static void AutoRespawn (kmp_t *kmp)
{
	...
	const uint n_ckpt
		= kmp->dlist[KMP_CKPT].used < KMP_MAX_POINT ? kmp->dlist[KMP_CKPT].used : KMP_MAX_POINT;
...
static void DefineReferenceKCLbyKMP (kmp_t *kmp)
{
	...
	if (kmp && kmp->enable_fall_kcl)
	{
		...
		for (; n > 0; n--, op++)
		{
			if (op->obj_id < N_KMP_GOBJ && ObjectInfo[op->obj_id].flags & OBF_ROAD
...
enumError ScanTextKMP (kmp_t *kmp, // KMP data structure
...
static void init_print_kmp_group (print_kmp_group_t *grp, // group element, will be initialized
...
static ccp print_kmp_group (File_t *F, // output file
	const kmp_t *kmp, // KMP data structure
...
)
{
	...
	if (grp->index < grp->max && index == grp->ph->pt_start)
	{
		...
		bool print_prev = ph->ac_mode != KMP_AC_PREV && ph->ac_mode != KMP_AC_AUTO_PREV
			|| ph->ac_flags & KMP_ACF_PR_PREV || KMP_MODE & KMPMD_PR_PREV;
		...
		if (idx < KMP_MAX_GROUP && grp->ph)
		{
			...
			if (ph->show_options || go2->setting[0] || go2->setting[1] || go2->rtype_user
				|| go2->rtype_active == KMP_RT_DISPATCH)
			{
				const KeywordTab_t *key = GetKewordById (kmp_rtype_name, go2->rtype_user);
				fprintf (F->f, "\r\n $SETTINGS: %u 0x%02x %s", go2->setting[0], go2->setting[1],
					key ? key->name1 : "AUTO");
...
static void print_kmp_pt (File_t *F, // output file
	const kmp_t *kmp, // KMP data structure
...
)
{
	...
	fprintf (F->f, text_kmp_pt_info_cr, REVISION_NUM, pt_name,
		kmp->value[sect_pt], // @%s-HEAD-VALUE = %#x
		ph_name, kmp->value[sect_ph], // @%s-HEAD-VALUE = %#x
		show_route, opt_export_flags ? opt_export_flags > 0 : kf->export);
	...
	if (opt_route_options > OFFON_OFF)
	{
		...
		{
			case KMP_ACF_FIX_PREV:
				ac_fix = " | ACF$FIX_PREV";
				break;
			case KMP_ACF_FIX_NEXT:
				ac_fix = " | ACF$FIX_NEXT";
				break;
			case KMP_ACF_FIX:
				ac_fix = " | ACF$FIX";
		...
		switch (ph->ac_mode)
		{
			...
			case KMP_AC_PREV:
				ac_mode = "AC$PREV";
				break;
			case KMP_AC_AUTO_PREV:
				ac_mode = "AC$AUTO_PREV";
				break;
			case KMP_AC_DISPATCH:
				ac_mode = "AC$DISPATCH";
				break;
	...
	for (i = 0; i < n; i++, gi++, pt++)
	{
		...
		{
			fprintf (
				F->f, " %7.2f", AngleVector (pt[-2].position, pt[-1].position, pt->position, true));
		}

		fprintf (F->f, "%s%4u  %11.3f %11.3f %11.3f  %7.3f  %#6x %#6x", eol, i, pt->position[0],
			pt->position[1], pt->position[2], pt->scale, pt->prop[0], pt->prop[1]);
		eol = "\r\n";
...
static void print_kmp_ph (File_t *F, // output file
	const kmp_t *kmp, // KMP data structure
...
)
{
	...
	fprintf (F->f, text_kmp_ph_cr, pt_name, REVISION_NUM, ph_name, kmp->value[sect_ph], ph_name);
	...
	for (i = 0; i < n; i++, p++)
	{
		fprintf (F->f, "%4u  %3u %3u %3u ", i, p->pt_start, p->pt_start + p->pt_len - 1, p->pt_len);
		...
		if (i < KMP_MAX_GROUP)
		{
			...
			fprintf (F->f, " %3u 0x%02x  # %-6s %s\r\n", go2->setting[0], go2->setting[1],
				ph->gname[i], i < kmp->index[sect_ph].used ? kmp->index[sect_ph].list[i] : "-");
#else
...
static void print_kmp_pt2 (File_t *F, // output file
	const kmp_t *kmp, // KMP data structure
	uint sect, // KMP section index
...
)
{
	...
	uint show_route = sect == KMP_JGPT ? (1 << SHRT_JGPT)
		: sect == KMP_CNPT			   ? (1 << SHRT_CNPT)
									   : (1 << SHRT_MSPT);
	const u8 *pt_flags = sect == KMP_JGPT ? kmp->jgpt_flag.flags : 0;

	fprintf (F->f, text_kmp_pt2_cr, REVISION_NUM, pt_name, kmp->value[sect], auto_id, show_route);
	...
	for (i = 0; i < n; i++, pt++)
	{
		...
		fprintf (F->f, " %11.3f %11.3f %11.3f %7.2f %7.2f %7.2f %6d %6s", pt->position[0],
			pt->position[1], pt->position[2], pt->rotation[0], pt->rotation[1], pt->rotation[2],
			pt->id, PrintU16M1 (pt->effect));

		if (pt_flags)
		{
			const kmp_pt_flag_t flags = i < KMP_MAX_POINT ? pt_flags[i] : kmp->jgpt_flag.mask;
...
enumError SaveTextKMP (const kmp_t *kmp, // pointer to valid KMP
...
)
{
	...
	if (brief_count || export_count)
	{
		fprintf (F.f,
			...
			"KMP-WIM0          = %d\r\n"
			"\r\n",
			section_sep, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE,
			kmp->info ? kmp->info : "", kmp->kmp_version, kmp->battle_mode, kmp->wim0_export);
	}
	else
	{
		fprintf (F.f, text_kmp_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE,
			kmp->info ? kmp->info : "", kmp->kmp_version, kmp->battle_mode, kmp->wim0_export);
		...
		for (;;)
		{
			...
			for (sect = 0; sect < KMP_N_SECT; sect++)
			{
				...
				if (strcmp (name, last_name) > 0 && (!found_name || strcmp (name, found_name) < 0))
				{
					found_name = name;
					found_sect = sect;
				}
			...
			if (found_sect == KMP_POTI)
				fprintf (F.f, " # N-%s = %5u,%5u\r\n", found_name, kmp->dlist[found_sect].used,
					kmp->poti_point.used);
			else
				fprintf (F.f, " # N-%s = %5u\r\n", found_name, kmp->dlist[found_sect].used);
	...
	fprintf (F.f, text_kmp_defaults_cr, GetObjectName (kmp->rtobj.obj), kmp->rtobj.shift.x,
		kmp->rtobj.shift.y, kmp->rtobj.shift.z, kmp->rtobj.scale.x, kmp->rtobj.scale.y,
		kmp->rtobj.scale.z);
	...
	for (i = 0; i < n; i++, area++)
	{
		*buf1 = 0;
		...
		fprintf (F.f, "%#4x %#4x  %12.3f %11.3f %11.3f  %#6x %#6x\r\n >", area->mode, area->type,
			area->position[0], area->position[1], area->position[2], area->setting[0],
			area->setting[1]);
		...
		fprintf (F.f,
			...
			" > %28.3f %11.3f %11.3f\r\n"
			"#%.78s\r\n",
			area->prio, area->rotation[0], area->rotation[1], area->rotation[2],
			PrintU8M1 (area->route), PrintU8M1 (area->enemy), area->unknown_2e, area->scale[0],
			area->scale[1], area->scale[2], Minus300);
	}
	...
	fprintf (F.f, text_kmp_came_cr, REVISION_NUM, GetNameIL (ill + KMP_CAME, ocam),
	...
	for (i = 0; i < n; i++, came++)
	{
		...
		for (count = idx = 0; idx < nn; idx++, area++)
			if (area->type == 0x00 && area->dest_id == i)
				fprintf (F.f, "%s %s",
					count++ ? "" : "\t# AREA reference:", GetNameIL (ill + KMP_AREA, idx));
		if (count)
			fputs ("\r\n", F.f);
		...
		count = 0;
		if (i == ocam)
			fprintf (F.f, "%s @OPENING-INDEX", count++ ? "" : "\t# CAME reference:");
		if (i == scam)
			fprintf (F.f, "%s @SELECTION-INDEX", count++ ? "" : "\t# CAME reference:");
		...
		for (idx = 0; idx < nn; idx++, cref++)
			if (cref->next == i)
				fprintf (F.f, "%s %s",
					count++ ? "" : "\t# CAME reference:", GetNameIL (ill + KMP_CAME, idx));
		if (count)
			fputs ("\r\n", F.f);
		...
		fprintf (F.f, " %3u %7u  %11.3f %11.3f %11.3f %10.3f", came->type, came->came_speed,
			came->position[0], came->position[1], came->position[2], came->zoom_begin);
		...
		if (buf1[i])
		{
			...
			ccp term = "\r\n >            ";
			if (idx == ocount)
				fprintf (F.f, " ( mode$ocam == %u ? %s : mode$test ? %s : %s )%s", idx,
					GetNameIL (ill + KMP_CAME, i),
					GetNameIL (ill + KMP_CAME, kmp->value[KMP_CAME] >> 8),
					GetNameIL (ill + KMP_CAME, came->next), term);
			else
				fprintf (F.f, " ( mode$ocam == %u ? %s : %s )%s", idx,
					GetNameIL (ill + KMP_CAME, i), GetNameIL (ill + KMP_CAME, came->next), term);
		}
		else
			PrintNameIL (F.f, ill + KMP_CAME, came->next, 10, 12, 2);

		fprintf (F.f, " %5u  %11.3f %11.3f %11.3f %10.3f", came->zoom_speed, came->rotation[0],
			came->rotation[1], came->rotation[2], came->zoom_end);
		const double zoom_length = fabs (came->zoom_begin - came->zoom_end);
		if (zoom_length >= 0.01 && came->zoom_speed)
			fprintf (F.f, "  # zoom time:%8.2fs", zoom_length / came->zoom_speed * (10.0 / 6.0));

		fprintf (F.f, "\r\n > %#9x %7u  %11.3f %11.3f %11.3f", came->unknown_0x02,
			came->viewpt_speed, came->viewpt_begin[0], came->viewpt_begin[1],
			came->viewpt_begin[2]);
		...
		fprintf (F.f, " %#6x  %11.3f %11.3f %11.3f %10.3f", came->unknown_0x0a, came->viewpt_end[0],
			came->viewpt_end[1], came->viewpt_end[2], came->time);
		if (viewpt_len >= 0.01)
	...
	for (gi = 0; gi < ng && auto_next; gi++, ckph++)
	{
		...
		const kmp_ckpt_entry_t *ckpt = (kmp_ckpt_entry_t *)kmp->dlist[KMP_CKPT].list + start;
		uint j;
		for (j = start; j <= end; j++, ckpt++)
		{
			if (ckpt->prev != (j == start ? -1 : j - 1) || ckpt->next != (j == end ? -1 : j + 1))
			{
				auto_next = false;
				break;
			}
	...
	fprintf (F.f, text_kmp_ckpt_cr, REVISION_NUM, kmp->value[KMP_CKPT], kmp->value[KMP_CKPH],
		auto_next, ObjectInfo[RTOBJ_1].name, ObjectInfo[RTOBJ_2].name, RTOBJ_SCALE_X, RTOBJ_SCALE_Y,
		RTOBJ_SCALE_Z, RTOBJ_SCALE_X, RTOBJ_SCALE_Y, RTOBJ_SCALE_Z, RTOBJ_BASE, RTOBJ_BASE);
	...
	for (i = 0; i < n; i++, ckpt++)
	{
		if (print_kmp_group (&F, kmp, &kmp->ckph, &grp, i, "", false) && IS_M1 (ckpt->mode)
			&& !IS_M1 (prev_mode))
		{
			fputs ("\r\n", F.f);
		}
		...
		const uint indent = fprintf (F.f, "%4u %11.3f %11.3f %11.3f %11.3f ", i, ckpt->left[0],
			ckpt->left[1], ckpt->right[0], ckpt->right[1]);
		PrintNameIL (F.f, ill + KMP_JGPT, ckpt->respawn, 5, 5, indent);
		fprintf (F.f, " %3d %3d %3d # %9.3f %7.2f\r\n", IS_M1 (ckpt->mode) ? -1 : (uint)ckpt->mode,
			IS_M1 (ckpt->prev) ? -1 : (uint)ckpt->prev, IS_M1 (ckpt->next) ? -1 : (uint)ckpt->next,
			width, direction);
	}
	...
	for (;;)
	{
		...
		else
		{
			...
			if (relevant_id < N_KMP_GOBJ && ObjectInfo[relevant_id].name)
			{
				...
				fprintf (F.f, "##%4u %#6x  %c%-*s %s\r\n", count, current_id,
					current_id == relevant_id ? ' ' : '|', OBJ_FW_NAME, oi->name, oi->charact);
			}
			else
				fprintf (F.f, "##%4u %#6x\r\n", count, current_id);
			n_types++;
	...
	fprintf (F.f, "##%.83s\r\n## GOBJ total: %u object%s, %u type%s", Minus300, nstd,
		nstd == 1 ? "" : "s", n_types, n_types == 1 ? "" : "s");
	if (n_defobj)
		fprintf (F.f, " + %u definition object%s", n_defobj, n_defobj == 1 ? "" : "s");
	fprintf (F.f, "\r\n##%.83s\r\n\r\n", Minus300);
	...
	for (i = 0; i < n; i++, gobj++)
	{
		...
		if (ad.is_defobj)
		{
			...
			if (usage)
			{
				...
				snprintf (usage_info_buf, sizeof (usage_info_buf),
					"It is used by %u regular object%s.", usage, usage == 1 ? "" : "s");
			}
			else
			...
			{
				fprintf (
					F.f, "# Definition object type %s. Param: %s\r\n", ad.cmd_name, ad.par_info);
				if (!ad.is_logical)
				{
				...
				fprintf (
					F.f, "# %s\r\n  $DEFOBJ-%-7s %#06x", usage_info, ad.cmd_name, gobj->obj_id);
				uint i;
			...
			fprintf (
				F.f, "# Type %#06x: Definition object!\r\n# %s\r\n", GOBJ_M_DEFINITION, usage_info);
		}
		else
		{
			...
			if (ar.is_condref || ar.is_condrnd || ar.is_condtest)
				...
				fprintf (F.f, "# Warning: Valid condition, but P-Flags mode is %u\r\n", ar.mode);
			else if (ar.objref_valid)
				fprintf (F.f, "# Warning: Valid reference id, but P-Flags mode is %u\r\n", ar.mode);
			else if (ar.mode_valid && gobj->ref_id)
				fprintf (F.f, "# Warning: Invalid reference id: %#06x\r\n", gobj->ref_id);
		}

		//--- print object information

		snprintf (buf1, sizeof (buf1), form1, gobj->setting[0], gobj->setting[1], gobj->setting[2],
			gobj->setting[3]);
		switch (relevant_id)
		{
			case GOBJ_ELINE_CONTROL:
			{
				...
				snprintf (buf2, sizeof (buf2), "%6u %6u %6u %6s", gobj->setting[4],
					gobj->setting[5], gobj->setting[6], temp_buf);
			}
			break;

			default:
				snprintf (buf2, sizeof (buf2), form2, gobj->setting[4], gobj->setting[5],
		...
		fprintf (F.f,
			...
			"    >        %11.3f %11.3f %11.3f\r\n",
			gobj->obj_id, gobj->position[0], gobj->position[1], gobj->position[2], buf1,
			GetNameIL (ill + KMP_POTI, gobj->route_id), gobj->ref_id, gobj->rotation[0],
			gobj->rotation[1], gobj->rotation[2], buf2, gobj->pflags, gobj->scale[0],
			gobj->scale[1], gobj->scale[2]);
		...
		if (print_condition)
		{
			...
			PRINT0 ("# CR[%#x,%#x]: is=%d, found=%d, cmd=%s, xor:%x->%#x, c=%s, i=%s\n",
				gobj->ref_id, gobj->obj_id, cri.is_condref, cri.condref != 0, cri.cmd_suffix,
	...
	for (i = 0; i < n; i++, ktpt++)
	{
		fprintf (F.f, "%4u  %11.3f %11.3f %11.3f  %7.2f %7.2f %7.2f %5d %#6x\r\n", i,
			ktpt->position[0], ktpt->position[1], ktpt->position[2], ktpt->rotation[0],
			ktpt->rotation[1], ktpt->rotation[2], ktpt->player_index, ktpt->unknown);
	}
	...
	for (gi = i = 0; gi < ng && i < np; gi++, pg++)
	{
		...
		fprintf (F.f, "\r\n $ROUTE %s, settings: %u %u # section %u\r\n",
			GetNameIL (ill + KMP_POTI, gi), pg->smooth, pg->back, gi + 1);
		...
		for (count = idx = 0; idx < nn; idx++, gobj++)
			if (gobj->route_id == (uint)gi)
			{
				...
				fprintf (F.f, "\t# GOBJ reference: %-8s type %#6x", GetNameIL (ill + KMP_GOBJ, idx),
					gobj->obj_id);
		...
		for (count = idx = 0; idx < nn; idx++, came++)
			if (came->route == gi)
			{
				...
				if (!came_ref)
					came_ref = came_name;
				fprintf (F.f, "%s %s", count++ ? "" : "\t# CAME reference:", came_name);
			}
		...
		{
			fprintf (F.f, "%4u  %11.3f %11.3f %11.3f %5u %5u  #", ei, pp->position[0],
				pp->position[1], pp->position[2], pp->speed, pp->unknown);
			...
			if (ei < en - 1 || is_cyclic)
			{
				...
				total_len += len;
				double tim = len > 0.0 && pp->speed > 0 ? len / (pp->speed * 60) : 0.0;
	...
	for (i = 0; i < n; i++, stgi++)
	{
		...
		fprintf (F.f, "%5u %4u %5u %5u   %#10x  0x%02x %#5x %#6x %#4x  # ", stgi->lap_count,
			stgi->pole_pos, stgi->narrow_start, stgi->enable_lens_flare, stgi->flare_color,
			stgi->flare_alpha, stgi->unknown_09, stgi->speed_mod >> 8, stgi->speed_mod & 0xff);
		if (stgi->speed_mod)
