...
syntax_err:
	ERROR0 (ERR_SYNTAX, "Invalid parameter for option --png: %s\n", si.cur_file->prev_ptr);
	ResetSI (&si);
	return 1;
}
...
{
	noTRACE ("FALL: *%u: %4u %4u / %6d %6d %6d / %u\n", multi, o->ximg, o->yimg, o->xpos, o->ypos,
		o->zpos, o->pix_size);
	...
	if (multi == 1)
	{
		...
		else
		{
			...
			col.val = idx < n_user_color ? user_color[idx].val : 0;
		}
		col.a = 0xff;
		TRACE ("DRAW: %05x -> %08x\n", res_kcl_flag, col.val);
		noPRINT_IF (res_kcl_flag >= N_KCL_FLAG, "DRAW: %05x -> %08x\n", res_kcl_flag, col.val);
...
enumError SaveImageKCL (kmp_t *kmp, // pointer to valid KMP
	kcl_t *kcl, // pointer to valid KCL
...
)
{
	...
	if (opt_png_rm)
	{
		...
		for (; n_tri-- > 0; td++)
		{
			if (td->cur_flag < N_KCL_FLAG && !(1u << (td->cur_flag & 0x1f) & opt_png_type_mask))
			{
	...
	if (n_ckpt)
	{
		...
		for (i = 0; i < n_ckpt; i++, ckpt++)
		{
			...
			if (xmax < ckpt->left[0])
				xmax = ckpt->left[0];
			if (xmin > ckpt->right[0])
				xmin = ckpt->right[0];
			if (xmax < ckpt->right[0])
				xmax = ckpt->right[0];
			...
			if (zmax < ckpt->left[1])
				zmax = ckpt->left[1];
			if (zmin > ckpt->right[1])
				zmin = ckpt->right[1];
			if (zmax < ckpt->right[1])
				zmax = ckpt->right[1];
		}
	...
	TRACE ("SETUP: %d .. %d / %d / %d .. %d => %u*%u *%u /%u\n", param.x1, param.x2, param.y,
		param.z1, param.z2, xn, zn, pix_aalise, pix_size);
	if (verbose >= 0 && !(KCL_MODE & KCLMD_SILENT))
		fprintf (stdlog,
			"  - png draw: x=%d..%d, y=%d, z=%d..%d"
			" => size=%u*%u [a.alising %u], units/pix=%u\n",
			param.x1, param.x2, param.y, param.z1, param.z2, xn, zn,
			pix_aalise > 1 ? pix_aalise : 0, pix_size);
	fflush (stdlog);
...
#pragma GCC diagnostic pop
	...
	if (pix_aalise > 1)
	{
		PRINT ("RESIZE: %u x %u -> %u x %u\n", param.img.width, param.img.height, xn / pix_aalise,
			zn / pix_aalise);
