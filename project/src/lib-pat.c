...
const KeywordTab_t pat_section_name[] = { { PAT_SECT_SETUP, "SETUP", 0, 0 },
	{ PAT_SECT_SIMPLE, "SIMPLE", 0, 0 }, { PAT_SECT_HEADER, "HEADER", 0, 0 },
	{ PAT_SECT_S0_BASE, "S0_BASE", 0, 0 }, { PAT_SECT_S0_LIST, "S0_LIST", 0, 0 },
	{ PAT_SECT_STRINGS, "STRINGS", 0, 0 }, { 0, 0, 0, 0 } };
...
static const KeywordTab_t opt_pat_tab[] = { { 0, "CLEAR", "RESET", PATMD_M_ALL | PATMD_F_HIDE },
	{ PATMD_M_DEFAULT, "DEFAULT", 0, PATMD_M_ALL | PATMD_F_HIDE },

	{ PATMD_LOG, "LOG", 0, 0 }, { PATMD_SILENT, "SILENT", 0, 0 },

	{ 0, "AUTO", 0, PATMD_M_BOTH | PATMD_F_HIDE }, { PATMD_M_BOTH, "BOTH", 0, PATMD_M_BOTH },
	{ PATMD_SIMPLE, "SIMPLE", 0, PATMD_M_BOTH }, { PATMD_COMPLEX, "COMPLEX", 0, PATMD_M_BOTH },

	{ PATMD_TEST0, "TEST0", "T0", PATMD_M_TEST | PATMD_F_HIDE },
	{ PATMD_TEST1, "TEST1", "T1", PATMD_M_TEST }, { PATMD_TEST2, "TEST2", "T2", PATMD_M_TEST },
...
void LoadParametersPAT (ccp log_prefix // not NULL:
...
)
{
	...
	if (!PAT_ACTION_LOG ("Global PAT Modes: %s\n", GetPatMode ()) && log_prefix
		&& PAT_MODE != PATMD_M_DEFAULT && !(PAT_MODE & PATMD_SILENT))
	{
...
int ScanOptPat (ccp arg)
{
	...
	s64 stat = ScanKeywordList (arg, opt_pat_tab, 0, true, 0, PAT_MODE, "Option --pat", ERR_SYNTAX);
	if (stat != -1)
	{
		SetPatMode (stat);
		return 0;
...
static int pat_iter_func (brsub_cut_t *bcut, // pointer to data structure
...
{
	ccp res = index < sizeof (pat_names) / sizeof (*pat_names) ? pat_names[index] : 0;
	if (!res || !*res)
	{
...
int IterateFilesPAT (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	return CutFilesBRSUB (
		it, pat_iter_func, 0, pat_names, sizeof (pat_names) / sizeof (*pat_names) - 1);
}
...
static bool IsSimplePAT (pat_t *pat)
{
	...
	if (pat->n_pelem != 1
		...
		|| pat->head.unknown_00 || pat->head.unknown_02 || pat->head.unknown_0a
		|| pat->head.unknown_0c

		|| pat->base.unknown_04 || pat->base.unknown_08 != htons (0xffff) || pat->base.unknown_0a
		|| pat->base.n_unknown != htons (1) || pat->base.unknown_0e || pat->base.unknown_10
		|| pat->base.unknown_12 || pat->base.unknown_14 || pat->base.unknown_16

		|| pe->belem.unknown_02 || pe->belem.unknown_04 || pe->belem.unknown_06 != htons (1)

		|| pe->sref.unknown_04 || pe->sref.type != htons (5)

		|| pe->shead.unknown_02

		|| !pe->bname || !pe->sname || strcmp (pe->bname, pe->sname))
		return false;
...
ccp GetStringPAT (pat_t *pat, // PAT data structure
...
static float CalcFactorSHEAD (pat_s0_selem_t *se, // element list, big endian
...
static void Close_SList (slist_param_t *param, // parameter, never NULL
...
)
{
	...
	if (list)
	{
		...
		list->factor = IsNormalF (param->factor) ? param->factor
...
enumError CreateRawPAT (pat_t *pat // pointer to valid PAT
)
{
	DASSERT (pat);
	TRACE ("CreateRawPAT()\n");
	...
	uint s0_size = sizeof (pat_s0_bhead_t)
		+ pat->n_pelem
			* (sizeof (pat_s0_belem_t) + sizeof (pat_s0_sref_t) + sizeof (pat_s0_shead_t));
	...
	for (i = 0; i < pat->n_pelem; i++)
	{
		...
		pe->sref.offset_strlist = htonl (shead_offset - sref_offset - sizeof (pat_s0_sref_t) * idx);
...
static enumError ScanText_SETUP (pat_t *pat, // PAT data structure
...
static enumError ScanText_SIMPLE (pat_t *pat, // PAT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (pat);
	...
	int enabled = 1;
	ScanParam_t ptab[] = { { "ENABLED", SPM_INT, &enabled },
		{ "BRRES-NAME", SPM_STRING, &pat->name }, { "PAT-NAME", SPM_STRING, &pe->bname },
		{ "N-FRAMES", SPM_U16, &pat->head.n_frames }, { "CYCLIC", SPM_U16, &pat->head.cyclic },
		{ "UNKNOWN-00", SPM_U16, &pe->belem.unknown_00 }, { "FACTOR", SPM_FLOAT, &parm.factor },
		{ "PADDING", SPM_U16, &parm.unknown_02 }, { 0 } };
	...
	for (;;)
	{
		...
		ParamFieldItem_t *it = FindInsertParamField (&pat->s1_str, str, true, 0, 0);
...
static enumError ScanText_HEADER (pat_t *pat, // PAT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (pat);
	...
	ScanParam_t ptab[]
		= { { "NAME", SPM_STRING, &pat->name }, { "UNKNOWN-00", SPM_U16, &pat->head.unknown_00 },
			  ...
			  { "CYCLIC", SPM_U16, &pat->head.cyclic }, { 0 } };
...
static enumError ScanText_S0_BASE (pat_t *pat, // PAT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (pat);
	...
	ScanParam_t ptab[] = { //	{ "SIZE",	SPM_U32, &pat->base.size },
		...
		{ "UNKNOWN-16", SPM_U16, &pat->base.unknown_16 }, { 0 }
	};
	...
	for (;;)
	{
		...
		if (ch == '$')
		{
			...
			if (!strcmp (name, "$BASE"))
			{
				...
				{
					WarnIgnoreSI (si, "Maximum number of base elements (%u) already defined.\n",
						PAT_MAX_ELEM);
					continue;
				}
...
static enumError ScanText_S0_LIST (pat_t *pat, // PAT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (pat);
	...
	ScanParam_t ptab[] = { { "FACTOR", SPM_FLOAT, &parm.factor },
		{ "PADDING", SPM_U16, &parm.unknown_02 }, { 0 } };
	...
	for (;;)
	{
		...
		if (ch == '$')
		{
			...
			if (!strcmp (name, "$LIST"))
			{
				...
				{
					WarnIgnoreSI (
						si, "Maximum number of string list (%u) already defined.\n", PAT_MAX_ELEM);
					elem = 0;
					continue;
				}
	...
	if (!si->no_warn)
	{
		...
		for (i = 0; i < pat->n_pelem; i++, pe++)
		{
			...
			if (h_idx >= n_list)
			{
				ERROR0 (ERR_WARNING, "No 'string list' for base element %u (index=%u).\n[%s @%u]\n",
...
static enumError ScanText_STRINGS (pat_t *pat, // PAT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (pat);
...
enumError ScanTextPAT (pat_t *pat, // PAT data structure
...
enumError ScanRawPAT (pat_t *pat, // PAT data structure
	bool init_pat, // true: initialize 'pat' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
...
)
{
	...
	{
		return ERROR0 (ERR_INVALID_DATA, "Invalid PAT file: %s\n", pat->fname ? pat->fname : "?");
	}
	...
	for (i = 0; i < pat->n_pelem; i++, pe++)
	{
		...
		pe->bname = STRDUP (
			GetStringPAT (pat, ana.s0_base, be32 (&ana.s0_base->elem[i].offset_name), "?"));
		pe->sname
			= STRDUP (GetStringPAT (pat, ana.s0_sref[i], be32 (&ana.s0_sref[i]->offset_name), "?"));
...
enumError ScanPAT (pat_t *pat, // PAT data structure
	bool init_pat, // true: initialize 'pat' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
	ContainerPAT_t *cdata, // NULL or container-data
...
)
{
	...
	switch (GetByMagicFF (data, data_size, data_size))
	{
		...
		default:
			...
			if (init_pat)
				InitializePAT (pat);
			return ERROR0 (ERR_INVALID_DATA, "No PAT file: %s\n", pat->fname ? pat->fname : "?");
	}
...
enumError ScanRawDataPAT (pat_t *pat, // PAT data structure
	bool init_pat, // true: initialize 'pat' first
...
)
{
	...
	raw->fname = 0;
#if USE_NEW_CONTAINER_PAT
	return ScanPAT (pat, false, raw->data, raw->data_size, LinkContainerRawData (raw), mode);
#else
...
enumError LoadPAT (pat_t *pat, // PAT data structure
	bool init_pat, // true: initialize 'pat' first
...
)
{
	...
	if (!err)
	{
		...
		raw.fname = 0;
#if USE_NEW_CONTAINER_PAT
		err = ScanPAT (pat, false, raw.data, raw.data_size, LinkContainerRawData (&raw), mode);
#else
...
enumError SaveRawPAT (pat_t *pat, // pointer to valid PAT
...
void SaveSimplePAT (pat_t *pat, // pointer to valid PAT
	FILE *f, // open FILE
...
)
{
	...
	if (outmode & PATMD_COMPLEX)
	{
		...
		fprintf (f,
			...
			"@ENABLED = %u\r\n",
			is_simple ? "" : " (default because of complex PAT)",
			is_simple ? " (default for this simple PAT)" : " (data will be lost if enabled)",
			is_simple);
	}
	...
	fprintf (f,
		...
		"@CYCLIC     = %5u\r\n"
		"@UNKNOWN-00 = %5u\r\n",
		GetNamePAT (pat), pe->bname, ntohs (pat->head.n_frames), ntohs (pat->head.cyclic),
		ntohs (pe->belem.unknown_00));

	float factor = bef4 (&pe->shead.factor);
	if (!IsSameF (factor, CalcFactorSHEAD (pe->selem, pe->selem_used), 2))
		fprintf (
			f, "@FACTOR     = %8.6f  # if defined, automatic calculation is disabled\r\n", factor);
	...
	fprintf (f,
		...
		"    #-----------------%.*s#-----------\r\n",
		fw - 4, Minus300, fw - 4, "", fw - 4, Minus300);
	...
	for (l = 0, se = pe->selem; l < pe->selem_used; l++, se++)
	{
		...
		fprintf (f, "%13.4f  \"%s\" %*s# %9.4f\r\n", time - prev, str, (int)(fw - strlen (str)), "",
			time);
		prev = time;
	}
...
enumError SaveTextPAT (pat_t *pat, // pointer to valid PAT
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
	...
	if (brief_count)
	{
		fprintf (F.f,
			...
			"OUTPUT-MODE = %s\r\n"
			"\r\n",
			section_sep, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE, pat->version, pat->n_sect,
			outmode_info);
	}
	else
	{
		fprintf (F.f, text_pat_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE,
	...
	fprintf (F.f,
		...
		"@CYCLIC     = %5u\r\n"
		"\r\n",
		GetNamePAT (pat), ntohs (h->unknown_00), ntohs (h->unknown_02), ntohs (h->n_frames),
		ntohs (h->n_sect0), calc_on_scan, ntohs (h->n_sect1), calc_on_scan, ntohs (h->unknown_0a),
		ntohs (h->unknown_0c), ntohs (h->cyclic));
	...
	fprintf (F.f,
		...
		"@UNKNOWN-16 = %#6x\r\n"
		"\r\n",
		ntohl (bh->size), calc_on_scan, ntohs (bh->unknown_04), ntohs (bh->n_elem), calc_on_scan,
	...
	for (i = 0; i < pat->n_pelem; i++, pe++)
	{
		fprintf (F.f, " $BASE B%-2u  %#6x %#6x %#6x %#6x   R%-3u \"%s\"\r\n", i,
			ntohs (pe->belem.unknown_00), ntohs (pe->belem.unknown_02),
			ntohs (pe->belem.unknown_04), ntohs (pe->belem.unknown_06), i, pe->bname);
	}
	...
	for (i = 0; i < pat->n_pelem; i++)
	{
		...
		fprintf (F.f, " $SREF R%-2u  %#6x %#6x   L%-3u \"%s\"\r\n", idx,
			ntohs (pe->sref.unknown_04), ntohs (pe->sref.type), idx, pe->sname);
	}
	...
	for (i = 0; i < pat->n_pelem; i++)
	{
		...
		fprintf (F.f,
			...
			"    #-----------------------#-----------\r\n",
			idx, ntohs (pe->shead.n_elem), 1.0 / factor,
			IsSameF (factor, CalcFactorSHEAD (pe->selem, pe->selem_used), 1) ? '#' : ' ', factor,
			ntohs (pe->shead.unknown_02));
		...
		for (l = 0; l < pe->selem_used; l++, se++)
		{
			...
			fprintf (F.f, "%13.4f  S%-3u %#6x  # %9.4f\r\n", tiome - prev, ntohs (se->index),
				ntohs (se->unknown_06), tiome);
			prev = tiome;
		}
