...
)
{
	...
	*path_ptr = 0;
	...
	for (ii = 0; ii < n_item; ii++)
	{
		...
		noPRINT ("%3u: %6x %6x  %s\n", ii, endian->rd32 (&idata->offset),
			endian->rd32 (&idata->size), name);
...
)
{
	...
	PRINT ("CreateBREFF() ndir=%d, totsize=%x, align=%x,%x\n", szs->subfile.used, total_size,
		opt_align_breff, opt_align_breft);
	...
	szs->ff_version = setup_param && setup_param->version > 0 ? setup_param->version : 9;

	uint align;
	if (is_breft)
	{
		align = opt_align_breft	   ? opt_align_breft
			: szs->ff_version <= 9 ? BREFT_9_DEFAULT_ALIGN
	...
	else
	{
		...
		while (beg >= source_dir && *beg != '/')
		{
			if (*beg == '.')
			{
				...
				if (len == 2 & !memcmp (beg, ".d", 2) || len == 6 & !memcmp (beg, ".breff", 6)
	...
	for (fptr = szs->subfile.list; fptr < fend; fptr++)
	{
		...
		{
			enumError err
				= LoadFILE (source_dir, fptr->path, 0, data, fptr->size, 0, &szs->fatt, true);
			if (max_err < err)
				max_err = err;
		}
