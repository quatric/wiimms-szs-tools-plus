...
typedef enum le_options_t
{
	...
	LEO_HEX2SLOT = 0x000400000000ull, // if reading an szs of format '%0[34}x.*', use it to force
									  // slot
...
static inline void ResetLT (le_track_t *lt)
{
	ClearLT (lt, 0);
}
...
{
	return !slot || slot >= MIN_DISTRIBUTION_SLOT && slot < MAX_DISTRIBUTION_TRACK;
}
...
void PrintDistribHead (ccp format, ...) __attribute__ ((__format__ (__printf__, 1, 2)));
