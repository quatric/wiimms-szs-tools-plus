...
typedef struct ctcode_header_t
{
	...
	/* 10 */ be32_t padding[4];
	/* 20 */ ctcode_sect_info_t sect_info[6]; // section offsets and names
	/* 50 */
...
int IterateFilesCTCODE (struct szs_iterator_t *it, // iterator struct with all infos
...
static inline enumError SaveTrackListByOptCTCODE (ctcode_t *ctcode, // pointer to valid CTCODE
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
	return SaveTrackListCTCODE (
		ctcode, fname, raw_mode, long_count, !brief_count, print_header, raw_mode);
}
