...
{
	return ScanSizeOptU32 (&set_flags, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
int ScanOptAScale (ccp arg)
{
	...
	{
		err = ERROR0 (ERR_SEMANTIC, "Option --ascale: Scale factor too small: %s\n", arg);
		goto abort;
	}
	...
	{
		err = ERROR0 (ERR_SEMANTIC, "Option --ascale: Vector is too small: %s\n", arg_vector);
		goto abort;
	}
	err = CheckEolSI (&si);
	if (err)
...
int ScanOptARotate (ccp arg)
{
	...
	if (len < NULL_EPSILON)
	{
		err = ERROR0 (ERR_SEMANTIC, "Option --arot: Points to near: %s\n", arg_vector);
		goto abort;
	}
	err = CheckEolSI (&si);
	if (err)
...
int ScanOptXSS (uint xyz, ccp arg)
{
	...
	{
		err = ERROR0 (ERR_SEMANTIC, "%s: 'old' values are to close together, delta=%6.3e", opt_info,
			fabs (old1 - old2));
		goto abort;
	}
...
static bool tfs_call_macro (const ScanMacro_t *macro, // macro to call
...
)
{
	...
	ScanFile_t *newsf
		= AddSF (&tfs_si, macro->data, macro->data_size, macro->src_name, REVISION_NUM, 0);
	DASSERT (newsf);
...
bool TformScriptEnd ()
{
	...
	PRINT ("### SCRIPT-STAT: %u | %u + %u = %u\n", tfs_index, tfs_count_f, tfs_count_d,
		tfs_count_f + tfs_count_d);
...
int NextTransformation (bool for_close) // enter next transformation step
{
	...
	if (opt_transform.transform_enabled || force_transform == 1 && for_close)
	{
		...
		if (n_transform_list >= N_TRANSFORM - !for_close)
		{
			...
			{
				err_done = true;
				ERROR0 (ERR_SEMANTIC, "Maximum number of --next (%u) already reached.\n",
					N_TRANSFORM - 1);
			}
			return 1;
		}

		memcpy (transform_list + n_transform_list, &opt_transform, sizeof (*transform_list));
...
void DumpTransformationOpt ()
{
	...
	if (have_set_value)
		printf ("  set-xyz-min:      %11.3f %11.3f %11.3f\n"
			...
			set_value_min.x, set_value_min.y, set_value_min.z, set_value_max.x, set_value_max.y,
			set_value_max.z);

	if (ascale != 0.0)
	{
		printf ("  last axis-scale:  %11.3f @ %11.3f %11.3f %11.3f\n", ascale, ascale_dir.x,
			ascale_dir.y, ascale_dir.z);
	}
	...
	if (arot_deg != 0.0)
	{
		printf ("  last axis-rot:    %11.3f @ %11.3f %11.3f %11.3f\n"
			...
			arot_deg, arot1.x, arot1.y, arot1.z, "->", arot2.x, arot2.y, arot2.z);
	}
...
)
{
	...
	if (IsNormValid () && opt_transform.scale_enabled)
	{
		...
		const float scale_1d
			= (fabs (opt_transform.norm_scale.x) + fabs (opt_transform.norm_scale.z)) / 2;
		...
		for (mode = 1; mode < max_mode; mode++)
			if (verbose > 0 || !tr_ok[mode])
				printf (" Transform[%u]:   %11.3f %11.3f %11.3f [%d]\n", mode, tr[mode].x,
					tr[mode].y, tr[mode].z, tr_ok[mode]);
		printf ("%.57s\n", Minus300);
	}
	return all_ok;
}
	...
	else
	{
		...
		for (i = 0; i < n_transform_list; i++)
		{
			...
			printf ("--------------- transformation matrix %u/%u [%s,%s,%s,%c] ---------------\n",
				i + 1, n_transform_list, xyz[t->scale_enabled & 7], xyz[t->rotate_enabled & 7],
				xyz[t->translate_enabled & 7], use[t->use_matrix & 3]);
			PrintMatrixD (stdout, 0, 0, t, 0, pmode_step);
		}
	}

	printf ("%.14s Final transformation matrix [%s,%s,%s,%c] %.14s\n", sep,
		xyz[opt_transform.scale_enabled & 7], xyz[opt_transform.rotate_enabled & 7],
		xyz[opt_transform.translate_enabled & 7], use[opt_transform.use_matrix & 3], sep);
