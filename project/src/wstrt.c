...
#define TITLE                                                                                      \
	WSTRT_SHORT ": " WSTRT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
static enumError cmd_test_options ()
{
	...
	printf ("  vs-region:   %16x = %12d  [%c%c]\n", opt_str_vs_region, opt_str_vs_region,
		opt_str_xvs_region ? 'x' : '-', opt_str_tvs_region ? 't' : '-');
	printf ("  bt-region:   %16x = %12d  [%c%c]\n", opt_str_bt_region, opt_str_bt_region,
		opt_str_xbt_region ? 'x' : '-', opt_str_tbt_region ? 't' : '-');
...
static enumError cmd_dump (bool use_c)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		{
			if (verbose >= 0)
				printf ("\n%sDUMP of %s:%s\n", use_c ? "C-" : "", GetNameFF (str.fform, 0), arg);
...
static enumError cmd_hexdump ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		{
			is_ok = false;
			err = ERROR0 (ERR_WRONG_FILE_TYPE, "DOL expected, file ignored: %s\n", arg);
		}
...
static void setup_order (int index[STR_ZBI_N], int delta)
{
	...
	if (opt_order)
	{
		...
		while (*ptr && dest_index < STR_ZBI_N)
		{
			...
			switch (*ptr++)
			{
				case 'p':
				case 'P':
					idx = STR_M_PAL - STR_ZBI_FIRST;
					break;
				case 'e':
				case 'E':
				case 'u':
				case 'U':
					idx = STR_M_USA - STR_ZBI_FIRST;
					break;
				case 'j':
				case 'J':
					idx = STR_M_JAP - STR_ZBI_FIRST;
					break;
				case 'k':
				case 'K':
					idx = STR_M_KOR - STR_ZBI_FIRST;
					break;
			}
	...
	if (logging >= 3)
		fprintf (stdlog, "# Region Order: %d:%s %d:%s %d:%s %d:%s\n", index[0],
			GetStrModeName (index[0] + delta), index[1], GetStrModeName (index[1] + delta),
			index[2], GetStrModeName (index[2] + delta), index[3],
			GetStrModeName (index[3] + delta));
}
...
static enumError cmd_port ()
{
	...
	if (verbose > 1)
	{
		...
		printf ("\nAddress Porting Database:\n"
				...
				"  N(records): %5u\n"
				"  Size:       %5s\n",
			addr_port_file, PrintTimeByFormat ("%F %T %Z", vers->timestamp), vers->db_version,
			vers->revision, vers->n_records,
	...
	for (param = first_param; param; param = param->next)
	{
		...
		{
			printf ("%s#%.*s\n%s#  %s%s      %s%s      %s%s      %s%s     where", colout->heading,
				sep_len_3, ThinLine300_3, colout->heading, extra,
				GetStrModeName (index[0] + STR_ZBI_FIRST), extra,
				GetStrModeName (index[1] + STR_ZBI_FIRST), extra,
				GetStrModeName (index[2] + STR_ZBI_FIRST), extra,
				GetStrModeName (index[3] + STR_ZBI_FIRST));

			if (long_count)
				printf ("%s off(%.1s)%s off(%.1s)%s off(%.1s)%s off(%.1s)", extra,
					GetStrModeName (index[0] + STR_ZBI_FIRST), extra,
					GetStrModeName (index[1] + STR_ZBI_FIRST), extra,
					GetStrModeName (index[2] + STR_ZBI_FIRST), extra,
					GetStrModeName (index[3] + STR_ZBI_FIRST));
		...
		if (!ap)
		{
			printf ("%s! Can't port %s address 0x%08x (%s)!%s\n", colout->warn,
				GetStrModeName (mode), hex.num, atname, colout->reset);
		}
		else
		{
			...
			printf ("  %s%-5.5s%s", GetAddressTypeColor (colout, atype), atname, colout->reset);
			...
			printf ("  %s%u: %s%s\n", GetAddressSizeColor (colout, asmode), asmode,
				addr_size_flags_info[asmode], colout->reset);
		}
	}
	...
	for (param = first_param; param; param = param->next)
	{
		...
		if (print_header && !count++)
		{
			...
			printf ("\n%s#%.*s\n%s#%s Address   %s %s %s %s %s %s %s %.*s\n%s#%.*s%s\n",
				colout->heading, sep_len_3, ThinLine300_3, colout->heading, opt_no_0x ? "" : "  ",
				GetStrModeName (index[0]), offset, GetStrModeName (index[1]), offset,
				GetStrModeName (index[2]), offset, GetStrModeName (index[3]), opt_no_0x ? 7 : 9,
				offset, colout->heading, sep_len_3, ThinLine300_3, colout->reset);
		}
		...
		for (int i = 0; i < STR_ZBI_N; i++)
		{
			...
			printf (" %s %s%-4.4s%s", long_count && i > 0 ? ":" : "",
				GetAddressTypeColor (colout, atype), GetAddressTypeName (atype), colout->reset);
...
static int compare_server_patch (const str_server_patch_t *a, const str_server_patch_t *b)
{
...
static enumError cmd_https ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (err <= ERR_WARNING && err != ERR_NOT_EXISTS)
		{
			...
			while (ptr < end)
			{
				...
				{
					ptr = ptr_g;
					if (memcmp (ptr, "gs.nintendowifi.net", 19) || isalnum ((int)ptr[19])
						|| isalnum ((int)ptr[-1]))
					{
						ptr++;
						continue;
	...
	if (n_list > 1)
		qsort (patch_list, n_list, sizeof (*patch_list), (qsort_func)compare_server_patch);
	...
	if (long_count > 0)
	{
		...
		for (i = 0; i < n_list; i++, pat++)
		{
			...
			printf ("  { STR_M_%s,%u, %u,SERV_%s,%*s 0x%06x, %2u, \"%s\" },\n",
				GetStrModeName (pat->mode), pat->flags, pat->is_http, name,
				(int)(9 - strlen (name)), "", pat->offset, pat->url_len, pat->param);
		}
		printf ("\n  {0,0,0,0,0,0,0}\n};\n\n");
	}
	else
	{
		...
		for (i = 0; i < n_list; i++, pat++)
		{
			...
			printf (" %c %s %s %s %-10s %06x %2u %s\n", differ ? '!' : ' ',
				GetStrModeName (pat->mode), pat->flags & STR_F_IS_DOL ? "DOL" : "REL",
				http_text[pat->is_http], ServerList[pat->serv_type].name, pat->offset, pat->url_len,
				pat->param);
			if (differ && verbose > 0)
			{
		...
		if (err <= ERR_WARNING && err != ERR_NOT_EXISTS)
		{
			...
			if (verbose >= 0 || testmode)
			{
				fprintf (stdlog, "%s%sEXTRACT %s:%s -> %s\n", verbose > 1 ? "\n" : "",
					testmode ? "WOULD " : "", GetNameFF (str.fform, 0), str.fname, dest);
				fflush (stdlog);
			}
...
static void print_found (uint idx1, uint idx2, uint n, const TrackInfo_t *info, char prefix)
{
	DASSERT (info);
	DASSERT (prefix);
	if (long_count > 2)
		printf ("%c%u.%u %2u %2u %s %s %s %s\n", prefix, idx1 / n + 1, idx1 % n + 1, idx1, idx2,
			info->track_fname, info->sound_n_fname, info->sound_f_fname,
			use_de ? info->name_de : info->name_en);
	else if (long_count > 1)
		printf ("%c%u.%u %s %s %s %s\n", prefix, idx1 / n + 1, idx1 % n + 1, info->track_fname,
			info->sound_n_fname, info->sound_f_fname, use_de ? info->name_de : info->name_en);
	else if (long_count > 0)
		printf ("%c%u.%u %s\n", prefix, info->def_slot / 10, info->def_slot % 10,
			use_de ? info->name_de : info->name_en);
	else
		printf ("%c%u.%u\n", prefix, info->def_slot / 10, info->def_slot % 10);
}
...
static enumError cmd_find ()
{
	...
	static const KeywordTab_t tab[] = { { SUBCMD_TRACKS, "TRACKS", 0, 0 },
		{ SUBCMD_ARENAS, "ARENAS", 0, 0 }, { SUBCMD_TRACKS, "+TRACKS", 0, 1 },
		{ SUBCMD_ARENAS, "+ARENAS", 0, 1 }, { 0, 0, 0, 0 } };

	const KeywordTab_t *cmd = ScanKeyword (0, first_param->arg, tab);
	if (!cmd)
		return ERROR0 (ERR_SYNTAX, "Invalid sub command for 'FIND': %s\n", first_param->arg);
	...
	for (param = first_param->next; param; param = param->next)
	{
		...
		{
			bool stat = cmd->id == SUBCMD_TRACKS ? find_track (name, cmd->opt != 0)
...
static enumError cmd_points ()
{
	...
	{
		ERROR0 (ERR_WARNING, "%u file parameter%s ignored because of --points\n", n_param,
			n_param == 1 ? "" : "s");
		n_param = 0;
	}
		...
		if (err <= ERR_WARNING && err != ERR_NOT_EXISTS)
		{
			...
			if (opt_dest && *opt_dest)
			{
				if (IsDirectory (opt_dest, 0))
				{
					...
					dest_fname = PathCatPP (
						path_buf, sizeof (path_buf), opt_dest, slash ? slash + 1 : arg);
				}
				else
					dest_fname = opt_dest;
			}
...
static enumError CheckOptions (int argc, char **argv, bool is_env)
{
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_CHDIR:
				...
				break;
			case GO_CONST:
				break; // [[2do]] err += ScanOptConst(optarg); break;
			...
			case GO_ARENAS:
				...
				break;
			case GO_REGION:
				err += ScanOptRegion (optarg);
				break;
			case GO_VS_REGION:
				err += ScanOptVersusRegion (optarg);
				break;
			case GO_BT_REGION:
				err += ScanOptBattleRegion (optarg);
				break;
			case GO_ALL_RANKS:
				err += ScanOptAllRanks (optarg);
				break;
			case GO_POINTS:
				err += ScanOptMkwPoints (optarg, false);
				break;
			case GO_CHEAT:
				err += ScanOptCheatRegion (optarg);
				break;
			case GO_ADD_LECODE:
				AppendStringField (&opt_wcode_list, "@LECODE", false);
				break;
			case GO_ADDS_LECODE:
				AppendStringField (&opt_sect_list, "@LECODE", false);
				break;
			case GO_ADD_OLD_LECODE:
				AppendStringField (&opt_wcode_list, "@OLDLECODE", false);
				break;
			case GO_ADD_CTCODE:
				opt_add_ctcode = true;
				break;
			case GO_CT_DIR:
				AppendStringField (&ct_dir_list, optarg, false);
				break;
			case GO_MOVE_D8:
				opt_move_d8 = true;
				break;
			case GO_ADD_SECTION:
				AppendStringFieldExpand (&opt_sect_list, optarg, 0, WM__DEFAULT);
				break;
			case GO_FULL_GCH:
				opt_full_gch = true;
				break;
			...
			case GO_GCT_SEP:
				...
				break;
			case GO_GCT_ASM_SEP:
				opt_gct_asm_sep = true;
				break;
			case GO_GCT_LIST:
				opt_gct_list++;
				break;
			case GO_GCT_MOVE:
				err += ScanOptGctMove (optarg);
				break;
			case GO_GCT_ADDR:
				err += ScanOptGctAddr (optarg);
				break;
			case GO_GCT_SPACE:
				err += ScanOptGctSpace (optarg);
				break;
			case GO_ALLOW_USER_GCH:
				err += ScanOptAllowUserGch (optarg);
				break;
			case GO_GCT_NO_SEP:
				opt_gct_scan_sep = false;
				break;

			case GO_CREATE_SECT:
				err += ScanOptCreateSect (optarg);
				break;
			case GO_WPF:
				AppendStringFieldExpand (&opt_wpf_list, optarg, 0, WM__DEFAULT);
				break;

			case GO_HTTPS:
				err += ScanOptHttps (optarg);
				break;
			case GO_DOMAIN:
				err += ScanOptDomain (0, optarg);
				break;
			case GO_WIIMMFI:
				err += ScanOptDomain ("domain", "wiimmfi.de");
				break;
			case GO_TWIIMMFI:
				err += ScanOptDomain ("domain", "test.wiimmfi.de");
				break;
			case GO_WC24:
				opt_wc24 = true;
				break;
			case GO_WCODE:
				err += ScanOptWCode (optarg);
				break;
			case GO_ADD_WCODE:
				AppendStringFieldExpand (&opt_wcode_list, optarg, 0, WM__DEFAULT);
				break;
			case GO_PB_MODE:
				err += ScanOptPBMode (optarg);
				break;
			case GO_PATCHED_BY:
				err += ScanOptPatchedBy (optarg);
				break;
			case GO_VS:
				err += ScanOptVS (0, 0, optarg);
				break;
			case GO_VS2:
				err += ScanOptVS (0, 1, optarg);
				break;
			case GO_BT:
				err += ScanOptVS (1, 0, optarg);
				break;
			case GO_BT2:
				err += ScanOptVS (1, 1, optarg);
				break;
			case GO_CANNON:
				err += ScanOptCannon (optarg);
				break;
			case GO_MENO:
				opt_meno = true;
				break;
			...
			case GO_VADDR:
				InsertAddressMemMap (&vaddr, true, optarg, 16);
				break;
			case GO_FADDR:
				InsertAddressMemMap (&faddr, true, optarg, 16);
				break;
			case GO_SNAME:
				dol_sections |= ScanDolSectionList (optarg, 0);
				break;
			...
			case GO_INT7:
			case GO_INT8:
				xparam.format = opt_stat - GO_INT1 + XDUMPF_INT_1;
				break;
			case GO_FLOAT:
				xparam.format = XDUMPF_FLOAT;
				break;
			case GO_DOUBLE:
				xparam.format = XDUMPF_DOUBLE;
				break;

			case GO_LE:
				xparam.endian = DC_LITTLE_ENDIAN;
				break;
			case GO_BE:
			...
			case GO_ZEROS:
				xparam.mode_zero = true;
				break;
			case GO_HEX:
				xparam.mode_dec = false;
				break;
			case GO_DEC:
				xparam.mode_dec = true;
				break;
			case GO_C_SYNTAX:
				xparam.mode_c = true;
				break;
			...
			case GO_ADDR:
				err += ScanOptU64 (&xparam.start_addr, "addr", optarg);
				break;
			case GO_ALIGN:
				xparam.mode_align = true;
				break;
			case GO_TRIGGER:
				err += ScanOptU64 (&xparam.trigger, "trigger", optarg);
				xparam.have_trigger = true;
				break;
			case GO_NO_ADDR:
				xparam.print_addr = false;
				break;
			case GO_NO_NUMBERS:
				xparam.print_number = false;
				break;
			case GO_NO_TEXT:
				xparam.print_text = false;
				break;
			case GO_FORMAT:
				xparam.print_format = true;
				break;
			case GO_NO_NULL:
				xparam.mode_ignore = true;
				break;

			case GO_NARROW:
				xparam.extra_space = -1;
				break;
			case GO_BYTES:
				xparam.min_width = str2ul (optarg, 0, 10);
				break;
	...
	if (!is_env)
	{
		if (opt_wcode == WCODE_AUTO)
			opt_wcode = wifi_domain_is_wiimmfi || opt_wcode_list.used ? WCODE_ON : WCODE_OFF;
...
static enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_FILEATTRIB:
			...
			break;
		case CMD_RAWDUMP:
			err = cmd_rawdump (maindol_data_tab);
			break;
