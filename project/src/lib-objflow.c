...
void InitializeOBJFLOW (objflow_t *objflow)
{
	...
	DASSERT (bz_orig);
	const uint copy_size
		= bz_orig->size < sizeof (objflow->bin) ? bz_orig->size : sizeof (objflow->bin);
...
)
{
	...
	const uint copy_size = data_size < sizeof (objflow->bin) ? data_size : sizeof (objflow->bin);
...
static const KeywordTab_t objflow_section_name[] = { { OBJFLOW_IGNORE, "END", 0, 0 },
	{ OBJFLOW_SETUP, "SETUP", 0, 0 }, { OBJFLOW_OBJECTS, "OBJECTS", 0, 0 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static enumError ScanTextSETUP (objflow_t *objflow, // OBJFLOW data structure
...
static enumError ScanTextOBJECTS (objflow_t *objflow, // OBJFLOW data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (objflow);
	...
	for (;;)
	{
		...
		else if (!strcmp (name, "PARAM"))
		{
			if (!obj)
			{
				...
				if (si->no_warn <= 0)
					ERROR0 (
						ERR_WARNING, "No object selected [%s @%u]: %s\n", sf->name, sf->line, name);
				GotoEolSI (si);
			}
			else
			{
		...
		else
		{
			...
			si->total_err++;
			if (si->no_warn <= 0)
				ERROR0 (ERR_WARNING, "Unknown keyword [%s @%u]: %s\n", sf->name, sf->line, name);
			GotoEolSI (si);
		}
		CheckEolSI (si);
	}
	...
	switch (GetByMagicFF (data, data_size, data_size))
	{
		...
		default:
			if (init_objflow)
				InitializeOBJFLOW (objflow);
			return ERROR0 (
				ERR_INVALID_DATA, "No OBJFLOW file: %s\n", objflow->fname ? objflow->fname : "?");
	}
...
)
{
	...
	if (!tiny)
		fprintf (F.f, text_objflow_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);

#if USE_OBJECT_MANAGER > 1
	WriteObjMgrToSetupSection (F.f, &objflow->obj_mgr, long_count > 0, tiny, explain);
#endif
