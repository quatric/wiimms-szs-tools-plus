...
static void WriteWavPCM16 ( u8 **out_data, size_t *out_size, const brstm_audio_t *audio )
{
    ...
    memcpy(p, &riff_size, 4); p += 4; // little-endian host assumed (x86/ARM); fine for a WAV writer
...
int main ( int argc, char **argv )
{
    ...
    for ( int i = 3; i < argc; i++ )
    {
        ...
        else if ( !strcmp(argv[i], "--bfstm") ) variant = BRSTM_VARIANT_FSTM;
        else if ( !strcmp(argv[i], "--bcstm") ) variant = BRSTM_VARIANT_CSTM;
        ...
        if ( !err )
        {
            ...
            if ( !err )
                printf("wbrstm: decoded %s -> %s (%d ch, %d Hz, %lld samples, %s)\n",
                    ...
                    (long long)audio.n_samples, audio.is_adpcm ? "ADPCM" : "PCM16");
        }
    }
    else if ( !strcasecmp(cmd, "from_wav") || !strcasecmp(cmd, "from-wav") )
    {
        ...
        if ( !output_path )
        {
            ccp ext = variant == BRSTM_VARIANT_FSTM ? ".bfstm" :
                      variant == BRSTM_VARIANT_CSTM ? ".bcstm" : ".brstm";
    ...
    else if ( !strcasecmp(cmd, "info") )
    {
        ...
        if ( !err )
        {
            ccp variant_name = audio.variant == BRSTM_VARIANT_FSTM ? "BFSTM (Wii U)" :
                                audio.variant == BRSTM_VARIANT_CSTM ? "BCSTM (3DS)" : "BRSTM (Wii)";
            printf("File: %s\nContainer: %s\nChannels: %d\nSample rate: %d\nSamples: %lld\nCodec: %s\nLoop: %s",
                input_path, variant_name, audio.channels, audio.sample_rate, (long long)audio.n_samples,
                audio.is_adpcm ? "ADPCM_THP" : "PCM16",
                audio.loop ? "yes" : "no");
            if ( audio.loop )
