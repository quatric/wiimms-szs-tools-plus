...
static const le_cup_ref_t standard_vs_ref[MKW_N_TRACKS] = { 8, 1, 2, 4, 16, 20, 25, 26, 0, 5, 6, 7,
	27, 31, 23, 18, 9, 15, 11, 3, 21, 30, 29, 17, 14, 10, 12, 13, 24, 22, 19, 28 };

static const le_cup_ref_t mkwfun_random_ref[MKW_N_LE_RANDOM] = { 62, 63, 64, 65 };
...
{
	return track_type & LTTY_TRACK ? "vs"
		: track_type & LTTY_ARENA  ? "bt"
		: track_type & LTTY_RANDOM ? "rd"
								   : "xx";
}
...
static KeywordTab_t keywords_leo[] = { { LEO__DEFAULT, "RESET", 0, ~0 }, { 0, "CLEAR", 0, ~0 },

	{ LTT_SHA1, "SHA1", 0, LEO_LTT_SELECTOR }, { LTT_SHA1_D, "D-SHA1", "DSHA1", LEO_LTT_SELECTOR },
	...
	{ LTT_TEMP1, "TEMP1", 0, LEO_LTT_SELECTOR }, { LTT_TEMP2, "TEMP2", 0, LEO_LTT_SELECTOR },
#endif

	{ LEO_MKW, "MKW", 0, 0 }, { LEO_MKW1, "MKW1", 0, 0 }, { LEO_MKW2, "MKW2", 0, 0 },
	{ LEO_CTCODE, "CT-CODE", "CTCODE", 0 }, { LEO_LECODE, "LE-CODE", "LECODE", 0 },

	{ LEO_IN_EMPTY, "IN-EMPTY", "INEMPTY", 0 }, { LEO_IN_EMPTY, "IEMPTY", 0, 0 },
	{ LEO_IN_MINUS, "IN-MINUS", "INMINUS", 0 }, { LEO_IN_MINUS, "IMINUS", 0, 0 },
	{ LEO_IN_ALL, "IN-ALL", "INALL", 0 }, { LEO_IN_ALL, "IALL", 0, 0 },

	{ LEO_OVERWRITE, "OVERWRITE", 0, LEO_M_HOW }, { LEO_INSERT, "INSERT", 0, LEO_M_HOW },
	{ LEO_REPLACE, "REPLACE", 0, LEO_M_HOW },

	{ LEO_VERSUS, "VERSUS", "VS", 0 }, { LEO_BATTLE, "BATTLE", "BT", 0 },
	{ LEO_CUSTOM, "CUSTOM", 0, 0 }, { LEO_ORIGINAL, "ORIGINAL", 0, 0 },
	{ LEO_NO_D_FILES, "NO-D", "NOD", 0 },

	{ LEO_OUT_EMPTY, "OUT-EMPTY", "OUTEMPTY", 0 }, { LEO_OUT_EMPTY, "OEMPTY", 0, 0 },
	{ LEO_OUT_MINUS, "OUT-MINUS", "OUTMINUS", 0 }, { LEO_OUT_MINUS, "OMINUS", 0, 0 },
	{ LEO_OUT_DUMMY, "OUT-DUMMY", "OUTDUMMY", 0 }, { LEO_OUT_DUMMY, "ODUMMY", 0, 0 },
	{ LEO_OUT_ALL, "OUT-ALL", "OUTALL", 0 }, { LEO_OUT_ALL, "OALL", 0, 0 },

	{ LEO_IN_LECODE, "IN-LECODE", "INLECODE", 0 }, { LEO_IN_LECODE, "ILECODE", 0, 0 },
	{ LEO_NO_SLOT, "NO-SLOT", "NOSLOT", 0 }, { LEO_HEX2SLOT, "HEX2SLOT", 0, 0 },
	{ LEO_NAME2SLOT, "NAME2SLOT", 0, 0 }, { LEO_CT_SLOTS, "CT-SLOTS", "CTSLOTS", 0 },
	{ LEO_BRIEF, "BRIEF", 0, 0 }, { LEO_AUTO_PATH, "AUTO-PATH", "AUTOPATH", 0 },

	{ LEO_PROGRESS0, "PR0", 0, LEO_M_PROGRESS }, { LEO_PROGRESS1, "PR1", 0, LEO_M_PROGRESS },
	{ LEO_PROGRESS2, "PR2", 0, LEO_M_PROGRESS }, { LEO_PROGRESS5, "PR5", 0, LEO_M_PROGRESS },
	{ LEO_PROGRESS10, "PR10", "PROGRESS", LEO_M_PROGRESS },
	{ LEO_PROGRESS20, "PR20", 0, LEO_M_PROGRESS }, { LEO_PROGRESS30, "PR30", 0, LEO_M_PROGRESS },
	{ LEO_PROGRESS60, "PR60", 0, LEO_M_PROGRESS },

	{ LEO_HELP, "HELP", "H", 0 },

	{ LEO_CUT_ALL, "CUT-ALL", "CUTALL", 0 }, { LEO_CUT_STD, "CUT-STD", "CUTSTD", 0 },
...
{
	return !name || !*name		   ? (opt & LEO_IN_EMPTY) != 0
		: *name == '-' && !name[1] ? (opt & LEO_IN_MINUS) != 0
								   : true;
}
...
{
	if (name && name[0] == '_' && strlen (name) == 4
	...
	return !name || !*name		   ? (opt & LEO_OUT_EMPTY) != 0
		: *name == '-' && !name[1] ? (opt & LEO_OUT_MINUS) != 0
								   : true;
}
...
bool IsAssignmentAllowedLEO (ccp cur_name, ccp new_name, le_options_t opt)
{
	...
	switch (opt & LEO_M_HOW)
	{
		...
		case LEO_OVERWRITE:
			return true;
		case LEO_INSERT:
			return !IsOutputNameValidLEO (cur_name, opt);
		case LEO_REPLACE:
			return IsOutputNameValidLEO (cur_name, opt);
		default:
			return false;
...
ccp GetTextLEO (le_options_t opt)
{
	...
	static const le_options_t ctab[] = { LEO_IN_ALL, LEO_M_ARENA, LEO_M_ORIGINAL, LEO_OUT_ALL, 0 };
...
{
	static const le_options_t mask = LEO_M_BMG | LEO_IN_ALL | LEO_M_HOW;
...
{
	static const le_options_t mask = LEO_M_ARENA | LEO_M_ORIGINAL | LEO_NO_D_FILES | LEO_OUT_ALL;
	return GetTextLEO (opt & mask);
}

///////////////////////////////////////////////////////////////////////////////

bool GetBmgBySlotLEO (char *buf, uint bufsize, const bmg_t *bmg, int slot, le_options_t opt)
{
	if (!bmg || slot < 0)
		return 0;
	...
	if (opt & LEO_MKW2)
	{
		const bmg_item_t *bi = IsMkwTrack (slot) ? FindItemBMG (bmg, slot + MID_TRACK1_BEG)
			: IsMkwArena (slot) ? FindItemBMG (bmg, slot - BMG_N_TRACK + MID_ARENA1_BEG)
								: 0;
	...
	if (opt & LEO_MKW1)
	{
		const bmg_item_t *bi = IsMkwTrack (slot) ? FindItemBMG (bmg, slot + MID_TRACK2_BEG)
			: IsMkwArena (slot)					 ? FindItemBMG (bmg, slot + MID_ARENA2_BEG)
												 : 0;
...
char *ScanLSP (le_strpar_t *par, ccp arg, int arg_len, enumError *ret_err)
{
	static const KeywordTab_t keytab[]
		= { { LTT_SHA1, "SHA1", 0, 0 }, { LTT_SHA1_D, "D-SHA1", "DSHA1", 0 },
			  { LTT_IDENT, "IDENT", 0, 0 }, { LTT_IDENT_D, "D-IDENT", "DIDENT", 0 },
			  { LTT_FILE, "FILES", 0, 0 }, { LTT_PATH, "PATHS", 0, 0 }, { LTT_NAME, "NAMES", 0, 0 },
			  { LTT_XNAME, "XNAMES", 0, 0 }, { LTT_XNAME2, "X2NAMES", 0, 0 },
#if LE_STRING_LIST_ENABLED
			  { LTT_TEMP1, "TEMP1", 0, 0 }, { LTT_TEMP2, "TEMP2", 0, 0 },
#endif
			  { 0, 0, 0, 0 } };
...
#endif
	{
		...
		*nptr = 0;
		...
		const KeywordTab_t *cmd
			= ScanKeywordEx (&abbrev_count, name, nptr - name, LOUP_UPPER, keytab);
...
#if LE_STRING_SET_ENABLED
	if (IsValidSetLSP (par))
		return PrintCircBuf ("[%s],%s", par->key, GetTextLEO (par->opt & ~LEO_LTT_SELECTOR & mask));
#endif
...
{
	return lc && cup_index < lc->used ? lc->list + cup_index * lc->tracks : 0;
}
...
uint GetCupSlotLECUP (const le_cup_t *lc, const le_cup_ref_t *ptr)
{
	...
	return cup < lc->used ? 10 * cup + (idx % lc->tracks) + 11 : 0;
}
...
int HaveUsualBattleSlots (le_track_t *lt)
{
	...
	return (lt->property == lt->track_slot ? 0 : 1) | (lt->music == ai->music_id ? 0 : 2);
}
...
void SetIdentLT (le_track_t *lt, const le_strpar_t *par, ccp ident)
{
	...
	if (IsAssignmentAllowedLEO (lti->ident, ident, par->opt))
	{
		...
		if (ident)
		{
			...
			if (lti->have_sha1)
			{
				const TrackInfo_t *ti = IsMkwTrack (lt->track_slot) ? track_info + lt->track_slot
					: IsMkwArena (lt->track_slot) ? arena_info + lt->track_slot - MKW_ARENA_BEG
												  : 0;
...
const u8 *GetSha1BinLT (const le_track_t *lt, bool use_d)
{
	...
	return lt && lt->track_status >= LTS_EXPORT && lti->have_sha1 ? lti->sha1bin : 0;
}
...
ccp GetIdentLT (const le_track_t *lt, bool use_d, bool use_sha1, ccp return_on_empty)
{
	...
	return lti->ident ? (*lti->ident ? lti->ident : return_on_empty)
		: use_sha1	  ? GetSha1LT (lt, use_d, return_on_empty)
					  : return_on_empty;
}
...
{
	return lt && lt->track_status >= LTS_EXPORT && lt->file && *lt->file ? lt->file
																		 : return_on_empty;
}
...
{
	ccp path = lt && lt->track_status >= LTS_EXPORT && lt->path && *lt->path ? lt->path
																			 : return_on_empty;
			...
			if (IsOutputNameValidLEO (fname, opt))
			{
				...
				if (found)
				{
					PRINT0 ("ld=%p, tfer_mode:%x, move:%x\n", ld, tfer_mode,
						tfer_mode & (TFMD_J_MOVE | TFMD_J_MOVE1));
...
ccp GetXName2LT (const le_track_t *lt, ccp return_on_empty)
{
	...
	return IsOutputNameValidLEO (lt->xname, 0) ? lt->xname
		: IsOutputNameValidLEO (lt->name, 0)   ? lt->name
											   : return_on_empty;
}
...
ccp GetListLT (const le_track_t *lt, le_options_t opt, ccp return_on_empty)
{
	...
	if (lt && lt->track_status >= LTS_EXPORT && ltt >= LTT__LIST_BEG && ltt < LTT__LIST_END)
	{
...
{
	if (lt && lt->track_status >= LTS_EXPORT && IsValidSetLSP (par))
	{
...
ccp GetTextOptLT (const le_track_t *lt, le_options_t opt, ccp return_on_empty)
{
	...
	{
		case LTT_SHA1:
			return GetSha1LT (lt, false, return_on_empty);
		case LTT_SHA1_D:
			return GetSha1LT (lt, true, return_on_empty);
		case LTT_IDENT:
			return GetIdentLT (lt, false, false, return_on_empty);
		case LTT_IDENT_D:
			return GetIdentLT (lt, true, false, return_on_empty);

		case LTT_FILE:
			return GetFileLT (lt, return_on_empty);
		case LTT_PATH:
			return GetPathLT (lt, opt, return_on_empty);
		case LTT_NAME:
			return GetNameLT (lt, return_on_empty);
		case LTT_XNAME:
			return GetXNameLT (lt, return_on_empty);
		case LTT_XNAME2:
			return GetXName2LT (lt, return_on_empty);
...
ccp GetSlotNameLT (const le_track_t *lt)
{
	...
	return name ? name : PrintCircBuf ("%s%u", GetNameLTTY (lt->track_type), lt->track_slot);
}
...
ccp GetCupLT (const le_track_t *lt, ccp return_on_empty)
{
	...
	return PrintCircBuf ("%s%s%u.%u", hidden ? "H" : "",
		IsArenaLTTY (lt->track_type)		? "A"
			: IsRandomLTTY (lt->track_type) ? "R"
											: "",
...
ccp GetCupAlignLT (const le_track_t *lt)
{
	...
	ccp res = GetCupLT (lt, "-");
	return IsHiddenLEFL (lt->flags) ? PrintCircBuf ("%8s", res) : PrintCircBuf ("%6s  ", res);
}
...
DistribFlags_t GetDistribFlagsLT (const le_track_t *lt, bool use_d)
{
	...
	DistribFlags_t dta
		= lt->distrib_flags & ~(G_DTA_IS_D | G_DTA_TITLE | G_DTA_HIDDEN | G_DTA_ORIGINAL);
...
ccp QuoteTextOptLT (const le_track_t *lt, le_options_t opt, ccp return_on_empty)
{
	...
	{
		case LTT_SHA1:
			return QuoteSha1LT (lt, false, return_on_empty);
			break;
		case LTT_SHA1_D:
			return QuoteSha1LT (lt, true, return_on_empty);
			break;
		case LTT_IDENT:
			return QuoteIdentLT (lt, false, false, return_on_empty);
			break;
		case LTT_IDENT_D:
			return QuoteIdentLT (lt, true, false, return_on_empty);
			break;

		case LTT_NAME:
			return QuoteNameLT (lt, return_on_empty);
			break;
		case LTT_XNAME:
			return QuoteXNameLT (lt, return_on_empty);
			break;
		case LTT_XNAME2:
			return QuoteXName2LT (lt, return_on_empty);
			break;
		case LTT_FILE:
			return QuoteFileLT (lt, return_on_empty);
			break;
		case LTT_PATH:
			return QuotePathLT (lt, opt, return_on_empty);
			break;
...
le_track_t *DefineFreeTrackLD (le_distrib_t *ld, le_track_type_t ltty, bool mark_export)
{
	...
	const int slot_beg = ltty == LTTY_TRACK ? MKW_TRACK_BEG
		: ltty == LTTY_ARENA				? MKW_ARENA_BEG
		: ltty & (LTTY_TRACK | LTTY_ARENA)	? LE_FIRST_CT_SLOT
											: -1;
...
void ScanDumpLD (le_distrib_t *ld, // valid structure
...
void ScanDumpSTLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
...
)
{
	...
	for (;;)
	{
		...
		else if (!strncasecmp (sect, "DISTRIBUTION-PARAM", 18))
		{
			...
			|| !strncasecmp (sect, "DEBUG-DOCU", 10) || !strncasecmp (sect, "DEBUG-1", 7)
			|| !strncasecmp (sect, "DEBUG-2", 7) || !strncasecmp (sect, "DEBUG-3", 7)
		...
		else if (!strncasecmp (sect, "END", 3) || !strncasecmp (sect, "LECODE-INFO", 6))
		{
			// simply ignore
			st->is_section = false;
		}
...
void ScanRefLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	const void *data, // data to scan
	uint data_size // size of 'data'
)
...
void ScanRefSTLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	ScanText_t *st // initialized source
)
{
	...
	while (NextLineScanText (st))
	{
		...
		if (n > IDX_SHA1)
			SetIdentOptLT (lt, xstring (buf, sizeof (buf), src[IDX_SHA1]), 0, 1, ld->spar.opt);
		if (n > IDX_IDENT)
			SetIdentOptLT (lt, xstring (buf, sizeof (buf), src[IDX_IDENT]), 0, 0, ld->spar.opt);
		if (n > IDX_SHA1_D)
			SetIdentOptLT (lt, xstring (buf, sizeof (buf), src[IDX_SHA1_D]), 1, 1, ld->spar.opt);
		if (n > IDX_IDENT_D)
			SetIdentOptLT (lt, xstring (buf, sizeof (buf), src[IDX_IDENT_D]), 1, 0, ld->spar.opt);
		if (n > IDX_FILE)
			SetFileLT (lt, &ld->spar, xstring (buf, sizeof (buf), src[IDX_FILE]));
		if (n > IDX_PATH)
			SetPathLT (lt, &ld->spar, xstring (buf, sizeof (buf), src[IDX_PATH]));
		if (n > IDX_NAME)
			SetNameLT (lt, &ld->spar, xstring (buf, sizeof (buf), src[IDX_NAME]));
		if (n > IDX_XNAME)
			SetXNameLT (lt, &ld->spar, xstring (buf, sizeof (buf), src[IDX_XNAME]));
...
void ScanStrLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	const void *data, // data to scan
	uint data_size // size of 'data'
)
...
void ScanStrSTLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	ScanText_t *st // initialized source
)
{
...
void ScanSha1LD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	const void *data, // data to scan
	uint data_size // size of 'data'
)
...
void ScanSha1STLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	ScanText_t *st // initialized source
)
{
...
bool ScanSha1LineLD (le_distrib_t *ld, // valid structure
...
static const KeywordTab_t *scan_cmd (ScanInfo_t *si, // valid structure
...
)
{
	...
	if (!ScanNameSI (si, buf, sizeof (buf), true, true, 0))
	{
		if (!si->no_warn)
		{
			...
			DASSERT (sf);
			ERROR0 (ERR_WARNING, "File %s, section [%s], line #%u:\nMissing command: %.*s\n",
				sf->name, section, sf->line, (int)(eol - sf->prev_ptr), sf->prev_ptr);
		}
...
static enumError scan_track (le_track_t *lt, ScanInfo_t *si)
{
	...
	PRINT0 ("TRACK %s: t=%d, p=%d, m=%d, f:%x\n", last_index_name, lt->track_type, lt->property,
		lt->music, lt->flags);
...
static enumError ScanLeDefTRACKS (le_distrib_t *ld, // valid structure
...
)
{
	...
	static const KeywordTab_t commands[] = { { C_SLOT, "SLOT", 0, 0 },
		{ C_IGNORE_SLOT, "IGNORE-SLOT", "IGNORESLOT", 0 }, { C_TRACK, "TRACK", 0, 0 },

		{ C_LAPS, "LAPS", "LAP", O_CURRENT }, { C_SPEED, "SPEED", 0, O_CURRENT },

		{ C_IDENT, "IDENT", 0, O_STRING }, { C_FILE, "FILE", 0, O_STRING },
		{ C_PATH, "PATH", 0, O_STRING }, { C_NAME, "NAME", 0, O_STRING },
		{ C_XNAME, "XNAME", 0, O_STRING },

#if LE_STRING_LIST_ENABLED
		{ C_LIST, "TEMP1", 0, O_STRING | LTT_TEMP1 }, { C_LIST, "TEMP2", 0, O_STRING | LTT_TEMP2 },
#endif

		{ C_STD_ARENAS, "STANDARD-BATTLE-ARENAS", "STANDARDBATTLEARENAS", 0 },
		{ C_STD_VERSUS, "STANDARD-VERSUS-TRACKS", "STANDARDVERSUSTRACKS", 0 },
	...
	for (;;)
	{
		...
		if (!current_lt && opt & (O_CURRENT | O_STRING))
		{
			if (!si->no_warn)
			{
				...
				ERROR0 (ERR_WARNING,
					"File %s, section [TRACK-LIST], line #%u:\nNo current track: %.*s\n", sf->name,
					sf->line, (int)(eol - sf->prev_ptr), sf->prev_ptr);
			}
		...
		switch (cmd->id)
		{
			...
			case C_TRACK:
			{
				...
				if (!scan_track (&temp, si))
				{
					...
					le_track_t *lt = next_slot >= 0 && !ignore_slot
						? DefineTrackLD (ld, next_slot, true) // SLOT has highest priority
						: IsRandomLEFL (temp.flags) ? DefineGroupTrackLD (ld, temp.track_type,
														  true) // append random tracks always
													: DefineFreeTrackLD (ld, temp.track_type,
														  true); // fallback: search a free slot
					get_track_nsec += GetTimerNSec ();
	...
	if (log_timing >= 1)
	{
		...
		fprintf (stdlog,
			...
			verbose >= 1 ? "   > " : "", count, PrintTimerNSec6 (0, 0, total_nsec, 0),
...
static void add_reflist_to_cup (
	le_distrib_t *ld, const le_cup_ref_t *slot_list, int n_slots, le_track_type_t force_ltty)
{
...
static enumError ScanLeDefCUPS (le_distrib_t *ld, // valid structure
	ScanInfo_t *si, // valid structure
	int pass // 1|2
)
{
	...
	static const KeywordTab_t commands[]
		= { { C_APPEND, "APPEND", 0, 0 }, { C_APPEND_BT, "APPEND-BT", "APPEND-BT", 0 },
			  { C_APPEND_VS, "APPEND-VS", "APPEND-VS", 0 }, { C_NEW_CUP, "NEW-CUP", "NEWCUP", 0 },
...
enumError ScanLeDefLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	const void *data, // data to scan
	uint data_size, // size of 'data'
...
enumError ScanLeDefSTLD (le_distrib_t *ld, // valid structure
	bool init_ld, // TRUE: initialze 'ld'
	ScanText_t *st // initialized source
)
{
	...
	static const KeywordTab_t sections[]
		= { { SECT_TOOL, "TOOL", 0, 1 }, { SECT_TRACKS, "TRACK-LIST", "TRACKLIST", 1 },
			  { SECT_CUPS, "CUP-LIST", "CUPLIST", 2 }, { SECT_END, "END", 0, 0 }, { 0, 0, 0, 0 } };
	...
	for (int pass = 1; pass <= 2; pass++)
	{
		...
		for (;;)
		{
			...
			{
				case SECT_TRACKS:
					err = ScanLeDefTRACKS (ld, &si, pass);
					break;
				case SECT_CUPS:
					err = ScanLeDefCUPS (ld, &si, pass);
					break;
...
static void update_cup_helper (le_distrib_t *ld, // valid distrib
...
)
{
	DASSERT (ld);
	DASSERT (lc);
	PRINT ("update_cup_helper(%d,fb=%d,add=%d) #%u: used=%d, dirty=%d\n", lc->tracks, p_fallback,
		add_missed, __LINE__, lc->used, lc->dirty);
...
void UpdateCupsLD (le_distrib_t *ld)
{
	...
	CloseArchLD (ld);
	if (ld->cup_battle.used && !ld->cup_battle.dirty && ld->cup_versus.used
		&& !ld->cup_versus.dirty)
		return;
	...
	update_cup_helper (
		ld, &ld->cup_battle, LTTY_ARENA, MKW_ARENA_BEG, !ld->scan.valid || ld->scan.bt.add_unused);
	...
	update_cup_helper (
		ld, &ld->cup_versus, LTTY_TRACK, MKW_TRACK_BEG, !ld->scan.valid || ld->scan.vs.add_unused);
	...
	if (log_timing >= 1)
	{
		...
		fprintf (stdlog, "%sUpdateCupsLD() N=%d,%d, done in %s\n", verbose >= 1 ? "   > " : "",
			ld->cup_battle.used, ld->cup_versus.used, PrintTimerNSec6 (0, 0, dur_nsec, 0));
	}
}
...
static void ShowProgressTracksLD (le_distrib_t *ld, bool force)
{
	...
	if (ld->show_progress > 0 && ld->progress_count > 0)
	{
			...
			|| force && now >= ld->last_progress_msec + 1000)
		{
			...
			fprintf (stdlog, "%s%u file%s scanned in %llu seconds.\n",
				verbose >= 1 ? "   > " : "   - ", ld->progress_count,
				ld->progress_count == 1 ? "" : "s", (now - ld->first_progress_msec) / 1000);
			fflush (stdlog);
		}
	}
}
...
enumError AddToArchLD (le_distrib_t *ld, raw_data_t *raw)
{
	...
	PRINT0 ("SHA1: %s, slots: r=%d a=%d m=%d\n", as.sha1_szs, as.slotinfo.race_slot,
		as.slotinfo.arena_slot, as.slotinfo.music_index);
	...
	if (as.valid_track && !spf.f_d.len)
	{
		...
		ta->lt.music = as.slotinfo.music_index ? as.slotinfo.music_index
...
bool CloseArchLD (le_distrib_t *ld)
{
	...
	for (; ek < gend; ek++)
	{
		...
		{
			int index = gi->list[i];
			PRINT0 ("A: %zd/%d : %d/%d/%d : %d/%d/%d\n", ek - ld->group.list, ld->group.used, i,
				gi->used, gi->size, index, ld->arch_used, ld->arch_size);
	...
	*ptr = 0;
	...
	for (ptr = order; *ptr; ptr++)
	{
		...
		le_track_t *lt = ta->force_slot >= 0 ? DefineTrackLD (ld, ta->force_slot, false)
			: IsRandomLEFL (ta->lt.flags)	 ? DefineGroupTrackLD (ld, ta->lt.track_type, true)
				...
				if (gi->group == ta->group)
				{
					for (int i = 0; i < gi->used; i++)
					{
						...
						PRINT0 ("B: %zd/%d : %d/%d/%d : %d/%d/%d\n", ek - ld->group.list,
							ld->group.used, i, gi->used, gi->size, index, ld->arch_used,
							ld->arch_size);
						ASSERT (index < ld->arch_used);
...
void ImportAnaLD (le_distrib_t *ld, const le_analyze_t *ana)
{
	...
	for (int slot = 0; slot < ana->n_slot; slot++)
	{
		...
		{
			if (ana->property)
				lt->property = ana->property[slot];
			if (ana->music)
				lt->music = ana->music[slot];
			if (ana->flags)
				lt->flags = ana->flags[slot];
...
enumError ImportRawDataLD (le_distrib_t *ld, // destination
...
static enumError search_func (mem_t path, // full path of existing file, never NULL
...
enumError ImportFileLD (le_distrib_t *ld, // destination
	...
	if (ld && fname)
	{
		...
		else
		{
			...
			if (use_wildcard && HaveWildcards (MemByString (fname)) && stat (fname, &st))
			{
				...
				ClearProgressLD (ld);
				return param.count ? param.max_err
...
static void PrintSectionTool (FILE *f, bool be_verbose)
{
	...
	if (be_verbose)
		fprintf (f, text_ledis_tool_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);
	else
		fprintf (f,
			"\r\n[TOOL]\r\n"
			"TOOL     = %s\r\n"
...
static bool check_filter (const le_track_t *lt, le_options_t opt)
{
	...
	if (a == LEO_VERSUS && !IsTrackLTTY (lt->track_type)
		|| a == LEO_BATTLE && !IsArenaLTTY (lt->track_type))
		return false;
...
{
	return lt->track_status >= LTS_EXPORT && check_filter (lt, opt);
}
...
enumError CreateDumpLD (FILE *f, le_distrib_t *ld)
{
	...
	fprintf (f,
		...
		"DATE     = %s\r\n"
		"\r\n",
		LE_DISTRIB_MAGIC8, section_sep, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);
...
enumError CreateRefLD (FILE *f, le_distrib_t *ld, bool add_strings)
{
	...
	fprintf (f, "%s : LE-CODE track reference v1.%u\r\n@VERSION=%u\r\n", LE_REFERENCE_MAGIC8,
		LE_REFERENCE_VERSION, LE_REFERENCE_VERSION);
	...
	fprintf (f,
		"@CONTENT=track_slot|type|orig|cup_slot|cup_slot_name|property|music|flags|hidden|laps|"
		"speed|sha1|sha1_d|%s\r\n"
		"#-----\r\n",
	...
	for (le_track_t **plt = ld->reflist; *plt; plt++)
	{
		...
		fprintf (f, "%u|%s|%u|%u|%s|%u|%u|%u|%u|%u|%5.3f|%s|%s|", lt->track_slot,
			GetNameLTTY (lt->track_type), lt->is_original, lt->cup_slot, GetCupLT (lt, ""),
			lt->property, lt->music, lt->flags, IsHiddenLEFL (lt->flags), lt->lap_count,
			lt->speed_factor, GetSha1LT (lt, 0, ""), no_d ? "" : GetSha1LT (lt, 1, ""));
		...
		if (add_strings)
		{
			...
			fprintf (f, "%s|%s|%s||%s||%s||%s||\r\n", id1.data.ptr, id2.data.ptr, file.data.ptr,
				name.data.ptr, xname.data.ptr, path.data.ptr);
...
enumError CreateStringsLD (FILE *f, le_distrib_t *ld)
{
	...
	fprintf (f, "%s : LE-CODE track strings v1.%u\r\n@VERSION=%u\r\n", LE_STRINGS_MAGIC8,
		LE_STRINGS_VERSION, LE_STRINGS_VERSION);
	...
	for (le_track_t **plt = ld->reflist; *plt; plt++)
	{
		...
		for (int ltt = 0; ltt < LTT__N; ltt++)
		{
			...
			if (ptr && *ptr)
			{
				...
				fprintf (f, "%u|%u|0|%s|%s|\r\n", lt->track_slot, GetDModeLTT (ltt),
					GetLowerNameLTT (ltt), text.data.ptr);
				FreeExMem (&text);
			}
		}
		...
		for (ParamFieldItem_t *ptr = lt->str_set.field; ptr < end; ptr++)
		{
			...
			fprintf (f, "%u|3|1|%s|%s|\r\n", lt->track_slot, name.data.ptr, text.data.ptr);
			FreeExMem (&name);
			FreeExMem (&text);
		}
#endif
...
enumError CreateInfoLD (FILE *f, le_distrib_t *ld, bool use_xname, bool add_rating)
{
	...
	for (le_track_t **plt = ld->reflist; *plt; plt++)
	{
		...
		exmem_t esc = EscapeStringEx (name, -1, EmptyString, EmptyString, CHMD__MODERN, 0, false);
		fprintf (f, "%s%s %s\t%s\t%s\t%s\r\n", add_rating ? "()\t" : "", GetCupAlignLT (lt),
			PrintPropertyID (lt->property, false), PrintMusicID (lt->music, false),
			PrintLEFL16 (lt->flags, true), esc.data.ptr);
		FreeExMem (&esc);
	}
...
enumError CreateSha1LD (FILE *f, le_distrib_t *ld, bool use_xname, bool add_comments)
{
	...
	if (add_comments)
	{
		fprintf (f,
			...
			LE_SHA1REF_MAGIC8, LE_SHA1REF_VERSION);
	...
	for (le_track_t **plt = ld->reflist; *plt; plt++)
	{
		...
		for (int is_d = 0; is_d <= max; is_d++)
		{
			...
			if (lti->have_sha1)
			{
				...
				exmem_t esc
					= EscapeStringEx (name, -1, EmptyString, EmptyString, CHMD__MODERN, 0, false);

				fprintf (f, "%s %s %s %s %4d %s  %s\r\n", GetNameLTTY (lt->track_type),
					PrintDTA (GetDistribFlagsLT (lt, is_d), true), PrintLEFL16 (lt->flags, true),
					hex, lt->track_slot, GetCupAlignLT (lt), esc.data.ptr);
				FreeExMem (&esc);
			}
		}
	}
...
enumError CreateCtDefLD (FILE *f, le_distrib_t *ld)
{
	...
	for (int slot = BMG_LE_FIRST_CTRACK; slot < ld->tlist_used; slot++)
	{
		...
		fprintf (f, "%c %s\t; %s\t; 0x%02x\t; %s; %s; %s; %s\r\n", type,
			PrintMusicID (lt->music, false), PrintPropertyID (lt->property, false), lt->flags,
			QuoteFileLT (lt, ""), QuoteNameLT (lt, ""), QuoteIdentLT (lt, false, true, ""),
			QuoteXNameLT (lt, ""));
	}
	...
	for (int slot = MKW_ARENA_BEG; slot < MKW_ARENA_END; slot++)
	{
		...
		fprintf (f, "%s\t%s\t%s\t# A%u\r\n", PrintPropertyID (lt->track_slot, false),
			PrintPropertyID (lt->property, false), PrintMusicID (lt->music, false), lt->cup_slot);
	}
	if (count)
		fputs ("\r\n", f);
...
static void print_ledef_text (FILE *f, const le_distrib_t *ld, int max_len, ccp name, ccp text)
{
	DASSERT (f);
	DASSERT (ld);
	DASSERT (name);
...
static void print_ledef_track (FILE *f, const le_distrib_t *ld, const le_track_t *lt)
{
	DASSERT (f);
	DASSERT (ld);
	DASSERT (lt);
	...
	*dest = 0;
	...
	fprintf (f, "TRACK %s%u\t%s\t%s\t%s\t\"%s\"%s%s\r\n", type, lt->track_slot, type,
		PrintPropertyID (lt->property, false), PrintMusicID (lt->music, false),
		PrintLEFL16 (lt->flags, true), *comment ? "\t#" : "", *comment ? comment + 1 : "");
...
enumError CreateBmgLD (FILE *f, le_distrib_t *ld, bool bmg_text)
{
	...
	for (int slot = 0; slot < ld->tlist_used; slot++)
	{
		...
		if (source & LEO_MKW1)
		{
			const u32 mid = IsMkwTrack (slot) ? slot + MID_TRACK1_BEG
				: IsMkwArena (slot)			  ? slot - BMG_N_TRACK + MID_ARENA1_BEG
											  : 0;
		...
		if (source & LEO_MKW2)
		{
			const u32 mid = IsMkwTrack (slot) ? slot + MID_TRACK2_BEG
				: IsMkwArena (slot)			  ? slot - BMG_N_TRACK + MID_ARENA2_BEG
											  : 0;
	...
	enumError err = bmg_text || isatty (fileno (f))
		? SaveTextFileBMG (
			  &bmg, f, 0, long_count > 0, brief_count > 0 ? brief_count : !print_header)
...
enumError CreateCupIconsLD (ld_out_param_t *lop, mem_t mem_opt, bool print_info)
{
	...
	static const KeywordTab_t keytab[] = { { O_ORIG, "ORIGINAL", 0, 0 },
		{ O_SWAPPED, "SWAPPED", 0, 0 }, { O_1WIIMM, "1WIIMM", 0, 0 },
		{ O_9WIIMM, "9WIIMM", "WIIMM", 0 }, { O_XWIIMM, "XWIIMM", 0, 0 },

		{ O_PLUS, "PLUS", 0, 0 }, { O_XPLUS, "XPLUS", 0, 0 }, { O_GAME, "GAME", 0, 0 },
		{ O_XGAME, "XGAME", 0, 0 }, { O_SPACE, "SPACE", 0, 0 }, { O_TEST, "TEST", 0, 0 },

		{ 0 * O_CHARS, "0", 0, O_M_CHARS }, { 1 * O_CHARS, "1", 0, O_M_CHARS },
		{ 2 * O_CHARS, "2", 0, O_M_CHARS }, { 3 * O_CHARS, "3", 0, O_M_CHARS },
		{ 4 * O_CHARS, "4", 0, O_M_CHARS }, { 5 * O_CHARS, "5", 0, O_M_CHARS },
		{ 6 * O_CHARS, "6", 0, O_M_CHARS }, { 7 * O_CHARS, "7", 0, O_M_CHARS },
		{ 8 * O_CHARS, "8", 0, O_M_CHARS }, { 9 * O_CHARS, "9", 0, O_M_CHARS },
		{ 10 * O_CHARS, "10", 0, O_M_CHARS }, { 11 * O_CHARS, "11", 0, O_M_CHARS },
		{ 12 * O_CHARS, "12", 0, O_M_CHARS }, { 13 * O_CHARS, "13", 0, O_M_CHARS },
		{ 14 * O_CHARS, "14", 0, O_M_CHARS }, { 15 * O_CHARS, "15", 0, O_M_CHARS },

		{ 1 * O_PIXEL, "8PX", 0, O_M_PIXEL }, { 2 * O_PIXEL, "16PX", 0, O_M_PIXEL },
		{ 3 * O_PIXEL, "24PX", 0, O_M_PIXEL }, { 4 * O_PIXEL, "32PX", 0, O_M_PIXEL },
		{ 5 * O_PIXEL, "40PX", 0, O_M_PIXEL }, { 6 * O_PIXEL, "48PX", 0, O_M_PIXEL },
		{ 7 * O_PIXEL, "56PX", 0, O_M_PIXEL }, { 8 * O_PIXEL, "64PX", 0, O_M_PIXEL },
		{ 9 * O_PIXEL, "72PX", 0, O_M_PIXEL }, { 10 * O_PIXEL, "80PX", 0, O_M_PIXEL },
		{ 11 * O_PIXEL, "88PX", 0, O_M_PIXEL }, { 12 * O_PIXEL, "96PX", 0, O_M_PIXEL },
		{ 13 * O_PIXEL, "104PX", 0, O_M_PIXEL }, { 14 * O_PIXEL, "112PX", 0, O_M_PIXEL },
		{ 15 * O_PIXEL, "120PX", 0, O_M_PIXEL }, { 16 * O_PIXEL, "128PX", 0, O_M_PIXEL },
		{ 17 * O_PIXEL, "136PX", 0, O_M_PIXEL }, { 18 * O_PIXEL, "144PX", 0, O_M_PIXEL },
		{ 19 * O_PIXEL, "152PX", 0, O_M_PIXEL }, { 20 * O_PIXEL, "160PX", 0, O_M_PIXEL },
		{ 21 * O_PIXEL, "168PX", 0, O_M_PIXEL }, { 22 * O_PIXEL, "176PX", 0, O_M_PIXEL },
		{ 23 * O_PIXEL, "184PX", 0, O_M_PIXEL }, { 24 * O_PIXEL, "192PX", 0, O_M_PIXEL },
		{ 25 * O_PIXEL, "200PX", 0, O_M_PIXEL }, { 26 * O_PIXEL, "208PX", 0, O_M_PIXEL },
		{ 27 * O_PIXEL, "216PX", 0, O_M_PIXEL }, { 28 * O_PIXEL, "224PX", 0, O_M_PIXEL },
		{ 29 * O_PIXEL, "232PX", 0, O_M_PIXEL }, { 30 * O_PIXEL, "240PX", 0, O_M_PIXEL },
			...
			if (text && *text)
			{
				...
				if (spf.name.len && dest < bufend)
				{
					...
					if (use_plus_prefix && spf.plus.len)
						ndest = StringCat2LE (
							ndest, nameend, spf.plus.ptr, spf.plus.len, append_plus, 1);
					if (use_game_prefix && spf.game.len)
						ndest = StringCat2LE (
							ndest, nameend, spf.game.ptr, spf.game.len, append_game, 1);
					ndest = StringCopyE (ndest, nameend, spf.name.ptr);
					if (ndest > namebuf + max_chars)
						ndest = namebuf + max_chars;
					while (ndest > namebuf
						&& (ndest[-1] == ' ' || ndest[-1] == '\t' || ndest[-1] == '_'))
						*--ndest = 0;
					*ndest = 0;
...
{
	fprintf (f, "%6u (0x%x) cups used, %u (0x%x) alloced (%s)\r\n", lc->used, lc->used, lc->size,
		lc->size, PrintSize1000 (0, 0, lc->size * sizeof (*lc->list) * lc->tracks, 0));
...
enumError CreateDebugLD (FILE *f, const le_distrib_t *ld)
{
	...
	fprintf (f,
		...
		"  Current options: %s\r\n",
		ld->auto_setup, GetOptionsLSP (&ld->spar, LEO__ALL));

	//--- tracks

	fprintf (f, "\r\n%sTracks:%s\r\n", colset->caption, colset->reset);
	fprintf (f, "  %u (0x%x) tracks used, %u (0x%x) alloced (%s)\r\n", ld->tlist_used,
		ld->tlist_used, ld->tlist_size, ld->tlist_size,
	...
	for (int slot = 0; slot < ld->tlist_used; slot++, lt++)
	{
		...
		if (lt->track_type & LTTY_TRACK)
			n_track++;
		if (lt->track_type & LTTY_RANDOM)
			n_random++;

		if (lt->lti[0].have_sha1)
			n_sha1++;
		if (lt->lti[1].have_sha1)
			n_sha1_d++;
	...
	if (n_arena)
		fprintf (f, "%8u of type BATTLE ARENA\r\n", n_arena);
	if (n_track)
		fprintf (f, "%8u of type VERSUS TRACK\r\n", n_track);
	if (n_random)
		fprintf (f, "%8u of type RANDOM\r\n", n_random);

	fprintf (f, "%s  String usage:%s\r\n", colset->heading, colset->reset);
	if (n_sha1)
		fprintf (
			f, "%8u string%c of type SHA1   (not alloced)\r\n", n_sha1, n_sha1 == 1 ? ' ' : 's');
	if (n_sha1_d)
	...
	for (int ltt = 0; ltt < LTT__N; ltt++)
	{
		if (n_text[ltt])
		{
			...
			fprintf (f, "%8u string%c of type %-8s %s\r\n", n_text[ltt],
				n_text[ltt] == 1 ? ' ' : 's', type,
				PrintSize1000 (0, 0, size_text[ltt], DC_SFORM_ALIGN));
		}
	}
	...
	if (str_count.used)
	{
		...
		{
			count_t *cnt = (count_t *)ek->data.data.ptr;
			fprintf (f, "%8u, %s : %s\r\n", cnt->n, PrintSize1000 (0, 0, cnt->size, DC_SFORM_ALIGN),
				ek->key);
		}
	}
#endif // LE_STRING_SET_ENABLED
...
static void report_counters (FILE *f, const report_counter_t *cnt, ccp type)
{
	...
	if (cnt->visible && cnt->hidden)
		fprintf (f,
			...
			"%8u visible %s%s\r\n"
			"%8u hidden %s%s\r\n",
			total, type, total == 1 ? "" : "s", cnt->visible, type, cnt->visible == 1 ? "" : "s",
...
static void report_duplicates (
	ld_out_param_t *lop, int idx, ccp singular, ccp plural, int dup_count, u16 *list, int n_list)
{
	if (!dup_count)
		return;

	fprintf (lop->f,
		"\r\n\r\n%s%u %s that have been used two or more times "
		"(tid,fid,cid,flags,slot,cup,name):%s\r\n",
		lop->colset->caption, dup_count, dup_count == 1 ? singular : plural, lop->colset->reset);
		...
		for (int slot = 0; slot < lop->ld->tlist_used; slot++, lt++)
		{
			...
			if (sha1)
			{
				...
				if (ref && ref->ref[idx] == il)
					fprintf (lop->f, "%7u %5u %5u  %s %4u %s %s\r\n", ref->track, ref->family,
						ref->clan, PrintLEFL16 (lt->flags, true), lt->track_slot,
						GetCupAlignLT (lt), GetXName2LT (lt, "-"));
			}
		}
	}
}
...
enumError CreateReportLD (ld_out_param_t *lop, mem_t mem_opt)
{
	...
	static const KeywordTab_t keytab[]
		= { { O_COUNTERS, "COUNTERS", 0, 0 }, { O_CLASSES, "CLASSES", 0, 0 },
			  { O_XCLASSES, "XCLASSES", 0, 0 }, { O_NAMES, "NAMES", 0, 0 },
			  { O_TRACKS, "TRACKS", 0, 0 }, { O_FAMILIES, "FAMILIES", "FAMILY", 0 },
			  { O_CLANS, "CLANS", 0, 0 }, { O_M_DUPLICATES, "DUPLICATES", 0, O_M_DUPLICATES },
			  { O_DEFAULT, "DEFAULT", 0, O_DEFAULT }, { O_ALL, "ALL", 0, O_ALL }, { 0, 0, 0, 0 } };
	...
	if (opt & O_COUNTERS)
	{
		...
		for (int slot = 0; slot < n_tracks; slot++, lt++)
		{
			...
			{
				case G_LEFL_RND_HEAD:
					cnt->g_header++;
					break;
				case G_LEFL_RND_GROUP:
					cnt->g_member++;
					break;
				case G_LEFL__RND:
					cnt->g_both++;
			...
			if (lt->flags & G_LEFL_NEW)
				cnt->le_new++;
			if (lt->flags & G_LEFL_TEXTURE)
				cnt->le_texture++;
	...
	if (opt & (O_CLASSES | O_XCLASSES))
	{
		...
		for (int ci = 0; ci < G_TCLSI__N; ci++)
		{
			...
			fprintf (lop->f, "\r\n\r\n%s%u track%s of class %s%s\r\n\r\n", lop->colset->caption, n,
				n == 1 ? "" : "s", GetTrackClassName (ci, 0), lop->colset->reset);
	...
	if (opt & O_NAMES)
	{
		...
		if (n)
		{
			fprintf (lop->f,
				...
				lop->colset->caption, n, n == 1 ? "" : "s", lop->colset->reset);
	...
	if (opt & O_M_DUPLICATES)
	{
		...
		if (opt & O_TRACKS)
			report_duplicates (
				lop, 0, "track", "tracks", have_dup_track, track_count, s1ref_max1_track);
		if (opt & O_FAMILIES)
			report_duplicates (
				lop, 1, "family", "families", have_dup_family, family_count, s1ref_max1_family);
		if (opt & O_CLANS)
...
int patch_ld (szs_iterator_t *it, // iterator struct with all infos
...
enumError PatchLD (le_distrib_t *ld, mem_t mem_opt, StringField_t *filelist)
{
	...
	static const KeywordTab_t keytab[] = { { O_MENU, "MENUS", 0, 0 },
		{ O_DEFAULT, "DEFAULT", 0, O_DEFAULT }, { O_ALL, "ALL", 0, O_ALL },
	...
	for (; ptr < end; ptr++)
	{
		...
		if (verbose >= 1 || testmode)
		{
			...
			if (dest_is_source)
				fprintf (stdlog, "  > %satch %s:%s\n", testmode ? "Would p" : "P", ff_name, dest);
			else
				fprintf (stdlog, "  > %satch %s:%s -> %s:%s\n", testmode ? "Would p" : "P", ff_name,
					raw.fname, ff_name, dest);
			fflush (stdlog);
		}
...
enumError CopyLD (le_distrib_t *ld, const le_strpar_t *dest, const le_strpar_t *src, int n_src)
{
...
enumError SplitLD (le_distrib_t *ld, const le_strpar_t *dest, const le_strpar_t *src, ccp arg)
{
	if (!ld || !dest || !src || !arg)
		return ERR_MISSING_PARAM;
...
bool SortTracksLD (le_distrib_t *ld, const le_strpar_t *src, mem_t mem_opt)
{
	...
	static const KeywordTab_t keytab[]
		= { { STO_NONE, "NONE", 0, STO_M_JOBS }, { STO_NAME, "NAME", 0, STO_M_JOBS },
			  { STO_ID, "ID", 0, STO_M_JOBS }, { STO_SHA1, "SHA1", 0, STO_M_JOBS },

			  { STO_KEEP_VS, "KEEP-VS", "KEEPVS", 0 }, { STO_KEEP_BT, "KEEP-BT", "KEEPBT", 0 },
...
bool SortCupsLD (le_distrib_t *ld, const le_strpar_t *src, mem_t mem_opt)
{
	...
	static const KeywordTab_t keytab[] = { { SCO_VS_ORIG, "VS-ORIGINAL", "VSORIGINAL", 0 },
		{ SCO_VS_CUSTOM, "VS-CUSTOM", "VSCUSTOM", 0 }, { SCO_VS_ALL, "VS-ALL", "VSALL", 0 },

		{ SCO_BT_ORIG, "BT-ORIGINAL", "BTORIGINAL", 0 },
		{ SCO_BT_CUSTOM, "BT-CUSTOM", "BTCUSTOM", 0 }, { SCO_BT_ALL, "BT-ALL", "BTALL", 0 },

		{ SCO_USE_PLUS, "PLUS", 0, 0 }, { SCO_USE_GAME, "GAME", 0, 0 },
...
bool AddDistribParamLD (le_distrib_t *ld, ccp src, ccp end)
{
	...
	if (name > name_buf)
	{
		...
		ReplaceParamField (
			&ld->dis_param, MEMDUP (name_buf, name - name_buf), true, 0, MEMDUP (src, end - src));
	}
	return true;
}
...
enumError CreateDistribLD (FILE *f, le_distrib_t *ld, bool use_xname)
{
	...
	fprintf (f, text_distrib_head_cr
		...
		GetParamFieldStr (&ld->dis_param, 1, "INFO-URL", "")
			...
			,
		uuid, GetParamFieldIntMM (&ld->dis_param, 1, "DISPLAY-MODE", 3, 0, 3),
		...
		GetParamFieldStr (&ld->dis_param, 1, "NOTE-FOR-WIMMM", "")
			...
			,
		uuid, VERSION, REVISION_NUM, GetParamFieldStr (&ld->dis_param, 1, "FIRST-CREATION", ""),
...
static void progress_lta (lta_manager_t *lm, bool force)
{
	...
	if (lm->show_progress > 0 && lm->base_slot >= 0)
	{
			...
			|| force && now >= lm->last_log_msec + 1000)
		{
			...
			fprintf (stdlog, "%s%u node%s with %u track%s processed in %llu seconds.\n",
				verbose >= 1 ? "   > " : "   - ", lm->progress_nodes,
				lm->progress_nodes == 1 ? "" : "s", lm->progress_tracks,
				lm->progress_tracks == 1 ? "" : "s", (now - lm->start_msec) / 1000);
			fflush (stdlog);
		}
	}
}
...
static enumError close_lta (lta_manager_t *lm)
{
	...
	if (lm->base_slot >= 0 && lm->F.f)
	{
		...
		if (verbose >= 0)
		{
			...
			*d_buf = *lfl_buf = *conv_buf = *cache_buf = *dup_buf = 0;
			...
			if (lm->lfl_dup_count)
				snprintf (
					dup_buf, sizeof (dup_buf), ", %u+%u dup", lm->szs_dup_count, lm->lfl_dup_count);
			else if (lm->szs_dup_count)
				snprintf (dup_buf, sizeof (dup_buf), ", %u dup", lm->szs_dup_count);

			fprintf (stdlog, "%sCreated LTA:%s (%s), slots %u..%u (%u std%s%s%s%s%s), %s\n",
				verbose >= 1 ? "   > " : "   - ", lm->F.fname,
				PrintSize1000 (0, 0, lm->data_offset, 0), lm->base_slot, lm->last_slot,
	...
	lm->start_msec = lm->last_log_msec = GetTimerMSec ();
...
static void load_file_lta (lta_manager_t *lm, // lta manager
...
)
{
	...
	if (!LoadFILE (fname, 0, 0, fdata, fsize, 0, 0, false))
	{
		...
		PRINT0 ("data=%d,%zu, cdata=data=%d,%zu, ff=%s\n", szs.data != 0, szs.size, szs.cdata != 0,
			szs.csize, GetNameFF_SZS (&szs));
...
enumError CreateTrackArchivesLD (le_distrib_t *ld, ccp destdir, mem_t mem_opt)
{
	...
	static const KeywordTab_t keytab[] = { { O_YAZ, "YAZ0", "SZS", O_M_METHOD },
		{ O_BZ, "WBZ", "BZ", O_M_METHOD }, { O_YBZ, "YBZ", 0, O_M_METHOD },
		{ O_BZIP2, "BZIP2", "BZ2", O_M_METHOD }, { O_LZ, "WLZ", "LZ", O_M_METHOD },
		{ O_YLZ, "YLZ", 0, O_M_METHOD }, { O_LZMA, "LZMA", 0, O_M_METHOD },
		{ O_RM, "REMOVE", "RM", 0 }, { O_EXT, "EXT", 0, 0 }, { O_REDIR, "REDIR", 0, 0 },
	...
	restart:;
		...
		if (lm.base_slot < 0)
		{
			...
			PRINT0 ("last_slot=%d, distrib_last_slot=%d => max_slots=%d\n", lm->last_slot,
				lm->distrib_last_slot, max_slots);
		...
		const uint node_size
			= ALIGN32 ((slot - lm.base_slot + 1) * sizeof (lta_node_t), opt_align_lta);
...
void InitializeDistributionInfo (DistributionInfo_t *dinf, bool add_default_param)
{
	DASSERT (dinf);
...
char *ScanSlot (uint *res_slot, ccp source, bool need_point)
{
	...
	*res_slot = 0;
	...
	if (!need_point && slot >= MIN_DISTRIBUTION_SLOT && slot < MAX_DISTRIBUTION_TRACK
		&& (uchar)*end <= ' ' && (end - source == 2 || end - source == 3))
	{
		*res_slot = slot;
		return end;
	}
...
void NormalizeSlotTranslation (
	char *buf, uint bufsize, ccp name, char **p_first_para, char **p_first_brack)
{
	DASSERT (buf);
	DASSERT (bufsize > 10);
...
uint DefineSlotTranslation (ParamField_t *translate, bool is_arena, uint slot, ccp name)
{
	DASSERT (translate);
	if (!name)
		return 0;
...
void ScanSlotTranslation (ParamField_t *translate, le_distrib_t *ld, ccp fname, bool ignore)
{
	DASSERT (translate);
	DASSERT (fname);
