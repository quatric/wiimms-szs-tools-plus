...
#undef HSF_EMIT
	...
	else
	{
		...
		// HSF stores vertex positions and normals in per-bone local space.
		// Standard 3D formats (glTF / COLLADA) require mesh vertices to be in
		// bind-pose model space so that IBM * JointWorldMatrix evaluates to
		// identity at rest and animates relative to rest pose.
		if (model.joints && model.num_joints)
		{
			for (uint m = 0; m < out_n; m++)
			{
				mesh_t *mesh = meshes + m;
				bool had_skin = false;
				for (size_t p = 0; p < mesh->num_positions; p++)
				{
					if (mesh->position_node && model.node_influences)
					{
						int ni = mesh->position_node[p];
						if (ni >= 0 && (size_t)ni < model.num_node_influences)
						{
							node_influence_t *inf = model.node_influences + ni;
							if (inf->num_weights > 0 && inf->weights)
							{
								vec3_t pos = { 0, 0, 0 };
								vec3_t nrm = { 0, 0, 0 };
								float total_w = 0;
								...
								{
									int b = inf->weights[w].bone_idx;
									float weight = inf->weights[w].weight;
									if (b >= 0 && (size_t)b < model.num_joints && weight > 0)
									{
										const float *bm = model.joints[b].bind;
										vec3_t tp = {
											bm[0] * mesh->positions[p].x + bm[1] * mesh->positions[p].y
												+ bm[2] * mesh->positions[p].z + bm[3],
											bm[4] * mesh->positions[p].x + bm[5] * mesh->positions[p].y
												+ bm[6] * mesh->positions[p].z + bm[7],
											bm[8] * mesh->positions[p].x + bm[9] * mesh->positions[p].y
												+ bm[10] * mesh->positions[p].z + bm[11]
										};
										pos.x += tp.x * weight;
										pos.y += tp.y * weight;
										pos.z += tp.z * weight;
										if (mesh->normals && p < mesh->num_normals)
										{
											vec3_t tn = {
												bm[0] * mesh->normals[p].x + bm[1] * mesh->normals[p].y
													+ bm[2] * mesh->normals[p].z,
												bm[4] * mesh->normals[p].x + bm[5] * mesh->normals[p].y
													+ bm[6] * mesh->normals[p].z,
												bm[8] * mesh->normals[p].x + bm[9] * mesh->normals[p].y
													+ bm[10] * mesh->normals[p].z
											};
											nrm.x += tn.x * weight;
											nrm.y += tn.y * weight;
											nrm.z += tn.z * weight;
				...
				if (!had_skin)
				{
					...
					for (size_t j = 0; j < model.num_joints; j++)
					{
						if (!strcmp (model.joints[j].name, mesh->name))
						{
							ji = (int)j;
							break;
						}
					}
					if (ji >= 0 && (size_t)ji < model.num_joints)
					{
						const float *bm = model.joints[ji].bind;
						for (size_t p = 0; p < mesh->num_positions; p++)
						{
							vec3_t tp = {
								bm[0] * mesh->positions[p].x + bm[1] * mesh->positions[p].y
						...
						if (mesh->normals)
						{
							for (size_t p = 0; p < mesh->num_normals; p++)
							{
								vec3_t tn = {
									bm[0] * mesh->normals[p].x + bm[1] * mesh->normals[p].y
										+ bm[2] * mesh->normals[p].z,
								...
								float nl = sqrtf (tn.x * tn.x + tn.y * tn.y + tn.z * tn.z);
								if (nl > 1e-6f)
								{
									tn.x /= nl;
									tn.y /= nl;
