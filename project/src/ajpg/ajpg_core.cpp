...
#include "ajpg.h"
#define ODH_RB32(p) ((((const u8*)(p))[0]<<24) | (((const u8*)(p))[1]<<16) | (((const u8*)(p))[2]<<8) | ((const u8*)(p))[3])
#define ODH_WB32(p, v) do { \
    ((u8*)(p))[0] = (u8)((v)>>24); \
    ((u8*)(p))[1] = (u8)((v)>>16); \
    ((u8*)(p))[2] = (u8)((v)>>8); \
    ((u8*)(p))[3] = (u8)(v); \
...
static int ODHEncodeRGB565(unsigned char *src, unsigned char *dst, int width, int height, int limit, int quality, unsigned char *work);
static int ODHEncodeRGBA8(unsigned char *src, unsigned char *dst, int width, int height, int limit, int quality, unsigned char *work);
static int ODHEncodeY8U8V8(unsigned char *src, unsigned char *dst, int width, int height, int limit, int quality, unsigned char *work);
static int ODHDecodeRGB565(unsigned char *src, unsigned char *dst, unsigned char *work);
static int ODHDecodeRGBA8(unsigned char *src, unsigned char *dst, unsigned char *work);
static int ODHDecodeY8U8V8(unsigned char *src, unsigned char *dst, unsigned char *work);
...
public:
    ...
    {
        return ODH_RB32(((const u8*)src) + 8);
    }
    // ODH?f?[?^???|?C???^??????ODH??????E????????
    int getOdhWidth(const void *src) const
    {
        return (int)(ODH_RB32(((const u8*)src) + 4) & 0x7ff);
    }
    int getOdhHeight(const void *src) const
    {
        return (int)((ODH_RB32(((const u8*)src) + 4) >> 11) & 0x7ff);
    }
    u8 getOdhSamplingRateH(const void *src) const
    {
        return (u8)(((ODH_RB32(((const u8*)src) + 4) >> 22) & 1) + 1);
    }
    u8 getOdhSamplingRateV(const void *src) const
    {
        return (u8)(((ODH_RB32(((const u8*)src) + 4) >> 23) & 1) + 1);
    }
    u8 getOdhQuality(const void *src) const
    {
        return (u8)((ODH_RB32(((const u8*)src) + 4) >> 24) & 0xff);
    }
...
int CArGBAOdh::decompressGbaOdh(u8 *src, u8 *dest, u8 *work, int format)
{
    ...
    if (dest) { result = cdj_d_colorDeconv(&cdjMasterCtrl, dest, format); }
    if (result)
    {
...
int CArGBAOdh::compressGbaOdh(u8 *src, u8 *dest, int width, int height, int quality, u32 sizeLimit, u8 *work, int format)
{
    ...
    if (src) { result = cdj_c_colorConv(&cdjMasterCtrl, src, format); } else { result = 0; }
    if (result)
    {
...
void CArGBAOdh::cdj_c_makeHeader(SArCDJ_OdhMaster *cdj_ctrl, u32 fileSize)
{
    ...
    // ?????A???????A?T???v?????O???[?g?A???k?i??
    ODH_WB32(&cdj_ctrl->mCodeBufferPtr[4], 
    ...
    // ???k???f?[?^?T?C?Y
    ODH_WB32(&cdj_ctrl->mCodeBufferPtr[8], fileSize);
    // ?\?????N???A
    ODH_WB32(&cdj_ctrl->mCodeBufferPtr[12], 0);
}
    ...
    SArCDJ_OdhMaster *cdj_ctrl, u8 *imgYCbCrBufPtr, u8 *CodeBufPtr)
{
    ...
    cdj_ctrl->mImageSize[DArCDJ_AXIS_X] =
        (u16)(ODH_RB32(((const u8*)CodeBufPtr) + 4) & 0x7ff);
    // ??????
    cdj_ctrl->mImageSize[DArCDJ_AXIS_Y] =
        (u16)((ODH_RB32(((const u8*)CodeBufPtr) + 4) >> 11) & 0x7ff);
    // ?????T???v?????O???[?g
    cdj_ctrl->mSmpRate[DArCDJ_AXIS_X] =
        (u8)(((ODH_RB32(((const u8*)CodeBufPtr) + 4) >> 22) & 1) + 1);
    // ?????T???v?????O???[?g
    cdj_ctrl->mSmpRate[DArCDJ_AXIS_Y] =
        (u8)(((ODH_RB32(((const u8*)CodeBufPtr) + 4) >> 23) & 1) + 1);
    // ???k?i??
    cdj_ctrl->mQuality = (u8)((ODH_RB32(((const u8*)CodeBufPtr) + 4) >> 24) & 0xff);
...
extern "C" {
    int ajpg_decode(u8 *src, u8 *dst, u8 *work, int sizeLimit) {
        CArGBAOdh odh;
        return odh.decompressGbaOdh(src, dst, work, sizeLimit);
    }
    
    int ajpg_encode(u8 *src, u8 *dst, int width, int height, int quality, u32 sampRate, u8 *work, int sizeLimit) {
        CArGBAOdh odh;
        return odh.compressGbaOdh(src, dst, width, height, quality, sampRate, work, sizeLimit);
    }
    
    u32 ajpg_get_filesize(const void *src) {
        CArGBAOdh odh;
        return odh.getOdhFileSize(src);
    }
    
    int ajpg_get_width(const void *src) {
        CArGBAOdh odh;
        return odh.getOdhWidth(src);
    }
    
    int ajpg_get_height(const void *src) {
        CArGBAOdh odh;
        return odh.getOdhHeight(src);
    }
}
