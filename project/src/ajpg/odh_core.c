...
/*
 * Standalone shim: this file is the mobipeg/FFmpeg ODH codec core with the
 * FFmpeg wrappers removed and a small RGBA API bolted on at the bottom.
 * Only these few libavutil helpers were used by the core, so they are
 * provided locally rather than pulling in any of FFmpeg.
 */
#define AV_RB32(p) ( (uint32_t)((const uint8_t*)(p))[0] << 24 | \
                     (uint32_t)((const uint8_t*)(p))[1] << 16 | \
                     (uint32_t)((const uint8_t*)(p))[2] <<  8 | \
                     (uint32_t)((const uint8_t*)(p))[3] )
#define AV_WB32(p,v) do { uint8_t *_p = (uint8_t*)(p); uint32_t _v = (uint32_t)(v); \
                          _p[0] = _v >> 24; _p[1] = _v >> 16; \
                          _p[2] = _v >> 8;  _p[3] = _v; } while (0)
#define AV_CEIL_RSHIFT(a,b) (-(((-(a)) >> (b))))
#define av_always_inline inline __attribute__((always_inline))
#define av_mallocz(n)  calloc(1,(n))
#define av_free(p)     free(p)
#define av_freep(pp)   do { free(*(pp)); *(pp) = NULL; } while (0)
...
/*
 * ---------------------------------------------------------------------------
 * Standalone RGBA API (replaces the FFmpeg encoder/decoder wrappers)
 * ---------------------------------------------------------------------------
 * The codec natively works in planar YCbCr; the reference's GameCube-tiled
 * RGB565/RGBA8/Y8U8V8 output was console glue and is not part of the format.
 * These wrappers do a plain JPEG full-range YCbCr <-> RGB conversion instead,
 * which is what the format's coefficients actually mean.
 */
...
static inline uint8_t odh_clip8(int v)
{
...
int AjpgGetInfo(const void *src, size_t src_size, int *width, int *height)
{
    ...
    if (!d || src_size < DArCDJ_HEADER_SIZE || memcmp(d, "AJPG", 4))
        return 0;
    if (width)  *width  = (int)(AV_RB32(d + 4) & 0x7ff);
    if (height) *height = (int)((AV_RB32(d + 4) >> 11) & 0x7ff);
    return 1;
}
                   ...
                   uint8_t **out_rgba, int *out_width, int *out_height)
{
    ...
    u8 *work = NULL, *rgba = NULL;
    int luma_stride, chroma_stride;
    size_t cb_off, cr_off, total;
    int width, height, sx, sy;
    u32 result;

    if (!src || !out_rgba || src_size < DArCDJ_HEADER_SIZE)
        return 0;
    if (memcmp(src, "AJPG", 4))
        return 0;
     ...
     */
    result = cdj_d_initializeDecompressOdh(&ctrl, NULL, (u8 *)(uintptr_t)src,
                                           (u32)src_size);
    if (result)
        return 0;
    ...
    sx = ctrl.mSmpRate[DArCDJ_AXIS_X];
    sy = ctrl.mSmpRate[DArCDJ_AXIS_Y];
    if (width <= 0 || height <= 0)
        return 0;
    ...
    for (int y = 0; y < height; y++) {
        const u8 *yrow = work + (size_t)y * luma_stride;
        /* Chroma is subsampled by sx/sy (1 or 2 on each axis). */
        const u8 *cbrow = work + cb_off + (size_t)(y / sy) * chroma_stride;
        const u8 *crrow = work + cr_off + (size_t)(y / sy) * chroma_stride;
        for (int x = 0; x < width; x++) {
            const int Y  = yrow[x];
            const int Cb = cbrow[x / sx] - 128;
            const int Cr = crrow[x / sx] - 128;
            /* JPEG full-range YCbCr -> RGB (ITU-R BT.601, no head/foot room) */
            u8 *p = rgba + 4 * ((size_t)y * width + x);
            p[0] = odh_clip8(Y + ((91881 * Cr) >> 16));
            p[1] = odh_clip8(Y - ((22554 * Cb + 46802 * Cr) >> 16));
            p[2] = odh_clip8(Y + ((116130 * Cb) >> 16));
            p[3] = 255;
        }
    }
...
{
    SArCDJ_OdhMaster ctrl;
    u8 *work = NULL, *out = NULL;
    int luma_stride, chroma_stride;
    size_t cb_off, cr_off, total;
    u32 result, size_limit;
    /* 4:4:4 -- no chroma subsampling, the highest-fidelity ratio. */
    const int samp_x = 1, samp_y = 1;

    if (!rgba || !out_data || width <= 0 || height <= 0)
        return 0;
    if (width > DArCDJ_PIXEL_SIZE_MAX_X || height > DArCDJ_PIXEL_SIZE_MAX_Y)
        return 0;
     ...
     */
    size_limit = (u32)width * height * 2 + DArCDJ_HEADER_SIZE;
    size_limit = (size_limit + 3) & ~3u;

    out = malloc(size_limit);
    if (!out)
        return 0;

    for (;;) {
        u16 imgSize[2] = { (u16)width, (u16)height };
        ...
        if (!work) {
            ...
            /* RGB -> JPEG full-range YCbCr (ITU-R BT.601). */
            for (int y = 0; y < height; y++) {
                u8 *yrow  = work + (size_t)y * luma_stride;
                u8 *cbrow = work + cb_off + (size_t)y * chroma_stride;
                u8 *crrow = work + cr_off + (size_t)y * chroma_stride;
                for (int x = 0; x < width; x++) {
                    const int R = rgba[4 * ((size_t)y * width + x) + 0];
                    const int G = rgba[4 * ((size_t)y * width + x) + 1];
                    const int B = rgba[4 * ((size_t)y * width + x) + 2];
                    yrow[x]  = odh_clip8(( 19595 * R + 38470 * G +  7471 * B) >> 16);
                    cbrow[x] = odh_clip8(((-11056 * R - 21712 * G + 32768 * B) >> 16) + 128);
                    crrow[x] = odh_clip8((( 32768 * R - 27440 * G -  5328 * B) >> 16) + 128);
                }
            }
        }
        ctrl.mImgYCbCrBufferPtr = work;
