...
static s32 rd_s32e ( const u8 *p, bool le ) { return (s32)rd_u32e(p,le); }
...
typedef struct leaf_t { u32 id; u32 string_index; } leaf_t;

static enumError scan_strg
(
    const u8 *data, uint size, const u8 *block, bool le,
    char ***out_strings, uint *out_n_strings,
    leaf_t **out_leaves, uint *out_n_leaves
)
{
    ...
    if ( (u64)(strtab-base) + 4 + 12ull*n_strings > size )
        return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: STRG string table exceeds buffer (%u entries)\n", n_strings);
    ...
    if ( lookup_table_off != NULL_PTR )
    {
        ...
        if ( (u64)(recs-base) + 20ull*n_nodes > size )
            return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: STRG lookup table exceeds buffer (%u nodes)\n", n_nodes);
...
static const struct { u16 ref_type; bfsar_sound_type_t type; ccp name; } info_sections[] =
{
...
static enumError scan_info
(
    const u8 *data, uint size, const u8 *block, bool le,
    const leaf_t *leaves, uint n_leaves, char **strings, uint n_strings,
    bfsar_table_t *out_table, uint *out_n_table
)
{
    ...
    for ( uint s = 0; s < sizeof(info_sections)/sizeof(*info_sections); s++ )
    {
        ...
        if ( (u64)(table-data) + 4 + 8ull*count > size )
            return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: INFO section '%s' exceeds buffer (%u entries)\n",
                info_sections[s].name, count);
        ...
        for ( u32 i = 0; i < count; i++ )
        {
            ...
            e->name = t->type != BFSAR_TYPE_NONE
                ? lookup_name(leaves,n_leaves,strings,n_strings,e->id) : 0;
        }
    }
...
enumError ScanBFSAR ( bfsar_t *bfsar, const u8 *data, uint size )
{
    ...
    bool is_ctr;
    if      (!memcmp(data,"FSAR",4)) is_ctr = false;
    else if (!memcmp(data,"CSAR",4)) is_ctr = true;
    else return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: not a FSAR/CSAR file\n");

    u16 bom = (u16)(data[4]<<8 | data[5]);
    bool le;
    if      (bom == 0xFEFF) le = false;
    else if (bom == 0xFFFE) le = true;
    else return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: bad byte-order mark\n");

    u32 version = rd_u32e(data+8,le);
    // 'F' type: 0x00MMIIRR; 'C' type: 0xMMIIRR00 (see lib-bfsar.h provenance).
    u8 vmaj, vmin, vrev;
    if (is_ctr) { vmaj = (version>>24)&0xff; vmin = (version>>16)&0xff; vrev = (version>>8)&0xff; }
    else        { vmaj = (version>>16)&0xff; vmin = (version>>8)&0xff;  vrev = version&0xff; }

    u16 n_blocks = rd_u16e(data+0x10,le);
    if ( (u64)0x14 + 12ull*n_blocks > size )
        return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: block table exceeds buffer (%u blocks)\n", n_blocks);
    ...
    for ( uint i = 0; i < n_blocks; i++ )
    {
        ...
        if      (!memcmp(blk,"STRG",4)) strg = blk;
        else if (!memcmp(blk,"INFO",4)) info = blk;
        // FILE block (raw sample-data pool) and any unknown block types are
        // intentionally not parsed here -- see lib-bfsar.h scope note.
    }

    if (!strg) return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: missing STRG block\n");
    if (!info) return ERROR0(ERR_INVALID_DATA, "ScanBFSAR: missing INFO block\n");

    leaf_t *leaves = 0; uint n_leaves = 0;
    enumError err = scan_strg(data,size,strg,le,&bfsar->strings,&bfsar->n_strings,&leaves,&n_leaves);
    if (!err)
...
enumError DumpBFSAR_XML ( const bfsar_t *bfsar, FILE *f, ccp source_name )
{
    ...
    fprintf(f, "<bfsar source=\"%s\" magic=\"%s\" version=\"%u.%u.%u\" endian=\"%s\">\n",
        source_name ? source_name : "", bfsar->is_ctr ? "CSAR" : "FSAR",
        bfsar->version_major, bfsar->version_minor, bfsar->version_revision,
        bfsar->little_endian ? "little" : "big" );
    ...
    for ( uint t = 0; t < bfsar->n_table; t++ )
    {
        ...
        for ( uint i = 0; i < tab->n_entry; i++ )
        {
            ...
            fprintf(f, "    <entry index=\"%u\" id=\"0x%08x\" present=\"%s\"",
                i, e->id, e->present ? "yes" : "no");
            if (e->name)
            {
                fprintf(f, " name=\"");
                xml_escape(f,e->name);
