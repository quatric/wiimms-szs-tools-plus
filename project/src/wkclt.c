...
#define TITLE                                                                                      \
	WKCLT_SHORT ": " WKCLT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
{
	if (current_command && current_command->id >= 0 && current_command->id < InfoUI_wkclt.n_cmd)
	{
...
static enumError cmd_test_options ()
{
	...
	if (opt_xtridata)
		printf ("  xtridata:    %16x = %12d [support %sabled]\n", opt_xtridata, opt_xtridata,
			SUPPORT_KCL_XTRIDATA ? "en" : "dis");
...
static enumError cmd_convert (int cmd_id, ccp cmd_name, ccp def_path)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!testmode)
		{
			err = dest_ff == FF_KCL ? SaveRawKCL (&kcl, dest, opt_preserve)
...
static enumError cmd_copy ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		const file_format_t dest_ff = fform_dest != FF_UNKNOWN ? fform_dest : raw.fform;
		...
		char dest[PATH_MAX];
		SubstDest (dest, sizeof (dest), arg, opt_dest, "\1P/\1N\1?T", GetExtFF (dest_ff, 0), false);
		if (verbose >= 0 || testmode)
		{
			fprintf (stdlog, "%s%sCOPY %s:%s -> %s:%s\n", verbose > 0 ? "\n" : "",
				testmode ? "WOULD " : "", GetNameFF (raw.fform, 0), kcl.fname,
				GetNameFF (dest_ff, 0), dest);
			fflush (stdlog);
		}
...
static ccp cff_func_f (u16 flag)
{
	...
	DASSERT (buf);
	snprintf (buf, bufsize, "f(0x%02x,0x%03x)", flag % N_KCL_TYPE, flag / N_KCL_TYPE);
	return buf;
}
...
static ccp cff_func_a (u16 flag)
{
	...
	char *buf = GetCircBuf (bufsize);
	DASSERT (buf);
	snprintf (buf, bufsize, "a(0x%02x,%u,%u,%u,%u)", flag % N_KCL_TYPE, flag >> 13 & 7,
		flag >> 11 & 3, flag >> 8 & 7, flag >> 5 & 7);
	return buf;
}
...
static ccp cff_func_i (u16 flag)
{
	...
	char *buf = GetCircBuf (bufsize);
	DASSERT (buf);
	snprintf (buf, bufsize, "i(0x%02x,%u,%u)", flag % N_KCL_TYPE, flag >> 5 & 7, flag >> 8 & 15);
	return buf;
}
...
static enumError cmd_cff ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!dest)
		{
			SubstDest (dest_buf, sizeof (dest_buf), arg, opt_dest, 0, KCL_FLAG_EXT, false);
			dest = dest_buf;
			append = false;
		}
			...
			if (!err)
			{
				...
				for (i = 0; i < kcl.flag_missing.used; i++)
				{
					...
					if (long_count > 1)
					{
						if (brief_count)
							fprintf (
								F.f, "  %s%.*s= %s", it->key, tablen, Tabs20, cff_func_a (num));
						else
							fprintf (F.f, "  %s%.*s= %s # %s  %s  0x%04x", it->key, tablen, Tabs20,
					...
					else if (long_count)
					{
						if (brief_count)
							fprintf (
								F.f, "  %s%.*s= %s", it->key, tablen, Tabs20, cff_func_f (num));
						else
							fprintf (F.f, "  %s%.*s= %s # %s  %s  0x%04x", it->key, tablen, Tabs20,
								cff_func_f (num), cff_func_a (num), cff_func_i (num), num);
					}
					else if (brief_count)
						fprintf (F.f, "  %s%.*s= 0x%04x", it->key, tablen, Tabs20, num);
					else
						fprintf (F.f, "  %s%.*s= 0x%04x  #  %s  %s  %s", it->key, tablen, Tabs20,
...
static enumError cmd_types ()
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		else
		{
			...
			printf (" 0x%04x = 0x%02x | 0x%03x << 5 : %c %c %c %c %2d\n", value, value & 0x1f,
				value >> 5, w ? '0' + w : '-', x ? '0' + x : '-', y ? '0' + y : '-',
				z ? '0' + z : '-', value & 0x1f);
		}
	}
				...
				for (j = i; j < N_KCL_FLAG; j += N_KCL_TYPE)
					if (count[j])
					{
						...
						if (!brief_count)
						{
							...
							printf (" 0x%04x = 0x%02x | 0x%03x << 5"
									" : %c %c %c %c %2d"
									" :%7u = %6.2f%%\n",
								j, j & 0x1f, j >> 5, w ? '0' + w : '-', x ? '0' + x : '-',
								y ? '0' + y : '-', z ? '0' + z : '-', j & 0x1f, count[j],
								100.0 * count[j] / n);
						}
					}
...
static enumError cmd_dump (bool brief_level, int cmd)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!testmode)
		{
			err = cmd == CMD_LIST	   ? DumpTrianglesKCL (&kcl, "-")
				: cmd == CMD_TRIANGLES ? ListTrianglesKCL (&kcl, "-", !brief_count, long_count)
...
static enumError cmd_traverse (bool fall)
{
	...
	if (fall)
	{
		...
		snprintf (fall_hint, sizeof (fall_hint), " [width=%u,mask=0x%x]", width, type_mask);
	}
	...
	for (param = param->next; param; param = param->next, np++)
	{
		...
		{
			int kcl_flag;
			double height = FallKCL (&kcl, stdout, pt.d3, width, long_count, type_mask, &kcl_flag);
			if (brief_count > 1)
				printf ("%11.3f\n", height);
			else if (brief_count)
			...
			err = TraverseOctreeKCL (&kcl, stdout, pt.d3, brief_count <= 0, long_count);
		fflush (stdout);
		if (max_err < err)
			max_err = err;
	}
...
static void analyze_kcl (kcl_t *kcl)
{
	...
	for (td = td_base; td < td_end; td++)
	{
		...
		for (; nd > 0; nd--, d++)
		{
			...
			if (v_max < *d)
				v_max = *d;
			if (*d > 0.0 && v_min_pos > *d)
				v_min_pos = *d;
			if (*d < 0.0 && v_max_neg < *d)
				v_max_neg = *d;
		}
		...
		for (; nf > 0; nf--, f++)
		{
			...
			if (n_max < *f)
				n_max = *f;
			if (*f > 0.0 && n_min_pos > *f)
				n_min_pos = *f;
			if (*f < 0.0 && n_max_neg < *f)
				n_max_neg = *f;
			// printf("XX: %08x\n",*(u32*)f);
		}
	}
	...
	printf ("\n"
			...
			" max:          %13.6e  %13.6e\n"
			"\n",
		n, n_tri, v_min, n_min, v_max_neg, n_max_neg, v_min_pos, n_min_pos, v_max, n_max);
...
static enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_CAT:
			...
			break;
		case CMD_DECODE:
			err = cmd_convert (cmd_ct->id, "DECODE", "\1P/\1N.obj");
			break;
		case CMD_ENCODE:
		...
		case CMD_FLAGS:
			...
			break;
		case CMD_DUMP:
			err = cmd_dump (0, CMD_DUMP);
			break;
		case CMD_DBRIEF:
			err = cmd_dump (1, CMD_DUMP);
			break;
		case CMD_LIST:
			err = cmd_dump (0, CMD_LIST);
			break;
		case CMD_TRIANGLES:
			err = cmd_dump (0, CMD_TRIANGLES);
			break;
		case CMD_TRAVERSE:
			err = cmd_traverse (false);
			break;
		case CMD_FALL:
			err = cmd_traverse (true);
			break;
		case CMD_CHECK:
			err = cmd_check ();
			break;
