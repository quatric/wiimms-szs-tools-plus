...
const u8 TestDataLEX[8 * 1] = { 0, // offline_online
	0, // n_offline
	0, // n_online
	~0, // cond_bit
	0, // game_mode
...
void SetupColors ()
{
	...
	PRINT ("COLORIZE: out=%d[%zd], err=%d[%zd], log=%d[%zd], msg=%d[%zd]\n", colorize_stdout,
		strlen (colout->reset), colorize_stderr, strlen (colerr->reset), colorize_stdlog,
		strlen (collog->reset), colorize_stdmsg, strlen (colmsg->reset));
}
...
void NormalizeOptions (uint log_level // >0: print PROGRAM_NAME and pathes
)
{
	SetupColors ();
	...
	global_check_mode
		= opt_no_check ? 0 : GetCheckMode (false, brief_count > 0, verbose < 0, long_count > 0);
...
ccp GetCtLeInfo ()
{
	return PrintCircBuf ("ct=%d, le=%d, mode=%d[%s]", ctcode_enabled, lecode_enabled, opt_ct_mode,
		GetCtModeNameBMG (opt_ct_mode, true));
}

///////////////////////////////////////////////////////////////////////////////

const KeywordTab_t *CheckCommandHelper (int argc, char **argv, const KeywordTab_t *key_tab)
{
	DASSERT (argv);
	DASSERT (key_tab);
...
{
	return stat == ERR_INTERRUPT ? stat
		: opt_zero > 0 && stat <= ERR_NOT_EXISTS || opt_zero > 1 && stat <= ERR_WARNING
			|| opt_zero > 2 && stat <= ERR_ERROR || opt_zero > 3
		? 0
		: stat;
}
...
ccp LibGetErrorName (int stat, ccp ret_not_found)
{
	...
	{
		case ERR_CACHE_USED:
			return "SZS CACHE USED";
		case ERR_INVALID_IFORM:
			return "INVALID IMAGE FORMAT";
		case ERR_INVALID_FFORM:
...
const compatible_info_t compatible_info[COMPAT__N] = { { COMPAT_0, 2385, "0.01" },
	{ COMPAT_1_23, 4682, "1.23" }, { COMPAT_1_39, 6123, "1.39" }, { COMPAT_1_44, 6558, "1.44" },
	{ COMPAT_1_46, 6614, "1.46" }, { COMPAT_2_08, 7961, "2.08" }, { COMPAT_2_40, 8910, "2.40" },
...
int ScanOptCompatible (ccp arg)
{
	...
	if (!ci)
	{
		ERROR0 (ERR_SYNTAX, "Wrong argument for option --compatible: %s\n", arg);
		return 1;
	}
...
char *PrintOptCompatible ()
{
	...
	const compatible_info_t *ci
		= compatible_info + ((uint)compatible < COMPAT__N ? compatible : COMPAT_CURRENT);
...
int ScanOptWarn (ccp arg)
{
	...
	s64 stat
		= ScanKeywordList (arg, opt_warn_tab, 0, true, 0, WARN_MODE, "Option --warn", ERR_SYNTAX);
	if (stat == -1)
		return 1;
...
{
	static const int order[] = { WARNSZS_ITEMPOS, WARNSZS_SELF_ITPH, WARNSZS_NO_MINIMAP, -1 };
...
DataContainer_t *UseSubContainer (DataContainer_t *dc, // old_container to reuse, if
...
DataContainer_t *NewContainer (DataContainer_t *dc, // old_container to reuse,
...
void PrintNumF (FILE *f, // output stream
...
uint CreateHaystackListWCMP (char *buf, // valid pointer to output buffer
...
uint CreateHaystackWCMP (char *buf, // valid pointer to output buffer
	uint buf_size, // soue of 'buf'
...
uint CreateNeedleWCMP (char *buf, // valid pointer to output buffer
	uint buf_size, // soue of 'buf'
	ccp source // NULL terminated list with source Strings
)
{
...
bool FindWCMP (ccp haystack, // search in this string
...
char *SubstString (char *buf, // destination buffer
...
)
{
	...
	if (source)
		while (dest < end && *source)
		{
			...
			char *new_dest = NormalizeFileName (
				dest, end - dest, tempbuf, ptr->allow_slash, use_utf8, TRSL_NONE);
			if (check_prev && *dest == '.')
			{
...
int SubstDest (char *buf, // destination buffer
	size_t bufsize, // size of 'buf'
	ccp source, // source text
...
)
{
	...
	SubstString_t subst_tab[] = { { 'q', 'Q', 1, source }, { 'p', 'P', 1, dir },
		{ 'f', 'F', 1, fname }, { 'n', 'N', 1, name }, { 'e', 'E', 1, ext },
		{ 't', 'T', 1, default_ext ? default_ext : ext }, { 0, 0, 0, 0 } };
...
static void SetFormatFieldParam (FormatFieldItem_t *item, // valid attrib item
...
FormatFieldItem_t *InsertFormatField (FormatField_t *ff, // valid attrib field
...
FormatFieldItem_t *InsertFormatFieldFF (FormatField_t *ff, // valid attrib field
	ccp key, // key to insert
...
FormatFieldItem_t *AppendFormatField (FormatField_t *ff, // valid attrib field
	ccp key, // key to insert
	bool scan_form, // true: scan 'key' for FORM= prefix
	bool move_key // true: move string, false: strdup()
)
...
enumError SaveFormatField (FormatField_t *ff, // valid attrib field
...
enumError WriteFormatField (FILE *f, // open destination file
...
)
{
	...
	for (end = ptr + ff->used; ptr < end; ptr++)
	{
		...
		if (ptr->num)
		{
			const int stat = hex_fw ? fprintf (f, "%s%#*x", sep, hex_fw, ptr->num)
...
abort:
	return ERROR1 (
		ERR_WRITE_FAILED, "Error while writing attrib field: %s\n", filename ? filename : "?");
}
...
int AddParam (ccp arg)
{
	...
	noTRACE ("INS: A=%p->%p P=%p &N=%p->%p\n", append_param, *append_param, param, &param->next,
		param->next);
	*append_param = param;
...
void CollectExpandParam (StringField_t *plist, // insert or append (inorder_count>0) to this list
...
enumError cmd_wildcards (int argc, char **argv)
{
	...
	{
		const CharMode_t mode
			= CHMD_UTF8 | CHMD_ESC | (script_fform == FF_BASH ? CHMD_BASH : CHMD_SH);
...
{
	static const KeywordTab_t tab[] = { { 0, "TINY-0", "OFF", 0 }, { 1, "TINY-1", "LOSSLESS", 0 },
		{ 2, "TINY-2", "RECREATE", 0 }, { 3, "TINY-3", "MINIMAL", 0 }, { 4, "TINY-4", "GOOD", 0 },
		{ 5, "TINY-5", "MEDIUM", 0 }, { 6, "TINY-6", "EXTREME", 0 },
		{ 7, "TINY-7", "DANGEROUS", 0 }, { 0, 0, 0, 0 } };
...
void *GrowListSize (List_t *list, uint n, uint add_size)
{
	...
	if (new_used > list->size || !list->size)
	{
		noPRINT ("GROW-LIST: %u -> %u (size=%u)\n", list->size, new_used + add_size,
			(new_used + add_size) * list->elem_size);
		list->size = new_used + add_size;
		list->list = REALLOC (list->list, list->size * list->elem_size);
	}
...
void *GetListElem (const List_t *list, int index, const void *return_not_found)
{
	DASSERT (list);
	DASSERT (list->elem_size > 0);
	DASSERT (list->used <= list->size);
	...
	return (uint)index < list->used ? list->list + index * list->elem_size
...
void SortMIL (List_t *list, bool use_sort_idx)
{
	...
	if (list->used > 1)
	{
		...
		qsort (list->list, list->used, sizeof (MemItem_t),
			use_sort_idx ? (qsort_func)sort_mem_list_by_sort : (qsort_func)sort_mem_list_by_idx);
	}
}
...
MemItem_t *GetMemListElem (const List_t *list, int index, const MemItem_t *return_not_found)
{
...
{
	static const KeywordTab_t alias[] = { { 0, "LIST", 0, -9 }, { 1, "C-LIST", "CLIST", -9 },
	...
	if (end > arg && !*end)
	{
		struct tab_t
		{
			s16 from, to, add, mode;
		};
		static const struct tab_t tab[] = { { -1, -1, 10, -1 }, { 0, 10, 0, 1 }, { 91, 93, 0, 9 },
			{ 100, 150, -100, 11 }, { -9, 0, 0, 0 } };
...
int GetComprByFF (file_format_t ff, int compr)
{
	...
	switch (ff)
	{
		...
		case FF_YBZ:
		case FF_BZIP2:
			return compr == COMPR_DEFAULT					 ? BZIP2_DEFAULT_COMPR
				: compr >= 1 && compr <= 9					 ? compr
				: opt_compr_mode >= 2 && opt_compr_mode <= 5 ? -1
															 : BZIP2_DEFAULT_COMPR;
...
int ScanOptPtDir (ccp arg)
{
	...
	static const KeywordTab_t tab[]
		= { { -1, "REMOVE", "0", 0 }, { 0, "AUTO", 0, 0 }, { 1, "FORCE", "1", 0 },
...
int ScanOptRecurse (ccp arg)
{
	...
	static const KeywordTab_t tab[] = { { 0, "NONE", 0, 0 }, { INT_MAX, "UNLIMITED", "ALL", 0 },
...
int ScanOptFModes (ccp arg)
{
	...
	static const KeywordTab_t keytab[] = { { FZM_SECTION, "SECTION", 0, BOTH * FZM_SECTION },
	...
	const s64 stat = ScanKeywordList (arg, keytab, 0, true, 0, 0, "Option --filter", ERR_SYNTAX);
	if (stat == -1)
		return 1;
...
static const KeywordTab_t opt_slot_tab[] = { { SLOTMD_NONE, "NONE", "-", 0 },

	{ SLOTMD_M_DAISY, "DAISY", "DESERT", 0 }, { SLOTMD_M_DAISY, "3.1", "31", 0 },
	{ SLOTMD_M_DAISY, "7.1", "71", 0 },

	{ SLOTMD_M_SHERBET, "SHERBET", "T61", 0 }, { SLOTMD_M_SHERBET, "6.1", "61", 0 },

	{ SLOTMD_M_SHYGUY, "SHYGUY", "T62", 0 }, { SLOTMD_M_SHYGUY, "6.2", "62", 0 },

	{ SLOTMD_M_STANDARD, "STANDARD", "STD", 0 },

	{ SLOTMD_M_MOST, "MOST", "ALL", 0 },

	{ SLOTMD_RM_ICE, "RM-ICE", "RMICE", 1 }, { SLOTMD_RM_SUNDS, "RM-SUNDS", "RMSUNDS", 1 },
	{ SLOTMD_RM_SHYGUY, "RM-SHYGUY", "RMSHYGUY", 1 }, { SLOTMD_ADD_GICE, "ADD-GICE", "ADDGICE", 1 },
...
static int scan_track_arena (ccp arg, const u32 *def_tab, u32 *tab, u32 *r_tab,
	const TrackInfo_t *info, uint n, ccp opt_name, int (*scan_func) (ccp name))
{
	DASSERT (def_tab);
	DASSERT (tab);
	DASSERT (r_tab);
	...
	for (;;)
	{
		...
		{
			PRINT ("SWAP --%s:  %2u=%2u, %2zu=%2u\n", opt_name, track_index, *tptr, tptr - tab,
				tab[track_index]);
...
{
	opt_tracks = true;
	return scan_track_arena (arg, track_pos_default, track_pos, track_pos_r, track_info,
		MKW_N_TRACKS, "track", ScanTrack);
}
...
{
	opt_arenas = true;
	return scan_track_arena (arg, arena_pos_default, arena_pos, arena_pos_r, arena_info,
		MKW_N_ARENAS, "arena", ScanArena);
}

///////////////////////////////////////////////////////////////////////////////

void DumpTrackList (FILE *f, // NULL(=stdout) or file
...
)
{
	...
	if (brief_mode < 1)
	{
		if (long_count)
			fprintf (f,
				...
				" idx slot slot  of track             slot  sound file name   f.name  abbr. "
				"name\n");
		else
			fprintf (f,
				"\n"
	...
	for (i = 0; i < MKW_N_TRACKS; i++)
	{
		...
		const uint idx = *tab++;
		const TrackInfo_t *info = idx < MKW_N_TRACKS ? track_info + idx : &InvalidTrack;
		fprintf (f, "%3u.  %u.%u  %u.%u  %-*s  ", idx, i / 4 + 1, i % 4 + 1, info->def_slot / 10,
			info->def_slot % 10, FW_TRACK_FNAME, info->track_fname);

		if (long_count)
			fprintf (f, "0x%02x  ", info->music_id);

		fprintf (f, "%-*s %s  ", FW_TRACK_SOUND_N, info->sound_n_fname,
...
void DumpArenaList (FILE *f, // NULL(=stdout) or file
...
)
{
	...
	for (i = 0; i < MKW_N_ARENAS; i++)
	{
		...
		const uint idx = *tab++;
		const TrackInfo_t *info = idx < MKW_N_ARENAS ? arena_info + idx : &InvalidTrack;
		fprintf (f, "%3u.  %u.%u  %u.%u  %-*s  ", idx, i / 5 + 1, i % 5 + 1, info->def_slot / 10,
			info->def_slot % 10, FW_ARENA_FNAME, info->track_fname);

		if (long_count)
			fprintf (f, "0x%02x  ", info->music_id);

		fprintf (f, "%-*s %s  ", FW_ARENA_SOUND_N, info->sound_n_fname,
			info->sound_f_fname + strlen (info->sound_f_fname) - 2);
...
void DumpTrackFileList (FILE *f, // NULL(=stdout) or file
...
)
{
	...
	for (i = 0; i < MKW_N_TRACKS; i++)
	{
		...
		fprintf (f, "T%u.%u %s %s %s\n", i / 4 + 1, i % 4 + 1, info->track_fname,
			info->sound_n_fname, info->sound_f_fname);
	}
...
SortMode_t GetSortMode (SortMode_t sort_mode, // wanted sort mode
...
static const KeywordTab_t opt_sort_tab[] = { { SORT_NONE, "NONE", "0", 0 },
	{ SORT_INAME, "NAME", "N", 0 }, { SORT_INAME, "INAME", 0, 0 }, { SORT_OFFSET, "OFFSET", 0, 0 },
	{ SORT_SIZE, "SIZE", "S", 0 },

	{ SORT_U8, "U8", 0, 0 }, { SORT_PACK, "PACK", 0, 0 }, { SORT_BRRES, "BRRES", "BRES", 0 },
...
void cmd_version_section (bool sect_header, ccp name_short, ccp name_long, int verbose)
{
	if (sect_header)
		fputs ("[version]\n", stdout);
	...
	printf ("prog=%s\n"
			...
			"date=" DATE "\n"
			"url=" URI_HOME "%s\n",
		name_short, name_long, ProgramPath (), BETA_VERSION, e[0], e[1], e[2], e[3],
		endian == 0x01020304	   ? "little"
			: endian == 0x04030201 ? "big"
								   : "mixed",
		name_short);
...
#endif
				,
			HAVE_CLOCK_GETTIME, HAVE_STATTIME_NSEC);
...
{
	SetupPager ();
	printf ("%sExpand %d Argument%s:%s\n", colout->caption, n_param, n_param == 1 ? "" : "s",
		colout->reset);
...
)
{
	...
	return bz_offset >= 0 && size > bz_offset ? IsBZIP2 (data + bz_offset, size - bz_offset) : 0;
}
...
)
{
	...
	return compr_level > 0 ? PrintCircBuf ("%s.%u", name, compr_level) : name;
}
		...
		if (err <= ERR_WARNING || !opt_ignore)
		{
			...
			if (long_count)
			{
				...
				if (IsYazFF (fform1))
				{
					...
					DecompressYAZ (buf1 + sizeof (yaz0_header_t), bufsize - sizeof (yaz0_header_t),
						buf2, bufsize, &written, arg, GetYazVersionFF (fform1), true, 0);
...
const KeywordTab_t cmdtab_file_attrib[] = { { FFT_VALID, "VALID", 0, 0 },
	{ FFT_VALID, "INVALID", "DIRECTORY", 1 },

	{ FFT_COMPRESS, "COMPRESSED", 0, 0 }, { FFT_ARCHIVE, "ARCHIVES", 0, 0 },
	{ FFT_GRAPHIC, "GRAPHICS", "IMAGES", 0 }, { FFT_TEXT, "TEXT", "TXT", 0 },
	{ FFT_M_FTYPE, "ETC", 0, 1 },

	{ FFT_BRSUB, "BRSUB", "BR-SUB", 0 }, { FFT_BRSUB, "BRESSUB", "BRES-SUB", 0 },
	{ FFT_BRSUB, "BRRESSUB", "BRRES-SUB", 0 }, { FFT_BRSUB2, "BRSUB2", "BR-SUB2", 0 },
	{ FFT_BRSUB2, "BRESSUB2", "BRES-SUB2", 0 }, { FFT_BRSUB2, "BRRESSUB2", "BRRES-SUB2", 0 },
	{ FFT_EXTERNAL, "EXTERNALS", 0, 0 }, { FFT_EXTERNAL, "INTERNALS", 0, 1 },

	{ FFT_CREATE, "CREATE", 0, 0 }, { FFT_EXTRACT, "EXTRACT", 0, 0 }, { FFT_CUT, "CUT", 0, 0 },
	{ FFT_DECODE, "DECODE", 0, 0 }, { FFT_ENCODE, "ENCODE", 0, 0 }, { FFT_PARSER, "PARSER", 0, 0 },
...
enumError cmd_fileattrib ()
{
	...
	if (n_param)
	{
		...
		for (param = first_param; param; param = param->next)
		{
			...
			{
				const KeywordTab_t *cmd = ScanKeyword (0, param->arg, cmdtab_file_attrib);
				if (!cmd)
					stat = ERROR0 (ERR_WARNING, "Unknown keyword ignored: %s\n", param->arg);
				else if (cmd->opt)
				{
					int i;
					for (i = 0; i < FF_N; i++)
	...
	for (;;)
	{
		...
		switch (fa & (FFT_VALID | FFT_M_FTYPE))
		{
			case 0:
				info = "directory";
				break;
			case FFT_VALID | FFT_COMPRESS:
				info = "compress";
				break;
			case FFT_VALID | FFT_ARCHIVE:
				info = "archive";
				break;
			case FFT_VALID | FFT_GRAPHIC:
				info = "graphic";
				break;
			case FFT_VALID | FFT_TEXT:
				info = "text";
				break;
			default:
				info = "misc";
		...
		printf ("  %s %s %s %s %s", fa & FFT_CREATE ? "create" : "-     ",
			fa & FFT_EXTRACT ? "extract" : "-      ", fa & FFT_CUT ? "cut" : "-  ",
...
void ExportFileAttribMakedoc (FILE *f)
{
	...
	fprintf (f, "\n#\f\n%.79s\n%.15s%17s%-32s%.15s\n%.79s\n\n", Hash200, Hash200, "",
		"File Attributes", Hash200, Hash200);
	...
	static const struct info_t info_tab[] = { DEF_ENTRY (VALID)

			DEF_ENTRY (COMPRESS) DEF_ENTRY (ARCHIVE) DEF_ENTRY (GRAPHIC) DEF_ENTRY (TEXT)
				DEF_ENTRY (M_FTYPE)

					DEF_ENTRY (BRSUB) DEF_ENTRY (BRSUB2) DEF_ENTRY (EXTERNAL) DEF_ENTRY (M_CLASS)

						DEF_ENTRY (CREATE) DEF_ENTRY (EXTRACT) DEF_ENTRY (CUT) DEF_ENTRY (DECODE)
							DEF_ENTRY (ENCODE) DEF_ENTRY (PARSER) DEF_ENTRY (M_SUPPORT)
	...
	for (info_ptr = info_tab; info_ptr->name; info_ptr++)
		fprintf (f, "#gdef fileatt$flag['%s'] %*s %#10x\n", info_ptr->name,
			(int)(10 - strlen (info_ptr->name)), "=", info_ptr->val);
	...
	for (fform = 1; fform < FF_N; fform++)
	{
		...
		fprintf (f,
			...
			fform, name, GetExtFF (0, fform), GetExtFF (fform_compr, fform), GetAttribFF (fform),
			name);
	}
}
...
enumError cmd_float ()
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		printf (" %04x %04x = %12.9g : %04x %04x %04x %04x = %24.18g : |%s|\n", be16 (w4 + 0),
			be16 (w4 + 1), bef4 (w4), be16 (w8 + 0), be16 (w8 + 1), be16 (w8 + 2), be16 (w8 + 3),
			bef8 (w8), arg);
		...
		if (opt_round)
		{
			...
			printf (" %04x %04x = %12.9g : %04x %04x %04x %04x = %22.18g"
				...
				be16 (r4 + 0), be16 (r4 + 1), bef4 (r4), be16 (r8 + 0), be16 (r8 + 1),
				be16 (r8 + 2), be16 (r8 + 3), bef8 (r8));
	...
	else
	{
		...
		if (print_header)
			printf ("\n"
					...
					"%s%.*s%s\n",
				colout->heading, seplen, ThinLine300_3, colout->reset, colout->heading,
				colout->reset, colout->heading, colout->reset, colout->heading, seplen,
				ThinLine300_3, colout->reset);
...
enumError cmd_vr_race ()
{
	...
	if (long_count)
	{
		...
		printf ("\n"
				...
				"%srk. start : %*s= final%s\n"
				"%s%.*s%s\n",
			colout->heading, seplen, ThinLine300_3, colout->reset, colout->heading, -5 * n, "   vr",
			colout->reset, colout->heading, -5 * n, "delta...", colout->reset, colout->heading,
			seplen, ThinLine300_3, colout->reset);
		...
		for (i = 0; i < n; i++)
		{
			...
			for (j = 0; j < n; j++)
			{
				...
				else
				{
					...
					ccp next_col = d < 0 ? colout->b_red : d > 0 ? colout->b_green : colout->reset;
					if (col != next_col)
					{
						col = next_col;
						printf (" %s%4d", col, d);
			...
			printf (" %s= %s%5d%s\n", col == colout->reset ? "" : colout->reset, next_col,
				vr0[i] + delta[i], *next_col ? colout->reset : "");
		}
		printf ("%s%.*s%s\n\n", colout->heading, seplen, ThinLine300_3, colout->reset);
	}
	else
	{
		...
		printf ("\n"
				...
				"%s%.*s%s\n",
			colout->heading, seplen, ThinLine300_3, colout->reset, colout->heading, colout->reset,
			colout->heading, colout->reset, colout->heading, seplen, ThinLine300_3, colout->reset);
...
{
	static const KeywordTab_t tab[] = { { -1, "HELP", "?", 0 },

		//	{  EXPORT_C,		"C",		0,		EXPORT_CMD_MASK },
		{ EXPORT_PHP, "PHP", 0, EXPORT_CMD_MASK },
		{ EXPORT_MAKEDOC, "MAKEDOC", 0, EXPORT_CMD_MASK },

		{ EXPORT_F_FUNCTIONS, "FUNCTIONS", 0, 0 }, { EXPORT_F_FILEATTRIB, "FILEATTRIB", "FA", 0 },
...
int ScanOptRepairMagic (ccp arg)
{
	...
	static const KeywordTab_t keytab[] = { { REPMD_OFF, "OFF", 0, REPMD_M_MODES },
		{ REPMD_ANALYZE, "ANALYZE", 0, REPMD_M_MODES },
		{ REPMD_REPAIR, "REPLACE", "ON", REPMD_M_MODES }, { REPMD_F_FNAME_FIRST, "FNAME", 0, 0 },

		{ 0, 0, 0, 0 } };

	const s64 stat
		= ScanKeywordList (arg, keytab, 0, true, 0, 0, "Option --repair-magic", ERR_SYNTAX);
	if (stat == -1)
		return 1;
...
ccp GetRepairMagicInfo (const repair_ff_t *rep)
{
	...
	char buf[80];
	int len
		= snprintfS (buf, sizeof (buf), "smode=%u%s%s%s, fform=%s, ext=%s, magic=%s/%s", rep->mode,
			  rep->flags & REPMD_F_FNAME_FIRST ? "f" : "", rep->flags & REPMD_R_FNAME ? "F" : "",
			  rep->flags & REPMD_R_FALLBACK ? "X" : "", GetNameFF (rep->fform, 0), rep->file_ext,
			  PrintID (rep->orig_magic, rep->magic_len, 0), PrintID (rep->magic, rep->magic_len, 0))
		+ 1;
...
)
{
	...
	memcpy (rep->magic, data, data_size < sizeof (rep->magic) ? data_size : sizeof (rep->magic));
	...
	DASSERT (ft);
	rep->fform = ft->fform;
	const uint mlen = ft->magic_len < sizeof (rep->magic) ? ft->magic_len : sizeof (rep->magic);
	rep->magic_len = mlen;
	...
	if (rep->mode == REPMD_ANALYZE && rep->magic_len)
	{
		if (logging >= 2)
			fprintf (stdlog, "REPAIR MAGIC: %s -> %s (%s) : %s\n",
				PrintID (data, rep->magic_len, 0), PrintID (rep->magic, rep->magic_len, 0),
...
void SetupPrintScriptByOptions (PrintScript_t *ps)
{
	...
	switch (script_fform)
	{
		...
		case FF_PHP:
			...
			break;
		case FF_MAKEDOC:
			ps->fform = PSFF_MAKEDOC;
			break;
		default:
			ps->fform = print_sections ? PSFF_CONFIG : PSFF_UNKNOWN;
			break;
	}
}
...
CheckMode_t GetCheckMode (bool no_head, // true: suppress header
...
{
	return mode & ~(CMOD_FOOTER | CMOD_FORCE_FOOTER) | CMOD_FORCE_HEADER | CMOD_GLOBAL_STAT;
}
...
const ImageGeometry_t *GetImageGeometry (image_format_t iform)
{
	switch (iform)
	{
		...
		case IMG_IA8:
			return &img_geo_IA8;
		case IMG_RGB565:
			return &img_geo_RGB565;
		case IMG_RGB5A3:
			return &img_geo_RGB5A3;
		case IMG_RGBA32:
			return &img_geo_RGBA32;
		case IMG_C4:
			return &img_geo_C4;
		case IMG_C8:
			return &img_geo_C8;
		case IMG_C14X2:
			return &img_geo_C14X2;
		case IMG_CMPR:
			return &img_geo_CMPR;
		case IMG_X_GRAY:
			return &img_geo_X_GRAY;
		case IMG_X_RGB:
			return &img_geo_X_RGB;
		case IMG_X_PAL4:
			return &img_geo_X_PAL_4;
		case IMG_X_PAL8:
			return &img_geo_X_PAL_8;
		case IMG_X_PAL14:
			return &img_geo_X_PAL_14;
		case IMG_X_PAL:
			return &img_geo_X_PAL_14;
...
ccp PrintImageFormat (image_format_t iform, // image format
...
uint CalcImageSize (uint width, // width of image in pixel
...
const ImageGeometry_t *CalcImageGeometry (image_format_t iform, // image format
...
)
{
	...
	if (geo)
	{
		const uint size = CalcImageSize (width, height, geo->bits_per_pixel, geo->block_width,
			geo->block_height, x_width, x_height, h_blocks, v_blocks);
		if (img_size)
			*img_size = size;
		return geo;
	}
...
const KeywordTab_t cmdtab_image_format[] = { { IMG_I4, "I4", 0, CF_IMAGE },
	{ IMG_I8, "I8", 0, CF_IMAGE }, { IMG_IA4, "IA4", 0, CF_IMAGE }, { IMG_IA8, "IA8", 0, CF_IMAGE },
	{ IMG_RGB565, "RGB565", 0, CF_IMAGE }, { IMG_RGB5A3, "RGB5A3", 0, CF_IMAGE },
	{ IMG_RGBA32, "RGBA32", "RGBA8", CF_IMAGE }, { IMG_C4, "C4", "CI4", CF_IMAGE | CF_PALETTE },
	{ IMG_C8, "C8", "CI8", CF_IMAGE | CF_PALETTE },
	{ IMG_C14X2, "C14X2", "CI14X2", CF_IMAGE | CF_PALETTE }, { IMG_CMPR, "CMPR", 0, CF_IMAGE },

	{ IMG_X_AUTO, "AUTO", 0, CF_AUTO }, { IMG_X_GRAY, "X-GRAY", "GRAY", 0 },
	{ IMG_X_RGB, "X-RGB", "RGB", 0 },

	{ IMG_X_PAL4, "X-PAL4", "PAL4", 0 }, { IMG_X_PAL8, "X-PAL8", "PAL8", 0 },
	{ IMG_X_PAL14, "X-PAL14", "PAL14", 0 }, { IMG_X_PAL, "X-PAL", "PAL", CF_AUTO },
...
const KeywordTab_t cmdtab_palette_format[]
	= { { PAL_IA8, "P-IA8", "PIA8", 0 }, { PAL_IA8, "GRAY", "IA8", 0 },

		  { PAL_RGB565, "P-RGB565", "PRGB565", 0 }, { PAL_RGB565, "COLOR", "RGB565", 0 },

		  { PAL_RGB5A3, "P-RGB5A3", "PRGB5A3", 0 }, { PAL_RGB5A3, "ALPHA", "RGB5A3", 0 },
...
uint GetPaletteCountIF (image_format_t iform // image format
)
{
	switch (iform)
	{
...
static const KeywordTab_t cmdtab_patch_image[] = { // order is important for back calculations

	{ 0, "DEFAULT", "D", PIM_MASK }, { PIM_CENTER, "CENTER", "C", PIM_M_V | PIM_M_H },

	{ PIM_VCENTER, "VCENTER", "V", PIM_M_V }, { PIM_TOP, "TOP", "T", PIM_M_V },
	{ PIM_BOTTOM, "BOTTOM", "B", PIM_M_V }, { PIM_INS_TOP, "INS-TOP", "IT", PIM_M_V },
	{ PIM_INS_BOTTOM, "INS-BOTTOM", "IB", PIM_M_V },

	{ PIM_HCENTER, "HCENTER", "H", PIM_M_H }, { PIM_LEFT, "LEFT", "L", PIM_M_H },
	{ PIM_RIGHT, "RIGHT", "R", PIM_M_H }, { PIM_INS_LEFT, "INS-LEFT", "IL", PIM_M_H },
	{ PIM_INS_RIGHT, "INS-RIGHT", "IR", PIM_M_H },

	{ PIM_MIX, "MIX", "MX", PIM_M_COPY }, { PIM_FOREGROUND, "FOREGROUND", "FG", PIM_M_COPY },
	{ PIM_BACKGROUND, "BACKGROUND", "BG", PIM_M_COPY }, { PIM_COPY, "COPY", "CP", PIM_M_COPY },

	{ PIM_LEAVE, "LEAVE", "LV", PIM_M_SIZE }, { PIM_SHRINK, "SHRINK", "SH", PIM_M_SIZE },
...
int ScanOptPatchImage (ccp arg)
{
	...
	if (eq > arg)
	{
		...
		stat = ScanKeywordList (
			keyword, cmdtab_patch_image, 0, true, 0, 0, "Option --patch", ERR_SYNTAX);
		if (stat == -1)
			return 1;
	}
...
int IterateTrackFiles (FileList_t mode, // bit field with requested files
	IterateTrackFilesFunc func, // call back function
...
)
{
	...
	if (mode & (FLT_OTHER | FLT_OLD))
	{
		...
		static const struct tab_t tab[] = { { 1, FLT_OTHER, "WIN", "winningrun_demo" },
			{ 1, FLT_OTHER, "LOS", "loser_demo" }, { 1, FLT_OTHER, "END", "ending_demo" },
			{ 1, FLT_OTHER, "MAR", "old_mario_gc_b" }, { 1, FLT_OTHER, "RNG", "ring_mission" },

			{ 1, FLT_OLD, "DRW", "draw_demo" }, { 0, FLT_OLD, "HAY", "old_mario_gc_hayasi" },
...
valid_t IsValidSZS (struct szs_file_t *szs, // valid szs archive
	bool warnings // true: print warnings
)
{
	DASSERT (szs);
	return IsValid (
		szs->data, szs->size, szs->file_size, szs, szs->fform_current, warnings ? szs->fname : 0);
}

///////////////////////////////////////////////////////////////////////////////

valid_t IsValid (const void *data, // valid data to test
	uint data_size, // size of 'data'
...
valid_t IsValidBRSUB (const void *data, // data
	uint data_size, // size of 'data'
...
)
{
	...
	noPRINT ("BRSUB: %d.%d.%d.%d\n", isalnum ((int)(bh->magic[0])), isalnum ((int)(bh->magic[1])),
		isalnum ((int)(bh->magic[2])), isalnum ((int)(bh->magic[3])));

	return isalnum ((int)(bh->magic[0])) && isalnum ((int)(bh->magic[1]))
			&& isalnum ((int)(bh->magic[2])) && isalnum ((int)(bh->magic[3]))
		? VALID_OK
		: VALID_ERROR;
}
...
valid_t IsValidBTI (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	ccp fname // not NULL: print warnings with file ref
)
...
valid_t IsValidTEXCT (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	szs_file_t *szs, // not NULL: base archive
	ccp fname // not NULL: print warnings with file ref
...
valid_t IsValidCTCODE (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	ccp fname // not NULL: print warnings with file ref
)
...
valid_t IsValidKMP (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	ccp fname // not NULL: print warnings with file ref
)
{
	...
	PRINT ("M=%.4s n-sect=%d, h-size=%u,%u,%zu  f-size=%u,%u\n", (ccp)data, hi.n_sect, hi.head_size,
		GetFileHeadSizeKMP (hi.n_sect), sizeof (kmp_file_mkw_t), hi.file_size, file_size);
...
valid_t IsValidLEX (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	ccp fname // not NULL: print warnings with file ref
)
...
valid_t IsValidMDL (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	struct szs_file_t *szs, // not NULL: base archive
	ccp fname, // not NULL: print warnings with file ref
...
valid_t IsValidPAT (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	struct szs_file_t *szs, // not NULL: base archive
	ccp fname, // not NULL: print warnings with file ref
...
)
{
	...
	PRINT ("ANA(PAT): valid=%d,%d, N=%d,%d,[%d,%d,%d,%d,%d], "
		...
		ana->valid, ana->data_complete, ana->n_sect0, ana->n_sect1,

		ana->n_s0_list[0], ana->n_s0_list[1], ana->n_s0_list[2], ana->n_s0_list[3],
		ana->n_s0_list[4],
...
valid_t IsValidSTATICR (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	ccp fname // not NULL: print warnings with file ref
)
...
valid_t IsValidLTA (const void *data, // data
	uint data_size, // size of 'data'
	uint file_size, // NULL or size of complete file
	ccp fname // not NULL: print warnings with file ref
)
