...
int main ( int argc, char **argv )
{
    ...
    for ( int i = 3; i < argc; i++ )
    {
        ...
        {
            if ( i + 1 < argc ) target_fmt = ParseSequenceFormatName(argv[++i]);
        }
        else if ( !strcasecmp(argv[i], "--center") || !strcasecmp(argv[i], "-c") )
        {
            if ( i + 1 < argc ) center_note = atoi(argv[++i]);
        }
    ...
    else if ( !strcasecmp(cmd, "asm") || !strcasecmp(cmd, "assemble") || !strcasecmp(cmd, "a") )
    {
        if ( !output_path )
        {
            const char *ext = (target_fmt == SEQ_FMT_CSEQ) ? ".cseq" :
                              ...
                              (target_fmt == SEQ_FMT_SSEQ) ? ".sseq" : ".rseq";
            snprintf(out_buf, sizeof(out_buf), "%.*s%s",
                (int)(strlen(input_path) > 4 && !strcasecmp(input_path + strlen(input_path) - 4, ".txt")
                    ? strlen(input_path) - 4 : strlen(input_path)),
                input_path, ext);
            output_path = out_buf;
        }
        ...
        if ( !err && bin )
        {
            ...
            printf("Assembled %s -> %s (%zu bytes, format: %s)\n",
                input_path, output_path, bin_size, GetSequenceFormatName(target_fmt));
        }
    }
    else if ( !strcasecmp(cmd, "to_midi") || !strcasecmp(cmd, "to-midi") || !strcasecmp(cmd, "mid") )
    {
        if ( !output_path )
        {
    ...
    else if ( !strcasecmp(cmd, "from_midi") || !strcasecmp(cmd, "from-midi") )
    {
        ...
        if ( !err && seq )
        {
            ...
            printf("Converted MIDI %s -> %s (%zu bytes, format: %s)\n",
                input_path, output_path, seq_size, GetSequenceFormatName(target_fmt));
        }
    }
    ...
    else if ( !strcasecmp(cmd, "info") )
    {
        ...
        printf("File: %s\nSize: %zu bytes\nDetected Format: %s\n",
            input_path, raw_size, GetSequenceFormatName(fmt));
    }
    else
    {
