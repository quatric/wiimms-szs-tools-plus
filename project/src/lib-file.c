...
#ifdef __CYGWIN__
	opt_source
		= IsWindowsDriveSpec (source) ? AllocNormalizedFilenameCygwin (source, false) : source;
#else
	opt_source = source;
#endif
...
{
#ifdef __CYGWIN__
	opt_dest = IsWindowsDriveSpec (dest) ? AllocNormalizedFilenameCygwin (dest, false) : dest;
#else
	opt_dest = dest;
#endif
	opt_mkdir = mkdir;
...
static bool is_seekable (File_t *F // file structure
)
{
	mode_t mode = F->st.st_mode;
	F->is_seekable = (S_ISREG (mode) || S_ISBLK (mode) || S_ISCHR (mode)) && F->st.st_size
...
enumError OpenFILE (File_t *f, // file structure
...
FileMode_t GetFileModeByOpt (int testmode, // test mode
...
enumError CreateFileOpt (File_t *f, // file structure
	bool initialize, // true: initialize 'f'
	ccp fname, // file to open
	bool testmode, // true: don't open
...
enumError CreateFileOptAppend (File_t *f, // file structure
	bool initialize, // true: initialize 'f'
	ccp fname, // file to open
	bool testmode, // true: don't open
	ccp srcfile // NULL or src file name
...
enumError AppendFILE (File_t *f, // file structure
	bool initialize, // true: initialize 'f'
	ccp fname, // file to open
	bool testmode, // true: don't open
	bool create_path // create path if necessary
...
char *SplitSubPath (char *buf, // destination buffer
	size_t buf_size, // size of 'buf'
	ccp path // source path, if NULL: use 'buf'
)
{
	char *end = path && path != buf ? StringCopyS (buf, buf_size, path) : buf + strlen (buf);
...
enumError OpenReadFILE (ccp path1, // NULL or part #1 of path
...
enumError LoadFILE (ccp path1, // NULL or part #1 of path
	ccp path2, // NULL or part #2 of path
...
enumError WriteFILE (FILE *f, // valid file for output
...
enumError SaveFILE (ccp path1, // NULL or part #1 of path
	ccp path2, // NULL or part #2 of path
...
enumError SaveFILE2 (FILE *f, // output file, if NULL use path1+path2
...
{
	DASSERT (data);
	return f ? WriteFILE (f, path1, path2, data, data_size)
...
ccp GetNameFF (file_format_t ff1, file_format_t ff2)
{
	...
	noPRINT ("GetNameFF(%u,%u) -> '%s' [tab]\n", ff1, ff2,
		FileTypeTab[ff2 && (uint)ff2 < FF_N ? ff2
				: (uint)ff1 < FF_N			? ff1
											: 0]
			.name);
...
ccp GetExtFF (file_format_t ff1, file_format_t ff2)
{
	...
	{
		return ff1 == FF_BZ	   ? (ff2 == FF_WU8 ? ".wbz" : ".bz")
			: ff1 == FF_LZ	   ? (ff2 == FF_WU8 ? ".wlz" : ".lz")
			: (uint)ff2 < FF_N ? FileTypeTab[ff2].ext_compr
							   : ".szs";
	}
	...
	return (uint)ff1 < FF_N ? FileTypeTab[ff1].ext : ".bin";
}
...
{
	return ff > FF_UNKNOWN && ff < FF_N && FileTypeTab[ff].magic_len ? (ccp)(FileTypeTab[ff].magic)
																	 : 0;
}
...
{
	return ff > FF_UNKNOWN && ff < FF_N && FileTypeTab[ff].fform_bin ? FileTypeTab[ff].fform_bin
																	 : FF_UNKNOWN;
}
...
{
	return ff > FF_UNKNOWN && ff < FF_N && FileTypeTab[ff].fform_txt ? FileTypeTab[ff].fform_txt
																	 : FF_UNKNOWN;
}
...
ccp GetMagicExtFF (file_format_t ff1, file_format_t ff2)
{
	...
	return (uint)ff1 < FF_N ? FileTypeTab[ff1].ext_magic : ".bin";
}
...
)
{
	...
	if (data_size >= 8)
	{
		if (IsBZ (data, data_size) >= 0)
			return FF_BZ;
		if (IsBZIP2 (data, data_size) >= 0)
			return FF_BZIP2;
		if (IsLZ (data, data_size) >= 0)
			return FF_LZ;
		if (IsLZMA (data, data_size) >= 0)
			return FF_LZMA;
		...
		switch (magic64)
		{
			...
			case GCT_MAGIC8_NUM:
				return FF_GCT;
			case ADDR_PORT_MAGIC_NUM:
				return FF_PORTDB;
			case 0x4d7367537464426eULL:
				return FF_MSBT; // MsgStdBn
			case 0x4d736750726a426eULL:
				return FF_MSBP; // MsgPrjBn
			case 0x4d7367466c77426eULL:
				return FF_MSBF; // MsgFlwBn
			case 0x23204d5342543a20ULL:
				return FF_MSBT_TXT; // "# MSBT: "
			case 0x23204d5342503a20ULL:
				return FF_MSBP_TXT; // "# MSBP: "
			case 0x23204d5342463a20ULL:
				return FF_MSBF_TXT; // "# MSBF: "

			//--- see below for BOM support
			case CT_DEF_MAGIC8_NUM:
				return FF_CTDEF;
			case DISTRIB_MAGIC8_NUM:
				return FF_DISTRIB;
			case ITEMSLT_TEXT_MAGIC_NUM:
				return FF_ITEMSLT_TXT;
			case OBJFLOW_TEXT_MAGIC8_NUM:
				return FF_OBJFLOW_TXT;
			case GH_ITEM_TEXT_MAGIC8_NUM:
				return FF_GH_ITEM_TXT;
			case GH_IOBJ_TEXT_MAGIC8_NUM:
				return FF_GH_IOBJ_TXT;
			case GH_KART_TEXT_MAGIC8_NUM:
				return FF_GH_KART_TXT;
			case GH_KOBJ_TEXT_MAGIC8_NUM:
				return FF_GH_KOBJ_TXT;
			case LE_LPAR_MAGIC_NUM:
				return FF_LPAR;
			case LE_DEFINE_MAGIC8_NUM:
				return FF_LEDEF;
			case LE_DISTRIB_MAGIC8_NUM:
				return FF_LEDIS;
			case LE_REFERENCE_MAGIC8_NUM:
				return FF_LEREF;
			case LE_STRINGS_MAGIC8_NUM:
				return FF_LESTR;
			case LE_SHA1REF_MAGIC8_NUM:
				return FF_SHA1REF;
			case LE_SHA1ID_MAGIC8_NUM:
				return FF_SHA1ID;
			case LE_PREFIX_MAGIC8_NUM:
				return FF_PREFIX;
			case LE_MTCAT_MAGIC8_NUM:
				return FF_MTCAT;
			case LE_CT_SHA1_MAGIC8_NUM:
				return FF_CT_SHA1;
			case USELTA_MAGIC_NUM:
				return FF_USE_LTA;
			case LTA_MAGIC_NUM:
				return FF_LTA;
		}
	...
	if (data_size >= 4)
	{
		...
		switch (magic32)
		{
			...
			case BMG_TEXT_MAGIC_NUM:
				return FF_BMG_TXT;
			case KCL_TEXT_MAGIC_NUM:
				return FF_KCL_TXT;
			case KMP_TEXT_MAGIC_NUM:
				return FF_KMP_TXT;
			case MDL_TEXT_MAGIC_NUM:
				return FF_MDL_TXT;
			case PAT_TEXT_MAGIC_NUM:
				return FF_PAT_TXT;
			case CHR_TEXT_MAGIC_NUM:
				return FF_CHR_TXT;
			case SRT_TEXT_MAGIC_NUM:
				return FF_SRT_TXT;
			case VIS_TEXT_MAGIC_NUM:
				return FF_VIS_TXT;
			case CLR_TEXT_MAGIC_NUM:
				return FF_CLR_TXT;
			...
			case KRT_TEXT_MAGIC_NUM:
				return FF_KRT_TXT;
			case KRM_TEXT_MAGIC_NUM:
				return FF_KRM_TXT;
			case BFLYT_TEXT_MAGIC_FLYT:
				return FF_BFLYT_TXT;
			case BFLYT_TEXT_MAGIC_FLAN:
				return FF_BFLYT_TXT;
			case BRLYT_TEXT_MAGIC_RLYT:
				return FF_BFLYT_TXT;
			case BRLYT_TEXT_MAGIC_RLAN:
				return FF_BFLYT_TXT;
			case BCLYT_TEXT_MAGIC_CLYT:
				return FF_BCLYT_TXT;
			case BCLYT_TEXT_MAGIC_CLAN:
				return FF_BCLYT_TXT;

			case GCTTXT_MAGIC_NUM:
				return FF_GCT_TXT;

			case YAZ0_MAGIC_NUM:
				return IsBZIP2 (data + sizeof (ybz_header_t), data_size - sizeof (ybz_header_t))
						>= 0
					? FF_YBZ
					: IsYLZ (data, data_size) >= 0 ? FF_YLZ
												   : FF_YAZ0;
			...
			case KMP_MAGIC_NUM:
				return FF_KMP;
			case BRASD_MAGIC_NUM:
				return FF_BRASD;

			case LE_BINARY_MAGIC_NUM:
				return FF_LE_BIN;
			case LEX_BIN_MAGIC_NUM:
				return FF_LEX;
			case LEX_TEXT_MAGIC_NUM:
				return FF_LEX_TXT;
			case LFL_MAGIC_NUM:
				return FF_LFL;

			case CT0_CODE_MAGIC_NUM:
				return FF_CT0_CODE;
			case CT0_DATA_MAGIC_NUM:
				return FF_CT0_DATA;
			case CT1_CODE_MAGIC_NUM:
				return FF_CT1_CODE;
			case CT1_DATA_MAGIC_NUM:
				return FF_CT1_DATA;

			case CT_CUP1_MAGIC_NUM:
				return FF_CUP1;
			case CT_CRS1_MAGIC_NUM:
				return FF_CRS1;
			case CT_MOD1_MAGIC_NUM:
				return FF_MOD1;
			case CT_MOD2_MAGIC_NUM:
				return FF_MOD2;
			case CT_OVR1_MAGIC_NUM:
				return FF_OVR1;
			...
			// CGFX / BCRES: the other 3DS graphics container
			...
			// Nintendo Switch XTX Texture Container (DFvN)
			...
			case BREFT_MAGIC_NUM:
				if (file_size >= 0x20)
					return data_size < 0x14 || !memcmp (data + 0x10, BREFT_MAGIC, 4) ? FF_BREFT
																					 : FF_UNKNOWN;
				break;
		}
	...
	if (data_size >= sizeof (breft_image_t) && !be32 (data))
	{
		...
		if (geo && !geo->is_x)
		{
			...
			const uint xwidth
				= (width + geo->block_width - 1) / geo->block_width * geo->block_width;
			const uint height = be16 (&bi->height);
			const uint xheight
				= (height + geo->block_height - 1) / geo->block_height * geo->block_height;
			const uint size = be32 (&bi->img_size);
			noPRINT ("RAW IMG FOUND: %02x, %u*%u -> %u*%u, %u/%u\n", bi->iform, width, height,
				xwidth, xheight, xwidth * xheight * geo->bits_per_pixel / 8, size);
			if (xwidth > 0 && xheight > 0 && xwidth * xheight * geo->bits_per_pixel <= 8 * size)
			{
				return FF_BREFT_IMG;
			}
		}
	...
	if (data_size > 3 && data8[1] == 12 && data8[2] == 19)
	{
		...
		if (data8[0] == 6 && file_size == ITEMSLT_SIZE6
			|| data8[0] == 12 && (file_size == ITEMSLT_SIZE12 || file_size == ITEMSLT_SIZE6))
		{
			return FF_ITEMSLT;
		}
	}
	...
	// test FF_WAV_OBJ & FF_SKP_OBJ
	{
		...
		if (end - ptr >= 7 && (!memcmp (ptr, "mtllib", 6) || !memcmp (ptr, "usemtl", 6))
			&& (ptr[6] == ' ' || ptr[6] == '\t'))
		{
			return FF_WAV_OBJ;
		}
	...
	if (beg < end)
	{
		...
		if (is_text)
		{
			...
			if (!memcmp (beg, "#!", 2))
			{
				...
				{
					case 2:
						if (!strncasecmp (name, "sh", 2))
							return FF_SH;
						break;

					case 3:
						if (!strncasecmp (name, "php", 3))
							return FF_PHP;
						break;

					case 4:
						if (!strncasecmp (name, "bash", 4))
							return FF_BASH;
						break;

					case 7:
						if (!strncasecmp (name, "makedoc", 7))
							return FF_MAKEDOC;
						break;
				}
				return FF_SCRIPT;
...
)
{
	...
	if (err <= ERR_WARNING)
	{
		...
		if (ext && (!strcasecmp (ext, ".msh") || !strcasecmp (ext, ".mod") || !strcasecmp (ext, ".glg") || !strcasecmp (ext, ".rlg") || !strcasecmp (ext, ".hgo") || !strcasecmp (ext, ".pers") || !strcasecmp (ext, ".tvol") || !strcasecmp (ext, ".txe")))
		{
...
)
{
	...
	for (p = FileTypeTab; p->name; p++)
		if ((!need_magic || p->magic_len) && p->ext && !strcasecmp (ext, p->ext + 1))
			return p;

	// second: try member 'ext_compr'
	for (p = FileTypeTab; p->name; p++)
		if ((!need_magic || p->magic_len) && p->ext_compr && !strcasecmp (ext, p->ext_compr + 1))
			return p;

	// third: try member 'ext_magic'
	for (p = FileTypeTab; p->name; p++)
		if ((!need_magic || p->magic_len) && p->ext_magic && !strcasecmp (ext, p->ext_magic + 1))
			return p;
...
)
{
	...
	if (p1)
	{
		...
		if (len < sizeof (name))
		{
			...
			const file_type_t *p;
			for (p = FileTypeTab; p->name; p++)
				if ((!need_magic || p->magic_len) && p->subdir && !strcasecmp (name, p->subdir))
					return p;
		}
	}
	return 0;
...
file_format_t GetImageFF (file_format_t fform1, // if a valid image ff, use this one
...
{
	PRINT ("GetImageFF(%s,%s,%s,%d,%s) %s\n", GetNameFF (0, fform1), GetNameFF (0, fform2),
		GetNameFF (0, default_fform), allow_png, GetNameFF (0, not_found), path ? path : "-");
	...
	if (path && *path)
	{
		...
		{
			// [[analyse-magic]]
			const file_format_t fform = IsImageFF (GetByMagicFF (buf, sizeof (buf), 0), allow_png);
			if (fform != FF_UNKNOWN)
				return fform;
		}
...
ccp LogObjType (const object_type_t *ot)
{
	...
	char buf[100];
	const uint len = snprintf (buf, sizeof (buf), "%s[%s%s%s%s] rs=%u off:%x:%x %s,ns:%x,mo:%x",
		GetNameFF (0, ot->fform), ot->is_objflow ? "O" : "", ot->is_geohit ? "G" : "",
		ot->is_kart			 ? "K"
			: ot->is_objflow ? ""
							 : "I",
		ot->is_obj ? "o" : "", ot->row_size, ot->slot_offset, ot->ref_offset,
		GetNewObjectName (ot->new_object), ot->object_slots, ot->max_object_id);
...
file_format_t GetObjTypeFF (const object_type_t *ot, bool want_text)
{
	...
	if (ot->is_geohit)
	{
		if (ot->is_kart)
		{
			return ot->is_obj ? (want_text ? FF_GH_KOBJ_TXT : FF_GH_KOBJ)
							  : (want_text ? FF_GH_KART_TXT : FF_GH_KART);
		}
		else
		{
			return ot->is_obj ? (want_text ? FF_GH_IOBJ_TXT : FF_GH_IOBJ)
...
static const KeywordTab_t opt_patch_tab[] = { { 0, "NONE", "CLEAR", PFILE_M_ALL | PFILE_F_HIDE },
	{ PFILE_M_DEFAULT, "DEFAULT", 0, PFILE_M_ALL | PFILE_F_HIDE },
	{ PFILE_M_ALL, "ALL", 0, PFILE_M_ALL | PFILE_F_HIDE }, { PFILE_F_LOG, "LOG", 0, 0 },
	{ PFILE_F_LOG_ALL, "LOG-ALL", "LOGALL", 0 },

	{ PFILE_COURSE_KMP, "KMP", 0, 0 },

	{ PFILE_M_KCL, "ALL-KCL", "ALLKCL", PFILE_M_KCL }, { PFILE_COURSE_KCL, "KCL", 0, 0 },
	{ PFILE_OTHER_KCL, "OTHER-KCL", "OTHERKCL", 0 },

	{ PFILE_M_BRRES, "ALL-BRRES", "ALLBRRES", PFILE_M_BRRES }, { PFILE_MODEL, "MODEL", 0, 0 },
	{ PFILE_MINIMAP, "MINIMAP", "MAP", 0 }, { PFILE_VRCORN, "VRCORN", 0, 0 },
...
int ScanOptPatchFiles (ccp arg)
{
	...
	s64 stat = ScanKeywordList (
		arg, opt_patch_tab, 0, true, 0, PATCH_FILE_MODE, "Option --patch-file", ERR_SYNTAX);
	if (stat != -1)
	{
...
patch_file_t GetFileClass (file_format_t fform, // not FF_UNKNOWN: check file type
...
)
{
	...
	{
		if (!strcasecmp (fname, "course_model.brres") || !strcasecmp (fname, "course_d_model.brres")
			|| !strcasecmp (fname, "course_model.d") || !strcasecmp (fname, "course_d_model.d"))
		{
			return PFILE_MODEL;
		}

		if (!strcasecmp (fname, "map_model.brres") || !strcasecmp (fname, "map_model.d"))
		{
			return PFILE_MINIMAP;
		}

		if (!strcasecmp (fname, "vrcorn_model.brres") || !strcasecmp (fname, "vrcorn_model.d"))
		{
			return PFILE_VRCORN;
		}
...
bool PatchFileClass (file_format_t fform, // not FF_UNKNOWN: check file type
	ccp fname // NULL or file name to check
)
{
...
bool PATCH_ACTION_LOG (ccp action, ccp object, ccp format, ...)
{
		...
		&& (verbose > 2 || PATCH_FILE_MODE & (PFILE_F_LOG | PFILE_F_LOG_ALL)))
	{
		fflush (stdout);
		fprintf (
			stdlog, "    %s>[PATCH] %-6s %-7s%s ", colset->heading, action, object, colset->info);
		if (format)
		{
			va_list arg;
			va_start (arg, format);
...
enumError ScanConfig (config_t *config, // store configuration here
...
)
{
	...
	const ListDef_t list_def[] = { { "PATHS", 0, 0, &paths }, { 0, 0, 0, 0 } };

	const enumError err = path && *path ? ScanSetupFile (0, list_def, path, 0, 0, silent) : ERR_OK;
	...
	for (assign = assign_tab; assign->name; assign++)
	{
		if (assign->var)
		{
			...
			if (ref)
			{
				...
				NormalizeFileName (
					buf, sizeof (buf), ref->data.data.ptr, true, true, TRSL_ADD_AUTO);
...
void SearchConfigHelper (search_file_list_t *sfl, int stop_mode)
{
	...
	ccp opt[] = {
		opt_config,
		opt_install ? "$(prog)/install.conf" : 0,
		0,
	};
	ccp xdg_home = "szs";
	ccp home = ".szs";
	...
	SearchConfig (
		sfl, CONFIG_FILE, opt, 2, &xdg_home, 1, &home, 1, etc, 2, etc + 1, 1, 0, 0, stop_mode);
	if (logging >= 4)
	...
	if (f && config)
	{
		...
		if (verbose)
			fprintf (f,
				...
				"  Install path:   %s\n"
				"  Install config: %s\n",
				config->base_path, config->install_path, config->install_config);
		fprintf (f,
			"  Share path:     %s\n"
			"  Auto-add path:  %s\n",
			config->share_path, config->autoadd_path);
	}
}

//-----------------------------------------------------------------------------

struct config_offset_t
{
	uint mode;
	ccp name;
	uint offset;
...
static const struct config_offset_t config_offset_tab[]
	= { { 0, "config", offsetof (config_t, config_file) },
		  ...
		  { 1, "autoadd", offsetof (config_t, autoadd_path) }, { 0, 0 } };
...
void PrintConfigScript (FILE *f, const config_t *config)
{
	if (f && config)
	{
		...
		PrintScriptVars (&ps, 3,
			...
			"share=\"%s\"\n"
			"autoadd=\"%s\"\n",
			config->base_path, config->install_path,
			config->install_config ? config->install_config : "", config->share_path,
			config->autoadd_path);
	}
}
	...
	if (f && config)
	{
		...
		if (brief_count > 0)
			fprintf (f,
				...
				"share\t= %s\n"
				"autoadd\t= %s\n",
				dest.base_path, dest.install_path, dest.install_config, dest.share_path,
				dest.autoadd_path);
		else
			fprintf (f, text_config_paths_cr, dest.base_path, dest.install_path,
				dest.install_config, dest.share_path, dest.autoadd_path);
...
enumError DecompressManager (CompressManager_t *mgr // manager data
)
{
	DASSERT (mgr);
...
{
	return (uint)ff < FF_N ? FileTypeTab[ff].attrib : (ff_attrib_t)0;
}

///////////////////////////////////////////////////////////////////////////////

ff_attrib_t GetAttribByMagicFF (const void *data, // valid pointer to data
	uint data_size, // size of 'data'
		...
		case FF_LE_BIN:
		{
			...
			if (res_suffix)
			{
				if (vers == 4 && data_size >= sizeof (le_binary_head_v4_t))
				{
					...
					*res_suffix
						= leh->build_mode == 'R' ? toupper (leh->region) : tolower (leh->region);
				}
			}
			return vers;
		}
...
{
	// [[analyse-magic]]
	return GetVersionFF (GetByMagicFF (data, data_size, 0), data, data_size, res_suffix);
}
...
{
	return ff == FF_GH_KART || ff == FF_GH_KART_TXT || ff == FF_GH_KOBJ || ff == FF_GH_KOBJ_TXT;
}
...
{
	return ff == FF_GH_IOBJ || ff == FF_GH_IOBJ_TXT || ff == FF_GH_KOBJ || ff == FF_GH_KOBJ_TXT;
}
...
{
	return fform_compr_force && (ff == FF_YAZ0 || ff == FF_YAZ1 || ff == FF_XYZ) ? fform_compr_force
		: IsYazFF (ff)															 ? ff
																				 : FF_YAZ0;
}
...
{
	return fform_compr_force && (uint)ff < FF_N && FileTypeTab[ff].attrib & FFT_COMPRESS
		? fform_compr_force
		: ff;
}
...
void SetCompressionFF (file_format_t ff_arch, // if >=0: set 'opt_fform'
...
size_t ResetSetup (SetupDef_t *list // object list terminated with an element 'name=NULL'
)
{
	DASSERT (list);
	size_t count;
...
enumError ScanSetupFile (SetupDef_t *sdef, // object list terminated with an element 'name=NULL'
...
static void SetupParamOptions (SetupParam_t *sp // setup param to scan
)
{
	//--- use options?
...
enumError ScanSetupParam (SetupParam_t *sp, // setup param to scan
...
)
{
	...
	SetupDef_t setup_def[] = { { "object-name", 0 }, { "object-id", 1 }, { "archive-format", 0 },
		{ "file-format", 0 }, { "version", 1 }, { "have-pt-dir", 1 }, { "min-data-offset", 1 },
		{ "max-data-offset", 1 }, { "data-align", 1 }, { 0, 0 } };
	...
	if (!opt_ignore_setup)
	{
		...
		const ListDef_t list_def[] = { { "include-pattern", 0, &sp->include_pattern },
			{ "include", 0, &sp->include_name }, { "exclude-pattern", 0, &sp->exclude_pattern },
			{ "exclude", 0, &sp->exclude_name }, { "encode", 0, &encode_helper },
			{ "create", 0, &sp->create_list }, { "order", 1, &order_helper }, { 0, 0 } };
...
static uint FindSubDirHelper (const SubDirList_t *sdl, mem_t path, bool *p_found)
{
	int beg = 0;
	if (sdl && path.len)
	{
...
static uint FindSubFileHelper (const SubFileList_t *sfl, mem_t path, bool *p_found)
{
	int beg = 0;
	if (sfl && path.len)
	{
...
static int SaveSubFile (SubFileIterator_t *sfi, // iterator data
...
)
{
	if (!mode && file && file->data)
	{
		...
		return SaveFile ((ccp)sfi->user_ptr, sfi->path, FM_REMOVE | FM_OVERWRITE | FM_MKDIR,
...
int ScanOptCoding64 (ccp arg)
{
	...
	static const KeywordTab_t ktab[]
		= { { ENCODE_OFF, "DEFAULT", 0, 0 }, { ENCODE_BASE64, "STANDARD", "STD", 0 },
			  { ENCODE_BASE64URL, "URL", 0, 0 }, { ENCODE_BASE64STAR, "STAR", 0, 0 },
			  { ENCODE_BASE64XML, "XML", 0, 0 }, { ENCODE_HEX, "HEX", 0, 0 }, { 0, 0, 0, 0 } };
...
EncodeMode_t SetupCoding64 (EncodeMode_t mode, EncodeMode_t fallback)
{
	...
	case ENCODE_HEX:
		...
		default:
			...
			return SetupCoding64 (fallback == ENCODE_OFF ? ENCODE_BASE64 : fallback, ENCODE_OFF);
	}
	return mode;
}
...
void DumpSlotInfo (FILE *f, // valid output stream
...
)
{
	...
	fprintf (f,
		...
		"%*s"
		"Music info:  %#4x [%u] %s\n",
		indent, "", si->slot_attrib, indent, "", si->race_slot, si->race_mode, si->race_info,
		indent, "", si->arena_slot, si->arena_mode, si->arena_info, indent, "", si->edit_slot,
		si->edit_mode, si->edit_info, indent, "", si->music_index, si->music_mode, si->music_info);
}
...
void AnalyzeSlotAttrib (slot_info_t *si, bool reset_si, mem_t attrib)
{
	...
	for (int i = 0; i < n_attrib; i++)
	{
		...
		switch (len)
		{
			...
			case 3:
				if (src[0] == 'r' && src[1] >= '1' && src[1] <= '8' && src[2] >= '1'
					&& src[2] <= '4')
				{
				...
				else if (src[0] == 'a' && src[1] >= '1' && src[1] <= '2' && src[2] >= '1'
					&& src[2] <= '5')
				{
				...
				else if (src[0] == 'e' && src[1] >= '1' && src[1] <= '8' && src[2] >= '1'
					&& src[2] <= '4')
				{
				...
				else if (src[0] == 'm' && src[1] >= '1' && src[1] <= '8' && src[2] >= '1'
					&& src[2] <= '4')
				{
			...
			case 4:
				if (src[0] == 'e' && src[1] == 'a' && src[2] >= '1' && src[2] <= '2'
					&& src[3] >= '1' && src[3] <= '5')
				{
				...
				else if (src[0] == 'm' && src[1] == 'a' && src[2] >= '1' && src[2] <= '2'
					&& src[3] >= '1' && src[3] <= '5')
				{
					si->music_mode = SIT_MANDATORY;
					StringCopySM (si->music_info, sizeof (si->music_info), src, 4);
				}
				else if (src[0] == 'm' && src[1] == 't' && src[2] >= '1' && src[2] <= '8'
					&& src[3] >= '1' && src[3] <= '4')
				{
					si->music_mode = SIT_MANDATORY;
...
void FinalizeSlotInfo (slot_info_t *si, bool minus_for_empty)
{
	...
	char slot_attrib[sizeof (si->slot_attrib) + 4], *dest = slot_attrib,
													*end = slot_attrib + sizeof (slot_attrib);
	if (si->race_info[0])
	...
	{
		if (!*si->race_info)
			si->race_info[0] = '-', si->race_info[1] = 0;
		if (!*si->arena_info)
			si->arena_info[0] = '-', si->arena_info[1] = 0;
		if (!*si->edit_info)
			si->edit_info[0] = '-', si->edit_info[1] = 0;
		if (!*si->music_info)
			si->music_info[0] = '-', si->music_info[1] = 0;
	}
}
...
const mkw_prefix_t *ScanPrefixTable (ScanText_t *st)
{
	...
	while (ptr < tab + n_elem < n_elem && NextLineScanText (st))
	{
		...
		PRINT0 ("LINE %d/%d: %.*s | %.*s | %.*s | %.*s | %.*s |\n", lineno, n_elem,
			src[IDX_PREFIX].len, src[IDX_PREFIX].ptr, src[IDX_INFO].len, src[IDX_INFO].ptr,
			src[IDX_FLAGS].len, src[IDX_FLAGS].ptr, src[IDX_ORDER].len, src[IDX_ORDER].ptr,
		...
		if (n > IDX_FLAGS)
		{
			...
			if (memchr (f.ptr, 'N', f.len))
				flags |= MPF_NINTENDO;
			if (memchr (f.ptr, 'M', f.len))
				flags |= MPF_MARIOKART;
			if (memchr (f.ptr, '1', f.len))
				flags |= MPF_PREFIX1;
			if (memchr (f.ptr, '2', f.len))
				flags |= MPF_PREFIX2;
			ptr->flags = flags;
		}
		ptr++;
...
enumError SavePrefixTable (FILE *f, const mkw_prefix_t *tab)
{
	...
	for (; tab->info; tab++)
	{
		...
		fprintf (f, "%s|%s|%s%s%s%s|%u|%u|\r\n", prefix.data.ptr, info.data.ptr,
			tab->flags & MPF_NINTENDO ? "N" : "", tab->flags & MPF_MARIOKART ? "M" : "",
			tab->flags & MPF_PREFIX1 ? "1" : "", tab->flags & MPF_PREFIX2 ? "2" : "", tab->order,
			tab->color);
...
#if HAVE_PRINT0
		{
			printf ("N=%d/%d, grow=%d, sort=%d, list=%s\n", clist->atm.used, clist->atm.size,
				clist->atm.grow, clist->atm.sort, clist->atm.list ? "yes" : "no");
...
mkw_category_list_t ScanCategoryList (struct ScanText_t *st)
{
	...
	while (NextLineScanText (st))
	{
		...
		PRINT0 ("LINE: %.*s | %.*s | %.*s | %.*s | %.*s | %.*s | %.*s | %.*s | %.*s | %.*s | %.*s "
				"| %.*s |\n",
			src[IDX_MTCAT].len, src[IDX_MTCAT].ptr, src[IDX_MODE].len, src[IDX_MODE].ptr,
			src[IDX_BAN].len, src[IDX_BAN].ptr, src[IDX_CHAR1].len, src[IDX_CHAR1].ptr,
			src[IDX_CHAR2].len, src[IDX_CHAR2].ptr, src[IDX_ABBREV].len, src[IDX_ABBREV].ptr,
			src[IDX_NAME].len, src[IDX_NAME].ptr, src[IDX_INFO].len, src[IDX_INFO].ptr,
			src[IDX_FG_COL].len, src[IDX_FG_COL].ptr, src[IDX_BG_COL].len, src[IDX_BG_COL].ptr,
			src[IDX_COL_NAME].len, src[IDX_COL_NAME].ptr, src[IDX_ATTRIB].len, src[IDX_ATTRIB].ptr);
...
enumError SaveCategoryList (FILE *f, const mkw_category_list_t *clist)
{
	...
	fprintf (f,
		"#MTCAT03\r\n"
		"@CONTENT=category|mode|ban|char1|char2|abbrev|name|info|fg_color|bg_color|color_name|"
		"attribs|\r\n"
	...
	for (int idx = 0; idx < clist->used; idx++)
	{
		...
		fprintf (f, "%u|%u|%u|%s|%s|%s|%s|%s|0x%06x|0x%06x|%s|%s|\r\n", tcat->mtcat, tcat->mode,
			tcat->ban_type, ch1.data.ptr, ch2.data.ptr, abbrev.data.ptr, name.data.ptr,
			info.data.ptr, tcat->fg_color, tcat->bg_color, colname.data.ptr, attrib.data.ptr);
...
void ScanSha1Ref (struct ScanText_t *st)
{
	...
	while (NextLineScanText (st))
	{
		...
		PRINT_IF0 (!(s1ref_used % 1000), "LINE[%u]: %.*s | %.*s | %.*s | %.*s | %.*s |\n",
			s1ref_used, src[IDX_SHA1].len, src[IDX_SHA1].ptr, src[IDX_TRACK].len,
			src[IDX_TRACK].ptr, src[IDX_FAMILY].len, src[IDX_FAMILY].ptr, src[IDX_CLAN].len,
			src[IDX_CLAN].ptr, src[IDX_CLASS].len, src[IDX_CLASS].ptr);
	...
	if (logging >= 2)
		fprintf (stdlog, "s1ref_list: %u/%u records (%4.2f MB) loaded, max=%u,%u,%u\n", s1ref_used,
			s1ref_size, s1ref_used * sizeof (*s1ref_list) * 1e-6, s1ref_max1_track,
			s1ref_max1_family, s1ref_max1_clan);
...
void AnalyseSPF (split_filename_t *spf, // valid pointer
	...
	*p1 = 0;
	...
	if (p1)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (p.len == 4 && !memcmp (p.ptr + 1, "lap", 3)
				|| p.len == 5 && !memcmp (p.ptr + 1, "laps", 4))
			{
	...
	if (p1)
	{
		...
		{
			case 1:
				valid = !memcmp (buf, "v", 1) || !memcmp (buf, "t", 1);
				break;

			case 2:
				valid = !memcmp (buf, "rc", 2) || !memcmp (buf, "vx", 2);
				break;

			case 3:
				valid
					= !memcmp (buf, "pre", 3) || !memcmp (buf, "poc", 3) || !memcmp (buf, "rcx", 3);
				break;

			case 4:
				valid = !memcmp (buf, "beta", 4) || !memcmp (buf, "test", 4)
					|| !memcmp (buf, "vjam", 4);
				break;
...
exmem_t PrintSPF (exmem_dest_t *dest, // Kind of destination, if NULL then MALLOC()
	const split_filename_t *spf, // Valid and initialized source
		...
		else
		{
			switch (par->split_level)
			{
				...
				case 3:
					...
					break;
				case 4:
					format = "%y%|%N%v%{e%c%|%!A%|%D%E|";
					break;
				case 5:
					format = "%y%|%N%v%{e%|%!c%|%!A%|%D%E|";
					break;
				case 6:
					format = "%y%|%N%{e%|%v%|%!c%|%!A%|%D%E|";
					break;
				case 7:
					format = "%y%|%N%{e%|%v%|%!a%|%!d%|%!A%|%D%E|";
					break;
				case 8:
					format = "%y%|%N%|%v%|%!e%|%!a%|%!d%|%!A%|%D%E|";
					break;
				case 9:
					format = "%y%|%P%b%p%|%n%|%v%|%!e%|%!a%|%!d%|%!A%|%D%E|";
					break;
				case 10:
					format = "%y%|%P%b%p%|%n%|%v%|%!e%|%!a%|%!d%|%!A%|%D%|%E|";
					break;
				...
				case 12:
					...
					break;
				case 13:
					format = "%y%|%P%|%b%|%g%|%G%|%n%|%v%|%!e%|%!a%|%!d%|%!A%|%D%|%E|";
					break;
			}
		}
	}
		...
		for (;;)
		{
			...
			end_scan_flags:;
				...
				switch (*format++)
				{
					...
					case 'a': // authors: list of authors, format "(authors)", options=raw, curly
							  // braces
						src = spf->authors;
						braces = "()";
						break;
					...
					case 'c': // combined authors + editors: format "(authors,,editors)",
							  // options=raw
						src = spf->authors;
						src2 = spf->editors;
						pre2.ptr = ",,";
					...
					case 'l': // lap_count
					{
						...
						if (always > 1 || laps > 0 && (always || laps != 3))
						{
							if (!laps)
								laps = 3;
							src.ptr = tempbuf;
							src.len = snprintf (tempbuf, sizeof (tempbuf), "%u%s%s", laps,
								no_braces ? "" : "lap",
								no_braces		? ""
									: laps != 1 ? "s"
									: fw > 4	? " "
					...
					{
						float speed
							= par->speed_factor > 0.0 ? par->speed_factor : spf->speed_factor;
				...
				else if (src.len)
				{
					...
					if (add_space > 0
						|| add_space == 0 && auto_space && ptr > begin && (u8)ptr[-1] > ' ')
						*ptr++ = ' ';
					...
					{
						exmem_t res
							= AlignUTF8 (&exdest, src.ptr, src.len, left_adjust ? -fw : fw, prec);
...
exmem_t PrintNameSPF (exmem_dest_t *dest, // Kind of destination, if NULL then MALLOC()
