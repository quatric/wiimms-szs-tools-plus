...
void SetTrackBMG (ctcode_t *ctcode, // valid CTCODE
...
static inline void SetTrackFile (ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	ccp text // any text
)
{
...
static inline void SetTrackString (ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	ccp text // any text
)
{
...
static inline void SetTrackXString (ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	ccp text // any text
)
{
...
static inline void SetTrackIdentifier (ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	ccp text // any text
)
{
...
// [[file-name]]
static void SetTrackInfo16 (ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
...
)
{
	...
	{
		bmg_item_t *bi
			= InsertItemBMG (&ctcode->track_string, ctcode->ctb.track_name1.beg + tidx, 0, 0, 0);
		DASSERT (bi);
	...
	{
		bmg_item_t *bi = FindItemBMG (&ctcode->track_string, ctcode->ctb.track_name1.beg + tidx);
		if (bi)
			FreeItemBMG (bi);
	}
}
...
static ccp GetRawTrackInfo (char *buf, // dest buffer
...
static ccp GetTrackFile (char *buf, // dest buffer
	uint size, // size of 'buf'
	ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	const bmg_item_t *bi // NULL or bmg text
...
static ccp GetTrackString (char *buf, // dest buffer
	uint size, // size of 'buf'
	ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	const bmg_item_t *bi // NULL or bmg text
...
static ccp GetTrackIdentifier (char *buf, // dest buffer
	uint size, // size of 'buf'
	ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	const bmg_item_t *bi // NULL or bmg text
...
const u16 *GetTrackName16 (uint *tlen, // not nULL: store length of result here
	ctcode_t *ctcode, // valid CTCODE
	uint tidx, // index of the track
	const bmg_item_t *bi // NULL or bmg text
)
...
static void SetupUsedSlotList (u8 *slot_list, uint n_slots, uint le_phase)
{
	...
	if (le_phase)
	{
		memset (slot_list + MKW_ARENA_BEG, 2, LE_FIRST_CT_SLOT - MKW_ARENA_BEG);
		if (le_phase == 1)
		{
			const uint max = n_slots < LE_FIRST_UPPER_CT_SLOT ? n_slots : LE_FIRST_UPPER_CT_SLOT;
			if (max > 0xff)
...
void SetupUsedSlotsCTCODE (ctcode_t *ctcode)
{
	...
	{
		SetupUsedSlotList (ctcode->used_slot, sizeof (ctcode->used_slot), ctcode->ctb.le_phase);
...
void ResetDataCTCODE (ctcode_t *ctcode, bool create_empty)
{
	...
	const uint max_racing_cups
		= (ctcode->ctb.le_phase ? LE_CODE_MAX_CUPS : CT_CODE_DEFAULT_CUPS) - max_battle_cups;
	...
	ctcode->crs_size = sizeof (ctcode_crs1_head_t) + max_tracks * sizeof (ctcode_crs1_data_t);
	...
	static const struct tab_t tab[] = { { 0x36, 0xc9, 0x36, "ring_mission" },
		{ 0x37, 0, 0, "winningrun_demo" }, { 0x38, 0, 0, "loser_demo" },
		{ 0x39, 0, 0, "draw_demo" }, { 0x3a, 0x75, 0x08, "ending_demo" },
		//	{ 0x42,    0,    0, "invalid" },
		{ 0x43, 0, 0, "- " TOOLSET_LONG " v" VERSION " r" REVISION }, { 0, 0, 0, 0 } };
	...
	PRINT ("STAT/CUP: racing: %u/%u, battle: %u/%u, unused: %u\n", ctcode->n_racing_cups,
		ctcode->max_racing_cups, ctcode->n_battle_cups, ctcode->max_battle_cups,
...
void CalcCupRefCTCODE (ctcode_t *ctcode, bool force)
{
	...
	for (cidx = 0; cidx < ctcode->n_racing_cups + ctcode->n_battle_cups; cidx++)
	{
		...
		for (ti = 0; ti < 4 + is_battle; ti++)
		{
			...
			if (tidx < ctcode->max_tracks)
			{
				...
				{
					cr->cup[cr->n]
						= is_battle ? cidx - ctcode->n_racing_cups + ctcode->max_racing_cups : cidx;
					cr->idx[cr->n] = ti;
					cr->n++;
				}
			}
	...
	for (tidx = 0; tidx < ctcode->n_tracks; tidx++)
	{
		...
		if (*namebuf)
		{
			...
			if (it)
				memcpy (ctcode->cupref + tidx, ctcode->cupref + it->num, sizeof (*ctcode->cupref));
		}
	}
...
void PatchNamesCTCODE (ctcode_t *ctcode)
{
	...
	for (cidx = 0; cidx < ctcode->n_racing_cups + ctcode->n_battle_cups; cidx++)
	{
		const bmg_item_t *bi = FindItemBMG (&opt_load_bmg, ctcode->ctb.rcup_name.beg + cidx);
		if (bi && bi->len)
		{
	...
	for (tidx = 0; tidx < ctcode->n_tracks; tidx++)
	{
		const bmg_item_t *bi = FindItemBMG (&opt_load_bmg, ctcode->ctb.track_name1.beg + tidx);
		if (bi && bi->len)
...
enumError ReorderTracksCTCODE (ctcode_t *ctcode)
{
	...
	if (!err)
	{
		...
		uint cidx;
		for (cidx = opt_order_all ? 0 : 8; cidx < ctcode->n_racing_cups; cidx++)
		{
			...
			for (ti = 0; ti < 4; ti++, ptr++)
			{
				...
				const uint tidx = be32 (cd->track_id + ti);
				const bmg_item_t *bi = FindItemBMG (&bmg, tidx + ctcode->ctb.track_name1.beg);
				if (bi && bi->len)
				{
					*ptr->key = 'A';
					PrintString16BMG (ptr->key + 1, sizeof (ptr->key) - 1, bi->text, bi->len,
...
static void hexdump (FILE *f, uint indent, const void *vdata, uint size, ccp title)
{
	DASSERT (f);
	DASSERT (title);
...
void DumpCTCODE (FILE *f, uint indent, ctcode_t *ctcode)
{
	...
	for (sect = 0; sect < n_sect; sect++)
	{
		...
		if (!off)
			continue;
		fprintf (
			f, "%*u: %6x : %s\n", numindent, sect, off, PrintID (ch->sect_info[sect].name, 4, 0));
	}
		...
		for (tidx = 0; tidx < ctcode->max_tracks; tidx++, cr++)
		{
			if (cr->n_racing)
			{
				...
				fprintf (f, "%*s%s [M=%s,P=%s,S=0x%02x,F=%s]\n", indent + 2, "", namebuf,
					...
					PrintPropertyID (ntohl (td->property_id), false), tidx, td->filename);
			}
		}
	}
...
static int InsertTrackIntoCup (ctcode_t *ctcode, uint tidx, bool hidden)
{
	...
	else if (ctcode->add_wiimm_cup && ctcode->ctb.le_phase && cidx == 8)
	{
		AddWiimmCup (ctcode);
...
static ctcode_crs1_data_t *GetNextArenaSlot (ctcode_t *ctcode, uint prop, ScanInfo_t *si)
{
	if (!ctcode->ctb.le_phase)
	{
	...
	if (!IsMkwArena (prop))
	{
		ERROR0 (ERR_WARNING,
			...
			MKW_N_TRACKS, MKW_N_TRACKS + MKW_N_ARENAS - 1, si->cur_file->name, si->cur_file->line);
		return 0;
	}
	...
	{
		ERROR0 (ERR_WARNING, "Arena slot 0x%02x already used: %s @%u\n", prop, si->cur_file->name,
			si->cur_file->line);
		return 0;
	}
...
static enumError ScanRTL_Option (ctcode_t *ctcode, ScanInfo_t *si)
{
	...
	if (!namelen)
	{
		if (si->no_warn <= 0)
		{
			...
			ERROR0 (ERR_WARNING, "Missing name for option: [%s @%u]: %.*s\n", sf->name, sf->line,
				(int)(eol - start), start);
		}
		return ERR_WARNING;
	}
	...
	{
		if (si->no_warn <= 0)
			ERROR0 (ERR_WARNING, "Unknown option [%s @%u]: %s\n", sf->name, sf->line, name);
		return ERR_WARNING;
	}
	return ERR_OK;
}
...
static enumError ScanRTL_Nintendo (ctcode_t *ctcode, ScanInfo_t *si)
{
	...
	static const struct coldef_t coltab[] = { { RTL_N_F_RED1, "red1" }, { RTL_N_F_RED2, "red2" },
		{ RTL_N_F_RED3, "red3" }, { RTL_N_F_RED4, "red4" }, { RTL_N_F_YELLOW, "yellow" },
		{ RTL_N_F_GREEN, "green" }, { RTL_N_F_BLUE1, "blue1" }, { RTL_N_F_BLUE2, "blue2" },
		{ RTL_N_F_WHITE, "white" }, { RTL_N_F_CLEAR, "clear" },
	...
	if (mode == RTL_N_HIDE || mode == RTL_N_SHOW || mode == RTL_N_SWAP)
	{
		...
		for (ti = 0; ti < MKW_N_TRACKS; ti++, info++, td++)
		{
			...
			if (have_prefix && col)
			{
				...
				if (space)
				{
					snprintf (buf2, sizeof (buf2), "\\c{%s}%.*s \\c{OFF}%s", col,
						(int)(space - name), name, space + 1);
					name = buf2;
				}
			}
...
static enumError ScanRTL_Slot (ctcode_t *ctcode, ScanInfo_t *si, bool mark_reserved)
{
	DASSERT (ctcode);
	DASSERT (si);
...
static enumError ScanRTL_Track (ctcode_t *ctcode, ScanInfo_t *si, bool is_arena, bool hidden)
{
	DASSERT (ctcode);
	DASSERT (si);
	// ScanFile_t *sf = si->cur_file;
	...
	*fname = 0;
	...
	ctcode_crs1_data_t *td
		= is_arena ? GetNextArenaSlot (ctcode, property_id, si) : GetNextTrackSlot (ctcode, hidden);
	if (!td)
		return ERR_WARNING;
...
enumError ScanSetupArena (ctcode_arena_t *ca, ScanInfo_t *si)
{
	...
	for (;;)
	{
		...
		if (slot < MKW_ARENA_BEG || slot >= MKW_ARENA_END)
		{
			...
			if (si->no_warn <= 0)
			{
				ERROR0 (ERR_WARNING, "Invalid arena slot #%d [%s @%u]: %.*s\n", slot, sf->name,
					sf->line, (int)(eol - ptr0), ptr0);
			}
			sf->ptr = eol;
			continue;
		}
		...
		if (property_id > 0 && !IsMkwArenaOrRandom (property_id))
		{
			...
			{
				ERROR0 (ERR_WARNING, "Invalid arena slot #%d [%s @%u]: %.*s\n", property_id,
					sf->name, sf->line, (int)(eol - ptr0), ptr0);
			}
			sf->ptr = eol;
			continue;
		}
...
static enumError ScanTrackList (ctcode_t *ctcode, ScanInfo_t *si)
{
	...
	for (;;)
	{
		...
		switch (ch)
		{
			...
			{
				ccp eol = FindNextLineFeedSI (si, true);
				err = ERROR0 (ERR_WARNING, "Line ignored [%s @%u]: %.*s\n", sf->name, sf->line,
...
enumError ScanTextArena (ctcode_arena_t *ca, // valid data structure
...
)
{
	...
	const KeywordTab_t sect_names[] = { { SECT_SETUP_ARENA, "SETUP-ARENA", 0, 0 }, { 0, 0, 0, 0 } };
...
enumError ScanTextCTCODE (ctcode_t *ctcode, // CTCODE data structure
...
)
{
	...
	enum
	{
		...
		SECT_SETUP_ARENA,
		SECT_TRACK_LIST,
		SECT_RACING_TRACK_LIST,
		N_SECT
	};
	const KeywordTab_t sect_names[] = { { SECT_SETUP, "SETUP", 0, 0 },
		{ SECT_SETUP_ARENA, "SETUP-ARENA", 0, 0 }, { SECT_TRACK_LIST, "TRACK-LIST", 0, 0 },
...
static int scan_raw_ctcode_func (struct szs_iterator_t *it, // iterator struct with all infos
	...
	if (!memcmp (data, "CUP1", 4))
	{
		...
		if (it->off + size <= it->szs->size)
		{
			...
			PRINT ("CTCODE/CUP1 found, SIZEx=%x/%x/%x, Nx=%x+%x, Nd=%d+%d, IN-C=%d\n", size,
				be32 (&hd->size), it->size, nc1, nc2, nc1, nc2,
				InContainerP (&ctcode->container, data));
#else
			PRINT ("CTCODE/CUP1 found, SIZEx=%x/%x/%x, Nx=%x+%x, Nd=%d+%d, IN-C=%d\n", size,
				be32 (&hd->size), it->size, nc1, nc2, nc1, nc2,
				InDataContainer (ctcode->old_container, data));
#endif
			...
			const uint max_cups
				= (data + size - (u8 *)ctcode->cup_racing) / sizeof (ctcode_cup1_data_t);
	...
	else if (!memcmp (data, "CRS1", 4))
	{
		...
		if (it->off + size <= it->szs->size)
		{
			...
			PRINT ("CTCODE/CRS1 found, SIZEx=%x/%x/%x, Nxd=%x,%d, IN-C=%d\n", size,
				be32 (&hd->size), it->size, nt, nt, InContainerP (&ctcode->container, data));
#else
			PRINT ("CTCODE/CRS1 found, SIZEx=%x/%x/%x, Nxd=%x,%d, IN-C=%d\n", size,
				be32 (&hd->size), it->size, nt, nt, InDataContainer (ctcode->old_container, data));
#endif
			...
			ctcode->n_tracks = nt;
			ctcode->max_tracks
				= (data + size - (u8 *)ctcode->crs->data) / sizeof (ctcode_crs1_data_t);
...
int IterateFilesCTCODE (szs_iterator_t *it, // iterator struct with all infos
...
)
{
	...
	const u32 min_data_off
		= sizeof (ctcode_header_t) + n_sect * sizeof (ctcode_sect_info_t) - sizeof (ch->sect_info);
	...
	for (sect = 0; sect < n_sect && !stat; sect++)
	{
		...
		snprintf (it->path, sizeof (it->path), "ctcode-%i-%s.bin", sect, PrintID (name, 4, 0));
...
static void ScanSetupCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
	const void *data, // data to scan
	uint data_size, // size of 'data'
...
static enumError ScanRawCTCODE_helper (ctcode_t *ctcode, // initialized CTCODE data structure
	const void *data, // data to scan
	uint data_size // size of 'data'
)
{
...
enumError ScanRawCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
	const void *data, // data to scan
	uint data_size, // size of 'data'
	ContainerCTC_t *cdata // NULL or container-data
)
{
	...
	{
		return ERROR0 (
			ERR_INVALID_DATA, "Invalid CT-CODE file: %s\n", ctcode->fname ? ctcode->fname : "?");
	}
...
enumError ScanTexCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
	const void *data, // data to scan
	uint data_size, // size of 'data'
	ContainerCTC_t *cdata // NULL or container-data
)
{
	...
	{
		return ERROR0 (
			ERR_INVALID_DATA, "Invalid TEX+CT file: %s\n", ctcode->fname ? ctcode->fname : "?");
	}

	const brsub_header_t *bh = (brsub_header_t *)data;
	const u32 img_off = be32 (bh->grp_offset);
	return ScanRawCTCODE_helper (ctcode, data + CT_CODE_TEX_OFFSET, img_off - CT_CODE_TEX_OFFSET);
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

static int iter_brres_ctcode (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	noPRINT ("%6x %6x : %02x %02x %02x %02x : %s\n", it->off, it->size, data[0], data[1], data[2],
		data[3], it->name);
	...
	if (IsValidTEXCT (data, it->size, it->size, 0, ctcode->fname) < VALID_ERROR)
	{
		const brsub_header_t *bh = (brsub_header_t *)data;
		const u32 img_off = be32 (bh->grp_offset);
		ScanRawCTCODE_helper (ctcode, data + CT_CODE_TEX_OFFSET, img_off - CT_CODE_TEX_OFFSET);
		return 1;
	}
	return 0;
}
...
enumError ScanBrresCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
	const void *data, // data to scan
	uint data_size, // size of 'data'
	ContainerCTC_t *cdata // NULL or container-data
...
enumError ScanLEBinCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
	const void *data, // data to scan
	uint data_size // size of 'data'
)
{
	...
	if (td && prop && music && flags)
	{
		...
		int max_tidx = -1;
		for (tidx = 0; tidx < max; tidx++, td++, prop++, music++, flags++, ct_flags++)
		{
...
enumError ScanCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
	const void *data, // data to scan
	uint data_size, // size of 'data'
	ContainerCTC_t *cdata, // NULL or container-data
...
)
{
	...
	switch (fform)
	{
		case FF_BRRES:
			...
			if (!ctcode->cup && !ctcode->crs)
			{
				ERROR0 (ERR_INVALID_DATA, "No CTCODE in BRRES found: %s\n",
					ctcode->fname ? ctcode->fname : "?");
				ResetCTCODE (ctcode);
...
enumError ScanRawDataCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
...
#if USE_NEW_CONTAINER_CTC
	return ScanCTCODE (
		ctcode, CTM_NO_INIT, raw->data, raw->data_size, LinkContainerRawData (raw), mode);
#else
	return ScanCTCODE (
		ctcode, CTM_NO_INIT, raw->data, raw->data_size, ContainerRawData (raw), mode);
#endif
}

///////////////////////////////////////////////////////////////////////////////

enumError LoadCTCODE (ctcode_t *ctcode, // CTCODE data structure
	ct_mode_t ct_mode, // not CTM_NO_INIT: Initialize
...
static void print_track (FILE *f, // output file
...
)
{
	...
	const bmg_item_t *bi = FindItemBMG (&opt_load_bmg, tidx + ctcode->ctb.track_name1.beg);
	GetTrackString (namebuf, sizeof (namebuf), ctcode, tidx, bi);
	const ctcode_crs1_data_t *td = ctcode->crs->data + tidx;

	fprintf (f, "%c %s\t; %s\t;", letter, PrintMusicID (ntohl (td->music_id), print_hex),
...
enumError SaveTextCTCODE (ctcode_t *ctcode, // pointer to valid CTCODE
...
)
{
	...
	fprintf (F.f, text_ctcode_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE,
		ctcode->ctb.ct_mode, ctcode->n_racing_cups, ctcode->max_racing_cups, ctcode->n_battle_cups,
		ctcode->max_battle_cups, ctcode->used_tracks, ctcode->used_arenas, ctcode->n_tracks,
		ctcode->max_tracks, ctcode->n_racing_cups, ctcode->max_racing_cups, ctcode->n_battle_cups,
		ctcode->max_battle_cups, ctcode->n_unused_cups);
	...
	if (omode.mode & 1 && ctcode->used_tracks)
	{
		fprintf (F.f, "%s[RACING-TRACK-LIST]\r\n%s", section_sep,
			omode.syntax ? text_ctcode_list_syntax_full_cr : text_ctcode_list_syntax_brief_cr);
		...
		for (cidx = 0; cidx < ctcode->n_racing_cups; cidx++)
		{
			...
			if (bi && bi->len)
				PrintString16BMG (
					namebuf, sizeof (namebuf), bi->text, bi->len, BMG_UTF8_MAX, 2, opt_bmg_colors);
			else
				PrintString16BMG (
					namebuf, sizeof (namebuf), cd->name, -1, BMG_UTF8_MAX, 2, opt_bmg_colors);
			fprintf (F.f, "\r\nC %s # 0x%02x = %2u\r\n", namebuf, cidx, cidx);

			fprintf (F.f, "S 0x%02x!, 0x%02x!, 0x%02x!, 0x%02x!\r\n", be32 (cd->track_id + 0),
				be32 (cd->track_id + 1), be32 (cd->track_id + 2), be32 (cd->track_id + 3));
	...
	if (omode.mode & 2 && ctcode->n_tracks)
	{
		...
		for (tidx = 0; tidx < ctcode->n_tracks; tidx++)
		{
			...
			{
				PrintString16BMG (
					namebuf, sizeof (namebuf), bi->text, bi->len, BMG_UTF8_MAX, 0, opt_bmg_colors);
				fprintf (F.f, "%-*s = %s\r\n", keyfw, "bmg", namebuf);
			}
				...
				else
				{
					if (cup_id == cr->cup[0])
					{
						...
						if (cd)
						{
							...
							PrintString16BMG (namebuf, sizeof (namebuf), cd->name, -1, BMG_UTF8_MAX,
								0, opt_bmg_colors);
	...
	if (omode.mode & 2 && ctcode->n_racing_cups + ctcode->n_battle_cups)
	{
		...
		for (cidx = 0; cidx < ctcode->n_racing_cups + ctcode->n_battle_cups; cidx++)
		{
			...
			PrintString16BMG (
				namebuf, sizeof (namebuf), cd->name, -1, BMG_UTF8_MAX, 0, opt_bmg_colors);
			fprintf (F.f, "%-*s = %s\r\n", keyfw, "name", namebuf);

			const bmg_item_t *bi = FindItemBMG (&opt_load_bmg, cidx + ctcode->ctb.rcup_name.beg);
			if (bi && bi->len)
			{
				PrintString16BMG (
					namebuf, sizeof (namebuf), bi->text, bi->len, BMG_UTF8_MAX, 0, opt_bmg_colors);
			...
			for (ti = 0; ti < 4 + is_battle; ti++)
			{
				...
				if (omode.cross_ref && tidx < ctcode->n_tracks)
				{
					...
					fprintf (F.f, " # M=%02x, P=%02x, %s, %s\r\n", be32 (&td->music_id),
						be32 (&td->property_id), td->filename, namebuf);
				}
				else
					fputs ("\r\n", F.f);
			}
...
enumError SaveMessageCTCODE (ctcode_t *ctcode, // pointer to valid CTCODE
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
	...
	uint begin = brief_count > 2 || brief_count > 1 && ctcode->cmd_n_used ? 32 : 0;
	...
	for (tidx = begin; tidx < max; tidx++)
	{
		...
		if (long_count > 1 && tidx < ctcode->max_tracks)
		{
			...
			if (cr->n)
			{
				...
				{
					dest += snprintf (
						dest, buf + sizeof (buf) - dest, ",%u.%u", cr->cup[i] + 1, cr->idx[i] + 1);
				}
				bmg_item_t *bi = InsertItemBMG (&bmg, ctcode->ctb.cup_ref.beg + tidx, 0, 0, 0);
				DASSERT (bi);
...
enumError SaveTrackListCTCODE (ctcode_t *ctcode, // pointer to valid CTCODE
	ccp fname, // filename of destination
	bool set_time, // true: set time stamps
	...
	for (cidx = select ? 0 : 8; cidx < max; cidx++)
	{
		...
		for (ti = 0; ti < 4 + is_battle; ti++)
		{
			...
			if (tidx < ctcode->n_tracks)
			{
				...
				if (print_slots)
				{
					fprintf (F.f, "\t%s\t%s", PrintPropertyID (property_id, false),
...
enumError SaveTrackTableCTCODE (ctcode_t *ctcode, // pointer to valid CTCODE
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
...
static uint DeactivateMOD2 (BZ2Source_t *bz2s, // data
...
)
{
	...
	if (data)
	{
		...
		{
			case CTL_US:
				search_list = search_usa;
				break;
			case CTL_JP:
				search_list = search_jap;
				break;
			case CTL_KO:
				search_list = search_kor;
				break;
			default:
				search_list = search_pal;
				break;
		}
...
enumError SaveCTCODE (ctcode_t *ctcode, // pointer to valid CTCODE
	ccp fname, // filename of destination
	bool set_time, // true: set time stamps
...
#define REF(p, n, a) p "/" n ";" n, a, sizeof (a)
	static BZ2Source_t srctab[] = { { REF ("rmcp", "boot_code.bin", ctcode_boot_code_pal_bz2),
										FF_CT0_CODE },
		...
		{ REF ("rmck", "bad1code.bin", 0), FF_CT1_CODE }, { REF ("rmck", "mod1.bin", 0), FF_MOD1 },
		{ REF ("rmck", "mod2.bin", 0), FF_MOD2 }, { REF ("rmck", "ovr1.bin", 0), FF_OVR1 },
...
#undef REF
	...
	switch (save_mode)
	{
		...
		case CTS_TEX0:
			copy_tex = true;
			copy_ct1data = true;
			load_data = 1 << BIDX_BAD1CODE | 1 << BIDX_MOD1 | 1 << BIDX_MOD2 | 1 << BIDX_OVR1;
	...
	if (load_data)
	{
		if (SearchListBZ2S (bz2s, BIDX__N, ct_dir_list.field, ct_dir_list.used, opt_ct_log)
			!= BIDX__N)
		{
			uint i, count = 0;
			char *dest = pathbuf;
				...
				if (load_data & 1 << i)
				{
					...
					if (!bs->data || !bs->size)
					{
						...
						dest = snprintfE (dest, pathbuf + sizeof (pathbuf), ", %.*s",
							(int)(sem ? sem - bs->search : strlen (bs->search)), bs->search);
					}
				}
			}
			if (count)
				return ERROR0 (ERR_INVALID_DATA, "Missing CT-CODE file%s: %s\n",
					count == 1 ? "" : "s", pathbuf + 2);
		}
	...
	if (use_dynamic)
	{
		fprintf (stderr, "off: %x %x %x %x %x %x | %x %x\n", off_head, off_cup, off_crs, off_mod1,
			off_mod2, off_ovr1, off_image, off_term);
	}
	...
	if (!err && copy_brres)
	{
		...
		if (opt_image_dir)
		{
			...
			static const struct ftab_t ftab[]
				= { { 0x000140, 832, "strapA_16_9_832x456" }, { 0x0b9580, 608, "strapA_608x456" },
					  { 0x140bc0, 832, "strapB_16_9_832x456" }, { 0x1fa000, 608, "strapB_608x456" },
					  { 0x281640, 832, "strapC_832x456" }, { 0x33aa80, 608, "strapC_608x456" },
					  { 0x3c20c0, 832, "strapD_832x456" }, // relevant image for 16:9 and 4:3
					  { 0x47b500, 608, "strapD_608x456" }, { 0, 0, 0 } };

			static const ccp exttab[] = { ".png", ".PNG", ".tex", ".TEX", ".tex0", ".TEX0", "", 0 };
			...
			for (fi = ftab; fi->width; fi++)
			{
				...
				{
					ccp path
						= PathCatPPE (pathbuf, sizeof (pathbuf), opt_image_dir, fi->fname, *ext);
					...
					if (img.width != fi->width || img.height != height_of_all)
					{
						if (verbose >= 0)
							fprintf (stdlog, " - load image [%s,%ux%u -> %ux%u]: %s\n",
								PrintFormatIMG (&img), img.width, img.height, fi->width,
								height_of_all, path);
					...
					TRACE (" -> [%s,%ux%u]: *** %u *** %s\n", PrintFormatIMG (&img), img.width,
						img.height, img.data_size, path);
	...
	if (!err && copy_tex)
	{
		...
		if (!err && bz2s[BIDX_BAD1CODE].status)
			err = WriteMemFileAt (
				&mf, off + 0x40, bz2s[BIDX_BAD1CODE].data, bz2s[BIDX_BAD1CODE].size);
	...
	if (!err && copy_ct1data)
	{
		...
		if (use_dynamic)
		{
			...
			if (!err)
				err = WriteMemFileAt (
					&mf, off + off_head, &ctcode->ct_head, sizeof (ctcode->ct_head));
			...
			if (!err && bz2s[BIDX_MOD1].data)
				err = WriteMemFileAt (
					&mf, off + off_mod1, bz2s[BIDX_MOD1].data, bz2s[BIDX_MOD1].size);

			if (!err && bz2s[BIDX_MOD2].data)
				err = WriteMemFileAt (
					&mf, off + off_mod2, bz2s[BIDX_MOD2].data, bz2s[BIDX_MOD2].size);

			if (!err && bz2s[BIDX_OVR1].data)
				err = WriteMemFileAt (
					&mf, off + off_ovr1, bz2s[BIDX_OVR1].data, bz2s[BIDX_OVR1].size);
			//--------------
		}
		else
		{
			...
			if (!err)
				err = WriteMemFileAt (
					&mf, off + CT_CODE_EXTEND_OFFSET + CT_CODE_EXTEND_SIZE, "", 0);

			err = WriteMemFileAt (&mf, off, &ctcode->ct_head, sizeof (ctcode->ct_head));
...
int ScanOptCtLog (ccp arg)
{
	static const KeywordTab_t keytab[]
		= { { SEA_LOG_OFF, "OFF", "NONE", 0 }, { SEA_LOG_SUCCESS, "SUCCESS", 0, 0 },
			  { SEA_LOG_FOUND, "FOUND", 0, 0 }, { SEA_LOG_ALL, "ALL", 0, 0 }, { 0, 0, 0, 0 } };
	...
	else
	{
		...
		if (!key)
		{
			PrintKeywordError (keytab, arg, abbrev, 0, "keyword for option --ct-log");
			return 1;
		}
		opt_ct_log = key->id;
	}
...
{
	return ERR_OK
		!= ScanSizeOptU32 (&crs1_property, // u32 * num
			arg, // ccp source
			1, // default_factor1
			0, // int force_base
			"crs1", // ccp opt_name
...
int ScanOptAllowSlots (ccp arg)
{
	...
	static const KeywordTab_t keytab[] = { { K_TRACKS, "TRACKS", 0, 0 },
		{ K_ARENAS, "ARENAS", 0, 0 }, { K_SPECIAL, "SPECIAL", 0, 0 },
		{ K_CTCODE, "CTCODE", "CT-CODE", 0 }, { 0, 0, 0, 0 } };
	...
	for (;;)
	{
		...
		else
		{
			...
			*dest = 0;
			...
			if (!key)
			{
				PrintKeywordError (keytab, namebuf, abbrev, 0, "keyword for option --allow-slots");
				return 1;
			}
			DASSERT (key);
