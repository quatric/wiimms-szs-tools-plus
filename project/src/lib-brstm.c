...
{
    if ( mb->size + need <= mb->capacity ) return;
    size_t cap = mb->capacity ? mb->capacity * 2 : 0x1000;
    while ( cap < mb->size + need ) cap *= 2;
...
{
    mb_reserve(mb, len);
    if ( len ) memcpy(mb->data + mb->size, src, len);
    mb->size += len;
}
...
{
    u8 b[2];
    if ( le ) { b[0] = (u8)v; b[1] = (u8)(v>>8); }
    else      { b[0] = (u8)(v>>8); b[1] = (u8)v; }
    mb_append(mb, b, 2);
}
static void mb_u32e ( membuf_t *mb, u32 v, bool le )
{
    u8 b[4];
    if ( le ) { b[0]=(u8)v; b[1]=(u8)(v>>8); b[2]=(u8)(v>>16); b[3]=(u8)(v>>24); }
    else      { b[0]=(u8)(v>>24); b[1]=(u8)(v>>16); b[2]=(u8)(v>>8); b[3]=(u8)v; }
    mb_append(mb, b, 4);
}
...
void FreeBRSTMAudio ( brstm_audio_t *audio )
{
    ...
    {
        if ( audio->pcm[ch] ) FREE(audio->pcm[ch]);
        audio->pcm[ch] = 0;
    }
}
...
static void EncodeAdpcmChannels ( const brstm_audio_t *audio, s16 coefs[][16],
                                   u8 *adpcm_data[], s64 *block_size )
{
...
static enumError EncodeRSTM ( u8 **out_data, size_t *out_size, const brstm_audio_t *audio, bool use_adpcm )
{
    ...
    mb_u32(&out, 0x40); mb_u32(&out, (u32)head_size);
    mb_u32(&out, use_adpcm ? (u32)(0x40 + head_size) : 0); mb_u32(&out, (u32)adpc_size);
    mb_u32(&out, (u32)data_off); mb_u32(&out, (u32)data_size);
    mb_pad_to(&out, 0x40);

    // HEAD
    mb_tag(&out, "HEAD");
    mb_u32(&out, (u32)head_size);
    mb_u32(&out, 0x01000000); mb_u32(&out, h1rel);
    mb_u32(&out, 0x01000000); mb_u32(&out, h2rel);
    mb_u32(&out, 0x01000000); mb_u32(&out, h3rel);
    ...
    mb_u8(&out, 1); mb_u8(&out, 1); mb_u16(&out, 0);
    mb_u32(&out, 0x01010000); mb_u32(&out, h2rel + 12);
    mb_u8(&out, 0x7F); mb_u8(&out, 0x40); mb_u16(&out, 0);
    mb_u32(&out, 0);
    mb_u8(&out, channels); mb_u8(&out, 0); mb_u8(&out, channels > 1 ? 1 : 0); mb_u8(&out, 0);

    // HEAD3
    mb_pad_to(&out, head_body_base + h3rel);
    mb_u8(&out, channels); mb_u8(&out, 0); mb_u16(&out, 0);
    ...
    for ( int ch = 0; ch < channels; ch++ )
    {
        ...
        mb_u16(&out, 0); mb_u16(&out, 0); // initial hist1/hist2
        mb_u16(&out, 0); mb_u16(&out, 0); mb_u16(&out, 0); // loop ps/hist1/hist2
        mb_u16(&out, 0);
    }
    ...
    for ( int ch = 0; ch < channels; ch++ )
        if ( adpcm_data[ch] ) FREE(adpcm_data[ch]);
...
static enumError EncodeFSTM ( u8 **out_data, size_t *out_size, const brstm_audio_t *audio, bool use_adpcm )
{
    ...
    mb_u16e(&out, 0, le);
    mb_u16e(&out, 0x4000, le); mb_u16e(&out, 0, le); mb_u32e(&out, (u32)info_off, le); mb_u32e(&out, (u32)info_size, le);
    if ( use_adpcm )
        { mb_u16e(&out, 0x4001, le); mb_u16e(&out, 0, le); mb_u32e(&out, (u32)seek_off, le); mb_u32e(&out, (u32)seek_size, le); }
    mb_u16e(&out, 0x4002, le); mb_u16e(&out, 0, le); mb_u32e(&out, (u32)data_off, le); mb_u32e(&out, (u32)data_size, le);
    mb_pad_to(&out, info_off);

    // INFO
    mb_tag(&out, "INFO");
    mb_u32e(&out, (u32)info_size, le);
    mb_u32e(&out, 0x41000000, le); mb_u32e(&out, h1rel, le);
    mb_u32e(&out, 0x01010000, le); mb_u32e(&out, 0xFFFFFFFF, le); // no track table
    mb_u32e(&out, 0x01010000, le); mb_u32e(&out, h3rel, le);
    ...
    for ( int ch = 0; ch < channels; ch++ )
        { mb_u32e(&out, 0x41020000, le); mb_u32e(&out, ci0 + 8 * ch, le); }
    for ( int ch = 0; ch < channels; ch++ )
    ...
    for ( int ch = 0; ch < channels; ch++ )
    {
        ...
        mb_u16e(&out, ps, le);
        mb_u16e(&out, 0, le); mb_u16e(&out, 0, le);
        mb_u16e(&out, 0, le); mb_u16e(&out, 0, le); mb_u16e(&out, 0, le);
        mb_u16e(&out, 0, le);
    }
    ...
    if ( use_adpcm )
    {
        ...
        for ( int i = 0; i < channels; i++ )
            { mb_u16e(&out, 0, true); mb_u16e(&out, 0, true); }
        mb_pad_to(&out, data_off);
    }
...
enumError EncodeBRSTM ( u8 **out_data, size_t *out_size, const brstm_audio_t *audio, bool use_adpcm )
{
    ...
    return audio->variant == BRSTM_VARIANT_RSTM
        ? EncodeRSTM(out_data, out_size, audio, use_adpcm)
...
static enumError DecodeRSTM ( brstm_audio_t *audio, const u8 *data, size_t size )
{
    ...
    const u8 *audio_start = data + data_offs + 0x20; // DATA content base (data_offs+8) + the 0x18 offset field written there
    ...
    for ( int ch = 0; ch < channels; ch++ )
        if ( ch_bytes[ch] ) FREE(ch_bytes[ch]);
...
static enumError DecodeFSTM ( brstm_audio_t *audio, const u8 *data, size_t size, bool le )
{
    ...
    for ( int i = 0; i < sections && pos + 12 <= size; i++, pos += 12 )
    {
        ...
        if ( flag == 0x4000 ) info_offs = rd_u32e(data + pos + 4, le);
        else if ( flag == 0x4002 ) data_offs = rd_u32e(data + pos + 4, le);
    }
