...
int main_dec (int argc, char **argv)
{
	...
	analyze_LZH8 (infile, outfile, file_length);
...
/* read MSB->LSB order */
static inline uint16_t get_next_bits (FILE *infile, long *const offset_p, uint8_t *const bit_pool_p,
	int *const bits_left_p, const int bit_count)
{
	uint16_t out_bits = 0;
	...
	while (num_bits_produced < bit_count)
	{
		...
		out_bits
			|= (*bit_pool_p >> (*bits_left_p - bits_this_round)) & ((1 << bits_this_round) - 1);
...
void analyze_LZH8 (FILE *infile, FILE *outfile, long file_length)
{
	...
	/* read header */
	{
		...
		if ((header & 0xFF) != 0x40)
		{
			/* Warioware Snapped has a 4-byte LE size prefix before the actual LZH8 stream */
			uint32_t next_header = get_32_le_seek (input_offset, infile);
	...
	uint8_t *const outbuf = malloc (uncompressed_length);
	CHECK_ERRNO (NULL == outbuf, "malloc");

	/* allocate backreference length decode table */
	const uint32_t length_table_bytes = (get_16_le_seek (input_offset, infile) + 1) * 4;
	input_offset += 2;
	const long length_decode_table_size = LENCNT * 2;
	uint16_t *const length_decode_table = malloc (length_decode_table_size * sizeof (uint16_t));
	...
	const uint32_t displen_table_bytes = (get_byte_seek (input_offset, infile) + 1) * 4;
	input_offset++;
	const long displen_decode_table_size = DISPCNT * 2;
	uint8_t *const displen_decode_table = malloc (displen_decode_table_size * sizeof (uint8_t));
	...
	while (bytes_decoded < uncompressed_length)
	{
		...
		for (;;)
		{
			...
			length_key_len++;
#endif
			unsigned int length_node_payload = length_decode_table[length_table_offset] & 0x7F;
			unsigned int next_length_table_offset = (length_table_offset / 2 * 2)
				+ (length_node_payload + 1) * 2 + (next_length_child ? 1 : 0);
			unsigned int next_length_child_isleaf
				= length_decode_table[length_table_offset] & (0x100 >> next_length_child);

			if (next_length_child_isleaf)
			{
#if SHOW_SYMBOLS
				printf ("%08lx symbol %d: ", (unsigned long)bytes_decoded, symbol_count);
#endif
...
#if SHOW_TREE
				printf ("%d: ", length);
				for (int i = length_key_len - 1; i >= 0; i--)
				{
				...
				else
				{
					...
					for (;;)
					{
...
#if SHOW_TREE
						displen_key_bits = (displen_key_bits << 1) | next_displen_child;
						displen_key_len++;
#endif
						unsigned int displen_node_payload
							= displen_decode_table[displen_table_offset] & 0x7;
						unsigned int next_displen_table_offset = (displen_table_offset / 2 * 2)
							+ (displen_node_payload + 1) * 2 + (next_displen_child ? 1 : 0);
						...
						{
							uint16_t displen = displen_decode_table[next_displen_table_offset];
							...
							for (int i = displen_key_len - 1; i >= 0; i--)
							{
								printf ("%c", (displen_key_bits & (1 << i)) ? '1' : '0');
							}
							printf ("\n");
							fflush (stdout);
#endif
							...
							/* apply backreference */
							for (long i = 0; i < length && bytes_decoded < uncompressed_length;
								bytes_decoded++, i++)
							{
						...
						else
						{
							...
							CHECK_ERROR (next_displen_table_offset == displen_table_offset,
			...
			else
			{
				...
				CHECK_ERROR (
					next_length_table_offset == length_table_offset, "stuck in a loop somehow");
