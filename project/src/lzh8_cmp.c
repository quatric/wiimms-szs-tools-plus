...
void LZH8_LZSS_compress (unsigned char *input_data, long input_length,
	struct lzss_symbol **lzss_stream_p, long *lzss_length_p);

/* Huffman prototypes */

void LZH8_Huff_produce_encodings (struct lzss_symbol *const lzss_stream, long lzss_length,
	struct huff_symbol *back_litlen_table, struct huff_symbol *back_displen_table,
	long *output_offset_p, FILE *outfile);

int LZH8_Huff_build_Huffman_tree (
	int *node_remains, long *freq, struct huff_node *node_array, int symbol_count);

void LZH8_Huff_compute_prefix (const struct huff_node *node_array, int root_idx,
	struct huff_symbol *sym_array, uint32_t key_bits, int key_len);

int LZH8_Huff_flatten_tree (
	const struct huff_node *node_array, uint16_t *tree_table, int root_idx, const int offset_bits);

void LZH8_Huff_flatten_single_node (const struct huff_node *node_array,
	struct huff_table_ctrl *ctrl, uint16_t *tree_table, const int offset_bits,
	unsigned int parent_idx, unsigned int *table_idx_p, unsigned int *outstanding_nodes_p);

bool LZH8_Huff_could_satisfy_outstanding_nodes (const struct huff_table_ctrl *ctrl, int table_idx,
...
static inline void flush_bits (FILE *const outfile, long *const offset_p,
	uint32_t *const bit_pool_p, int *const bits_written_p)
{
	if (0 != *bits_written_p)
	{
...
static inline void write_bits (FILE *const outfile, long *const offset_p,
	uint32_t *const bit_pool_p, int *const bits_written_p, const uint32_t bits_to_write,
	const int bit_count)
{
	...
	while (num_bits_produced < bit_count)
	{
		...
		uint32_t selected_bits = bits_to_write >> (bit_count - bits_this_round - num_bits_produced);
	...
	*/
	{
		...
		{
			put_32_le_seek ((((uint32_t)file_length) << 8) | 0x40, output_offset, outfile);
			output_offset += 4;
		}
		else
		{
	...
	*/
...
#if !READ_DUMP
	{
		...
		// temp, dump LZSS stuff to file
		{
			...
			put_bytes (
				lzss_dump, (unsigned char *)lzss_stream, lzss_length * sizeof (struct lzss_symbol));
	...
	// temp, read LZSS stuff from file
	{
		...
		get_bytes_seek (0, lzss_dump, (unsigned char *)lzss_stream, lzss_dump_length);
	...
	*/
	{
		...
		{
			struct huff_symbol litlen_symbol = back_litlen_table[(lzss_stream[i].is_reference << 8)
			...
			if (lzss_stream[i].is_reference)
			{
				...
				struct huff_symbol displen_symbol = back_displen_table[displen_length];
	...
	struct lzss_symbol **lzss_stream_p, long *lzss_length_p)
{
	...
	struct lzss_hash_node *hash_queue = malloc (max_window_size * sizeof (struct lzss_hash_node));
	...
	struct lzss_hash_node *hash_table = malloc (hash_size * sizeof (struct lzss_hash_node));
	...
	CHECK_ERROR (NULL != lzss_stream || 0 != lzss_length, "should start with nothing");
	...
	for (bytes_done = 0; bytes_done < input_length;)
	{
		...
		if (bytes_done - last_report >= 0x40000l)
		{
			fprintf (stderr, "%ld bytes done (%.0f%%)\n", bytes_done,
				(float)bytes_done / input_length * 100);
		...
		if (bytes_done + min_length <= input_length)
		{
			...
			int input_key = LZSS_hash (&input_data[bytes_done], min_length, hash_size);

			for (struct lzss_hash_node *cur_match = hash_table[input_key].next_node;
				NULL != cur_match; cur_match = cur_match->next_node)
			{
				long match_length = 0;
				...
				for (long match_check_offset = cur_match->offset, input_check_offset = bytes_done;
					input_check_offset < next_input_offset
					&& input_data[input_check_offset] == input_data[match_check_offset];
		...
		if (lzss_length >= lzss_stream_capacity)
		{
			...
			lzss_stream = realloc (lzss_stream, lzss_stream_capacity * sizeof (struct lzss_symbol));
		...
		for (long i = 0; i < bytes_in_this_symbol; i++, bytes_done++)
		{
			if (window_size == max_window_size)
			{
				...
				CHECK_ERROR (NULL != old_node->next_node, "oldest node must be at end of list");
				CHECK_ERROR (NULL == old_node->prev_node, "oldest node must link from something");
			...
			if (input_length - bytes_done >= min_length)
			{
				...
				CHECK_ERROR (0 != window_size && hash_queue_head == hash_queue_tail, "overflow");
				struct lzss_hash_node *new_node = &hash_queue[hash_queue_tail];
				int hash_key = LZSS_hash (&input_data[bytes_done], min_length, hash_size);

				CHECK_ERROR (-1 != new_node->offset, "new node wasn't marked unused");
	...
	struct lzss_symbol **lzss_stream_p, long *lzss_length_p)
{
	...
	for (bytes_done = 0; bytes_done < input_length;)
	{
		...
		for (long search_offset =
			...
			search_offset > search_end; search_offset--)
		{
			long match_length = 0;
			for (long match_check_offset = search_offset, input_check_offset = bytes_done;
				input_check_offset < next_input_offset
				&& input_data[input_check_offset] == input_data[match_check_offset];
				input_check_offset++, match_check_offset++, match_length++)
			{
	...
	long *output_offset_p, FILE *outfile)
{
	...
	/* Build Huffman codes for length/literal */
	{
		...
		printf ("\nlength/literal tree:\n");
#endif
		int root_idx = LZH8_Huff_build_Huffman_tree (node_remains, length_freq, node_array, LENCNT);

#if SHOW_TREE_BITS
		printf ("\nlength/literal tree:\n");
#endif

		LZH8_Huff_compute_prefix (node_array, root_idx, back_litlen_table, 0, 0);

		uint16_t tree_table[LENCNT * 2] = { 0 };
		int table_size = LZH8_Huff_flatten_tree (node_array, tree_table, root_idx, LENBITS - 2);
...
#endif
		int root_idx
			= LZH8_Huff_build_Huffman_tree (node_remains, displen_freq, node_array, DISPCNT);

#if SHOW_TREE_BITS
		printf ("\ndisplacement length tree:\n");
#endif

		LZH8_Huff_compute_prefix (node_array, root_idx, back_displen_table, 0, 0);

		uint16_t tree_table[DISPCNT * 2] = { 0 };
		int table_size = LZH8_Huff_flatten_tree (node_array, tree_table, root_idx, DISPBITS - 2);
	...
	int *node_remains, long *freq, struct huff_node *node_array, int symbol_count)
{
	...
	for (int i = next_new_node_idx - 1; i >= 0; i--)
	{
		...
		{
			printf ("lchild %d rchild %d\n", node_array[i].lchild, node_array[i].rchild);
		}
	}
	printf ("\n");
#endif
	...
	struct huff_symbol *sym_array, uint32_t key_bits, int key_len)
{
	...
	CHECK_ERROR (
		(-1 == root->lchild && -1 != root->rchild) || (-1 != root->lchild && -1 == root->rchild),
	...
	const struct huff_node *node_array, uint16_t *tree_table, int root_idx, const int offset_bits)
{
	...
	struct huff_table_ctrl *ctrl = malloc (sizeof (struct huff_table_ctrl) * (root_idx + 2));
	...
	while (0 < outstanding_nodes)
	{
#if EXPLAIN_TABLE
		printf (
			"New round, table_idx = %d, outstanding_nodes = %d\n", table_idx, outstanding_nodes);
#endif
		...
		for (int i = table_idx - 1; i >= 0; i--)
		{
			if (!ctrl[i].placed)
			{
				const struct huff_node *candidate = &node_array[ctrl[i].node_idx];

#if EXPLAIN_TABLE
				printf (" considering %d, size %d (limit %d)\n", i, candidate->subtree_size,
					(1 << offset_bits) - outstanding_nodes);
#endif

				/* we'd like to place the whole subtree this points at */
				if (candidate->subtree_size + outstanding_nodes <= (1 << offset_bits)
		...
		if (fitting_subtree_idx != table_idx)
		{
			...
			/* place the root */
			LZH8_Huff_flatten_single_node (node_array, ctrl, tree_table, offset_bits,
				fitting_subtree_idx, &table_idx, &outstanding_nodes);
			...
			for (; i < table_idx; i++)
			{
				if (!ctrl[i].placed)
				{
					LZH8_Huff_flatten_single_node (node_array, ctrl, tree_table, offset_bits, i,
						&table_idx, &outstanding_nodes);
				}
			}
		}
	...
	unsigned int *outstanding_nodes_p) /* count of unplaced nodes (updated) */
{
	...
	const struct huff_node *parent_node = &node_array[ctrl[parent_idx].node_idx];
