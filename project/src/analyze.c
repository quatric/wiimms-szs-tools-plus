...
{
	static const KeywordTab_t tab[] = { { 0, "NONE", "-", ANA_M_ALL }, { ANA_M_ALL, "ALL", 0, 0 },

		{ ANA_MD_BRRES, "BRRES", "BRES", 0 }, { ANA_MD_MDL, "MDL0", 0, 0 },
		{ ANA_MD_PAT, "PAT0", 0, 0 }, { ANA_MD_TEX, "TEX0", 0, 0 },

		{ ANA_MD_BREFF, "BREFF", "REFF", 0 }, { ANA_MD_BREFT, "BREFT", "REFT", 0 },
		{ ANA_MD_TPL, "TPL", 0, 0 },

		{ ANA_MD_KCL, "KCL", 0, 0 },

		{ ANA_M_KMP, "KMP", 0, 0 }, { ANA_KMP_GOBJ, "GOBJ", 0, 0 }, { ANA_KMP_KTPT, "KTPT", 0, 0 },

		{ ANA_MKWANA, "MKW_ANA", "MKWANA", 0 },

		{ 0, 0, 0, 0 } };

	int stat = ScanKeywordList (arg, tab, 0, true, 0, 0, "Option --analyze-mode", ERR_SYNTAX);
	if (stat != -1)
	{
...
static analyze_item_t *add_item (xanalyze_t *ana, // analyse
	ccp info, // short item info
	brres_group_t *grp, // pointer to group
...
static int collect_raw_data (struct brsub_iterator_t *it, // iterator struct with all infos
...
static int collect_group_data (struct brsub_iterator_t *it, // iterator struct with all infos
	brres_group_t *grp, // pointer to group
...
static int collect_entry_data (struct brsub_iterator_t *it, // iterator struct with all infos
	brres_group_t *grp, // pointer to group
	uint grp_index, // index of group
	brres_entry_t *entry, // pointer to entry
	u8 *data // pointer to entry related data
)
{
	...
	item->entry_name = GetStringBRRES (it->szs, grp, it->endian->rd32 (&entry->name_off), "?");
	...
	switch (BRSUB_MODE3 (ana->fform, ana->version, grp_index))
	{
		...
		{
			noPRINT (">>> %s.%u.%02u: %5x,%5x,%5x\n", ana->ff_name, grp_index,
				(int)(entry - grp->entry), it->endian->rd32 (data), it->endian->rd32 (data + 0x08),
				it->endian->rd32 (data + 0x0c));
			add_item (
				ana, "end of section", grp, grp_index, 99, entry, data + it->endian->rd32 (data));
		}
		break;
...
void AnalyzeBRSUB (szs_file_t *szs, u8 *data, uint data_size, ccp name)
{
	...
	if (fform == FF_PAT && version == 4)
	{
		...
		u32 off1 = endian->rd32 (data + 16 + 1 * 4);
		if (!off1)
			off1 = off0;
		u32 off2 = endian->rd32 (data + 16 + 2 * 4);
		if (!off2)
			off2 = off1;
		u32 off3 = endian->rd32 (data + 16 + 3 * 4);
		if (!off3)
			off3 = off2;
		u32 off4 = endian->rd32 (data + 16 + 4 * 4);
		if (!off4)
			off4 = off3;
		u32 off5 = endian->rd32 (data + 16 + 5 * 4);
		if (!off5)
			off5 = off4;
		...
		fprintf (f,
			"#PAT-N# %4d %4d %4d %4d  %4d %4d %4d %4d"
			" | %4d %4d %4d %4d %4d %4d | v=%d,%d %s\n",
			endian->rd16 (data + 0x2c + 0 * 2), endian->rd16 (data + 0x2c + 1 * 2),
			endian->rd16 (data + 0x2c + 2 * 2), endian->rd16 (data + 0x2c + 3 * 2),
			endian->rd16 (data + 0x2c + 4 * 2), endian->rd16 (data + 0x2c + 5 * 2),
			endian->rd16 (data + 0x2c + 6 * 2), endian->rd16 (data + 0x2c + 7 * 2), off1 - off0,
			off2 - off1, off3 - off2, off4 - off3, off5 - off4, data_size - off5, ana.valid,
			ana.data_complete, fname);

		if (ana.data_complete)
		{
			const pat_s0_bhead_t *bh = ana.s0_base;
			fprintf (f, "#PAT-BH# %4x %4x %4x %4x %4x %4x %4x %4x %4x %4x %4x | %s\n",
				be32 (&bh->size), be16 (&bh->unknown_04), be16 (&bh->n_elem),
				be16 (&bh->unknown_08), be16 (&bh->unknown_0a), be16 (&bh->n_unknown),
				be16 (&bh->unknown_0e), be16 (&bh->unknown_10), be16 (&bh->unknown_12),
				be16 (&bh->unknown_14), be16 (&bh->unknown_16), fname);

			const pat_s0_belem_t *be = bh->elem;
			uint i;
			for (i = 0; i < ana.n_sect0; i++, be++)
				fprintf (f, "#PAT-BE.%u# %4x %4x %4x %4x %4x %4x | %s\n", i, be16 (&be->unknown_00),
					be16 (&be->unknown_02), be16 (&be->unknown_04), be16 (&be->unknown_06),
					be32 (&be->offset_name), be32 (&be->offset_strref), fname);
			...
			for (i = 0; i < PAT_MAX_ELEM; i++)
			{
				...
				if (!sr)
					continue;
				fprintf (f, "#PAT-SR.%u# %4x %4x %4x %4x | %s\n", i, be32 (&sr->offset_name),
					be16 (&sr->unknown_04), be16 (&sr->type), be32 (&sr->offset_strlist), fname);
			}
			...
			for (i = 0; i < PAT_MAX_ELEM; i++)
			{
				...
				fprintf (f,
					...
					"#PAT-SE#\n",
					i, be16 (&sh->n_elem), be16 (&sh->unknown_02), fm, fm * f2, fm * (f2 - f1),
					fname);
				...
				for (j = 0; j < ana.n_s0_list[i]; j++)
				{
					...
					fprintf (f, "#PAT-SE.%u.%02u# %11.5f [%8.5f] %4x %4x | %s\n", i, j, time,
						time - last, be16 (&se->index), be16 (&se->unknown_06), fname);
					last = time;
				}
			}
		}
...
#define ADD_STR(s)                                                                                 \
	if (n_special_str < MAX_SPECIAL_STR && !strcmp (sptr, s))                                      \
...
#undef ADD_STR
	...
	for (i = 0; i < ana.used - 1; i++)
	{
		...
		if (item->index1 < 0)
			snprintf (idxbuf1, sizeof (idxbuf1), "%s.++.%02u", ff_name, item->index3);
		else if (item->index3 == 99)
			snprintf (idxbuf1, sizeof (idxbuf1), "%s.%02u.~~", ff_name, item->index1);
		else
		{
			snprintf (
				idxbuf1, sizeof (idxbuf1), "%s.%02u.%02u", ff_name, item->index1, item->index3);
		}
		snprintf (idxbuf2, sizeof (idxbuf2), "%s.%02d", idxbuf1, item->index2);
		...
		fprintf (f, "\n#SECT# %3u. %6x..%5x, size=%x, u32=%x->%x : %s >>> %s <<<\n", i,
			item->offset, item[1].offset, sect_size, val, item->offset + val, idxbuf2, item->info);

		if (item->index2 > 0) // ( && item->index3 != 99 )
			fprintf (f, "#SECT-SIZE# %s : %2u %2u %2u : %4u = 0x%04x : %s\n", ff_name, item->index1,
				item->index2, item->index3, sect_size, sect_size, item->info);
		...
		for (ptr = base; ptr < end; ptr += 4)
		{
			...
			if (item->grp && ptr >= (u8 *)item->grp)
			{
				...
				{
					fprintf (f, "### %s.%05x.GRP : %s val=%8x : %s\n", idxbuf1, (int)(ptr - base),
						idxbuf2, offset, string);
					found = true;
				}
			}
#endif
			...
			if (string && *string > ' ' && *string < 0x7f)
			{
				...
				if (item->index1 == -1 && ptr - base >= 4 * n_sect)
					fprintf (f, "### %s.%05x.NNN : %s val=%8x : %s\n", idxbuf1,
						(int)(ptr - base - 4 * n_sect), idxbuf2, offset, string);
			}
					...
					for (i = 0; i < n_special_str; i++)
						if (string == special_str[i])
							fprintf (f, "#SPEC# %s.%05x+%05x=%05x : off=%6x : %s\n", idxbuf1,
								(int)(ptr - base), (int)(p2 - ptr), (int)(p2 - base), offset,
								string);
				}
			}
#endif
	...
	if ( fform == FF_MDL && item->old_index == 0xc0 )
	{
		...
		if (item->brsub_mode == BRSUB_MODE3 (FF_MDL, 11, 0))
		{
			...
			if (ms0.fail)
			{
				...
				HexDump (f, 1, ms0.fail - ms0.data_beg, 3, -50, ms0.fail, ms0.data_end - ms0.fail);
			}
		}
#endif
		...
		if (item->brsub_mode == BRSUB_MODE3 (FF_MDL, 11, 8))
		{
			...
			if (item->index3 == 0)
			{
				...
				fprintf (f,
					"%sMEM #=%05x %u*l=%04x-%04x free=%03x>%02x u=%04x[%04x,s=%03x]"
					" : s=%04x[%08x]\n",
					prefix, endian->rd32 (base), layer_cnt, layer_off, end_layer,
					grcode_off - end_layer, (int)(base + grcode_off - data) & 0x1f, grcode_off,
					endian->rd16 (base + grcode_off), endian->rd32 (base) - grcode_off, shader_off,
					endian->rd32 (base + shader_off));
		...
		if (ana.fform == FF_TEX && item->index1 < 0 && item->index3 == 1)
		{
			...
			if (geo)
			{
				...
				fprintf (f, " %6u %+6d b=%5u %4u.%u:", img_size, raw_size - img_size, n_blk,
					m_blk - n_blk, xbytes);
			}
			else
...
static int ana_breff_subfile (struct szs_iterator_t *it, // iterator struct with all infos
...
static int ana_breft_subfile (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	fprintf (f, "#BREFF# HEAD: %8x %8x %8x %8x  %8x %8x %8x %8x [%x]\n", it->endian->rd32 (d),
		it->endian->rd32 (d + 1), it->endian->rd32 (d + 2), it->endian->rd32 (d + 3),
		it->endian->rd32 (d + 4), it->endian->rd32 (d + 5), it->endian->rd32 (d + 6),
		it->endian->rd32 (d + 7), it->size);
...
void AnalyzeBREFF (szs_file_t *szs, u8 *data, uint data_size, ccp name)
{
	...
	fprintf (f, "#%s#UNKNOWN: %8x %8x %4x\n", ff_name, endian->rd32 (&root->unknown1),
		endian->rd32 (&root->unknown2), endian->rd16 (&root->unknown3));

	szs_file_t szs2;
	// [[fname-]]
	InitializeSubSZS (&szs2, szs, data - szs->data, szs->size, fform, 0, false);
	IterateFilesParSZS (&szs2, fform == FF_BREFF ? ana_breff_subfile : ana_breft_subfile, f, false,
		true, false, false, -1, SORT_NONE);
	ResetSZS (&szs2);
}
...
void AnalyzeTPL (szs_file_t *szs, u8 *data, uint data_size, ccp name)
{
	...
	file_format_t fform = GetByMagicFF (data, data_size, data_size);
	// [[tpl-ex+]]
	if (!IsTplFF (fform) || opt_analyze_mode != ANA_MD_DEFAULT && !(opt_analyze_mode & ANA_MD_TPL))
	{
		return;
	}
	...
	for (ni = 0; ni < n_img; ni++)
	{
		...
		if (SetupPointerTPL (
				data, data_size, ni, &head, &tab, &pal, &img, &pal_data, &img_data, endian))
		{
			noPRINT ("TPL #%u: %6x %6x %6x %6x : %6x %6x : %6x\n", ni, (int)((u8 *)head - data),
				(int)((u8 *)tab - data), pal ? (int)((u8 *)pal - data) : 0, (int)((u8 *)img - data),
				pal_data ? (int)(pal_data - data) : 0, (int)(img_data - data), data_size);
			fprintf (f, "#TPL-IMG# %u: %6x %6x %6x %6x : %6x %6x\n", ni, (int)((u8 *)head - data),
				(int)((u8 *)tab - data), pal ? (int)((u8 *)pal - data) : 0, (int)((u8 *)img - data),
				pal_data ? (int)(pal_data - data) : 0, (int)(img_data - data));
		}
		else
		{
			PRINT ("TPL #%u: FAILED\n", ni);
...
{
	if (opt_analyze_mode != ANA_MD_DEFAULT && !(opt_analyze_mode & (ANA_M_KMP | ANA_MKWANA)))
	{
		return;
	}
	...
	if (opt_analyze_mode == ANA_MD_DEFAULT || (opt_analyze_mode & ANA_KMP_GOBJ))
	{
		...
		for (i = 0; i < n; i++, gobj++)
		{
			...
			fprintf (f, "#KMP-GOBJ-ALL#  %4x %4x  %s  %4x %4x %4x %4x %4x %4x %4x %4x %4x  %s\n",
				gobj->obj_id, gobj->ref_id, gobj->route_id == 0xffff ? "-" : "R", s[0], s[1], s[2],
				s[3], s[4], s[5], s[6], s[7], gobj->pflags, name);
	...
	if (opt_analyze_mode == ANA_MD_DEFAULT || (opt_analyze_mode & ANA_KMP_KTPT))
	{
		kmp_finish_t kf;
		CheckFinishLine (kmp, &kf);
		fprintf (f, "#KTPT-DIST# %11.3f %7.2f  *%u,%u  %s : %s\n", kf.distance, kf.dir_delta,
			kf.n_ckpt_0, kf.n_ktpt_m,
			kf.warn		  ? "WARN"
				: kf.hint ? "HINT"
						  : "-   ",
			base_fname);
	...
	if (opt_analyze_mode & ANA_MKWANA)
	{
		...
		if (n > 0)
		{
			...
			fprintf (f, "#MKWANA: START-POS = %11.3f %11.3f %11.3f\n", ktpt->position[0],
				ktpt->position[1], ktpt->position[2]);
			fprintf (f, "#MKWANA: START-DIR = %11.3f %11.3f %11.3f\n", ktpt->rotation[0],
				ktpt->rotation[1], ktpt->rotation[2]);
		}
...
{
	if (opt_analyze_mode != ANA_MD_DEFAULT && !(opt_analyze_mode & ANA_MD_KCL))
	{
		return;
	}
	...
	for (i = 0; i < N_KCL_FLAG; i++)
		if (count[i])
			fprintf (f, "KCL-TYPE 0x%02x  0x%03x  0x%04x %6.2f%%  T%02u %s\n", i & 0x1f, i >> 5,
				i & ~0x1f, 100.0 * count[i] / n, index, slash);
