...
static void print_float (uint n, const float *f1, const float *f2, double epsilon)
{
	while (n-- > 0)
	{
		printf (" %s%11.3f",
			*f1 == *f2						  ? colout->COL_OK
				: fabsf (*f1 - *f2) > epsilon ? colout->COL_WARN
											  : colout->COL_HINT,
			*f1);
		f1++;
		f2++;
...
static void print_rot (uint n, const float *f1, const float *f2)
{
	while (n-- > 0)
	{
		printf (" %s%7.2f",
			*f1 == *f2							  ? colout->COL_OK
				: fabsf (*f1 - *f2) > epsilon_rot ? colout->COL_WARN
												  : colout->COL_HINT,
			*f1);
		f1++;
		f2++;
...
static void print_dec8_6 (uint n, const u8 *u1, const u8 *u2)
{
	while (n-- > 0)
	{
		printf (" %s%6s", *u1 == *u2 ? colout->COL_OK : colout->COL_WARN, PrintU8M1 (*u1));
		u1++;
		u2++;
	}
}
...
static void print_hex16 (uint n, const u16 *u1, const u16 *u2)
{
	while (n-- > 0)
	{
		printf (" %s%#6x", *u1 == *u2 ? colout->COL_OK : colout->COL_WARN, *u1);
		u1++;
		u2++;
	}
}
...
static uint diff_kmp_value (kmp_t *kmp1, // first kmp to compare
...
static void print_ph (char mode, const kmp_enph_entry_t *p1, const kmp_enph_entry_t *p2)
{
	if (p2 && colout->colorize)
	{
		...
		printf ("%c %s%3u %s%3u %s%3u %s:", mode, p1->pt_start == p2->pt_start ? ok : warn,
			p1->pt_start, end1 == end2 ? ok : colout->COL_HINT, end1,
			p1->pt_len == p2->pt_len ? ok : warn, p1->pt_len, colout->reset);
		...
		printf (" : %s0x%02x %s0x%02x%s\n", p1->setting[0] == p2->setting[0] ? ok : warn,
			p1->setting[0], p1->setting[1] == p2->setting[1] ? ok : warn, p1->setting[1],
			colout->reset);
	}
	else
	{
		printf ("%c %3u %3u %3u :", mode, p1->pt_start, p1->pt_start + p1->pt_len - 1, p1->pt_len);
...
static sort_ph_t *print_ph_single (
	const section_info_t *si, char mode, const sort_ph_t *e, sort_ph_t *end)
{
	DASSERT (si);
	DASSERT (e);
	DASSERT (end);
	...
	while (e < end)
	{
		printf ("%s%s: Only found in source %c @%u%s\n",
			mode == '<' ? colout->COL_SRC1 : colout->COL_SRC2, si->sect_name,
			mode == '<' ? '1' : '2', e->rank, colout->reset);
...
static int diff_ph (const sort_ph_t *e1, const sort_ph_t *e2)
{
	...
	uint count = (e1->data.pt_start != e2->data.pt_start) + (e1->data.pt_len != e2->data.pt_len);
...
static uint diff_ph_si (const section_info_t *si)
{
	...
	if (l1->used != l2->used)
	{
		...
		printf ("%s%s: Number of %s groups differ: Have %u and %u.%s\n", colout->COL_DIFF,
			si->sect_name, si->subject, l1->used, l2->used, colout->reset);
		if (si->verbose == 1)
			return 1;
		diff_count++;
	}
	...
	while (e1 < end1 && e2 < end2)
	{
		...
		if (stat)
		{
			...
			printf ("%s%s: %c%s group @%u:%u differ%s\n", colout->COL_DIFF, si->sect_name,
				toupper ((int)si->subject[0]), si->subject + 1, e1->rank, e2->rank, colout->reset);
...
static void print_pt (char mode, const kmp_enpt_entry_t *p1, const kmp_enpt_entry_t *p2)
{
	if (p2 && colout->colorize)
	{
		ccp ok = colout->COL_OK;
		...
		printf ("  %s%7.3f  %s%#6x %s%#6x%s\n",
			p1->scale == p2->scale								? ok
				: fabsf (p1->scale - p2->scale) > epsilon_scale ? colout->COL_WARN
																: colout->COL_HINT,
			p1->scale, p1->prop[0] == p2->prop[0] ? ok : warn, p1->prop[0],
			p1->prop[1] == p2->prop[1] ? ok : warn, p1->prop[1], colout->reset);
	}
	else
	{
		printf ("%c  %11.3f %11.3f %11.3f  %7.3f  %#6x %#6x\n", mode, p1->position[0],
			p1->position[1], p1->position[2], p1->scale, p1->prop[0], p1->prop[1]);
	}
}

//-----------------------------------------------------------------------------

static sort_pt_t *print_pt_single (
	const section_info_t *si, char mode, const sort_pt_t *e, sort_pt_t *end)
{
	DASSERT (si);
	DASSERT (e);
	DASSERT (end);
...
static int diff_pt (const sort_pt_t *e1, const sort_pt_t *e2)
{
	...
	uint count = (e1->data.prop[0] != e2->data.prop[0]) + (e1->data.prop[1] != e2->data.prop[1]);
...
static uint diff_pt_si (const section_info_t *si)
{
	...
	if (l1->used != l2->used)
	{
		...
		printf ("%s%s: Number of %ss differ: Have %u and %u.%s\n", colout->COL_DIFF, si->sect_name,
			si->subject, l1->used, l2->used, colout->reset);
		if (si->verbose == 1)
			return 1;
		diff_count++;
	}
	...
	while (e1 < end1 && e2 < end2)
	{
		...
		if (stat)
		{
			...
			printf ("%s%s: %s @%u:%u differ%s\n", colout->COL_DIFF, si->sect_name, si->subject,
...
static void print_pt2 (char mode, const kmp_jgpt_entry_t *p1, const kmp_jgpt_entry_t *p2)
{
	if (p2 && colout->colorize)
	{
		ccp ok = colout->COL_OK;
		...
		printf ("  %s%6d %s%#6x%s\n", p1->id == p2->id ? ok : colout->COL_HINT, p1->id,
			p1->effect == p2->effect ? ok : colout->COL_WARN, p1->effect, colout->reset);
	}
	else
	{
		printf ("%c %11.3f %11.3f %11.3f  %7.2f %7.2f %7.2f  %6d %#6x\n", mode, p1->position[0],
			p1->position[1], p1->position[2], p1->rotation[0], p1->rotation[1], p1->rotation[2],
			p1->id, p1->effect);
	}
}
...
static sort_pt2_t *print_pt2_single (
	const section_info_t *si, char mode, const sort_pt2_t *e, sort_pt2_t *end)
{
	DASSERT (si);
	DASSERT (e);
	DASSERT (end);
	...
	while (e1 < end1 && e2 < end2)
	{
		...
		if (stat)
		{
			...
			printf ("%s%s: %s group @%u:%u differ%s\n", colout->COL_DIFF, si->sect_name,
				si->subject, e1->rank, e2->rank, colout->reset);
...
static void print_area (char mode, const kmp_area_entry_t *e1, const kmp_area_entry_t *e2)
{
	if (e2 && colout->colorize)
	{
		...
		printf ("%c %s%5u  %s%5u", mode, OK_WARN1 (e1, e2, mode), OK_WARN1 (e1, e2, type));
		print_float (3, e1->position, e2->position, epsilon_pos);
		putchar (' ');
		print_hex16 (2, e1->setting, e2->setting);

		printf ("%s\n%c %s%5u  %s%5u", colout->reset, mode, OK_WARN1 (e1, e2, dest_id),
			OK_WARN1 (e1, e2, prio));
	...
	else
	{
		printf ("%c %5u %5u  %11.3f %11.3f %11.3f  %#6x %#6x\n"
				...
				"%c %24.3f %11.3f %11.3f\n",
			mode, e1->mode, e1->type, e1->position[0], e1->position[1], e1->position[2],
			e1->setting[0], e1->setting[1], mode, e1->dest_id, e1->prio, e1->rotation[0],
			e1->rotation[1], e1->rotation[2], PrintU8M1 (e1->route), PrintU8M1 (e1->enemy),
			e1->unknown_2e, mode, e1->scale[0], e1->scale[1], e1->scale[2]);
	}
}

//-----------------------------------------------------------------------------

static sort_area_t *print_area_single (char mode, const sort_area_t *e, sort_area_t *end)
{
	DASSERT (e);
	DASSERT (end);
	...
	while (e < end)
	{
		printf ("%sAREA: Only found in source %c @%u%s\n",
			mode == '<' ? colout->COL_SRC1 : colout->COL_SRC2, mode == '<' ? '1' : '2', e->rank,
			colout->reset);
...
static int diff_area (const sort_area_t *e1, const sort_area_t *e2)
{
	...
	uint count = (e1->data.mode != e2->data.mode) + (e1->data.type != e2->data.type)
		+ (e1->data.dest_id != e2->data.dest_id) + (e1->data.prio != e2->data.prio)
		+ (e1->data.setting[0] != e2->data.setting[0])
		+ (e1->data.setting[1] != e2->data.setting[1]) + (e1->data.route != e2->data.route)
		+ (e1->data.enemy != e2->data.enemy) + (e1->data.unknown_2e != e2->data.unknown_2e);
...
static uint diff_KMP_AREA (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static void print_came (char mode, const kmp_came_entry_t *e1, const kmp_came_entry_t *e2)
{
	if (e2 && colout->colorize)
	{
		ccp ok = colout->COL_OK;
		...
		printf ("%c %s%5u %s%5u ", mode, OK_WARN1 (e1, e2, type), OK_WARN1 (e1, e2, came_speed));
		print_float (3, e1->position, e2->position, epsilon_pos);
		putchar (' ');
		print_float (1, &e1->zoom_begin, &e2->zoom_begin, epsilon_pos);

		printf ("%s\n%c %s%5s %s%5u ", colout->reset, mode, OK_WARN0 (e1, e2, next),
			PrintU8M1 (e1->next), OK_WARN1 (e1, e2, zoom_speed));
		print_float (3, e1->rotation, e2->rotation, epsilon_pos);
		putchar (' ');
		print_float (1, &e1->zoom_end, &e2->zoom_end, epsilon_pos);

		printf ("%s\n%c %s%5u %s%5u ", colout->reset, mode, OK_WARN1 (e1, e2, unknown_0x02),
			OK_WARN1 (e1, e2, viewpt_speed));
		print_float (3, e1->viewpt_begin, e2->viewpt_begin, epsilon_pos);

		printf ("%s\n%c %s%5s %s%5u ", colout->reset, mode, OK_WARN0 (e1, e2, route),
			PrintU8M1 (e1->route), OK_WARN1 (e1, e2, unknown_0x0a));
	...
	else
	{
		printf ("%c %5u %5u  %11.3f %11.3f %11.3f  %11.3f\n"
				...
				"%c %5s %5u  %11.3f %11.3f %11.3f  %11.3f\n",
			mode, e1->type, e1->came_speed, e1->position[0], e1->position[1], e1->position[2],
			e1->zoom_begin, mode, PrintU8M1 (e1->next), e1->zoom_speed, e1->rotation[0],
			e1->rotation[1], e1->rotation[2], e1->zoom_end, mode, e1->unknown_0x02,
			e1->viewpt_speed, e1->viewpt_begin[0], e1->viewpt_begin[1], e1->viewpt_begin[2], mode,
			PrintU8M1 (e1->route), e1->unknown_0x0a, e1->viewpt_end[0], e1->viewpt_end[1],
			e1->viewpt_end[2], e1->time);
	}
}

//-----------------------------------------------------------------------------

static sort_came_t *print_came_single (char mode, const sort_came_t *e, sort_came_t *end)
{
	DASSERT (e);
	DASSERT (end);
...
static int diff_came (const sort_came_t *e1, const sort_came_t *e2)
{
	...
	uint count = (e1->data.type != e2->data.type) + (e1->data.next != e2->data.next)
		+ (e1->data.unknown_0x02 != e2->data.unknown_0x02) + (e1->data.route != e2->data.route)
...
static uint diff_KMP_CAME (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
		...
		for (int vi = 0; vi < 2; vi++)
		{
			...
			if (v1 != v2)
			{
				...
				printf ("%sCAME: Index of %s camera differ: Have %u and %u.%s\n", colout->COL_WARN,
					vi ? "opening" : "selection", v1, v2, colout->reset);
				if (verbose == 1)
					return 1;
				diff_count++;
			}
...
static uint diff_KMP_CKPH (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static void print_ckpt (char mode, const kmp_ckpt_entry_t *e1, const kmp_ckpt_entry_t *e2)
{
	if (e2 && colout->colorize)
	{
		ccp ok = colout->COL_OK;
		...
		printf ("  %s%3d %s%3s %s%3s %s%3s%s\n", OK_WARN1 (e1, e2, respawn),
			OK_WARN0 (e1, e2, mode), PrintU8M1 (e1->mode), OK_WARN0 (e1, e2, prev),
			PrintU8M1 (e1->prev), OK_WARN0 (e1, e2, next), PrintU8M1 (e1->next), colout->reset);
	}
	else
	{
		printf ("%c %11.3f %11.3f  %11.3f %11.3f  %3d %3s %3s %3s\n", mode, e1->left[0],
			e1->left[1], e1->right[0], e1->right[1], e1->respawn, PrintU8M1 (e1->mode),
			PrintU8M1 (e1->prev), PrintU8M1 (e1->next));
	}
}

//-----------------------------------------------------------------------------

static sort_ckpt_t *print_ckpt_single (char mode, const sort_ckpt_t *e, sort_ckpt_t *end)
{
	DASSERT (e);
	DASSERT (end);
...
static int diff_ckpt (const sort_ckpt_t *e1, const sort_ckpt_t *e2)
{
	...
	uint count = (e1->data.respawn != e2->data.respawn) + (e1->data.mode != e2->data.mode)
		+ (e1->data.prev != e2->data.prev) + (e1->data.next != e2->data.next);
...
static uint diff_KMP_CKPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_CNPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_ENPH (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_ENPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static void print_gobj (
	char mode, const kmp_gobj_entry_t *g1, const kmp_gobj_entry_t *g2, const ObjectInfo_t *oi)
{
	...
	if (g2 && colout->colorize)
	{
		...
		printf ("%c %s%#6x", mode, g1->obj_id == g2->obj_id ? ok : warn, g1->obj_id);
		...
		printf ("  %s%6s%s\n%c %s%#6x", g1->route_id == g2->route_id ? ok : warn,
			PrintU16M1 (g1->route_id), colout->reset, mode, g1->ref_id == g2->ref_id ? ok : warn,
			g1->ref_id);
		...
		printf ("  %s%#6x%s\n%c       ", g1->pflags == g2->pflags ? ok : warn, g1->pflags,
			colout->reset, mode);
	...
	{
		snprintf (buf1, sizeof (buf1), oi->format1 ? oi->format1->std : settings_format_uuuu.std,
			g1->setting[0], g1->setting[1], g1->setting[2], g1->setting[3]);

		snprintf (buf2, sizeof (buf2), oi->format2 ? oi->format2->std : settings_format_uuuu.std,
			g1->setting[4], g1->setting[5], g1->setting[6], g1->setting[7]);

		printf ("%c %#6x %11.3f %11.3f %11.3f  %s  %6s\n"
				"%c %#6x %11.3f %11.3f %11.3f  %s  %#6x\n"
				"%c        %11.3f %11.3f %11.3f\n",
			mode, g1->obj_id, g1->position[0], g1->position[1], g1->position[2], buf1,
			PrintU16M1 (g1->route_id), mode, g1->ref_id, g1->rotation[0], g1->rotation[1],
			g1->rotation[2], buf2, g1->pflags, mode, g1->scale[0], g1->scale[1], g1->scale[2]);
	}
}

//-----------------------------------------------------------------------------

static sort_gobj_t *print_gobj_single (char mode, const sort_gobj_t *e, sort_gobj_t *end)
{
	DASSERT (e);
	DASSERT (end);
...
static int diff_gobj (const sort_gobj_t *e1, const sort_gobj_t *e2)
{
	...
	uint count = (e1->data.obj_id != e2->data.obj_id) + (e1->data.ref_id != e2->data.ref_id)
		+ (e1->data.route_id != e2->data.route_id) + (e1->data.pflags != e2->data.pflags);
...
static uint diff_KMP_GOBJ (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_ITPH (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_ITPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_JGPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static inline void print_ktpt (char mode, const kmp_ktpt_entry_t *e1, const kmp_ktpt_entry_t *e2)
{
...
static sort_ktpt_t *print_ktpt_single (char mode, const sort_ktpt_t *e, sort_ktpt_t *end)
{
	DASSERT (e);
	DASSERT (end);
...
static int diff_ktpt (const sort_ktpt_t *e1, const sort_ktpt_t *e2)
{
	...
	uint count
		= (e1->data.player_index != e2->data.player_index) + (e1->data.unknown != e2->data.unknown);
...
static uint diff_KMP_KTPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static uint diff_KMP_MSPT (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
...
static int diff_poti_group (const kmp_poti_group_t *e1, const kmp_poti_group_t *e2)
{
	...
	return (e1->n_point != e2->n_point) + (e1->smooth != e2->smooth) + (e1->back != e2->back);
}
...
static int diff_poti_point (const kmp_poti_point_t *e1, const kmp_poti_point_t *e2)
{
	...
	uint count = (e1->speed != e2->speed) + (e1->unknown != e2->unknown);
...
static uint diff_KMP_POTI (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
	...
	if (pl1->used != pl2->used)
	{
		if (!verbose)
			return 1;
		printf (
			"POTI: Number of total route points differ: Have %u and %u.\n", pl1->used, pl2->used);
		if (verbose == 1)
			return 1;
		diff_count++;
	}
...
static inline void print_stgi (char mode, const kmp_stgi_entry_t *e1, const kmp_stgi_entry_t *e2)
{
	if (e2 && colout->colorize)
	{
		ccp ok = colout->COL_OK;
		...
		printf ("%c %s%5u %s%4u %s%5u %s%5u   %s%#10x  %s0x%02x %s%#5x %s%#6x", mode,
			OK_WARN1 (e1, e2, lap_count), OK_WARN1 (e1, e2, pole_pos),
			OK_WARN1 (e1, e2, narrow_start), OK_WARN1 (e1, e2, enable_lens_flare),
			OK_WARN1 (e1, e2, flare_color), OK_WARN1 (e1, e2, flare_alpha),
			OK_WARN1 (e1, e2, unknown_09), OK_WARN1 (e1, e2, speed_mod));
	...
	{
		printf ("%c %5u %4u %5u %5u   %#10x  0x%02x %#5x %#6x", mode, e1->lap_count, e1->pole_pos,
			e1->narrow_start, e1->enable_lens_flare, e1->flare_color, e1->flare_alpha,
			e1->unknown_09, e1->speed_mod);
		if (e1->speed_mod)
...
static sort_stgi_t *print_stgi_single (char mode, const sort_stgi_t *e, sort_stgi_t *end)
{
	DASSERT (e);
	DASSERT (end);
...
static int diff_stgi (const sort_stgi_t *e1, const sort_stgi_t *e2)
{
	...
	return (e1->data.lap_count != e2->data.lap_count) + (e1->data.pole_pos != e2->data.pole_pos)
		...
		+ (e1->data.unknown_09 != e2->data.unknown_09) + (e1->data.speed_mod != e2->data.speed_mod);
}

//-----------------------------------------------------------------------------

static uint diff_KMP_STGI (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
	...
	if (order_differ)
	{
		if (verbose)
			printf (
				"%sSTGI: Order of stage info records differ%s\n", colout->COL_DIFF, colout->reset);
		if (verbose < 2)
			return 1;
		diff_count++;
	}
...
enumError DiffKMP (kmp_t *kmp1, // first kmp to compare
	kmp_t *kmp2, // second kmp tp compare
	uint verbose // 0:quiet, 1:summary only, 2:full report
)
{
