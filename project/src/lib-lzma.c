...
{
	return compr_level < 1 ? LZMA_DEFAULT_COMPR : compr_level < 9 ? compr_level : 9;
}
...
)
{
	switch (err)
	{
		...
		case SZ_ERROR_OUTPUT_EOF:
			return "Putput buffer overflow";
		case SZ_ERROR_PARAM:
			return "Incorrect parameter";
		case SZ_ERROR_PROGRESS:
			return "Some break from progress callback";
		case SZ_ERROR_READ:
			return "Reading error";
		case SZ_ERROR_THREAD:
			return "Errors in multithreading functions";
	...
	*dest_written = 0;
	...
	if (res == SZ_OK)
	{
		...
		if (res == SZ_OK)
		{
			...
			res = LzmaEnc_MemEncode (
				lzma, dest, &out_len, src, in_len, true, 0, &lzma_alloc, &lzma_alloc);
		}
	}
...
)
{
	...
	enumError err
		= EncodeLZMAbuf (dest, dest_size, dest_written, src, src_size, compr_level, add_dec_size);
	if (err)
	{
		FREE (dest_alloced);
		*dest_ptr = 0;
...
)
{
	...
	*dest_written = 0;
	...
	if (res != SZ_OK)
		return ERROR0 (
			ERR_LZMA, "Error while setup LZMA properties: %s\n", GetMessageLZMA (res, "?"));
	...
	while (src_size > 0)
	{
		...
		PRINT0 ("DECODED, res=%s, stat=%d, in=%zu/%u out=%zu/%u\n", GetMessageLZMA (res, "?"),
			status, in_len, src_size, out_len, outbuf_size);
		...
		if (res != SZ_OK)
		{
			ERROR0 (ERR_LZMA, "Error while reading LZMA stream: %s\n", GetMessageLZMA (res, "?"));
			goto error;
		}
...
)
{
	...
	*dest_written = header_size;
	...
	enumError err
		= DecodeLZMApart (dest + header_size, dest_size, dest_written, src + 4, src_size - 4);
	if (err)
	{
		FREE (dest);
		*dest_ptr = 0;
