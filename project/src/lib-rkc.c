...
)
{
	...
	if (!it->no_recurse && it->recurse_level < it->recurse_max)
	{
		...
		szs_iterator_func it_func = GetIteratorFunction (szs2.fform_arch, it->itpar.cut_files);
...
static uint get_file_size (ccp source_dir, ccp fname, uint min, uint max)
{
	...
	if (szs_size < 0)
	{
		ERROR1 (ERR_CANT_OPEN, "Can't open file: %s/%s", source_dir, fname);
		return 0;
	}

	if (szs_size < min || szs_size > max)
	{
		ERROR0 (ERR_INVALID_DATA, "Wrong filesize (not %u..%u, but %llu): %s/%s", min, max,
			szs_size, source_dir, fname);
		return 0;
	}
...
)
{
	...
	const uint rkco_size = get_file_size (
		source_dir, RKCO_FILENAME, sizeof (rkco_t), RKC_U8_OFFSET - sizeof (rkct_t));
	const uint szs_size
		= get_file_size (source_dir, RKC_U8_FILENAME, sizeof (yaz0_header_t), RKC_U8_LIMIT);
	...
	enumError err;
	err = LoadFile (
		source_dir, RKCO_FILENAME, 0, szs->data + sizeof (rkct_t), rkco_size, 0, &szs->fatt, true);
	if (err)
		return err;

	err = LoadFile (
		source_dir, RKC_U8_FILENAME, 0, szs->data + RKC_U8_OFFSET, szs_size, 0, &szs->fatt, true);
	if (err)
		return err;
