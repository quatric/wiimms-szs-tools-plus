...
static const KeywordTab_t opt_draw_tab[] = { { 0, "NONE", 0, DMD_M_RESET },
	{ 0, "-ALL", 0, DMD_M_RESET }, { DMD_M_ALL, "ALL", 0, DMD_M_RESET },
	{ 0, "-POINTS", "-PTS", DMD_M_PTS }, { DMD_M_PTS, "POINTS", "PTS", DMD_M_PTS },
	{ 0, "-OBJECTS", "-GOBJ", DMD_M_GOBJ }, { DMD_M_GOBJ, "OBJECTS", "GOBJ", DMD_M_GOBJ },
	{ DMD_M_RESPAWN, "RESPAWN", 0, DMD_M_RESPAWN }, { DMD_M_DEFAULT, "DEFAULT", 0, DMD_M_ALL },

	{ 0, "-OFFLINE", 0, DMD_M_OFFLINE }, { DMD_M_OFFLINE, "OFFLINE", 0, DMD_M_OFFLINE },
	{ 0, "-ONLINE", 0, DMD_M_ONLINE }, { DMD_M_ONLINE, "ONLINE", 0, DMD_M_ONLINE },

	{ DMD_1OFFLINE, "1OFFLINE", 0, 0 }, { DMD_2OFFLINE, "2OFFLINE", 0, 0 },
	{ DMD_3OFFLINE, "3OFFLINE", "4OFFLINE", 0 }, { DMD_1ONLINE, "1ONLINE", 0, 0 },
	{ DMD_2ONLINE, "2ONLINE", 0, 0 }, { DMD_3ONLINE, "3ONLINE", "4ONLINE", 0 },

	{ DMD_ENPT, "ENPT", 0, 0 }, { DMD_ITPT, "ITPT", 0, 0 }, { DMD_CKPT, "CKPT", 0, 0 },
	{ DMD_CJGPT, "CJGPT", 0, 0 }, { DMD_JGPT, "JGPT", 0, 0 }, { DMD_KTPT, "KTPT", 0, 0 },
	{ DMD_CNPT, "CNPT", 0, 0 }, { DMD_POTI, "POTI", 0, 0 }, { DMD_AREA, "AREA", 0, 0 },
	{ DMD_ITEMBOX, "ITEMBOXES", 0, 0 }, { DMD_COIN, "COINS", 0, 0 },
	{ DMD_ROADOBJ, "ROADOBJECTS", 0, 0 }, { DMD_SOLIDOBJ, "SOLIDOBJECTS", 0, 0 },
	{ DMD_DECORATION, "DECORATION", 0, 0 }, { DMD_BLACK, "BLACK", 0, 0 },
	{ DMD_WHITE, "WHITE", 0, 0 }, { DMD_KCL, "KCL", 0, 0 },

	{ DMD_DETAILED, "DETAILED", 0, 0 }, { DMD_DISPATCH, "DISPATCH", 0, 0 },
...
int ScanOptDraw (ccp arg)
{
	...
	s64 stat = ScanKeywordList (arg, opt_draw_tab, 0, true, 0,
		*arg == '-' ? DMD_M_ALL : (DMD_M_ALL & ~DMD_M_RESET), "Option --draw", ERR_SYNTAX);
	if (stat != M1 (stat))
	{
...
ccp GetDrawMode ()
{
	...
	if (!*buf)
		PrintKeywordList (buf, sizeof (buf), 0, opt_draw_tab, KMP_DRAW, DMD_M_DEFAULT, 0);
	return buf;
}
...
static void TransformByGOBJ (const kmp_gobj_entry_t *obj, // object to draw
...
static double3 *CreateRotatedShape (
	...
	const kmp_gobj_entry_t *obj, // NULL or object for transformation
	DbFlags_t flags // flags DBF_SCALE and DBF_ROTATE
)
{
...
static int DrawRotatedShape (struct kcl_t *dk, // pointer to valid KCL
...
void AddAreas2KCL (DrawKCL_t *dk // pointer to initalized object
)
{
	...
	for (i = 0; i < n; i++, area++)
	{
		...
		if (dk->detailed)
			AppendPrismKCL (dk->draw, KCL_FLAG_AREA_ARROW, &base, &pos, 0, width, 6, 0,
				KCL_FLAG_AREA_ARROW, width, KCL_FLAG_AREA_ARROW, 1.8, false);
		else
			AppendPrismKCL (dk->draw, KCL_FLAG_AREA_ARROW, &base, &pos, 0, width, 4, 1, ~0, 0.0, ~0,
				0.0, false);

		if (area->mode == 1)
		{
			AppendCylinderKCL (
				dk->draw, KCL_FLAG_AREA_BORDER, &pos, &scale, 5000, &rot, 10, KCL_FLAG_AREA_BORDER);
		}
		else
		{
			double3 size, shift;
...
static void add_ckpt_joist (kcl_t *kcl, const kmp_ckpt_entry_t *ckpt, const kmp_ckpt_entry_t *c1,
	uint next, double min, double max, double width, u32 kcl_flag_lt, u32 kcl_flag_rt)
{
...
void AddCheckPoints2KCL (DrawKCL_t *dk, // pointer to initalized object
...
)
{
	...
	for (i = 0; i < n; i++, c1++)
	{
		...
		{
			bool new_group;
			int next = GetNextPointKMP (
				dk->kmp, KMP_CKPT, c1 - ckpt, link, true, -1, 0, 0, &new_group, 0);
			if (next >= 0)
			{
				if (std_next == next)
					std_next = -1;
...
void AddArrow2KCL (DrawKCL_t *dk, // pointer to initalized object
...
)
{
	...
	for (i = 0; i < n; i++, pt++)
	{
		...
		if (dk->detailed)
			AppendPrismKCL (dk->draw, arrow_color, &base, &pos, 0, width, 6, 0, arrow_color, width,
				arrow_color, 1.8, false);
		else
			AppendPrismKCL (
				dk->draw, arrow_color, &base, &pos, 0, width, 4, 1, ~0, 0.0, ~0, 0.0, false);
...
static void AddPotiRoute2KCL (struct kcl_t *kcl, // pointer to valid KCL
...
)
{
	...
	for (ei = 1; ei < en; ei++)
	{
		...
		if (detailed)
			AppendPrismKCL (
				kcl, color, &pt1, &pt2, &helper, width, 6, 0, color, width, ~0, 0.0, false);
		else
			AppendPrismKCL (kcl, color, &pt1, &pt2, &helper, width, 4, 1, ~0, 0.0, ~0, 0.0, false);
	}
	...
	if (!pg->back && en > 2) // is cyclic
	{
		...
		if (detailed)
			AppendPrismKCL (
				kcl, color, &pt2, &pt0, &helper, width, 6, 0, color, width, ~0, 0.0, false);
		else
			AppendPrismKCL (kcl, color, &pt2, &pt0, &helper, width, 4, 1, ~0, 0.0, ~0, 0.0, false);
	}
}
...
void AddRoutes2KCL (DrawKCL_t *dk, // pointer to initalized object
...
)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		else
		{
			...
			{
				if (dk->detailed)
					AppendPrismKCL (dk->draw, color_std, &pt0, &pt1, &helper, width_std, 6, 0,
						color_std, width_std, ~0, 0.0, false);
				else
					AppendPrismKCL (dk->draw, color_std, &pt0, &pt1, &helper, width_std, 4, 1, ~0,
						0.0, ~0, 0.0, false);
			}
		...
		for (i = 0; i < li->used; i++)
		{
			...
			for (j0 = 0; j0 < li->used; j0++)
			{
				...
				if (color)
				{
					...
					if (dk->detailed)
						AppendPrismKCL (dk->draw, color, pt1, pt2, &helper, width, 6, 0, color,
							width, arrow_col, arrow_size, arrow_flip);
					else
						AppendPrismKCL (dk->draw, color, pt1, pt2, &helper, width, 4, 1, ~0, 0.0,
...
void AddPotiRoutes2KCL (DrawKCL_t *dk // pointer to initalized object
)
{
	DASSERT (dk);
	DASSERT (dk->draw);
...
static uint td_minmax (const kcl_tridata_t *td, // valid tridata
...
void AddCannons2KCL (DrawKCL_t *dk // pointer to initalized object
)
{
	DASSERT (dk);
	DASSERT (dk->draw);
	...
	while (ic < nc)
	{
		...
		for (; td < td_end; td++)
			if (td->status & TD_MARK && td->cur_flag == ftype)
			{
				...
				if (dk->detailed)
					AppendPrismKCL (dk->draw, KCL_FLAG_CNPT_WAY, &sum, &dest, &helper,
						cube_size / 4, 6, 0, KCL_FLAG_CNPT_WAY, cube_size, KCL_FLAG_CNPT_WAY, 2.0,
						false);
				else
					AppendPrismKCL (dk->draw, KCL_FLAG_CNPT_WAY, &sum, &dest, &helper,
						cube_size / 4, 4, 1, ~0, 0.0, ~0, 0.0, false);
...
static void SetupMatrixD (MatrixD_t *mat, // will be initialized
	const kmp_gobj_entry_t *op, // object to draw
...
typedef uint (*add_func) (struct kcl_t *dk, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
...
static uint Add_RECTANGLE (struct kcl_t *dk, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_CUBE (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_CUBOID (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_CYLINDER_Y (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_CYLINDER_Z (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_ITEMBOX (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
)
{
	...
	const u32 col = op->setting[1] ? KCL_FLAG_ITEMBOX_PLAYER
		: op->setting[2]		   ? KCL_FLAG_ITEMBOX_ENEMY
								   : KCL_FLAG_ITEMBOX;
	AppendCuboidKCL (draw, col, &cub);
	return 1;
}
...
static uint Add_COIN (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_KINOKO (struct kcl_t *dk, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
)
{
	...
	const u32 col = as_road	  ? OBJ_ROAD_COL
		: op->obj_id == 0x1fa ? KCL_FLAG_KINOKO_GREEN
							  : KCL_FLAG_KINOKO_RED;
...
static uint Add_PYRAMID (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
static uint Add_TREE (struct kcl_t *draw, // pointer to valid KCL
	const kmp_gobj_entry_t *op, // object to draw
	DbFlags_t flags, // file flags
	const s16 *data, // valid pointer to data
	uint data_size, // size of data
...
)
{
	...
	s16 d1[] = { KCL_FLAG_TREE_TRUNK - N_KCL_FLAG, data[1] + data[2], data[0] / 2, -data[1] };
	Add_CYLINDER_Y (draw, op, flags, d1, 4, false, false);

	s16 d2[] = { KCL_FLAG_TREE_CROWN - N_KCL_FLAG, 4, data[4] + data[5], data[3] / 2,
		data[2] - data[4] };
...
void AddObject2KCL (DrawKCL_t *dk, // pointer to initalized object
	const kmp_gobj_entry_t *op // object to draw
)
{
	DASSERT (dk);
...
void AddObjectsById2KCL (DrawKCL_t *dk, // pointer to initalized object
...
void AddObjectsByFlag2KCL (DrawKCL_t *dk, // pointer to initalized object
...
void AddZeroGround2KCL (DrawKCL_t *dk, // pointer to initalized object
...
int ScanOptPosMode (ccp arg)
{
	...
	static const KeywordTab_t keytab[] = { { SC_XPOS, "X", "XPOS", 0 }, { SC_YPOS, "Y", "YPOS", 0 },
		{ SC_ZPOS, "Z", "ZPOS", 0 }, { SC_DIR, "D", "DIR", 0 },

		{ SC_COLOR, "C", "COLOR", 0 }, { SC_TRANSP, "T", "TRANSPARENT", 0 },

		{ SC_FILTER, "FA", "FILTER-A", 0 }, { SC_FILTER, "FB", "FILTER-B", 1 },
	...
	for (;;)
	{
		...
		if (*arg == '+')
		{
			if (current_pos_param.n_param == MAX_POS_PARAM)
				return ERROR0 (ERR_SYNTAX, "Too many '+' terms for option --pos-mode: %s\n", arg);
		...
		if (!i)
			return ERROR0 (ERR_SYNTAX, "Option --pos-mode: Missing sub command: %s\n", arg);
		name[i] = 0;
		// ccp subcmd = arg;
		arg += i;
		...
		if (!key)
		{
			...
			if (stat < 1 && cstat > 1)
				return PrintKeywordError (
					kcl_color_key_tab, name, cstat, 0, "Color of --pos-mode:");
			return PrintKeywordError (keytab, name, stat, 0, "Sub command of --pos-mode:");
		}
		...
		{
			case SC_XPOS:
				pp->idx_x = str2ul (arg, &end, 10);
				break;
			case SC_YPOS:
				pp->idx_y = str2ul (arg, &end, 10);
				break;
			case SC_ZPOS:
				pp->idx_z = str2ul (arg, &end, 10);
				break;
			case SC_DIR:
				pp->idx_d = str2ul (arg, &end, 10);
				break;
			...
			case SC_FILTER:
			{
				...
				if ((ccp)end > arg)
				{
					...
					while (*arg && *arg != ',' && *arg != '+')
						arg++;
					StringCopySM (pp->filter[key->opt], sizeof (*pp->filter), end, arg - end);
					end = 0;
		...
		{
			if (arg == (ccp)end)
				return ERROR0 (ERR_SYNTAX, "Option --pos-mode: Missing parameter for '%s': %s\n",
					key->name1, arg);
			arg = end;
		}
	...
	next_parm:
		...
		if (!*arg)
			break;
		if (*arg != ',' && *arg != '+')
			return ERROR0 (ERR_SYNTAX, "Option --pos-mode: Comma or plus expected: %s\n", arg);
	}
