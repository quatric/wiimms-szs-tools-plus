...
#define TITLE                                                                                      \
	WCTCT_SHORT ": " WCTCT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
static enumError cmd_decode ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		char dest[PATH_MAX];
		SubstDest (dest, sizeof (dest), arg, opt_dest, def_path, GetExtFF (FF_CTDEF, 0), false);
...
static enumError cmd_create ()
{
	...
	static const KeywordTab_t tab[] = { { CTS_CT0CODE, "BAD0CODE", "BOOT_CODE", 1 },
		{ CTS_CT0DATA, "BAD0DATA", "BOOT_DATA", 1 }, { CTS_CT1CODE, "BAD1CODE", "CODE", 1 },
		{ CTS_CT1DATA, "BAD1DATA", "DATA", 1 },

		{ CTS_CUP1, "CUP1", 0, 0 }, { CTS_CRS1, "CRS1", 0, 0 }, { CTS_MOD1, "MOD1", 0, 1 },
		{ CTS_MOD2, "MOD2", 0, 1 }, { CTS_OVR1, "OVR1", 0, 1 },

		{ CTS_TEX0, "TEX0", 0, 1 }, { CTS_BRRES, "BRRES", 0, 1 }, { CTS_SZS, "SZS", 0, 1 },

		{ CTS_BMG, "BMG", 0, 0 }, { CTS_TRACKS, "TRACKS", 0, 0 }, { CTS_LIST, "LIST", 0, 0 },
		{ CTS_REF, "REF", 0, 0 }, { CTS_FULL, "FULL", 0, 0 },
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		char dest[PATH_MAX];
		SubstDest (
			dest, sizeof (dest), arg, opt_dest, def_path, GetExtFF (dest_ff1, dest_ff2), false);
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!testmode)
		{
			...
			if (IsCompressedFF (raw.fform_file))
			{
				...
				err = SaveFILE (
					dest, 0, dest_is_source || opt_overwrite, raw.data, raw.data_size, 0);
			if (err > ERR_WARNING)
				return err;
		}
		ResetCTCODE (&ctcode);
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_CT_DIR:
				...
				break;
			case GO_CT_LOG:
				err += ScanOptCtLog (optarg);
				break;
			case GO_ALLOW_SLOTS:
				err += ScanOptAllowSlots (optarg);
				break;
			case GO_IMAGES:
				opt_image_dir = optarg;
				break;
			case GO_ORDER_BY:
				opt_order_by = optarg;
				break;
			case GO_ORDER_ALL:
				...
				break;
			case GO_DYNAMIC:
				opt_dynamic_ctcode = true;
				break;
			case GO_WRITE_TRACKS:
				opt_write_tracks = optarg;
				break;
...
static enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_EXPORT:
			...
			break;
		case CMD_RAWDUMP:
			err = cmd_rawdump (ctcode_data_tab);
			break;

		case CMD_SYMBOLS:
			err = DumpSymbols (SetupVarsCTCODE ());
			break;
		case CMD_FUNCTIONS:
			SetupVarsCTCODE ();
			...
			break;
		case CMD_CALCULATE:
			err = ParserCalc (SetupVarsCTCODE ());
			break;
		case CMD_FLOAT:
			err = cmd_float ();
			break;
