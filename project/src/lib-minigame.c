...
static const KeywordTab_t minigame_param_list[]
	= { { 0x008, "OFFSET-008", 0, HEAD_PARAM }, { 0x00a, "OFFSET-00A", 0, HEAD_PARAM },
		  ...
		  { 0x1d4, "OFFSET-1D4", 0, BALLOON_PARAM }, { 0x1d6, "OFFSET-1D6", 0, BALLOON_PARAM },
		  { 0x1d8, "OFFSET-1D8", 0, BALLOON_PARAM }, { 0x1da, "OFFSET-1DA", 0, BALLOON_PARAM },

		  { 0x704, "OFFSET-704", 0, COIN_PARAM }, { 0x706, "OFFSET-706", 0, COIN_PARAM },
		  { 0x708, "OFFSET-708", 0, COIN_PARAM }, { 0x70a, "OFFSET-70A", 0, COIN_PARAM },
		  { 0x70c, "OFFSET-70C", 0, COIN_PARAM }, { 0x70e, "OFFSET-70E", 0, COIN_PARAM },
...
static const KeywordTab_t minigame_table_list[] = { { 0x010, "OFFSET-010", "BALLOON-DURATION",
														BALLOON_PARAM },
	// [[obsolete]], typo "BALLON" fixed in 2020-01
	{ 0x010, "BALLON-DURATION", 0, BALLOON_PARAM }, { 0x0ec, "OFFSET-0EC", 0, BALLOON_PARAM },

	{ 0x1dc, "OFFSET-1DC", "COIN-DURATION", COIN_PARAM }, { 0x2b8, "OFFSET-2B8", 0, COIN_PARAM },
	{ 0x394, "OFFSET-394", "START-COINS", COIN_PARAM },
	{ 0x470, "OFFSET-470", "MAX-COINS", COIN_PARAM }, { 0x54c, "OFFSET-54C", 0, COIN_PARAM },
...
enumError ScanRawMINIGAME (minigame_t *minigame, // MINIGAME data structure
...
)
{
	...
	if (!data || data_size != sizeof (minigame->data))
	{
		...
		return ERROR0 (ERR_INVALID_DATA, "Invalid file size (%u but not %zu bytes: %s\n", data_size,
			sizeof (minigame->data), minigame->fname);
	}
...
static const KeywordTab_t minigame_section_name[]
	= { { MINIGAME_IGNORE, "END", 0, 0 }, { MINIGAME_SETUP, "SETUP", 0, 0 },
		  { MINIGAME_PARAMETERS, "PARAMETERS", 0, 0 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static enumError ScanTextSETUP (minigame_t *minigame, // MINIGAME data structure
...
static enumError ScanTextPARAMETERS (minigame_t *minigame, // MINIGAME data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (minigame);
...
static enumError ScanTextTABLE (minigame_t *minigame, // MINIGAME data structure
	ScanInfo_t *si, // valid data
...
)
{
	...
	for (;;)
	{
		...
		if (slot >= 10)
		{
			...
			if (si->no_warn <= 0)
				ERROR0 (ERR_WARNING, "Slot must be between 0 and 9: [%s @%u]: %.*s\n", sf->name,
...
enumError ScanTextMINIGAME (minigame_t *minigame, // MINIGAME data structure
	bool init_minigame, // true: initialize 'minigame' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
...
enumError ScanMINIGAME (minigame_t *minigame, // MINIGAME data structure
	bool init_minigame, // true: initialize 'minigame' first
	const void *data, // data to scan
	uint data_size // size of 'data'
)
{
	...
	switch (fform)
	{
		...
		default:
			...
			return ERROR0 (ERR_INVALID_DATA, "No MINIGAME file: %s\n",
...
enumError LoadMINIGAME (minigame_t *minigame, // MINIGAME data structure
	bool init_minigame, // true: initialize 'minigame' first
...
enumError SaveRawMINIGAME (minigame_t *minigame, // pointer to valid MINIGAME
...
enumError SaveTextMINIGAME (const minigame_t *minigame, // pointer to valid MINIGAME
	ccp fname, // filename of destination
	bool set_time, // true: set time stamps
	// [[minimize]]
...
)
{
	...
	if (!tiny)
		fprintf (F.f, text_minigame_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);
	...
	for (par = minigame_table_list; par->name1; par++)
	{
		...
		{
			fputs (brief_count < 2 ? section_sep : "\r\n", F.f);
			fprintf (F.f, text_minigame_table_cr, par->name2 ? par->name2 : par->name1, offset,
				REVISION_NUM);
			...
			for (slot = 0; slot < 10; slot++)
			{
				...
				if (!minimize || differ)
				{
					...
					fprintf (F.f, "  # A%02u %s\r\n", arena_info[slot].def_slot,
						arena_info[slot].name_en);
...
const KeywordTab_t arena_attrib_tab[] = { { 0, "0", "A12", 12 }, { 1, "1", "A11", 11 },
	{ 2, "2", "A14", 14 }, { 3, "3", "A13", 13 }, { 4, "4", "A15", 15 }, { 5, "5", "A24", 24 },
	{ 6, "6", "A25", 25 }, { 7, "7", "A21", 21 }, { 8, "8", "A22", 22 }, { 9, "9", "A23", 23 },
	{ -1, "-1", "OFF", 0 }, { 0, 0, 0, 0 } };
...
int ScanOptKmgCopy (ccp arg)
{
	...
	if (!key)
	{
		PrintKeywordError (arena_attrib_tab, arg, abbrev, 0, "keyword for option --kmg-arena");
		return 1;
	}
	opt_kmg_arena = key->id;
	return 0;
