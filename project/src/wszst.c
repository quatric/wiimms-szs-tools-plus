...
static enumError create_rarc_dir ( ccp source, ccp dest )
{
    ...
    u8 *data = 0;
    uint size = 0;
    if (!err) err = CreateRARC(&data,&size,list.entry,list.used);
    if (!err && !testmode)
    {
	File_t F;
	err = CreateFileOpt(&F,true,dest,false,source);
...
static enumError extract_one_file ( ccp arg, ccp basedir, uint depth )
{
	...
	if ( n >= 4 )
	{
	    ...
	    if ( IsArchiveFF(ff) || IsBRSUB(ff)
		|| ff == FF_YAZ0 || ff == FF_YAZ1
		|| !memcmp(head,"Yaz0",4) || !memcmp(head,"Yaz1",4)
		|| !memcmp(head,"SARC",4) || !memcmp(head,"NARC",4)
		|| !memcmp(head,"CRAN",4) || !memcmp(head,"CTPK",4)
		|| is_ext(arg,".szs") || is_ext(arg,".arc") || is_ext(arg,".carc")
		|| is_ext(arg,".sarc") || is_ext(arg,".narc") || is_ext(arg,".ctpk")
