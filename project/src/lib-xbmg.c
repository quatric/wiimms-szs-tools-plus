...
void SetupBMG (ccp opt_filter_bmg)
{
	...
	ASSERT (MID_LE_END <= MAX_FILTER_MSG);
	ASSERT (MID_LE_CUP_END <= MAX_FILTER_MSG);
	ASSERT (MID_LE_TRACK_END <= MAX_FILTER_MSG);
	ASSERT (MID_LE_CUP_REF_END <= MAX_FILTER_MSG);
	...
	if (compatible < COMPAT_2_40
		|| compatible == COMPAT_CURRENT) // [[obsolete]] COMPAT_CURRENT 2023-11
	{
...
static int read_bmg (struct szs_iterator_t *it, // iterator struct with all infos
...
)
{
	DASSERT (bmg);
	DASSERT (fname);
	PRINT ("LoadXBMG(%p,init=%d,,aa=%d,ign=%d) fname=%s\n", bmg, initialize, allow_archive,
		ignore_no_file, fname);
...
)
{
	...
	return SaveRawBMG (bmg, fname, GetFileModeByOpt (testmode, fname, bmg->fname), set_time);
}
...
)
{
	...
	return SaveTextBMG (bmg, fname, GetFileModeByOpt (testmode, fname, bmg->fname), set_time,
		long_count > 0, brief_count > 0 ? brief_count : !print_header);
}
...
int ScanOptPatchMessage (ccp arg)
{
	...
	if (cmd->opt && !eq)
	{
		ERROR0 (ERR_SYNTAX, "Option --patch-bmg: Missing '=%s' for keyword '%s': %s\n",
			cmd->opt == 1 ? "filename" : "parameter", cmd->name1, arg);
		return 1;
	}
	else if (!cmd->opt && eq)
	{
		ERROR0 (ERR_SYNTAX, "Option --patch-bmg: Parameter not allowed for '%s': %s\n", cmd->name1,
			arg);
		return 1;
	}
	...
	if (sep && *sep)
	{
		...
		if (stat < 0 || *end)
			ERROR0 (ERR_WARNING, "Option --patch-bmg: Invalid message condition: %s %c %s\n",
				cmd->name1, cond.not? '!' : '?', sep);
		else
			cond_valid = stat > 0;
		PRINT ("STAT=%d, NOT=%d, MID=%x,%x,%x => valid=%d\n", stat, cond.not, cond.mid[0],
			cond.mid[1], cond.mid[2], cond_valid);
	}
	...
	for (i = 0; i < n_patch_bmg; i++)
	{
		...
		if (pi->data)
		{
			...
			PRINT ("COND FOUND: mid=%x,%x,%x, not=%d => found=%d => %s\n", cond->mid[0],
				cond->mid[1], cond->mid[2], cond->not, found, found ^ cond->not? "EXEC" : "SKIP");

			if (!(found ^ cond->not))
				continue;
		}

		enumError err = PatchBMG (dest, patch_bmg + i, pi->num & BMG_PM__MASK_CMD, pi->key, false);
		if (err > ERR_WARNING)
			return err;
		if (err == ERR_DIFFER)
			stat = ERR_DIFFER;
...
static void print_only_in (uint index, // index of relevant bmg (1|2)
...
)
{
	...
	const int msg_len
		= snprintfS (msg, sizeof (msg), "* Only in source #%u:%6x %s", index, bi->mid, attrib);
	...
	{
		PrintString16BMG (
			text, sizeof (text), bi->text, bi->len, BMG_UTF8_MAX_DIFF, 0, opt_bmg_colors);
		printf ("%s %.*s\n", msg, width, text);
	}
};
...
static void print_differ (const bmg_item_t *bi1, // first bmg item
...
)
{
	...
	const int msg_len = !strcmp (attrib1, attrib2)
		? snprintf (msg, sizeof (msg), "* %-18s%6x %s", message, bi1->mid, attrib1)
		: snprintf (msg, sizeof (msg), "* %-18s%6x %s %s", message, bi1->mid, attrib1, attrib2);
	...
	if (text_differ)
	{
		...
		PrintString16BMG (text, sizeof (text), bi1->text + diff_idx, bi1->len - diff_idx,
			BMG_UTF8_MAX_DIFF, 0, opt_bmg_colors);
		printf ("    1: %.*s\n", width, text);
		PrintString16BMG (text, sizeof (text), bi2->text + diff_idx, bi2->len - diff_idx,
			BMG_UTF8_MAX_DIFF, 0, opt_bmg_colors);
		printf ("    2: %.*s\n", width, text);
	}
	else
	{
		...
		{
			PrintString16BMG (
				text, sizeof (text), bi1->text, bi1->len, BMG_UTF8_MAX, 0, opt_bmg_colors);
			printf ("%s %.*s\n", msg, width, text);
		}
	}
}
...
)
{
	...
	PRINT ("PTR: %p..%p/%u, %p..%p/%u\n", p1, e1, bmg1->item_used, p2, e2, bmg2->item_used);
	...
	while (p1 && p1 < e1 && p2 && p2 < e2)
	{
		...
		const bool attrib_differ = p1->attrib_used != p2->attrib_used
			|| memcmp (p1->attrib, p2->attrib, p1->attrib_used);
		if (p1->len != p2->len || memcmp (p1->text, p2->text, p1->len * sizeof (*p1->text)))
		{
			if (quiet)
				return ERR_DIFFER;
			differ++;
...
static void ScanOptMsgJob (int mode, u8 value)
{
	...
	switch (mode)
	{
		...
		case MD_PARAM:
			filter_message[MID_PARAM_IDENTIFY] = value;
			memset (filter_message + MID_PARAM_BEG, value, MID_PARAM_END - MID_PARAM_BEG);
			break;

		case MD_CUPS:
			PRINT ("SET CUPS = %d\n", value);
			memset (filter_message + MID_RCUP_BEG, value, MID_RCUP_END - MID_RCUP_BEG);
			memset (filter_message + MID_BCUP_BEG, value, MID_BCUP_END - MID_BCUP_BEG);
		...
		case MD_CTCODE:
			PRINT ("SET CT-CODE = %d\n", value);
			memset (filter_message + MID_CT_BEG, value, MID_CT_END - MID_CT_BEG);
			break;

		case MD_CTCUPS:
			PRINT ("SET CT-CUPS = %d\n", value);
			memset (filter_message + MID_CT_CUP_BEG, value, MID_CT_CUP_END - MID_CT_CUP_BEG);
			break;

		case MD_CTTRACKS:
			PRINT ("SET CT-TRACKS = %d\n", value);
			memset (filter_message + MID_CT_TRACK_BEG, value, MID_CT_TRACK_END - MID_CT_TRACK_BEG);
			break;

		case MD_CTARENAS:
			PRINT ("SET CT-ARENAS = %d\n", value);
			memset (filter_message + MID_CT_ARENA_BEG, value, MID_CT_ARENA_END - MID_CT_ARENA_BEG);
			break;
		...
		case MD_LECODE:
			PRINT ("SET LE-CODE = %d\n", value);
			memset (filter_message + MID_LE_BEG, value, MID_LE_END - MID_LE_BEG);
			break;

		case MD_LECUPS:
			PRINT ("SET LE-CUPS = %d\n", value);
			memset (filter_message + MID_LE_CUP_BEG, value, MID_LE_CUP_END - MID_LE_CUP_BEG);
			break;

		case MD_LETRACKS:
			PRINT ("SET LE-TRACKS = %d\n", value);
			memset (filter_message + MID_LE_TRACK_BEG, value, MID_LE_TRACK_END - MID_LE_TRACK_BEG);
			break;

		case MD_LEARENAS:
			PRINT ("SET LE-ARENAS = %d\n", value);
			memset (filter_message + MID_LE_ARENA_BEG, value, MID_LE_ARENA_END - MID_LE_ARENA_BEG);
			break;
...
int ScanOptFilterBMG (ccp arg)
{
	...
	static const KeywordTab_t opt_msg[] = { { MD_NONE, "NONE", 0, 0 },
		{ MD_ID, "IDENTIFICATION", "ID", 0 }, { MD_PARAM, "PARAM", 0, 0 },
		{ MD_GENERIC, "GENERIC", 0, 0 }, { MD_ALL, "ALL", 0, 0 }, { MD_CUPS, "CUPS", 0, 0 },
		{ MD_TRACKS, "TRACKS", 0, 0 }, { MD_ARENAS, "ARENAS", 0, 0 }, { MD_CHAT, "CHAT", 0, 0 },
		{ MD_CTCODE, "CTCODE", "CT", 0 }, { MD_CTCUPS, "CTCUPS", 0, 0 },
		{ MD_CTTRACKS, "CTTRACKS", 0, 0 }, { MD_CTARENAS, "CTARENAS", 0, 0 },
		{ MD_CTREFS, "CTREFS", 0, 0 }, { MD_LECODE, "LECODE", "LE", 0 },
		{ MD_LECUPS, "LECUPS", 0, 0 }, { MD_LETRACKS, "LETRACKS", 0, 0 },
		{ MD_LEARENAS, "LEARENAS", 0, 0 }, { MD_LEREFS, "LEREFS", 0, 0 },
		{ MD_XCODE, "XCODE", "X", 0 }, { MD_XCUPS, "XCUPS", 0, 0 }, { MD_XTRACKS, "XTRACKS", 0, 0 },
		{ MD_XARENAS, "XARENAS", 0, 0 }, { MD_XREFS, "XREFS", 0, 0 },
		{ MD_ALLCODE, "ALLCODE", 0, 0 }, { MD_ALLCUPS, "ALLCUPS", 0, 0 },
		{ MD_ALLTRACKS, "ALLTRACKS", 0, 0 }, { MD_ALLARENAS, "ALLARENAS", 0, 0 },
		{ MD_ALLREFS, "ALLREFS", 0, 0 }, { 0, 0, 0, 0 } };
...
int ScanOptBmgEndian (ccp arg)
{
	...
	if (!cmd)
	{
		ERROR0 (ERR_SYNTAX, "Option --bmg-endian: Invalid mode: '%s'\n", arg);
		return 1;
	}
...
int ScanOptBmgEncoding (ccp arg)
{
	...
	if (!cmd)
	{
		ERROR0 (ERR_SYNTAX, "Option --bmg-encoding: Invalid mode: '%s'\n", arg);
		return 1;
	}

	if (cmd->opt)
		ERROR0 (ERR_WARNING, "Option --bmg-encoding: Mode »%s« not supported yet!\n",
...
int ScanOptBmgInfSize (ccp arg, bool is_wbmgt)
{
	...
	if (end == arg || *end)
	{
		ERROR0 (ERR_SYNTAX, "Option --%sinf-size: Not a number: %s\n", is_wbmgt ? "" : "bmg-", arg);
		return 1;
	}

	if (num < 4 || num > BMG_INF_LIMIT)
	{
		ERROR0 (ERR_SEMANTIC, "Option --%sinf-size: Number must be in range 4..%u: %s\n",
...
static int scan_opt_attrib (bool *flag, u8 *attrib, ccp opt, ccp arg)
{
	...
	*flag = false;
	...
	if (!end || *end)
	{
		ERROR0 (ERR_SYNTAX, "Option --%s: Invalid attribute vector: %s\n", opt, arg);
		return 1;
	}
