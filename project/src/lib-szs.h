...
{
	return lta && lta->head_size >= offsetof (lta_header_t, ext_size) + sizeof (lta->ext_size)
		&& lta->ext_size;
}
...
static inline void LinkCacheSZS (szs_file_t *szs, // valid szs
	ccp src_fname // source filename
)
{
	DASSERT (szs);
...
typedef int (*szs_iterator_func) (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
);
...
typedef struct szs_iterator_t
{
	...
	struct szs_iterator_t *parent_it; // NULL or parent iterator
...
static inline szs_subfile_t *AppendSubfileSZS (szs_file_t *szs, // valid data structure
	szs_iterator_t *it, // not NULL: copy source info
	ccp path // not NULL: copy to member 'path'
)
{
...
typedef int (*brsub_cut_func_t) (brsub_cut_t *bcut, // pointer to data structure
