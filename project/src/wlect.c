...
#define TITLE                                                                                      \
	WLECT_SHORT ": " WLECT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
static enumError cmd_test ()
{
	...
	for (mi = 0x75; mi <= 0xc9; mi += 2)
	{
		...
		if (ti > 0)
		{
			...
			printf ("0x%02x = 0x%02x  %2u %2u  %s\n", mi, info->music_id, info->def_index,
				info->def_slot, info->sound_n_fname);
		}
	}
	return ERR_OK;
...
static enumError LoadLECODE (raw_data_t *raw, le_analyze_t *ana, ccp fname)
{
	...
	if (raw->fform != FF_LE_BIN)
	{
		err = ERROR0 (ERR_WRONG_FILE_TYPE, "Not a LECODE binary file: %s:%s\n",
			GetNameFF (raw->fform, 0), raw->fname);
		goto abort;
	}
...
static int BinDiffLECODE (lecode_bin_diff_t *bd, const le_analyze_t *a1, const le_analyze_t *a2)
{
	DASSERT (bd);
	DASSERT (a1);
	DASSERT (a2);
	...
	bd->stat_body = bodysize1 < 0 || bodysize2 < 0 || bodysize1 != bodysize2
		|| memcmp (body1, body2, bodysize1);

	bd->stat_timestamp = h1->v3.version != h2->v3.version ? true
		: h1->v3.version >= 5 ? a1->head->v5.creation_time != a2->head->v5.creation_time
							  : strcmp (a1->head->v4.timestamp, a2->head->v4.timestamp) != 0;

	bd->stat_param = memcmp (&a1->lpar, &a2->lpar, sizeof (a1->lpar));

	bd->stat_data = a1->n_cup_track != a2->n_cup_track || a1->n_cup_arena != a2->n_cup_arena
		|| a1->n_slot != a2->n_slot || a1->flags_bits != a2->flags_bits;
	if (!bd->stat_data)
	{
		bd->stat_data
			= memcmp (a1->cup_track, a2->cup_track, a1->n_cup_track * sizeof (*a1->cup_track))
			|| memcmp (a1->cup_arena, a2->cup_arena, a1->n_cup_arena * sizeof (*a1->cup_arena))
			|| memcmp (a1->property, a2->property, a1->n_slot * sizeof (*a1->property))
			|| memcmp (a1->music, a2->music, a1->n_slot * sizeof (*a1->music))
			|| memcmp (a1->flags, a2->flags, a1->n_slot * sizeof (*a1->flags));
	}

	//-- code is separated into 2 parts

	PRINT0 ("CODE1: %zx..%zx + %zx..%x\n", body1 - a1->data, a1->beg_of_data - a1->data,
		a1->end_of_data - a1->data, a1->data_size);
	PRINT0 ("CODE2: %zx..%zx + %zx..%x\n", body2 - a2->data, a2->beg_of_data - a2->data,
		a2->end_of_data - a2->data, a2->data_size);

	bd->stat_code = body1 - a1->data != body2 - a2->data
	...
	int ec = 0;
	if (bd->diff_size && bd->stat_size)
	{
		ec++;
		bd->stat_size = 2;
	}
	if (bd->diff_header && bd->stat_header)
	{
		ec++;
		bd->stat_header = 2;
	}
	if (bd->diff_timestamp && bd->stat_timestamp)
	{
		ec++;
		bd->stat_timestamp = 2;
	}
	if (bd->diff_body && bd->stat_body)
	{
		ec++;
		bd->stat_body = 2;
	}
	if (bd->diff_param && bd->stat_param)
	{
		ec++;
		bd->stat_param = 2;
	}
	if (bd->diff_data && bd->stat_data)
	{
		ec++;
		bd->stat_data = 2;
	}
	if (bd->diff_code && bd->stat_code)
	{
		ec++;
		bd->stat_code = 2;
	}
...
static enumError cmd_bin_diff ()
{
	if (n_param != 3)
		return ERROR0 (ERR_SYNTAX,
			"Exact 3 parameters expected for BIN-DIFF:"
	...
	{
		if (bd.stat_size > 1)
			fputs ("> File size differ\n", stdout);
		if (bd.stat_header > 1)
			fputs ("> File header differ\n", stdout);
		if (bd.stat_timestamp > 1)
			fputs ("> Timestamp differ\n", stdout);
		if (bd.stat_body > 1)
			fputs ("> File body differ\n", stdout);
		if (bd.stat_param > 1)
			fputs ("> Parameters differ\n", stdout);
		if (bd.stat_data > 1)
			fputs ("> Cup and track data differ\n", stdout);
		if (bd.stat_code > 1)
			fputs ("> Code differ\n", stdout);
...
static enumError cmd_patch ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (verbose >= 0 || testmode)
		{
			...
			if (dest_is_source)
				fprintf (stdlog, "%s%sPATCH %s:%s\n", verbose > 0 ? "\n" : "",
					testmode ? "WOULD " : "", ff_name, dest);
			else
				fprintf (stdlog, "%s%sPATCH %s:%s -> %s:%s\n", verbose > 0 ? "\n" : "",
					testmode ? "WOULD " : "", ff_name, raw.fname, ff_name, dest);
			fflush (stdlog);
		}
		...
		err = PatchLECODE (&ana);
		if (!err)
			err = SaveFILE (dest, 0, dest_is_source || opt_overwrite, raw.data, raw.data_size, 0);
		ImportAnaLDUMP (&ana);
		ResetLEAnalyze (&ana);
		CloseLDUMP ();
	}
...
static void help_create (enumError exit_code)
{
	...
	static const char text[]
		= "\n"
		  ...
		  "\n"
		  "\tCreate a LEX file:\t|{name|LEX}, {name|LEX+}, {name|SET1}, {name|CANNONS}, "
		  "{name|HIPT}, {name|TEST}.\n"
	...
	PrintColoredLines (stdout, colout, 0, good_width - 1, 0, 0, text, ProgInfo.progname,
		ProgInfo.progname, ProgInfo.progname);
	ClosePager ();
	ExitFixed (exit_code);
}
...
static enumError cmd_create ()
{
	...
	static const KeywordTab_t tab[] = { { C_HELP, "HELP", "H", 0 },

		{ C_LEX, "LEX", 0, 0 }, { C_LEX, "LEX+", 0, 1 }, { C_LEX, "DEVELOP", 0, 2 },
		{ C_FEATURES, "FEATURES", 0, 0 }, { C_DEV1, "DEV1", 0, 0 }, { C_SET1, "SET1", 0, 0 },
		{ C_CANNON, "CANNONS", 0, 0 }, { C_COUNTDOWN, "CTDN", "COUNTDOWN", 0 },
		{ C_HIDE_PT, "HIPT", 0, 0 }, { C_RND_ITPH, "RITP ", 0, 0 }, { C_TEST, "TEST", 0, 0 },

		{ C_LPAR, "LPAR", 0, 0 }, { C_LEDEF, "LE-DEF", "LEDEF", 0 }, { C_PREFIX, "PREFIX", 0, 0 },
		...
		case C_LEINFO:
		{
			...
			if (!err)
			{
				fprintf (F.f,
					...
					"  %s\n"
					"\n",
					GetLecodeInfoLEREG (LEREG_PAL), GetLecodeInfoLEREG (LEREG_USA),
					GetLecodeInfoLEREG (LEREG_JAP), GetLecodeInfoLEREG (LEREG_KOR));
			}
			CloseFile (&F, 0);
		}
	}
...
static void help_distrib (enumError exit_code)
{
	...
	static const char text[]
		= "\n"
		  ...
		  " Then the remaining arguments of the command line are evaluated step by step from left "
		  "to right."
		  ...
		  "\n"
		  "  |All arguments are executed in the order in which they were entered without any "
		  "logging."
		  ...
		  "\n"
		  "\t{name|*.*}:\t|"
		  "If a file is a valid track file,"
		  " then it is added to one of two different internal track lists"
		  " (one for racing tracks and one for battle areans)."
		  " If the argument is quoted then wlect will resolve the wildcards by itself."
		  "\n"
		  ...
		  " Therefor count track and arena types and find duplicate names, tracks, families and "
		  "clans."
		  "\r\r"
		  ...
		  "\t{par|classes}:\t|List tracks by track class, except classes {name|SELECT} and "
		  "{name|NINTENDO}.\n"
		  ...
		  "\t{par|1wiimm}:\t|Use Wiimms avatar for the first cup, but only if original icons are "
		  "not used.\n"
		  ...
		  " Underline characters are printed like spaces, but they are ignored by option "
		  "{par|space}.\n"
		  ...
		  "  {heading|Syntax of PARAMETER:} |{syntax|STORAGE ',' SEP REGEXP SEP REPLACEMENT SEP "
		  "OPTIONS}\n"
		  "\n\t\t|"
		  ...
		  "|[14,24]\n"
		  "\t{par|remove}:\t|Remove all used track source files at the very end."
		  " »{par|rm}« is s short cut.\n"
		  "\t{par|ext}:\t|Append a track file extension list to each LTA.\n"
		  "\t{par|redir}:\t|Create redirection file {file|lta.szs} type {name|USE-LTA}.\n"
		  ...
		  "\t{par|wbz}:\t|Force WBZ (BZ) compression for each track file.\n"
		  "\t{par|wlz}:\t|Force WLZ (LZ) compression for each track file.\n"
		  "\n|[4,12,14,22]"
		  "\t\t|Controlled by the global processing options {opt|+PR0} and"
		  " {opt|+PR1} to {opt|+PR60}, progress messages can be output."
		  " These settings can be overwritten using the local options"
		  " {par|pr0}, {par|pr1}, {par|pr2}, {par|pr5},"
		  " {par|pr10}, {par|pr20}, {par|pr30} or {par|pr60},"
		  " where the number indicates the number of seconds between 2 messages.\n"
		  "\n"
		  "\t\t|{syntax|DIRECTORY} specifies the destination directory."
		  " Only files with valid member »{name|PATH}« are added to the archives."
		  " Track files with unknown paths are searched in directories"
		  ...
		  "\n"
		  "\t|The following options determine whether and at what intervals progress messages"
		  " are output. These progress messages are only displayed when importing track files"
		  " with placeholders and when creating LTA files.\n"
		  "\n"
		  "\t{name|PR0}:\t|Disable progress messages.\n"
		  "\t{name|PR1}:\t|Output a progress message approximately every second.\n"
		  "\t{name|PR2}:\t|Output a progress message approximately every 2 seconds.\n"
		  "\t{name|PR5}:\t|Output a progress message approximately every 5 seconds.\n"
		  "\t{name|PR10}:\t|Output a progress message approximately every 10 seconds.\n"
		  "\t{name|PR20}:\t|Output a progress message approximately every 20 seconds.\n"
		  "\t{name|PR30}:\t|Output a progress message approximately every 30 seconds.\n"
		  "\t{name|PR60}:\t|Output a progress message approximately every 60 seconds.\n"
		  "\n"
		  ...
		  "\n"
		  "\t{name|IN-LECODE}:\t|"
		  "When reading a LE-CODE binary file (e.g. »{file|lecode-PAL.bin}«),"
		  ...
		  "\n"
		  "\t{name|NO-SLOT}:\t|"
		  "Suppress »{name|SLOT <index>}« lines when creating a LE-CODE defintion file "
		  "({name|LE-DEF}).\n"
		  "\t{name|/NO-SLOT}:\t|"
		  ...
		  "\t{name|/CT-SLOTS}:\t|"
		  "Switch option {name|CT-SLOTS} off.\n"
		  "\n"
		  "\t{name|BRIEF}:\t|"
	...
	PrintColoredLines (stdout, colout, 0, good_width - 1, 0, 0, text, ProgInfo.progname,
		...
		//	,wildcards
		,
		GetLecodeBuildLEREG (LEREG_PAL), GetLecodeInfoLEREG (LEREG_PAL),
		GetLecodeInfoLEREG (LEREG_USA), GetLecodeInfoLEREG (LEREG_JAP),
...
static enumError cmd_distrib_instruction (le_distrib_t *ld, ccp mode, char *arg)
{
	...
	static const KeywordTab_t tab[] = { { C_NAMES, "NAMES", 0, 0 }, { C_NAMES, "XNAMES", 0, 1 },
		{ C_INFO, "INFO", 0, 0 }, { C_INFO, "XINFO", 0, 1 }, { C_RATING, "RATING", 0, 0 },
		{ C_RATING, "XRATING", 0, 1 }, { C_LEINFO, "LE-INFO", "LEINFO", 0 },

		{ C_SHA1, "SHA1", 0, 0 }, { C_SHA1, "XSHA1", 0, 1 }, { C_DISTRIB, "DISTRIBUTION", 0, 0 },
		{ C_DISTRIB, "XDISTRIBUTION", 0, 1 },

		{ C_CTDEF, "CT-DEF", "CTDEF", 0 }, { C_LEDEF, "LE-DEF", "LEDEF", 0 },
		{ C_LEDEF, "DEFINITION", "DEF", 0 }, { C_LEREF, "LE-REF", "LEREF", 0 },
		{ C_LEREF, "REFERENCE", "REF", 0 }, { C_STRINGS, "STRINGS", "STR", 0 },
		{ C_DUMP, "DUMP", 0, 0 },

#if LE_DIS_PATCH_ENABLED
		{ C_DEST, "DEST", 0, 0 }, { C_PATCH, "PATCH", 0, O_PARAM_OPT2 | O_FILE_LIST },
#endif

		{ C_LECODE, "PAL", 0, LEREG_PAL }, { C_LECODE, "USA", 0, LEREG_USA },
		{ C_LECODE, "JAP", 0, LEREG_JAP }, { C_LECODE, "KOR", 0, LEREG_KOR },
		{ C_LECODE4, "LECODE", 0, 0 },

		{ C_LPAR, "LPAR", 0, 0 },

		{ C_BMG, "BMG", "BMGBIN", 0 }, { C_BMG, "BMG-TXT", "BMGTXT", 1 },

		{ C_CUPICON, "CUP-ICONS", "CUPICONS", O_PARAM_OPT2 },
		{ C_CUPICON, "CUP-INFO", "CUPINFO", O_PARAM_OPT2 | 1 },

		{ C_SEPARATOR, "SEPARATOR", "SEP", 0 }, { C_COPY, "COPY", 0, O_PARAM_D | O_PARAM_SS },
		{ C_SPLIT, "SPLIT", 0, O_PARAM_D | O_PARAM_S | O_PARAM_A },
		{ C_SUBST, "SUBST", 0, O_PARAM_D | O_PARAM_A },

		{ C_TRACKS, "TRACKS", 0, 0 }, { C_LTA, "LTA", 0, O_PARAM_OPT2 },
		...
		{ C_PREFIX, "PREFIX", 0, 0 }, { C_RESERVE, "RESERVE", 0, 0 }, { C_DEBUG, "DEBUG", 0, 0 },
	...
	static const KeywordTab_t tracks_tab[] = { { 0, "NO-LOG", "NOLOG", 0 }, { 0, "LOG", 0, O_LOG },
		{ 0, "TEST", 0, O_LOG | O_TEST }, { 0, 0, 0, 0 } };
	...
	if (opt & (O_PARAM_D | O_PARAM_S))
	{
		if (opt & O_PARAM_D)
		{
			...
			{
			err_param12:;
				return ERROR0 (ERR_SEMANTIC, "%s: Invalid string name: %s\n", cmd->name1, arg);
			}
			arg = end;
			if (verbose >= 2)
				fprintf (stdlog, "   > %s%s %s\n", cmd->name1, opt & O_PARAM_S ? " DEST:   " : ":",
			...
			while (n_par_src < max)
			{
				*src = ld->spar;
				...
				if (err)
					goto err_param12;
				if (verbose >= 2)
					fprintf (stdlog, "   > %s SRC[%02u]: %s\n", cmd->name1, n_par_src + 1,
						GetOptionsLSP (src, LEO__SRC));
				n_par_src++;
				src++;
				arg = end;
				if (*arg != '+' && *arg != ',')
	...
	{
		case C_LECODE4:
			err = CreateLecode4LD (arg, ld);
			break;
		case C_SEPARATOR:
			err = SeparatorLD (ld, arg);
			break;
		case C_COPY:
			err = CopyLD (ld, &par_dest, par_src, n_par_src);
			break;
		case C_SPLIT:
			err = SplitLD (ld, &par_dest, par_src, arg);
			break;
		case C_SUBST:
			err = SubstLD (ld, &par_dest, arg);
			break;
		case C_LTA:
			err = CreateTrackArchivesLD (ld, arg, mem_opt);
			break;
		case C_LTA_RM:
			err = CreateTrackArchivesLD (ld, arg, MemByString ("rm"));
			break;

#if LE_DIS_SORTTRACKS_ENABLED
		case C_SORT_TRACKS:
			err = SortTracksLD (ld, &par_opt, mem_opt);
			break;
#endif

#if LE_DIS_SORTCUPS_ENABLED
		case C_SORT_CUPS:
			err = SortCupsLD (ld, &par_opt, mem_opt);
			break;
#endif
	...
	if (err == ~0 && opt & O_FILE_LIST)
	{
		...
		{
			case C_PATCH:
				err = PatchLD (ld, mem_opt, &filelist);
				break;
			default:
				err = ERR_ERROR;
		}
	...
	if (err == ~0)
	{
		...
		{
			ld_out_param_t lop
				= { .fname = arg, .f = F.f, .colset = GetFileColorSet (F.f), .ld = ld };
			PRINT0 ("%sCOLOR-SET: %d,%d,%d  istty=%d,%d%s\n", lop.colset->status,
				lop.colset->col_mode, lop.colset->colorize, lop.colset->n_colors,
				isatty (fileno (F.f)), isatty (fileno (stdout)), lop.colset->reset);

			const bool variant = cmd->opt & 1; // use xnames || force text mode
			switch (cmd->id)
			{
				case C_NAMES:
					err = CreateNamesLD (F.f, ld, variant);
					break;
				case C_INFO:
					err = CreateInfoLD (F.f, ld, variant, false);
					break;
				case C_RATING:
					err = CreateInfoLD (F.f, ld, variant, true);
					break;
				case C_LEINFO:
					err = CreateLeInfoLD (F.f, ld, false);
					break;
				case C_SHA1:
					err = CreateSha1LD (F.f, ld, variant, true);
					break;
				case C_DISTRIB:
					err = CreateDistribLD (F.f, ld, variant);
					break;

				case C_CTDEF:
					err = CreateCtDefLD (F.f, ld);
					break;
				case C_LEDEF:
					err = CreateLeDefLD (F.f, ld);
					break;
				case C_LEREF:
					err = CreateRefLD (F.f, ld, true);
					break;
				case C_STRINGS:
					err = CreateStringsLD (F.f, ld);
					break;
				case C_DUMP:
					err = CreateDumpLD (F.f, ld);
					break;

				case C_LECODE:
					err = CreateLecodeLD (F.f, ld, cmd->opt);
					break;
				case C_LPAR:
					err = CreateLparLD (F.f, ld, true);
					break;
				case C_BMG:
					err = CreateBmgLD (F.f, ld, variant || F.is_stdio);
					break;
				case C_CUPICON:
					err = CreateCupIconsLD (&lop, mem_opt, variant || F.is_stdio);
					break;
				case C_PREFIX:
					err = SavePrefixTable (F.f, 0);
					break;
				case C_DEBUG:
					err = CreateDebugLD (F.f, ld);
					break;
				case C_REPORT:
					err = CreateReportLD (&lop, mem_opt);
					break;
			}
		}
		CloseFile (&F, 0);
...
static enumError cmd_convert (int cmd_id, ccp cmd_name, ccp def_path)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!testmode)
		{
			err = dest_ff == FF_LEX ? SaveRawLEX (&lex, dest, opt_preserve)
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_LPAR:
				...
				break;
			case GO_ALIAS:
				err += ScanOptAlias (optarg);
				break;
			case GO_ENGINE:
				err += ScanOptEngine (optarg);
				break;
			case GO_200CC:
				err += ScanOpt200cc (optarg);
				break;
			case GO_PERFMON:
				err += ScanOptPerfMon (optarg);
				break;
			case GO_CUSTOM_TT:
				err += ScanOptCustomTT (optarg);
				break;
			case GO_XPFLAGS:
				err += ScanOptXPFlags (optarg);
				break;
			case GO_SPEEDOMETER:
				err += ScanOptSpeedometer (optarg);
				break;
			case GO_DEBUG:
				err += ScanOptDebug (optarg);
				break;

			case GO_TRACK_DIR:
				opt_track_dest = optarg;
				break;
			case GO_COPY_TRACKS:
				err += ScanOptTrackSource (optarg, -1, TFMD_COPY);
				break;
			case GO_MOVE_TRACKS:
				err += ScanOptTrackSource (optarg, -1, TFMD_MOVE);
				break;
			case GO_MOVE1_TRACKS:
				err += ScanOptTrackSource (optarg, -1, TFMD_MOVE1);
				break;
			case GO_LINK_TRACKS:
				err += ScanOptTrackSource (optarg, -1, TFMD_LINK);
				break;
			case GO_SZS_MODE:
				err += ScanOptSzsMode (optarg);
				break;
			case GO_CACHE:
				opt_cache = optarg;
				opt_remove_dest = true;
...
static enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_SYMBOLS:
			err = DumpSymbols (SetupVarsLECODE ());
			break;
		case CMD_FUNCTIONS:
			SetupVarsLECODE ();
			...
			break;
		case CMD_CALCULATE:
			err = ParserCalc (SetupVarsLECODE ());
			break;
		case CMD_FLOAT:
			err = cmd_float ();
			break;
