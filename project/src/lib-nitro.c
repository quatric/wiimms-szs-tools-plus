...
static inline u16 nrd16 ( const u8 *p ) { return (u16)p[0] | (u16)p[1]<<8; }
...
)
{
	...
	if ( off >= ncgr->tiles_size ) return -1;
...
)
{
    ...
    if (!objs) return ERR_CANT_CREATE;
	...
	n_live++;
	if ( objs[i].x < minx ) minx = objs[i].x;
	if ( objs[i].y < miny ) miny = objs[i].y;
	if ( (int)(objs[i].x+objs[i].w) > maxx ) maxx = objs[i].x+objs[i].w;
	if ( (int)(objs[i].y+objs[i].h) > maxy ) maxy = objs[i].y+objs[i].h;
    }
    if (!n_live)
    {
	FREE(objs);
    ...
    if (!rgba) { FREE(objs); return ERR_CANT_CREATE; }
