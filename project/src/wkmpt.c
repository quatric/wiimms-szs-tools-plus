...
#define TITLE                                                                                      \
	WKMPT_SHORT ": " WKMPT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
static enumError cmd_objects ()
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		for (id = 0; id < N_KMP_GOBJ; id++, oi++)
		{
			...
			filter[id].mode
				= strcasestr (oi->name, arg) || oi->fname && strcasestr (oi->fname, arg);
			if (!filter[id].mode)
			{
				if (!filter[id].string)
				{
...
static enumError cmd_startpos ()
{
	...
	static const KeywordTab_t keytab[] = { { M_RESET, "CLEAR", "RESET", 0 },

		{ M_TABLE, "IMAGE", "I", 0 }, { M_TABLE, "TABLE", "T", 1 },

		{ M_RELATIVE, "ABSOLUTE", "ABS", 0 }, { M_RELATIVE, "RELATIVE", "REL", 1 },

		{ M_MIRROR, "LEFT", "L", 0 }, { M_MIRROR, "RIGHT", "R", 1 },

		{ M_NARROW, "WIDE", "W", 0 }, { M_NARROW, "NARROW", "N", 1 },

		{ M_AUTO, "AUTO", "A", 0x06 }, { M_AUTO, "NO-AUTO", "NOAUTO", 0x00 },

		{ M_HEX, "DECIMAL", "DEC", 0 }, { M_HEX, "HEXADECIMAL", "HEX", 1 },

		{ M_Z, "Z0", 0, 0 }, { M_Z, "Z1", 0, 1 }, { M_Z, "Z2", 0, 2 }, { M_Z, "Z3", 0, 3 },

		{ M_NUMBER, "1", 0, 1 }, { M_NUMBER, "2", 0, 2 }, { M_NUMBER, "3", 0, 3 },
		{ M_NUMBER, "4", 0, 4 }, { M_NUMBER, "5", 0, 5 }, { M_NUMBER, "6", 0, 6 },
		{ M_NUMBER, "7", 0, 7 }, { M_NUMBER, "8", 0, 8 }, { M_NUMBER, "9", 0, 9 },
		{ M_NUMBER, "10", 0, 10 }, { M_NUMBER, "11", 0, 11 }, { M_NUMBER, "12", 0, 12 },
	...
	for (param = first_param; param; param = param->next)
	{
		...
		switch (job->id)
		{
			...
			case M_NUMBER:
			{
				...
				const double3 *tab = GetStartPosKMP (0, n, mirror, narrow, &kmp, amode);
				if (!tab)
					break;

				printf ("\n%u player%s, %s (%s%u), %s (%s%u), %s%s:\n\n", n, n == 1 ? "" : "s",
					kmp_start_pos_mirror ? "pole is right" : "pole is left",
					kmp_start_pos_auto & 2 ? "KMP=" : "", kmp_start_pos_mirror,
					kmp_start_pos_narrow ? "narrow mode" : "wide mode",
					kmp_start_pos_auto & 4 ? "KMP=" : "", kmp_start_pos_narrow,
				...
				if (table)
				{
					if (hexmode)
					{
						const int origin = kmp.dlist[KMP_KTPT].used ? POSITION_ORIGIN : 0;
					...
					else
					{
						...
						for (i = 0; i < n; i++)
							printf (
								"%4u: %11.3f %11.3f %11.3f\n", i + 1, tab[i].x, tab[i].y, tab[i].z);
					}
				}
				else
				{
					const bool have_sep = n > 2 && IsNormalD (tab[0].z) && IsNormalD (tab[1].z);
					const double sep = have_sep ? 1.5 * (tab[1].z - tab[0].z) : 1e9;

					uint i;
					for (i = 0; i < n; i++)
					{
						if (have_sep && i > 0 && IsNormalD (tab[i - 1].z)
						...
						int x = (int)tab[i].x;
						const int fw = x < -900 ? 0 : x > 900 ? 1800 / 80 : (900 + x) / 80;
						const char sign = x < 0 ? '-' : x > 0 ? '+' : ' ';
						if (hexmode)
							printf (
								"%3u:%*c%04x%*c", i + 1, fw + 3, sign, abs (x) * 16, 26 - fw, ':');
						else
							printf ("%3u:%*c%03u%*c", i + 1, fw + 3, sign, abs (x), 26 - fw, ':');
...
static enumError cmd_convert (int cmd_id, ccp cmd_name, ccp def_path)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!testmode)
		{
			err = dest_ff == FF_KMP ? SaveRawKMP (&kmp, dest, opt_preserve)
...
static enumError cmd_draw ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		char dest[PATH_MAX];
		SubstDest (dest, sizeof (dest), arg, opt_dest, def_path, GetExtFF (ff_dest, 0), false);

		if (verbose >= 0 || testmode)
		{
			fprintf (stdlog, "%s%sCREATE %s:%s -> %s:%s\n", verbose > 0 ? "\n" : "",
				testmode ? "WOULD " : "", GetNameFF (raw.fform, 0), raw.fname,
		...
		if (!testmode)
		{
			...
			if (KMP_DRAW & DMD_JGPT)
				AddArrow2KCL (&dk, KMP_JGPT, KCL_FLAG_JGPT, KCL_FLAG_JGPT_AREA, 900.0, 1200.0);
			...
			if (KMP_DRAW & DMD_KTPT)
			{
				...
				AddArrow2KCL (&dk, KMP_KTPT, KCL_FLAG_KTPT, KCL_FLAG_KTPT_AREA, len, 2000.0);
			}
						...
						if (flags & OBF_SOLID && flags & (OBF_OPT_ROUTE | OBF_ALWAYS_ROUTE))
						{
							...
							if (AnalyseItembox (&ibox, op))
							{
								if (KMP_DRAW & (DMD_ITEMBOX | DMD_POTI))
									drtm = ibox.item_player ? DRTM_ITEMBOX_PLAYER
										: ibox.item_enemy	? DRTM_ITEMBOX_ENEMY
															: DRTM_ITEMBOX;
							}
...
enumError cmd_routes ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		static struct
		{
			...
			tab[] = { { KMP_CKPH, -1, "CKPH (check point groups)" },
				...
				{ KMP_ITPT, 1, "ITPT (item route points)" }, { 0, 0 } };
...
static enumError cmd_gobj ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (print_header)
			printf ("\n%sFile %s%s:%s%s\n", col->status, col->file, GetNameFF (raw.fform, 0),
				raw.fname, col->reset);
		...
		for (i = 0, info = ag.info, gobj = ag.gobj; i < ag.n_gobj; i++, info++, gobj++)
			printf ("%4d. %4x %4x : %4x %4x : %u %c %2x %3u : %3d\n", i, gobj->obj_id, info->obj_id,
				gobj->ref_id, info->ref_id, info->type, type_char[info->type], info->status,
				info->ref_count, info->defobj);

		if (print_header)
		{
			printf ("%s%u object%s analyzed in %s: ", col->status, ag.n_gobj,
				ag.n_gobj == 1 ? "" : "s", PrintTimerUSec6 (0, 0, ag.dur_usec, 0));

			printf ("o=%u=%u+%u, en=%d,%d, do=%u/%u=%u+%u+%u\n", ag.n_object, ag.n_disabled,
				ag.n_enabled, ag.n_engine, ag.n_random, ag.n_defobj_used, ag.n_defobj,
...
static enumError cmd_gamemodes ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		for (ri = 0, res = ap.res_list; ri < ap.n_res; ri++, res++)
			if (brief_count)
				...
				printf ("%3d. %3u %-13s  %u,%u,%02u,%02d,%u,%u,%u  %s\n", ri + 1, res->version + 1,
					res->name, res->lex_test.offline_online, res->lex_test.n_offline,
					res->lex_test.n_online, res->lex_test.cond_bit, res->lex_test.game_mode,
					res->lex_test.random, res->lex_test.engine, res->status);
			else if (long_count)
		...
		if (print_header)
		{
			printf ("%s%u object%s analyzed in %s: ", col->status, ap.ag.n_gobj,
				ap.ag.n_gobj == 1 ? "" : "s", PrintTimerUSec6 (0, 0, ap.dur_usec, 0));

			if (ap.n_version == ap.n_res)
				printf ("%u different scenario%s total", ap.n_res, ap.n_res == 1 ? "" : "s");
			else
				printf ("%u scenario%s total, %u different", ap.n_res, ap.n_res == 1 ? "" : "s",
					ap.n_version);

			if (ap.n_res_std && ap.n_res_ext)
				printf (" (%u standard code, %u extended code)", ap.n_res_std, ap.n_res_ext);
...
static enumError cmd_wim0 ()
{
	...
	for (param = first_param; param; param = param->next)
	{
		...
		printf ("\n%sFile %s%s:%s%s\n", col->status, col->file, GetNameFF (raw.fform, 0), raw.fname,
			col->reset);
		...
		else
		{
			...
			printf ("%s> WIM0 section (%u bytes total, %u payload) found, ", col->caption,
				kmp.wim0_import_size, kmp.wim0_size);
			...
			if (long_count > 0)
			{
					...
					avail = GetNextSectionWim0 (&info, &kmp))
				{
					printf ("%s  index %d, offset 0x%x, %u bytes, section %s, type %s%s\n",
						col->heading, info.index, info.offset, info.size, info.name,
						Wim0FormatName[info.format], col->reset);
					if (info.size > 0)
			...
			else
			{
				...
				printf ("%s%.*s%s\n"
						...
						"%s%.*s%s\n",
					col->heading, 3 * fw, ThinLine300_3, col->reset, col->heading, col->reset,
				...
				for (avail = GetFirstSectionWim0 (&info, &kmp); avail;
					avail = GetNextSectionWim0 (&info, &kmp))
				{
					printf ("%3d %05x %5u %-7s %-6s ", info.index, info.offset, info.size,
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_SLOT:
				...
				break;
			case GO_DRAW:
				err += ScanOptDraw (optarg);
				break;
			case GO_POS_FILE:
				err += ScanOptPosFile (optarg);
				break;
			case GO_POS_MODE:
				err += ScanOptPosMode (optarg);
				break;
			case GO_PNG:
				err += ScanOptPng (optarg);
				break;
			case GO_LOAD_KCL:
...
#endif
{
	...
	enumError err = CheckEnvOptions2 ("WKMPT_OPT", CheckOptions);
	if (err)
		hint_exit (err);
