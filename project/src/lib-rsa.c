...
typedef struct der_t { const uint8_t *p, *end; } der_t;
...
static int der_read_tlv ( der_t *d, uint8_t *tag, const uint8_t **content, size_t *content_len )
{
    ...
    *tag = *d->p++;
    ...
    {
	int nbytes = b0 & 0x7F;
	if ( nbytes == 0 || nbytes > 4 || d->p+nbytes > d->end ) return 0;
	len = 0;
...
{
    uint8_t tag; const uint8_t *content; size_t len;
    ...
    while ( len > 1 && content[0] == 0 ) { content++; len--; }
    if ( out )
...
{
    size_t body_len = 0;
    const char *body = find_pem_body((const char*)pem,pem_len,
	"-----BEGIN RSA PRIVATE KEY-----","-----END RSA PRIVATE KEY-----",&body_len);
    if (!body) return 0;

    uint8_t *der = (uint8_t*)malloc(body_len);
    if (!der) return 0;
    int der_len = base64_decode(body,body_len,der);
    if ( der_len <= 0 ) { free(der); return 0; }

    der_t d = { der, der+der_len };
    uint8_t tag; const uint8_t *content; size_t len;
    if ( !der_read_tlv(&d,&tag,&content,&len) || tag != 0x30 ) { free(der); return 0; }
    der_t seq = { content, content+len };

    bignum_t version;
    if ( !der_read_integer(&seq,&version) )        { free(der); return 0; }
    if ( !der_read_integer(&seq,&key->n) )          { free(der); return 0; }
    if ( !der_read_integer(&seq,&key->e) )          { free(der); return 0; }
    if ( !der_read_integer(&seq,&key->d) )          { free(der); return 0; }
...
int RSA_LoadPublicKeyPEM ( rsa_key_t *key, const uint8_t *pem, size_t pem_len )
{
    size_t body_len = 0;
    const char *body = find_pem_body((const char*)pem,pem_len,
	"-----BEGIN RSA PUBLIC KEY-----","-----END RSA PUBLIC KEY-----",&body_len);
    if (!body) return 0;
...
void RSA_SetPublicKey ( rsa_key_t *key, const uint8_t *n, int n_len, uint32_t e )
{
    ...
    BN_Zero(&key->e); key->e.d[0] = e; key->e.n = e ? 1 : 0;
    memset(&key->d,0,sizeof(key->d));
    key->size = n_len;
}
...
static int build_pkcs1_v15_block ( int k, const uint8_t digest[20], uint8_t *em )
{
    ...
    if ( ps_len < 8 ) return 0; // key too small for SHA-1 PKCS#1 v1.5
    em[0] = 0x00;
    em[1] = 0x01;
