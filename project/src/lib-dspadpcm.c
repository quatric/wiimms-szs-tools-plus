...
{
    return sample / DSP_ADPCM_SAMPLES_PER_FRAME * 16
         + sample % DSP_ADPCM_SAMPLES_PER_FRAME + 2;
}
...
void DspAdpcmDecodeBlock ( const u8 *src, int count, s16 *samples, const s16 *coefs, int *hist1, int *hist2 )
{
    int byte = src[0];
    int index = (byte >> 4) & 7;
...
void DspAdpcmEncodeBlock ( const s16 *samples, int count, u8 *dst, const s16 *coefs, int *hist1, int *hist2 )
{
    ...
    for ( int index = 0; index < 8; index++ )
    {
        ...
        for ( int n = 0; n < count; n++ )
        {
            ...
            int res = samples[n] - pred; if ( res < 0 ) res = -res;
