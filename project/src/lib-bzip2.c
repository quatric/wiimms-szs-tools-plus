...
)
{
	...
	const u8 *d = data;
	if (!d || size < 0x20 || memcmp (d, BZ_MAGIC, 4) || memcmp (d + delta, "BZh", 3)
...
{
	return compr_level < 1 ? BZIP2_DEFAULT_COMPR : compr_level < 9 ? compr_level : 9;
}
...
)
{
	...
	return is_writing ? (4 + 8 * compr_level) * 102400 : (1 + 4 * compr_level) * 102400;
}
...
)
{
	...
	*dest_written = dest_size;
	...
	int bzerror
		= BZ2_bzBuffToBuffCompress (dest, dest_written, (char *)src, src_size, compr_level, 0, 0);

	if (bzerror != BZ_OK)
		return ERROR0 (ERR_BZIP2, "Error while compressing data.\n-> bzip2 error: %s\n",
...
)
{
	...
	enumError err
		= EncodeBZIP2buf (dest, dest_size, dest_written, src, src_size, compr_level, add_dec_size);
	if (err)
	{
		FREE (dest_alloced);
		*dest_ptr = 0;
...
)
{
	...
	*dest_written = dest_need;
	int bzerror
		= BZ2_bzBuffToBuffDecompress ((char *)dest, dest_written, (char *)src, src_size, 0, 0);

	if (bzerror != BZ_OK)
		return ERROR0 (ERR_BZIP2, "Error while decompressing data.\n-> bzip2 error: %s\n",
			GetMessageBZIP2 (bzerror, "?"));
...
)
{
	...
	enumError err
		= DecodeBZIP2buf (dest + header_size, dest_size, dest_written, src + 4, src_size - 4);
	if (err)
	{
		FREE (dest);
		*dest_ptr = 0;
			...
			while (*search)
			{
				...
				*pptr = 0;
				...
				if (src->fform != FF_UNKNOWN)
				{
					...
					if (ff != src->fform)
					{
						ERROR0 (ERR_WARNING, "Need file format %s, but file is %s -> ignore %s\n",
