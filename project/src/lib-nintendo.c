...
)
{
    ...
    *dest = 0; *dest_size = 0;
    ...
    if (!cell_w) cell_w = (width >= 16) ? 16 : width;
    if (!cell_h) cell_h = (height >= 16) ? 16 : height;
    ...
    if (!tex_data) return ERR_OUT_OF_MEMORY;
...
)
{
    ...
    *dest = 0; *dest_size = 0;
    ...
    #define CF_W16(p, v) do { if (is_wiiu) wr_be16(p, v); else wr_le16(p, v); } while(0)
    #define CF_W32(p, v) do { if (is_wiiu) wr_be32(p, v); else wr_le32(p, v); } while(0)
...
#define LZO_NEED_OUT(n) do { \
    const uint lzo_need_ = (n); \
    if ( lzo_need_ > (256u<<20) - op ) goto bad; \
    const uint lzo_want_ = op + lzo_need_; \
    if ( lzo_want_ > cap ) { \
	uint lzo_cap_ = cap; \
	while ( lzo_cap_ < lzo_want_ ) { \
	    if ( lzo_cap_ >= (256u<<20)/2 ) { lzo_cap_ = 256u<<20; break; } \
	    lzo_cap_ *= 2; \
	} \
	u8 *lzo_out_ = REALLOC(out,lzo_cap_); \
	if (!lzo_out_) goto oom; \
	out = lzo_out_; cap = lzo_cap_; \
    } \
} while(0)
#define LZO_COPY_LITERALS(n) do { \
    const uint lzo_n_ = (n); \
    if ( lzo_n_ > src_size-ip ) goto bad; \
    LZO_NEED_OUT(lzo_n_); \
    memcpy(out+op,src+ip,lzo_n_); ip += lzo_n_; op += lzo_n_; \
} while(0)
...
#define LZO_LENGTH(bits,base,value,result) do { \
    uint lzo_v_ = (value); \
    const uint lzo_mask_ = (1u<<(bits))-1; \
    if (!lzo_v_) { \
	lzo_v_ = lzo_mask_; \
	while ( ip < src_size && src[ip] == 0 ) { \
	    if ( lzo_v_ > UINT_MAX-255 ) goto bad; \
	    lzo_v_ += 255; ip++; \
	} \
	if (ip >= src_size) goto bad; \
	lzo_v_ += src[ip++]; \
    } \
    if ( lzo_v_ > UINT_MAX-(base) ) goto bad; \
    (result) = lzo_v_ + (base); \
} while(0)
	...
	else if ( token < 32 )
	{
	    ...
	    if (dist == 16384)
	    {
		if (ip != src_size) goto bad; // a segment is exactly one LZO stream
		*dest = out; *dest_size = op;
		return ERR_OK;
	    }
	}
	else if ( token < 64 )
	...
	if (!dist || dist > op) goto bad;
	LZO_NEED_OUT(len);
	for ( uint i = 0; i < len; i++ ) out[op+i] = out[op-dist+i];
	op += len;
...
enumError ScanGPAK ( gpak_t *pak, const u8 *data, uint size )
{
    ...
    for ( uint i = 0; i < n; i++ )
	if (!entries[i].data)
	    { FREE(entries); return ERR_NOTHING_TO_DO; }
...
u8 * DecompressRPAKEntry ( const u8 *data, uint size, uint *res_size )
{
	...
	else
	{
		...
		if (raw)
		{
		    if ( sn > usize-so ) { segmented = false; break; }
		...
		else
		{
		    ...
		    const enumError derr = zlib
			? InflateZlibGrow(&dec,&dec_size,seg,sn)
			: DecodeLZO1XGrow(&dec,&dec_size,seg,sn);
		    if ( derr != ERR_OK || dec_size > usize-so )
			{ FREE(dec); segmented = false; break; }
...
)
{
    ...
    #define APPEND_STR(s, out_off) do { \
...
static inline u16 sar_r16(const u8 *p, bool be) { return be ? rd_be16(p) : rd_le16(p); }
static inline u32 sar_r32(const u8 *p, bool be) { return be ? rd_be32(p) : rd_le32(p); }
static inline s32 sar_rs32(const u8 *p, bool be) { return (s32)sar_r32(p, be); }
static inline void sar_w16(u8 *p, u16 v, bool be) { if (be) wr_be16(p, v); else wr_le16(p, v); }
static inline void sar_w32(u8 *p, u32 v, bool be) { if (be) wr_be32(p, v); else wr_le32(p, v); }
...
enumError ScanSoundArchive ( sound_archive_t *sar, const u8 *data, size_t size )
{
    ...
    for (uint b = 0; b < num_blocks; b++)
    {
        ...
        if (boff + bsz <= size)
        {
            ...
            else if (type_id == 0x2001 || type_id == 0x6800 || type_id == 0x7800 || !memcmp(data + boff, "INFO", 4))
            {
                info_off = boff;
                info_size = bsz;
            }
            else if (type_id == 0x2002 || type_id == 0x6801 || type_id == 0x7801 || !memcmp(data + boff, "FILE", 4))
            {
                file_off = boff;
                file_size_block = bsz;
            }
    ...
    size_t max_file_payload = (file_size_block > 8) ? (file_size_block - 8) : (size - (file_off + 8));

    // Handle CWAR / FWAR (Wave Archive)
    if (is_cwar || is_fwar)
    {
        if (info_size < 12) return ERR_INVALID_DATA;
        u32 n_waves = sar_r32(info_body, be);
        if (n_waves > 0x10000) return ERR_INVALID_DATA;
        ...
        for (uint i = 0; i < n_waves; i++)
        {
            ...
            snprintf(sar->entries[i].ext, sizeof(sar->entries[i].ext), is_fwar ? ".bfwav" : ".bcwav");
        }
        return ERR_OK;
    }
    ...
    if (is_cgrp || is_fgrp)
    {
        ...
        if (n_files > 0x10000) return ERR_INVALID_DATA;
        ...
        for (uint i = 0; i < n_files; i++)
        {
            ...
            if (ref_off < 0 || (uint)ref_off + 16 > info_size) continue;
            ...
            if (foff + fsz <= max_file_payload)
            {
                ...
                if (fsz >= 4)
                {
                    if (!memcmp(d, "CSEQ", 4)) strcpy(sar->entries[i].ext, ".bcseq");
                    else if (!memcmp(d, "FSEQ", 4)) strcpy(sar->entries[i].ext, ".bfseq");
                    else if (!memcmp(d, "CBNK", 4)) strcpy(sar->entries[i].ext, ".bcbnk");
                    else if (!memcmp(d, "FBNK", 4)) strcpy(sar->entries[i].ext, ".bfbnk");
                    else if (!memcmp(d, "CWAR", 4)) strcpy(sar->entries[i].ext, ".bcwar");
                    else if (!memcmp(d, "FWAR", 4)) strcpy(sar->entries[i].ext, ".bfwar");
                    else if (!memcmp(d, "CWSD", 4)) strcpy(sar->entries[i].ext, ".bcwsd");
                    else if (!memcmp(d, "FWSD", 4)) strcpy(sar->entries[i].ext, ".bfwsd");
                    else if (!memcmp(d, "CWAV", 4)) strcpy(sar->entries[i].ext, ".bcwav");
                    else if (!memcmp(d, "FWAV", 4)) strcpy(sar->entries[i].ext, ".bfwav");
                    else strcpy(sar->entries[i].ext, ".bin");
                }
                else strcpy(sar->entries[i].ext, ".bin");
            }
        }
    ...
    {
        if (string_pool) { for (uint s = 0; s < n_strings; s++) FREE(string_pool[s]); FREE(string_pool); }
        return ERR_INVALID_DATA;
    }
    ...
    for (uint i = 0; i < n_files; i++)
    {
        ...
        if (ref_off < 0 || (uint)ref_off + 8 > info_size) continue;
    ...
    if (bnk_tab_off >= 0 && (uint)bnk_tab_off + 4 <= info_size)
    {
        ...
        for (uint b = 0; b < n_bnks && b < 0x20000; b++)
        {
            ...
            if (ref_off < 0 || (uint)ref_off + 12 > info_size) continue;
    ...
    for (uint i = 0; i < sar->n_entries; i++)
    {
        if (sar->entries[i].data && sar->entries[i].size >= 4)
        {
            ...
            else if (!memcmp(d, "CGRP", 4)) strcpy(sar->entries[i].ext, ".bcgrp");
            else if (!memcmp(d, "FGRP", 4)) strcpy(sar->entries[i].ext, ".bfgrp");
            else if (!memcmp(d, "CWAV", 4)) strcpy(sar->entries[i].ext, ".bcwav");
            else if (!memcmp(d, "FWAV", 4)) strcpy(sar->entries[i].ext, ".bfwav");
            else if (!memcmp(d, "CSTM", 4)) strcpy(sar->entries[i].ext, ".bcstm");
            else if (!memcmp(d, "FSTM", 4)) strcpy(sar->entries[i].ext, ".bfstm");
            else strcpy(sar->entries[i].ext, ".bin");
        }
        else
...
enumError CreateSoundArchive ( u8 **dest, uint *dest_size, const sound_archive_t *sar )
{
    ...
    uint n_strings = 0;
    for (uint i = 0; i < sar->n_entries; i++)
        if (sar->entries[i].name && *sar->entries[i].name) n_strings++;
    ...
    if (strg_block_size < 0x20) strg_block_size = 0x20;

    // 2. Build INFO block
    // Header has 7 references (0 to 6) = 56 bytes
    // File table at offset 56: 4 bytes count, n_entries * 8 bytes refs, n_entries * 24 bytes file entries
...
enumError DecryptArikaInfo ( u8 *buf, uint size )
{
    ...
    if ( size < 0x10 || buf[0] == 0 ) return ERR_OK; // already unencrypted
    u8 key[0x10];
    memcpy(key,buf,0x10);
...
{
    if ( !buf ) return EINVAL;
    if ( size < 0x10 || buf[0] == 0 ) return ERR_OK; // stays unencrypted
    u8 key[0x10];
    memcpy(key,buf,0x10);
    for ( uint i = 0x10; i < size; i++ )
    {
...
(
    arika_list_t *list, const u8 *data, uint size,
    uint off, u32 leaf_size, ccp name
)
{
...
{
    if ( !dest_info || !dest_info_size || !dest_game || !dest_game_size
	 || !entries || !n_entries || n_entries > 0x40000 )
	return EINVAL;
    ...
    if ( !payload || !psize )
    {
	FREE(info); FREE(payload); FREE(psize);
	return ERR_CANT_CREATE;
    }
	...
	else
	{
	    ...
	    if (ssize) memcpy(payload[i],src,ssize);
	    psize[i] = ssize;
	}
	FREE(zdata);
    ...
    {
	for ( uint i = 0; i < n_entries; i++ ) FREE(payload[i]);
	FREE(payload); FREE(psize); FREE(info);
	return EFBIG;
    }
	...
	if (psize[i]) memcpy(game+cur,payload[i],psize[i]);
	FREE(payload[i]);
