...
static const KeywordTab_t opt_mdl_tab[] = { { 0, "CLEAR", "RESET", MDLMD_M_ALL | MDLMD_F_HIDE },
	{ MDLMD_M_DEFAULT, "DEFAULT", 0, MDLMD_M_ALL | MDLMD_F_HIDE },

	{ MDLMD_LOG, "LOG", 0, 0 }, { MDLMD_SILENT, "SILENT", 0, 0 },
	...
	{ MDLMD_VECTOR, "VECTOR", 0, 0 }, { MDLMD_MATRIX, "MATRIX", 0, 0 },
	{ MDLMD_VERTEX, "VERTEX", "VERTICES", 0 },

	{ MDLMD_TEST0, "TEST0", "T0", MDLMD_M_TEST | MDLMD_F_HIDE },
	{ MDLMD_TEST1, "TEST1", "T1", MDLMD_M_TEST }, { MDLMD_TEST2, "TEST2", "T2", MDLMD_M_TEST },
...
void LoadParametersMDL (ccp log_prefix // not NULL:
...
)
{
	...
	if (!MDL_ACTION_LOG (false, "Global MDL Modes: %s\n", GetMdlMode ()) && log_prefix
		&& MDL_MODE != MDLMD_M_DEFAULT && !(MDL_MODE & MDLMD_SILENT))
	{
...
void SetMdlMode (uint new_mode)
{
	...
	if (mdl_switched_to_vertex && (new_mode & (MDLMD_VECTOR | MDLMD_VERTEX)) == MDLMD_VECTOR)
	{
...
int ScanOptMdl (ccp arg)
{
	...
	s64 stat = ScanKeywordList (arg, opt_mdl_tab, 0, true, 0, MDL_MODE, "Option --mdl", ERR_SYNTAX);
	if (stat != -1)
	{
		SetMdlMode (stat);
		return 0;
...
static void *GetVectorMDL (float3 *dest, const void *src, uint format, uint r_shift)
{
	DASSERT (dest);
	DASSERT (src);
...
static inline int float_to_mdl_int (float num, uint r_shift, int min, int max)
{
...
static void *SetVectorMDL (const float3 *src, void *dest, uint format, uint r_shift)
{
	DASSERT (dest);
	DASSERT (src);
...
static const MemItem_t *GetMemItemMDLs1 (const mdl_t *mdl, // pointer to valid MDL
...
static ccp GetNameMDLs1 (const mdl_t *mdl, // pointer to valid MDL
	const MemItem_t *eptr, // pointer to section data
	s32 offset, // offset to neigbour section 1
...
static MemItem_t *FindSectionbyTypeMDL (const mdl_t *mdl, // pointer to valid MDL
...
static MemItem_t *FindSectionbyPtrMDL (const mdl_t *mdl, // valid MDL
...
static void scan_sections (struct StringIteratorMDL_t *sit, // string iterator data
...
void scan_strings (struct StringIteratorMDL_t *sit, // string iterator data
...
void string_collect (struct StringIteratorMDL_t *sit, // string iterator data
	u32 *sptr, // pointer to string index
	u8 *base, // base of item
	u32 *data // pointer to string reference
)
...
void string_export (struct StringIteratorMDL_t *sit, // string iterator data
	u32 *sptr, // pointer to string index
	u8 *base, // base of item
	u32 *data // pointer to string reference
)
{
	...
	if (idx < sit->mdl->spool.n2s_size)
	{
		...
		noPRINT ("-> %d/%d : %zx %x %s\n", idx, sit->sp->n2s_size, res + base - sit->mdl->raw_data,
			res, str);
	}
	write_be32 (data, res);
}
...
void offset_export (struct StringIteratorMDL_t *sit, // string iterator data
	u8 *base, // base of item
	u32 *data // pointer to string reference
)
{
...
void string_copy (struct StringIteratorMDL_t *sit, // string iterator data
	u32 *sptr, // pointer to string index
	u8 *base, // base of item
	u32 *data // pointer to string reference
)
...
void IterateStringsMDL (StringIteratorMDL_t *sit, // string iterator data
...
void IterateAllStringsMDL (StringIteratorMDL_t *sit // string iterator data
)
{
	DASSERT (sit);
	DASSERT (sit->mdl);
...
static int mdl_iter_func (brsub_cut_t *bcut, // pointer to data structure
...
static ccp mdl_names[] = { ".draw-list", //  0
	".bones", //  1
	".verticies", //  2
	".normals", //  3
	".colors", //  4
...
{
	ccp res = index < sizeof (mdl_names) / sizeof (*mdl_names) ? mdl_names[index] : 0;
	if (!res || !*res)
	{
...
int IterateFilesMDL (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	return CutFilesBRSUB (
		it, mdl_iter_func, 0, mdl_names, sizeof (mdl_names) / sizeof (*mdl_names) - 1);
}
...
ccp GetStringMDL (mdl_t *mdl, // MDL data structure
...
static ccp GetStringDataMDL (mdl_t *mdl, // MDL data structure
...
ccp GetStringItMDL (struct szs_iterator_t *it, // iterator struct with all infos
...
static enumError ScanSetupMDL (mdl_t *mdl, // MDL data structure
...
)
{
	...
	ScanParam_t ptab[] = { { "MDL-NAME", SPM_STRING, &mdl->name }, { "MDL-VERSION", SPM_NONE, 0 },
		{ "N-SECTIONS", SPM_NONE, 0 }, { 0 } };
...
static enumError ScanHexMDL (mdl_t *mdl, // MDL data structure
...
)
{
	...
	uint sort_index = 0;
	ScanParam_t ptab[]
		= { { "NAME", SPM_STRING, &name }, { "SORT-INDEX", SPM_UINT, &sort_index }, { 0 } };
...
static const KeywordTab_t mdl_section_name[] = { { MDL_IGNORE, "END", "--", 0 },
	{ MDL_SETUP, "SETUP", "Setup", 0 }, { MDL_SETUP, "BONE-TABLE", "Bone Table", 0 },

	{ MDL_SECTION_0, "SECTION-0", "Section 0", 0 }, { MDL_SECTION_1, "SECTION-1", "Section 1", 0 },
	{ MDL_SECTION_2, "SECTION-2", "Section 2", 0 }, { MDL_SECTION_3, "SECTION-3", "Section 3", 0 },
	{ MDL_SECTION_4, "SECTION-4", "Section 4", 0 }, { MDL_SECTION_5, "SECTION-5", "Section 5", 0 },
	{ MDL_SECTION_6, "SECTION-6", "Section 6", 0 }, { MDL_SECTION_7, "SECTION-7", "Section 7", 0 },
	{ MDL_SECTION_8, "SECTION-8", "Section 8", 0 }, { MDL_SECTION_9, "SECTION-9", "Section 9", 0 },
...
enumError ScanTextMDL (mdl_t *mdl, // MDL data structure
...
static int scan_raw_mdl_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
enumError ScanRawMDL (mdl_t *mdl, // MDL data structure
	bool init_mdl, // true: initialize 'mdl' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
...
)
{
	...
	PRINT ("CONTAINER: %p,0x%x -> %p,0x%x [%d]\n", data, data_size, mdl->old_container->data,
		mdl->old_container->size, mdl->old_container->ref_count);
	...
	{
		return ERROR0 (ERR_INVALID_DATA, "Invalid MDL file: %s\n", mdl->fname ? mdl->fname : "?");
	}
...
static int iter_raw_data_mdl (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
#if USE_NEW_CONTAINER_MDL
	enumError err = ScanMDL (
		&mdl, false, data, it->size, LinkContainerRawData (iparam->raw), iparam->check_mode);
#else
	enumError err
		= ScanMDL (&mdl, false, data, it->size, ContainerRawData (iparam->raw), iparam->check_mode);
#endif
	if (err < ERR_WARNING)
...
enumError IterateRawDataMDL (struct raw_data_t *raw, // valid raw data
	CheckMode_t check_mode, // not NULL: call CheckMDL(mode)
...
)
{
	...
	if (raw->fform == FF_MDL || raw->fform == FF_MDL_TXT || !IsArchiveFF (raw->fform))
	{
		mdl_t mdl;
...
enumError ScanMDL (mdl_t *mdl, // MDL data structure
	bool init_mdl, // true: initialize 'mdl' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
	ContainerMDL_t *cdata, // NULL or container-data
...
)
{
	...
	switch (GetByMagicFF (data, data_size, data_size))
	{
		...
		default:
			...
			if (init_mdl)
				InitializeMDL (mdl);
			return ERROR0 (ERR_INVALID_DATA, "No MDL file: %s\n", mdl->fname ? mdl->fname : "?");
	}
...
enumError ScanRawDataMDL (mdl_t *mdl, // MDL data structure
	bool init_mdl, // true: initialize 'mdl' first
	struct raw_data_t *raw, // valid raw data
	CheckMode_t mode // not NULL: call CheckMDL(mode)
)
...
enumError LoadMDL (mdl_t *mdl, // MDL data structure
	bool init_mdl, // true: initialize 'mdl' first
...
enumError CreateRawMDL (mdl_t *mdl // pointer to valid MDL
)
{
	DASSERT (mdl);
	TRACE ("CreateRawMDL()\n");
	...
	const uint header_size = bone_offset + 4 + mdl->bone_n * sizeof (*mdl->bone_tab);
...
#ifdef TEST0
	{
		...
		for (i = 0; i < brsub_n; i++)
		{
			...
			printf ("\t%04x %04x %04x : %06x %06x : [%2d] |%s|\n", bi->id, bi->left_idx,
				bi->right_idx, bi->name_off, bi->data_off, bi->nlen, bi->name);
		}
	}
#endif
...
enumError SaveRawMDL (mdl_t *mdl, // pointer to valid MDL
...
static enumError SaveTextMDLhex (FILE *f, // file to print to
...
static enumError SaveTextMDLs0 (FILE *f, // file to print to
	const mdl_t *mdl, // pointer to valid MDL
	const MemItem_t *eptr // pointer to section data
)
{
...
static enumError SaveTextMDLs1 (FILE *f, // file to print to
	const mdl_t *mdl, // pointer to valid MDL
	const MemItem_t *eptr // pointer to section data
)
{
	...
	fprintf (f,
		...
		//	,ms.name_off
		,
		ms.index, ms.id, ms.flags, ms.padding[0], ms.padding[1]

		,
		ms.parent_off, GetNameMDLs1 (mdl, eptr, ms.parent_off, ""), ms.child_off,
		GetNameMDLs1 (mdl, eptr, ms.child_off, ""), ms.next_off,
		GetNameMDLs1 (mdl, eptr, ms.next_off, ""), ms.prev_off,
		GetNameMDLs1 (mdl, eptr, ms.prev_off, ""), ms.part2_off,
		GetNameMDLs1 (mdl, eptr, ms.part2_off, ""));
...
static enumError SaveTextMDLs2 (FILE *f, // file to print to
	const mdl_t *mdl, // pointer to valid MDL
	const MemItem_t *eptr // pointer to section data
)
{
	...
	fprintf (f,
		...
		"\r\n",
		ms.length, ms.mdl0_off,
		ms.data_off
		//	,ms.name_off
		,
		ms.index, ms.has_bounds, ms.format, ms.r_shift, ms.stride, ms.n_vertex, ms.n_vertex,
		ms.minimum.x, ms.minimum.y, ms.minimum.z, ms.maximum.x, ms.maximum.y, ms.maximum.z);
...
static enumError SaveTextMDLs10 (FILE *f, // file to print to
	const mdl_t *mdl, // pointer to valid MDL
	const MemItem_t *eptr // pointer to section data
)
{
	...
	fprintf (f,
		...
		"unknown_0x60	= %#10x\r\n"
		"bone_offset	= %#10x\r\n",
		ms.length, ms.mdl0_off, ms.bone_idx, ms.cp_vtx, ms.cp_tex, ms.xf_nor_spec, ms.v_decl_size,
		ms.flags, ms.v_decl_offset, ms.v_data_size1, ms.v_data_size2, ms.v_data_offset, ms.xf_flags,
		ms.unknown_0x34, ms.name_off, ms.idx, ms.idx, ms.n_vertex, ms.n_vertex, ms.n_face,
		ms.n_face, ms.v_group, ms.v_group, ms.n_group, ms.n_group, ms.col_group[0], ms.col_group[1],
		ms.tex_group[0], ms.tex_group[1], ms.tex_group[2], ms.tex_group[3], ms.tex_group[4],
		ms.tex_group[5], ms.tex_group[6], ms.tex_group[7], ms.unknown_0x60, ms.bone_offset);
		...
		while (ptr + 3 < endptr && *ptr)
		{
			...
			if (s2data)
			{
				for (; n > 0; n--, ptr += 4)
				{
					...
					{
						GetVectorMDL (&temp, s2data + vidx * ms2.stride, ms2.format, ms2.r_shift);
					}
					else
...
enumError SaveTextMDL (const mdl_t *mdl, // pointer to valid MDL
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
	...
	if (brief_count)
	{
		fprintf (F.f,
			...
			mdl->name ? mdl->name : "", mdl->version, mdl->n_sect);
	}
	else
	{
		fprintf (F.f, text_mdl_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE,
			mdl->name ? mdl->name : "", mdl->version, mdl->n_sect);
	}
	...
	fprintf (F.f,
		...
		"\r\n",
		h->head_len, h->offset, h->unknown_0x08, h->unknown_0x0c, h->n_vertex, h->n_face,
		h->unknown_0x18, h->unknown_0x0c, h->n_bone, h->unknown_0x0c, h->bone_offset, h->min[0],
		h->min[1], h->min[2], h->max[0], h->max[1], h->max[2]);
	...
	for (; err < ERR_WARNING && eptr < eend; eptr++)
	{
		fprintf (F.f,
			"%s[SECTION-%u]\r\n"
			"# MDL%s.%02u",
			section_sep, eptr->idx1, GetSectNameMDL (eptr->idx1), eptr->idx2);
		...
		if (InContainerP (&mdl->container, eptr->data))
		{
			...
			if (mdl->mdl_offset)
				fprintf (F.f, ", MDL file offset 0x%x, BRRES file offset 0x%x",
					(int)(eptr->data - base - mdl->mdl_offset), (int)(eptr->data - base));
			else
				fprintf (F.f, ", MDL file offset 0x%x", (int)(eptr->data - base));
		}
#else
		if (InDataContainer (mdl->old_container, eptr->data))
		{
			...
			if (mdl->mdl_offset)
				...
				fprintf (
					F.f, ", MDL file offset 0x%x", (int)(eptr->data - mdl->old_container->data));
		}
#endif
		...
		fprintf (F.f,
			...
			"@NAME        = \"%s\"\r\n"
			"\r\n",
			REVISION_NUM, eptr->idx2, eptr->sort_idx, eptr->name);

		switch (eptr->idx1) // [[newsect]]
		{
				//	    case  0:  err = SaveTextMDLs0 (F.f,mdl,eptr); break;
			case 1:
				err = SaveTextMDLs1 (F.f, mdl, eptr);
				break;
			case 2:
				err = SaveTextMDLs2 (F.f, mdl, eptr);
				break;
				//	    case 10:  err = SaveTextMDLs10(F.f,mdl,eptr); break;
			default:
				err = SaveTextMDLhex (F.f, mdl, eptr);
				break;
		}
	}
...
{
	return index < sizeof (MDLs0_types) / sizeof (*MDLs0_types) ? MDLs0_types[index] : 0;
}
...
void CalcMatrixMDLs1 (mdl_sect1_t *ms)
{
	...
	SetupMatrixF34 (&ms->trans_matrix, &ms->scale, &rotate_rad, &ms->translate);

	CalcInverseMatrixF34 (&ms->inv_matrix, &ms->trans_matrix);
}
...
bool HaveStandardVectorsMDLs1 (mdl_sect1_t *ms, bool network_order)
{
	...
	return fabs (ms->scale.x - 1.0) < 1e-6 && fabs (ms->scale.y - 1.0) < 1e-6
		&& fabs (ms->scale.z - 1.0) < 1e-6 && fabs (ms->rotate.y) < 1e-6
		&& fabs (ms->rotate.y) < 1e-6 && fabs (ms->rotate.z) < 1e-6 && fabs (ms->translate.x) < 1e-6
		&& fabs (ms->translate.y) < 1e-6 && fabs (ms->translate.z) < 1e-6;
}
...
void PrintMatrixMDLs1 (FILE *f, // file to print to
...
)
{
	...
	if (mode & 1)
	{
		fprintf (f,
			...
			indent, "", ms1->scale.x, ms1->scale.y, ms1->scale.z, eol,

			indent, "", ms1->rotate.x, ms1->rotate.y, ms1->rotate.z, eol,

			indent, "", ms1->translate.x, ms1->translate.y, ms1->translate.z, eol);
	}
	...
	if (mode & 2)
	{
		fprintf (f,
			...
			indent, "", ms1->minimum.x, ms1->minimum.y, ms1->minimum.z, eol,

			indent, "", ms1->maximum.x, ms1->maximum.y, ms1->maximum.z, eol);
	}
	...
	if (mode & 4)
	{
		fprintf (f,
			...
			indent, "", ms1->trans_matrix.m[0][0], ms1->trans_matrix.m[0][1],
			ms1->trans_matrix.m[0][2], ms1->trans_matrix.m[0][3], eol,

			indent, "", ms1->trans_matrix.m[1][0], ms1->trans_matrix.m[1][1],
			ms1->trans_matrix.m[1][2], ms1->trans_matrix.m[1][3], eol,

			indent, "", ms1->trans_matrix.m[2][0], ms1->trans_matrix.m[2][1],
			ms1->trans_matrix.m[2][2], ms1->trans_matrix.m[2][3], eol, eol);
	}
	...
	if (mode & 8)
	{
		fprintf (f,
			...
			indent, "", ms1->inv_matrix.m[0][0], ms1->inv_matrix.m[0][1], ms1->inv_matrix.m[0][2],
			ms1->inv_matrix.m[0][3], eol,

			indent, "", ms1->inv_matrix.m[1][0], ms1->inv_matrix.m[1][1], ms1->inv_matrix.m[1][2],
			ms1->inv_matrix.m[1][3], eol,

			indent, "", ms1->inv_matrix.m[2][0], ms1->inv_matrix.m[2][1], ms1->inv_matrix.m[2][2],
			ms1->inv_matrix.m[2][3], eol, eol);
	}
}
...
static int find_minimap_mdl (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	else if (it->group == 2)
	{
		...
		while (n_vertex-- > 0)
		{
			...
			noPRINT ("GET= %11.3f %11.3f %11.3f [%d,%d,%d->%d]\n", temp.x, temp.y, temp.z,
				IsNormalF (temp.x), IsNormalF (temp.y), IsNormalF (temp.z), IsNormalF3 (temp.v));

			if (IsNormalF3 (temp.v) && fabsf (temp.x) < 1e7 && fabsf (temp.y) < 1e7
				&& fabsf (temp.z) < 1e7)
			{
				if (min.x > temp.x)
					min.x = temp.x;
		...
		if (max.y > MINIMAP_MAX_VISIBLE)
			mmap->rec_trans_ld.y = mmap->rec_trans_ru.y = 1.34 * (max.y - MINIMAP_MAX_VISIBLE);

		mmap->rec_trans_valid = IsNormalF (mmap->rec_trans_ld.x)
			&& fabsf (mmap->rec_trans_ld.x) < 1e6 && IsNormalF (mmap->rec_trans_ld.y)
			&& fabsf (mmap->rec_trans_ld.y) < 1e6 && IsNormalF (mmap->rec_trans_ld.z)
			&& fabsf (mmap->rec_trans_ld.z) < 1e6 && IsNormalF (mmap->rec_trans_ru.x)
			&& fabsf (mmap->rec_trans_ru.x) < 1e6 && IsNormalF (mmap->rec_trans_ru.z)
			&& fabsf (mmap->rec_trans_ru.z) < 1e6;
	}
	return 0;
}
...
static int find_minimap_brres (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (!term && (it->fform == FF_MDL || it->fform == FF_UNKNOWN && !memcmp (data, MDL_MAGIC, 4)))
	{
		...
		int stat = IterateFilesParSZS (
			&szs, find_minimap_mdl, it->param, false, true, false, 0, 1, SORT_NONE);
		ResetSZS (&szs);
		return stat;
	}
	return 0;
...
static int find_minimap_szs (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (!term
		&& (it->fform == FF_BRRES || it->fform == FF_UNKNOWN && !memcmp (data, BRRES_MAGIC, 4))
		&& !strcmp (it->name, "map_model.brres"))
	{
		...
		int stat = IterateFilesParSZS (
			&szs, find_minimap_brres, it->param, false, true, false, 0, -1, SORT_NONE);
		ResetSZS (&szs);
		return stat;
	}
	return 0;
...
bool FindMinimapData (mdl_minimap_t *mmap, // minmap data to fill
...
)
{
	...
	if (FindMinimapData (&mmap, szs) && mmap.rec_trans_valid)
	{
		...
		hton_MDLs1 (&ld, &ld);
		hton_MDLs1 (&ru, &ru);
		if (memcmp (mmap.posLD, &ld, sizeof (ld)) || memcmp (mmap.posRU, &ru, sizeof (ru)))
		{
...
Slot42MaterialInfo_t Slot42MaterialInfo[S42M__N + 1]
	= { def42 (GOAL_MERG, "Goal_Merg") def42 (IWA, "Iwa") def42 (IWA_ALFA, "Iwa_alfa")
			  def42 (NUKI_RYOUMEN, "Nuki_Ryoumen") def42 (WALLMERG00, "WallMerg00")
				  def42 (MOON_KABE0000, "moon_kabe0000") def42 (MOON_ROAD00, "moon_road00")
					  def42 (ROAD, "road") def42 (ROAD01, "road01") def42 (ROAD02, "road02")
						  def42 (ROAD03, "road03") def42 (SIBA00, "siba00") { 0, "", 0 } };
...
static int ana_slot42_mdl (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
static int ana_slot42_brres (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (!term && it->off + 4 <= it->size
		&& (it->fform == FF_MDL || it->fform == FF_UNKNOWN && !memcmp (data, MDL_MAGIC, 4))
		&& !strcasecmp (it->name, "course"))
	{
		...
		InitializeSubSZS (&szs, it->szs, it->off, it->size, FF_MDL, it->path, false);
		int stat = IterateFilesParSZS (
			&szs, ana_slot42_mdl, it->param, false, true, false, 0, 1, SORT_NONE);
...
static int ana_slot42_szs (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (!term
		&& (it->fform == FF_BRRES || it->fform == FF_UNKNOWN && !memcmp (data, BRRES_MAGIC, 4))
		&& (!strcmp (it->name, "course_model.brres") || !strcmp (it->name, "course_d_model.brres")))
	{
		...
		InitializeSubSZS (&szs, it->szs, it->off, it->size, FF_BRRES, it->path, false);
		int stat = IterateFilesParSZS (
			&szs, ana_slot42_brres, it->param, false, true, false, 0, -1, SORT_NONE);
		ResetSZS (&szs);
		return stat;
	}
	return 0;
...
Slot42MaterialStat_t GetSlot42SupportSZS (struct szs_file_t *szs)
{
	...
	PRINT ("MOONVIEW [%zu] %s: found=%x, mod=%x -> all=%d, content=%d -> ok=%d\n", szs->size,
		GetNameFF (szs->fform_arch, 0), stat42.found, stat42.modified, stat42.all_found,
		stat42.content_ok, stat42.ok);
...
ccp RenameForSlot42MDL (void *rawdata, // pointer to data, modified
...
static void print_sections_mdl (struct StringIteratorMDL_t *sit, // string iterator data
	MemItem_t *mi // memory item of section
)
{
	DASSERT (sit);
...
enumError PrintStringsMDL (FILE *f, // output file
...
enumError AppendMDL (mdl_t *mdl, // working MDL (valid)
...
float3 TransformByMatrix (float3 *val, float m[3][4])
{
	DASSERT (val);
	DASSERT (m);
...
static int patch_mdl (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (it->group == 1)
	{
		...
		if (be32 (&ms->length) == sizeof (*ms))
		{
			if (!mdlpat->root_s1_valid && !ms->parent_off)
			{
				...
				mdlpat->is_transformed = !(mdlpat->brres_type & PFILE_MINIMAP)
