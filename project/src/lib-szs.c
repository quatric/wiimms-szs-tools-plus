...
ccp CreateSpecialInfoAttrib (have_attrib_t attrib, bool add_value, ccp return_if_empty)
{
	static char buf[500];
	char *dest = buf;
...
void InitializeSubSZS (szs_file_t *szs, // SZS to initialize
...
)
{
	...
	if (fform == FF_UNKNOWN)
	{
		fform = RepairMagicByOpt (0, szs->data, szs->file_size, szs->file_size, FF_UNKNOWN, fname);
...
void AssignSZS (szs_file_t *szs, // SZS to initialize
...
void CopySZS (szs_file_t *szs, // SZS to initialize
	bool init_szs, // true: InitializeSZS(), false: ResetSZS()
...
)
{
	...
	if (source && source->data)
		AssignSZS (szs, init_szs, MEMDUP (source->data, source->size), source->size, true,
...
enumError LoadSZS (szs_file_t *szs, // valid szs
...
)
{
	...
	// [[analyse-magic+]]
	szs->fform_file = RepairMagicByOpt (0, szs->cdata, szs->csize, szs->csize, FF_UNKNOWN, fname);
...
enumError SaveSZS (szs_file_t *szs, // valid szs
	ccp fname, // valid pointer to filenname
...
)
{
	...
	File_t F;
	enumError err
		= CreateFILE (&F, true, fname, testmode, false, overwrite, opt_remove_dest, false);
	if (F.f)
	{
		...
		if (wstat != size)
			err = FILEERROR1 (
				&F, ERR_WRITE_FAILED, "Saving file (%zu bytes) failed: %s\n", size, fname);
	}
...
void MarkReadonlySZS (szs_file_t *szs // valid szs
)
{
...
size_t GetDecompressedSizeYAZ (const void *data, // source data
...
)
{
	...
	while (src < src_end && dest < dest_end)
	{
		if (!code_len--)
		{
			...
			{
				count0++;
				fprintf (hexdump, "%*x: %02x -- -- : type byte\n", addr_fw,
					(uint)(src - (u8 *)data), *src);
			}
		...
		if (code & 0x80)
		{
			...
			{
				count1++;
				fprintf (hexdump, "%*x: %02x -- -- : copy direct\n", addr_fw,
					(uint)(src - (u8 *)data), *src);
			}
		...
		else
		{
			...
			if (dest + n > dest_end)
			{
				...
				return silent ? ERR_WARNING
							  : ERROR0 (ERR_INVALID_DATA,
									...
									yaz_version, GetDecompressedSizeYAZ (data, data_size),
									dest_buf_size, fname ? fname : "?");
			}
			...
			if (hexdump)
			{
				...
				{
					count2++;
					fprintf (hexdump, "%*x: %02x %02x --", addr_fw, (uint)(src - 2 - (u8 *)data),
						src[-2], src[-1]);
				}
				else
				{
					count3++;
					fprintf (hexdump, "%*x: %02x %02x %02x", addr_fw, (uint)(src - 3 - (u8 *)data),
						src[-3], src[-2], src[-1]);
				}
				fprintf (hexdump, " : copy %03x off %04d:", n, (uint)(copy_src - dest));
				int max = n < 10 ? n : 10;
...
enumError DecompressSZS (szs_file_t *szs, // valid SZS source, use cdata
...
)
{
	...
	switch (szs->fform_file)
	{
		...
		case FF_YBZ:
			return DecompressYBZ (szs, rm_compressed);
		case FF_BZIP2:
			return DecompressBZIP2 (szs, rm_compressed);
		case FF_LZ:
			return DecompressLZ (szs, rm_compressed);
		case FF_YLZ:
			return DecompressYLZ (szs, rm_compressed);
		case FF_LZMA:
	...
	enumError err
		= DecompressYAZ (cdata + sizeof (yaz0_header_t), szs->csize - sizeof (yaz0_header_t),
			szs->data, szs->size, &szs->size, szs->fname, yaz0->magic[3] - '0', false, hexdump);
	if (err)
		return err;
...
bool TryDecompressSZS (szs_file_t *szs, // valid SZS source, use data if not NULL
	bool rm_compressed // true: remove compressed data
)
{
	DASSERT (szs);
...
void ClearSpecialFilesSZS (szs_file_t *szs // valid szs
)
{
	DASSERT (szs);
	...
	while (src < src_end)
	{
		if (!yref->len)
		{
			fprintf (stdlog, "src=%x, y=%zx,%d,%d\n", (uint)(src - yaz->src),
				sizeof (*yref) * (yref - yaz_buf), yref->ref, yref->len);
...
enumError ClassicCompressYAZ (yaz_compr_t *yaz, int compr)
{
	...
	const u32 range = fast ? 0x100
		: !compr		   ? 0
		: opt_compr < 9	   ? 0x10e0 * opt_compr / 9 - 0x0e0
						   : 0x1000;
	...
	if (range == 0x1000 && IsSZSCacheEnabled () && yaz->szs)
	{
		...
		if (CheckSZSCache (&cc, yaz->szs, yaz->src, yaz->src_len, FF_YAZ0, ".szs"))
		{
			...
			yaz->dest_ptr = yaz->dest_end = cc.cache.cdata + cc.cache.csize;
		...
		if (range && src + 2 < src_end)
		{
			...
			else
			{
				...
				for (; search < src; search++)
				{
					...
					const int len = cmp2 - src;
					DASSERT (cmp1 - search == len);
					noPRINT_IF (search + len > src, "FOUND l=%u,%u pos=%zu,%zu\n", len, found_len,
						search - szs->data, src - szs->data);
			...
			if (optimize && found_len >= 3)
			{
				...
				for (; search < src2; search++)
				{
					...
					DASSERT (cmp1 - search == len);
					noPRINT_IF (search + len > src2, "FOUND l=%u,%u pos=%zu,%zu\n", len, found_len2,
						search - szs->data, src2 - szs->data);
					if (found_len2 < len)
					{
						found_len2 = len;
						saved = search;
...
)
{
	...
	{
		case FF_BZ:
			return CompressBZ (szs, compr, rm_uncompr);
		case FF_YBZ:
			return CompressYBZ (szs, compr, rm_uncompr);
		case FF_BZIP2:
			return CompressBZIP2 (szs, compr, rm_uncompr);
		case FF_LZ:
			return CompressLZ (szs, compr, rm_uncompr);
		case FF_YLZ:
			return CompressYLZ (szs, compr, rm_uncompr);
		case FF_LZMA:
...
enumError CompressYAZ (szs_file_t *szs, int compr, bool remove_uncompressed)
{
	...
	else
	{
		...
		{
			case 11:
				err = BackTrackYAZ (&yaz, opt_compr);
				break; // back tracking
			default:
				err = ClassicCompressYAZ (&yaz, 0);
		}
	}
	...
	if (err == ERR_OK)
	{
		...
		if (szs->fform_file == FF_XYZ)
			...
			memcpy (yaz0->magic, szs->fform_file == FF_YAZ1 ? YAZ1_MAGIC : YAZ0_MAGIC,
				sizeof (yaz0->magic));
...
static enumError SetupWU8Info (wu8_info_t *wu8, szs_file_t *szs, bool encode)
{
	...
	if (!IsAutoAddAvailable ())
		return ERROR0 (
			ERR_CANT_OPEN, "WU8-%sCODE: Can't find autoadd library.\n", encode ? "EN" : "DE");
...
static enumError load_wu8 (wu8_info_t *wu8, // valud wu8
...
)
{
	...
	{
		wu8->err_count++;
		return ERROR0 (ERR_CANT_OPEN, "WU8-%sCODE: Missing file in autoadd library: %s\n",
...
static int code_wu8_pass_1 (struct szs_iterator_t *it, // iterator struct with all infos
...
static int code_wu8_pass_2 (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
static void iterate_wu8 (szs_file_t *szs, // valid szs
...
u8 *DecodeXYZ (u8 *dest, // destination buffer,
...
u8 *EncodeXYZ (u8 *dest, // destination buffer,
						 // If NULL: malloc() is used.
	const u8 *src, // source data
	uint size // size of dest and source
...
static c_bzip2_t get_compressed_bzip2 (szs_file_t *szs, // valid SZS data
...
)
{
	...
	if (compr < 1 && (opt_norm || opt_compr_mode > 1))
	{
		...
		for (cptr = ctab; *cptr > 0 && n > 0; cptr++, n--)
		{
			...
			u8 *temp_data;
			uint temp_size;
			enumError err = EncodeBZIP2 (&temp_data, &temp_size, true, header_size, add_dec_size,
				szs->data, szs->size, compr);
			if (err)
				continue;
	...
	else
	{
		...
		enumError err = EncodeBZIP2 (
			&res.cdata, &res.csize, true, header_size, add_dec_size, szs->data, szs->size, compr);
		if (err)
			res.err = err;
	}
...
enumError DecompressBZ (szs_file_t *szs, bool rm_compressed)
{
	...
	u8 *data;
	uint size;
	enumError err = DecodeBZIP2 (&data, &size, 0, wh->cdata, szs->csize - sizeof (*wh));
	if (err)
		return err;
...
enumError DecompressYBZ (szs_file_t *szs, bool rm_compressed)
{
	...
	uint size;
	enumError err
		= DecodeBZIP2buf (data, usize, &size, yh->cdata, szs->csize - sizeof (ybz_header_t));
	if (err)
	{
		FREE (data);
		return err;
...
enumError DecompressLZ (szs_file_t *szs, bool rm_compressed)
{
	...
	enumError err = DecodeLZMAbin (&data, &size, 0, szs->cdata + delta, szs->csize - delta);
	if (err)
		return err;
...
enumError CompressLZ (szs_file_t *szs, int compr, bool remove_uncompressed)
{
	...
	if (compr == LZMA_DEFAULT_COMPR && CheckSZSCacheSZS (szs, FF_LZ, ".lz", remove_uncompressed))
	{
		return ERR_OK;
	}
	...
	u8 *cdata;
	uint csize;
	enumError err = EncodeLZMA (
		&cdata, &csize, true, sizeof (wlz_header_t), true, szs->data, szs->size, compr);
	if (err)
		return err;
	szs->cdata = cdata;
	szs->csize = csize;
...
enumError CompressYLZ (szs_file_t *szs, int compr, bool remove_uncompressed)
{
	...
	if (compr == LZMA_DEFAULT_COMPR && CheckSZSCacheSZS (szs, FF_YLZ, ".ylz", remove_uncompressed))
	{
		return ERR_OK;
	}
	...
	uint csize;
	enumError err = EncodeLZMA (
		&cdata, &csize, true, sizeof (ylz_header_t), false, szs->data, szs->size, compr);
	if (err)
		return err;
	szs->cdata = cdata;
	szs->csize = csize;
...
enumError CompressLZMA (szs_file_t *szs, int compr, bool remove_uncompressed)
{
	...
	u8 *cdata;
	uint csize;
	enumError err = EncodeLZMA (&cdata, &csize, true, 0, false, szs->data, szs->size, compr);
	...
	*buf = 0;
	...
	if (IsBRSUB (fform) && it->size >= 12)
	{
		...
		if (!bi || bi->warn >= BIMD_FAIL)
			snprintf (buf, buf_size, "%s%3d!%s", colout->fatal, vers, colout->reset);
		else if (bi->warn >= BIMD_INFO)
			snprintf (buf, buf_size, "%s%3d %s", colout->hint, vers, colout->reset);
		else
...
int PrintFileSZS (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (it->is_dir)
	{
		printf ("%s%3x: %7x<       -%8x>       -  -    DIR      -  %.*s%s%s\n", colset->cmd,
			it->index, it->off, it->size, 2 * it->recurse_level, indent_msg, it->path,
			colset->reset);
	}
	else
	{
		...
		const file_format_t fform
			= AnalyseMagicByOpt (0, data, it->size, it->szs->size - it->off, FF_UNKNOWN, it->path);
		FixIteratorExtByFF (it, fform, fform);

		char vers_buf[50];
		PrintVersionSZS (vers_buf, sizeof (vers_buf), it);

		printf ("%s%3x: %8x..%8x %7x %8u  %-4s %-6s %s %s%.*s%s%s\n", col->heading, it->index,
			it->off, it->off + it->size, it->size, it->size,
			PrintID (data, it->size < 4 ? it->size : 4, 0), GetNameFF (0, fform), vers_buf,
			it->no_dirs ? "" : " ", 2 * it->recurse_level, indent_msg, it->path, col->reset);
	}
	return 0;
}
...
enumError CreateU8 (szs_file_t *szs, // valid szs
...
{
	DASSERT (szs);
	PRINT ("CreateU8() namesize=%x, totsize=%x, align=%x, .dir=%d\n", namepool_size, total_size,
		opt_align_u8, create_pt_dir);
	...
	PRINT ("ADDR: %zx..%zx..%zx..%zx..%zx\n", (u8 *)u8head - szs->data, (u8 *)u8node - szs->data,
		(u8 *)name_pool - szs->data, (u8 *)data_ptr - szs->data, szs->size);
	...
	*name_ptr++ = 0;
	...
	for (idx = 0; idx < szs->subfile.used; idx++)
	{
		...
		else
		{
			...
			else
			{
				...
				enumError err = f->load_path
					? LoadFILE (f->load_path, 0, 0, data_ptr, relevant_size, 0, &szs->fatt, true)
					: LoadFILE (
						  source_dir, f->path, 0, data_ptr, relevant_size, 0, &szs->fatt, true);
			...
			DASSERT_MSG (data_ptr <= szs->data + szs->size, "data overlow: %zu=0x%zx bytes\n",
				data_ptr - (szs->data + szs->size), data_ptr - (szs->data + szs->size));
		}
		...
		if (append_name)
		{
			...
			while (pstart < pend)
				*name_ptr++ = *pstart++;
			*name_ptr++ = 0;
			DASSERT_MSG (name_ptr <= name_pool + namepool_size, "name pool overflow: %zx/%x\n",
	...
	DASSERT_MSG (name_ptr == name_pool + namepool_size, "name pool underflow: %zx/%x\n",
		name_ptr - name_pool, namepool_size);
...
enumError CreateU8ByInfo (szs_file_t *szs, // valid szs
	ccp source_dir, // NULL or path to source dir
	u8 *source_data, // NULL or source data
...
)
{
	DASSERT (szs);
	DASSERT (u8info);
	return CreateU8 (szs, source_dir, source_data, u8info->namepool_size, u8info->total_size,
		u8info->have_pt_dir);
}
...
{
	DASSERT (eszs);
	AssignContainer (&eszs->container, eszs->container.protect_level, eszs->data, eszs->data_size,
...
static int node_found (struct szs_iterator_t *it, // iterator struct with all infos
...
static int extract_data_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (!term && !it->is_dir)
	{
		...
		if (eszs->exlevel >= EXLEV_F_ICASE && !strcasecmp (it->path, eszs->subpath))
			return node_found (it, EXLEV_F_ICASE);

		if (eszs->exlevel >= (EXLEV_F_ICASE | EXLEV_F_RMPT) && nopt_path
...
enumError ExtractSZS (szs_extract_t *eszs, // valid data structure
...
)
{
	...
	if (eszs->found_count == 1)
	{
		...
		err = ERROR0 (ERR_CANT_OPEN, "Sub file not exist: %s -> %s\n", path, subpath + 1);
	else
		err = ERROR0 (ERR_CANT_OPEN, "Sub file ambiguous (N=%d): %s -> %s\n", eszs->found_count,
			path, subpath + 1);
...
enumError PrintErrorExtractSZS (szs_extract_t *eszs, // valid data structure
	ccp fname // filename of source
)
{
	DASSERT (eszs);
...
{
	DASSERT (raw);
	AssignContainer (&raw->container, raw->container.protect_level, raw->data, raw->data_size,
...
enumError LoadRawData (raw_data_t *raw, // data structure
...
const ccp ui_type_name[UIT__N + 1] = { "?", "-", "Award", "Channel", "Event", "Font", "Globe",
	"MenuMulti", "MenuOther", "MenuSingle", "Present", "Race", "Title", "language", 0 };
...
static int ui_check_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	if (uc->is_korean == OFFON_AUTO
		&& strstr (path, "/blyt/chara_flag_machine_picture_common.brlyt"))
	{
		if (it->size == 232)
			uc->is_korean = OFFON_ON;
...
int IterateFilesU8 (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	if (term)
	...
	for (fst = fst_base + !it->itpar.show_root_node; fst < fst_end && !stat; fst++)
	{
		...
		if (it->itpar.clean_path && path_dest == path_ptr + 2 && path_ptr[0] == '.'
			&& path_ptr[1] == '.')
			path_dest--;
...
int IterateFilesLTA (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	if (term)
...
bool DumpLTA (FILE *f, // output file
...
)
{
	...
	printf ("%*s"
			...
			"%*s"
			"Node size:    %10u = %#10x\n",
		indent, "", (char *)&xhead->magic, indent, "", head.version, indent, "", head.head_size,
		head.head_size, indent, "", head.file_size, head.file_size, indent, "", head.node_off,
		head.node_off, indent, "", head.node_size, head.node_size);
	...
	if (HaveExtLTA (&head))
		printf ("%*s"
				...
				"%*s"
				"Ext size:     %10u = %#10x\n",
			indent, "", head.ext_off, head.ext_off, indent, "", head.ext_size, head.ext_size);

	printf ("%*s"
			"Base slot:    %10u = %#10x\n"
			"%*s"
			"Num of slots: %10u = %#10x\n"
			"%*s"
			"Last slot:    %10u = %#10x\n",
		indent, "", head.base_slot, head.base_slot, indent, "", head.n_slots, head.n_slots, indent,
		"", last_slot, last_slot);
...
int IterateFilesLFL (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	if (term)
...
enumError CreateLFL (struct szs_file_t *szs, // valid szs
...
)
{
	...
	for (; ptr < end; ptr++)
	{
		...
		const uint size
			= sizeof (lfl_node_t) + ALIGN32 (strlen (path) + 1, 4) + ALIGN32 (ptr->size, 4);
		if (max_size < size)
			max_size = size;
		total_size += size;
	}
...
int IterateFilesDOL (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	if (term)
...
szs_iterator_func GetIteratorFunction (file_format_t fform, // file format format
...
)
{
	...
	return (uint)fform >= FF_N ? 0 : cut_files ? cut_iter_func[fform] : std_iter_func[fform];
}

///////////////////////////////////////////////////////////////////////////////

void FixIteratorExtByFF (szs_iterator_t *it, // valid iterator data
...
void FixIteratorExtByData (szs_iterator_t *it, // valid iterator data
...
void FixIteratorExt (szs_iterator_t *it // valid iterator data
)
{
	if (it && it->fix_extension)
	{
...
void *UpdateIteratorFF (struct szs_iterator_t *it)
{
	...
	it->fform = RepairMagicByOpt (0, data, it->size, it->szs->size - it->off, FF_UNKNOWN, it->path);
	return data;
}

///////////////////////////////////////////////////////////////////////////////

static int iterate_sort_files (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
		...
		if (it->off > it->szs->size || it->off + it->size > it->szs->size)
		{
			if (it->size != M1 (it->size) && WARN_MODE & WARN_INVALID_OFFSET && ErrorLogEnabled ())
				ERROR0 (ERR_WARNING,
					...
					"=> File ignored: %s%s%s\n",
					it->off, it->off + it->size, it->szs->size, it->szs->fname,
					*it->szs->fname ? "/" : "", it->path);
			return 0;
		}
	...
	for (i = 0; ptr < end && !stat; i++, ptr++)
	{
		...
		if (IS_M1 (it->size))
		{
			...
			noPRINT ("SIZE==-1 => ptr=%zd/%d => %x+%x=%x | %zx\n",
				ptr - (szs_subfile_t *)it->szs->subfile.list, it->szs->subfile.used, it->off,
				it->size, it->off + it->size, it->szs->size);
...
static int iterate_sub_files (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
static int simple_norm_collect_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
int IterateFilesSZS (szs_file_t *szs, // valid szs
	szs_iterator_func func, // call back function
	void *param, // user defined parameter
	const iterator_param_t *p_itpar, // NULL or iteration parameters
...
)
{
	...
	PRINT ("IterateFilesParSZS(ipar=%d,clean=%d,rec=%d,cut=%d,sort=%d) ff=%s\n", p_itpar != 0,
		itpar.clean_path, recurse, itpar.cut_files, itpar.sort_mode,
		GetNameFF (szs->fform_file, szs->fform_arch));

	szs_iterator_func ifunc = GetIteratorFunction (szs->fform_arch, itpar.cut_files >= 0);
	if (!ifunc)
		return -1;
	...
	if (itpar.job_norm_create && (szs->fform_arch == FF_U8 || szs->fform_arch == FF_WU8))
	{
		...
		// [[norm]]
		IterateFilesParSZS (
			szs, simple_norm_collect_func, &it.norm_create, false, false, false, 0, -1, SORT_NONE);
	}
...
int IterateFilesParSZS (szs_file_t *szs, // valid szs
	szs_iterator_func func, // call back function
	void *param, // user defined parameter
...
static int collect_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
static int collect_common_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
static int collect_r_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
	...
	else
	{
		...
		noPRINT ("%2u %c%c %6x %6x %s\n", cr->level, file->is_dir ? 'd' : '-',
			file->has_subfiles ? 's' : '-', file->offset, file->size, file->path);
	}
	return 0;
}
...
uint CollectFilesSZS (szs_file_t *szs, // valid szs
...
)
{
	...
	if (recurse > 0 || cut_files >= 0)
	{
		...
		IterateFilesParSZS (
			szs, collect_r_func, &cr, false, false, false, recurse, cut_files, SORT_NONE);
	}
	else
...
uint CollectCommonFilesSZS (szs_file_t *szs, // valid szs
	bool clear, // true: clear list before adding
	SortMode_t sort_mode // sort subfiles
)
{
...
int IterateFilesData (const void *data, // valid data pointer
...
)
{
	...
	const int stat = IterateFilesParSZS (&szs, func, param, false, true, false, -1, 1, SORT_OFFSET);
	ResetSZS (&szs);
	return stat;
}
...
int find_subfile_iter (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	if (!term)
...
static bool find_next_brsub_group (brsub_cut_t *bcut // valid data
)
{
	DASSERT (bcut);
	DASSERT (bcut->bh);
...
int CutFilesBRSUB (szs_iterator_t *it, // iterator struct with all infos
...
)
{
	...
	it->is_dir = 0;
	PrintHeaderNameBRSUB (it->path, sizeof (it->path), (brsub_header_t *)data, n_grp, it->endian);
	...
	if (bcut.found_grp >= 0 && bcut.found_off > it->size)
	{
		...
		bcut.path_end = it->path + snprintf (it->path, sizeof (it->path), "%s.header.bin", bcut.id);
	...
	while (bcut.found_grp >= 0)
	{
		...
		if (bcut.fattrib & FFT_BRSUB2)
		{
			...
			if (it->itpar.cut_files)
			{
				bcut.path_end = it->path
					+ snprintf (it->path, sizeof (it->path), ".%s.s%0*u%s.header.n%u.bin", bcut.id,
						n_grp > 10 ? 2 : 1, cur_grp, sname, bg_n_entry);
				it->size = bg_size;
				it->name = 0;
			...
			for (cur_entry = 0; cur_entry < bg_n_entry; cur_entry++, bcut.eptr++)
			{
				if ((u8 *)bcut.eptr > max_eptr)
				{
					if (WARN_MODE & WARN_INVALID_OFFSET && ErrorLogEnabled ())
						ERROR0 (ERR_WARNING,
							...
							bg_n_entry - cur_entry, bg_n_entry, it->szs->fname,
							*it->szs->fname ? "/" : "", it->path);
					break;
				}
				...
				bcut.path_end = it->path
					+ snprintf (it->path, sizeof (it->path), "%s.s%0*u_%0*u%s.%s.bin", bcut.id,
						n_grp > 10 ? 2 : 1, cur_grp, bg_n_entry > 10 ? 2 : 1, cur_entry, sname,
						name_ptr);
				it->name = name_ptr;
		...
		plain:
...
#else
			it->size = bcut.found_off > cur_off ? bcut.found_off - cur_off : bcut.size - cur_off;
#endif
			noPRINT (
			...
			{
				bcut.path_end = it->path
					+ snprintf (it->path, sizeof (it->path), "%s.s%0*u%s.data.bin", bcut.id,
						n_grp > 10 ? 2 : 1, cur_grp, sname);
				it->name = 0;
...
int IterateFilesFuncBRSUB (brsub_cut_t *bcut, // pointer to data structure
...
int IterateFilesBRSUB (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
szs_subfile_t *InsertSubFileList (szs_subfile_list_t *sl, // valid subfile list
...
)
{
	...
	{
		// insert
		memmove (sl->list + insert_pos + 1, sl->list + insert_pos,
...
szs_subfile_t *InsertSubfileSZS (szs_file_t *szs, // valid data structure
	uint insert_pos, // insert position index, robust
...
szs_subfile_t *FindSubFileSZS (szs_subfile_list_t *sl, // valid subfile list
...
static int get_brres_sort_value (ccp name)
{
	...
	{
		case '3':
			return !memcmp (name, "3DModels(NW4R)/", 15) ? SORTVAL_3DMODELS : SORTVAL_OTHER;

		case 'T':
			return !memcmp (name, "Textures(NW4R)/", 15) ? SORTVAL_TEXTURES : SORTVAL_OTHER;

		case 'A':
			switch (name[3])
			{
				case 'C':
					return !memcmp (name, "AnmChr(NW4R)/", 13) ? SORTVAL_ANMCHR
						: !memcmp (name, "AnmClr(NW4R)/", 13)  ? SORTVAL_ANMCLR
															   : SORTVAL_OTHER;

				case 'T':
					return !memcmp (name, "AnmTexPat(NW4R)/", 16) ? SORTVAL_ANMTEXPAT
						: !memcmp (name, "AnmTexSrt(NW4R)/", 16)  ? SORTVAL_ANMTEXSRT
																  : SORTVAL_OTHER;

				case 'S':
					return !memcmp (name, "AnmScn(NW4R)/", 13) ? SORTVAL_ANMSCN
						: !memcmp (name, "AnmShp(NW4R)/", 13)  ? SORTVAL_ANMSCN
															   : SORTVAL_OTHER;
			}
	}
...
CmpFuncSubFile GetCmpFunc (SortMode_t sort_mode, // wanted sort mode
...
)
{
	...
	if (sl->sort_mode != sort_mode && sort_mode != SORT_NONE)
	{
		...
		if (n > 1)
		{
			...
			if (n > 1)
				qsort (list, n, sizeof (*sl->list), (qsort_func)GetCmpFunc (sort_mode, 0));
			return true;
		}
	}
	return false;
...
bool SortSubFilesSZS (szs_file_t *szs, // valid szs structure
	SortMode_t sort_mode // wanted sort mode
)
{
	DASSERT (szs);
	return SortSubFiles (&szs->subfile, sort_mode, szs->fform_arch, szs->order_list);
}
...
enumError DiffSZS (szs_file_t *szs1, // first szs to compare
...
)
{
	...
	noPRINT ("PTR: %p..%p/%u, %p..%p/%u\n", p1, e1, szs1->subfile.used, p2, e2, szs2->subfile.used);
	...
	while (p1 && p1 < e1 && p2 && p2 < e2)
	{
		...
		if (!p1->is_dir)
		{
			...
			else if (p1->size && p1->size <= szs1->size && p1->offset + p1->size <= szs1->size
				&& memcmp (szs1->data + p1->offset, szs2->data + p2->offset, p1->size))
			{
				if (quiet)
					return ERR_DIFFER;
				differ++;
		...
		if (!sp->data)
		{
			...
			if (pi)
			{
				...
				if (pi->num >= sp->n2s_size)
				{
					...
					sp->n2s_list = REALLOC (sp->n2s_list, sizeof (*sp->n2s_list) * new_size);
...
void CalcStringPool (string_pool_t *sp, u32 base_offset, const endian_func_t *endian)
{
	DASSERT (sp);
	DASSERT (!sp->data);
	DASSERT (!sp->offset);
