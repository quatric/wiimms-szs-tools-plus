...
void CreateSSChecksum (char *buf, uint bufsize, const sha1_size_hash_t *ss)
{
	...
	{
		PRINT ("TAB64=%.65s\n", TableEncode64BySetup);
		EncodeBase64 (buf, bufsize, ss->hash, sizeof (ss->hash), TableEncode64BySetup, false, 0, 0);
	}
	else if (opt_id)
	{
		sha1_id_t id;
...
void LogCacheActivity (ccp keyword, ccp format, ...)
{
	...
	*buf = 0;
	...
	fprintf (f, "%s %s %-4s %s\n", PrintMTimeByFormat ("%F %T.@@@", GetTimeMSec (false)), myid,
		keyword, buf);
	fflush (f);
}
...
enumError LoadSZSCache (void)
{
	...
	CloseFile (&F, 0);
	if (opt_log_cache)
		LogCacheActivity ("LOAD", "%u entries in %s: %s", szs_cache.used,
			PrintTimerNSec6 (0, 0, GetTimerNSec () - start_time, 0), path);
...
enumError SaveSZSCache (bool force)
{
	...
	fprintf (F.f,
		...
		"\n",
		szs_cache.used, szs_cache.used == 1 ? "" : "s", VERSION, REVISION_NUM, szs_cache_last_scan,
		tbuf);
...
)
{
	...
	bool exists;
	const ParamFieldItem_t *cit = StoreSZSCache (dest_fname, cc->csum, true, &exists);
	...
	if (exists && cit)
	{
		...
		if (!LoadSZS (&cc->cache, cache_fname, false, true, true) && cc->cache.cdata
			&& cc->cache.csize && cc->cache.fform_file == fform)
		{
