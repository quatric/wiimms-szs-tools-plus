...
#define TITLE                                                                                      \
	WPATT_SHORT ": " WPATT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
static enumError iter_cat (pat_t *pat, // PAT data structure
...
static enumError cmd_convert (int cmd_id, ccp cmd_name, ccp def_path)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!testmode)
		{
			err = dest_ff == FF_PAT ? SaveRawPAT (&pat, dest, opt_preserve)
