...
#define TITLE                                                                                      \
	WBMGT_SHORT ": " WBMGT_LONG " v" VERSION " r" REVISION " " SYSTEM2 " - " AUTHOR " - " DATE
...
static enumError cmd_points ()
{
	if (n_param)
	{
		ERROR0 (ERR_WARNING, "%u parameter%s ignored.\n", n_param, n_param == 1 ? "" : "s");
		n_param = 0;
	}
...
static enumError cmd_extract ()
{
	if (n_param != 2)
		return ERROR0 (ERR_SYNTAX,
			"Excat 2 parameters expected:"
			" Section name and file\n");

	ccp sname = first_param->arg;
	const uint slen = strlen (sname);
	if (slen < 1 || slen > 4)
		return ERROR0 (ERR_SYNTAX, "Section name must consist of 1 to 4 characters: %s", sname);
	...
	if (si)
	{
		...
		if (!print_header)
		{
			const uint head_size = si->head_size ? si->head_size : sizeof (bmg_section_t);
			if (size >= head_size)
			{
				data += head_size;
				size -= head_size;
		...
		PRINT ("FOUND: %.4s, off: %u 0x%x, size: %u 0x%x [h=%d]\n", si->sect->magic, si->offset,
			si->offset, size, size, print_header);
...
static enumError cmd_sections ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		printf ("\n"
				...
				"%s%.*s%s\n",
			colout->heading, arg, colout->heading, sl->source_size, sl->source_size,
			sl->endian->is_be		? " BE"
				: sl->endian->is_le ? " LE"
									: "",
			sl->n_sections, sl->header->encoding, GetEncodingNameBMG (sl->header->encoding, "?"),
			colout->heading, seplen, ThinLine300_3, colout->heading, colout->heading,
			colout->heading, seplen, ThinLine300_3, colout->reset);

		for (si = sl->info; si->sect; si++)
		{
			printf ("%8x %7x %7u", si->offset, si->real_size, si->real_size);

			if (si->real_size < si->sect_size)
				printf (" %s%3u%s  ", colout->warn, si->sect_size - si->real_size, colout->reset);
			else
				fputs ("   -  ", stdout);
...
static enumError cmd_list ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!lerr && raw_data)
		{
			...
			if (IsMSBT (raw_data, (uint)raw_size))
			{
				...
				if (ScanMSBT (&msbt, raw_data, (uint)raw_size, arg) == ERR_OK)
				{
					...
					{
						printf ("%5u = [%s] %s\n", i,
							msbt.entries[i].label ? msbt.entries[i].label : "",
				...
				if (ScanMSBP (&msbp, raw_data, (uint)raw_size, arg) == ERR_OK)
				{
					...
					for (uint i = 0; i < msbp.num_colors; i++)
						printf ("%5u = %s (#%02X%02X%02X%02X)\n", i,
							msbp.colors[i].name ? msbp.colors[i].name : "", msbp.colors[i].r,
				...
				if (ScanMSBF (&msbf, raw_data, (uint)raw_size, arg) == ERR_OK)
				{
					...
					for (uint i = 0; i < msbf.num_nodes; i++)
						printf ("%5u = [type=%u, label=%s, next=%u]\n", i, msbf.nodes[i].type,
							msbf.nodes[i].label ? msbf.nodes[i].label : "",
							msbf.nodes[i].next_node);
					ResetMSBF (&msbf);
					FREE (raw_data);
					continue;
				}
		...
		for (; bi < bi_end; bi++)
		{
			...
			{
				PrintString16BMG (
					iobuf, sizeof (iobuf), bi->text, bi->len, BMG_UTF8_MAX, 0, opt_bmg_colors);
...
static enumError cmd_slots ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		printf ("\n"
				...
				"%s%.*s%s\n",
			colout->heading, GetNameFF_BMG (&bmg), arg, colout->heading, seplen, ThinLine300_3,
			colout->heading, colout->heading, seplen, ThinLine300_3, colout->reset);
...
static enumError process_msbt_file (int cmd_id, ccp arg, ccp def_path)
{
	...
	else if (cmd_id == CMD_ENCODE
		|| (cmd_id == CMD_PATCH && (is_msbt_txt || is_msbp_txt || is_msbf_txt)))
	{
		char dest[PATH_MAX];
		SubstDest (dest, sizeof (dest), arg, opt_dest, def_path,
			is_msbt_txt		  ? ".msbt"
		...
		if (verbose >= 0 || testmode)
			fprintf (stdlog, "%s%s %s:%s -> %s\n", verbose > 0 ? "\n" : "",
				...
				is_msbt_txt		  ? "MSBT-TXT"
					: is_msbp_txt ? "MSBP-TXT"
								  : "MSBF-TXT",
				arg, dest);
...
static enumError cmd_patch (int cmd_id, ccp cmd_name, ccp def_path)
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		const file_format_t dest_ff
			= cmd_id == CMD_ENCODE || cmd_id == CMD_PATCH && !bmg.is_text_src ? FF_BMG : FF_BMG_TXT;

		SubstDest (dest, sizeof (dest), arg, opt_dest, def_path, GetExtFF (dest_ff, 0), false);

		if (verbose >= 0 || testmode)
			fprintf (stdlog, "%s%s%s %s:%s -> %s:%s\n", verbose > 0 ? "\n" : "",
				testmode ? "WOULD " : "", cmd_name, GetNameFF_BMG (&bmg), bmg.fname,
		...
		if (!testmode)
		{
			err = dest_ff == FF_BMG ? SaveRawXBMG (&bmg, dest, opt_preserve)
	...
	for (;;)
	{
		...
		switch ((enumGetOpt)opt_stat)
		{
			...
			case GO_BMG_INF_SIZE:
				err += ScanOptBmgInfSize (optarg, true);
				break;
			case GO_BMG_MID:
...
static enumError CheckCommand (int argc, char **argv)
{
	...
	switch ((enumCommands)cmd_ct->id)
	{
		...
		case CMD_CAT:
			err = cmd_bmg_cat (false);
			break;
		case CMD_MIX:
			err = cmd_bmg_cat (true);
			break;
		case CMD_IDENTIFIER:
			err = cmd_bmg_identifier ();
			break;
		case CMD_DECODE:
			err = cmd_patch (cmd_ct->id, "DECODE", "\1P/\1N.txt");
			break;
		case CMD_ENCODE:
			err = cmd_patch (cmd_ct->id, "ENCODE", "\1P/\1N\1?T");
			break;
		case CMD_PATCH:
			err = cmd_patch (cmd_ct->id, "PATCH", "\1P/\1F");
			break;
