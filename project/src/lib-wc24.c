...
static int resolve_aes_key ( const char *arg, uint8_t key[16] )
{
	...
	fclose(f);
	if (!ok)
	    fprintf(stderr,"WC24: %s exists but is neither a 16-byte key "
...
enumError WC24EncryptFile ( ccp infile, ccp outfile, ccp aes_keyarg,
			     ccp rsa_pem_path, ccp ivarg )
{
    ...
    if (!rsa_ok)
    {
	fprintf(stderr,"WC24: %s is not a PKCS#1 'RSA PRIVATE KEY' PEM "
