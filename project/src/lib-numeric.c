...
{
	return ScanSizeOptU32 (&opt_align_u8, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	return ScanSizeOptU32 (&opt_align_lta, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	return ScanSizeOptU32 (&opt_align_pack, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	return ScanSizeOptU32 (&opt_align_brres, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	return ScanSizeOptU32 (&opt_align_breff, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	return ScanSizeOptU32 (&opt_align_breft, // u32 * num
			   arg, // ccp source
			   1, // default_factor1
			   0, // int force_base
...
{
	const int stat = ScanSizeOptU32 (&opt_align, // u32 * num
						 arg, // ccp source
						 1, // default_factor1
						 0, // int force_base
						 "align", // ccp opt_name
	...
	if (!stat)
		opt_align_u8 = opt_align_pack = opt_align_brres = opt_align_breff = opt_align_breft
			= opt_align;
	return stat;
}
...
{
	return ScanSizeOptU32 (&opt_max_file_size, // u32 * num
			   arg, // ccp source
			   MiB, // default_factor1
			   0, // int force_base
...
int ScanOptEpsilon (ccp arg)
{
	...
	else
	{
		...
		epsilon_pos = epsilon_rot = epsilon_scale = epsilon_time = epsilon;
	}
...
{
	static const KeywordTab_t tab[] = { { 0, "OFF", "AUTO", 0 }, { 0, 0, 0, 0 } };
	...
	uint num = 0;
	const int stat = ScanSizeOptU32 (&num, // u32 * num
						 arg, // ccp source
						 1, // default_factor1
						 0, // int force_base
...
char *ScanNumU32 (ccp arg, // text to scan (valid pointer)
...
char *ScanRangeU32 (ccp arg, // text to scan (valid pointer)
	u32 *p_stat, // NULL or return status:
...
unsigned long int strtoul16 (const u16 *ptr16, u16 **end16, int base)
{
	DASSERT (ptr16);
...
u16 *ScanNum16U32 (const u16 *arg, // text to scan (valid pointer)
	u32 *p_stat, // NULL or return status:
				 //   number of scanned numbers (0..1)
	u32 *p_num, // return value (valid pointer)
	u32 min, // min allowed value (fix num)
...
u16 *ScanRange16U32 (const u16 *arg, // text to scan (valid pointer)
	u32 *p_stat, // NULL or return status:
				 //   number of scanned numbers (0..2)
	u32 *p_n1, // first return value (valid pointer)
	u32 *p_n2, // second return value (valid pointer)
...
double LengthF (const float *a, // A vector (x,y,z)
...
double Length2F (const float *a, // A vector (x,y,z)
	const float *b // B vector (x,y,z)
)
{
	DASSERT (a);
...
double LengthD (const double *a, // A vector (x,y,z)
...
double Length2D (const double *a, // A vector (x,y,z)
	const double *b // B vector (x,y,z)
)
{
	DASSERT (a);
...
double AngleVector (const float *prev, // vector (x,y,z) of previous point
...
void WidthAndDirection (double *r_width, // if not NULL: return width here
...
double CalcDirection2F (const float *a, // A point (x,y)
	const float *b // B point (x,y)
)
{
	const double dx = b[0] - a[0];
	const double dy = b[1] - a[1];
	return fabs (dx) + fabs (dy) < 1e-6 ? 0.0 : atan2 (dx, dy);
}

///////////////////////////////////////////////////////////////////////////////

float3 CalcDirection3F (const float *a, // A point (x,y,z)
...
double3 CalcDirection3D (const double3 *a, // A point (x,y,z)
...
void Scale (const double3 *origin, // origin of rotation, NULL = v(0,0,0)
...
void Rotate (const double3 *origin, // origin of rotation, NULL = v(0,0,0)
...
void Translate (const double3 *trans, // NULL or translation vector
	double3 *pt, // pointer to first point to rotate
	uint pt_delta, // delta in bytes to next point
	uint n_pt // number of points to rotate
)
...
void AxisRotate (const double3 *a1, // first axis point
...
void MinMax3 (double3 *min, // valid pointer to initialized min value
...
bool OverlapCubeTri (double3 *cube_mid, // middle of the cube
...
void CalcTriMetricsByPoints (tri_metrics_t *met, // result
...
void CalcTriMetrics (tri_metrics_t *met)
{
	...
	// calc side lengths
	const double a2 = LengthSqare3 (x);
	met->l1 = sqrt (a2);
	const double b2 = LengthSqare3 (y);
	met->l2 = sqrt (b2);
	const double c2 = LengthSqare3 (z);
	met->l3 = sqrt (c2);
...
void CreateRegularOctahedron (octahedron_t *oct, // valid pointer, initialize
...
void CreateRegularOctahedronF (octahedron_t *oct, // valid pointer, initialize
	double radius, // radius of octahedron
...
void CreateOctahedron (octahedron_t *oct, // valid pointer, initialize
...
void ScaleOctahedron (octahedron_t *oct, // valid octahedron data
...
void RotateOctahedron (octahedron_t *oct, // valid octahedron data
	const double3 *origin, // origin of rotation, NULL = v(0,0,0)
	const double3 *rot // not NULL: rotate octahedron
)
{
...
void ShiftOctahedron (octahedron_t *oct, // valid octahedron data
	const double3 *shift // not NULL: shift octahedron
)
{
	DASSERT (oct);
...
void CreateCube (cuboid_t *cub, // valid pointer, initialize
...
void CreateCubeF (cuboid_t *cub, // valid pointer, initialize
	double width, // width of cube
...
void CreateCuboid (cuboid_t *cub, // valid pointer, initialize
...
void CreateCuboidF (cuboid_t *cub, // valid pointer, initialize
...
void ScaleCuboid (cuboid_t *cub, // valid cuboid data
	const double3 *origin, // origin of rotation, NULL = v(0,0,0)
...
void RotateCuboid (cuboid_t *cub, // valid cuboid data
	const double3 *origin, // origin of rotation, NULL = v(0,0,0)
	const double3 *rot // not NULL: rotate cuboid
)
{
...
void ShiftCuboid (cuboid_t *cub, // valid cuboid data
	const double3 *shift // not NULL: shift cuboid
)
{
	DASSERT (cub);
...
void FillIL (IndexList_t *il, // valid index list
...
void DefineIL (IndexList_t *il, // valid index list
...
bool RemoveIL (IndexList_t *il, // valid index list
	uint index // index of object
)
{
	DASSERT (il);
...
ccp GetNameIL (IndexList_t *il, // valid index list
	uint index // index of object
)
{
	DASSERT (il);
...
void PrintNameIL (FILE *f, // output files
...
const u8 cc84[256] = // convert 8-bit color to 4-bit color
	{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
		0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02,
		0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x03, 0x03,
		0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03,
		0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04,
		0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05,
		...
		0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x0a, 0x0a, 0x0a,
		0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0a, 0x0b,
		0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
		0x0b, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c,
		0x0c, 0x0c, 0x0c, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0d,
		0x0d, 0x0d, 0x0d, 0x0d, 0x0d, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e,
		0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0e, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f,
		0x0f };

//-----------------------------------------------------------------------------

const u8 cc85[256] = // convert 8-bit color to 5-bit color
	{ ,
		...
		0x1d, 0x1d, 0x1d, 0x1e, 0x1e, 0x1e, 0x1e, 0x1e, 0x1e, 0x1e, 0x1e, 0x1f, 0x1f, 0x1f, 0x1f,
		0x1f };

//-----------------------------------------------------------------------------

const u8 cc85s1[256] = // convert 8-bit color to 5-bit color, shifted by 1 bit
	{  };

//-----------------------------------------------------------------------------

const u8 cc86[256] = // convert 8-bit color to 6-bit color
	{ ,
		...
		0x29, 0x29, 0x29, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2b, 0x2b, 0x2b, 0x2b, 0x2c, 0x2c, 0x2c,
		0x2c, 0x2d, 0x2d, 0x2d, 0x2d, 0x2e, 0x2e, 0x2e, 0x2e, 0x2f, 0x2f, 0x2f, 0x2f, 0x30, 0x30,
		0x30, 0x30, 0x31, 0x31, 0x31, 0x31, 0x32, 0x32, 0x32, 0x32, 0x33, 0x33, 0x33, 0x33, 0x34,
		0x34, 0x34, 0x34, 0x35, 0x35, 0x35, 0x35, 0x36, 0x36, 0x36, 0x36, 0x37, 0x37, 0x37, 0x37,
		0x38, 0x38, 0x38, 0x38, 0x39, 0x39, 0x39, 0x39, 0x3a, 0x3a, 0x3a, 0x3a, 0x3b, 0x3b, 0x3b,
		0x3b, 0x3c, 0x3c, 0x3c, 0x3c, 0x3d, 0x3d, 0x3d, 0x3d, 0x3e, 0x3e, 0x3e, 0x3e, 0x3f, 0x3f,
		0x3f };
