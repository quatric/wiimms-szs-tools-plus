...
typedef struct kmp_wim0_info_t
{
	...
	const kmp_wim0_sect_t *sect; // pointer to section
...
typedef enum kmp_mode_t
{
	...
	KMPMD_M_POLE = KMPMD_POLE_LEFT | KMPMD_POLE_RIGHT,
	KMPMD_M_STARTPOS = KMPMD_SP_WIDE | KMPMD_SP_NARROW,
	KMPMD_M_STGI = KMPMD_M_POLE | KMPMD_M_STARTPOS | KMPMD_M_LAPS,
	...
	KMPMD_M_FIX_PH = KMPMD_FIX_CKPH | KMPMD_FIX_ENPH | KMPMD_FIX_ITPH,
	KMPMD_M_FIX_CK = KMPMD_FIX_CKPH | KMPMD_FIX_CKNEXT | KMPMD_FIX_CKJGPT,
	KMPMD_M_FIX_ALL = KMPMD_M_FIX_PH | KMPMD_M_FIX_CK,
	...
	KMPMD_M_XPF = KMPMD_MASK_PFLAGS | KMPMD_RM_LECODE | KMPMD_PURGE_GOBJ | KMPMD_FULL_DEFOBJ,
	...
	KMPMD_M_DUMP_ALL = KMPMD_DUMP_CLASS | KMPMD_DUMP_ONEWAY,
	...
	KMPMD_M_HIDDEN = KMPMD_LOG | KMPMD_SILENT | KMPMD_INPLACE,
	...
	KMPMD_M_TEST = KMPMD_TEST1 | KMPMD_TEST2,
	...
	KMPMD_M_PATCH = KMPMD_NEW // patching flags
		| KMPMD_RM_SPCITEM | KMPMD_REVERSE | KMPMD_MAX_LAPS | KMPMD_M_STGI | KMPMD_M_FIX_ALL
		| KMPMD_M_XPF | KMPMD_M_TINY | KMPMD_RM_EMPTY | KMPMD_MINIMIZE | KMPMD_M_MISC
		| KMPMD_M_TEST,

	KMPMD_M_ALL = KMPMD_M_GENERAL // all relevant bits
		| KMPMD_M_PATCH | KMPMD_M_DUMP_ALL | KMPMD_M_HIDDEN,
...
typedef enum kmp_diff_mode_t
{
	...
	KDMD_M_GOOD = KDMD_CKPH // good setting (fully implemented)
		| KDMD_CNPT | KDMD_ENPH | KDMD_GOBJ | KDMD_ITPH | KDMD_JGPT | KDMD_MSPT | KDMD_M_SORT,

	KDMD_M_ALL = KDMD_M_DEFAULT | KDMD_M_SORT,
...
typedef enum kmp_pt_flag_t
{
	...
	KPFL_M_ENPT = KPFL_FALL | KPFL_SNAP | KPFL_JGPT,

	KPFL_M_ITPT = KPFL_FALL | KPFL_JGPT,

	KPFL_M_JGPT = KPFL_FALL | KPFL_SNAP,

	KPFL_M_ALL = KPFL_M_ENPT | KPFL_M_ITPT | KPFL_M_JGPT,
...
{
	ObjectFlags_t obj_flags; // >0: ObjectFlags_t of analyzed itembox
...
typedef enum draw_mode_t
{
	...
	DMD_M_OFFLINE = DMD_1OFFLINE // offline presence flags
		| DMD_2OFFLINE | DMD_3OFFLINE,

	DMD_M_ONLINE = DMD_1ONLINE // online presence flags
		| DMD_2ONLINE | DMD_3ONLINE,
	...
	DMD_M_RESPAWN = DMD_CKPT // all about respawning
		| DMD_JGPT | DMD_CJGPT | DMD_KTPT | DMD_KCL,

	DMD_M_PTS = DMD_JGPT // single points
		| DMD_KTPT,

	DMD_M_GOBJ = DMD_ITEMBOX // global objects
		| DMD_COIN | DMD_ROADOBJ | DMD_SOLIDOBJ | DMD_DECORATION,

	DMD_M_ADD = DMD_M_PTS // modes that add faces
		| DMD_M_GOBJ | DMD_ENPT | DMD_ITPT | DMD_CKPT | DMD_POTI | DMD_AREA | DMD_CNPT | DMD_CJGPT
		| DMD_DISPATCH | DMD_BLACK | DMD_WHITE,

	DMD_M_ALL = DMD_M_ADD // all possible modes
		| DMD_KCL | DMD_M_PFLAGS | DMD_M_FLAGS,
...
typedef struct cond_ref_info_t
{
	...
	const gobj_cond_ref_t *condref; // not NULL: definition found
...
{
	return ref_id >= GOBJ_MIN_CONDITION && ref_id <= GOBJ_MAX_CONDITION;
}

static inline bool IsConditionRefEngineId (u16 ref_id)
{
	return ref_id >= GOBJ_MIN_COND_ENGINE && ref_id <= GOBJ_MAX_COND_ENGINE;
}

static inline bool IsConditionRefRandomId (u16 ref_id)
{
	return ref_id >= GOBJ_MIN_COND_RND && ref_id <= GOBJ_MAX_COND_RND;
}
...
{
	return ref_id >= GOBJ_MIN_COND_TEST && ref_id <= GOBJ_MAX_COND_TEST;
}
...
{
	return obj_id >= GOBJ_MIN_DEF_OR && obj_id <= GOBJ_MAX_DEF_OR;
}

static inline bool IsDefinitionObjectIdAND (u16 obj_id)
{
	return obj_id >= GOBJ_MIN_DEF_AND && obj_id <= GOBJ_MAX_DEF_AND;
}
...
{
	return g && IsDefinitionObjectId (g->ref_id) && (g->pflags & GOBJ_M_PF_MODE) == GOBJ_PF_MODE1;
}
...
typedef enum opt_gamemode_t
{
	...
	GMD_M_SORT = GMD_NSORT | GMD_ISORT,

	GMD_M_OPTIONS = GMD_ENGINE // start flags if scanning options
		| GMD_RANDOM,

	GMD_M_DEFAULT = GMD_MD_DEFAULT | GMD_FI_DEFAULT | GMD_ENGINE | GMD_RANDOM,

	GMD_M_ALL = GMD_MD_ALL | GMD_FI_ALL | GMD_OP_ALL
} opt_gamemode_t;
...
typedef struct kmp_ana_gobj_t
{
	...
	const kmp_gobj_entry_t *gobj; // pointer to GOBJ list
...
void AnalysePFlagScenarios (kmp_ana_pflag_t *ap, const kmp_t *kmp, opt_gamemode_t gmode);
