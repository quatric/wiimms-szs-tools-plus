...
)
{
	...
	if (ntohl (pack->magic) != PACK_MAGIC_NUM || size < szs->size || offset_metric < 0x20
...
{
	DASSERT (szs);
	PRINT ("CreatePACK() ndir=%d, totfsize=%x, align=%x,%x\n", szs->subfile.used, total_fsize,
		opt_align_breff, opt_align_pack);
	...
	const u32 metric_size = ALIGN32 (n_files * sizeof (pack_metric_t),
	...
	for (i = 0, fptr = szs->subfile.list; i < szs->subfile.used; i++, fptr++)
	{
		...
		if (!fptr->is_dir)
		{
			...
			else
			{
				...
				enumError err = LoadFILE (source_dir, fptr->path, 0, szs->data + file_offset,
					fptr->size, 0, &szs->fatt, true);
				if (max_err < err)
					max_err = err;
			}
