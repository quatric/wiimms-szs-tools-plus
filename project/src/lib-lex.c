...
bool LEX_ACTION_LOG (bool is_patch, const char *format, ...)
{
	...
	{
		fflush (stdout);
		fprintf (stdlog, "    %s>[LEX]%s ", colset->heading,
...
static const feature_var_t szs_feature_list[] = { DEF_VAR (1, HAVESZS_COURSE_LEX, f_lex,
													  "File course.lex"),
...
static const feature_var_t kmp_feature_list[] = { DEF_VAR (0, HAVEKMP_GOOMBA_SIZE, kmp_goomba_size,
													  "KMP: Goomba with scale != 1.00"),
...
static const feature_var_t lex_section_list[]
	= { DEF_VAR (1, HAVELEXS_FEAT, lex_sect_feat, "LEX section FEAT"),
		  ...
		  DEF_VAR (0, HAVELEXS_DEV1, lex_sect_dev1, "LEX section DEV1"),
		  DEF_VAR (0, HAVELEXS_SET1, lex_sect_set1, "LEX section SET1"),
		  DEF_VAR (0, HAVELEXS_CANN, lex_sect_cann, "LEX section CANN"),
		  DEF_VAR (0, HAVELEXS_CTDN, lex_sect_ctdn, "LEX section CTDN"),
...
static const feature_var_t lex_feature_list[] = { DEF_VAR (0, HAVELEXF_TEST_ACTIVE, lex_test_active,
													  "LEX: Active test scenario"),
	...
	DEF_VAR (
		0, HAVELEXF_START_ITEM, lex_start_item, "LEX: Player can get item before start of race."),
	DEF_VAR (
		0, HAVELEXF_APPLY_OTL, lex_apply_otl, "LEX: Track applies alternative online time limit."),
...
static const feature_list_t feature_lists[] = {
	{ szs_feature_list, FVT_BYTE_ARRAY, offsetof (szs_have_t, szs), FZV_MAYBE_IMPACT, "SZS file",
		"Additional sub-files found in SZS file", "0:no file, 1:unmodified file, 2:modified file" },

	{ kmp_feature_list, FVT_BOOL_ARRAY, offsetof (szs_have_t, kmp), FZV_IMPACT, "KMP feature",
		"Special features found in KMP (sub-file course.kmp)",
		"0:not found, 1:found without impact, 3:found with impact" },

	{ lex_section_list, FVT_UINT_BITS, offsetof (szs_have_t, lex_sect), FZV_MAYBE_IMPACT,
		"LEX section", "Sections found in LEX (sub-file course.lex)",
		"0:section not found, 1:section found, 2:found and relevant" },

	{ lex_feature_list, FVT_UINT_BITS, offsetof (szs_have_t, lex_feat), FZV_IMPACT, "LEX feature",
	...
	for (list = feature_lists; list->var; list++)
	{
		switch (list->var_type)
		{
			case FVT_BOOL_ARRAY:
			{
				...
				for (var = list->var; var->offset; var++)
					((u8 *)fs)[var->offset] = is_lex && var->no_lex ? list->max_value
						: src[var->index] > 0						? list->max_value
																	: 0;
			}
			break;
			...
			case FVT_BYTE_ARRAY:
			{
				...
				for (var = list->var; var->offset; var++)
					((u8 *)fs)[var->offset]
						= is_lex && var->no_lex ? list->max_value : src[var->index];
			}
			break;
			...
			case FVT_UINT_BITS:
			{
				...
				for (var = list->var; var->offset; var++)
					((u8 *)fs)[var->offset] = is_lex && var->no_lex ? list->max_value
						: (val & 1 << var->index) != 0				? list->max_value
																	: 0;
			}
			break;
...
void PrintFeaturesSZS (PrintScript_t *ps, // valid output definition
			...
			if (value || comments >= -2)
			{
				...
				if (comments >= 0)
				{
					...
					if (comments >= 2)
						PrintScriptVars (ps, 0, "# %s #%u, feature offset %u, source index %u\n",
							list->ref_prefix, (int)(var - list->var), var->offset, var->index);
					PrintScriptVars (ps, 0, "# %s\n", var->comment);
				...
				if (print_modes > 0)
					PrintScriptVars (ps, 0, "%s=\"%s,%s\"\n", var->name, valbuf,
						GetFeaturesMode (mode, print_modes > 1));
				else
...
ccp GetFeaturesEffects (features_szs_mode_t mode)
{
	...
	static const struct tab_t *ptr,
		tab[] = { { 0, FZM_SECTION, "section" },

			{ 1, FZM_M_KIND, "no impact" }, { 0, FZM_M_KIND, "visual and gameplay impact" },
			{ 0, FZM_VISUAL, "visual impact only" }, { 0, FZM_GAMEPLAY, "gameplay impact" },

			{ 0, FZM_M_TYPE, 0 }, { 0, FZM_BATTLE, "battle only" },
			{ 0, FZM_RACING, "racing only" },

			{ 0, FZM_M_WHERE, 0 }, { 0, FZM_OFFLINE | FZM_ONLINE, "except time trial" },
			...
			{ 0, FZM_OFFLINE, "offline except time trial" }, { 0, FZM_ONLINE, "online only" },
...
enumError ScanLexFile (cvp data, uint data_size, lex_stream_func func, cvp user_ptr, int user_int)
{
...
static void FixLEXElement_DEV1 (lex_dev1_t *dev1)
{
	DASSERT (dev1);
	// [[new-lex-dev1]]
}
...
static void FixLEXItem_DEV1 (lex_t *lex, // LEX data structure
	lex_item_t *item // item to fix
)
{
	DASSERT (lex);
	DASSERT (item);
	DASSERT (ntohl (item->elem.size) >= sizeof (lex_dev1_t));

	lex_dev1_t *dev1 = (lex_dev1_t *)item->elem.data;
	FixLEXElement_DEV1 (dev1);
}

//-----------------------------------------------------------------------------

static void ClearLEXElement_DEV1 (lex_dev1_t *dev1)
{
	DASSERT (dev1);
...
static bool IsActiveLEXElement_DEV1 (const lex_dev1_t *dev1)
{
	DASSERT (dev1);
	lex_dev1_t empty, test = *dev1;
	ClearLEXElement_DEV1 (&empty);
	FixLEXElement_DEV1 (&test);
...
static void FixLEXElement_SET1 (lex_set1_t *set1)
{
	...
	set1->apply_online_sec
		= htons (MINMAX0 (ntohs (set1->apply_online_sec), LE_MIN_ONLINE_SEC, LE_MAX_ONLINE_SEC));
...
static void FixLEXItem_SET1 (lex_t *lex, // LEX data structure
	lex_item_t *item // item to fix
)
{
	DASSERT (lex);
...
static void FixLEXElement_CTDN (lex_ctdn_t *ctdn)
{
	DASSERT (ctdn);

	u16 tlimit[LEC__N];
	be16n (tlimit, ctdn->time_limit_sec, LEC__N);

	if (!tlimit[LEC_150CC])
		tlimit[LEC_150CC] = 150;
	for (int i = LEC_150CC - 1; i >= 0; i--)
		if (!tlimit[i])
			tlimit[i] = tlimit[i + 1];
	for (int i = LEC_150CC + 1; i < LEC__N; i++)
		if (!tlimit[i])
			tlimit[i] = tlimit[i - 1];

	write_be16n (ctdn->time_limit_sec, tlimit, LEC__N);
	// [[new-lex-ctdn]]
}

//-----------------------------------------------------------------------------

static void FixLEXItem_CTDN (lex_t *lex, // LEX data structure
	lex_item_t *item // item to fix
)
{
	DASSERT (lex);
	DASSERT (item);
	DASSERT (ntohl (item->elem.size) >= sizeof (lex_ctdn_t));

	lex_ctdn_t *ctdn = (lex_ctdn_t *)item->elem.data;
	FixLEXElement_CTDN (ctdn);
}

//-----------------------------------------------------------------------------

static void ClearLEXElement_CTDN (lex_ctdn_t *ctdn)
{
	DASSERT (ctdn);
	memset (ctdn, 0, sizeof (*ctdn));
	FixLEXElement_CTDN (ctdn);
}

//-----------------------------------------------------------------------------

static bool IsActiveLEXElement_CTDN (const lex_ctdn_t *ctdn)
{
	DASSERT (ctdn);
	lex_ctdn_t empty, test = *ctdn;
	ClearLEXElement_CTDN (&empty);
	FixLEXElement_CTDN (&test);
	return memcmp (&empty, &test, sizeof (empty)) != 0;
}
...
static void FixLEXItem_TEST (lex_t *lex, // LEX data structure
	lex_item_t *item // item to fix
)
{
	DASSERT (lex);
...
static lex_item_t *FindElementLEX (const lex_t *lex, // LEX data structure
	u32 magic // magic
)
{
	magic = htonl (magic);
...
static void UpdateHaveLex (lex_t *lex)
{
	...
	for (i = 0, ss = lex->item; i < lex->item_used; i++, ss++)
	{
		...
		for (j = 0; j < HAVELEXS__N; j++)
			if (magic == have_lex_sect_info[j].magic && size >= have_lex_sect_info[j].min_size)
			{
				have_sect |= 1 << j;
...
static lex_item_t *AppendElementLEX (lex_t *lex, // LEX data structure
	u32 magic, // magic
...
)
{
	DASSERT (lex);
	DASSERT (data || !size);
	PRINT ("AppendElementLEX(%.4s) have=%02x,%02x, n=%u\n", (ccp)&magic, lex->have_sect,
		lex->have_feat, lex->item_used);
	...
	switch (magic)
	{
		...
		case LEXS_DEV1:
			fix_func = FixLEXItem_DEV1;
			if (size4 < sizeof (lex_dev1_t))
				size4 = ALIGN32 (sizeof (lex_dev1_t), 4);
			break;
		...
		case LEXS_CTDN:
			fix_func = FixLEXItem_CTDN;
			if (size4 < sizeof (lex_ctdn_t))
				size4 = ALIGN32 (sizeof (lex_ctdn_t), 4);
			break;
...
static bool RemoveElementLEX (lex_t *lex, // LEX data structure
...
static uint RemoveElementByMagicLEX (lex_t *lex, // LEX data structure
	u32 magic // magic
)
{
	if (!magic)
...
static int cmp_lex_item (const lex_item_t **pa, const lex_item_t **pb)
{
	...
	noPRINT (
		">> SORT: %u.%u : %u.%u\n", a->sort_order, a->insert_order, b->sort_order, b->insert_order);
...
static void SortItemsLEX (lex_t *lex // LEX data structure
)
{
	if (lex->item_used > 1)
		qsort (lex->item, lex->item_used, sizeof (*lex->item), (qsort_func)cmp_lex_item);
}
...
static uint GetLexFeaturesSET1 (const lex_element_t *elem, lex_t *lex)
{
	...
	{
		if (bef4 (set1->item_factor.v + 0) != 1.0 || bef4 (set1->item_factor.v + 1) != 1.0
...
{
	return ntohl (elem->size) >= sizeof (lex_hipt_rule_t) ? 1 << HAVELEXF_HIDE_POS : 0;
}
...
{
	return ntohl (elem->size) >= sizeof (lex_ritp_rule_t) ? 1 << HAVELEXF_RND_ITPH : 0;
}
...
{
	DASSERT (elem);
	return IsActiveLEXElement_TEST ((lex_test_t *)elem->data) ? 1 << HAVELEXF_TEST_ACTIVE : 0;
}
...
uint GetLexFeatures (const lex_element_t *elem, lex_t *lex)
{
	...
	{
		// case LEXS_DEV1: return GetLexFeaturesDEV1(elem,lex); // will never have features
		case LEXS_SET1:
			return GetLexFeaturesSET1 (elem, lex);
		case LEXS_CANN:
			return GetLexFeaturesCANN (elem);
			// X	case LEXS_CTDN: return GetLexFeaturesCTDN(elem);
		case LEXS_HIPT:
			return GetLexFeaturesHIPT (elem);
		case LEXS_RITP:
			return GetLexFeaturesRITP (elem);
		case LEXS_TEST:
			return GetLexFeaturesTEST (elem);
			// [[new-lex-sect]]
	}
	return 0;
}
...
ccp CreateFeatureInfoLEX (have_lex_feat_t special, bool add_value, ccp return_if_empty)
{
	static char buf[500];
	char *dest = buf;
...
const have_lex_info_t have_lex_sect_info[HAVELEXS__N] = {
	...
	{ LEXS_DEV1, "dev1", 1 }, // HAVELEXS_DEV1
	{ LEXS_CTDN, "countdown", 0 }, // HAVELEXS_CTDN
...
ccp CreateSectionInfoLEX (have_lex_sect_t special, bool add_value, ccp return_if_empty)
{
	static char buf[500];
	char *dest = buf;
...
{
	lex_dev1_t dev1 = { 0 };
	// AppendElementLEX() does call FixLEXItem_DEV1()
	return AppendElementLEX (lex, LEXS_DEV1, &dev1, sizeof (dev1), overwrite);
}
...
{
	return AppendElementLEX (lex, LEXS_CANN, CannonDataLEX, sizeof (CannonDataLEX), overwrite);
}
...
{
	lex_ctdn_t ctdn = { 0 };
	// AppendElementLEX() does call FixLEXItem_CTDN()
	return AppendElementLEX (lex, LEXS_CTDN, &ctdn, sizeof (ctdn), overwrite);
}
...
{
	return AppendElementLEX (lex, LEXS_TEST, TestDataLEX, sizeof (TestDataLEX), overwrite);
}
...
void DumpElementsLEX (FILE *f, lex_t *lex, bool hexdump)
{
	...
	for (i = 0; i < lex->item_used; i++)
	{
		...
		fprintf (f, "   %-4.4s %08x, size: %5u = 0x%04x\n", PrintID (&s->elem.magic, 4, 0),
			s->elem.magic, size, size);
		if (size && hexdump)
...
)
{
	...
	if (si && si->lex)
	{
		...
		if (si->lex->fname)
		{
			...
			if (!strcasecmp (fname, "course.lex") || !strcasecmp (fname, "course.txt")
...
enumError DumpLEXFunc (u32 magic, const u8 *data, uint size, cvp user_ptr, int user_int)
{
	u32 m = htonl (magic);
...
enumError ScanLEXFunc (u32 magic, const u8 *data, uint size, cvp user_ptr, int user_int)
{
#if HAVE_PRINT
	u32 m = htonl (magic);
...
enumError ScanRawLEX (lex_t *lex, // LEX data structure
...
)
{
	...
	lex->fform = FF_LEX;
	enumError err
		= ScanLexFile (data, data_size, dump_mode ? DumpLEXFunc : ScanLEXFunc, lex, dump_mode);
	if (err)
...
static const KeywordTab_t lex_section_name[] = {
	...
	{ LID_IGNORE, "END", "--", 0 }, { LID_SETUP, "SETUP", "Setup", 0 },
	{ LID_HEXDUMP, "HEXDUMP", "Hexdump of unknown section", 0 },

	{ LID_DEVELOP1, "DEV1", "Developer settings", 0 },
	{ LID_SETTINGS1, "SET1", "Primary settings", 0 }, { LID_CANNON, "CANN", "Cannon settings", 0 },
	{ LID_COUNTDOWN, "CTDN", "Countdown settings", 0 },
...
static const KeywordTab_t lex_offline_online[]
	= { { LEX_OO_AUTO, "AUTO", "0", 0 }, { LEX_OO_OFFLINE, "OFFLINE", "1", 0 },
		  { LEX_OO_OFFLINE, "NEVER", 0, 0 }, { LEX_OO_ONLINE, "ONLINE", "2", 0 },
		  { LEX_OO_ONLINE, "ALWAYS", 0, 0 }, { -9, "OFF", "-1", -1 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static const KeywordTab_t lex_game_mode[] = { { LEX_GMODE_AUTO, "AUTO", "0", -1 },
	{ LEX_GMODE_STD, "STANDARD", "1", KMP_GMODE_STD }, { LEX_GMODE_STD, "STD", 0, KMP_GMODE_STD },
	...
	{ LEX_GMODE_ITEMRAIN, "ITEMRAIN", "5", KMP_GMODE_ITEMRAIN }, { -1, "OFF", "-1", -1 },
	{ 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static const KeywordTab_t lex_engine[]
	= { { LEX_ENGINE_AUTO, "AUTO", "0", -1 }, { LEX_ENGINE_BATTLE, "BATTLE", 0, ENGINE_BATTLE },
		  { LEX_ENGINE_50, "50CC", 0, ENGINE_50 }, { LEX_ENGINE_100, "100CC", 0, ENGINE_100 },
		  { LEX_ENGINE_150, "150CC", 0, ENGINE_150 }, { LEX_ENGINE_200, "200CC", 0, ENGINE_200 },
		  { LEX_ENGINE_150M, "150M", "150MIRROR", ENGINE_150M },
		  { LEX_ENGINE_200M, "200M", "200MIRROR", ENGINE_200M }, { -1, "OFF", "-1", -1 },
		  { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

static enumError ScanLEXElementHexdump (lex_t *lex, // KMP data structure
...
static enumError ScanLEXElement_DEV1 (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
	DASSERT (si);
	TRACE ("ScanLEXElement_DEV1()\n");

	//--- setup data

	lex_dev1_t dev1 = { 0 };
	const ScanParam_t ptab[]
		= { { "ENABLED", SPM_U8, &dev1.enabled }, { "MODE1", SPM_U8, &dev1.mode1 },
			  { "MODE2", SPM_U8, &dev1.mode2 }, { "MODE3", SPM_U8, &dev1.mode3 },
			  // [[new-lex-dev1]]
			  { 0 } };

	char fb_buf[1000];
	FastBuf_t *fb = InitializeFastBuf (fb_buf, sizeof (fb_buf));
	AppendFastBuf (fb, &dev1, sizeof (dev1));
	...
	memcpy (fb->buf, &dev1, sizeof (dev1));
	HEXDUMP16 (0, 0, fb->buf, fb->ptr - fb->buf);
	AppendElementLEX (lex, LEXS_DEV1, fb->buf, fb->ptr - fb->buf, true);
...
static enumError ScanLEXElement_SET1 (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
	...
	const ScanParam_t ptab[] = { { "ITEM-POS-FACTOR", SPM_FLOAT3_BE, &set1.item_factor },
		{ "START-ITEM", SPM_U8, &set1.start_item }, { "PADDING-0D", SPM_U8, &set1.padding_0d },
...
static enumError ScanLEXElement_CANN (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
...
static enumError ScanLEXElement_CTDN (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
	DASSERT (si);
	TRACE ("ScanLEXElement_CTDN()\n");

	//--- setup data

	lex_ctdn_t ctdn = { 0 };

	ScanParam_t ptab[] = { { "TIME-LIMIT-BATTLE", SPM_U16_BE, ctdn.time_limit_sec + LEC_BATTLE },
		{ "TIME-LIMIT-50CC", SPM_U16_BE, ctdn.time_limit_sec + LEC_50CC },
		{ "TIME-LIMIT-100CC", SPM_U16_BE, ctdn.time_limit_sec + LEC_100CC },
		{ "TIME-LIMIT-150CC", SPM_U16_BE, ctdn.time_limit_sec + LEC_150CC },
		{ "TIME-LIMIT-200CC", SPM_U16_BE, ctdn.time_limit_sec + LEC_200CC },
		{ "TIME-LIMIT-300CC", SPM_U16_BE, ctdn.time_limit_sec + LEC_300CC }, { 0 } };

	char fb_buf[1000];
	FastBuf_t *fb = InitializeFastBuf (fb_buf, sizeof (fb_buf));
	AppendFastBuf (fb, &ctdn, sizeof (ctdn));
	...
	FixLEXElement_CTDN (&ctdn);
	memcpy (fb->buf, &ctdn, sizeof (ctdn));
	// HexDump16(stderr,0,0,fb->buf,fb->ptr-fb->buf);
	AppendElementLEX (lex, LEXS_CTDN, fb->buf, fb->ptr - fb->buf, true);
...
static enumError ScanLEXElement_HIPT (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
...
static enumError ScanLEXElement_RITP (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
...
static enumError ScanLEXElement_TEST (lex_t *lex, // KMP data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (lex);
	...
	const ScanParam_t ptab[] = { { "OFFLINE-ONLINE", SPM_U8, &test.offline_online },
		{ "N-OFFLINE", SPM_U8, &test.n_offline }, { "N-ONLINE", SPM_U8, &test.n_online },
		{ "COND-BIT", SPM_S8, &test.cond_bit }, { "GAME-MODE", SPM_U8, &test.game_mode },
		{ "ENGINE", SPM_U8, &test.engine }, { "RANDOM", SPM_U8, &test.random }, { 0 } };
...
enumError ScanTextLEX (lex_t *lex, // LEX data structure
	bool init_lex, // true: initialize 'lex' first
	const void *data, // data to scan
	uint data_size // size of 'data'
)
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			switch (cmd->id)
			{
				...
				case LID_DEVELOP1:
					err = ScanLEXElement_DEV1 (lex, &si);
					break;
				...
				case LID_COUNTDOWN:
					err = ScanLEXElement_CTDN (lex, &si);
					break;
...
enumError ScanLEX (lex_t *lex, // LEX data structure
	bool init_lex, // true: initialize 'lex' first
	const void *data, // data to scan
	uint data_size // size of 'data'
)
{
	...
	switch (GetByMagicFF (data, data_size, data_size))
	{
		...
		default:
			if (init_lex)
				InitializeLEX (lex);
			return ERROR0 (ERR_INVALID_DATA, "No LEX file: %s\n", lex->fname ? lex->fname : "?");
	}
...
enumError ScanRawDataLEX (lex_t *lex, // LEX data structure
	bool init_lex, // true: initialize 'lex' first
...
enumError LoadLEX (lex_t *lex, // LEX data structure
	bool init_lex, // true: initialize 'lex' first
...
enumError CreateRawLEX (lex_t *lex // pointer to valid LEX
)
{
	DASSERT (lex);
	if (lex->raw_data)
...
enumError SaveRawLEX (lex_t *lex, // pointer to valid LEX
...
static enumError SaveTextLEX_FEAT (FILE *f, // file to write
...
static enumError SaveTextLEX_DEV1 (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
	...
	const uint size = ntohl (item->elem.size);
	noPRINT ("SaveTextLEX_DEV1(), size=%u\n", size);
	DASSERT (size >= sizeof (lex_dev1_t));
	if (size < sizeof (lex_dev1_t))
		return ERR_OK;

	const u8 *data = item->elem.data;
	const lex_dev1_t *dev1 = (lex_dev1_t *)data;
	...
	if (brief_count || export_count)
		fprintf (f,
			...
			"@MODE3   = %u\r\n"
			"\r\n",
			section_sep, dev1->enabled, dev1->mode1, dev1->mode2, dev1->mode3
			// [[new-lex-dev1]]
		);
	else
		fprintf (f, text_lex_elem_dev1_cr, item->sort_order, dev1->enabled, dev1->mode1,
			dev1->mode2, dev1->mode3
			// [[new-lex-dev1]]
		);
	...
	{
		fputs ("\r\n# Hex dump for unknown settings:\r\n", f);
		HexDump (f, 0, sizeof (lex_dev1_t), 5, 16, item->elem.data + sizeof (lex_dev1_t),
			size - sizeof (lex_dev1_t));
	}
...
static enumError SaveTextLEX_SET1 (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
	...
	if (brief_count || export_count)
		fprintf (f,
			...
			"\r\n",
			section_sep, bef4 (&set1->item_factor.x), bef4 (&set1->item_factor.y),
			bef4 (&set1->item_factor.z), set1->start_item, apply_online_sec,
			PrintHMS (0, 0, apply_online_sec, LE_VIEW_ONLINE_HMS, "  # ", 0)
			// [[new-lex-set1]]
		);
	else
		fprintf (f, text_lex_elem_set1_cr, item->sort_order, bef4 (&set1->item_factor.x),
			bef4 (&set1->item_factor.y), bef4 (&set1->item_factor.z), set1->start_item,
	...
	if (set1->padding_0d)
		fprintf (f, text_lex_elem_set1_develop_cr, set1->padding_0d);

	if (size > sizeof (lex_set1_t))
	{
		fputs ("\r\n# Hex dump for unknown settings:\r\n", f);
		HexDump (f, 0, sizeof (lex_set1_t), 5, 16, item->elem.data + sizeof (lex_set1_t),
...
static enumError SaveTextLEX_CTDN (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
	...
	const uint size = ntohl (item->elem.size);
	noPRINT ("SaveTextLEX_CTDN(), size=%u\n", size);
	DASSERT (size >= sizeof (lex_ctdn_t));
	if (size < sizeof (lex_ctdn_t))
		return ERR_OK;

	const u8 *data = item->elem.data;
	const lex_ctdn_t *ctdn = (lex_ctdn_t *)data;

	const uint cc050 = be16 (ctdn->time_limit_sec + LEC_50CC);
	const uint cc100 = be16 (ctdn->time_limit_sec + LEC_100CC);
	const uint cc150 = be16 (ctdn->time_limit_sec + LEC_150CC);
	const uint cc200 = be16 (ctdn->time_limit_sec + LEC_200CC);
	const uint cc300 = be16 (ctdn->time_limit_sec + LEC_300CC);
	...
	if (brief_count || export_count)
		fprintf (f,
			...
			"@TIME-LIMIT-300CC = %3u\r\n"
			"\r\n",
			section_sep, cc050, cc100, cc150, cc200, cc300
			// [[new-lex-ctdn]]
		);
	else
		fprintf (f, text_lex_elem_ctdn_cr, item->sort_order, cc050,
			PrintHMS (0, 0, cc050, 1, " # ", ""), cc100, PrintHMS (0, 0, cc100, 1, " # ", ""),
			cc150, PrintHMS (0, 0, cc150, 1, " # ", ""), cc200,
			PrintHMS (0, 0, cc200, 1, " # ", ""), cc300, PrintHMS (0, 0, cc300, 1, " # ", "")
			// [[new-lex-ctdn]]
		);
	...
	{
		fputs ("\r\n# Hex dump for unknown settings:\r\n", f);
		HexDump (f, 0, sizeof (lex_ctdn_t), 5, 16, item->elem.data + sizeof (lex_ctdn_t),
			size - sizeof (lex_ctdn_t));
	}
...
static enumError SaveTextLEX_CANN (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
	...
	for (i = 0; i < n; i++, off += 16)
		fprintf (f, "%4u %10.3f %10.3f %10.3f %10.3f\r\n", i, bef4 (data + off),
			bef4 (data + off + 4), bef4 (data + off + 8), bef4 (data + off + 12));
...
static enumError SaveTextLEX_HIPT (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
...
static enumError SaveTextLEX_RITP (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
...
///////////////////////////////////////////////////////////////////////////////
// [[new-lex-sect]] save function
///////////////////////////////////////////////////////////////////////////////

static enumError SaveTextLEX_TEST (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
	...
	fprintf (f, text_lex_elem_test_cr, item->sort_order, off_on->name1 ? off_on->name1 : "AUTO",
		test->n_offline, test->n_online, test->cond_bit, gmode->name1 ? gmode->name1 : "AUTO",
		engine->name1 ? engine->name1 : "AUTO", test->random);

	if (size > sizeof (lex_test_t))
	{
		fputs ("\r\n# Hex dump for unknown settings:\r\n", f);
		HexDump (f, 0, sizeof (lex_test_t), 5, 16, item->elem.data + sizeof (lex_test_t),
...
static enumError SaveTextLEX_HEXDUMP (FILE *f, // file to write
	const lex_t *lex, // pointer to valid LEX
	lex_item_t *item // item to save
)
{
	...
	if (brief)
		...
		fprintf (
			f, text_lex_hexdump_cr, item->sort_order, magic, PrintID (&item->elem.magic, 4, 0));
...
enumError SaveTextLEX (const lex_t *lex, // pointer to valid LEX
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
	...
	{
		fprintf (F.f, text_lex_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);
	...
	for (i = 0; i < lex->item_used; i++)
	{
		...
		{
			case LEXS_FEAT:
				err = SaveTextLEX_FEAT (F.f, lex, s);
				break;
			case LEXS_DEV1:
				err = SaveTextLEX_DEV1 (F.f, lex, s);
				break;
			case LEXS_SET1:
			...
			case LEXS_CANN:
				...
				break;
			case LEXS_CTDN:
				err = SaveTextLEX_CTDN (F.f, lex, s);
				break;
			case LEXS_HIPT:
				err = SaveTextLEX_HIPT (F.f, lex, s);
				break;
			case LEXS_RITP:
				err = SaveTextLEX_RITP (F.f, lex, s);
				break;
			case LEXS_TEST:
			...
			default:
				err = SaveTextLEX_HEXDUMP (F.f, lex, s);
				break;
		}
...
int ScanOptLtOnline (ccp arg)
{
	...
	else
	{
		...
		{
			PrintKeywordError (
				lex_offline_online, arg, abbrev, 0, "keyword for option --lt-online");
			return 1;
		}
...
int ScanOptLtGameMode (ccp arg)
{
	...
	else
	{
		...
		if (!key)
		{
			PrintKeywordError (lex_game_mode, arg, abbrev, 0, "keyword for option --lt-game-mode");
			return 1;
		}
...
int ScanOptLtEngine (ccp arg)
{
	...
	else
	{
		...
		if (!key)
		{
			PrintKeywordError (lex_engine, arg, abbrev, 0, "keyword for option --lt-engine");
			return 1;
		}
...
{
	return force_lex_test || opt_lt_clear || opt_lt_online >= 0 || opt_lt_n_offline >= 0
		|| opt_lt_n_online >= 0 || opt_lt_game_mode >= 0 || opt_lt_engine >= 0
		|| opt_lt_random >= 0;
}
...
{
	return force_lex_test || opt_lt_online > 0 || opt_lt_n_offline > 0 || opt_lt_n_online > 0
		|| opt_lt_game_mode > LEX_GMODE_AUTO || opt_lt_engine > LEX_ENGINE_AUTO
		|| opt_lt_random >= 0;
}
...
bool PatchTestLEX (lex_test_t *lt, bool *empty)
{
	...
	PRINT0 ("--lt-*: clear=%d, online=%d, n=%d,%d, gm=%d, en=%d, rnd=%d\n", opt_lt_clear,
		opt_lt_online, opt_lt_n_offline, opt_lt_n_online, opt_lt_game_mode, opt_lt_engine,
		opt_lt_random);
...
bool PatchFeaturesLEX (lex_t *lex, const szs_have_t *have, bool *empty)
{
	...
	PRINT ("--lex-features = %d, --lex-rm-features = %d : have=%d, need=%d\n", opt_lex_features,
		opt_lex_rm_features, have_features, need_features);
	...
	else if (opt_lex_features && need_features
...
bool PatchLEX (lex_t *lex, const szs_have_t *have)
{
	...
	if (HavePatchTestLEX ())
	{
		...
		PRINT ("PATCH+ LEX/TEST: had=%d, empty=%d, modified=%d => have_lex:%x,%x\n", had_test,
			empty, modified, lex->have_sect, lex->have_feat);
	...
	if (opt_lt_clear)
	{
		lex_item_t *li = FindElementLEX (lex, LEXS_DEV1);
		if (li)
		{
			RemoveElementLEX (lex, li);
			modified = true;
		}
	}
...
bool PurgeLEX (lex_t *lex)
{
	...
	while (i < lex->item_used)
	{
		...
		switch (ntohl ((*ss)->elem.magic))
		{
			...
			case LEXS_DEV1:
				remove
					= size == sizeof (lex_dev1_t) && !IsActiveLEXElement_DEV1 ((lex_dev1_t *)data);
				break;

			case LEXS_SET1:
				remove
					= size == sizeof (lex_set1_t) && !IsActiveLEXElement_SET1 ((lex_set1_t *)data);
				break;

			case LEXS_CANN:
				remove = size < sizeof (CannonDataLEX)
					|| size == sizeof (CannonDataLEX)
						&& !memcmp (data, &CannonDataLEX, sizeof (CannonDataLEX));
				break;

			case LEXS_CTDN:
				remove
					= size == sizeof (lex_ctdn_t) && !IsActiveLEXElement_CTDN ((lex_ctdn_t *)data);
				break;
			...
			case LEXS_TEST:
				remove
					= size == sizeof (lex_test_t) && !IsActiveLEXElement_TEST ((lex_test_t *)data);
				break;
