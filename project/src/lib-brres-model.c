...
static int set_node_weights
    ( node_transform_t *node, const uint16_t *bones, const float *weights,
      size_t count )
{
    uint16_t *b = malloc(count*sizeof(*b));
    float *w = malloc(count*sizeof(*w));
    if (!b || !w) { free(b); free(w); return 0; }
    memcpy(b,bones,count*sizeof(*b));
    memcpy(w,weights,count*sizeof(*w));
    free(node->bones); free(node->weights);
    node->bones = b;
    node->weights = w;
...
static int ensure_node_transform
    ( node_transform_t **table, size_t *count, unsigned node_id )
{
    ...
    if (node_id > 0xffff) return 0;
    if (node_id < *count) return 1;
    size_t next = *count ? *count : 16;
    while (next <= node_id) next *= 2;
...
static void multiply_affine43 ( float out[12], const float a[12], const float b[12] )
{
    ...
    for (unsigned r = 0; r < 3; r++)
    {
        for (unsigned c = 0; c < 3; c++)
            out[r*4+c] = a[r*4+0]*b[c+0]
                         + a[r*4+1]*b[c+4]
                         + a[r*4+2]*b[c+8];
        out[r*4+3] = a[r*4+0]*b[3]
                     + a[r*4+1]*b[7]
...
static int append_triangle
    ( mesh_t *mesh, size_t *capacity, vertex_t a, vertex_t b, vertex_t c )
{
    if (mesh->num_vertices + 3 > *capacity)
    {
        size_t next = *capacity ? *capacity * 2 : 1024;
        while (next < mesh->num_vertices + 3) next *= 2;
...
static int decode_gx_primitives
    ( const uint8_t *prim_data, int32_t prim_size, uint32_t cp_lo,
      uint32_t cp_hi, mesh_t *mesh )
{
    ...
    for (unsigned i = 0; i < 8; i++) stride += cp_lo >> (i+1) & 1;
    stride += attr_size(pos_fmt) + attr_size(nrm_fmt);
    stride += attr_size(cp_lo >> 13 & 3) + attr_size(cp_lo >> 15 & 3);
    for (unsigned i = 0; i < 8; i++) stride += attr_size(cp_hi >> (i*2) & 3);
    if (!stride) return 0;

    uint16_t matrix_nodes[16];
    for (unsigned i = 0; i < 16; i++) matrix_nodes[i] = 0xffff;
    ...
    while (off < (size_t)prim_size)
    {
        ...
        {
            if (off + 4 > (size_t)prim_size) return 0;
            if (command == 0x20)
            {
                const unsigned slot = (read_be16(prim_data+off+2) & 0xfff) / 12;
                if (slot < 16) matrix_nodes[slot] = read_be16(prim_data+off);
            }
            off += 4;
            continue;
        }
        ...
        if (primitive != 0x80 && primitive != 0x90 && primitive != 0x98
            && primitive != 0xa0 && primitive != 0xa8
            && primitive != 0xb0 && primitive != 0xb8)
            return 0;
        if (off + 2 > (size_t)prim_size) return 0;
        ...
        for (unsigned p = 0; p < count; p++)
        {
            ...
            for (unsigned i = 0; i < 8; i++) cursor += cp_lo >> (i+1) & 1;
            ...
            points[p].normal_idx = ns ? ns == 1 ? prim_data[cursor] : read_be16(prim_data+cursor) : 0;
            cursor += ns;
            for (unsigned c = 0; c < 2; c++)
            {
                const unsigned cs = attr_size(cp_lo >> (13+c*2) & 3);
                points[p].color_idx[c] = cs
                    ? cs == 1 ? prim_data[cursor] : read_be16(prim_data+cursor) : 0;
                cursor += cs;
            }
            for (unsigned t = 0; t < 8; t++)
            {
                ...
                const int index = ts
                    ? ts == 1 ? prim_data[cursor] : read_be16(prim_data+cursor) : 0;
                if (!t) points[p].texcoord_idx = index;
                else points[p].extra_texcoord_idx[t-1] = index;
                cursor += ts;
            }
        }
        ...
        if (primitive == 0x90)
            ...
            for (unsigned p = 0; ok && p+3 < count; p += 4)
            {
                ...
                if (ok) ok = append_triangle(mesh,&capacity,points[p],points[p+3],points[p+2]);
            }
...
enum mdl_group_type
{
    MDL_DEFINITIONS, MDL_BONES, MDL_POSITIONS, MDL_NORMALS, MDL_COLORS, MDL_UVS,
    MDL_MATERIALS, MDL_OBJECTS, MDL_TEXTURES
};
    ...
    ( const uint8_t *data, size_t size, uint32_t version, enum mdl_group_type type )
{
    ...
    switch (type)
    {
        case MDL_DEFINITIONS:index = 0; break;
        case MDL_BONES:     index = 1; break;
        case MDL_POSITIONS: index = 2; break;
        case MDL_NORMALS:   index = 3; break;
        case MDL_COLORS:    index = 4; break;
        case MDL_UVS:       index = 5; break;
        case MDL_MATERIALS: index = version >= 10 ? 8 : 6; break;
        case MDL_OBJECTS:   index = version >= 10 ? 10 : 8; break;
        case MDL_TEXTURES:  index = version >= 10 ? 11 : 9; break;
        default: return NULL;
    }
    const size_t field = 0x10 + index*4;
    if (field+4 > size) return NULL;
...
static float read_component ( const uint8_t *p, uint32_t type, uint8_t divisor )
{
    ...
    switch (type)
    {
        ...
        case 2: return read_be16(p) * scale;
        case 3: return (int16_t)read_be16(p) * scale;
        case 4:
        {
      ...
      size_t header_size, vec3_t **dest, size_t *count )
{
    ...
    if (data_offset <= 0 || !num || !elem || stride < elem*components
        || !in_bounds(data,size,src,(size_t)stride*num)) return 0;
      ...
      color4_t **dest, size_t *count )
{
    ...
    if (format >= sizeof(format_size) || data_offset <= 0 || !num
        || stride < format_size[format]) return 0;
    const uint8_t *src = node + data_offset;
    if (!in_bounds(data,size,src,(size_t)stride*num)) return 0;
    ...
    for (unsigned i = 0; i < num; i++)
    {
        ...
        switch (format)
        {
            ...
            case 1: r=p[0]; g=p[1]; b=p[2]; break; // RGB8
            case 2: r=p[0]; g=p[1]; b=p[2]; break; // RGBX8
            case 3: // RGBA4
            {
                const unsigned v = read_be16(p);
                r = (v >> 12 & 15) * 17;
            ...
            case 5: r=p[0]; g=p[1]; b=p[2]; a=p[3]; break; // RGBA8
        }
...
static void read_material_textures(const uint8_t *data, size_t size,
                                   const uint8_t *mat_base, int32_t mat_len,
                                   uint32_t version, material_t *mat) {
    ...
    if (refs_offset <= 0 || count > max_layers) return;
    const uint8_t *ref = mat_base + refs_offset;
    if (!in_bounds(data,size,ref,(size_t)count*0x34)) return;
    ...
    if (display_field+4 > (size_t)mat_len) return;
    ...
    if (display_offset <= 0 || !in_bounds(data,size,xf,1)) return;
    unsigned command = 0;
    while (command < (unsigned)mat->num_textures*2 && in_bounds(data,size,xf,5) && xf[0] == 0x10)
    {
        const unsigned values = read_be16(xf+1)+1;
        const size_t command_size = 5 + (size_t)values*4;
        if (!in_bounds(data,size,xf,command_size)) break;
        if (!(command & 1) && values)
        {
            const uint32_t info = swap32(*(const uint32_t*)(xf+5));
            const unsigned source_row = info >> 7 & 31;
            mat->texture_coord[command/2] = source_row >= 5 && source_row <= 12
                ? (int)source_row-5 : 0;
        }
        command++;
        xf += command_size;
...
static void read_linked_material_textures
    ( const uint8_t *data, size_t size, uint32_t version,
      material_t *materials, size_t num_materials )
{
    ...
    if (!mat_group || !tex_group || !materials) return;
    ...
    for (uint32_t ti = 1; ti <= n_tex; ti++)
    {
        ...
        const char *name = read_pooled_string(data,size,
            (const uint8_t*)&tex_entries[ti].stringOffset,
            (const uint8_t*)tex_group,&name_len);
...
static void read_draw_materials(const uint8_t *data, size_t size,
                                uint32_t version, int *object_material,
                                size_t num_objects, size_t num_materials) {
    ResourceGroup *defs = get_mdl_group(data,size,version,MDL_DEFINITIONS);
    if (!defs) return;
    uint32_t count = swap32(defs->numNodes);
    if (!in_bounds(data,size,defs+1,(size_t)(count+1)*sizeof(ResourceEntry))) return;
...
static void read_node_mix_transforms
    ( const uint8_t *data, size_t size, uint32_t version,
      node_transform_t **nodes, size_t *num_nodes )
{
    ResourceGroup *defs = get_mdl_group(data,size,version,MDL_DEFINITIONS);
    if (!defs) return;
    ...
    for (uint32_t i = 1; i <= count; i++)
    {
        ...
        if (name && name_len == 7 && !memcmp(name,"NodeMix",7))
        {
            ...
            if (offset >= 0 && in_bounds(data,size,candidate,1)) p = candidate;
            break;
        }
    }
    if (!p) return;
    ...
    while (in_bounds(data,size,p,1))
    {
        if (*p == 5)
        {
            if (!in_bounds(data,size,p,5)) break;
            p += 5;
            continue;
        }
        if (*p != 3 || !in_bounds(data,size,p,4)) break;

        const unsigned node_id = read_be16(p+1);
        const unsigned weights = p[3];
        const uint8_t *entry = p+4;
        if (!weights || !in_bounds(data,size,entry,(size_t)weights*6)) break;
        if (!ensure_node_transform(nodes,num_nodes,node_id)) break;
        ...
        else
        {
            ...
            for (unsigned w = 0; w < weights; w++, entry += 6)
            {
                ...
                float product[12];
                multiply_affine43(product,(*nodes)[bone_id].matrix,
                                  (*nodes)[bone_id].inverse);
                for (unsigned n = 0; n < 12; n++)
                    result.matrix[n] += product[n]*weight;
                if (num_weight_values
                        < sizeof(weight_bones)/sizeof(*weight_bones))
                {
            ...
            result.valid = valid;
            if (!valid) num_weight_values = 0;
        }
        if (result.valid)
        {
            node_transform_t *target = (*nodes)+node_id;
            free(target->bones); free(target->weights);
            memcpy(target,&result,sizeof(*target));
            target->bones = NULL; target->weights = NULL; target->num_weights = 0;
            if (num_weight_values)
...
static int remap_bind_array
    ( mesh_t *mesh, int object_node, const node_transform_t *nodes,
      size_t num_nodes, int normals )
{
    vec3_t *source = normals ? mesh->normals : mesh->positions;
    const size_t source_count = normals ? mesh->num_normals : mesh->num_positions;
    if (!source || !source_count || !mesh->num_vertices) return 1;

    int needs_transform = 0;
    for (size_t i = 0; i < mesh->num_vertices; i++)
    {
        const int node = mesh->vertices[i].matrix_idx >= 0
            ? mesh->vertices[i].matrix_idx : object_node;
        if (node >= 0 && (size_t)node < num_nodes && nodes[node].valid)
            { needs_transform = 1; break; }
    }
    if (!needs_transform) return 1;

    size_t hash_count = 16;
    while (hash_count < mesh->num_vertices*2) hash_count *= 2;
    ...
    {
        free(keys); free(values); free(used); free(result);
        return 0;
    }
    ...
    for (size_t i = 0; i < mesh->num_vertices; i++)
    {
        ...
        while (used[slot] && keys[slot] != key) slot = (slot+1)&(hash_count-1);
        if (!used[slot])
        {
            used[slot] = 1;
            keys[slot] = key;
            ...
            if (result_node) result_node[result_count] = node;
            result[result_count++] = value;
        }
        if (normals) vertex->normal_idx = (int)values[slot];
        else vertex->position_idx = (int)values[slot];
    }

    free(keys); free(values); free(used); free(source);
    vec3_t *shrunk = realloc(result,result_count*sizeof(*result));
    if (shrunk) result = shrunk;
    if (normals)
        { mesh->normals = result; mesh->num_normals = result_count; }
    else
    {
        mesh->positions = result; mesh->num_positions = result_count;
...
model_t* ParseMDL0(const uint8_t *data, size_t size) {
    ...
    if (version < 8 || version > 11) return NULL;
    ...
    // Parse Bones
    {
        ...
        for (int i = 1; i <= numBones; i++) {
            ...
            if (bone_nodes) bone_nodes[i-1] = bNode;

            model->joints[i-1].parent_idx = -1;
            uint32_t name_len;
            const char *name = read_pooled_string(data,size,
                (const uint8_t*)&entries[i].stringOffset,(const uint8_t*)grp,&name_len);
            if (name) copy_pooled_string(model->joints[i-1].name,
                                         sizeof(model->joints[i-1].name),name,name_len);
            ...
            if (node_id >= 0
                && ensure_node_transform(&node_transforms,&num_node_transforms,
                                         (unsigned)node_id))
            {
        ...
        if (bone_nodes)
            for (int i = 0; i < numBones; i++)
            {
                ...
                if (!parent_offset) continue;
                const uint8_t *parent = (const uint8_t*)bone + parent_offset;
                if (!in_bounds(data,size,parent,sizeof(MDL0Bone))) continue;
    ...
    // exporters can actually emit/bind the textures a model needs).
    {
        ...
        for (int i = 1; i <= numMats; i++) {
            ...
            if (!nm)
                nm = read_pooled_string(data,size,
                    (const uint8_t*)&entries[i].stringOffset,(const uint8_t*)grp,&nlen);
            if (nm) copy_pooled_string(mat->name, sizeof(mat->name), nm, nlen);

            read_material_textures(data, size, matBase, matLen, version, mat);
        }
        read_linked_material_textures(data,size,version,
    ...
    // Parse Meshes
    {
        ...
        if (!object_material) goto skip_meshes;
        for (int i = 0; i < numMeshes; i++) object_material[i] = -1;
        ...
        for (int i = 1; i <= numMeshes; i++) {
                ...
                || !decode_gx_primitives(primData,primSize,cpLo,cpHi,mesh))
            {
                ...
                for (unsigned t = 0; t < 7; t++)
                    { free(mesh->extra_texcoords[t]); mesh->extra_texcoords[t]=NULL; mesh->num_extra_texcoords[t]=0; }
                free(mesh->vertices); mesh->vertices = NULL; mesh->num_vertices = 0;
                continue;
            }
            ...
            for (size_t p = 0; p < mesh->num_vertices; p++)
                if ((size_t)mesh->vertices[p].position_idx >= mesh->num_positions
                    || mesh->num_normals && (size_t)mesh->vertices[p].normal_idx >= mesh->num_normals
                    || mesh->num_texcoords && (size_t)mesh->vertices[p].texcoord_idx >= mesh->num_texcoords
                    || mesh->num_colors[0] && (size_t)mesh->vertices[p].color_idx[0] >= mesh->num_colors[0]
                    || mesh->num_colors[1] && (size_t)mesh->vertices[p].color_idx[1] >= mesh->num_colors[1]
                    || mesh->num_extra_texcoords[0] && (size_t)mesh->vertices[p].extra_texcoord_idx[0] >= mesh->num_extra_texcoords[0]
                    || mesh->num_extra_texcoords[1] && (size_t)mesh->vertices[p].extra_texcoord_idx[1] >= mesh->num_extra_texcoords[1]
                    || mesh->num_extra_texcoords[2] && (size_t)mesh->vertices[p].extra_texcoord_idx[2] >= mesh->num_extra_texcoords[2]
                    || mesh->num_extra_texcoords[3] && (size_t)mesh->vertices[p].extra_texcoord_idx[3] >= mesh->num_extra_texcoords[3]
                    || mesh->num_extra_texcoords[4] && (size_t)mesh->vertices[p].extra_texcoord_idx[4] >= mesh->num_extra_texcoords[4]
                    || mesh->num_extra_texcoords[5] && (size_t)mesh->vertices[p].extra_texcoord_idx[5] >= mesh->num_extra_texcoords[5]
                    || mesh->num_extra_texcoords[6] && (size_t)mesh->vertices[p].extra_texcoord_idx[6] >= mesh->num_extra_texcoords[6])
                {
                    free(mesh->vertices); mesh->vertices = NULL; mesh->num_vertices = 0;
                    break;
                }
            ...
            if (!apply_bind_pose_transforms(mesh,object_node,node_transforms,
                                            num_node_transforms))
            {
            ...
            const char *mesh_name = read_pooled_string(data,size,
                (const uint8_t*)&oNode->stringOffset,(const uint8_t*)oNode,
                &mesh_name_len);
            if (!mesh_name)
                mesh_name = read_pooled_string(data,size,
                    (const uint8_t*)&entries[i].stringOffset,
                    (const uint8_t*)grp,&mesh_name_len);
            if (mesh_name)
                copy_pooled_string(mesh->name,sizeof(mesh->name),
    ...
    if (num_node_transforms && model->num_joints)
    {
        ...
        node_influence_t *table =
            calloc(num_node_transforms,sizeof(*table));
        if (node_to_joint && table)
        {
            for (size_t n = 0; n < num_node_transforms; n++) node_to_joint[n] = -1;
            {
                ResourceGroup *bone_group =
                    get_mdl_group(data,size,version,MDL_BONES);
                ResourceEntry *bone_entries = bone_group
                    ? (ResourceEntry*)(bone_group+1) : NULL;
                for (size_t j = 0; bone_entries && j < model->num_joints; j++)
                {
                    const int32_t off = (int32_t)swap32(bone_entries[j+1].dataOffset);
                    const MDL0Bone *bone =
                        (const MDL0Bone*)((const uint8_t*)bone_group + off);
            ...
            for (size_t n = 0; n < num_node_transforms; n++)
            {
                ...
                for (size_t k = 0; k < src->num_weights; k++)
                {
                    ...
                    if (bone_node >= num_node_transforms
                        || node_to_joint[bone_node] < 0) continue;
                    w[used].bone_idx = node_to_joint[bone_node];
                    w[used++].weight = src->weights[k];
                }
                if (used) { table[n].weights = w; table[n].num_weights = used; }
                else free(w);
            }
