...
static const KeywordTab_t opt_gamemodes_tab[] = { { 0, "CLEAR", "RESET", GMD_M_ALL | GMD_F_HIDE },
	{ GMD_M_DEFAULT, "DEFAULT", 0, GMD_M_ALL | GMD_F_HIDE },

	{ GMD_STD, "STANDARD", "STD", 0 }, { GMD_BALLOON, "BALLOON", 0, 0 }, { GMD_COIN, "COIN", 0, 0 },
	{ GMD_MD_BATTLE, "BATTLE", 0, 0 }, { GMD_VERSUS, "VERSUS", "VS", 0 },
	{ GMD_ITEMRAIN, "ITEMRAIN", 0, 0 }, { GMD_MD_RACE, "RACE", 0, 0 }, { GMD_MD_ALL, "ALL", 0, 0 },

	{ GMD_NO_MODES, "NO-MODES", "NOMODES", 0 }, { GMD_OFFLINE, "OFFLINE", 0, 0 },
	{ GMD_ONLINE, "ONLINE", 0, 0 },

	{ GMD_AUTO, "AUTO", 0, 0 }, { GMD_STD_FIRST, "1STANDARD", "1STD", 0 },
	{ GMD_ENGINE, "ENGINE", 0, 0 }, { 0, "NO-ENGINE", "NOENGINE", GMD_ENGINE },
	{ GMD_RANDOM, "RANDOM", 0, 0 }, { 0, "NO-RANDOM", "NORANDOM", GMD_RANDOM },
	{ GMD_NO_TT, "NO-TT", "NOTT", 0 }, { GMD_FORCE_TT, "TT", 0, 0 }, { GMD_FULL, "FULL", 0, 0 },

	{ GMD_NSORT, "SORT", "NSORT", GMD_M_SORT }, { GMD_ISORT, "ISORT", 0, GMD_M_SORT },
	{ 0, "NO-SORT", "NOSORT", GMD_M_SORT },

	{ GMD_DEBUG, "DEBUG", 0, 0 }, { GMD_DUMP, "DUMP", 0, 0 },
...
int ScanOptGamemodes (ccp arg)
{
	...
	s64 stat = ScanKeywordList (
		arg, opt_gamemodes_tab, 0, true, 0, GMD_M_OPTIONS, "Option --gamemodes", ERR_SYNTAX);
	if (stat != -1)
	{
		opt_gamemodes = stat;
		...
		if (opt_gamemodes & GMD_DEBUG)
		{
			char buf[200] = { 0 };
			PrintKeywordList (
				buf, sizeof (buf), 0, opt_gamemodes_tab, opt_gamemodes, GMD_M_DEFAULT, GMD_F_HIDE);
			fflush (stdout);
...
void AnalyseDefObject (kmp_ana_defobj_t *ad, const kmp_gobj_entry_t *gobj)
{
	...
	{
		ad->is_defobj = true;
		ad->is_pure_defobj = (gobj->ref_id == 0 || IsConditionRefId (gobj->ref_id))
			&& gobj->position[0] == 0.0 && gobj->position[1] == 0.0 && gobj->position[2] == 0.0
			&& gobj->rotation[0] == 0.0 && gobj->rotation[1] == 0.0 && gobj->rotation[2] == 0.0
			&& gobj->scale[0] == 0.0 && gobj->scale[1] == 0.0 && gobj->scale[2] == 0.0
			&& gobj->route_id == 0xffff && (gobj->pflags == 0 || gobj->pflags == GOBJ_PF_MODE1);
	}
...
bool FixDefObj (kmp_t *kmp, // pointer to valid KMP
...
static int sort_defobj_begin (const kmp_gobj_entry_t *a, const kmp_gobj_entry_t *b)
{
	if (IsDefinitionObjectId (a->obj_id))
	{
		...
		return a->obj_id < b->obj_id ? -1 : a->obj_id > b->obj_id ? 1 : a < b ? -1 : 1;
	}
...
static int sort_defobj_end (const kmp_gobj_entry_t *a, const kmp_gobj_entry_t *b)
{
	if (IsDefinitionObjectId (a->obj_id))
	{
		if (!IsDefinitionObjectId (b->obj_id))
...
bool SortDefObj (kmp_t *kmp, // pointer to valid KMP
...
bool FixReference (kmp_t *kmp, // pointer to valid KMP
...
bool SortReference (kmp_t *kmp, // pointer to valid KMP
...
u16 GetActiveObjectId (const kmp_gobj_entry_t *g)
{
	...
	return obj_id < N_KMP_GOBJ
			&& (g->pflags & GOBJ_M_PF_STD || (g->pflags & GOBJ_M_PF_MODE) == GOBJ_PF_MODE1)
		? obj_id
		: 0;
}
...
const gobj_condition_t gobj_condition[] = { { 0xffff, 0xff, -1, 1, 4, 0, 99, "All conditions." },
	{ 0x3fff, 0xfb, -1, 1, 4, 0, 99, "All conditions except reserved." },
	{ 0x7fff, 0x04, -1, 1, 4, 0, 99, "All conditions except reserved 2." },
	{ 0x3fff, 0xff, -1, 1, 4, 0, 99, "All conditions except time trial and reserved." },

	//--- offline players

	{ 0x000f, 0xff, -1, 1, 4, 0, 0, "Offline race with 1-4 human players." },
	{ 0x0007, 0xff, -1, 1, 3, 0, 0, "Offline race with 1-3 human players." },
	{ 0x0003, 0xff, -1, 1, 2, 0, 0, "Offline race with 1-2 human players." },
	{ 0x0001, 0xff, 0, 1, 1, 0, 0, "Offline race with 1 human player." },

	{ 0x000e, 0xff, -1, 2, 4, 0, 0, "Offline race with 2-4 human players." },
	{ 0x0006, 0xff, -1, 2, 3, 0, 0, "Offline race with 2-3 human players." },
	{ 0x0002, 0xff, 1, 2, 2, 0, 0, "Offline race with 2 human players." },

	{ 0x000c, 0xff, -1, 3, 4, 0, 0, "Offline race with 3-4 human players." },
	{ 0x0004, 0xff, 2, 3, 3, 0, 0, "Offline race with 3 human players." },
	{ 0x0008, 0xff, 3, 4, 4, 0, 0, "Offline race with 4 human players." },

	//--- online, 1-N players total

	{ 0x3ff0, 0xff, -1, 1, 2, 1, 99, "Online race with any number of players." },
	{ 0x1550, 0xff, -1, 1, 1, 1, 99, "Online race with 1 human player at Wii." },
	{ 0x2aa0, 0xff, -1, 2, 2, 1, 99, "Online race with 2 human players at Wii." },

	{ 0x0ff0, 0xff, -1, 1, 2, 1, 18, "Online race with 1-18 players total." },
	{ 0x0550, 0xff, -1, 1, 1, 1, 18, "Online race with 1-18 players total and 1 human at Wii." },
	{ 0x0aa0, 0xff, -1, 2, 2, 1, 18, "Online race with 1-18 players total and 2 humans at Wii." },

	{ 0x03f0, 0xff, -1, 1, 2, 1, 12, "Online race with 1-12 players total." },
	{ 0x0150, 0xff, -1, 1, 1, 1, 12, "Online race with 1-12 players total and 1 human at Wii." },
	{ 0x02a0, 0xff, -1, 2, 2, 1, 12, "Online race with 1-12 players total and 2 humans at Wii." },

	{ 0x00f0, 0xff, -1, 1, 2, 1, 9, "Online race with 1-9 players total." },
	{ 0x0050, 0xff, -1, 1, 1, 1, 9, "Online race with 1-9 players total and 1 human at Wii." },
	{ 0x00a0, 0xff, -1, 2, 2, 1, 9, "Online race with 1-9 players total and 2 humans at Wii." },

	{ 0x0030, 0xff, -1, 1, 2, 1, 9, "Online race with 1-6 players total." },
	{ 0x0010, 0xff, 4, 1, 1, 1, 6, "Online race with 1-6 players total and 1 human at Wii." },
	{ 0x0020, 0xff, 5, 2, 2, 1, 6, "Online race with 1-6 players total and 2 humans at Wii." },

	//--- online, 7-N players total

	{ 0x3fc0, 0xff, -1, 1, 2, 7, 99, "Online race with 7 or more players total." },
	...
	{ 0x0fc0, 0xff, -1, 1, 2, 7, 18, "Online race with 7-18 players total." },
	{ 0x0540, 0xff, -1, 1, 1, 7, 18, "Online race with 7-18 players total and 1 human at Wii." },
	{ 0x0a80, 0xff, -1, 2, 2, 7, 18, "Online race with 7-18 players total and 2 humans at Wii." },

	{ 0x03c0, 0xff, -1, 1, 2, 7, 12, "Online race with 7-12 players total." },
	{ 0x01e0, 0xff, -1, 1, 1, 7, 12, "Online race with 7-12 players total and 1 human at Wii." },
	{ 0x0280, 0xff, -1, 2, 2, 7, 12, "Online race with 7-12 players total and 2 humans at Wii." },

	{ 0x00c0, 0xff, -1, 1, 2, 7, 9, "Online race with 7-9 players total." },
	{ 0x0040, 0xff, 6, 1, 1, 7, 9, "Online race with 7-9 players total and 1 human at Wii." },
	{ 0x0080, 0xff, 7, 2, 2, 7, 9, "Online race with 7-9 players total and 2 humans at Wii." },

	//--- online, 10-N players total

	{ 0x3f00, 0xff, -1, 1, 2, 10, 99, "Online race with 10 or more players total." },
	...
	{ 0x0f00, 0xff, -1, 1, 2, 10, 18, "Online race with 10-18 players total." },
	{ 0x0500, 0xff, -1, 1, 1, 10, 18, "Online race with 10-18 players total and 1 human at Wii." },
	{ 0x0a00, 0xff, -1, 2, 2, 10, 18, "Online race with 10-18 players total and 2 humans at Wii." },

	{ 0x0300, 0xff, -1, 1, 2, 10, 12, "Online race with 10-12 players total." },
	{ 0x0100, 0xff, 8, 1, 1, 10, 12, "Online race with 10-12 players total and 1 human at Wii." },
	{ 0x0200, 0xff, 9, 2, 2, 10, 12, "Online race with 10-12 players total and 2 humans at Wii." },

	//--- online, 13-N players total

	{ 0x3c00, 0xff, -1, 1, 2, 13, 99, "Online race with 13 or more players total." },
	...
	{ 0x0c00, 0xff, -1, 1, 2, 13, 18, "Online race with 13-18 players total." },
	{ 0x0400, 0xff, 10, 1, 1, 13, 18, "Online race with 13-18 players total and 1 human at Wii." },
	{ 0x0800, 0xff, 11, 2, 2, 13, 18, "Online race with 13-18 players total and 2 humans at Wii." },

	//--- online, 17-N players total

	{ 0x3000, 0xff, -1, 1, 2, 19, 99, "Online race with 19 or more players total." },
	...
	{ 0x4000, 0x04, 14, 1, 1, -1, -1, "Time Trial (Reserved 1)." },
	{ 0xc000, 0xfb, -1, -1, -1, -1, -1, "Reserved 1 and 2." },
	{ 0x4000, 0xfb, 14, -1, -1, -1, -1, "Reserved 1." },
	{ 0x8000, 0xff, 15, -1, -1, -1, -1, "Reserved 2." }, { 0, 0, -1, -1, -1, -1, -1, 0 } };
...
uint GetGobjConditions (gobj_condition_set_t *gcs, int setting, u16 mask)
{
	...
	while (src->info)
	{
		if (setting_mask & src->setting && (mask & src->mask) == src->mask
...
static int sort_apf_name (const kmp_ana_pflag_res_t *a, const kmp_ana_pflag_res_t *b)
{
...
static void analyze_helper (kmp_ana_pflag_t *ap, // valid struct
...
)
{
		...
		else
		{
			...
			switch (ref_list[i])
			{
				...
				default:
					...
					{
						const gobj_cond_ref_t *cref = cond_ref_tab + (ref_list[i] - X_CONDREF_BEG);
...
#if SUPPORT_SEP
		if (res_ptr > res && (gobj->pflags & 0x100 || ap->auto_sep && !(i % 5)))
		{
			*res_ptr++ = CH_SEP;
		}
#endif
	...
	if (found_ri < 0 || ap->opt_gamemode & GMD_FULL || is_tt && ap->opt_gamemode & GMD_FORCE_TT)
	{
		...
		pr->lex_test.offline_online = xpf->n_online > 0 ? LEX_OO_ONLINE : LEX_OO_OFFLINE;
...
void AnalysePFlagScenarios (kmp_ana_pflag_t *ap, const kmp_t *kmp, opt_gamemode_t opt_gamemode)
{
	DASSERT (ap);
	DASSERT (kmp);
	...
	static const struct job_t race_job_list[]
		= { { "std", KMP_GMODE_STD, GMD_STD }, { "V", KMP_GMODE_VERSUS, GMD_VERSUS },
			  { "I", KMP_GMODE_ITEMRAIN, GMD_ITEMRAIN }, { "B", KMP_GMODE_BALLOON, GMD_BALLOON },
			  { "C", KMP_GMODE_COIN, GMD_COIN }, { "std", KMP_GMODE_STD, GMD_STD }, { "", -1, 0 } };

	static const struct job_t arena_job_list[] = { { "std", KMP_GMODE_STD, GMD_STD },
		{ "B", KMP_GMODE_BALLOON, GMD_BALLOON }, { "C", KMP_GMODE_COIN, GMD_COIN },
		{ "V", KMP_GMODE_VERSUS, GMD_VERSUS }, { "I", KMP_GMODE_ITEMRAIN, GMD_ITEMRAIN },
		{ "std", KMP_GMODE_STD, GMD_STD }, { "", -1, 0 } };

	static const struct job_t auto_job_list[] = { { "std", KMP_GMODE_STD, GMD_STD },
		{ "a", KMP_GMODE_AUTO, GMD_NO_MODES }, { "std", KMP_GMODE_STD, GMD_STD }, { "", -1, 0 } };
	...
	const struct job_t *job = (opt_gamemode & GMD_NO_MODES) ? auto_job_list
		: CheckBattleModeKMP (kmp) >= ARENA_FOUND			? arena_job_list
															: race_job_list;
	...
	if (ap->n_res > 1)
	{
		if (opt_gamemode & GMD_ISORT)
			qsort (ap->res_list, ap->n_res, sizeof (*ap->res_list), (qsort_func)sort_apf_index);
		else if (opt_gamemode & GMD_NSORT)
			qsort (ap->res_list, ap->n_res, sizeof (*ap->res_list), (qsort_func)sort_apf_name);
	}
...
typedef struct xpf_mode1_elem_t
{
	...
	struct xpf_mode1_elem_t *hit; // best element found
...
static void LogPrintElement (xpf_mode1_elem_t *e1, xpf_mode1_elem_t *e2)
{
	if (e2 && e2->hit == e1)
		fprintf (stdlog, " : %s%3d. %s%04x %s%04x %s%04x%s",
			...
			e1->obj->obj_id == e2->obj->obj_id ? collog->b_green : collog->b_red, e1->obj->obj_id,
			e1->obj->ref_id == e2->obj->ref_id ? collog->b_green : collog->b_red, e1->obj->ref_id,
			e1->obj->pflags == e2->obj->pflags ? collog->b_green : collog->b_red, e1->obj->pflags,
			collog->reset);
	else
		fprintf (stdlog, " : %3d. %04x %04x %04x", e1->index, e1->obj->obj_id, e1->obj->ref_id,
			e1->obj->pflags);
}

//-----------------------------------------------------------------------------

static void LogListXPF (xpf_mode1_info_t *info, int index, ccp text, int loop, int update_eval)
{
		...
		for (i2 = 0, e2 = l2->list; i2 < l2->used; i2++, e2++)
		{
			...
			else
			{
				diff_sum = Length2F (o1->position, o2->position)
					+ Length2F (o1->rotation, o2->rotation) + Length2F (o1->scale, o2->scale);
...
enumError RepairXPF (kmp_t *kmp)
{
	...
	PRINT0 ("REPAIR XPF! rep=%d/%d, kmp=%d/%d\n", rinfo->broken, rinfo->used, kinfo->broken,
		kinfo->used);
