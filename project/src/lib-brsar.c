...
{
    u8 b[2];
    if ( le ) { b[0] = (u8)val; b[1] = (u8)(val>>8); }
    else      { b[0] = (u8)(val>>8); b[1] = (u8)val; }
...
{
    u8 b[4];
    if ( le ) { b[0]=(u8)val; b[1]=(u8)(val>>8); b[2]=(u8)(val>>16); b[3]=(u8)(val>>24); }
    else      { b[0]=(u8)(val>>24); b[1]=(u8)(val>>16); b[2]=(u8)(val>>8); b[3]=(u8)val; }
...
{
    assert(offs + 4 <= mb->size);
    if ( le ) { mb->data[offs]=(u8)val; mb->data[offs+1]=(u8)(val>>8); mb->data[offs+2]=(u8)(val>>16); mb->data[offs+3]=(u8)(val>>24); }
    else      { mb->data[offs]=(u8)(val>>24); mb->data[offs+1]=(u8)(val>>16); mb->data[offs+2]=(u8)(val>>8); mb->data[offs+3]=(u8)val; }
}
...
static u32 rd_u32e ( const u8 *p, bool le ) { return le ? ((u32)p[3]<<24|(u32)p[2]<<16|(u32)p[1]<<8|p[0]) : rd_u32(p); }
static u16 rd_u16e ( const u8 *p, bool le ) { return le ? (u16)((u16)p[1]<<8|p[0]) : rd_u16(p); }
...
static ccp sniff_extension ( const u8 *data, size_t size )
{
    if ( size >= 4 )
    {
        if ( !memcmp(data, "RSEQ", 4) ) return ".rseq";
        if ( !memcmp(data, "RBNK", 4) ) return ".rbnk";
        if ( !memcmp(data, "RWAR", 4) ) return ".rwar";
        if ( !memcmp(data, "RWSD", 4) ) return ".rwsd";
    }
    return ".bin";
}
...
static void BuildBrsarContent ( const brsar_asset_t *assets, uint n_assets,
                                 membuf_t *symb_block, membuf_t *info_block, membuf_t *file_block,
                                 size_t *group_data_offs_rel )
{
    symb_builder_t symb;
    symb_init(&symb);
    ...
    membuf_t info;
    mb_init(&info);
    for ( int w = 0; w < 10; w++ ) mb_append_u32(&info, 0); // reserves +0x00..+0x24
    ...
    for ( uint i = 0; i < n_assets; i++ )
    {
        if ( assets[i].type == BRSAR_ASSET_RSEQ ) n_seq++;
        else if ( assets[i].type == BRSAR_ASSET_RBNK ) n_bnk++;
    }
    ...
    for ( uint i = 0; i < n_seq; i++ ) reftab_entry(&info, 0);
    ...
    for ( uint i = 0; i < n_bnk; i++ ) reftab_entry(&info, 0);
    ...
    for ( uint i = 0; i < n_assets; i++ ) reftab_entry(&info, 0);
    ...
    for ( uint i = 0; i < n_assets; i++ )
    {
        ...
        mb_append_u32(&info, (u32)(pos_entry_offs + 4)); // +0x08 slot is read directly as filePosBase
    ...
    size_t group_data_offs_slot = info.size; mb_append_u32(&info, 0); // +0x10 data.offset, patched by envelope writer
    ...
    while ( info.size - group_base < 0x24 ) mb_append_u32(&info, 0);
    size_t item_tab_offs_slot = info.size; mb_append_u32(&info, 0);
    ...
    for ( uint i = 0; i < n_assets; i++ )
    {
        ...
        mb_append(&info, tail, 8);        // +0x14 volume, +0x15 prio, +0x16 type, +0x17 remoteFilter, then pad to +0x1C
        ...
        mb_append_u32(&info, 0);                 // dataOffset: start-of-track offset within the RSEQ bytecode (0 = from the top)
    ...
    *group_data_offs_rel = 8 + group_data_offs_slot; // relative to info_block's own start (incl. tag+size)
                                 ...
                                 membuf_t *file_block, size_t group_data_offs_rel )
{
    ...
    mb_append(out, "\x01\x04", 2); // version 1.4 (matches the "\x01" major byte MatchBytes checks for)
...
{
    bool le = cstm; // CSAR defaults little-endian, mirroring BCSTM's "c->little_endian = variant==CSTM"
    ...
    for ( int i = 0; i < 3; i++ )
    {
        mb_append_u16e(out, 0, le); mb_append_u16e(out, 0, le); // flag, pad
        mb_append_u32e(out, 0, le); mb_append_u32e(out, 0, le); // offset, size (patched below)
    }
    mb_align(out, 0x40);
    ...
    for ( int i = 0; i < 3; i++ )
    {
        ...
        mb_put_u16(out, e, le ? (u16)((flags[i]>>8)|(flags[i]<<8)) : flags[i]); // mb_put_u16 always writes BE; feed it pre-swapped bytes for LE
...
enumError PackBRSAR ( u8 **out_data, size_t *out_size, const brsar_asset_t *assets, uint n_assets, brsar_variant_t variant )
{
    if ( !assets || !n_assets )
        return ERROR0(ERR_INVALID_DATA, "PackBRSAR: no assets given\n");

    membuf_t symb_block, info_block, file_block;
    mb_init(&symb_block); mb_init(&info_block); mb_init(&file_block);
    size_t group_data_offs_rel = 0;

    BuildBrsarContent(assets, n_assets, &symb_block, &info_block, &file_block, &group_data_offs_rel);
...
enumError PackBRSARDir ( u8 **out_data, size_t *out_size, ccp input_dir, brsar_variant_t variant )
{
    ...
    if ( !err && !n_assets )
        err = ERROR0(ERR_INVALID_DATA, "PackBRSARDir: no RSEQ/RBNK/RWAR/RWSD assets found in '%s'\n", input_dir);
    ...
    if ( owned_names ) FREE(owned_names);
    if ( owned_data ) FREE(owned_data);
    if ( assets ) FREE(assets);
...
typedef struct symb_reader_t { const u8 *base; u32 count; const u8 *table; } symb_reader_t;

static void symb_read ( symb_reader_t *sr, const u8 *block ) // block = SYMB content base (past tag+size)
{
    sr->base = block;
...
static enumError UnpackBrsarContent ( const u8 *symb, const u8 *info, const u8 *file_base, size_t file_size, ccp out_dir )
{
    symb_reader_t sr;
    symb_read(&sr, symb);
    ...
    for ( u32 i = 0; i < item_count; i++ )
    {
        ...
        char path[PATH_MAX];
        if ( name )
            snprintf(path, sizeof(path), "%s/%s%s", out_dir, name, sniff_extension(bytes, data_size));
        else
            snprintf(path, sizeof(path), "%s/file_%03u%s", out_dir, fid, sniff_extension(bytes, data_size));
...
enumError UnpackBRSAR ( const u8 *data, size_t size, ccp out_dir )
{
    ...
    if ( !memcmp(data, "RSAR", 4) )
    {
        ...
        return UnpackBrsarContent(data + symb_offs + 8, data + info_offs + 8,
                                   data, size, out_dir); // file_content base = whole buffer; group.data.offset is absolute
    }
    ...
    if ( !memcmp(data, "FSAR", 4) || !memcmp(data, "CSAR", 4) )
    {
        ...
        for ( int i = 0; i < sections && pos + 12 <= size; i++, pos += 12 )
        {
            ...
            if ( flag == 0x4000 ) symb_offs = offs;
            else if ( flag == 0x4001 ) info_offs = offs;
            else if ( flag == 0x4002 ) file_offs = offs;
        }
