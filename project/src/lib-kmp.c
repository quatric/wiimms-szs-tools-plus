...
bool KMP_ACTION_LOG (const kmp_t *kmp, bool is_patch, const char *format, ...)
{
	...
	{
		fflush (stdout);
		fprintf (stdlog, "    %s>[KMP]%s ", colset->heading,
...
const KeywordTab_t Wim0SectionName[]
	= { { W0ID_END, "END", 0, W0F_BINARY }, { W0ID_VERSION, "VERSION", 0, W0F_TEXT },
...
enumError CompressWim0 (kmp_wim0_t *w0)
{
	...
	enumError err
		= EncodeBZIP2 (&w0->bz2data, &w0->bz2size, false, 0, false, w0->data, w0->used, 9);
...
kmp_wim0_sect_t *AppendSectionWim0 (kmp_wim0_t *w0, // data structure
...
static bool CreateWim0 (const u8 **res_data, // not NULL: store data pointer here
...
static bool GetSectionHelperWim0 (kmp_wim0_info_t *info, // store result here (never NULL)
...
bool GetFirstSectionWim0 (kmp_wim0_info_t *info, // store result here (never NULL)
	const struct kmp_t *kmp // pointer to valid KMP
)
{
	DASSERT (info);
...
bool GetNextSectionWim0 (kmp_wim0_info_t *info, // update result (never NULL)
	const struct kmp_t *kmp // pointer to valid KMP
)
{
	DASSERT (info);
...
static enumError ScanWim0 (kmp_t *kmp)
{
...
#if HAVE_PRINT
	{
		...
		bool avail;
		for (avail = GetFirstSectionWim0 (&info, kmp); avail;
			avail = GetNextSectionWim0 (&info, kmp))
		{
			printf ("%5d. %04x %-10s %4u bytes (%s) %s\n", info.index, info.offset, info.name,
				info.size, Wim0FormatName[info.format],
...
static const KeywordTab_t opt_kmp_tab[] = { { 0, "CLEAR", "RESET", KMPMD_M_ALL | KMPMD_F_HIDE },
	{ KMPMD_M_DEFAULT, "DEFAULT", 0, KMPMD_M_ALL | KMPMD_F_HIDE },

	{ KMPMD_FORCE, "FORCE", 0, 0 }, { KMPMD_NEW, "NEW", 0, 0 },
	{ KMPMD_RM_SPCITEM, "RM-SPCITEM", "RMSPCITEM", 0 }, { KMPMD_REVERSE, "REVERSE", 0, 0 },
	...
	{ KMPMD_01_LAP, "1LAPS", "L1", KMPMD_M_LAPS }, { KMPMD_02_LAPS, "2LAPS", "L2", KMPMD_M_LAPS },
	{ KMPMD_03_LAPS, "3LAPS", "L3", KMPMD_M_LAPS }, { KMPMD_04_LAPS, "4LAPS", "L4", KMPMD_M_LAPS },
	{ KMPMD_05_LAPS, "5LAPS", "L5", KMPMD_M_LAPS }, { KMPMD_06_LAPS, "6LAPS", "L6", KMPMD_M_LAPS },
	{ KMPMD_07_LAPS, "7LAPS", "L7", KMPMD_M_LAPS }, { KMPMD_08_LAPS, "8LAPS", "L8", KMPMD_M_LAPS },
	...
	{ KMPMD_FIX_CKPH, "FIX-CKPH", "FIXCKPH", 0 }, { KMPMD_FIX_ENPH, "FIX-ENPH", "FIXENPH", 0 },
	...
	{ KMPMD_RM_EMPTY, "RM-EMPTY", "RMEMPTY", 0 }, { KMPMD_MINIMIZE, "MINIMIZE", 0, 0 },

	{ KMPMD_PR_PREV, "PR-PREV", "PRPREV", 0 },

	{ KMPMD_TEST1, "TEST1", 0, 0 }, { KMPMD_TEST2, "TEST2", 0, 0 },

	{ KMPMD_LOG, "LOG", 0, 0 }, { KMPMD_SILENT, "SILENT", 0, 0 },
...
int ScanOptKmp (ccp arg)
{
	...
	s64 stat = ScanKeywordList (arg, opt_kmp_tab, 0, true, 0, KMP_MODE, "Option --kmp", ERR_SYNTAX);
	if (stat != -1)
	{
		SetKmpMode (stat);
		return 0;
...
ccp GetKmpMode ()
{
	...
	if (!*buf)
		PrintKeywordList (
			buf, sizeof (buf), 0, opt_kmp_tab, KMP_MODE, KMPMD_M_DEFAULT, KMPMD_F_HIDE);
	return buf;
}
...
void SetupKMP ()
{
	if (opt_tiny > 0)
	{
		const kmp_mode_t tiny = ((kmp_mode_t)opt_tiny << KMPMD_S_TINY) & KMPMD_M_TINY;
...
static const KeywordTab_t opt_kmp_diff_tab[] = { { KDMD_NONE, "NONE", 0, KDMD_M_ALL | KDMD_F_HIDE },
	...
	{ KDMD_KTPT, "KTPT", 0, 0 }, { KDMD_ENPT, "ENPT", 0, 0 }, { KDMD_ENPH, "ENPH", 0, 0 },
	{ KDMD_ITPT, "ITPT", 0, 0 }, { KDMD_ITPH, "ITPH", 0, 0 }, { KDMD_CKPT, "CKPT", 0, 0 },
	{ KDMD_CKPH, "CKPH", 0, 0 }, { KDMD_GOBJ, "GOBJ", 0, 0 }, { KDMD_POTI, "POTI", 0, 0 },
	{ KDMD_AREA, "AREA", 0, 0 }, { KDMD_CAME, "CAME", 0, 0 }, { KDMD_JGPT, "JGPT", 0, 0 },
	{ KDMD_CNPT, "CNPT", 0, 0 }, { KDMD_MSPT, "MSPT", 0, 0 }, { KDMD_STGI, "STGI", 0, 0 },

	{ KDMD_SORT, "SORT", "-NOSORT", KDMD_M_SORT }, { KDMD_NOSORT, "NOSORT", "-SORT", KDMD_M_SORT },
...
int ScanOptKmpDiff (ccp arg)
{
	...
	s64 stat
		= ScanKeywordList (arg, opt_kmp_diff_tab, 0, true, 0, KMP_MODE, "Option --kmp", ERR_SYNTAX);
	if (stat != -1)
	{
...
ccp GetKmpDiffMode ()
{
	...
	if (!*buf)
		PrintKeywordList (
			buf, sizeof (buf), 0, opt_kmp_diff_tab, KMP_DIFF_MODE, KDMD_M_DEFAULT, KDMD_F_HIDE);
	return buf;
}
...
int ScanOptTformKmp (ccp arg)
{
	...
	s64 stat = ScanKeywordList (
		arg, opt_tformkmp_tab, 0, true, 0, start, "Option --tform-kmp", ERR_SYNTAX);
	if (stat != -1)
	{
...
int GetNextPointKMP (const kmp_t *kmp, // KMP data structure
...
)
{
	...
	noPRINT (
		">> %u %zu[%u+%u]\n", idx, ph - (kmp_enph_entry_t *)phlist->list, ph->pt_start, ph->pt_len);
...
const kmp_enph_entry_t *FindPH (const kmp_t *kmp, // KMP data structure
...
void InsertLinkPH (u8 *vector, // insert into vector
...
)
{
	...
	if (si && !si->no_warn)
	{
		...
		DASSERT (sf);
		ERROR0 (ERR_WARNING, "To many links [%s @%u]: %s\n", sf->name, sf->line, name ? name : "?");
	}
}
...
void SetupClassNamePH (kmp_ph_t *ph // valid PH data structure
)
{
	struct inttab_t
	{
...
)
{
	...
	if (!old_found)
	{
		...
		if (it->num > MAX_CLASS_NAMES)
		{
			...
			if (!no_warn)
				ERROR0 (ERR_WARNING, "Class name list is full (%u names), can't add: %s\n",
...
static u64 IterateGroup (
	const kmp_enph_entry_t *elist, group_t *grp, uint idx, uint n, uint last_index, u64 steps)
{
	DASSERT (elist);
	DASSERT (grp);
	DASSERT (idx < n);
...
void RenameGroupKMP (kmp_t *kmp, // KMP data structure
	...
	for (gi = 0, gp = grp; gi < n; gi++, gp++, gname++)
	{
		...
		else
		{
			...
			if (nc <= 1)
				...
				snprintf (*gname, sizeof (*gname), "G%0*u%c", fw, gp->index, 'A' + gp->subindex);
			else
				snprintf (*gname, sizeof (*gname), "G%0*u%c%c", fw, gp->index,
...
static void AnalyzeKmpPath (const kmp_t *kmp, // KMP data structure
	uint sect_ph, // ph section index
...
)
{
		...
		for (; idx <= end && ip < iend; idx++)
		{
			if (path->itm_grp[idx] == 0xff)
			{
				...
				if (idx == end)
				{
					...
					{
						int next
							= GetNextPointKMP (kmp, path->sect_pt, idx, link, true, -1, 0, 0, 0, 0);
						if (next >= 0)
						{
							ip->cur = idx;
							ip->next = next;
...
#if HAVE_PRINT0
	{
		...
		for (i = 0; i < n_ph; i++)
		{
			...
			PRINT ("%3u. GRP %3u: %3u.%u : %3u .. %3u [+%3u]\n", gp->order, gi, gp->index,
				gp->subindex, ep->pt_start, ep->pt_start + ep->pt_len - 1, ep->pt_len);
		}
	}
#endif
...
#if HAVE_PRINT0
	{
		...
		for (i = 0; i < path->n_link; i++)
		{
			...
			PRINT ("%3u. ITM %3u -> %3u : %2u -> %3u\n", i, link->cur, link->next, link->grp_cur,
				link->grp_next);
		}
	}
#endif
...
const kmp_link_t *FindCKPT (const kmp_t *kmp, // KMP data structure
...
const uint kmp_entry_size[KMP_N_SECT + 1]
	= { sizeof (kmp_ktpt_entry_t), sizeof (kmp_enpt_entry_t), sizeof (kmp_enph_entry_t),
		  sizeof (kmp_itpt_entry_t), sizeof (kmp_itph_entry_t), sizeof (kmp_ckpt_entry_t),
		  sizeof (kmp_ckph_entry_t), sizeof (kmp_gobj_entry_t), sizeof (kmp_poti_group_t),
		  sizeof (kmp_area_entry_t), sizeof (kmp_came_entry_t), sizeof (kmp_jgpt_entry_t),
		  sizeof (kmp_cnpt_entry_t), sizeof (kmp_mspt_entry_t), sizeof (kmp_stgi_entry_t), 0 };

const KeywordTab_t kmp_section_name[] = { { KMP_KTPT, "KTPT", "Start position", 0 },
	{ KMP_ENPT, "ENPT", "Enemy route point", 0 }, { KMP_ENPH, "ENPH", "Enemy route group", 0 },
	{ KMP_ITPT, "ITPT", "Item route point", 0 }, { KMP_ITPH, "ITPH", "Item route group", 0 },
	{ KMP_CKPT, "CKPT", "Check point", 0 }, { KMP_CKPH, "CKPH", "Check point group", 0 },
	{ KMP_GOBJ, "GOBJ", "Global object", 0 }, { KMP_POTI, "POTI", "Route", 0 },
	{ KMP_AREA, "AREA", "Area", 0 }, { KMP_CAME, "CAME", "Camera", 0 },
	{ KMP_JGPT, "JGPT", "Respawn point", 0 }, { KMP_CNPT, "CNPT", "Canon point", 0 },
	{ KMP_MSPT, "MSPT", "Battle position", 0 }, { KMP_STGI, "STGI", "Stage info", 0 },

	{ KMP_N_SECT, "SETUP", 0, 1 }, { KMP_WIM0, "WIM0", 0, 2 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

const KeywordTab_t kmp_rtype_name[] = { { KMP_RT_AUTO, "AUTO", "A", 0 },
	{ KMP_RT_ROUTE, "ROUTE", "R", 0 }, { KMP_RT_DISPATCH, "DISPATCH", "D", 0 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

const KeywordTab_t kmp_oneway_name[]
	= { { ONEWAY_NONE, "NONE", "OFF", 0 }, { ONEWAY_PREV, "PREV", 0, 0 },
		  { ONEWAY_NEXT, "NEXT", 0, 0 }, { ONEWAY_BOTH, "BOTH", "ALL", 0 }, { 0, 0, 0, 0 } };
...
void InsertOnewayPH (kmp_ph_t *ph, u8 from, u8 to)
{
	...
	if (!FindOnewayPH (ph, from, to))
	{
		if (ph->oneway_used == ph->oneway_size)
		{
			ph->oneway_size += 100;
			ph->oneway_list
				= REALLOC (ph->oneway_list, ph->oneway_size * sizeof (*ph->oneway_list));
		}
		DASSERT (ph->oneway_used < ph->oneway_size);
...
void SortOnewayPH (const kmp_ph_t *ph)
{
	DASSERT (ph);
	if (ph->oneway_used > 1)
		qsort (ph->oneway_list, ph->oneway_used, sizeof (*ph->oneway_list), SortOnewayPH_helper);
}
...
)
{
	...
	uint sect_size
		= sizeof (kmp_list_head_t) + ntohs (sect_ptr->n_entry) * kmp_entry_size[ks->sect];
	if (ks->sect == KMP_POTI)
		sect_size += sizeof (kmp_poti_point_t) * ntohs (sect_ptr->value);

	//--- check limit

	uint next_off = base_off + (index + 1 < n_sect ? ntohl (kmp->sect_off[index + 1]) : size);
	if (next_off > size)
		next_off = size;
...
)
{
	...
	if (!list)
	{
		...
		for (s = -1; s < hi.n_sect; s++)
		{
			...
			const kmp_list_head_t *temp = (kmp_list_head_t *)(data + hi.head_size + off);
		...
		if (!list)
		{
			if (sect != KMP_WIM0 && !kmp->nowarn_format)
				ERROR0 (ERR_WARNING, "KMP: Section [%s] not found: %s\n",
	...
	{
		if (ErrorLogEnabled ())
			ERROR0 (ERR_WARNING, "Number of [%s] entries exceed file size (%u>%u): %s\n",
...
static uint test_unknown_sections (const void *data, // data to scan
	uint data_size, // size of 'data'
...
)
{
	...
	if (is_shrinked && ErrorLogEnabled ())
		ERROR0 (ERR_WARNING, "KMP: Is shrinked => some sections missed, some moved: %s\n", fname);
	...
	for (s = -1; s < hi.n_sect; s++)
	{
		...
		ccp expected_sect_name = s < 0 || s >= KMP_N_SECT ? "----" : kmp_section_name[s].name1;
		const kmp_list_head_t *temp = (kmp_list_head_t *)(data + hi.head_size + off);
		if (memcmp (temp->magic, expected_sect_name, sizeof (temp->magic)))
		{
			...
			for (i = 0; i < KMP_N_SECT; i++)
			{
				...
				if (off2 > hi.max_off || off2 & 3)
					continue;
				const kmp_list_head_t *temp2 = (kmp_list_head_t *)(data + hi.head_size + off2);
				if (!memcmp (temp2->magic, expected_sect_name, sizeof (temp2->magic)))
					break;
			}

			if (i < KMP_N_SECT && !is_shrinked)
				ERROR0 (ERR_WARNING, "KMP: Section [%s] found at index %u, but not at %d: %s\n",
...
static void scan_kmp_pt (kmp_t *kmp, // KMP data structure
	const void *data, // data to scan
	uint data_size, // size of 'data'
	uint sect // KMP section index
)
{
	...
	uint n;
	const kmp_enpt_entry_t *pt = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, sect);
	...
	const bool import = n > 1 && !(flags_or & KMP_FE_ALWAYS_0)
	...
	PRINT ("(%s): or=%08x>%x, and=%08x>%x => %d (EXPORT)\n", kmp_section_name[sect].name1, flags_or,
		flags_or & KMP_FE_ALWAYS_0, flags_and, flags_and & KMP_FE_ALWAYS_1, import);
}
...
static void scan_kmp_ph (kmp_t *kmp, // KMP data structure
	const void *data, // data to scan
	uint data_size, // size of 'data'
	uint sect_ph, // PH section index
...
)
{
	...
	uint n;
	const kmp_enph_entry_t *phe = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, sect_ph);
	uint i;
	for (i = 0; i < n; i++, phe++)
	{
	...
	PRINT ("scan_kmp_ph(%s), ac:%x,%x, battle=%d\n", KMP_SECT_NAME (sect_ph), ph->ac_mode,
		ph->ac_flags, kmp->battle_mode);
	RenameGroupKMP (kmp, sect_ph, ph->gname, ph->ac_mode == KMP_AC_DISPATCH);
}

///////////////////////////////////////////////////////////////////////////////

static void scan_kmp_pt2 (kmp_t *kmp, // KMP data structure
	const void *data, // data to scan
	uint data_size, // size of 'data'
	uint sect // KMP section index
)
{
	...
	uint n;
	const kmp_jgpt_entry_t *pt = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, sect);
	...
	const bool import = sect == KMP_JGPT && !(flags_or & KMP_FE_ALWAYS_0)
		&& (flags_and & KMP_FE_ALWAYS_1) == KMP_FE_ALWAYS_1;
	...
	PRINT ("(%s): or=%08x>%x, and=%08x>%x => %d => %d %d (EXPORT)\n", kmp_section_name[sect].name1,
		flags_or, flags_or & KMP_FE_ALWAYS_0, flags_and, flags_and & KMP_FE_ALWAYS_1, import,
		kmp->jgpt_flag.import, kmp->jgpt_flag.export);
}
...
enumError ScanRawKMP (kmp_t *kmp, // KMP data structure
...
)
{
	...
	uint n;
	const kmp_ktpt_entry_t *ktpt = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, KMP_KTPT);
	for (; n > 0; n--, ktpt++)
	{
	...
	const kmp_ckpt_entry_t *ckpt = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, KMP_CKPT);
	for (; n > 0; n--, ckpt++)
	{
	...
	const kmp_gobj_entry_t *gobj = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, KMP_GOBJ);
	for (; n > 0; n--, gobj++)
	{
	...
	uint n_pt;
	const kmp_poti_group_t *pgroup = get_kmp_pointer (&n, &n_pt, 0, kmp, data, data_size, KMP_POTI);
	while (n_pt > 0 && n-- > 0)
	{
	...
	const kmp_area_entry_t *area = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, KMP_AREA);
	for (; n > 0; n--, area++)
	{
	...
	const kmp_came_entry_t *came = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, KMP_CAME);
	for (; n > 0; n--, came++)
	{
	...
	const kmp_stgi_entry_t *stgi = get_kmp_pointer (&n, 0, 0, kmp, data, data_size, KMP_STGI);
	if (n > 0)
		kmp->stgi = stgi;
	for (; n > 0; n--, stgi++)
	{
	...
	if (wim0_data && n)
	{
		...
		if (opt_wim0 >= 0)
		{
			...
			if (DecompressWim0 (&w0) == ERR_OK && w0.used)
			{
				...
				HEXDUMP16 (11, 0, kmp->wim0_data, kmp->wim0_size < 0x80 ? kmp->wim0_size : 0x80);
				ScanWim0 (kmp);
			}
			w0.bz2data = 0;
			ResetWim0 (&w0);
...
enumError ScanKMP (kmp_t *kmp, // KMP data structure
	bool init_kmp, // true: initialize 'kmp' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
	...
	switch (GetByMagicFF (data, data_size, data_size))
	{
		...
		default:
			if (init_kmp)
				InitializeKMP (kmp);
			if (ErrorLogEnabled ())
				ERROR0 (ERR_INVALID_DATA, "No KMP file: %s\n", kmp->fname ? kmp->fname : "?");
			return ERR_INVALID_DATA;
	}

	PRINT (
		"BATTLE-MODE: %d [opt=%d,isa=%d]\n", kmp->battle_mode, opt_battle_mode, IsArenaKMP (kmp));
...
enumError ScanRawDataKMP (kmp_t *kmp, // KMP data structure
	bool init_kmp, // true: initialize 'kmp' first
...
enumError LoadKMP (kmp_t *kmp, // KMP data structure
	bool init_kmp, // true: initialize 'kmp' first
...
)
{
	...
	for (sect = 0; sect < src_sect; sect++)
	{
			...
			&& iptr->size > sizeof (kmp_list_head_t))
		{
			...
			TRACE (">> sect=%2d, index=%2d, off=%6zx .. %6zx, size=%5x %5x\n", iptr->sect,
				iptr->index, (u8 *)iptr->ptr - src_data, (u8 *)iptr->ptr - src_data + iptr->size,
	...
	if (log)
		KMP_ACTION_LOG (0, false, "ShrinkKMP() n-sect=%u/%u, size=%u+%u\n", n_sect,
			be16 (&kfile->n_sect), head_size, total_size - head_size);
...
static enumError ShrinkKMP (kmp_t *kmp // pointer to valid KMP
)
{
	DASSERT (kmp);
	TRACE ("ShrinkKMP()\n");
	...
	return ShrinkRawKMP (&kmp->raw_data, &kmp->raw_data_size, true, 0, 0, true) ? ERR_DIFFER
																				: ERR_OK;
}
...
static void *setup_raw_kmp (const kmp_t *kmp, // input: KMP
...
static void create_kmp_pt (const kmp_t *kmp, // input: KMP
	uint sect, // input: section number
	kmp_list_head_t **hd // modify: header pointer
)
{
	...
	PRINT (">> EXPORT_FLAGS[%s] = %d [%d], max=%d\n", kmp_section_name[sect].name1, kf->export,
		opt_export_flags, max_export);
	...
	for (i = 0; i < dlist->used; i++, pt++)
	{
		...
		if (kf->export)
		{
			...
			ptr[3] = (i < max_export ? kf->flags[i] : kf->mask) << KMP_FE_FLAGS_SHIFT
				| KMP_FE_ALWAYS_1;
		}
...
static void create_kmp_ph (const kmp_t *kmp, // input: KMP
	uint sect, // input: section number
	kmp_list_head_t **hd, // modify: header pointer
	kmp_ph_t *ph // related PH info
)
...
static void create_kmp_pt2 (const kmp_t *kmp, // input: KMP
	uint sect, // input: section number
	kmp_list_head_t **hd // modify: header pointer
)
{
...
enumError CreateRawKMP (kmp_t *kmp // pointer to valid KMP
)
{
	DASSERT (kmp);
	TRACE ("CreateRawKMP()\n");
	...
	uint data_size = head_size + sizeof (kmp_list_head_t) * n_sect
		+ sizeof (kmp_poti_point_t) * kmp->poti_point.used + info_size + wim0_size;
	...
	if (kmp->wim0_export)
	{
		...
		write_be32 (kfile->sect_off + KMP_N_SECT, (u8 *)h - kmp->raw_data - kmp->raw_head_size);
...
enumError SaveRawKMP (kmp_t *kmp, // pointer to valid KMP
...
{
	int stat = memcmp (a->data.setting, b->data.setting,
		sizeof (kmp_gobj_entry_t) - offsetof (kmp_gobj_entry_t, setting));

	if (!stat)
	{
		stat = memcmp (a->data.rotation, b->data.rotation,
...
bool SortGOBJ (kmp_t *kmp, // pointer to valid KMP
...
bool TransformKMP (kmp_t *kmp // pointer to valid KMP
)
{
...
int ScanOptRmGobj (ccp arg)
{
	...
	{
		PRINT ("## have_rm_kmp_gobj = %d [%d,%d]\n", have_rm_kmp_gobj, have_patch_count,
			have_kmp_patch_count);
		uint i;
...
static bool FixPH (kmp_t *kmp, // KMP data structure
...
)
{
	...
	PRINT ("FixPH(%s,%s,%d)\n", kmp_section_name[sect_ph].name1, kmp_section_name[sect_pt].name1,
		allow_dup);
	...
	const bool dirty = saved_n != ph_list->used || memcmp (saved_data, ph_list->list, saved_size);
	FREE (saved_data);
...
static kmp_gobj_entry_t *AppendGOBJ (kmp_t *kmp, u16 obj_id)
{
	...
	if (go)
	{
		...
		go->pflags = 0x3f;
		go->scale[0] = go->scale[1] = go->scale[2] = 1.0;
	}
	return go;
}
...
static bool ReversePT (const kmp_t *kmp, // KMP data structure
	uint sect // KMP PT section index
)
{
	DASSERT (kmp);
...
static bool ReversePOTI (const kmp_t *kmp // KMP data structure
)
{
	DASSERT (kmp);
...
static bool ReverseCKPT (const kmp_t *kmp // KMP data structure
)
{
	DASSERT (kmp);
	// [[2do]] ???
...
static bool ReverseKTPT (const kmp_t *kmp // KMP data structure
)
{
	DASSERT (kmp);
	// [[2do]] ???
...
static bool ReverseJGPT (const kmp_t *kmp // KMP data structure
)
{
	DASSERT (kmp);
	// [[2do]] ???
...
static bool ReverseKMP (kmp_t *kmp // pointer to valid KMP
)
{
	DASSERT (kmp);
	return ReversePT (kmp, KMP_ENPT) | ReversePT (kmp, KMP_ITPT) | ReversePOTI (kmp)
		| ReverseCKPT (kmp) // [[2do]]
		| ReverseKTPT (kmp) // [[2do]]
		| ReverseJGPT (kmp); // [[2do]]
}
...
bool PatchKMP (kmp_t *kmp // pointer to valid KMP
)
{
	DASSERT (kmp);
	...
	if (KMP_MODE & KMPMD_M_STGI || speed_mod_active || opt_n_laps)
	{
		...
		const uint new_lap_count
			= opt_n_laps ? opt_n_laps : (KMP_MODE & KMPMD_M_LAPS) >> KMPMD_SHIFT_LAPS;
		...
		for (i = 0; i < n; i++, stgi++)
		{
			if (new_lap_count && stgi->lap_count != new_lap_count
			...
			if (n == 1)
			{
				if (speed_mod_val)
					KMP_ACTION_LOG (kmp, true,
						...
						SpeedMod2float (old_speed_mod), speed_mod_factor, old_speed_mod,
						speed_mod_val);
				else
					KMP_ACTION_LOG (kmp, true,
	...
	if (k2_active)
	{
		...
		{
			const kmp_ktpt_entry_t *ktpt = (kmp_ktpt_entry_t *)kmp->dlist[KMP_KTPT].list + 1;

			KMP_ACTION_LOG (kmp, true,
				"KTPT: Second KTPT %s: position %1.0f,%1.0f,%1.0f (%4.2f°)\n",
				stat == 1 ? "inserted" : "replaced", ktpt->position[0], ktpt->position[1],
				ktpt->position[2], ktpt->rotation[1]);
			dirty = true;
		}
	}
	...
	if (KMP_MODE & (KMPMD_FIX_CKNEXT | KMPMD_FIX_CKJGPT))
	{
		...
		if (max_pt > 0)
		{
			...
			if (KMP_MODE & KMPMD_FIX_CKJGPT)
			{
				...
				{
					dirty = true;
					KMP_ACTION_LOG (kmp, true, "Section CKPT: %u invalid respawn link%s fixed.\n",
						count, count == 1 ? "" : "s");
				}
			}
		}
	...
	if (KMP_MODE & (KMPMD_RM_LECODE | KMPMD_PURGE_GOBJ))
	{
		...
		for (i = 0; i < n; i++, gobj++)
		{
			...
			if (KMP_MODE & KMPMD_PURGE_GOBJ && gobj->obj_id <= GOBJ_MIN_DEF
...
#if defined(DEBUG) || defined(TEST)
	{
		...
		for (kt = kmp_tiny_tab; kt->sect < KMP_N_SECT; kt++)
			ASSERT_MSG (kt->sect <= kt[1].sect, "kmp_tiny_tab[%zu] > kmp_tiny_tab[%zu] (%u>%u)\n",
				kt - kmp_tiny_tab, kt - kmp_tiny_tab + 1, kt->sect, kt[1].sect);
	}
#endif
	...
	if (tiny_mode > 0)
	{
		...
		for (sect = 0; sect < KMP_N_SECT; sect++)
		{
			...
			for (; kt->sect == sect; kt++)
			{
				PRINT ("%2zu > %2u.%02u, n=%u, md=%u, elem=%3u*%2u\n", kt - kmp_tiny_tab, kt->sect,
					kt->offset, kt->n, kt->mode, dlist->used, dlist->elem_size);
...
bool PatchRawDataKMP (void *data, // data to scan
	uint data_size // size of 'data'
)
{
	if (have_patch_count <= 0 || have_kmp_patch_count <= 0 && !transform_active)
...
void GetRouteTypeInfoKMP (kmp_rtype_info_t *rti, // store result here (never NULL)
...
)
{
	...
	PRINT ("RouteType[%s/%s]: g=%u, pt=%u, routes[0,1,>1]=%u+%u+%u, dispatch=%u\n",
		kmp_section_name[rti->sect_ph].name1, kmp_section_name[rti->sect_pt].name1, rti->n_group,
		rti->n_point, rti->n_route0, rti->n_route1, rti->n_route2, rti->n_dispatch);
}
...
IsArena_t IsArenaKMP (const kmp_t *kmp)
{
	...
	noPRINT ("IsArenaKMP() %d,%d,%d,%d => %d [battle=%d]\n", kmp->dlist[KMP_CKPT].used,
		kmp->dlist[KMP_ITPT].used, kmp->dlist[KMP_JGPT].used, kmp->dlist[KMP_MSPT].used,
	...
	return arena_count >= 9 ? ARENA_FOUND : arena_count > 6 ? ARENA_MAYBE : ARENA_NONE;
}
...
IsArena_t CheckBattleModeKMP (const kmp_t *kmp)
{
	...
	return opt_battle_mode <= OFFON_OFF							   ? ARENA_NONE
		: opt_battle_mode == OFFON_AUTO || is_arena >= ARENA_FOUND ? is_arena
																   : ARENA_FOUND;
}
...
double CalcCheckPointDistance (const kmp_ckpt_entry_t *ckpt, double x, double z)
{
	DASSERT (ckpt);
...
void CalcFinishLine (const kmp_t *kmp, // not NULL: use it for recommendation
...
)
{
	...
	if (kf->enpt_rec >= 0)
	{
		...
		PRINT ("REC: %1.0f %1.0f %1.0f | %1.2f° | dist %1.0f\n", kf->pos_ktpt_rec.x,
			kf->pos_ktpt_rec.y, kf->pos_ktpt_rec.z, kf->dir_ktpt_rec, kf->distance_rec);
	}
}
...
bool CheckFinishLine (const kmp_t *kmp, kmp_finish_t *kf)
{
	...
	const kmp_ckpt_entry_t *ckpt0 = (kmp_ckpt_entry_t *)kmp->dlist[KMP_CKPT].list;
...
ccp LogFinishLine (kmp_finish_t *kf)
{
	...
	char buf[100];
	uint len = snprintf (
		buf, sizeof (buf), "n=%d/%d, idx=%d/%d", kf->n_ckpt_0, kf->n_ktpt_m, kf->ckpt, kf->ktpt);

	if (kf->valid)
		len += snprintf (buf + len, sizeof (buf) - len, ", dir=%4.2f/%4.2f >%4.2f, dist=%5.3f",
...
void EvaluateUsedPosObj (kmp_usedpos_obj_t *obj)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (rating < WLEVEL_FAIL && value > fail)
			rating = WLEVEL_FAIL;
		else if (rating < WLEVEL_ERROR && value > err)
			rating = WLEVEL_ERROR;
		else if (rating < WLEVEL_WARN && value > warn)
			rating = WLEVEL_WARN;
		else if (rating < WLEVEL_HINT && value > hint)
			rating = WLEVEL_HINT;
		...
		if (i == 1)
		{
			...
			if (rating < WLEVEL_FAIL && ymin < 0.0)
				rating = WLEVEL_FAIL;
			else if (rating < WLEVEL_WARN && ymin < 1.0)
				rating = WLEVEL_WARN;
			else if (rating < WLEVEL_HINT && ymin < 5.0)
				rating = WLEVEL_HINT;
		}
	...
	obj->stretch_active = obj->stretch.x > 1.0 || obj->stretch.y > 1.0 || obj->stretch.z > 1.0;
	...
	if (src)
	{
		...
		for (i = 0; i < 3; i++)
		{
			if (src->rating[i] > WLEVEL_FIXED && src->rating[i] > obj->rating[i])
				src->rating[i] = obj->rating[i] > WLEVEL_FIXED ? obj->rating[i] : WLEVEL_FIXED;
...
ccp GetUsedPosObjSuggestion (const kmp_usedpos_obj_t *obj, bool force, ccp return_if_empty)
{
	if (obj && (force || obj->max_rating >= WLEVEL_HINT))
	{
		if (obj->shift_active && obj->pos_factor_valid)
			return PrintCircBuf (
				"--shift %1.0f,%1.0f,%1.0f & @ITEM-POS-FACTOR=v(%4.2f,%4.2f,%4.2f)", obj->shift.x,
				obj->shift.y, obj->shift.z, obj->pos_factor.x, obj->pos_factor.y,
				obj->pos_factor.z);

		if (obj->shift_active)
			return PrintCircBuf (
				"--shift %1.0f,%1.0f,%1.0f", obj->shift.x, obj->shift.y, obj->shift.z);

		if (obj->pos_factor_valid)
			return PrintCircBuf ("@ITEM-POS-FACTOR=v(%4.2f,%4.2f,%4.2f)", obj->pos_factor.x,
				obj->pos_factor.y, obj->pos_factor.z);
	}
...
void PrintUsedPosObj (FILE *f, int indent, const kmp_usedpos_obj_t *obj)
{
	...
	fprintf (f,
		"%*s"
		"[%u%u%u%u] mm=%1.0f,%1.0f,%1.0fk/%1.0f,%1.0f,%1.0fk, "
		"a=%1.0f,%1.0f,%1.0fk, ss=%1.0f,%1.0f,%1.0fk/%1.2f,%1.2f,%1.2f -> "
		"R=%d,%d,%d, P=%d/%1.2f,%1.2f,%1.2f\n",
		indent, "", obj->valid, obj->useful, obj->shift_active, obj->stretch_active,
		obj->min.x / 1000, obj->min.y / 1000, obj->min.z / 1000, obj->max.x / 1000,
		obj->max.y / 1000, obj->max.z / 1000, obj->allowed.x / 1000, obj->allowed.y / 1000,
		obj->allowed.z / 1000, obj->shift.x / 1000, obj->shift.y / 1000, obj->shift.z / 1000,
		obj->stretch.x, obj->stretch.y, obj->stretch.z, obj->rating[0], obj->rating[1],
		obj->rating[2], obj->pos_factor_valid, obj->pos_factor.x, obj->pos_factor.y,
		obj->pos_factor.z);
}
...
void DetectSpecialKMP (const kmp_t *kmp, // pointer to valid KMP
...
ccp CreateSpecialInfoKMP (kmp_special_t special, bool add_value, ccp return_if_empty)
{
	static char buf[500];
	char *dest = buf;
...
static void print_check_error (check_kmp_t *chk, // valid data structure
...
)
{
	...
	if (!chk->head_printed)
	{
		...
		if (chk->mode & (CMOD_HEADER | CMOD_FORCE_HEADER))
			fprintf (stdlog, "%s* CHECK %s:%s%s\n", chk->col.heading,
				GetNameFF (chk->kmp->fform, 0), chk->kmp->fname, chk->col.reset);
	}
	...
	if (sect != -99 && (chk->sect != sect || chk->index != index))
	{
		...
		{
			fprintf (stdlog, "  %s> Section %s, %s #%u of %u%s", chk->col.info,
				kmp_section_name[sect].name1, kmp_section_name[sect].name2, index,
				chk->kmp->dlist[sect].used, print_name (chk->kmp, sect, index));
...
static void print_item_hint (check_kmp_t *chk, // valid data structure
...
static void check_pt (check_kmp_t *chk, // valid data structure
...
)
{
	...
	if (sect_pt == KMP_ENPT ? chk->kmp->enpt_flag.import : chk->kmp->itpt_flag.import)
		print_check_error (
			chk, CMOD_HINT, sect_pt, -1, 0, "Exported flags detected (extension of tool wkmpt).\n");
}

///////////////////////////////////////////////////////////////////////////////

static void check_ph (check_kmp_t *chk, // valid data structure
...
)
{
	...
	{
		print_check_error (
			chk, CMOD_WARNING, sect_ph, -1, 0, "Empty section => track will freeze.\n");
	}
		...
		if (sect_ph == KMP_ITPH)
		{
			if (ph->next[0] == i)
			{
				if (ph->pt_len == 1)
					...
					print_check_error (chk, CMOD_HINT, sect_ph, i, 0, "Self linked item route.\n");
			}
			else if (ph->pt_len == 1)
				print_check_error (
					chk, CMOD_HINT, sect_ph, i, 0, "Item route with exact 1 point.\n");
		}
	}
...
void print_stat (check_kmp_t *chk, // valid data structure
...
)
{
	...
	if (sect_ph >= 0)
	{
		...
		const uint nt
			= sect_pt == KMP_POTI ? chk->kmp->poti_point.used : chk->kmp->dlist[sect_pt].used;
		print_check_error (chk, CMOD_INFO, -1, -1, 0, "%3u %s%s for %u %s%s defined.\n", nt,
			name_pt, nt == 1 ? "" : "s", nh, name_ph, nh == 1 ? "" : "s");
	}
	else
	{
		...
		if (np || print_null)
			print_check_error (
				chk, CMOD_INFO, -1, -1, 0, "%3u %s%s defined.\n", np, name_pt, np == 1 ? "" : "s");
	}
}
...
)
{
	...
	{
		case ARENA_NONE:
			print_check_error (&chk, CMOD_INFO, -99, -1, 0, "%sRacing track detected.%s\n",
		...
		case ARENA_FOUND:
			chk.t_track = "arena";
			print_check_error (&chk, CMOD_HINT, -99, -1, 0, "%sBattle arena detected.%s\n",
				chk.col.select, chk.col.reset);
			break;
	...
	if (battle_mode != chk.is_arena)
	{
		...
		{
			chk.t_track = "arena";
			info = battle_mode == ARENA_DISPATCH ? "battle arena with dispatch mode"
												 : "battle arena";
		}
		else
		{
		...
		print_check_error (&chk, CMOD_HINT, -99, -1, 0, "%sOption --battle forces mode »%s«!%s\n",
	...
	for (i = 0; i < n; i++, area++)
	{
		...
		if (area->dest_id == 0xff)
			print_check_error (&chk, CMOD_WARNING, KMP_AREA, i, "CAME", "Missing camera link.\n");
	...
	for (i = 0; i < ncame; i++, came++)
	{
		...
		switch (came->type)
		{
			...
			case 6:
				if (IS_M1 (route))
					print_check_error (
						&chk, CMOD_WARNING, KMP_CAME, i, buf, "Missing route link.\n");
				break;
			...
			default:
				print_check_error (
					&chk, CMOD_WARNING, KMP_CAME, i, buf, "Unknown camera type %u.\n", came->type);
		}
	...
	if (!came0count)
		print_check_error (
			&chk, CMOD_HINT, KMP_CAME, -1, "Camera type 0", "Missing a camera with type 0.\n");
	...
	else if (chk.is_arena >= ARENA_FOUND)
	{
		...
		if (n_multi)
		{
			...
			print_check_error (&chk, CMOD_HINT, KMP_KTPT, -1, 0,
				...
				n_multi == 1 ? "" : "s", n_multi == 1 ? "index" : "indices", buf);
		}
	}
	else if (chk.is_arena == ARENA_NONE)
	{
		...
		if (n > max)
			print_check_error (&chk, CMOD_HINT, KMP_KTPT, -1, 0,
				...
				max > 1 ? "first and second are" : "the first is");
	}
	...
	if (!mode0count && chk.is_arena < ARENA_FOUND)
		print_check_error (
			&chk, CMOD_WARNING, KMP_CKPT, -1, 0, "No check point with mode 0 (lap counter).\n");
	...
	n = kmp->dlist[KMP_GOBJ].used;
	if (n >= 200)
		print_check_error (&chk, CMOD_HINT, KMP_GOBJ, -1, 0, "%u global object%s defined.\n", n,
			n == 1 ? "" : "s");
		...
		if (!IsDefinitionObjectId (id))
		{
			...
			if (id >= N_KMP_GOBJ || !ObjectInfo[id].name)
			{
				if (!id)
					...
					print_check_error (
						&chk, CMOD_WARNING, KMP_GOBJ, i, 0, "Unknown object with ID 0x%x.\n", id);
				continue;
			}
			...
			if (IS_M1 (route))
			{
				if (oi->flags & OBF_ALWAYS_ROUTE)
					print_check_error (
						&chk, CMOD_WARNING, KMP_GOBJ, i, oi->name, "Missing route link.\n");
			}
			else
			{
				if (route >= npoti)
	...
	if (n_obj_0 > 0)
		print_check_error (&chk, CMOD_HINT, KMP_GOBJ, -1, 0, "%u object%s with ID 0 found.\n",
			n_obj_0, n_obj_0 > 1 ? "s" : "");
	...
	for (i = 0; i < HAVEKMP__N; i++)
		if (special[i])
		{
			print_check_error (&chk, CMOD_HINT, KMP_GOBJ, -1, 0, "%u special object%s: %s.\n",
	...
	if (n_coin_start || n_coin_respawn)
		print_check_error (&chk, CMOD_INFO, KMP_GOBJ, -1, 0,
			...
			n_coin_start, n_coin_start == 1 ? "" : "s", n_coin_respawn,
			n_coin_respawn == 1 ? "" : "s");
	...
	if (chk.is_arena >= ARENA_FOUND)
	{
		if (!n)
			...
			print_check_error (&chk, n < 4 ? CMOD_WARNING : CMOD_HINT, KMP_MSPT, -1, 0,
	...
	if (!n)
		print_check_error (&chk, CMOD_WARNING, KMP_STGI, -1, 0, "No element defined.\n");
	else if (n > 1)
		print_check_error (&chk, CMOD_WARNING, KMP_STGI, -1, 0, "%u elements defined.\n", n);
	else
	{
		const kmp_stgi_entry_t *stgi = (kmp_stgi_entry_t *)kmp->dlist[KMP_STGI].list;

		if (stgi->lap_count != 3)
			print_check_error (&chk, CMOD_HINT, KMP_STGI, 0, "STGI", "Lap counter changed to %u.\n",
				stgi->lap_count);
		else if (mode & CMOD_INFO)
			print_check_error (
				&chk, CMOD_INFO, KMP_STGI, 0, "STGI", "Lap counter is to %u.\n", stgi->lap_count);
		...
		if (stgi->narrow_start > 1)
			...
			print_check_error (&chk, CMOD_INFO, KMP_STGI, 0, "STGI", "Start mode is '%s' (%u).\n",
		...
		if (stgi->speed_mod)
			...
			print_check_error (
				&chk, CMOD_INFO, KMP_STGI, 0, "STGI", "No speed modifier set (0000/hex).\n");
	}
	...
	if (lex_hint)
	{
		...
		if (obj->shift_active && obj->pos_factor_valid)
		{
			fprintf (stdlog,
				...
				chk.col.info, check_bosugl, chk.col.reset, obj->shift.x, obj->shift.y, obj->shift.z,
			...
			if (obj != &up.orig && up.orig.pos_factor_valid)
				fprintf (stdlog,
					...
					chk.col.info, check_bosugl, chk.col.reset, up.orig.pos_factor.x,
					up.orig.pos_factor.y, up.orig.pos_factor.z);
		}
		else if (obj->shift_active)
			fprintf (stdlog,
				"%s%s%s"
				"Shift the whole track by --shift %1.0f,%1.0f,%1.0f\n",
				chk.col.info, check_bosugl, chk.col.reset, obj->shift.x, obj->shift.y,
				obj->shift.z);
		else if (obj->pos_factor_valid)
			fprintf (stdlog,
				"%s%s%s"
				"Setup LEX/SET1 with @ITEM-POS-FACTOR=v(%4.2f,%4.2f,%4.2f)\n",
				chk.col.info, check_bosugl, chk.col.reset, obj->pos_factor.x, obj->pos_factor.y,
				obj->pos_factor.z);
	}
	...
	{
		static ccp hint = " => see https://szs.wiimm.de/cmd/wkmpt/check#desc for more info.";
		...
		if (chk.warn_count)
			fprintf (stdlog, " %s=> %s%u warning%s%s%s%s for %s:%s\n%s%s%s\n\n", chk.col.heading,
				...
				ibuf, GetNameFF (kmp->fform, 0), kmp->fname, chk.col.heading, hint, chk.col.reset);
...
enumError cmd_stgi ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		else if (opt_ignore > 1 && lap_count == 1 && stgi->pole_pos < 2 && stgi->narrow_start < 2
			&& !stgi->speed_mod && stgi->lap_count == 3)
		{
			ResetKMP (&kmp);
			continue;
		}
		else
		{
			...
			printf (" %s%3u%s",
				stgi->lap_count == 3	  ? ""
					: stgi->lap_count < 3 ? colout->info
										  : colout->hint,
				stgi->lap_count, colout->reset);
		}
		printf (
			" %s%3u%s : %s\n", lap_count == 1 ? "" : colout->hint, lap_count, colout->reset, arg);
		ResetKMP (&kmp);
	}
	if (print_header)
		printf ("%.*s\n\n", 79, Minus300);
...
enumError cmd_ktpt ()
{
	...
	for (int argi = 0; argi < plist.used; argi++)
	{
		...
		if (!kf.valid)
		{
			...
			{
				if (kf.ckpt < 0)
					ERROR0 (ERR_WARNING, "No relevant lap counter (CKPT) found: %s\n", arg);
				else if (kf.ktpt < 0)
					ERROR0 (ERR_WARNING, "No relevant start points (KTPT) found: %s\n", arg);
			}
			continue;
		}
		...
		printf ("%s> Path and file name:             %s%s\n"
			...
			colout->caption, arg, colout->reset, kf.n_ktpt_m, kf.ktpt, kf.n_ckpt_0, kf.ckpt,
			kf.pos_ktpt.x, kf.pos_ktpt.z, kf.pos_ktpt.z, kf.dir_ktpt);

		if (kf.ckpt_opt >= 0 && (kf.ckpt_opt != kf.ckpt || kf.hint))
			printf ("%sOptimized start point (pos/dir):%12.3f %11.3f %11.3f /%8.2f°%s\n",
				colout->hint, kf.pos_ktpt_opt.x, kf.pos_ktpt_opt.z, kf.pos_ktpt_opt.z,
				kf.dir_ktpt_opt, colout->reset);

		printf ("Relevant check point (pos/dir): %12.3f %23.3f /%8.2f°\n"
				"Distance between both points: %s%14.3f%s%25s/%s%8.2f°%s\n",
			kf.pos_ckpt.x, kf.pos_ckpt.z, kf.dir_ckpt, kf.hint & 2 ? colout->warn : "", kf.distance,
			colout->reset, "", kf.hint & 1 ? colout->warn : "", kf.dir_delta, colout->reset);
		...
		if (kf.enpt_rec >= 0)
			printf ("\n"
				...
				colout->hint, kf.enpt_rec, colout->reset, kf.pos_ktpt_rec.x, kf.pos_ktpt_rec.y,
				kf.pos_ktpt_rec.z, kf.dir_ktpt_rec, kf.pos_ktpt_rec.x, kf.pos_ktpt_rec.z);
		...
		if (long_count > 0)
		{
			...
			printf ("\n"
				...
				colout->heading, colout->reset, colout->heading, fw1, ThinLine300_3, fw2,
				ThinLine300_3, fw3, ThinLine300_3, fw4, ThinLine300_3, colout->heading,
			...
			for (ik = 0; ik < nk; ik++, ktpt++)
			{
				...
				if (!ik || kf.n_ckpt_0 > 1)
					printf ("%s├%.*s┼%.*s┼%.*s┼%.*s┤%s\n", colout->heading, fw1, ThinLine300_3, fw2,
						ThinLine300_3, fw3, ThinLine300_3, fw4, ThinLine300_3, colout->reset);
				...
				for (ic = 0; ic < nc; ic++, ckpt++)
				{
					...
					if (kf.enpt_rec >= 0)
					{
						snprintf (opt_buf, sizeof (opt_buf), "%1.0f,%1.0f", kf.pos_ktpt_rec.x,
							kf.pos_ktpt_rec.z);
						opt = opt_buf;
					}

					printf ("%s%3u %7.0f %6.0f %7.0f %3.0f "
							"%s%3u %7.0f %7.0f %3.0f %s %s%6.0f %s%7.2f %s%s %-15s%s\n",
						sep, ik, kf.pos_ktpt.x, kf.pos_ktpt.y, kf.pos_ktpt.z, kf.dir_ktpt, sep, ic,
						kf.pos_ckpt.x, kf.pos_ckpt.z, kf.dir_ckpt, sep,
						kf.warn & 2		  ? colout->warn
							: kf.hint & 2 ? colout->hint
										  : "",
						kf.distance,
						kf.hint & 1		  ? colout->warn
							: kf.hint & 1 ? colout->hint
										  : colout->reset,
						kf.dir_delta, sep, ik == kf.ktpt && ic == kf.ckpt ? colout->hint : "", opt,
						sep);
				}
			}
