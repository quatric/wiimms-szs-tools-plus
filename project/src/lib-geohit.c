...
void SetupBZ2MgrGEOHIT (geohit_t *geohit, file_format_t fform, bool reload_bin)
{
	DASSERT (geohit);
	if (geohit->bin)
		ScanObjType (&geohit->obj_mgr.otype, geohit->bin, geohit->bin_size, geohit->bin_size);
	...
	PRINT0 ("GEOHIT: ff=%s,%s, nr=%03d, np=%02zd, rs=%02u, kart=%d, obj=%d\n",
		GetNameFF (0, geohit->fform_source), GetNameFF (0, geohit->obj_mgr.otype.fform),
#if USE_OBJECT_MANAGER < 2
		geohit->n_rec,
#else
		-1,
#endif
		geohit->obj_mgr.otype.row_size / sizeof (s16) - 1, geohit->obj_mgr.otype.row_size,
...
geohit_bin_t *GetBinGEOHIT (cvp data, uint index)
{
	...
	return index < nr ? (geohit_bin_t *)((u8 *)data + 4 + index * rsize) : 0;
}
...
static const KeywordTab_t geohit_section_name[] = { { GEOHIT_IGNORE, "END", 0, 0 },
	{ GEOHIT_SETUP, "SETUP", 0, 0 }, { GEOHIT_OBJECTS, "OBJECTS", 0, 0 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static enumError ScanTextSETUP (geohit_t *geohit, // GEOHIT data structure
...
static enumError ScanTextOBJECTS (geohit_t *geohit, // GEOHIT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (geohit);
	...
	for (;;)
	{
		...
		if (!obj)
		{
			if (si->no_warn <= 0)
			{
				...
				ERROR0 (ERR_WARNING, "Object 0x%03lx not found: [%s @%u]: %.*s\n", oid, sf->name,
...
)
{
	...
	switch (fform)
	{
		...
		default:
			...
			return ERROR0 (
				ERR_INVALID_DATA, "No GEOHIT file: %s\n", geohit->fname ? geohit->fname : "?");
	}
...
)
{
	...
	if (geohit->obj_mgr.otype.is_kart)
	{
		...
		if (!tiny)
			fprintf (
				F.f, text_geohit_kart_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);

#if USE_OBJECT_MANAGER > 1
		WriteObjMgrToSetupSection (F.f, &geohit->obj_mgr, long_count > 0, tiny, explain);
#endif
	}
	else
	{
		...
		if (!tiny)
			fprintf (
				F.f, text_geohit_item_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);
