...
static int resolve_aes_key ( const char *arg, uint8_t key[16] )
{
	...
	fclose(f);
	if (!ok)
	    fprintf(stderr,"wwc24crypt: %s exists but is neither a 16-byte key "
...
static int cmd_wc24_decrypt ( const char *infile, const char *outfile, const char *keyarg )
{
    uint8_t key[16];
    if ( !resolve_aes_key(keyarg,key) )
    {
	fprintf(stderr,"wwc24crypt: can't resolve AES key from '%s'\n",keyarg);
	return 1;
    }
    ...
    if ( size < 320 )
    {
	fprintf(stderr,"wwc24crypt: %s is too small to be a WC24 file (%ld bytes, need >= 320)\n",infile,size);
	FREE(file);
	return 1;
    }
    ...
    fclose(out);
    FREE(file);
    printf("wwc24crypt: decrypted %s -> %s (%zu bytes)\n",infile,outfile,data_size);
    return 0;
}

static int cmd_wc24_encrypt ( const char *infile, const char *outfile,
			       const char *aes_keyarg, const char *rsa_pem_path,
			       const char *ivarg )
{
    uint8_t aes_key[16];
	...
	{
	    fprintf(stderr,"wwc24crypt: can't resolve IV from '%s'\n",ivarg);
	    return 1;
	}
    }
    else
	...
	{
	    fprintf(stderr,"wwc24crypt: can't read random IV from /dev/urandom\n");
	    if (rf) fclose(rf);
	    return 1;
	}
	fclose(rf);
    ...
    if ( !pem )
    {
	fprintf(stderr,"wwc24crypt: can't read RSA private key %s\n",rsa_pem_path);
	return 1;
    }
    rsa_key_t rsakey;
    ...
    if (!rsa_ok)
    {
	fprintf(stderr,"wwc24crypt: %s is not a PKCS#1 'RSA PRIVATE KEY' PEM "
    ...
    {
	fprintf(stderr,"wwc24crypt: RSA signing failed\n");
	FREE(sig); FREE(data);
	return 1;
    }
    ...
    uint8_t pad[3] = {0,0,0};
    uint8_t reserved[32]; memset(reserved,0,sizeof(reserved));
    ...
    FREE(sig);
    FREE(enc);
    printf("wwc24crypt: encrypted+signed %s -> %s (%ld byte payload, %d byte signature)\n",
	   infile,outfile,data_size,rsakey.size);
    return 0;
}
...
static const uint8_t kat_rsa_sig[64] =
{
    0x96,0xb1,0x49,0xa4,0x5e,0xb6,0x01,0x1c,0x10,0x5b,0xb5,0x7c,0xd6,0x50,0x69,0x39,
    0x60,0x95,0x5c,0x7e,0x5b,0x28,0x90,0xbc,0x60,0x77,0x5c,0xca,0x16,0xe6,0x69,0xa8,
    0x35,0xe7,0x13,0x61,0x43,0x25,0xb6,0x1a,0xe9,0x8d,0x66,0x35,0xb7,0xf6,0x8b,0xdf,
    0x87,0xeb,0x86,0x5c,0x2a,0x30,0x4b,0x13,0xcd,0x7e,0xce,0x59,0x3d,0xea,0xb7,0x35
};
...
static int cmd_selftest ( void )
{
    ...
    uint8_t ofb_key[16] = {0x2b,0x7e,0x15,0x16,0x28,0xae,0xd2,0xa6,
			    0xab,0xf7,0x15,0x88,0x09,0xcf,0x4f,0x3c};
    uint8_t ofb_iv[16]  = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
			    0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};
    uint8_t ofb_pt[32] = {0x6b,0xc1,0xbe,0xe2,0x2e,0x40,0x9f,0x96,0xe9,0x3d,0x7e,0x11,0x73,0x93,0x17,0x2a,
			   0xae,0x2d,0x8a,0x57,0x1e,0x03,0xac,0x9c,0x9e,0xb7,0x6f,0xac,0x45,0xaf,0x8e,0x51};
    const uint8_t ofb_expect[32] = {
    ...
    int ofb_rt_ok = !memcmp(ofb_pt,(uint8_t[32]){
	0x6b,0xc1,0xbe,0xe2,0x2e,0x40,0x9f,0x96,0xe9,0x3d,0x7e,0x11,0x73,0x93,0x17,0x2a,
	0xae,0x2d,0x8a,0x57,0x1e,0x03,0xac,0x9c,0x9e,0xb7,0x6f,0xac,0x45,0xaf,0x8e,0x51},32);
    ...
    rsa_key_t rsakey;
    int rsa_load_ok = RSA_LoadPrivateKeyPEM(&rsakey,(const uint8_t*)kat_rsa_pem,strlen(kat_rsa_pem));
...
static void usage ( const char *prog )
{
    printf("wwc24crypt - WC24 (Wii Connect24) file encrypt/decrypt utility\n"
	   "Matches RiiConnect24/wc24-tools' wc24decrypt.py/wc24encrypt.py logic:\n"
...
int main ( int argc, char **argv )
{
	...
	return cmd_wc24_decrypt(argv[2],argv[3],argv[4]);
    }

    if ( !strcmp(argv[1],"wc24-encrypt") )
    {
	if ( argc != 6 && argc != 7 ) { usage(argv[0]); return 1; }
	return cmd_wc24_encrypt(argv[2],argv[3],argv[4],argv[5],argc==7?argv[6]:0);
    }
