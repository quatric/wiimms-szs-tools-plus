...
)
{
	...
	PrintScriptVars (&ap->ps, 1,
		...
		"file_type=\"%s\"\n"
		"analyze=\"%s\"\n",
		buf, GetNameFF (ap->szs.fform_file, ap->szs.fform_arch), analyze);
}
...
)
{
	...
	if (szs->course_kmp_data)
	{
		...
		as->sha1_kmp_slot = as->sha1_kmp_norm_slot = GetSha1Slot (SHA1T_KMP, sha1_data.hash);

		kmp_t kmp;
		InitializeKMP (&kmp);
		kmp.check_only = true;
		kmp.lexinfo = &as->lexinfo;
		const enumError err = ScanKMP (&kmp, false, szs->course_kmp_data, szs->course_kmp_size, 0);
		if (err <= ERR_WARNING)
		{
			if (kmp.stgi)
			{
		...
		as->ctid_list = GetCtIdByList(as->sha1_szs_norm);
			...
			if (kmp.dlist[KMP_STGI].used > 0)
			{
				...
				if (!szs->is_arena)
				{
					if (as->ckpt0_count > 1)
					{
						...
						ct_dest = snprintfE (ct_dest, ct_end, ",%ulap%s", as->lap_count,
							as->lap_count == 1 ? "" : "s");
				}
			...
			snprintf (as->gobj_info, sizeof (as->gobj_info), "%d %d %d %d", ap.ag.n_gobj,
				ap.n_version, ap.n_res_std, ap.n_res_ext);
			ResetPFlagScenarios (&ap);
		}

		DetectSpecialKMP (&kmp, szs->have.kmp);
		if (szs->have.kmp[HAVEKMP_WOODBOX_HT] || szs->have.kmp[HAVEKMP_MUSHROOM_CAR]
			|| szs->have.kmp[HAVEKMP_PENGUIN_POS] || szs->have.kmp[HAVEKMP_EPROP_SPEED]
			|| szs->have.kmp[HAVEKMP_GOOMBA_SIZE])
		{
			ct_dest = StringCopyE (ct_dest, ct_end, ",gobj");
		}

		if (szs->have.kmp[HAVEKMP_X_PFLAGS] || szs->have.kmp[HAVEKMP_X_COND]
			|| szs->have.kmp[HAVEKMP_X_DEFOBJ] || szs->have.kmp[HAVEKMP_X_RANDOM])
		{
	...
	if (szs->course_lex_data)
	{
		...
		if (szs->have.lex_apply_otl)
		{
			...
			else
			{
				...
				if (sec)
					ct_dest = snprintfE (
						ct_dest, ct_end, ",otl=%um%02u", szs->have.lex_apply_otl / 60, sec);
				else
					ct_dest = snprintfE (ct_dest, ct_end, ",otl=%um", szs->have.lex_apply_otl / 60);
			}
		}
	}
...
enumError ExecAnalyzeSZS (analyze_param_t *ap)
{
		...
		if (end)
		{
			...
			for (ccp ptr = name_attrib; ptr < end;)
			{
				...
				if (!memcmp (ptr, "id=", 3))
					...
					{
						case 4:
							if (!memcmp (ptr, "edit", 4))
								szs->have.attrib |= 1 << HAVEATT_EDIT;
							break;

						case 5:
							if (!memcmp (ptr, "cheat", 5))
								szs->have.attrib |= 1 << HAVEATT_CHEAT;
							break;

						case 7:
							if (!memcmp (ptr, "reverse", 7))
								szs->have.attrib |= 1 << HAVEATT_REVERSE;
							break;
					}
				ptr = sep + 1;
			}
	...
	PrintScriptVars (&ap->ps, 0,
		...
		as.slotana.slot_info, as.slotinfo.slot_attrib, as.slotinfo.race_slot, as.slotinfo.race_info,
		as.slotinfo.arena_slot, as.slotinfo.arena_info, as.slotinfo.edit_slot,
		as.slotinfo.edit_info, as.slotinfo.music_index, as.slotinfo.music_info,
		as.lexinfo.item_factor.x, as.lexinfo.item_factor.y, as.lexinfo.item_factor.z,
		as.used_pos.orig.rating[0], WarnLevelNameLo[as.used_pos.orig.rating[0]],
		as.used_pos.orig.min.x, as.used_pos.orig.max.x,
		fabsf (as.used_pos.orig.max.x - as.used_pos.orig.min.x),
		(as.used_pos.orig.min.x + as.used_pos.orig.max.x) / 2, as.used_pos.orig.rating[1],
		WarnLevelNameLo[as.used_pos.orig.rating[1]], as.used_pos.orig.min.y, as.used_pos.orig.max.y,
		fabsf (as.used_pos.orig.max.y - as.used_pos.orig.min.y),
		(as.used_pos.orig.min.y + as.used_pos.orig.max.y) / 2, as.used_pos.orig.rating[2],
		WarnLevelNameLo[as.used_pos.orig.rating[2]], as.used_pos.orig.min.z, as.used_pos.orig.max.z,
		...
		as.kmp_finish.warn		 ? "warn"
			: as.kmp_finish.hint ? "hint"
								 : "ok",
		as.kmp_finish.distance, as.kmp_finish.dir_delta, as.gobj_info,
		GetStatusMissedFile (szs->missed_file), GetStatusMissedFile (szs->modified_file),
		...
		CreateFeatureInfoLEX (szs->have.lex_feat, true, ""), szs->warn_bits,
		GetWarnSZSNames (szs->warn_bits, ' '), as.have_common,
		as.ctid_list ? as.ctid_list : as.ctid_attrib, as.ctid_list, as.ctid_attrib,
		as.ct_attrib + 1, name_attrib_len, name_attrib);
...
enumError ExecAnalyzeLECODE (analyze_param_t *ap)
{
	...
	if (valid)
	{
		PrintScriptVars (&ap->ps, 0,
			...
			ana.szs_recommended, ana.szs_recommended ? DecodeVersion (ana.szs_recommended) : "-",
			ana.edit_version, ana.edit_version ? DecodeVersion (ana.edit_version) : "-");
	}
