...
#if QLZ_COMPRESSION_LEVEL <= 2
static __inline void update_hash (qlz_hash_decompress h[QLZ_HASH_VALUES],
	unsigned char counter[QLZ_HASH_VALUES], const unsigned char *s)
{
...
static void update_hash_upto (qlz_hash_decompress h[QLZ_HASH_VALUES],
	unsigned char counter[QLZ_HASH_VALUES], unsigned char **lh, const unsigned char *max)
{
	while (*lh < max)
	{
		(*lh)++;
...
static size_t qlz_compress_core (const unsigned char *source, unsigned char *destination,
	size_t size, qlz_hash_compress hashtable[QLZ_HASH_VALUES],
	...
	while (src <= last_matchstart)
	{
			...
			if (cached == 0 && src - o > MINOFFSET && hash_counter[hash] != 0)
			{
				...
				else
				{
					...
					if (*(o + (src - old_src)) == *src)
					{
						...
						if (*(o + (src - old_src)) == *src)
						{
							size_t remaining = (last_byte - UNCOMPRESSED_END - (src - 5) + 1) > 255
								? 255
								: (last_byte - UNCOMPRESSED_END - (src - 5) + 1);
							src++;
							while (*(o + (src - old_src)) == *src
								&& (size_t)(src - old_src) < remaining)
								src++;
						}
					}
...
#elif QLZ_COMPRESSION_LEVEL >= 2
		{
			...
			size_t remaining = (last_byte - UNCOMPRESSED_END - src + 1) > 255
				? 255
				: (last_byte - UNCOMPRESSED_END - src + 1);
			...
			if (offset2 < src - MINOFFSET && c > 0
				&& ((fast_read (offset2, 3) ^ fetch) & 0xffffff) == 0)
			{
				matchlen = 3;
			...
			for (k = 1; k < QLZ_POINTERS && c > k; k++)
			{
				...
				if (*(src + matchlen) == *(o + matchlen)
					&& ((fast_read (o, 3) ^ fetch) & 0xffffff) == 0 && o < src - MINOFFSET)
#endif
				{
					m = 3;
...
static size_t qlz_decompress_core (const unsigned char *source, unsigned char *destination,
	size_t size, qlz_hash_decompress hashtable[QLZ_HASH_VALUES],
	unsigned char hash_counter[QLZ_HASH_VALUES], unsigned char *history, const char *source_2)
{
	const unsigned char *src = source;
	...
	const unsigned char *last_matchstart
		= last_destination_byte - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END;
	unsigned char *last_hashed = destination - 1;
	const unsigned char *last_source_byte
		= (const unsigned char *)source_2 + qlz_size_compressed (source_2) - 1;
...
#if QLZ_COMPRESSION_LEVEL <= 2
				update_hash_upto (hashtable, hash_counter, &last_hashed,
					last_destination_byte - 3); // todo, use constant
#endif
				return size;
			}
		}
...
{
	unsigned char *scratch_aligned = (unsigned char *)scratch_compress + QLZ_ALIGNMENT_PADD
	...
	unsigned char *hash_counter
		= (unsigned char *)hashtable + sizeof (qlz_hash_compress[QLZ_HASH_VALUES]);
...
#endif
	{
		...
		r = base
			+ qlz_compress_core ((const unsigned char *)source, (unsigned char *)destination + base,
				size, hashtable, hash_counter);
#if QLZ_STREAMING_BUFFER > 0
		reset_state (hash_counter);
#endif
		if (r == base)
	...
	else
	{
		...
		r = base
			+ qlz_compress_core ((const unsigned char *)streambuffer + *buffersize,
				(unsigned char *)destination + base, size, hashtable, hash_counter);
...
size_t qlz_decompress (const char *source, void *destination, char *scratch_compress)
{
	...
	unsigned char *hash_counter
		= (unsigned char *)hashtable + sizeof (qlz_hash_decompress[QLZ_HASH_VALUES]);
#if QLZ_STREAMING_BUFFER > 0
	unsigned char *streambuffer = hash_counter + QLZ_HASH_VALUES;
#endif
#endif
...
#endif
	{
		if ((*source & 1) == 1)
		{
			...
			reset_state (hash_counter);
#endif
			dsiz = qlz_decompress_core ((const unsigned char *)source + headerlen,
				(unsigned char *)destination, dsiz, hashtable, hash_counter,
	...
	else
	{
		...
		{
			dsiz = qlz_decompress_core ((const unsigned char *)source + headerlen,
				streambuffer + *buffersize, dsiz, hashtable, hash_counter, streambuffer, source);
		}
		else
		{
...
int main (int argc, char *argv[])
{
	...
	{
		offset = (unsigned int)strtol (argv[2], NULL, 0);
		arg_idx = 3;
	}

	if (argc < arg_idx + 2)
	{
		printf ("Usage: %s [-o offset] infile outfile\n", argv[0]);
		return 1;
	}

	ifile = fopen (argv[arg_idx], "rb");
	ofile = fopen (argv[arg_idx + 1], "wb");
	...
	len = qlz_size_decompressed (src + offset);
	dst = (char *)malloc (len);
	...
	len = qlz_decompress (src + offset, dst, scratch);
