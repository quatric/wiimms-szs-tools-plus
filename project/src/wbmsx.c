...
int main ( int argc, char **argv )
{
    ...
    if (!sf) { fprintf(stderr,"wbmsx: can't open script %s\n",argv[1]); return 1; }
    ...
    {
	if ( line[strlen(line)-0>0 ? strlen(line)-1 : 0] == '\r' )
    ...
    if (!inf) { fprintf(stderr,"wbmsx: can't open input %s\n",argv[2]); return 1; }
    fseek(inf,0,SEEK_END);
    long fsize = ftell(inf);
    fseek(inf,0,SEEK_SET);
    uint8_t *fdata = (uint8_t*)MALLOC(fsize>0?fsize:1);
    if ( fsize>0 && fread(fdata,1,fsize,inf) != (size_t)fsize )
    {
	fprintf(stderr,"wbmsx: short read on %s\n",argv[2]);
	fclose(inf);
	return 1;
    }
    fclose(inf);
