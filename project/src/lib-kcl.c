...
static const KeywordTab_t opt_kcl_tab[] = { { 0, "CLEAR", "RESET", KCLMD_M_ALL | KCLMD_F_HIDE },
	{ KCLMD_M_DEFAULT, "DEFAULT", 0, KCLMD_M_ALL | KCLMD_F_HIDE },

	{ KCLMD_FAST, "FAST", 0, 0 }, { KCLMD_ROUND, "ROUND", 0, 0 },
	{ KCLMD_NORMALS, "NORMALS", 0, 0 }, { KCLMD_MTL, "MTL", "MATERIALS", 0 },
	{ KCLMD_WIIMM, "WIIMM", 0, 0 }, { KCLMD_TRIANGLES, "TRIANGLES", 0, 0 },
	{ KCLMD_OUT_SWAP, "OUT-SWAP", "OUTSWAP", 0 }, { KCLMD_OUT_SWAP, "OSWAP", 0, 0 },

	{ KCLMD_G, "G", 0, 0 }, { KCLMD_USEMTL, "USEMTL", 0, 0 }, { KCLMD_CLIP, "CLIP", 0, 0 },
	{ KCLMD_IN_SWAP, "IN-SWAP", "INSWAP", 0 }, { KCLMD_IN_SWAP, "ISWAP", 0, 0 },
	{ KCLMD_AUTO, "AUTO", 0, 0 },

	{ KCLMD_CENTER, "CENTER", 0, 0 }, { KCLMD_ADD_ROAD, "ADD-ROAD", "ADDROAD", 0 },

	{ KCLMD_HEX4, "HEX4", "H4", 0 }, { KCLMD_HEX23, "HEX23", "H23", 0 },
	...
	{ KCLMD_DROP_UNUSED | KCLMD_DROP_AUTO, "DROP-UNUSED", "DROPUNUSED", 0 },
	{ KCLMD_DROP_FIXED | KCLMD_DROP_AUTO, "DROP-FIXED", "DROPFIXED", 0 },
	{ KCLMD_DROP_INVALID | KCLMD_DROP_AUTO, "DROP-INVALID", "DROPINVALID", 0 },
	...
	{ KCLMD_NEW, "NEW", 0, 0 }, { KCLMD_SORT, "SORT", 0, 0 },
	...
	{ KCLMD_LOG, "LOG", 0, 0 }, { KCLMD_SILENT, "SILENT", 0, 0 }, { KCLMD_POSLEN, "POSLEN", 0, 0 },
	{ KCLMD_INPLACE, "INPLACE", 0, 0 },

	{ KCLMD_F_SCRIPT, "SCRIPT", 0, 0 }, { KCLMD_TEST, "TEST", "T", 0 },
...
int ScanOptKcl (ccp arg)
{
	...
	s64 stat = ScanKeywordList (arg, opt_kcl_tab, 0, true, 0, KCL_MODE, "Option --kcl", ERR_SYNTAX);
	if (stat != -1)
	{
		SetKclMode (stat);
		return 0;
...
void SetupKCL ()
{
	if (opt_tiny > 0)
	{
		const kcl_mode_t tiny = ((kcl_mode_t)opt_tiny << KCLMD_S_TINY) & KCLMD_M_TINY;
...
CommandKCL_t GetCommandKCL (ccp name // name to scan, upper case is assumed
)
{
	...
	if (!cmdtab.used)
	{
		...
		static const struct kcmdtab_t def_tab[] = { { "V", KCMD_V }, { "VN", KCMD_VN },
			{ "F", KCMD_F }, { "G", KCMD_G }, { "USEMTL", KCMD_USEMTL }, { "TRI", KCMD_TRI },
...
void LoadParametersKCL (ccp log_prefix // not NULL:
...
)
{
	...
	if (KCL_MAX.x < KCL_MIN.x)
		KCL_MAX.x = KCL_MIN.x;
	if (KCL_MAX.y < KCL_MIN.y)
		KCL_MAX.y = KCL_MIN.y;
	if (KCL_MAX.z < KCL_MIN.z)
		KCL_MAX.z = KCL_MIN.z;
	...
	if (!KCL_ACTION_LOG (0, "Global KCL Modes: %s\n", GetKclMode ()) && log_prefix
		&& KCL_MODE != KCLMD_M_DEFAULT && !(KCL_MODE & KCLMD_SILENT))
	{
	...
	PRINT ("  > BITS=%u, MAX_TRI=%u, MAX_DEPTH=%u, CUBE_SIZE=%u..%u\n", KCL_BITS, KCL_MAX_TRI,
		KCL_MAX_DEPTH, KCL_MIN_SIZE, KCL_MAX_SIZE);
	PRINT ("  > CLIP: %11.3f %11.3f %11.3f, enabled=%d \n", KCL_CLIP.x, KCL_CLIP.y, KCL_CLIP.z,
		(KCL_MODE & KCLMD_CLIP) != 0);
...
)
{
	...
	if (kcl)
	{
		PRINT ("CheckMetricsTriData(%u), invalid: %u -> %u, done in %s\n", n, kcl->tri_inv_count,
			invalid_count, PrintTimerUSec (0, 0, duration, 6));
...
)
{
	...
	for (; n-- > 0; td++)
	{
		if (td->status & TD_INVALID || !isnormal (td->length) || fabs (td->length) < opt_tri_height)
		{
		invalid:
...
#define XPRINT noPRINT
	...
	for (; n-- > 0; td++)
	{
		...
		CrossProd3 (normal[0], diff_1_0, diff_2_0);
		Unit3 (normal[0]);
		CrossProd3 (normal[1], normal[0], diff_2_0);
		Unit3 (normal[1]);
		CrossProd3 (normal[2], diff_1_0, normal[0]);
		Unit3 (normal[2]);
		CrossProd3 (normal[3], normal[0], diff_1_2);
		Unit3 (normal[3]);
		...
		if (!isnormal (td->length) || fabs (td->length) < opt_tri_height || !isfinite (normal[0].x)
			|| !isfinite (normal[0].y) || !isfinite (normal[0].z) || !isfinite (normal[1].x)
			|| !isfinite (normal[1].y) || !isfinite (normal[1].z) || !isfinite (normal[2].x)
			|| !isfinite (normal[2].y) || !isfinite (normal[2].z) || !isfinite (normal[3].x)
			|| !isfinite (normal[3].y) || !isfinite (normal[3].z))
		{
			if (kcl)
			{
...
)
{
	...
	{
		CalcNormalsTriData (kcl, (kcl_tridata_t *)kcl->tridata.list, kcl->tridata.used);
		kcl->norm_valid = true;

#ifdef TEST0
		// verify valid triangels
		PRINT (" verify valid triangels\n");
		CalcPointsTriData ((kcl_tridata_t *)kcl->tridata.list, kcl->tridata.used, 0, false);
#endif
	}
}
...
kcl_tridata_t *GetTridataKCL (kcl_t *kcl, // NULL or valid kcl
	uint tri_index // triangle index
)
{
	return kcl && tri_index < kcl->tridata.used ? (kcl_tridata_t *)kcl->tridata.list + tri_index
												: 0;
}
...
)
{
	...
	if (!kcl->limit_warning)
	{
		...
		ERROR0 (ERR_SEMANTIC, "Can't add new triangles, because limit (%u) reached!",
			KCL_MAX_TRIANGLES);
	}
	return 0;
}
	...
	if (td)
	{
		...
		if (par->norm)
		{
			...
			if (norm_len > 1e-3)
			{
				...
				PRINT ("NORM/DIR: %8.5f %8.5f %8.5f | %11.3f\n", norm_unit.x, norm_unit.y,
					norm_unit.z, norm_len);
...
)
{
	...
	if (n_pt <= 4)
		return n_pt == 3 ? AppendTriangleKCL (par, pt1, (double3 *)((u8 *)pt1 + pt_delta),
							   (double3 *)((u8 *)pt1 + 2 * pt_delta))
			: n_pt == 4
			? AppendQuadrilateralKCL (par, pt1, (double3 *)((u8 *)pt1 + pt_delta),
				  (double3 *)((u8 *)pt1 + 2 * pt_delta), (double3 *)((u8 *)pt1 + 3 * pt_delta))
			: 0;
	...
	while (n_pt > 3)
	{
		...
		AppendTriangleKCL (par, tab[found].pt, tab[found1].pt, tab[(found + 2) % n_pt].pt);
...
kcl_tridata_t *AppendOctahedronKCL (kcl_t *kcl, // valid KCL data structure
	u32 kcl_flag, // value of kcl flag
...
kcl_tridata_t *AppendCuboidKCL (kcl_t *kcl, // valid KCL data structure
	u32 kcl_flag, // value of kcl flag
...
kcl_tridata_t *AppendPyramidKCL (kcl_t *kcl, // valid KCL data structure
	u32 kcl_flag, // value of kcl flag
	...
	else
	{
		...
		for (i = 0; i < n_pt; i++)
		{
			...
			noPRINT ("%6.1f %6.1f %6.1f / %6.1f %6.1f %6.1f / %6.1f %6.1f %6.1f\n", apex->x,
				apex->y, apex->z, pt->x, pt->y, pt->z, next->x, next->y, next->z);
			AppendTriangleKCL (&par, apex, pt, next);
			pt = next;
		}
	}
...
kcl_tridata_t *AppendPrismKCL (kcl_t *kcl, // valid KCL data structure
	u32 kcl_flag, // value of kcl flag
...
)
{
	...
	else
	{
		...
		for (i = 0; i < n_side; i++, p1++, p2++)
		{
			...
			double a;
			a = fcos * n[1].x + fsin * n[2].x;
			p1->x = base1.x + a;
			p2->x = base2.x + a;
			a = fcos * n[1].y + fsin * n[2].y;
			p1->y = base1.y + a;
			p2->y = base2.y + a;
			a = fcos * n[1].z + fsin * n[2].z;
			p1->z = base1.z + a;
			p2->z = base2.z + a;
			if (i > 0)
...
kcl_tridata_t *AppendCylinderKCL (kcl_t *kcl, // valid KCL data structure
	u32 kcl_flag, // value of kcl flag
...
kcl_tridata_t *AppendJoistKCL (kcl_t *kcl, // pointer to valid KCL
...
)
{
	...
	for (p = 0; p < 3; p++)
	{
		...
		delta[2].v[p] = d13 + dx;
		delta[3].v[p] = d13 - dx;
		noPRINT ("DELTA[%u]: %6.1f %6.1f %6.1f %6.1f\n", p, delta[0].v[p], delta[1].v[p],
			delta[2].v[p], delta[3].v[p]);
	}
			...
			if (val < kcl->n_trilist)
			{
				...
				if (len)
				{
					...
					if (kcl->max_tri_len < len)
						kcl->max_tri_len = len;
					if (depth < KCL_MAX_STAT_DEPTH && kcl->max_depth_tri_len[depth] < len)
					{
...
void CalcStatisticsKCL (kcl_t *kcl // valid KCL
)
{
	DASSERT (kcl);
	...
	for (p = 0; p < 3; p++)
	{
		...
		kcl->max_octree.v[p] = kcl->min_octree.v[p] + kcl->bcube_width * kcl->n_bcubes[p];
	}
	...
	noPRINT ("KCL BASE STATS:\n"
			 ...
			 "\t\t---\n",
		kcl->mask[0], kcl->mask[1], kcl->mask[2], kcl->coord_rshift, kcl->y_lshift, kcl->z_lshift,
		kcl->mask_bits[0], kcl->mask_bits[1], kcl->mask_bits[2], kcl->bcube_bits[0],
		kcl->bcube_bits[1], kcl->bcube_bits[2], kcl->hash_bits, kcl->n_bcubes[0], kcl->n_bcubes[1],
		kcl->n_bcubes[2], kcl->total_bcubes, kcl->bcube_width, kcl->min_octree.x, kcl->min_octree.y,
		kcl->min_octree.z, kcl->max_octree.x, kcl->max_octree.y, kcl->max_octree.z);
	...
	noPRINT ("KCL OCTREE STATS:\n"
			 ...
			 "\t\t---\n",
		kcl->min_o_offset, kcl->max_o_offset, kcl->n_cube_nodes, kcl->min_l_offset,
		kcl->max_l_offset, kcl->n_trilist);
	...
	PRINT ("KCL OCTREE STATS: 0=%u,%u,  tri=%u,%u,  len=%u,%u,  depth=%u..%u\n", kcl->n_0_list,
		kcl->n_0_link, kcl->n_tri_list, kcl->n_tri_link, kcl->max_tri_len, kcl->sum_tri_len,
...
void CalcMinMaxKCL (kcl_t *kcl // KCL data structure
)
{
	DASSERT (kcl);
...
static void setup_oct_tri (kcl_t *kcl, // valid kcl
...
static bool OctCubeTriangleOverlaped (kcl_cube_t *cube, // cube info
...
)
{
	...
	if (tri->select == M1 (tri->select) || tri->cube.min[0] > cube->max[0]
		|| tri->cube.max[0] < cube->min[0] || tri->cube.min[1] > cube->max[1]
		|| tri->cube.max[1] < cube->min[1] || tri->cube.min[2] > cube->max[2]
		|| tri->cube.max[2] < cube->min[2])
	{
		return false;
	}
	...
	return DotProd3 (normal, vmin) <= 0.0 && DotProd3 (normal, vmax) >= 0.0;
}

///////////////////////////////////////////////////////////////////////////////

static u32 CalcCube (oct_info_t *oi, // base data
...
)
{
	...
	if (cube_size <= oi->kcl->max_cube_size && tri_count <= oi->kcl->max_cube_triangles
		|| cube_size <= oi->kcl->min_cube_size || depth > oi->kcl->max_octree_depth)
	{
		noPRINT ("--> TLIST:   n-tri =%5u, width =%6u, base = %6u,%6u,%6u, depth=%u/%u\n",
			tri_count, cube_size, cube->min[0], cube->min[1], cube->min[2], depth,
			kcl->max_octree_depth);
...
enumError CreateOctreeKCL (kcl_t *kcl // KCL data structure
)
{
	DASSERT (kcl);
	...
	{
		kcl->min_octree.v[p]
			= (IsNormalD (KCL_MIN.v[p]) ? KCL_MIN.v[p] : floor (kcl->min.v[p])) - kcl->cube_blow;

		int exp;
		max[p] = (IsNormalD (KCL_MAX.v[p]) ? KCL_MAX.v[p] : kcl->max.v[p]) + kcl->cube_blow;
	...
	PRINT ("KCL-PARAM: BITS=%u, MAX_TRI=%u, MAX_DEPTH=%u, CUBE_SIZE=%u..%u, BLOW=%u\n", max_bits,
		kcl->max_cube_triangles, kcl->max_octree_depth, kcl->min_cube_size, kcl->max_cube_size,
		kcl->cube_blow);
	...
	for (;;)
	{
		...
		noPRINT ("RSHIFT=%2u, NBITS= %2d + %2d + %2d = %2d / %2d\n", rshift, nbits[0], nbits[1],
			nbits[2], sumbits, max_bits);
		if (sumbits <= max_bits)
			break;
		rshift++;
	}
	...
	PRINT ("RSHIFT=%u, NBITS=%u,%u,%u, RELBITS=%u,%u,%u, N_CUBES=%u\n", rshift, nbits[0], nbits[1],
		nbits[2], relevant_bits[0], relevant_bits[1], relevant_bits[2], n_cubes);
	...
	if (KCL_MODE & KCLMD_CENTER)
	{
		for (p = 0; p < 3; p++)
		{
			const double space = (1 << rshift + nbits[p]) + kcl->min_octree.v[p] - max[p];
	...
	if (verbose >= 0 && !(KCL_MODE & KCLMD_SILENT) && !kcl->silent_octree)
		fprintf (stdlog,
			...
			rshift, n_cubes, kcl->min_cube_size, kcl->max_cube_size, kcl->cube_blow,
...
#endif
		noPRINT ("%5u: min=%5d,%5d,%5d, max=%5d,%5d,%5d, select=%d\n", t, tri->min[0], tri->min[1],
			tri->min[2], tri->max[0], tri->max[1], tri->max[2], tri->select);
	}
...
enumError ScanFlagFile (kcl_t *kcl, // pointer to valid KCL
...
)
{
	...
	if (!err)
	{
		...
		while (NextCharSI (&si, true))
		{
			...
			FormatFieldItem_t *it = InsertFormatField (&kcl->flag_name, name, false, false, 0);
			DASSERT (it);
			it->num = num;
...
void LoadFlagFileKCL (kcl_t *kcl // pointer to valid KCL
)
{
	DASSERT (kcl);
	...
	if (!kcl->flag_db_loaded && kcl->fname)
	{
		...
		if (!opt_flag_file || *opt_flag_file)
		{
			...
			{
				SubstDest (
					iobuf, sizeof (iobuf), kcl->fname, opt_flag_file, 0, KCL_FLAG_EXT, false);
			...
			{
				SubstDest (
					iobuf, sizeof (iobuf), kcl->fname, KCL_FLAG_FILE1, 0, KCL_FLAG_EXT, false);
				err = ScanFlagFile (kcl, iobuf);
			}
			...
			{
				SubstDest (
					iobuf, sizeof (iobuf), kcl->fname, KCL_FLAG_FILE2, 0, KCL_FLAG_EXT, false);
				err = ScanFlagFile (kcl, iobuf);
			}
			...
			PRINT ("KCL.FLAGS loaded, N=%u+%u, hex4=%d, hex23=%d\n", kcl->flag_name.used,
				kcl->flag_pattern.used, kcl->accept_hex4, kcl->accept_hex23);
		}
		else
...
int GetFlagByNameKCL (kcl_t *kcl, // pointer to valid KCL
...
ccp GetGroupNameKCL (uint kcl_flag, bool add_info)
{
	if (kcl_flag < N_KCL_FLAG)
	{
		...
		if (add_info)
			snprintf (
				buf, bufsize, "%s_%02x_F%04x (%s)", cls->name, type, kcl_flag, kcl_type[type].info);
		else
			snprintf (buf, bufsize, "%s_%02x_F%04x", cls->name, type, kcl_flag);
		return buf;
	}
	...
	if (kcl_flag >= KCL_FLAG_USER_0 && kcl_flag <= KCL_FLAG_USER_MAX)
	{
		...
		char *buf = GetCircBuf (bufsize);
		snprintf (buf, bufsize, "user_%02x%02x%02x%02x", col.a, col.r, col.g, col.b);
		return buf;
	}
...
)
{
	...
	const uint *norm_assign = KCL_MODE & KCLMD_IN_SWAP ? norm_assign_32 : norm_assign_23;
	...
	while (NextCharSI (&si, true))
	{
		if (!ScanNameSI (&si, buf, sizeof (buf), true, true, 0))
		{
			...
			ScanFile_t *sf = si.cur_file;
			DASSERT (sf);
			ERROR0 (ERR_WARNING, "Missing name [%s @%u]: %.*s\n", sf->name, sf->line,
				...
				for (k = 0;; k++)
				{
					...
					if (*sf->ptr == '/')
					{
						...
						if (*sf->ptr == '/')
						{
							// normal index found
							sf->ptr++;
							if (*sf->ptr >= '0' && *sf->ptr <= '9' && !ScanU32SI (&si, &idx, 1, 0)
								&& !k && (KCL_MODE & KCLMD_AUTO))
							{
	...
	if (!(KCL_MODE & KCLMD_SILENT) && (verbose > 0 || verbose >= 0 && par.swap_count))
	{
		fprintf (stdlog,
			"  - %u triangles read,"
	...
	if (bad_count)
	{
		...
		ERROR0 (ERR_WARNING, "%u of %u (%4.2f%%) triangles ignored because of wrong indices: %s\n",
			bad_count, total, 100.0 * bad_count / total, kcl->fname);
		if (max_err < ERR_WARNING)
			max_err = ERR_WARNING;
	}
...
enumError ScanRawKCL (kcl_t *kcl, // KCL data structure
	bool init_kcl, // true: initialize 'kcl' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
...
enumError ScanKCL (kcl_t *kcl, // KCL data structure
	bool init_kcl, // true: initialize 'kcl' first
	const void *data, // data to scan
	uint data_size, // size of 'data'
	bool use_data, // true: data is valid on 'kcl' live time
...
enumError ScanRawDataKCL (kcl_t *kcl, // KCL data structure
	bool init_kcl, // true: initialize 'kcl' first
...
enumError LoadKCL (kcl_t *kcl, // KCL data structure
	bool init_kcl, // true: initialize 'kcl' first
...
static enumError F_kcl_fall (struct Var_t *res, // store result here
...
)
{
	...
	{
		u32 typemask
			= n_param > 2 && param[2].mode != VAR_UNSET ? GetIntV (param + 2) : M1 (typemask);
		double temp
			= FallKCL (kcl, 0, param->d3, GetDoubleV (param + 1), 0, typemask, &kcl_fall_flag);
		if (temp >= 0.0)
			y = temp;
	}
...
static enumError F_kcl_fall_flag (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_isKCL (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si && si->kcl)
	{
		...
		if (si->kcl->fname)
		{
			...
			if (!strcasecmp (fname, "course.kcl") || !strcasecmp (fname, "course.txt")
...
static const struct FuncTable_t kcl_ref_func_tab[]
	= { { 2, 3, "KCL$FALL", F_kcl_fall, 0, "vector", "kcl$fall(pt,width[,typemask])",
		  ...
		  { 0, 0, "KCL$FALLFLAG", F_kcl_fall_flag, 0, "int", "kcl$fallFlag()",
...
void SetupReferenceKCL (VarMap_t *vm // not NULL: define KCL variables here
)
{
	static bool done = false;
	if (!done)
...
static enumError FastCreateRawKCL (kcl_t *kcl, // pointer to valid KCL
...
#if defined(TEST) || 0
		KCL_ACTION_LOG (
			kcl, "Have %u normals: try to reduce to 65535 (level %u)\n", normal.used, round);
#endif
...
enumError SaveRawKCL (kcl_t *kcl, // pointer to valid KCL
...
enumError SaveTextKCL (kcl_t *kcl, // pointer to valid KCL
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
)
{
	...
	PRINT (
		"n_vert=%u, n_norm=%u, n_tri=%u/%u, n_grp=%u\n", n_vert, n_norm, n_valid_tri, n_tri, n_grp);

	//--- print header

	fprintf (F.f, text_kcl_head_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE, GetKclMode (),
		...
		for (; j < j_max; j += j_inc)
			if (flag_count[j])
				fprintf (F.f, "# %8u triangles in group %s\r\n", flag_count[j],
					GetGroupNameKCL (j, true));
	}

	char mtl_fname[PATH_MAX];
	const bool have_mtl = !print_tri && !F.is_stdio && KCL_MODE & (KCLMD_MTL | KCLMD_WIIMM);
	if (have_mtl)
	{
			...
			for (td = td_base; td < td_end; td++)
			{
				if (!(td->status & TD_INVALID))
				{
					fprintf (F.f,
						...
						"v %11.3f %11.3f %11.3f\r\n"
						"v %11.3f %11.3f %11.3f\r\n",
						td->pt[0].x, td->pt[0].y, td->pt[0].z, td->pt[1].x, td->pt[1].y,
						td->pt[1].z, td->pt[2].x, td->pt[2].y, td->pt[2].z);
					*idx++ = vidx++;
					*idx++ = vidx++;
					*idx++ = vidx++;
					*idx++ = nidx++;
	...
	if (n_norm)
	{
		...
		if (kcl->fast)
		{
			for (td = td_base; td < td_end; td++)
				if (!(td->status & TD_INVALID))
					fprintf (F.f, "vn %11.3f %11.3f %11.3f\r\n", td->normal->x, td->normal->y,
						td->normal->z);
		}
		else
		{
	...
	for (i = 0; i <= N_KCL_TYPE; i++)
	{
		...
		for (; j < j_max; j += j_inc)
		{
			...
			fprintf (F.f,
				...
				"g %s\r\n"
				"s off\r\n",
				sep, flag_count[j], grp_name, j, i < N_KCL_TYPE ? i : 0x100,
				i < N_KCL_TYPE ? j >> 5 : j & 0xffff, grp_name);
				...
				for (td = td_base; td < td_end; td++)
					if (td->cur_flag == j && !(td->status & TD_INVALID))
					{
						fprintf (F.f,
							...
							" | %11.3f %11.3f %11.3f"
							" | %11.3f %11.3f %11.3f\r\n",
							GetExportFlagKCL (td->cur_flag), td->pt[0].x, td->pt[0].y, td->pt[0].z,
							td->pt[1].x, td->pt[1].y, td->pt[1].z, td->pt[2].x, td->pt[2].y,
							td->pt[2].z);
					}
#ifdef TEST0
					else
			...
			else if (n_norm)
			{
				for (td = td_base; td < td_end; td++)
					if (td->cur_flag == j && !(td->status & TD_INVALID))
					{
						...
						fprintf (F.f, "f %u//%u %u//%u %u//%u\r\n", idx[0], idx[3], idx[idx2],
							idx[3], idx[idx3], idx[3]);
					}
			}
			else
			{
...
static bool PrintTriangles (File_t *F, kcl_t *kcl, bool have_used)
{
	...
	for (td = td_base; td < td_end; td++)
	{
		...
		f3++;
		fprintf (f, "T.N2  %11.6f %11.6f %11.6f | %svalid", f3->x, f3->y, f3->z,
		...
		fprintf (f, "T.P1  %11.3f %11.3f %11.3f | len %g%s, delta %g/%g = %g\r\n", td->pt[0].x,
			td->pt[0].y, td->pt[0].z, td->length, td->length,
			< opt_tri_height ? " (too small)" : "", td->delta_max, td->delta_min,
			td->delta_min > 0 ? td->delta_max / td->delta_min : 0.0);
#else
		fprintf (f, "T.P1  %11.3f %11.3f %11.3f | len %g%s\r\n", td->pt[0].x, td->pt[0].y,
			td->pt[0].z, td->length, td->length < opt_tri_height ? " (too small)" : "");
#endif

		int p;
		for (p = 1; p < 3; p++)
			fprintf (f, "T.P%u  %11.3f %11.3f %11.3f | %10.3f %10.3f %10.3f \r\n", p + 1,
				td->pt[p].x, td->pt[p].y, td->pt[p].z, td->pt[p].x - td->pt[0].x,
				td->pt[p].y - td->pt[0].y, td->pt[p].z - td->pt[0].z);

		tri_metrics_t met;
		CalcTriMetricsByPoints (&met, td->pt, td->pt + 1, td->pt + 2);
		fprintf (f, "T.LEN %11.3f %11.3f %11.3f | area %g%s\r\n", met.l1, met.l2, met.l3, met.area,
			met.area < opt_tri_area ? " (too small)" : "");
		fprintf (f, "T.HT  %11.3f %11.3f %11.3f | min %g%s, max %g\r\n", met.h1, met.h2, met.h3,
			met.min_ht, met.min_ht < opt_tri_height ? " (too small)" : "", met.max_ht);
...
)
{
	...
	else
	{
		...
		PRINT (">> %u/%u/%u verteces and %u/%u/%u normals inserted in %6.4f sec.\n", vertex.used,
			vertex.size, n_tri, normal.used, normal.size, 4 * n_tri,
	...
	fprintf (F.f, text_kcl_dump_head_cr, n_vert, n_norm, n_tri, n_tri - kcl->n_tri_used,
		kcl->tri_fix_count, kcl->tri_inv_count, kcl->octree_size, kcl->raw_data_size,

		kcl->unknown_0x10, kcl->unknown_0x38,

		kcl->min_octree.x, kcl->min_octree.y, kcl->min_octree.z, kcl->min.x, kcl->min.y, kcl->min.z,
		kcl->mean.x, kcl->mean.y, kcl->mean.z, kcl->max.x, kcl->max.y, kcl->max.z,
		kcl->max_octree.x, kcl->max_octree.y, kcl->max_octree.z,

		kcl->mask[0], kcl->mask[1], kcl->mask[2], kcl->coord_rshift, kcl->y_lshift, kcl->z_lshift,
		kcl->mask_bits[0], kcl->mask_bits[1], kcl->mask_bits[2], kcl->bcube_bits[0],
		kcl->bcube_bits[1], kcl->bcube_bits[2], kcl->n_bcubes[0], kcl->n_bcubes[1],
		kcl->n_bcubes[2],

		kcl->tmin_dist, kcl->tmax_dist,

		kcl->bcube_width, kcl->bcube_width >> kcl->min_depth, kcl->bcube_width >> kcl->max_depth,
		// blow_size,
		kcl->total_bcubes, kcl->n_cube_nodes, kcl->total_cubes, kcl->max_depth, kcl->ave_depth,

		kcl->n_0_list, kcl->n_tri_list, kcl->n_0_link, kcl->n_tri_link);
	...
	{
		fprintf (F.f, text_kcl_dump_octree_cr, kcl->total_bcubes,
			kcl->total_cubes - kcl->total_bcubes, kcl->n_tri_list);

		u32 empty_offset = kcl->octree_size >= 2 && !be16 (kcl->octree + kcl->octree_size - 2)
			? kcl->octree_size - 2
			: 0;
...
enumError DumpTrianglesKCL (kcl_t *kcl, // pointer to valid KCL
	ccp fname // filename of destination
)
{
	DASSERT (kcl);
...
enumError ListTrianglesKCL (kcl_t *kcl, // pointer to valid KCL
	ccp fname, // filename of destination
...
)
{
	...
	{
		fprintf (F.f, "%04x %c %*.*f", td->cur_flag,
			!kcl->used_valid			 ? '?'
				: td->status & TD_UNUSED ? '-'
										 : '+',
		...
		const double3 *d3 = td->pt;
		for (ti = 0; ti < 3; ti++, d3++)
			fprintf (F.f, "%s %*.*f %*.*f %*.*f", ti ? "," : " |", fw_pt, prec_pt, d3->x, fw_pt,
				prec_pt, d3->y, fw_pt, prec_pt, d3->z);
		...
		if (print_normals)
		{
			...
			for (ni = 0; ni < 4; ni++, f3++)
			{
				fprintf (F.f, "%s %*.*f %*.*f %*.*f", ni ? "," : " |", fw_norm, prec_norm, f3->x,
					fw_norm, prec_norm, f3->y, fw_norm, prec_norm, f3->z);
			}
		}
		fputs ("\r\n", F.f);
	}
...
)
{
	...
	if (pt.x < kcl->min_octree.x || pt.y < kcl->min_octree.y || pt.z < kcl->min_octree.z)
	{
		if (verbosity > 0)
			fprintf (f,
				...
				"#\n",
				pt.x, pt.y, pt.z, kcl->min_octree.x, kcl->min_octree.y, kcl->min_octree.z);
		return 0;
	}
	...
	if (basept.x > 2e9 || basept.y > 2e9 || basept.z > 2e9)
	{
		if (verbosity > 0)
			fprintf (f,
				...
				"#\n",
				pt.x, pt.y, pt.z, basept.x, basept.y, basept.z);
		return 0;
	}
	...
	for (p = 0; p < 3; p++)
	{
		...
		nbits[p] = nmask[p] > kcl->coord_rshift ? nmask[p] - kcl->coord_rshift : 0;
	}
	...
	if (failed)
	{
		if (verbosity > 0)
			fprintf (f,
				...
				"#    * mask:        %#11x %#11x %#11x\n"
				"#\n",
				pt.x, pt.y, pt.z, kcl->min_octree.x, kcl->min_octree.y, kcl->min_octree.z, base[0],
				base[1], base[2], base[0], base[1], base[2], kcl->mask[0], kcl->mask[1],
				kcl->mask[2]);
		return ERR_OK;
	}

	u32 oct_index = base[0] >> kcl->coord_rshift | base[1] >> kcl->coord_rshift << kcl->y_lshift
	...
	if (verbosity > 1)
	{
		fprintf (f,
			...
			"#\n",
			pt.x, pt.y, pt.z, kcl->min_octree.x, kcl->min_octree.y, kcl->min_octree.z, base[0],
			base[1], base[2], base[0], base[1], base[2], kcl->mask[0], kcl->mask[1], kcl->mask[2],
			nmask[0], nmask[1], nmask[2], nbits[0], nbits[1], nbits[2], kcl->coord_rshift,
			relevant[0], relevant[1], relevant[2], base[0] >> kcl->coord_rshift,
			base[1] >> kcl->coord_rshift << kcl->y_lshift,
			base[2] >> kcl->coord_rshift << kcl->z_lshift, oct_index, offset, cmin.x, cmin.y,
			cmin.z, idx_mask, cmin.x + idx_mask, cmin.y + idx_mask, cmin.z + idx_mask);
	}
	...
	for (step = 1;; step++)
	{
		if (baseoff + 0x20 > kcl->octree_size)
		{
			if (verbosity > 0)
				fprintf (f,
					"# Abort, because octree index out of bounds!\n"
					"#    * point:  %11.3f %11.3f %11.3f\n"
					"#    * Offset: %#10x\n"
					"#    * Size:   %#10x\n"
					"#\n",
					pt.x, pt.y, pt.z, offset, kcl->octree_size);
			return -1;
		}
		...
		if (offset & 3)
		{
			if (verbosity > 0)
				fprintf (f,
					"# Abort, because invalid octree index!\n"
					"#    * point:      %11.3f %11.3f %11.3f\n"
					"#    * Offset:     %#10x\n"
					"#    * Low 2 bits: %#10x\n"
					"#\n",
					pt.x, pt.y, pt.z, offset, offset & 3);
			return -1;
		}
		...
		if (val & 0x80000000)
		{
			...
			if (offset + 2 > kcl->octree_size)
			{
				if (verbosity > 0)
					fprintf (f,
						"# Abort, because triangle list index out of bounds!\n"
						"#    * point:  %11.3f %11.3f %11.3f\n"
						"#    * Offset: %#10x\n"
						"#    * Size:   %#10x\n"
						"#\n",
			...
			if (offset & 1)
			{
				if (verbosity > 0)
					fprintf (f,
						"# Abort, because invalid triangle list index!\n"
						"#    * point:   %11.3f %11.3f %11.3f\n"
						"#    * Offset:  %#10x\n"
						"#    * Low bit: %#10x\n"
						"#\n",
						pt.x, pt.y, pt.z, offset, offset & 1);
				return -1;
			}
		...
		{
			fprintf (f,
				"# Step #%u, next octree entry found, value %#x:\n"
				"#    * mask %#x,  index %u+%u+%u = %u,  offset %#x,%#x\n"
				"#    * cube min:      %11.3f %11.3f %11.3f  (width=%u)\n"
				"#    * cube max:      %11.3f %11.3f %11.3f\n"
				"#\n",
				step, val, idx_mask, next_index & 1, next_index & 2, next_index & 4, next_index,
				baseoff, offset, cmin.x, cmin.y, cmin.z, idx_mask, cmin.x + idx_mask,
				cmin.y + idx_mask, cmin.z + idx_mask);
		}
	}
}
...
enumError TraverseOctreeKCL (kcl_t *kcl, // pointer to valid KCL
...
)
{
	...
	if (!print_tri_val)
	{
		...
		for (; stat > 0; stat--, tri_ptr++)
		{
			...
			if (pcount > fw_tlist)
				pcount = fprintf (f, "\n      ");
			pcount += fprintf (f, " %u%s", tidx, tidx <= kcl->tridata.used ? "" : "!");
		}
	}
	else
	{
		...
		for (; stat > 0; stat--, tri_ptr++)
		{
			...
			if (tidx < kcl->tridata.used)
			{
				...
				fprintf (f,
					...
					"\n",
					td->pt[0].x, td->pt[0].y, td->pt[0].z, tidx, td->pt[1].x, td->pt[1].y,
					td->pt[1].z, td->cur_flag, td->pt[2].x, td->pt[2].y, td->pt[2].z);
			}
			else
...
static int Collide2 (kcl_fall_t *kf, // valid data
	double3 pt // point for octree search
)
{
	DASSERT (kf);
	noPRINT ("Collide2() n=%u, v=%d, pt=%.0f %.0f %.0f\n", kf->n_tri_list, kf->last_valid, pt.x,
		pt.y, pt.z);

	//--- test if point is in previous cube

	if (kf->last_valid && pt.x >= kf->last_min.x && pt.x <= kf->last_min.x + kf->last_width
	...
	u16 *tri_list;
	int stat = TraverseOctree (
		kf->kcl, kf->f, pt, kf->verbose - 1, &kf->last_min, &kf->last_width, &tri_list);
	if (stat < 0)
		return -1;
...
static int Collide1 (kcl_fall_t *kf, // valid data
...
bool DropSortHelper (kcl_t *kcl, // valid KCL
...
bool DropSortTriangles (kcl_t *kcl)
{
	...
	{
		local_mode
			|= kcl->octree_valid && kcl->fform_outfile == FF_KCL ? KCLMD_DROP_UNUSED : KCLMD_M_DROP;
		PRINT ("DROP-AUTO[%d,%d] => %llx -> %llx\n", kcl->octree_valid,
			kcl->fform_outfile == FF_KCL, (u64)KCL_MODE, (u64)local_mode);
	}

	kcl_tridata_status_t mode = 0;
	if (local_mode & KCLMD_DROP_UNUSED)
		mode |= TD_UNUSED;
	if (local_mode & KCLMD_DROP_FIXED)
		mode |= TD_FIXED_PT;
	if (local_mode & KCLMD_DROP_INVALID)
		mode |= TD_INVALID;
	PRINT ("DROP-MODE: %x\n", mode);

	bool sort_triangles = 0 != (local_mode & KCLMD_SORT);
	PRINT (
		"*** DropSortTriangles() mode=%x|%u=%u ***\n", mode, sort_triangles, drop_sort_triangles);

	if (local_mode & KCLMD_DROP_UNUSED && kcl->octree_valid)
		CalcStatisticsKCL (kcl);

	if (local_mode & (KCLMD_DROP_INVALID | KCLMD_RM_FACEDOWN | KCLMD_RM_FACEUP | KCLMD_CONV_FACEUP))
	{
		CalcNormalsKCL (kcl, false);
...
static bool ScriptKCL (kcl_t *kcl, // valid KCL data
...
bool TransformKCL (kcl_t *kcl // pointer to valid KMP
)
{
	DASSERT (kcl);
	...
	kcl->octree_valid = kcl->min_max_valid = kcl->stat_valid = kcl->norm_valid = false;
...
int ScanOptKclFlag (ccp arg)
{
	...
	for (;;)
	{
		...
		for (; *arg != '='; arg++)
		{
			if (!*arg || *arg == ',')
			{
				ERROR0 (ERR_SYNTAX, "Missing '=' in parameter of --kcl-flag: %s\n", old);
				goto abort;
			}
		}
		DASSERT (*arg == '=');
		...
		if (!valid)
		{
			ERROR0 (ERR_SYNTAX, "Missing number in parameter of --kcl-flag: %s\n", arg);
			goto abort;
		}
		PRINT ("NEW-VAL: - %04x\n", new_val);
		...
		for (;;)
		{
			...
			u32 old1, old2;
			old = ScanRangeU32 (
				old, &valid, &old1, &old2, 0, have_t ? N_KCL_TYPE - 1 : N_KCL_FLAG - 1);
			if (!valid)
			{
				ERROR0 (ERR_SYNTAX, "Missing number in parameter of --kcl-flag: %s\n", old);
				goto abort;
...
bool PatchRawDataKCL (void *data, // data to scan
	uint data_size, // size of 'data'
...
)
{
	...
	if (!err && PatchKCL (&kcl))
	{
		...
		{
			KCL_ACTION_LOG (&kcl, "New KCL is %u bytes too large => round normals.\n",
				kcl.raw_data_size - data_size);
	...
	if ( !err && kcl.raw_data_size > data_size )
	{
		...
		if (!err && kcl.raw_data_size > data_size)
		{
			KCL_ACTION_LOG (&kcl, "New KCL is %u bytes too large => enlarge triangle lists.\n",
				kcl.raw_data_size - data_size);
		...
		if (!err && kcl.raw_data_size > data_size)
		{
			KCL_ACTION_LOG (&kcl, "New KCL is %u bytes too large => new octree param.\n",
				kcl.raw_data_size - data_size);
		...
		if (!err && kcl.raw_data_size > data_size)
		{
			KCL_ACTION_LOG (&kcl, "New KCL is %u bytes too large => new octree param again.\n",
				kcl.raw_data_size - data_size);
		...
		{
			KCL_ACTION_LOG (
				&kcl, "Replace KCL [inplace], size %u -> %u.\n", data_size, kcl.raw_data_size);
...
bool PatchKCL (kcl_t *kcl // pointer to valid KCL
)
{
	DASSERT (kcl);
	if (have_patch_count <= 0 || have_kcl_patch_count <= 0 && !transform_active)
	...
	const bool stat = ScriptKCL (kcl, true) | DropSortTriangles (kcl) | TransformKCL (kcl)
		| ScriptKCL (kcl, false) | patch_kcl_flag != 0
...
static int iterate_oct8 (const oct_iter_t *iter, // iteration parameter
...
int OctreeIteratorKCL (kcl_t *kcl, // pointer to valid KCL
	oct_func func, // call back function
	void *param // user parameter
)
{
...
static int calc_blow (kcl_t *kcl, // pointer to valid KCL
	void *param, // user parameter
...
)
{
	...
	noPRINT ("** %7u,%7u,%7u .. %7u,%7u,%7u   WD=%7u, N=%5u, D=%6zx, BLOW=%u\n", cube->min[0],
		cube->min[1], cube->min[2], cube->max[0], cube->max[1], cube->max[2],
		cube->max[0] - cube->min[0], n_list, tri_list ? (u8 *)tri_list - kcl->octree : 0,
		blow->min_blow);
	...
	while (n_list-- > 0)
	{
		...
		// use [[cube]] list param?
		if (tidx >= blow->n_tri || OctCubeTriangleOverlaped (&xcube, blow->tlist + tidx, 0))
		{
			continue;
		}
#else
		...
		if (tidx >= blow->n_tri || OctCubeTriangleOverlaped (&xcube, blow->tlist + tidx))
		{
			continue;
		}
...
static void print_check_error (check_kcl_t *chk, // valid data structure
	...
	if (!chk->head_printed)
	{
		...
		if (chk->mode & (CMOD_HEADER | CMOD_FORCE_HEADER))
			fprintf (stdlog, "%s* CHECK %s:%s%s\n", chk->col.heading,
				GetNameFF (chk->kcl->fform, 0), chk->kcl->fname, chk->col.reset);
	}
...
)
{
	...
	for (; n-- > 0; td++)
		if (td->cur_flag < N_KCL_FLAG && kcl_type[td->cur_flag & 0x1f].attrib & KCLT_DRIVE)
		{
			total++;
			if (td->status & TD_INVALID_NORM)
	...
	if (count_0)
		print_check_error (&chk, CMOD_HINT,
			...
			" => --kcl=RM-FACEDOWN\n",
			count_0, total, total == 1 ? "" : "s", count_0 == 1 ? "is" : "are");
	if (count_30)
		print_check_error (&chk, CMOD_HINT, "%u of %u drivable triangle%s %s face down (>30°).\n",
			count_30, total, total == 1 ? "" : "s", count_30 == 1 ? "is" : "are");
	...
	if (kcl->octree_valid)
	{
		...
		if (kcl->n_tri_used < kcl->tridata.used)
		{
			...
			print_check_error (&chk, CMOD_HINT,
				...
				" => --kcl=DROP-UNUSED\n",
				unused, kcl->tridata.used, unused == 1 ? " is" : "s are");
		}
	}
	...
	if (kcl->tri_fix_count)
		print_check_error (&chk, CMOD_HINT,
			...
			" => --kcl=DROP-FIXED\n",
			kcl->tri_fix_count, kcl->tridata.used, kcl->tridata.used == 1 ? "" : "s");

	if (kcl->tri_inv_count)
		print_check_error (&chk, CMOD_WARNING,
			"%u of %u triangle%s invalid values"
			" => --kcl=DROP-INVALID\n",
			kcl->tri_inv_count, kcl->tridata.used, kcl->tridata.used == 1 ? " has" : "s have");
	...
	{
		static ccp hint = " => see https://szs.wiimm.de/cmd/wkclt/check#desc for more info.";
...
static enumError F_tri_n (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_pt (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_length (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_normal (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_setPt (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_scale (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_shift (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_hRot (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_remove (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_unremove (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_isRemoved (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_create (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si && si->kcl)
	{
		...
		kcl_tridata_t *td = AppendTrianglesKCLp (
			si->kcl, GetIntV (param), &param[1].d3, sizeof (*param), n_param - 1);
		if (td)
		{
...
static enumError F_tri_createOctahedron (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_createCuboid (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_createPyramid (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si && si->kcl)
	{
		...
		kcl_tridata_t *td = AppendPyramidKCL (si->kcl, GetIntV (param), &param[1].d3, &param[2].d3,
			sizeof (*param), n_param - 2, false);
		if (td)
		{
			si->kcl->script_tri_added = true;
			status = td - (kcl_tridata_t *)si->kcl->tridata.list;
...
static enumError F_tri_createPrism (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
)
{
	...
	if (si && si->kcl)
	{
		...
		kcl_tridata_t *td = AppendPrismKCL (si->kcl, GetIntV (param), &param[1].d3, &param[2].d3,
			&param[3].d3, GetDoubleV (param + 4), GetIntV (param + 5), fpar->user_id & 1, base_flag,
			base_height, arrow_flag, arrow_size, false);
		if (td)
		{
			si->kcl->script_tri_added = true;
			status = td - (kcl_tridata_t *)si->kcl->tridata.list;
...
static enumError F_tri_createJoist (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si && si->kcl)
	{
		...
		kcl_tridata_t *td = AppendJoistKCL (si->kcl, GetIntV (param), GetDoubleV (param + 1),
			&param[2].d3, &param[3].d3, &param[4].d3, n_param > 5 ? GetIntV (param + 5) : 0,
...
static enumError F_tri_ptsInCuboid (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si)
	{
		...
		if (td)
		{
			...
			for (p = 0; p < 3; p++, d3++)
			{
				if (d3->x >= param[1].x && d3->x <= param[2].x && d3->y >= param[1].y
					&& d3->y <= param[2].y && d3->z >= param[1].z && d3->z <= param[2].z)
				{
					status++;
				}
			}
...
static enumError F_tri_OverlapCube (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
)
{
	...
	if (si)
	{
		...
		if (td)
		{
			status = OverlapCubeTri (&ToVectorV (param + 1)->d3, GetDoubleV (param + 2), td->pt + 0,
				td->pt + 1, td->pt + 2);
		}
	}
...
static enumError F_tri_flag (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_setFlag (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_defColor (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static enumError F_tri_getColor (struct Var_t *res, // store result here
	struct Var_t *param, // parameters, modified
	uint n_param, // number of parameters
	struct ScanInfo_t *si, // not NULL: use for error messages
	const FuncParam_t *fpar // pointer to user parameter
...
static const struct FuncTable_t func_tab[]
	= { { 0, 0, "TRI$N", F_tri_n, 0, "vector", "tri$n()",
		  ...
		  { 3, 4, "TRI$CREATEOCTAHEDRON", F_tri_createOctahedron, 0, "int",
			  "tri$createOctahedron(kcl_flag,pos,radius[,rot])",
		  ...
		  { 3, 4, "TRI$CREATECUBOID", F_tri_createCuboid, 0, "int",
			  "tri$createCuboid(kcl_flag,pos,size[,rot])",
		  ...
		  { 4, MAX_FUNC_PARAM, "TRI$CREATEPYRAMID", F_tri_createPyramid, 0, "int",
			  "tri$createPyramid(kcl_flag,apex,pt1,pt2,...)",
			  "This function creates a pyramid by adding N triangles."
			  " 'pt1'..'ptN' descibribes the base polygon, which is not created."
			  " The function returns the index of the first new triangle or -1 if failed." },

		  { 6, 8, "TRI$CREATEPRISM", F_tri_createPrism, 0, "int",
			  "tri$createPrism(kcl_flag,pt1,pt2,ptX,r,n[,base_flag][,base_height])",
		  ...
		  { 6, 8, "TRI$CREATEANTIPRISM", F_tri_createPrism, 1, "int",
			  "tri$createAntiPrism(kcl_flag,pt1,pt2,ptX,r,n[,base_flag][,base_height])",
		  ...
		  { 7, 10, "TRI$CREATEARROW", F_tri_createPrism, 2, "int",
			  "tri$createArrow(flag,pt1,pt2,ptX,r,n,a_flag[,a_size][,b_flag][,b_ht])",
		  ...
		  { 7, 10, "TRI$CREATEANTIARROW", F_tri_createPrism, 3, "int",
			  "tri$createAntiArrow(flag,pt1,pt2,ptX,r,n,a_flag[,a_size][,b_flag][,b_ht])",
		  ...
		  { 5, 6, "TRI$CREATEJOIST", F_tri_createJoist, 0, "int",
			  "tri$createJoist(kcl_flag,length,pt1,pt2,pt_dir[,n_marker][,mark5_flag])",
		  ...
		  { 3, 3, "TRI$PTSINCUBOID", F_tri_ptsInCuboid, 0, "int",
			  "tri$ptsInCuboid(tri_index,cube_min,cube_max)",
...
#ifdef TEST // [[2do]]
		  { 3, 3, "TRI$OVERLAPCUBE", F_tri_OverlapCube, 1, "int",
			  "tri$OverlapCube(tri_index,cube_mid,cube_width)",
		  ...
		  { 1, 2, "TRI$FLAG", F_tri_flag, 0, "int", "tri$flag(tri_index[,mode])",
		  ...
		  { 1, 2, "TRI$TYPE", F_tri_flag, 1, "int", "tri$type(tri_index[,mode])",
		  ...
		  { 2, 2, "TRI$SETFLAG", F_tri_setFlag, 0, "int", "tri$setFlag(tri_index,new_flag)",
		  ...
		  { 1, 4, "TRI$DEFCOLOR", F_tri_defColor, 0, "int", "tri$defColor(argb[,alpha])",
			  "Define a new color."
		  ...
		  { 0, 0, 0, F_tri_defColor, 0, "int", "tri$defColor(red,green,blue[,alpha])",
			  "Define a new color."
		  ...
		  { 1, 1, "TRI$GETCOLOR", F_tri_getColor, 0, "int", "tri$getColor(index)",
