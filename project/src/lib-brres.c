...
static u16 calc_brres_id (ccp prev_name, uint prev_len, ccp cur_name, uint cur_len)
{
	if (prev_len < cur_len)
...
static void dump_brres_group (const brres_root_t *root, const endian_func_t *endian)
{
	...
	while ((u8 *)grp < root_end)
	{
		...
		for (entry_idx = 0; entry < entry_end; entry++, entry_idx++)
		{
			...
			printf ("%4x: %4x %4x %4x %4x %8x %8x  %-4s  %.*s\n", entry_idx,
				endian->rd16 (&entry->id), endian->rd16 (&entry->unknown),
				endian->rd16 (&entry->left_idx), endian->rd16 (&entry->right_idx),
				endian->rd32 (&entry->name_off), endian->rd32 (&entry->data_off),
				data_off ? PrintID (data, 4, 0) : "-", name_len, name);
		}
...
///////////////			get BRSUB info			///////////////
///////////////////////////////////////////////////////////////////////////////
// https://wiki.tockdom.com/wiki/BRRES_Sub_Files_(File_Format)#Common_Format
...
const brsub_info_t *GetInfoBRSUB (file_format_t fform, // file format
...
)
{
	if (fform)
	{
		...
		for (bi = brsub_info; bi->fform1; bi++)
			if ((bi->fform1 == fform || bi->fform2 == fform) && bi->version == version)
			{
				return bi;
			}
	}
...
const brsub_info_t *GetCorrectBRSUB (file_format_t fform // file format
)
{
	if (fform)
	{
...
int GetKnownSectionNumBRSUB (file_format_t fform, // file format
	u32 version // sub version
)
{
...
int GetGenericSectionNumBRSUB (const u8 *data, // valid data pointer
...
int GetSectionNumBRSUB (const u8 *data, // valid data pointer
	uint data_size, // size of 'data'
	const endian_func_t *endian // NULL or endian functions
)
{
...
ccp PrintHeaderNameBRSUB (char *buf, // result buffer
...
)
{
	...
	snprintf (buf, buf_size, ".BRSUB.header.%s-v%u.n%u.bin", PrintID (bhead->magic, 4, 0),
		endian->rd32 (&bhead->version), n_grp);
...
static bool get_brres_id_bit (brres_info_t *info, u16 id)
{
	DASSERT (info);
	const uint char_idx = id >> 3;
	noPRINT ("get_brres_id_bit(%x,%.*s) -> %d+%d/%d -> %d\n", id, info->nlen, info->name, char_idx,
		id & 7, info->nlen, char_idx < info->nlen && info->name[char_idx] >> (id & 7) & 1);
	return char_idx < info->nlen && info->name[char_idx] >> (id & 7) & 1;
}

///////////////////////////////////////////////////////////////////////////////

void CalcEntryBRRES (brres_info_t *info, uint entry_idx)
{
	DASSERT (info);
	...
	noPRINT ("++ID: e=%zx,%x, p=%zx,%x, c=%zx,%x\n", entry - info, entry->id, prev - info, prev->id,
	...
	while (entry->id <= current->id && current->id < prev->id)
	{
		...
		noPRINT ("ID: e=%zx,%x, p=%zx,%x, c=%zx,%x\n", entry - info, entry->id, prev - info,
			prev->id, current - info, current->id);
	}
...
enumError CreateBRRES (szs_file_t *szs, // valid szs
...
)
{
...

	{
		...
		for (f_ptr = szs->subfile.list; f_ptr < f_end; f_ptr++)
		{
			...
			else
			{
				...
				{
					enumError err = LoadFILE (
						source_dir, f_ptr->path, 0, data_ptr, data_size, 0, &szs->fatt, true);
					if (max_err < err)
						max_err = err;
				}
				...
				if (IsValidBRSUB (
						data_ptr, data_size, data_size, 0, f_ptr->fform, true, f_ptr->path, endian)
					< VALID_ERROR)
				{
					noPRINT ("----- %s -> %s -----\n", f_ptr->path, path_buf);
					szs_file_t sub;
	...
	u32 grp_end = grp_offset + sizeof (brres_group_t) * n_directories
	...
	u32 data_offset = grp_end <= szs->min_data_off
		...
		: ALIGN32 (grp_end, opt_align_brres > 0x20 ? opt_align_brres : 0x20);
	u32 str_offset = ALIGN32 (data_offset + total_data_size, opt_align_brres);
	CalcStringPool (&szs->string_pool, str_offset, endian);
	u32 data_size = ALIGN32 (
		str_offset + szs->string_pool.size, opt_align_brres > 0x80 ? opt_align_brres : 0x80);

	PRINT ("* GRP=%x..%x, DATA=%x, STR=%x, END=%x\n", grp_offset, grp_end, data_offset, str_offset,
		data_size);
	...
	for (dir_index = 0; dir_index < n_directories; dir_index++)
	{
		...
		noPRINT ("EP-READ  #%zu: %zx-%x := %zx\n", ep_read - ep_list,
			ep_read->dest ? ep_read->dest - szs->data : 0, ep_read->delta,
		...
		endian->wr32 (
			&grp->size, sizeof (brres_group_t) + (n_entries + 1) * sizeof (brres_entry_t));
		...
		for (; f_ptr < f_end; f_ptr++)
		{
			...
			if (f_ptr->is_dir)
			{
				...
				noPRINT ("EP-WRITE #%zu: %zx-%x\n", ep_write - ep_list, ep_write->dest - szs->data,
					ep_write->delta);
				ep_write++;
			...
			else
			{
				...
				// [[fname+]]
				InitializeSubSZS (
					&sub, szs, data_ptr - szs->data, data_size, FF_UNKNOWN, f_ptr->path, false);
	...
	for (; entry < entry_end && !stat; entry++)
	{
		...
		else
		{
			...
			if (opt_ext && it->szs->fform_arch == FF_BRRES)
			{
				...
				ccp ext = opt_ext > 1 ? GetMagicExtFF (0, it->fform) : GetExtFF (0, it->fform);
...
int IterateFilesBRRES (struct szs_iterator_t *it, // iterator struct with all infos
...
ccp GetStringBRSUB (const void *data, // relevant data
...
ccp GetStringIteratorBRSUB (struct brsub_iterator_t *it, // iterator struct with all infos
...
)
{
	...
	return !offset
		? if_null
		: GetStringBRSUB (it->brsub, (u8 *)it->brsub + it->file_size,
			  it->static_strings ? 0 : it->data_size, base, offset, if_invalid, it->endian);
}

///////////////////////////////////////////////////////////////////////////////

ccp GetStringBRRES (const szs_file_t *szs, // base BRRES file
	const void *base, // base for offset
	u32 offset, // offset of string
	ccp if_invalid // result if invalid
)
{
	...
	return szs ? GetStringBRSUB (szs->data, szs->data + szs->size,
					 0, // szs->max_data_off,
					 base, offset, if_invalid, szs->endian)
			   : if_invalid;
}
...
)
{
	...
	{
		PRINT_IF (bcut->fform == FF_PAT, "#PAT# g+e=%d,%d | %s #%u\n", grp, entry, __FUNCTION__,
			__LINE__);
		if (brit->grp_func)
	...
	{
		PRINT_IF (bcut->fform == FF_PAT, "#PAT# g+e=%d,%d v=%d | %s #%u\n", grp, entry,
			bcut->version, __FUNCTION__, __LINE__);
		...
		switch (BRSUB_MODE3 (bcut->fform, bcut->version, grp))
		{
			case BRSUB_MODE3 (FF_PAT, 4, 0):
			{
				...
				PRINT ("\e[32m#PAT# grp=%d val=%d,%d | %s #%u\e[0m\n", grp, ana.valid,
					ana.data_complete, __FUNCTION__, __LINE__);
	...
	{
		PRINT_IF (bcut->fform == FF_PAT, "#PAT# g+e=%d,%d raw=%p | %s #%u\n", grp, entry,
			brit->raw_func, __FUNCTION__, __LINE__);
...
int IterateStringsBRSUB (brsub_iterator_t *it // valid iterator data
)
{
	DASSERT (it);
	DASSERT (it->szs);
	...
	else
	{
		...
		if (IsValidBRSUB (it->brsub, it->file_size, it->file_size, it->adjust ? 0 : it->szs, fform,
				false, 0, endian)
			>= VALID_ERROR)
		{
...
static int clear_brsub_offset (struct brsub_iterator_t *it, // iterator struct with all infos
...
static int adjust_brsub_offset (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
...
static int insert_brsub_string (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr // pointer to u32 string offset
)
{
...
static int adjust_brsub_string_szs (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr // pointer to u32 string offset
)
{
	...
	PRINT_IF (it->fform == FF_PAT, "#PAT# ADJUST %x -> %x : %s @%s\n", it->endian->rd32 (ptr), off,
		string, __FUNCTION__);
...
static int adjust_brsub_string (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr // pointer to u32 string offset
)
{
...
int CollectStringsBRSUB (string_pool_t *sp, // string pool
...
int AdjustStringsBRSUB (szs_file_t *szs, // base BRRES file
...
static int dump_brsub_raw (struct brsub_iterator_t *it, // iterator struct with all infos
...
)
{
	...
	fprintf (
		f, "    - RAW DATA %x .. %x, size %x\n", data_offset, data_offset + data_size, data_size);
	return 0;
}

//-----------------------------------------------------------------------------

static int dump_brsub_grp (struct brsub_iterator_t *it, // iterator struct with all infos
...
static int dump_brsub_entry (struct brsub_iterator_t *it, // iterator struct with all infos
	brres_group_t *grp, // pointer to group
	uint grp_index, // index of group
...
)
{
	...
	ccp string = GetStringIteratorBRSUB (it, grp, &entry->name_off, "<ignored>", "**invalid**");

	fprintf (f, "      - ENTRY #%-2u %4x [+%3x] : %8x %s-> %s\n", (int)(entry - grp->entry),
		(int)((u8 *)entry - (u8 *)it->brsub), (int)((u8 *)entry - (u8 *)grp),
		it->endian->rd32 (&entry->name_off),
		string <= (ccp)it->brsub + it->data_size ? "[intern] " : "", string);
	return 0;
}

//-----------------------------------------------------------------------------

static int dump_brsub_string (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr // pointer to u32 string offset
)
{
	...
	fprintf (f, "        > STRING %5x [+%3x] : %8x %s-> %s\n", (int)((u8 *)ptr - (u8 *)it->brsub),
		(int)((u8 *)ptr - base), it->endian->rd32 (ptr),
		string <= (ccp)it->brsub + it->data_size ? "[intern] " : "", string);
	return 0;
}
...
static int dump_brsub_offset (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr // pointer to u32 string offset
)
{
...
int DumpStructureBRSUB (FILE *f, // output file
...
static int dump_func (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
int DumpStructureBRRES (FILE *f, // output file
...
static void add_ref_string (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr, // pointer to u32 string offset
...
static int insert_name_ref (struct brsub_iterator_t *it, // iterator struct with all infos
	u8 *base, // pointer to base for calculation
	u32 *ptr // pointer to u32 string offset
)
{
...
static void iterate_nr_brsub (name_ref_t *nr, // valid struct
...
)
{
	...
	{
		if (nr->brief > 2)
			snprintf (nr->file_info, sizeof (nr->file_info), "%s", GetNameFF (0, fform));
		else
			snprintf (nr->file_info, sizeof (nr->file_info), "%s @%x, ", GetNameFF (0, fform),
				(uint)((u8 *)data - szs->data));
...
static int iterate_nr_brres (struct szs_iterator_t *it, // iterator struct with all infos
	bool term // true: termination hint
)
{
	DASSERT (it);
...
enumError CreateNameRef (name_ref_t *nr, bool init_nr, szs_file_t *szs, int brief)
{
	DASSERT (nr);
	DASSERT (szs);
...
static void list_var (FILE *f, int indent, name_ref_var_t *vptr, int brief)
{
	...
	if (brief > 0)
		fprintf (f, "\n%*s»%s«\n", indent, "", vptr->var_name ? vptr->var_name : "");
	else
		fprintf (
			f, "\n%*s%6x  »%s«\n", indent, "", vptr->var_off, vptr->var_name ? vptr->var_name : "");

	name_ref_elem_t *eptr, *eend = vptr->elist + vptr->eused;
	if (brief > 1)
		for (eptr = vptr->elist; eptr < eend; eptr++)
			fprintf (f, "%*s  %s\n", indent, "", eptr->info ? eptr->info : "");
	else
		for (eptr = vptr->elist; eptr < eend; eptr++)
			fprintf (f, "%*s  %6x  %s\n", indent, "", eptr->file_off, eptr->info ? eptr->info : "");
}
