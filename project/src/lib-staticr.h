...
typedef enum str_status_t
{
	...
	STR_S_TRACK_ORDER__FAIL = STR_S_TRACK_ORDER_MULTI | STR_S_TRACK_ORDER_BAD
		| STR_S_TRACK_ORDER_DIFF,

	STR_S_ARENA_ORDER__FAIL = STR_S_ARENA_ORDER_MULTI | STR_S_ARENA_ORDER_BAD
...
{
	atype &= ~ADDRT_F_MIRROR;
	return atype >= ADDRT_DOL && atype <= ADDRT_BSS;
}

static inline bool IsAddrTypeREL (addr_type_t atype)
{
	atype &= ~ADDRT_F_MIRROR;
	return atype >= ADDRT_REL_SECT && atype <= ADDRT_REL;
}
...
{
	return addr >= 0xc0000000 && addr < 0xc1800000 || addr >= 0xd0000000 && addr < 0xd4000000;
}
...
void AnalyzeMAIN (staticr_t *str // valid pointer
);

void PrintStatusMAIN (FILE *f, // destination file
	int indent, // indention
	staticr_t *str, // valid pointer
	int long_mode // >0: print more track details if changed
);
