...
static enumError SaveFLIM ( Image_t *img, ccp dest, ccp source, bool bclim )
{
    ...
    for (uint y = 0; y < img->height; y++)
        memcpy(rgba + 4*y*img->width, img->data + 4*y*img->xwidth,
            4*img->width);
    u8 *data = 0;
    uint size = 0;
    ...
    FREE(rgba);
    if (err)
        return ERROR0(ERR_INVALID_DATA,"Can't encode %s texture: %s\n",
...
static enumError CheckCommand ( int argc, char ** argv )
{
	...
	case CMD_HELP:		PrintHelpColor(&InfoUI_wimgt); break;
