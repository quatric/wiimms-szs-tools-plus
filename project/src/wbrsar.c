...
static int cmd_pack ( int argc, char *argv[] )
{
    if ( argc < 3 )
    {
        fprintf(stderr, "wbrsar pack: missing input directory\n"
    ...
    {
        if ( !strcmp(argv[i], "--bfsar") ) variant = BRSAR_VARIANT_FSAR;
        else if ( !strcmp(argv[i], "--bcsar") ) variant = BRSAR_VARIANT_CSAR;
    ...
    if ( !output_path )
    {
        ccp ext = variant == BRSAR_VARIANT_FSAR ? ".bfsar" :
                  variant == BRSAR_VARIANT_CSAR ? ".bcsar" : ".brsar";
...
static int cmd_unpack ( int argc, char *argv[] )
{
    if ( argc < 3 )
    {
        fprintf(stderr, "wbrsar unpack: missing input archive\n"
