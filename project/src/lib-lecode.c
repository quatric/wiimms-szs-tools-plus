...
{
	DASSERT (ana);
	return !(ana->valid & LE_HEAD_VALID) ? "LE-CODE file header is invalid"
...
int ScanOptTrackSource (ccp arg, int arg_len, int mode)
{
	...
	if (arg)
	{
		...
		if (arg_len > 0)
		{
			...
			if (arg_len > 0 && strcmp (last_key, path) && !FindParamField (&opt_track_source, path))
			{
				PRINT ("APPEND TRACK_SOURCE: 0x%03x %s\n", mode, path);
				if (!opt_track_source.field)
					InitializeParamField (&opt_track_source);
				ParamFieldItem_t *it = AppendParamField (&opt_track_source, path, false, mode, 0);
				if (it)
					last_key = it->key;
			}
		}
...
int ScanOptSzsMode (ccp arg)
{
	const KeywordTab_t keytab[]
		= { { 0, "OFF", 0, 0 }, { TFMD_COPY, "COPY", "CP", 0 }, { TFMD_MOVE, "MOVE", "MV", 0 },
			  { TFMD_MOVE1, "MOVE1", "MV1", 0 }, { TFMD_LINK, "LINK", "LN", 0 }, { 0, 0, 0, 0 } };
...
{
	const int stat = ScanKeywordOffAutoOn (arg, OFFON_ON, OFFON_ON, "Option --200cc");
	if (stat == OFFON_ERROR)
		return 1;
...
{
	const int stat = ScanKeywordOffAutoOn (arg, OFFON_ON, OFFON_ON, "Option --perfmon");
	if (stat == OFFON_ERROR)
		return 1;
...
{
	const int stat = ScanKeywordOffAutoOn (arg, OFFON_ON, OFFON_ON, "Option --custom-tt");
	if (stat == OFFON_ERROR)
		return 1;
...
{
	const int stat = ScanKeywordOffAutoOn (arg, OFFON_ON, OFFON_ON, "Option --xpflags");
	if (stat == OFFON_ERROR)
		return 1;
...
{
	const KeywordTab_t keytab[]
		= { { OFFON_OFF, "OFF", "-1", 0 }, { OFFON_AUTO, "AUTO", 0, 0 }, { SPEEDO_ON, "ON", 0, 0 },
			  { SPEEDO_ON, "FORCE", 0, 0 }, // legacy, [[obsolete]], remove in 2022
			  { SPEEDO_ON, "0DIGITS", "0", 0 }, { SPEEDO_FRACTION1, "FRACTION", 0, 0 },
			  { SPEEDO_FRACTION1, "1DIGITS", "1", 0 }, { SPEEDO_FRACTION2, "2DIGITS", "2", 0 },
			  { SPEEDO_FRACTION3, "3DIGITS", "3", 0 }, { 0, 0, 0, 0 } };

	const int stat = ScanKeywordOffAutoOnEx (keytab, arg, OFFON_ON, 0, "Option --speedo");
	if (stat == OFFON_ERROR)
		return 1;
...
ccp GetLecodeSpeedoName (speedo_mode_t mode)
{
	switch (mode)
	{
		...
		case SPEEDO_ON:
			return "0";
		case SPEEDO_FRACTION1:
			return "1";
		case SPEEDO_FRACTION2:
			return "2";
		case SPEEDO_FRACTION3:
			return "3";
...
ccp GetLecodeDebugName (le_debug_mode_t mode)
{
	switch (mode)
	{
		...
		case DEBUGMD__N:
			break;
		case DEBUGMD_ENABLED:
			return "ENABLED";
		case DEBUGMD_DEBUG1:
		case DEBUGMD_DEBUG2:
		case DEBUGMD_DEBUG3:
		case DEBUGMD_DEBUG4:
			return PrintCircBuf ("%u", mode - DEBUGMD_DEBUG1 + 1);
			// no default! => compiler warnings for missed values
	}
	return "OFF";
}
...
ccp GetLecodeDebugInfo (le_debug_mode_t mode)
{
	switch (mode)
	{
		...
		case DEBUGMD_DEBUG3:
		case DEBUGMD_DEBUG4:
			return PrintCircBuf ("view DEBUG-%u", mode - DEBUGMD_DEBUG1 + 1);
		case DEBUGMD__N:
			break;
			// no default! => compiler warnings for missed values
	}
...
{
	const KeywordTab_t keytab[] = { { OFFON_OFF, "OFF", "-1", 0 }, { OFFON_AUTO, "AUTO", "0", 0 },
		{ DEBUGMD_ENABLED, "USER", "1", 0 }, { DEBUGMD_DEBUG1, "DEBUG1", "D1", 0 },
		{ DEBUGMD_DEBUG2, "DEBUG2", "D2", 0 }, { DEBUGMD_DEBUG3, "DEBUG3", "D3", 0 },
		{ DEBUGMD_DEBUG4, "DEBUG4", "D4", 0 }, { 0, 0, 0, 0 } };
...
int ScanOptEngine (ccp arg)
{
	...
	if (n != 3)
		return ERROR0 (
			ERR_SYNTAX, "Invalid argument for option --engine, »a,b,c« expected: %s", arg);
	...
	PRINT ("ENGINE: %g,%g,%g => %u + %u + %u = %u [200cc=%d])\n", val[0], val[1], val[2],
		opt_engine[0], opt_engine[1], opt_engine[2], opt_engine[0] + opt_engine[1] + opt_engine[2],
		opt_200cc);
	return 0;
}
...
const VarMap_t *SetupVarsLECODE ()
{
	...
	if (!vm.used)
	{
		...
		static const struct inttab_t inttab[] = { { "DEBUG$OFF", PREDEBUG_OFF },
			{ "DEBUG$CLEAR", PREDEBUG_CLEAR }, { "DEBUG$STANDARD", PREDEBUG_STANDARD },
			{ "DEBUG$OPPONENT", PREDEBUG_OPPONENT }, { "DEBUG$VERTICAL", PREDEBUG_VERTICAL },

			{ "DEBUG$ENABLED", DEBUGMD_ENABLED }, { "DEBUG$0", DEBUGMD_ENABLED },
			{ "DEBUG$1", DEBUGMD_DEBUG1 }, { "DEBUG$2", DEBUGMD_DEBUG2 },
			{ "DEBUG$3", DEBUGMD_DEBUG3 }, { "DEBUG$4", DEBUGMD_DEBUG4 },

			{ "SPEEDO$OFF", SPEEDO_OFF }, { "SPEEDO$0", SPEEDO_ON },
			{ "SPEEDO$1", SPEEDO_FRACTION1 }, { "SPEEDO$2", SPEEDO_FRACTION2 },
			{ "SPEEDO$3", SPEEDO_FRACTION3 },

			{ "BT", LTTY_ARENA }, { "VS", LTTY_TRACK }, { "FILL", -1 },
...
{
	DASSERT (ldi);
	return PrintCircBuf ("I=%d,%d, M:%02x,%02x, %s=%u", ldi->index, ldi->filter_active, ldi->mode,
		ldi->relevant, ldi->name ? ldi->name : "-", ldi->value);
}

///////////////////////////////////////////////////////////////////////////////

bool GetFirstDebugMode (lecode_debug_info_t *ldi, lecode_debug_t mode, bool filter_active)
{
	DASSERT (ldi);
...
ccp GetPredefDebugName (predef_debug_t pd_mode)
{
	switch (pd_mode)
	{
		...
		case PREDEBUG_CLEAR:
			return "CLEAR";
		case PREDEBUG_STANDARD:
			return "STANDARD";
		case PREDEBUG_OPPONENT:
			return "OPPONENT";
		case PREDEBUG_VERTICAL:
			return "VERTICAL";
			// no default! => compiler warnings for missed values
	}
	return "OFF";
...
bool SetupLecodeDebugLPAR (le_lpar_t *lpar, // LE-CODE parameters
...
)
{
	...
	static const struct setup_t setup_tab[] = { { PREDEBUG_CLEAR, 0, 0 },

		{ PREDEBUG_STANDARD, 1, LEDEB__STD | LEDEB_SHORT_XPF },

		{ PREDEBUG_OPPONENT, 1, LEDEB__STD }, { PREDEBUG_OPPONENT, 2, LEDEB__STD | LEDEB_OPPONENT },

		{ PREDEBUG_VERTICAL, -1, 0 }, // hide speedo
		{ PREDEBUG_VERTICAL, 1, LEDEB_CHECK_POINT }, { PREDEBUG_VERTICAL, 2, LEDEB_RESPAWN },
		{ PREDEBUG_VERTICAL, 3, LEDEB_ITEM_POINT }, { PREDEBUG_VERTICAL, 4, LEDEB_KCL_TYPE },
		{ PREDEBUG_VERTICAL, 5, LEDEB_LAP_POS }, { PREDEBUG_VERTICAL, 6, LEDEB_POSITION },
		{ PREDEBUG_VERTICAL, 7, LEDEB_TRACK_ID }, { PREDEBUG_VERTICAL, 8, LEDEB_LONG_XPF },
		// [[new-debug]]
...
lpar_mode_t CalcCurrentLparMode (const le_lpar_t *lp, bool use_limit_param)
{
	...
	return lp->enable_perfmon || !lp->enable_xpflags || lp->enable_speedo > SPEEDO_FRACTION1
			|| lp->cheat_mode > 1
		? LPM_TESTING
		: LPM_PRODUCTIVE;
}
...
ccp GetLparModeName (lpar_mode_t lpmode, bool export)
{
	...
	{
		case LPM_PRODUCTIVE:
			return "LE$PRODUCTIVE";
		case LPM_TESTING:
			return "LE$TESTING";
		case LPM_EXPERIMENTAL:
			return "LE$EXPERIMENTAL";
		default:
			return "LE$AUTOMATIC";
	}
}
...
ccp GetChatModeName (u16 mode)
{
	...
	switch (mode)
	{
		...
		case CHATMD_ANY_TRACK:
			return "CHAT$ANY_TRACK";
		case CHATMD_TRACK_BY_HOST:
			return "CHAT$TRACK_BY_HOST";

		case CHATMD_BLOCK_CLEAR:
			return "CHAT$BLOCK_CLEAR";
		case CHATMD_BLOCK_DISABLE:
			return "CHAT$BLOCK_DISABLE";
		case CHATMD_BLOCK_ENABLE:
			return "CHAT$BLOCK_ENABLE";

		case CHATMD_ANY_VEHICLE:
			return "CHAT$ANY_VEHICLE";
		case CHATMD_KARTS_ONLY:
			return "CHAT$KARTS_ONLY";
		case CHATMD_BIKES_ONLY:
			return "CHAT$BIKES_ONLY";

		case CHATMD_RESET_ENGINE:
			return "CHAT$RESET_ENGINE";
		case CHATMD_USE_ENGINE_1:
			return "CHAT$USE_ENGINE_1";
		case CHATMD_USE_ENGINE_2:
			return "CHAT$USE_ENGINE_2";
		case CHATMD_USE_ENGINE_3:
			return "CHAT$USE_ENGINE_3";
	}
...
static void NormalizeLPAR (le_lpar_t *lp, int build)
{
	...
	if (sum != 0 && sum != 100)
	{
		...
		PRINT0 ("NORM ENGINE: %u + %u + %u = %u [200cc=%d])\n", lp->engine[0], lp->engine[1],
			lp->engine[2], lp->engine[0] + lp->engine[1] + lp->engine[2], lp->enable_200cc);
	}
	...
	lp->block_track = lp->block_track < LE_MAX_BLOCK_TRACK ? lp->block_track : LE_MAX_BLOCK_TRACK;
...
enumError SaveTextLPAR (le_lpar_t *lpar, // LE-CODE parameters
...
enumError SaveTextFileLPAR (le_lpar_t *lpar, // LE-CODE parameters
	FILE *f, // open file
...
static ccp PrintChatMode (u16 msg, u16 mode1, u16 mode2, int numeric)
{
	...
	{
		if (numeric)
			return PrintCircBuf ("M%02u = %4u, %4u # %s, %s", msg, mode1, mode2,
				GetChatModeName (mode1), GetChatModeName (mode2));
		return PrintCircBuf (
			"M%02u = %s, %s", msg, GetChatModeName (mode1), GetChatModeName (mode2));
	}
	else if (mode1 || mode2)
	{
		if (!mode1)
			mode1 = mode2;
		if (numeric)
			return PrintCircBuf ("M%02u = %4u # %s", msg, mode1, GetChatModeName (mode1));
...
{
	return PrintCircBuf ("LE$%sABLE%s%s%s%s%s", flags & LE_ENABLED ? "EN" : "DIS",
		flags & LE_ALTERABLE ? " | LE$ALTERABLE" : "", flags & LE_EXCLUDED ? " | LE$EXCLUDE" : "",
		flags & LE_INCLUDED ? " | LE$INCLUDE" : "", flags & LE_EXTENT ? " | LE$EXTENT" : "",
		flags & LE_REGION ? " | LE$REGION" : "");
}

//-----------------------------------------------------------------------------

enumError WriteSectionLPAR (FILE *f, // output files
...
)
{
	...
	if (verbose)
	{
		...
		fprintf (f, text_lpar_sect_param_cr,
			GetLparModeName (CalcCurrentLparMode (lpar, true), export_count > 0), lpar->cheat_mode,
			lpar->engine[0], lpar->engine[1], lpar->engine[2], lpar->enable_200cc,
			lpar->enable_perfmon, lpar->enable_custom_tt, lpar->enable_xpflags, LE_MAX_BLOCK_TRACK,
			lpar->block_track, GetLecodeSpeedoName (lpar->enable_speedo),
			GetLecodeDebugName (lpar->debug_mode), lpar->item_cheat, lpar->drag_blue_shell,
			...
			lpar_std_flags (lpar->vs_textures & LE_M_TEXTURE), lpar->block_textures,
			lpar->staticr_points, LE_MIN_ONLINE_SEC,
			PrintHMS (0, 0, LE_MIN_ONLINE_SEC, LE_VIEW_ONLINE_HMS, " (", ")"), LE_MAX_ONLINE_SEC,
			PrintHMS (0, 0, LE_MAX_ONLINE_SEC, LE_VIEW_ONLINE_HMS, " (", ")"), LE_MAX_WD_ONLINE_SEC,
			PrintHMS (0, 0, LE_MAX_WD_ONLINE_SEC, LE_VIEW_ONLINE_HMS, " (", ")"),
			lpar->default_online_sec,
			PrintHMS (0, 0, lpar->default_online_sec, LE_VIEW_ONLINE_HMS, "  # ", 0),
			lpar->min_online_sec,
			PrintHMS (0, 0, lpar->min_online_sec, LE_VIEW_ONLINE_HMS, "  # ", 0),
			lpar->max_online_sec,
			PrintHMS (0, 0, lpar->max_online_sec, LE_VIEW_ONLINE_HMS, "  # ", 0),
			lpar->cup_icon_size, lpar->slot_04x, lpar->developer_modes, lpar->dev_mode1,
			lpar->dev_mode2, lpar->dev_mode3);
	}
	else
	{
		// [[new-lpar]]
		fprintf (f,
			...
			GetLparModeName (CalcCurrentLparMode (lpar, true), export_count > 0), lpar->cheat_mode,
			lpar->engine[0], lpar->engine[1], lpar->engine[2], lpar->enable_200cc,
			lpar->enable_perfmon, lpar->enable_custom_tt, lpar->enable_xpflags, lpar->block_track,
			GetLecodeSpeedoName (lpar->enable_speedo), GetLecodeDebugName (lpar->debug_mode),
			lpar->item_cheat, lpar->drag_blue_shell, lpar->thcloud_frames,
			lpar->thcloud_frames / 60.0, lpar_std_flags (lpar->bt_worldwide & LE_M_BT_WORLDWIDE),
			lpar_std_flags (lpar->vs_worldwide & LE_M_VS_WORLDWIDE),
			lpar_std_flags (lpar->bt_textures & LE_M_TEXTURE),
			lpar_std_flags (lpar->vs_textures & LE_M_TEXTURE), lpar->block_textures,
			lpar->staticr_points, lpar->default_online_sec,
			PrintHMS (0, 0, lpar->default_online_sec, LE_VIEW_ONLINE_HMS, " # ", 0),
			lpar->min_online_sec,
			PrintHMS (0, 0, lpar->min_online_sec, LE_VIEW_ONLINE_HMS, " # ", 0),
			lpar->max_online_sec,
	...
	if (verbose)
		fprintf (
			f, text_lpar_sect_debug_docu_cr, LEDEB__N_LINE, LEDEB__N_LINE - 1, LEDEB__N_LINE - 1);
	...
	for (int conf = 0; conf < LEDEB__N_CONFIG; conf++)
	{
		...
		for (int line = 0; line <= LEDEB__N_LINE; line++)
		{
			...
			for (GetFirstDebugMode (&ldi, mode, !verbose); ldi.name; GetNextDebugMode (&ldi))
			{
				...
				fprintf (f, "%s%.*s= %u\r\n", ldi.name, (23 - (int)strlen (ldi.name)) / 8, Tabs20,
					ldi.value);
			}
		}
	}
...
enumError AnalyzeLEBinary (le_analyze_t *ana, // NULL or destination of analysis
...
)
{
	...
	if (!data || data_size < sizeof (le_binary_head_v3_t) || memcmp (data, LE_BINARY_MAGIC, 4))
	{
...
#define DEF_TAB(s, o, p) { offsetof (s, o), offsetof (le_analyze_t, p) },
	...
	if (off_param && !(off_param & 3) && ana->data_size >= off_param + sizeof (le_binary_param_t)
		&& !memcmp (data + off_param, LE_PARAM_MAGIC, 4))
	{
		...
		if (off_param + param_size <= data_size && off_param + param_size <= ana->data_size)
		{
			...
			switch (ana->param_vers)
			{
				case 1:
					...
					{
						static const ptr_ana_t tab[] = { DEF_TAB (le_binpar_v1_35_t, off_eod,
							end_of_data) DEF_TAB (le_binpar_v1_35_t, off_cup_par, cup_par)
										...
										DEF_TAB (le_binpar_v1_35_t, off_course_par, course_par)
											DEF_TAB (le_binpar_v1_35_t, off_property, property)
												DEF_TAB (le_binpar_v1_35_t, off_music, music)
													DEF_TAB (le_binpar_v1_35_t, off_flags,
														flags_bin) { -1, -1 } };
					...
					if (param_size >= sizeof (le_binpar_v1_f8_t))
					{
						...
						memcpy (ana->lpar.chat_mode_1, chat_mode_legacy,
							sizeof (ana->lpar.chat_mode_1));
					...
					if (param_size >= sizeof (le_binpar_v1_260_t))
					{
						...
						memcpy (ana->lpar.debug_predef, p->debug_predef,
							sizeof (ana->lpar.debug_predef));
...
#undef DEF_TAB
#define DEF_TAB(p, n, m, s)                                                                        \
	{ offsetof (le_analyze_t, p), offsetof (le_analyze_t, n), offsetof (le_analyze_t, m),          \
		sizeof (*ana->p) * s },

	static const ana_n_max_t n_max_tab[] = { DEF_TAB (cup_track, n_cup_track, max_cup_track, 4)
			DEF_TAB (cup_arena, n_cup_arena, max_cup_arena, 5)
				DEF_TAB (property, n_slot, max_property, 1) DEF_TAB (music, n_slot, max_music, 1)
					DEF_TAB (flags_bin, n_slot, max_flags, 1) { -1, -1, -1 } };
...
void CalculateStatsLE (le_analyze_t *ana)
{
	...
	ana->max_slot = ana->max_property < ana->max_music ? ana->max_property : ana->max_music;
...
const le_usage_t *GetLEUsage (const le_analyze_t *ana0, bool force_recalc)
{
	...
	memset (usage + ana->le_random_beg, LEU_S_LE_RANDOM | LEU_F_ONLINE,
	...
	uint slot;
	for (slot = 0; slot < MKW_N_TRACKS; slot++)
		usage[slot] = IsLESlotUsed (ana, slot) ? LEU_S_TRACK | LEU_F_ONLINE : LEU_F_ONLINE;
	...
	for (slot = LE_FIRST_CT_SLOT; slot < ana->usage_size; slot++)
	{
		...
		if (IsLESlotUsed (ana, slot))
		{
			if (flags & G_LEFL_ALIAS)
				...
				usage[slot] = flags & G_LEFL_RND_HEAD			 ? LEU_S_ARENA_RND
					: flags & (G_LEFL_RND_GROUP | G_LEFL_HIDDEN) ? LEU_S_ARENA_HIDE
																 : LEU_S_ARENA;
			else
				usage[slot] = flags & G_LEFL_RND_HEAD			 ? LEU_S_TRACK_RND
	...
	if (colset && colset->colorize)
	{
		...
		switch (usage & LEU_S_MASK)
		{
			...
			case LEU_S_ARENA:
				col = colset->b_cyan;
				break;
			case LEU_S_ARENA_HIDE:
				col = colset->b_blue;
				break;
			case LEU_S_TRACK:
				col = colset->b_yellow;
				break;
			case LEU_S_TRACK_HIDE:
				col = colset->b_green;
				break;

			case LEU_S_ARENA_RND:
				col = colset->orange;
				break;
			case LEU_S_TRACK_RND:
				col = colset->b_orange;
				break;
			case LEU_S_LE_RANDOM:
				col = colset->red;
				break;

			case LEU_S_ALIAS:
			default:
				col = colset->b_magenta;
				break;
		}
...
{
	if (!(valid & LE_HEAD_FOUND))
		return "File header not found";
	if (!(valid & LE_HEAD_VALID))
		return "Invalid file header";
	if (!(valid & LE_HEAD_KNOWN))
		return "Unknown file header";
	if (!(valid & LE_HEAD_VERSION))
		return "Unsupported file header version";
	if (!(valid & LE_PARAM_FOUND))
		return "LPAR not found";
	if (!(valid & LE_PARAM_VALID))
		return "Invalid LPAR";
	if (!(valid & LE_PARAM_KNOWN))
		return "Unknown LPAR";
	return 0;
}
...
static ccp GetSlotInfo (const le_analyze_t *ana, // valid structure
	int tid, // track id
...
)
{
	if (IsLecodeRandom (tid))
		return PrintCircBuf ("[%s%-7.7s%s,%s] ", col->info, GetLecodeRandomName (tid, "random"),
			col->reset, PrintLEFL (ana->flags_bits, ana->flags[tid], true));
	...
	if (tid < ana->n_slot || IsMkwArena (tid))
	{
		...
		return flags & G_LEFL_ALIAS
			? PrintCircBuf ("[%s->%4x %s,%s] ", col->differ, TrackByAliasLE (prop, music),
				  col->reset, PrintLEFLcol (ana->flags_bits, flags, true, col))
			: PrintCircBuf ("[%s%s%s,%s%s%s,%s]%c", prop1, propx, prop0, music1, musicx, music0,
...
static le_cup_track_t *DumpLECup (FILE *f, // print file
...
)
{
	...
	for (tr = 0; tr < n_track; tr++)
	{
		...
		{
			case CHECK_TRACK:
				highlight = !IsLecodeTrack (tid);
				break;
			case CHECK_ARENA:
				highlight = !IsMkwArena (tid);
				break;
			default:
				highlight = IsMkwSpecial (tid);
				break;
		}

		fprintf (f, format, highlight ? col->info : "", tid, highlight ? col->reset : "",
...
static void DumpLETracks (FILE *f, uint indent, ColorSet_t *col, const le_analyze_t *ana)
{
	DASSERT (f);
	DASSERT (indent >= 2);
	DASSERT (col);
	...
	if (ana->head->v3.phase == 1)
	{
		uint max = ana->n_slot < LE_FIRST_UPPER_CT_SLOT ? ana->n_slot : LE_FIRST_UPPER_CT_SLOT;
		uint i;
		for (i = 0xff; i < max; i++)
			done[i]++;
	}
	...
	fprintf (f,
		...
		"%*s"
		"The general syntax is: %shex_id [ property_slot, music_index, flags ] warning%s\n",
		indent - 2, "", col->heading, ana->flags_bits, col->reset, indent, "", col->param,
		col->reset);
	...
	if (ana->flags_bits == 16)
		fprintf (f,
			...
			indent + 2, "", col->param, col->reset, col->info, col->reset, col->info, col->reset,
			col->info, col->reset, indent + 2, "", col->param, col->reset, col->info, col->reset,
			col->info, col->reset, col->info, col->reset, indent + 2, "", col->param, col->reset,
			col->info, col->reset, col->info, col->reset, col->info, col->reset, indent + 2, "",
			col->param, col->reset, col->info, col->reset, col->info, col->reset, col->info,
			col->reset, indent + 2, "", col->param, col->reset, col->info, col->reset, col->info,
			col->reset);
	else
		fprintf (f,
			"%*s%sFlags%s: "
			...
			indent, "", col->param, col->reset, col->info, col->reset, col->info, col->reset,
			col->info, col->reset, col->info, col->reset, col->info, col->reset, col->info,
			col->reset);

	//--- battle cups

	ccp format_cup
		= ana->n_cup_arena < 100 && ana->n_cup_track < 1000 ? "%*sCup %3u:" : "%*sCup %4u:";

	if (ana->n_cup_arena && ana->cup_arena)
	{
		fprintf (f,
			"\n%*s%s"
			"%u cup%s with %u battle arenas:%s\n",
			indent - 2, "", col->heading, ana->n_cup_arena, ana->n_cup_arena == 1 ? "" : "s",
	...
	if (ana->n_cup_track && ana->cup_track)
	{
		fprintf (f,
			"\n%*s%s"
			"%u cup%s with %u racing tracks:%s\n",
			indent - 2, "", col->heading, ana->n_cup_track, ana->n_cup_track == 1 ? "" : "s",
	...
	if (n_multi)
	{
		fprintf (f,
			"\n%*s%s"
			"%u multiple used track slot%s:%s\n",
			indent - 2, "", col->heading, n_multi, n_multi == 1 ? "" : "s", col->reset);

		for (i = 0; i < ana->n_slot; i++)
			if (done[i] > 1 && IsRacingTrackLE (i))
				fprintf (f, "%*sSlot %4d/dec : %3x%s : %3ux\n", indent, "", i, i,
					GetSlotInfo (ana, i, col, CHECK_OFF, 0), done[i]);
	}
	...
	if (n_group)
	{
		fprintf (f,
			"\n%*s%s"
			"%u track slot%s reserved for random groups:%s\n",
			indent - 2, "", col->heading, n_group, n_group == 1 ? "" : "s", col->reset);
	...
	if (n_unused)
	{
		...
		if (count > 0)
		{
			fprintf (f,
				"\n%*s%s"
				"%u track%s without cup reference:%s",
				indent - 2, "", col->heading, count, count == 1 ? "" : "s", col->reset);
...
static void DumpLEUsageMap (FILE *f, uint indent, ColorSet_t *col, const le_analyze_t *ana)
{
	DASSERT (f);
	DASSERT (indent >= 2);
	DASSERT (col);
	...
	fprintf (f,
		"\n%*s%s"
		"Slot usage map for %u slots%s\n"
		"%*s"
		"%s: Arena%s, %s: Hidden Arena%s, %s: Random Group Arena%s, "
		"%s: LE-CODE random%s, %s: Alias%s,\n"
		"%*s"
		"%s: Track%s, %s: Hidden Track%s, %s: Random Group Track%s, "
		"%s: Network%s, %s: Special%s, %s: unused%s.\n",

		indent - 2, "", col->heading, max_view, col->reset, indent, "",
		...
		GetLEUsageCharCol (LEU_F_ONLINE | LEU_S_ALIAS, col, 0), col->reset, indent, "",
...
{
	return PrintCircBuf ("%u (%sabled%s%s%s%s%s)", flags, flags & LE_ENABLED ? "en" : "dis",
		flags & LE_ALTERABLE ? ",alterable" : "", flags & LE_EXCLUDED ? ",exclude" : "",
		flags & LE_INCLUDED ? ",include" : "", flags & LE_EXTENT ? ",extent" : "",
...
void DumpLEAnalyse (FILE *f, uint indent, const le_analyze_t *ana)
{
	...
	if (invalid)
		fprintf (f, "%*s%sLE-CODE binary, %s%s%s\n", indent, "", col.caption, col.warn, invalid,
			col.reset);
	else
	...
	if (ana->valid & LE_HEAD_VALID)
	{
		...
		fprintf (f,
			"%*s"
			"Header size:       %8x/hex = %6u bytes\n"
			"%*s"
			"Total size:        %8x/hex = %6u bytes\n"
			"%*s"
			"Offset of LPAR:    %8x/hex\n"
			"%*s"
			"Base address:      %8x/hex\n"
			"%*s"
			"Entry point:       %8x/hex\n",
		...
		if (ana->szs_required || ana->szs_recommended)
		{
			...
			if (ana->szs_required)
			{
				...
				fprintf (f,
					"%*s"
					"Required SZS:      v%-*s (%s%s%s)\n",
					indent, "", fw, required, valid ? col.success : col.warn,
			...
			if (ana->szs_recommended)
			{
				...
				fprintf (f,
					"%*s"
					"Recommended SZS:   v%-*s (%s%s%s)\n",
					indent, "", fw, recommended, valid ? col.success : col.warn,
					valid ? "ok" : "outdated", col.reset);
			}
		}
	...
	if (ana->valid & LE_PARAM_VALID)
	{
		...
		if (sizeof (le_binpar_v1_t) < lpar_size)
			fprintf (f,
				"%*s"
				"  Supported size:  %s%zx/hex = %zu bytes   Warning!%s\n",
				indent, "", col.warn, sizeof (le_binpar_v1_t), sizeof (le_binpar_v1_t), col.reset);
	}
	...
	switch (ana->param_vers)
	{
		case 1:
		{
			...
			if (ana->param_size >= sizeof (le_binpar_v1_274_t))
				fprintf (f,
					"%*s"
					"Developer modes:   %u = %sabled / Modes: %u, %u, %u\n",
					indent, "", h->developer_modes, h->developer_modes ? "en" : "dis", h->dev_mode1,
					h->dev_mode2, h->dev_mode3);

			if (ana->param_size >= sizeof (le_binpar_v1_264_t))
				fprintf (f,
					"%*s"
					"Allow cheat codes: %u = %s\n",
			...
			if (ana->param_size >= sizeof (le_binpar_v1_260_t))
			{
				fprintf (f,
					...
					"%*s"
					"Debug mode:        %u = %s\n",
					indent, "", h->item_cheat, h->item_cheat ? "enabled" : "disabled", indent, "",
					h->debug_mode, GetLecodeDebugInfo (h->debug_mode));
			}
			...
			if (ana->param_size >= sizeof (le_binpar_v1_276_t))
				fprintf (f,
					"%*s"
					"Cup icon size:     %u x %u pixels (%u bytes)\n",
			...
			fprintf (f,
				...
				indent, "", h->enable_200cc, h->enable_200cc ? "en" : "dis", indent, "",
			...
			if (ana->param_size >= sizeof (le_binpar_v1_37_t))
				fprintf (f,
					"%*s"
					"Custom Timetrial:  %u = %sabled\n"
					"%*s"
					"Extended P-Flags:  %u = %sabled\n"
					"%*s"
					"Block tracks for:  %u race%s\n",
					indent, "", h->enable_custom_tt, h->enable_custom_tt ? "en" : "dis", indent, "",
					h->enable_xpflags, h->enable_xpflags ? "en" : "dis", indent, "", h->block_track,
					h->block_track == 1 ? "" : "s");
			...
			if (ana->param_size >= sizeof (le_binpar_v1_264_t))
			{
				...
				fprintf (f,
					"%*s"
					"Drag blue shells:  %u = %sabled\n"
					"%*s"
					"Thundercloud time: %u frames = %.2fs\n",
					indent, "", h->drag_blue_shell, h->drag_blue_shell ? "en" : "dis", indent, "",
					nframes, nframes / 60.0);
			}
			...
			if (ana->param_size >= sizeof (le_binpar_v1_269_t))
			{
				fprintf (f,
					"%*s"
					"Worldwide:         battle=%s, versus=%s\n"
					"%*s"
					"Textures:          battle=%s, versus=%s\n"
					"%*s"
					"Block textures:    %u = %sabled\n",
					indent, "", ana_std_flags (h->bt_worldwide & LE_M_BT_WORLDWIDE),
					ana_std_flags (h->vs_worldwide & LE_M_VS_WORLDWIDE), indent, "",
					ana_std_flags (h->bt_textures & LE_M_TEXTURE),
					ana_std_flags (h->vs_textures & LE_M_TEXTURE), indent, "", h->block_textures,
					h->block_textures ? "en" : "dis");
			}

			if (ana->param_size >= sizeof (le_binpar_v1_26a_t))
				fprintf (f,
					"%*s"
					"Points by StaticR: %u = %sabled\n",
			...
			if (ana->param_size >= sizeof (le_binpar_v1_270_t))
			{
				...
				fprintf (f,
					"%*s"
					"Online time limit: %us%s / Range for LEX: %us%s .. %us%s\n",
					indent, "", def, PrintHMS (0, 0, def, LE_VIEW_ONLINE_HMS, " (", ")"), min,
					PrintHMS (0, 0, min, LE_VIEW_ONLINE_HMS, " (", ")"), max,
					PrintHMS (0, 0, max, LE_VIEW_ONLINE_HMS, " (", ")"));
			}
	...
	if ( ana->param_size >= sizeof(le_binpar_v1_1bc_t) )
	{
		...
		fprintf(f,"%*s" "Reserved 1BB:      %u\n",
			...
			fprintf (f,
				"%*s"
				"Offset cup params: %5x/hex => %5x/hex\n"
				"%*s"
				"Offset cup tracks: %5x/hex => %5x/hex\n"
				"%*s"
				"Offset cup arenas: %5x/hex => %5x/hex\n"
				"%*s"
				"Offset course par: %5x/hex => %5x/hex\n"
				"%*s"
				"Offset properties: %5x/hex => %5x/hex\n"
				"%*s"
				"Offset music list: %5x/hex => %5x/hex\n",
				indent, "", ntohl (h->off_cup_par), ntohl (h->off_cup_par) + po, indent, "",
				ntohl (h->off_cup_track), ntohl (h->off_cup_track) + po, indent, "",
				ntohl (h->off_cup_arena), ntohl (h->off_cup_arena) + po, indent, "",
				ntohl (h->off_course_par), ntohl (h->off_course_par) + po, indent, "",
				ntohl (h->off_property), ntohl (h->off_property) + po, indent, "",
				ntohl (h->off_music), ntohl (h->off_music) + po);
#endif
			...
			if (n_special)
			{
				fprintf (f,
					"\n%*s%s"
					"%u special chat message%s%s:%s\n",
					indent - 2, "", col.heading, n_special, n_special == 1 ? "" : "s",
					hard_coded ? " (hard coded)" : "", col.reset);
	...
	fprintf (f,
		"\n%*s%s"
		"Sections (offset,n/max):%s\n",
		indent - 2, "", col.heading, col.reset);

	if (ana->cup_par)
		fprintf (f,
			"%*s"
			"Cup parameters:    %5x/hex\n",
			indent, "", PTR_DISTANCE_INT (ana->cup_par, ana->data));

	if (ana->cup_track)
		fprintf (f,
			"%*s"
			"Racing track cups: %5x/hex, %4u / %4u%s\n",
			indent, "", PTR_DISTANCE_INT (ana->cup_track, ana->data), ana->n_cup_track,
			ana->max_cup_track, ana->n_cup_track > ana->max_cup_track ? fail : "");

	if (ana->cup_arena)
		fprintf (f,
			"%*s"
			"Battle arena cups: %5x/hex, %4u / %4u%s\n",
			indent, "", PTR_DISTANCE_INT (ana->cup_arena, ana->data), ana->n_cup_arena,
			ana->max_cup_arena, ana->n_cup_arena > ana->max_cup_arena ? fail : "");

	if (ana->course_par)
		fprintf (f,
			"%*s"
			"Course parameters: %5x/hex\n",
			indent, "", PTR_DISTANCE_INT (ana->course_par, ana->data));

	if (ana->property)
		fprintf (f,
			"%*s"
			"Property list:     %5x/hex, %4u / %4u%s\n",
			indent, "", PTR_DISTANCE_INT (ana->property, ana->data), ana->n_slot, ana->max_property,
			ana->n_slot > ana->max_property ? fail : "");

	if (ana->music)
		fprintf (f,
			"%*s"
			"Music list:        %5x/hex, %4u / %4u%s\n",
			indent, "", PTR_DISTANCE_INT (ana->music, ana->data), ana->n_slot, ana->max_music,
			ana->n_slot > ana->max_music ? fail : "");

	if (ana->flags)
		fprintf (f,
			"%*s"
			"Flags list:        %5x/hex, %4u / %4u (%u bits)%s\n",
			indent, "", PTR_DISTANCE_INT (ana->flags_bin, ana->data), ana->n_slot, ana->max_flags,
			ana->flags_bits, ana->n_slot > ana->max_flags ? fail : "");
	...
	if (ana->cup_par || ana->course_par)
	{
		fprintf (f,
			"\n%*s%s"
			"Cup and course parameters (n/max):%s\n",
			indent - 2, "", col.heading, col.reset);
		...
		if (ana->cup_par)
			fprintf (f,
				...
				"Cup magic:         %s\n"
				"%*s"
				"Racing track cups: %4u / %4u%s\n"
				"%*s"
				"Battle arena cups: %4u / %4u%s\n",
				indent, "", ana->cup_par->magic, indent, "", ana->n_cup_track, ana->max_cup_track,
				ana->n_cup_track > ana->max_cup_track ? fail : "", indent, "", ana->n_cup_arena,
				ana->max_cup_arena, ana->n_cup_arena > ana->max_cup_arena ? fail : "");
		...
		if (ana->course_par)
		{
			fprintf (f,
				...
				"Course magic:      %s\n"
				"%*s"
				"Total track slots: %4u / %4u%s\n",
				indent, "", ana->course_par->magic, indent, "", ana->n_slot, ana->max_slot,
				ana->n_slot > ana->max_slot ? fail : "");

			if (LE_CODE_MAX_TRACKS < ana->max_slot)
				fprintf (f,
					"%*s"
					"  Supported slots:%s%12u  Warning!%s\n",
					indent, "", col.warn, LE_CODE_MAX_TRACKS, col.reset);

			fprintf (f,
				"%*s"
				"Used racing slots: %4u / %4u\n"
				"%*s"
				"Used battle slots: %4u / %4u\n",
				indent, "", ana->used_rslots, ana->max_rslots, indent, "", ana->used_bslots,
				ana->max_bslots);
		}
	}
...
{
	if (opt_track_dest && IsDirectory (opt_track_dest, false) && opt_track_source.used)
	{
		char src[PATH_MAX];
		char dest[PATH_MAX];
		flags &= TFMD_M_FLAGS;
...
{
	if (IsValidLecodeSlot (dest_slot) && IsValidLecodeSlot (src_slot) && opt_track_dest
...
enumError ApplyCTCODE (le_analyze_t *ana, const ctcode_t *ctcode)
{
	...
	if (prop && music && flags)
	{
		const uint max = ana->max_slot < LE_FIRST_CT_SLOT ? ana->max_slot : LE_FIRST_CT_SLOT;
		uint tid;
	...
	for (; bi < bi_end; bi++)
	{
		PrintString16BMG (buf, sizeof (buf), bi->text, bi->len, BMG_UTF8_MAX, 0, true);
...
bool DefineAliasLE (le_analyze_t *ana, uint slot, uint alias)
{
	...
	for (;;)
	{
		if (alias >= ana->n_slot || alias >= MKW_ARENA_BEG && alias < LE_FIRST_CT_SLOT)
		{
...
void PatchLPAR (le_lpar_t *lp)
{
	...
	if (opt_speedo != OFFON_AUTO)
		lp->enable_speedo = opt_speedo >= SPEEDO_MAX ? SPEEDO_MAX
			: opt_speedo >= SPEEDO_ON				 ? opt_speedo
													 : 0;
	...
	if (ana->n_cup_track && ana->cup_track)
	{
		...
		for (int tr = 0; tr < max_tr; tr++)
		{
			...
			if (slot < ana->max_flags && ana->flags[slot] & (G_LEFL_VERSUS | G_LEFL_RANDOM))
				ana->flags[slot]
					|= G_LEFL_CUP | (tr < MKW_N_TRACKS ? G_LEFL_ORIG_CUP : G_LEFL_CUSTOM_CUP);
		}
	}
	...
	if (ana->n_cup_arena && ana->cup_arena)
	{
		...
		for (int tr = 0; tr < max_tr; tr++)
		{
			...
			if (slot < ana->max_flags && ana->flags[slot] & (G_LEFL_BATTLE | G_LEFL_RANDOM))
				ana->flags[slot]
					|= G_LEFL_CUP | (tr < MKW_N_ARENAS ? G_LEFL_ORIG_CUP : G_LEFL_CUSTOM_CUP);
		}
	}
}
...
static enumError ScanTextLPAR_PARAM (le_lpar_t *lpar, // LE-CODE parameters
...
)
{
	...
	const ScanParam_t ptab[] = { { "LIMIT-MODE", SPM_INT, &lpar->new_limit_mode },
		{ "DEVELOPER-MODES", SPM_U8, &lpar->developer_modes },
		{ "DEV-MODE1", SPM_U8, &lpar->dev_mode1 }, { "DEV-MODE2", SPM_U8, &lpar->dev_mode2 },
		{ "DEV-MODE3", SPM_U8, &lpar->dev_mode3 }, { "CHEAT-MODE", SPM_U8, &lpar->cheat_mode },
		{ "ENGINE", SPM_U8, lpar->engine, 3, 3 }, { "ENABLE-200CC", SPM_U8, &lpar->enable_200cc },
		{ "PERF-MONITOR", SPM_U8, &lpar->enable_perfmon },
		{ "CUSTOM-TT", SPM_S8, &lpar->enable_custom_tt },
		{ "XPFLAGS", SPM_U8, &lpar->enable_xpflags }, { "BLOCK-TRACK", SPM_U8, &lpar->block_track },
		{ "SPEEDOMETER", SPM_U8, &lpar->enable_speedo }, { "DEBUG", SPM_U8, &lpar->debug_mode },
		...
		{ "CUP-ICON-SIZE", SPM_U8, &lpar->cup_icon_size }, { "SLOT-04X", SPM_U8, &lpar->slot_04x },
		// [[new-lpar]]
...
static enumError ScanTextLPAR_CHAT (le_lpar_t *lpar, // LE-CODE parameters
	ScanInfo_t *si, // valid data
	bool is_pass2 // false: pass1; true: pass2
)
{
	...
	const ScanParam_t ptab[] = { { "RESET", SPM_BOOL, &reset }, { "LEGACY", SPM_BOOL, &legacy },
		{ "MKW-FUN", SPM_BOOL, &mkwfun }, { 0 } };
...
static enumError ScanTextLPAR_DEBUG (le_lpar_t *lpar, // LE-CODE parameters
	ScanInfo_t *si, // valid data
	bool is_pass2, // false: pass1; true: pass2
	uint config // configuration index 0..
)
{
	...
	const ScanParam_t ptab[] = { { "SETUP", SPM_INT, &setup },
		{ "HIDE-SPEEDO", SPM_INT, &hide_speedo }, { "LINE", SPM_INT, &next_line },
		{ "ENABLED", SPM_U8, &debug.enabled }, { "OPPONENT", SPM_U8, &debug.opponent },
		{ "SPACE", SPM_U8, &debug.space }, { "POSITION", SPM_U8, &debug.position },
		{ "CHECK-POINT", SPM_U8, &debug.check_point }, { "ITEM-POINT", SPM_U8, &debug.item_point },
		{ "KCL-TYPE", SPM_U8, &debug.kcl_type }, { "RESPAWN", SPM_U8, &debug.respawn },
		{ "LAP-POS", SPM_U8, &debug.lap_pos }, { "TRACK-ID", SPM_U8, &debug.track_id },
...
enumError LoadLPAR (le_lpar_t *lpar, // LE-CODE parameters
...
)
{
	...
	if (!err)
	{
		if (raw.fform != FF_LPAR)
			err = ERROR0 (ERR_WRONG_FILE_TYPE, "Not a LPAR file: %s:%s\n", GetNameFF (raw.fform, 0),
				raw.fname);
		else
...
enumError ScanTextLPAR (le_lpar_t *lpar, // LE-CODE parameters
	bool init_lpar, // true: initialize 'lpar' first
...
)
{
	...
	static const KeywordTab_t section_name[] = { { C_PARAM, "LECODE-PARAMETERS", 0, 0 },
		{ C_CHAT, "CHAT-MESSAGE-MODES", 0, 0 }, { C_END, "DEBUG-DOCU", 0, 0 },
		{ C_DEBUG1, "DEBUG-1", 0, 0 }, { C_DEBUG2, "DEBUG-2", 0, 0 }, { C_DEBUG3, "DEBUG-3", 0, 0 },
		{ C_DEBUG4, "DEBUG-4", 0, 0 }, { C_END, "END", 0, 0 }, { 0, 0, 0, 0 } };
