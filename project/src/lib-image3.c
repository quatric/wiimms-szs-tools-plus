...
enumError CreateIMG (Image_t *img, // image object
...
enumError CopyRectIMG (Image_t *dest, // dest image
...
enumError DrawPointIMG (Image_t *img, // image
...
enumError DrawHLineIMG (Image_t *img, // image
...
enumError DrawVLineIMG (Image_t *img, // image
...
enumError DrawFrameIMG (Image_t *img, // image
...
enumError FillRectIMG (Image_t *img, // image
	int x1, // left coordinate (robust)
	int y1, // top coordinate (robust)
	int x2, // right coordinate (robust)
	int y2, // bottom coordinate (robust)
...
enumError FillHGradientIMG (Image_t *img, // image
	int x1, // left coordinate (robust)
	int y1, // top coordinate (robust)
	int x2, // right coordinate (robust)
	int y2, // bottom coordinate (robust)
...
enumError FillVGradientIMG (Image_t *img, // image
	int x1, // left coordinate (robust)
	int y1, // top coordinate (robust)
	int x2, // right coordinate (robust)
	int y2, // bottom coordinate (robust)
...
enumError ResizeIMG (Image_t *dest, // dest image (may be same as src)
...
{
	return fast_resize_enabled ? FastResizeIMG (dest, init_dest, src, width, height)
							   : SmartResizeIMG (dest, init_dest, src, width, height);
}

///////////////////////////////////////////////////////////////////////////////

enumError FastResizeIMG (Image_t *dest, // dest image (may be same as src)
	bool init_dest, // true: initialize 'dest' first
	const Image_t *src, // source image (if NULL: use dest)
	uint width, // new width of image
	uint height // new height of image
...
)
{
	...
	if (src->iform != IMG_X_RGB)
	{
		...
		if (src->iform != IMG_X_RGB)
			return ERROR0 (ERR_INTERNAL, "Image format 'X-RGB' expected, but '%s' found\n",
				PrintFormatIMG (src));
	}
		...
		else
		{
			...
			for (wd = 1; wd <= wmax; wd++)
			{
				...
				noPRINT ("COPY: %4u..%4u*%4u..%4u -> %4u..%4u*%4u..%4u [ %2u*%2u -> %2u*%2u ]\n",
					ws1, ws2, hs1, hs2, wd1, wd2, hd1, hd2, wsn, wdn, hsn, hdn);
				DASSERT (wsn >= 1 && wdn >= 1);
				DASSERT (wsn == 1 || wdn == 1);
				...
				else if (hsn == 2)
				{
					...
					while (nw-- > 0)
					{
						sum[0] += (uint)*src2 + src2[src_line_size];
						src2++;
						sum[1] += (uint)*src2 + src2[src_line_size];
						src2++;
						sum[2] += (uint)*src2 + src2[src_line_size];
						src2++;
						sum[3] += (uint)*src2 + src2[src_line_size];
						src2++;
					}
				}
				else
...
enumError SmartResizeIMG (Image_t *dest, // dest image (may be same as src)
	bool init_dest, // true: initialize 'dest' first
	const Image_t *src, // source image (if NULL: use dest)
	uint width, // new width of image
	uint height // new height of image
...
)
{
	...
	for (v_ok = FirstResize (&rv); v_ok; v_ok = NextResize (&rv))
	{
			...
			for (re = rv.elem; re->n_elem; re++)
			{
				...
				for (ne = re->n_elem; ne > 0; ne--)
				{
					noPRINT ("COPY %u, %u: %zu / %zu / %u\n", rv.src_idx, ne,
						(src2 - src->data) / src_line_size, (src3 - src->data) / src_line_size,
						src->height);
		...
		noPRINT ("DEST1: %u/4 = %u, line %u.%u/%u\n", (int)(dest1 - data),
			(int)(dest1 - data) / bytes_per_pixel, (int)(dest1 - data) / (bytes_per_pixel * xwidth),
			(int)(dest1 - data) % (bytes_per_pixel * xwidth), height);
	}
	DASSERT_MSG (dest1 == data + (bytes_per_pixel * xwidth) * height,
		"%p - %p %zd, dest= %lu, col = %lu.%lu/%u line\n", dest1, data + data_size,
		dest1 - data - data_size, (dest1 - data) / bytes_per_pixel,
		(dest1 - data) / (bytes_per_pixel * xwidth), (dest1 - data) % (bytes_per_pixel * xwidth),
		height);
	FREE (temp_src);
...
enumError HalfIMG (Image_t *img // source and dest
)
{
	DASSERT (img);
	Transform2XIMG (img);
	...
	PRINT ("HalfIMG() %u*%u -> %u*%u -> %u*%u\n", img->width, img->height, newwd, newht, newxwd,
		newxht);
	...
	if (img->iform == IMG_X_GRAY)
	{
		...
		while (ih-- > 0)
		{
			...
			{
				*dest2++
					= (*src2 + src2[pixel_bytes] + src2[src_add] + src2[src_add + pixel_bytes] + 2)
					/ 4;
				src2++;
				*dest2++
					= (*src2 + src2[pixel_bytes] + src2[src_add] + src2[src_add + pixel_bytes] + 2)
			...
			noPRINT ("%3u: %u*%u %5zx..%5zx/%x -> %u*%u %5zx..%5zx/%x\n", newht - ih - 1,
				img->width, img->height, src1 - img->data, src2 - img->data, img->data_size, newwd,
				newht, dest1 - data, dest2 - data, data_size);
...
static PatchImage_t calc_copy_range (PatchRange1_t *dest, PatchRange1_t *src1, PatchRange1_t *src2,
	PatchImage_t patch_mode, PatchImage_t align_mode)
{
	DASSERT (dest);
	DASSERT (src1);
	DASSERT (src2);
...
enumError PatchIMG (Image_t *dest_img, // destination image, can be one of the sources
...
)
{
	...
	patch_mode
		= calc_copy_range (&dest.x, &src1.x, &src2.x, patch_mode, patch_mode >> PIM_SHIFT_V_H);
	patch_mode = calc_copy_range (&dest.y, &src1.y, &src2.y, patch_mode, patch_mode);

	noPRINT (" - COPY-X: %3d..%-3d -> %3d..%-3d  |  %3d..%-3d -> %3d..%-3d  | %3d %3d -> %3d\n",
		src1.x.src, src1.x.src + src1.x.len, src1.x.dest, src1.x.dest + src1.x.len, src2.x.src,
		src2.x.src + src2.x.len, src2.x.dest, src2.x.dest + src2.x.len, src1.x.size, src2.x.size,
		dest.x.size);
	noPRINT (" - COPY-Y: %3d..%-3d -> %3d..%-3d  |  %3d..%-3d -> %3d..%-3d  | %3d %3d -> %3d\n",
		src1.y.src, src1.y.src + src1.y.len, src1.y.dest, src1.y.dest + src1.y.len, src2.y.src,
		src2.y.src + src2.y.len, src2.y.dest, src2.y.dest + src2.y.len, src1.y.size, src2.y.size,
		dest.y.size);

	//--- create the new image

	if (dest_img == src_img1 && dest.x.size == src1.x.size && dest.y.size == src1.y.size)
	{
		//--- one direct operation

		PRINT ("DIRECT IMAGE OPERATION\n");
		CopyRectIMG (dest_img, src2.x.dest, src2.y.dest, src_img2, src2.x.src, src2.y.src,
			src2.x.len, src2.y.len, patch_mode);
	}
	else
	{
		...
		CopyRectIMG (&temp_img, src1.x.dest, src1.y.dest, src_img1, src1.x.src, src1.y.src,
			src1.x.len, src1.y.len, PIM_COPY);

		CopyRectIMG (&temp_img, src2.x.dest, src2.y.dest, src_img2, src2.x.src, src2.y.src,
			src2.x.len, src2.y.len, patch_mode);
...
enumError PatchListIMG (Image_t *img // image to transform
)
{
	DASSERT (img);
...
void PrintImageHead (int line_indent, // indention of complete line
...
static ccp SizeColor (u32 size)
{
	...
	return m != size ? colset->warn : m > 1024 ? colset->hint : "";
}

///////////////////////////////////////////////////////////////////////////////

int PrintImage (const void *data, // valid data pointer
...
)
{
	...
	if (geo)
	{
		printf ("%-6s %02x %-6s", GetNameFF (0, img.info_fform), img.iform,
			GetImageFormatName (img.iform, "?"));
		...
		if (long_count)
		{
			...
			printf (" %s%4u%s*%s%-4u%s %8u", wcol, img.width, *wcol ? TermTextModeReset : "", hcol,
				img.height, *hcol ? TermTextModeReset : "",
				img.xwidth * img.xheight * geo->bits_per_pixel / 8);
		}

		if (long_count > 1)
			printf ("  %c%c%c %u*%u", geo->is_gray ? 'G' : '-', geo->has_alpha ? 'A' : '-',
				geo->max_palette ? 'P' : '-', geo->block_width, geo->block_height);
