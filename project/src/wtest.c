...
static int test_matrix (int argc, char **argv)
{
	...
	double dtab[] = { -10000, -5000, -10000 / 3.0, 0, 10000.0 / 3.0, 5000, 10000, d.x, d.y, d.z };
#undef N_NUM
...
#define N_VEC (N_NUM * N_NUM * N_NUM)
	...
	for (pass = 0; pass < n_pass; pass++)
	{
		...
		CloseTransformation ();
		global_check_mode
			= opt_no_check ? 0 : GetCheckMode (false, brief_count > 0, !verbose, long_count > 0);

		if (verbose)
		{
			char buf[60];
			uint len = snprintf (buf, sizeof (buf), "#####  PASS %u/%u [err=%u]  #####", pass + 1,
				n_pass, err_count);
		...
		if (fail >= 0)
		{
			...
			printf ("\n\n"
					...
					" %5.3f,%5.3f,%5.3f"
					"\n\n",
				m->scale.x, m->scale.y, m->scale.z, m->scale_origin.x, m->scale_origin.y,
				m->scale_origin.z, m->shift.x, m->shift.y, m->shift.z, m->rotate_deg.v[0],
				m->rotate_origin[0].x, m->rotate_origin[0].y, m->rotate_origin[0].z,
				m->rotate_deg.v[1], m->rotate_origin[1].x, m->rotate_origin[1].y,
				m->rotate_origin[1].z, m->rotate_deg.v[2], m->rotate_origin[2].x,
				m->rotate_origin[2].y, m->rotate_origin[2].z, m->translate.x, m->translate.y,
				m->translate.z, vtab[fail].x, vtab[fail].y, vtab[fail].z);
		}
	}
...
void CMPR_distance (const u8 *data, // source data
...
static enumError test_cmpr (int argc, char **argv)
{
	...
	for (i = 0; i < n_info; i++)
	{
		...
		while (n-- > 0)
			...
			{
				case 0:
					CMPR_distance (vector[0], info, false);
					break;
				case 1:
					CMPR_distance (vector[0], info, true);
					break;
				case 2:
					CMPR_wiimm (vector[0], info);
					break;
				default:
					ASSERT (0);
			}
...
static enumError save_image (Image_t *img, // pointer to valid img
...
static int test_resize (int argc, char **argv)
{
	...
	printf ("\nRESIZE: %u -> %u, step: %u + %u/%u, sum_factor: %u\n\n", r.src_size, r.dest_size,
		r.idx_inc, r.frac_inc, r.frac_next, r.sum_factor);
...
static int resize_image (int argc, char **argv)
{
	...
	for (i = 3; i < argc; i++)
	{
		...
		const uint ht = aspect ? img1.height * wd / img1.width
			: ht_percent	   ? (img1.height * ht_param + 50) / 100
							   : ht_param;
...
static int test_filename (int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		SubstDest (dest, sizeof (dest), param.arg, opt_dest && *opt_dest ? opt_dest : "\1P/\1N\1?T",
			"\1N\1?T", GetExtFF (FF_BZ, FF_WU8), false);
		printf ("dest: %s\n", dest);
...
static enumError draw_startpos_helper (uint n)
{
	...
	col1.val = 0xff0000ff;
	FillRectIMG (&imgw, 0, (zmid - 25) * scale, imgw.width, (zmid + 25) * scale, col1, true);
	...
	for (i = 0; i < n; i++)
	{
		...
		FillRectIMG (&imgn, (xmid + d->x - 100) * scale, (zmid + d->z - 180) * scale,
			(xmid + d->x + 100) * scale, (zmid + d->z + 180) * scale, col1, true);

		FillRectIMG (&imgx, (xmid + d->x - 100) * scale, (zmid + d->z - 180) * scale,
			(xmid + d->x + 100) * scale, (zmid + d->z + 180) * scale, col2, true);

		//--- draw wide

		d = tab_w + i;
		FillRectIMG (&imgw, (xmid + d->x - 100) * scale, (zmid + d->z - 180) * scale,
			(xmid + d->x + 100) * scale, (zmid + d->z + 180) * scale, col1, true);
	...
	FillRectIMG (&imgw, (xmid - 80) * scale, (zmid - 80) * scale, (xmid + 80) * scale,
		(zmid + 80) * scale, col1, true);

	FillRectIMG (&imgn, (xmid - 80) * scale, (zmid - 80) * scale, (xmid + 80) * scale,
		(zmid + 80) * scale, col1, true);

	FillRectIMG (&imgx, (xmid - 80) * scale, (zmid - 80) * scale, (xmid + 80) * scale,
		(zmid + 80) * scale, col1, true);
...
#define PLEN (sizeof (odata->param) / sizeof (*odata->param))
		...
		uint linelen = printf (" Pos   ID %-*s", max_name_size, "Name") + max_resources_size + 3;
		uint p;
		for (p = 0; p < PLEN; p++)
			if (param_fw[p])
			{
...
static enumError scan_double (double *pt, int n_dim, ScanInfo_t *si, double def_x, double def_y)
{
	DASSERT (pt);
	DASSERT (si);
...
static enumError scan_float (float *pt, int n_dim, ScanInfo_t *si, double def_x, double def_y)
{
	DASSERT (pt);
	DASSERT (si);
...
static int SubFileIt (SubFileIterator_t *sfi, // iterator data
...
static enumError test_vr (int argc, char **argv)
{
	const struct pair
	{
		int delta;
		int vr_add;
	} known[] = { { -20000, 112 }, { -9998, 112 }, { -9980, 112 }, { -9890, 111 }, { -9190, 105 },
		{ -8000, 92 }, { -6000, 67 }, { -4000, 44 }, { -2000, 26 }, { -1000, 19 }, { 0, 13 },
		{ 2000, 6 }, { 4000, 2 }, { 6000, 1 }, { 8000, 0 }, { 9980, 0 }, { 9998, 0 }, { 20000, 0 },
		{ -1, -1 } };
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (verbose || !strcmp (argv[i], "test"))
		{
			...
			printf ("-------------------------------------------\n"
					"    >>> %u of %u differ,%s\n",
				count, total, verbose ? "" : " [use test+ to show all]");
			continue;
		}
...
int GetVr1DiffByTab (int diff) // diff = VR(winner) - VR(loser)
{
	...
	{
		static int done = 0;
		PRINT_IF (!done++, "vr1_diff_tab: %d,%d => %d,%d / %d,%d\n", vr1_diff_tab[i].diff,
			vr1_diff_tab[i].add_vr, vr1_diff_tab[j].diff, vr1_diff_tab[j].add_vr,
...
static enumError test_shift_jis (int argc, char **argv)
{
...

	{
		...
		while (*src0 || *src1)
		{
			...
			printf ("%4x %4x : %3zd %3zd : %4x %4x\n", code0, code1, src0 - (cucp)info,
				src1 - (cucp)info, ch0, ch1);
		}
	}
...
static enumError test_since2001 (int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		printf (format, 'H', num, t1, t2, PrintTimeByFormat ("%F %T", tim), hour2duration (num));
		...
		printf (format, 'D', num, t1, t2, PrintTimeByFormat ("%F %T", tim), day2duration (num));
		...
		printf (format, 'W', num, t1, t2, PrintTimeByFormat ("%F %T", tim), week2duration (num));
		...
		printf (format, 'M', num, t1, t2, PrintTimeByFormat ("%F %T", tim), month2duration (num));
		...
		printf (format, 'Q', num, t1, t2, PrintTimeByFormat ("%F %T", tim), quarter2duration (num));
		...
		printf (format, 'Y', num, t1, t2, PrintTimeByFormat ("%F %T", tim), year2duration (num));
	}
	return 0;
}
...
static enumError test_timeadjust (int argc, char **argv)
{
	...
	static ccp tztab[] = { "UTC", "Europe/Berlin", "America/Detroit", 0 };
		...
		for (tim = 1577664030; tim < 1609545600; tim++)
		{
			...
			if (delta != prev_delta || delta != adj)
			{
				...
				printf ("%6d | %03u %04u-%02u-%02u %2u:%02u:%02u |"
					...
					delta, gmt.tm_yday, gmt.tm_year + 1900, gmt.tm_mon + 1, gmt.tm_mday,
					gmt.tm_hour, gmt.tm_min, gmt.tm_sec, loc.tm_yday, loc.tm_year + 1900,
					loc.tm_mon + 1, loc.tm_mday, loc.tm_hour, loc.tm_min, loc.tm_sec);
				if (delta != adj)
					printf (" | %6d\n", adj);
				else
					putchar ('\n');
...
static enumError test_condition (int argc, char **argv)
{
	...
	for (argi = 1; argi < argc; argi++)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			printf ("  %04x.%02x %2d : %2d..%-2d %2d..%-2d : %s\n", c->mask, c->setting, c->bitnum,
				c->min_offline, c->max_offline, c->min_online, c->max_online, c->info);
		}
	}
	putchar ('\n');
	return 0;
...
int GetXPFBitNum (int is_online, int n_offline, int n_online)
{
	...
	{
		return n_offline < 2 ? 0 : n_offline > 4 ? 3 : n_offline - 1;
	}
}
...
static enumError test_clock (int argc, char **argv)
{
	...
	if (argc < 2)
	{
		...
		for (i = 0; i < N; i++)
			clock_gettime (CLOCK_REALTIME, &ts);
		printf (
			"%9.3f nsec / clock_gettime(CLOCK_REALTIME)\n", (double)(GetTimerUSec () - start) / N);
...
static enumError test_endian (int argc, char **argv)
{
	...
	printf ("BE_FUNC: bom=%02x,%02x, is_be=%u, is_le=%u, endian=%d\n", be_func.bom[0],
		be_func.bom[1], be_func.is_be, be_func.is_le, be_func.endian);
	if (verbose)
		HexDump16 (stdout, 0, 0, &be_func, sizeof (be_func));

	printf ("LE_FUNC: bom=%02x,%02x, is_be=%u, is_le=%u, endian=%d\n", le_func.bom[0],
		le_func.bom[1], le_func.is_be, le_func.is_le, le_func.endian);
	if (verbose)
	...
	printf ("BE: h2n(1) = %x %x %llx / n2h(1) = %x %x %llx\n", be_func.h2ns (1), be_func.h2nl (1),
		be_func.h2n64 (1), be_func.n2hs (1), be_func.n2hl (1), be_func.n2h64 (1));
	printf ("LE: h2n(1) = %x %x %llx / n2h(1) = %x %x %llx\n", le_func.h2ns (1), le_func.h2nl (1),
		le_func.h2n64 (1), le_func.n2hs (1), le_func.n2hl (1), le_func.n2h64 (1));
	putchar ('\n');
	return 0;
}
...
static enumError test_cygpath (int argc, char **argv)
{
	for (int i = 1; i < argc; i++)
	{
		...
		printf ("---\n"
				...
				">[%u,%c] %s\n"
				">%s\n",
			arg, IsWindowsDriveSpec (arg), IsCircBuf (cyg) ? 'C' : '-', cyg, PrintExMem (&mem));
		FreeString (cyg);
		FreeExMem (&mem);
	}
	putchar ('\n');
...
static enumError test_utf8 (int argc, char **argv)
{
	...
	for (ccp *ptr = text; *ptr; ptr++)
	{
		...
		printf (" | %.*s | %.*s | %.*s |  ~~  |%s|%s|%s|%s|\n", mid.len, mid.ptr, left.len,
			left.ptr, right.len, right.ptr, AlignUTF8ToCircBuf (*ptr, 10, -1),
			AlignUTF8ToCircBuf (*ptr, -10, -1), AlignUTF8ToCircBuf (*ptr, -10, 2), *ptr);
	}
	return 0;
}
...
static enumError test_nice (int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (num > 0 || *arg == '0')
			printf ("%20s => %8d : %s %s %s\n", arg, num, PrintNiceID4 (num), PrintNiceID5 (num),
				PrintNiceID6 (num));
	}
	putchar ('\n');
	return 0;
...
static enumError test_key_list (int argc, char **argv)
{
	...
	static const KeywordTab_t keytab[] = { { 1, "EINS", "1", 0 }, { 2, "ZWEI", "2", 0 },
		{ 4, "VIER", "4", 0 }, { 8, "ACHT", "8", 0 },

		{ 0x00, "NONE", "0", 0x70 }, { 0x10, "ALPHA", "A", 0x70 }, { 0x20, "BETA", "B", 0x70 },
		{ 0x30, "CHARLY", "C", 0x70 }, { 0x40, "DELTA", "D", 0x70 }, { 0x50, "ECHO", "E", 0x70 },
		{ 0x60, "FOXTROT", "F", 0x70 }, { 0x70, "GOLF", "G", 0x70 },

		{ 0, 0, 0, 0 } };

	KeyListParam_t par = { .force_case = LOUP_UPPER,
		.allow_prefix = true,
		.min_number = 50,
		.max_number = 60,
...
static inline bool isEnabledLTA ()
{
	return true;
}
	...
	for (int idx = 0; idx < LTA_MAX_LIST; idx++)
	{
		...
		if (stat == load_size)
		{
			...
			if (!memcmp (&head->magic, LTA_MAGIC, sizeof (head->magic))
				&& head->version == LTA_VERSION && head->node_size == sizeof (lta_node_t))
			{
...
char *CreateSlotInfoLTA (char *buf, unsigned bufsize, const char *prefix)
{
	...
	for (int idx = 0; idx < LTA_MAX_LIST && dest < end; idx++, li++)
	{
		if (li->first_slot < li->end_slot)
			dest += snprintf (dest, end - dest, "%s%u-%u", dest > start ? "/" : "", li->first_slot,
				li->end_slot - 1);
	}
	if (dest == start)
		*dest++ = '-';
...
static enumError develop (int argc, char **argv)
{
	...
	CloseFile (&F, 0);
	fprintf (stderr, "%s mode %d, col %d,%d,%d%s\n", colset->status, mode, colset->col_mode,
		colset->colorize, colset->n_colors, colset->reset);
	...
	for (int i = 1; i < argc; i++)
	{
		...
		else
		{
			...
			for (GetFirstDebugMode (&ldi, mode, filter_active); ldi.name; GetNextDebugMode (&ldi))
			{
	...
	for (ptr = tab; *ptr; ptr += 2)
	{
		...
		printf (" %s total, %s/1000, %u*1000 * FindConditionRef (%#x..%#x)\n",
			...
			PrintTimerUSec6 (0, 0, duration / N1000 / (ptr[1] - ptr[0] + 1), DC_SFORM_ALIGN), N1000,
			ptr[0], ptr[1]);
...
#elif 1
	static int ff_list[]
		= { FF_CHR, FF_CLR, FF_MDL, FF_PAT, FF_SCN, FF_SHP, FF_SRT, FF_TEX, FF_TEX_CT, 0 };
	int *ff;
	for (ff = ff_list; *ff; ff++)
	{
		...
		for (v = 0; v < 20; v++)
		{
			...
			if ((bi ? bi->n_sect : -1) != ns)
				printf (
					"FAIL: %s v%u: %d != %d\n", GetNameFF (0, *ff), v, bi ? bi->n_sect : -1, ns);
		}
	}
	...
	printf ("size=%zd : %zd : %d,%d: %d,%d\n", sizeof (special),
		(u8 *)(special + 1) - (u8 *)(special), special[0], special[1], ptr[0], ptr[1]);
	...
	AppendFastBuf (&fb1, "aaa", -1);
	printf ("fb1: %s: %s\n", GetFastBufStatus (&fb1), fb1.buf);
	AppendFastBuf (&fb1, "BB", -1);
	printf ("fb1: %s: %s\n", GetFastBufStatus (&fb1), fb1.buf);
	InsertFastBuf (&fb1, 1, "111", -1);
	printf ("fb1: %s: %s\n", GetFastBufStatus (&fb1), fb1.buf);
	InsertFastBuf (&fb1, -1, "222", -1);
	printf ("fb1: %s: %s\n", GetFastBufStatus (&fb1), fb1.buf);
	...
	{
		char buf[1000];
		uint len = EncodeBase64 (
			buf, sizeof (buf), argv[i], strlen (argv[i]), TableEncode64x, true, "\n\t", 10);
...
static const KeywordTab_t CommandTab[] = { { CMD_HELP, "HELP", "?", 0 },
	{ CMD_RELAX, "RELAX", 0, 0 }, { CMD_TEST, "TEST", "T", 0 }, { CMD_DEVELOP, "DEVELOP", "D", 0 },

	{ CMD_FLOAT, "FLOAT", "F", 0 }, { CMD_FLOAT_TAB, "FLOAT-TAB", "FT", 0 },
	{ CMD_MATRIX, "MATRIX", 0, 0 }, { CMD_MATCH_PATTERN, "MATCH", 0, 0 },
	{ CMD_MEDIAN_CUT, "MEDIAN-CUT", "MC", 0 }, { CMD_CMPR, "CMPR", 0, 0 },
	{ CMD_CREATE_IMAGES, "CREATE-IMAGES", "CI", 0 }, { CMD_RESIZE, "RESIZE", "RES", 0 },
	{ CMD_RESIZE_IMAGE, "RESIZE-IMAGE", "RI", 0 }, { CMD_SCAN_TEXT, "SCAN-TEXT", "ST", 0 },
	{ CMD_SCAN_STRING, "SCAN-STRING", "SS", 0 }, { CMD_SCAN_FILE, "SCAN-FILE", "SF", 0 },
	{ CMD_FILENAME, "FILENAME", "FNAME", 0 }, { CMD_DRAW_STARTPOS, "DRAW-STARTPOS", "DSP", 0 },
	{ CMD_PATH_CMP, "PATH-CMP", "PC", 0 }, { CMD_LINE2, "LINE2", "L2", 0 },
	{ CMD_LINE3, "LINE3", "L3", 0 }, { CMD_OBJFLOW, "ANA-OBJFLOW", "AOF", 0 },
	{ CMD_MEMFILE, "MEMFILE", "MF", 0 }, { CMD_FIND_BSS, "FIND-BSS", "FB", 0 },
	{ CMD_SUBFILE, "SUBFILE", 0, 0 }, { CMD_STR_LIST, "STR-LIST", "SL", 0 },
	{ CMD_CONTAINER, "CONTAINER", 0, 0 }, { CMD_UUID, "UUID", 0, 0 },
	{ CMD_ANA_MAGIC, "ANA-MAGIC", "ANAMAGIC", 0 }, { CMD_VR, "VR", 0, 0 }, { CMD_VR2, "VR2", 0, 0 },
	{ CMD_REGEX, "REGEXP", "RE", 0 }, { CMD_CT_MODE, "CT-MODE", "CTM", 0 },
	{ CMD_SCAN_HEX, "SCAN-HEX", "SH", 0 }, { CMD_SHIFT_JIS, "SHIFT-JIS", "SJIS", 0 },
	{ CMD_ITEMSLOT, "ITEM-SLOT", "IS", 0 }, { CMD_SINCE2001, "SINCE2001", "S2", 0 },
	{ CMD_TIMEZONE, "TIMEZONE", "TZ", 0 }, { CMD_TIMEADJUST, "TIMEADJUST", "TA", 0 },
	{ CMD_CONDITION, "CONDITION", "CO", 0 }, { CMD_XBIT, "XBIT", 0, 0 },
	{ CMD_IN_ARCH, "IN-ARCH", "INARCH", 0 }, { CMD_SLOT_ATTRIB, "SLOT-ATTRIB", "SA", 0 },
	{ CMD_SLOT_INFO, "SLOT-INFO", "SI", 0 }, { CMD_CLOCK, "CLOCK", 0, 0 },
	{ CMD_ENDIAN, "ENDIAN", 0, 0 }, { CMD_CONFIG, "CONFIG", 0, 0 }, { CMD_QUOTE, "QUOTE", "Q", 0 },
	{ CMD_EML, "EML", 0, 0 }, { CMD_CYGPATH, "CYGPATH", 0, 0 }, { CMD_UTF8, "UTF8", 0, 0 },
	{ CMD_NICE, "NICE", 0, 0 }, { CMD_ARG_MANAGER, "ARG-MANAGER", "AM", 0 },
	{ CMD_XCODE_HEX, "XCODE-HEX", "XH", 0 }, { CMD_IS_CHECKSUM, "IS-CHECKSUM", "ICSUM", 0 },
	{ CMD_TEST_PREFIX, "TEST-PREFIX", "TP", 0 }, { CMD_PF_EXPAND, "PF-EXPAND", "PFE", 0 },
	{ CMD_CODED_VERSION, "CVERSION", "CV", 0 }, { CMD_KEY_LIST, "KEY-LIST", "KL", 0 },
	{ CMD_RANDOM, "RANDOM", "RND", 0 }, { CMD_HMS, "HMS", 0, 0 }, { CMD_D, "_D", 0, 0 },
	{ CMD_LFL, "LFL", 0, 0 },

#ifdef HAVE_WIIMM_EXT
	{ CMD_WIIMM, "WIIMM", "W", 0 }, { CMD_SCAN_PORT, "SCAN-PORT", "SP", 1 },
	{ CMD_SCAN_PORT_V, "SCAN-PORT-V", "SPV", 1 }, { CMD_FIND_DOL, "FIND-DOL", "FD", 0 },
...
int main (int argc, char **argv)
{
	...
	PRINT0 ("ADDRT_*: mem-beg:%x, mem-end:%x, mir-beg:%x, mir-end:%x, N:%x,%x\n",
		ADDRT_MEMORY_BEGIN, ADDRT_MEMORY_END, ADDRT_MIRROR_BEGIN, ADDRT_MIRROR_END, ADDRT__N,
		ADDRT__NN);
	...
	switch (cmd_ct->id)
	{
		...
		case CMD_MATRIX:
			...
			break;
		case CMD_MATCH_PATTERN:
			test_match_pattern (argc, argv);
			break;
		case CMD_MEDIAN_CUT:
			test_median_cut (argc, argv);
			break;
		case CMD_CMPR:
			test_cmpr (argc, argv);
			break;
		case CMD_CREATE_IMAGES:
			create_images (argc, argv);
			break;
		case CMD_RESIZE:
			test_resize (argc, argv);
			break;
		case CMD_RESIZE_IMAGE:
			resize_image (argc, argv);
			break;
		case CMD_SCAN_TEXT:
			test_scan_text (argc, argv);
			break;
		case CMD_SCAN_STRING:
			test_scan_string (argc, argv);
			break;
		case CMD_SCAN_FILE:
			test_scan_file (argc, argv);
			break;
		case CMD_FILENAME:
			test_filename (argc, argv);
			break;
		case CMD_DRAW_STARTPOS:
			draw_startpos (argc, argv);
			break;
		case CMD_PATH_CMP:
			test_path_cmp (argc, argv);
			break;
		case CMD_LINE2:
			test_line (2, argc, argv);
			break;
		case CMD_LINE3:
			test_line (3, argc, argv);
			break;
		case CMD_OBJFLOW:
			ana_file_objflow (argc, argv);
			break;
		case CMD_MEMFILE:
			test_memfile (argc, argv);
			break;
		case CMD_FIND_BSS:
			test_find_bss (argc, argv);
			break;
		case CMD_SUBFILE:
			test_subfile (argc, argv);
			break;
		case CMD_STR_LIST:
			test_str_list (argc, argv);
			break;
		case CMD_CONTAINER:
		...
		case CMD_VR2:
			...
			break;
		case CMD_REGEX:
			test_regex (argc, argv);
			break;
		case CMD_CT_MODE:
			test_ct_mode (argc, argv);
			break;
		case CMD_SCAN_HEX:
			test_scan_hex (argc, argv);
			break;
		case CMD_SHIFT_JIS:
			test_shift_jis (argc, argv);
			break;
		case CMD_ITEMSLOT:
			test_item_slot (argc, argv);
			break;
		case CMD_SINCE2001:
			test_since2001 (argc, argv);
			break;
		case CMD_TIMEZONE:
			test_timezone (argc, argv);
			break;
		case CMD_TIMEADJUST:
			test_timeadjust (argc, argv);
			break;
		case CMD_CONDITION:
		...
		case CMD_XBIT:
			...
			break;
		case CMD_IN_ARCH:
			test_in_arch (argc, argv);
			break;
		case CMD_SLOT_ATTRIB:
			test_slot_attrib (argc, argv);
			break;
		case CMD_SLOT_INFO:
			test_slot_info (argc, argv);
			break;
		case CMD_CLOCK:
			test_clock (argc, argv);
			break;
		case CMD_ENDIAN:
			test_endian (argc, argv);
			break;
		case CMD_CONFIG:
			test_config (argc, argv);
			break;
		case CMD_QUOTE:
			test_quote (argc, argv);
			break;
		case CMD_EML:
			test_eml (argc, argv);
			break;
		case CMD_CYGPATH:
			test_cygpath (argc, argv);
			break;
		case CMD_UTF8:
			test_utf8 (argc, argv);
			break;
		case CMD_NICE:
			test_nice (argc, argv);
			break;
		case CMD_ARG_MANAGER:
			test_arg_manager (argc, argv);
			break;
		case CMD_XCODE_HEX:
			test_xcode_hex (argc, argv);
			break;
		case CMD_IS_CHECKSUM:
			test_is_checksum (argc, argv);
			break;
		case CMD_TEST_PREFIX:
			test_test_prefix (argc, argv);
			break;
		case CMD_PF_EXPAND:
			test_pf_expand (argc, argv);
			break;
		case CMD_CODED_VERSION:
			test_coded_version (argc, argv);
			break;
		case CMD_KEY_LIST:
			test_key_list (argc, argv);
			break;
		case CMD_RANDOM:
...
#ifdef HAVE_WIIMM_EXT
		case CMD_WIIMM:
			test_wiimm (argc, argv);
			break;
		case CMD_SCAN_PORT:
			test_scan_port (argc, argv, 0);
			break;
		case CMD_SCAN_PORT_V:
			test_scan_port (argc, argv, 1);
			break;
		case CMD_FIND_DOL:
			test_find_dol (argc, argv, 0);
			break;
		case CMD_FIND_REL:
			test_find_dol (argc, argv, 1);
			break;
#endif
