...
static const KeywordTab_t itemslot_table_list[]
	= { { 0x001, "GRAND-PRIX-PLAYER", 0, RACE_PARAM }, { 0x0e7, "GRAND-PRIX-ENEMY", 0, RACE_PARAM },
		  { 0x1cd, "VERSUS-PLAYER", 0, RACE_PARAM }, { 0x2b3, "VERSUS-ENEMY", 0, RACE_PARAM },
		  { 0x399, "VERSUS-ONLINE", 0, RACE_PARAM }, { 0x47f, "SPECIAL", 0, RACE_PARAM },
		  ...
		  { 0x627, "COIN-PLAYER", 0, BATTLE_PARAM }, { 0x662, "COIN-ENEMY", 0, BATTLE_PARAM },
...
static const KeywordTab_t itemslot_section_name[]
	= { { ITEMSLOT_IGNORE, "END", 0, 0 }, { ITEMSLOT_SETUP, "SETUP", 0, 0 },
		  { ITEMSLOT_PARAMETERS, "PARAMETERS", 0, 0 }, { 0, 0, 0, 0 } };

///////////////////////////////////////////////////////////////////////////////

static enumError ScanTextSETUP (itemslot_t *itemslot, // ITEMSLOT data structure
...
static enumError ScanTextPARAMETERS (itemslot_t *itemslot, // ITEMSLOT data structure
	ScanInfo_t *si // valid data
)
{
	DASSERT (itemslot);
...
static enumError ScanTextTABLE (itemslot_t *itemslot, // ITEMSLOT data structure
	ScanInfo_t *si, // valid data
		...
		if (item >= n_row)
		{
			...
			if (si->no_warn <= 0)
				ERROR0 (ERR_WARNING,
					...
					n_row - 1, sf->name, sf->line, (int)(eol - sf->ptr), sf->ptr);
			sf->ptr = eol;
			continue;
		}
...
)
{
	...
	switch (fform)
	{
		...
		default:
			...
			return ERROR0 (ERR_INVALID_DATA, "No ITEMSLOT file: %s\n",
...
)
{
	...
	if (!tiny)
		fprintf (F.f, text_itemslot_setup_cr, tool_name, SYSTEM2, VERSION, REVISION_NUM, DATE);
	...
	if (minimize < 10)
	{
		...
		fprintf (
			F.f, text_itemslot_param_cr, REVISION_NUM, itemslot->add_battle, itemslot->use_slt);
	}
	...
	for (par = itemslot_table_list; par->name1; par++)
	{
		...
		if (minimize < 1 || memcmp (val, orig, table_size))
		{
			...
			fputs (brief_count < 2 ? section_sep : "\r\n", F.f);
			fprintf (F.f, text_itemslot_table_cr, par->name2 ? par->name2 : par->name1,
				is_battle ? "battle" : "racing", offset, n_row, n_col, REVISION_NUM);
