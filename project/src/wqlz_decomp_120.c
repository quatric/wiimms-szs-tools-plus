    ...
    *(destination_c + 4) = *source_c;
    ...
    while (src < guarantee_uncompressed)
    {
...
#endif
        {
            ...
            hashtable[hash] = src;
#ifndef x86x64_only
            if (VALIDATE_OFFSET src - o <= 131071 && o < src - 3 && *src == *o && *(src + 1) == *(o + 1) && *(src + 2) == *(o + 2))
#else
            if (VALIDATE_OFFSET src - o <= 131071 && o < src - 3 && (((*(unsigned int *)o) ^ (*(const unsigned int *)src)) & 0xffffff) == 0)
#endif
            {
                unsigned int offset;
