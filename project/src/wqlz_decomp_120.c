...
unsigned int qlz_compress_packet (
	const void *source, char *destination, unsigned int size, char *buffer);
...
void fast_write (unsigned int f, void *dst, unsigned int bytes)
{
	switch (bytes)
	{
		case 4:
...
void memcpy_up (char *dst, const char *src, unsigned int n)
{
	if (n < 5)
...
unsigned int fast_read_safe (void const *src, unsigned int bytes, const char *invalid)
{
	invalid = invalid;
...
unsigned int qlz_compress_core (const void *source, char *destination, unsigned int size,
	const char **hashtable, const char *first_valid)
{
	...
	while (src < guarantee_uncompressed)
	{
		...
		if ((cword_val & 1) == 1)
		{
			...
			if (dst - prev_dst > src - prev_src && src > source_c + 1000)
			{
				int q;
				for (q = 0;
					q < 20 && src + 31 < guarantee_uncompressed && dst + 35 < destination_c + size;
					q++)
				{
		...
		if (fast_read (src, 4) == fast_read (src + 1, 4))
		{
			...
			do
				src = src + 4;
			while (src <= guarantee_uncompressed - 4 && fetch == fast_read (src, 4));
			if ((src - orig_src) <= 2047)
			{
				fast_write (
					((fetch & 0xff) << 16) | (unsigned int)((src - orig_src) << 4) | 15, dst, 4);
				dst = dst + 3;
			}
			else
			{
...
#endif
		{
			...
			hashtable[hash] = src;
#ifndef x86x64_only
			if (VALIDATE_OFFSET src - o <= 131071 && o < src - 3 && *src == *o
				&& *(src + 1) == *(o + 1) && *(src + 2) == *(o + 2))
#else
			if (VALIDATE_OFFSET src - o <= 131071 && o < src - 3
				&& (((*(unsigned int *)o) ^ (*(const unsigned int *)src)) & 0xffffff) == 0)
#endif
			{
				unsigned int offset;
...
void qlz_decompress_core (const char *source, void *destination, unsigned int size,
	unsigned int source_size, char *first_valid)
{
	const char *source_c = (const char *)source;
	char *destination_c = (char *)destination;
	...
	for (;;)
	{
		...
		if ((cword_val & 1) == 1)
		{
			...
			if ((fetch & 3) == 0)
			{
				offset = (fetch & 0xff) >> 2;
#ifdef memory_safe
				if (3 > (unsigned int)(guaranteed_uncompressed - dst)
					|| offset > (unsigned int)(dst - first_valid))
					return;
#endif
...
#endif
	r = base
		+ qlz_compress_core (source, destination + base, size, hashtable, (const char *)source);
...
unsigned int qlz_decompress (const char *source, void *destination)
{
	...
	if (memcmp (&header[QCLZ], "QCLZ", 4) == 0)
	{
		if (fast_read (&header[COMPRESSIBLE], 4) != 1)
			...
			qlz_decompress_core (
				source + 32, destination, size_decompressed, size_compressed, (char *)destination);
		return size_decompressed;
	}
	else if (((*source) & 0xfc) == 0)
	{
		if ((*source & 1) == 1)
			qlz_decompress_core (source + base, destination, size_decompressed, size_compressed,
				(char *)destination);
		else
...
unsigned int qlz_compress_packet (
	const void *source, char *destination, unsigned int size, char *buffer)
{
	// 0-7 bytes for aligning; 4 bytes for buffersize; 32768 byte hashtable; STREAM_BUFFER_SIZE -
	// 40000 bytes streambuffer
	...
	else
	{
		...
		r = base
			+ qlz_compress_core (
				streambuffer + *buffersize, destination + base, size, hashtable, streambuffer);
...
unsigned int qlz_decompress_packet (const char *source, void *destination, char *buffer)
{
	...
	{
		if ((*source & 1) == 1)
			qlz_decompress_core (source + base, streambuffer + *buffersize, size_decompressed,
				size_compressed, streambuffer);
		else
...
int main (int argc, char *argv[])
{
	...
	len = qlz_decompress (src + offset, dst);
