...
typedef enum kcl_mode_t
{
	...
	KCLMD_M_HEX = KCLMD_HEX4 | KCLMD_HEX23,
	...
	KCLMD_M_PRESET = KCLMD_SMALL | KCLMD_CHARY | KCLMD_DRAW,

	KCLMD_M_GENERAL = KCLMD_FAST | KCLMD_ROUND | KCLMD_NORMALS | KCLMD_MTL | KCLMD_WIIMM
		| KCLMD_TRIANGLES | KCLMD_OUT_SWAP | KCLMD_G | KCLMD_USEMTL | KCLMD_CLIP | KCLMD_IN_SWAP
		| KCLMD_AUTO | KCLMD_CENTER | KCLMD_M_HEX | KCLMD_CUBE | KCLMD_M_PRESET,
	...
	KCLMD_M_DROP = KCLMD_DROP_AUTO | KCLMD_DROP_UNUSED | KCLMD_DROP_FIXED | KCLMD_DROP_INVALID,
	...
	KCLMD_M_PATCH = KCLMD_M_DROP | KCLMD_M_RM | KCLMD_CONV_FACEUP | KCLMD_WEAK_WALLS
		| KCLMD_CLR_VISUAL | KCLMD_NEW | KCLMD_SORT,
	...
	KCLMD_M_HIDDEN = KCLMD_LOG | KCLMD_SILENT | KCLMD_POSLEN | KCLMD_INPLACE,
...
kcl_tridata_t *GetTridataKCL (kcl_t *kcl, // valid KCL
...
static inline void InitializeTriParam (kcl_tri_param_t *tp, kcl_t *kcl, u32 kcl_flag)
{
	DASSERT (tp);
	DASSERT (kcl);
...
typedef int (*oct_func) (kcl_t *kcl, // pointer to valid KCL
...
struct kmp_t;
enumError SaveImageKCL (struct kmp_t *kmp, // pointer to valid KMP
	kcl_t *kcl, // pointer to valid KCL
	ccp fname, // filename of destination
	bool set_time // true: set time stamps
);
